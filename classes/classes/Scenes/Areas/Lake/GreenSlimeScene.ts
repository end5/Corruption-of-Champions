...
private  serviceLowCorruptionHighLust():void
{
	...
	if(player.cockTotal() > 0) {
		...
		if (player.cockTotal() > 1) outputText("Your own erect " + multiCockDescriptLight() + " rub against the creature's as you work, getting in the way somewhat.  ", false);
		//Big single cocked folk get in their own way too!
		else if(player.cocks[0].cockThickness >= 36) outputText("Your own erect " + cockDescript(0) + " rubs against the creature's as you work, getting in the way somewhat.  ", false);
		//Single dick contemplation
		if(player.cockTotal() == 1) outputText("You feel your " + cockDescript(0) + " begin to throb and entertain the idea of trying to penetrate it somehow.  You turn a bit and look around the creature for an orifice, but find nothing.  After a few frustrated moments of searching you give up and decide to handle things yourself.", false);
		//Multidick contemplation
		else outputText("You feel your " + multiCockDescriptLight() + " begin to throb and entertain the idea of trying to penetrate it somehow.  You turn a bit and look around the creature for an orifice, but find nothing.  After a few frustrated moments of searching you give up and decide to handle things yourself.", false);
		outputText("\n\n", false);
	}
	//Next PG
	//Get it hard
	...
	if(player.tallness < 82)
	{
		...
		if (player.gender == 1 || (player.gender == 3 && rand(2) == 0))
		{
			...
			{
				if(player.cocks.length > 0) outputText("Eventually your own arousal becomes unbearable and you reach down to work your one of your " + multiCockDescriptLight() + ", massaging the monster's swollen, throbbing head with your other hand.  You gasp slightly as soon as you wrap your hand around your " + cockDescript(1) + ", the creature's fluid making you exceptionally sensitive.  You moan in spite of yourself and start to thrust your hips a bit against your fist, feeling both yourself and the creature begin to throb harder and harder.  Before long you can hold out no longer, and feel your body racked by an orgasmic spasm, squeezing tightly with both hands as you blow your " + multiCockDescriptLight() + " cover the creature with cum.  ", false);
		...
		else if (player.gender == 2 || player.gender == 3)
		{
			...
			if (player.cockTotal() > 0)
			{
				...
				{
					outputText("You feel your " + multiCockDescriptLight() + " surge to their full size as the slime works you over.  They quickly begin to throb and ache for release, a need made only more pressing by your desire to have it fondled.  ",false);
				...
				{
					outputText("You feel your " + cockDescript(0) + " surge to full size as the slime works you over.  It begins to throb and ache for release, a need made only more pressing by your desire to have it fondled.  ",false);
					//Play with it if your boobs aren't being manhandled
					if(player.biggestTitSize() == 0) outputText("You delicately grasp the head of your " + cockDescript(0) + ", your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.", false);
				}
			}
			...
			{
				//Multidicked herms
				if (player.cockTotal() > 1)	outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + multiCockDescript() + " explodes into orgasm, showering both you and the creature in jizz.  ", false);
				//Single cocked herms
				else outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + cockDescript(0) + " explodes into orgasm, sending a jet of your warm jizz into the air.  ",false);
			}
	...
	else if(player.tallness >= 82)
	{
		...
		else if(player.gender == 2 || player.gender == 3)
		{
			...
			if(player.cockTotal() > 0)
			{
				...
				{
					outputText("You begin to moan as the creature rubs the " + vaginaDescript(0) + " beneath your " + multiCockDescript() + ", working you with incredible skill, but sadly ignoring your needy ", false);
					//multi!
...
private  maleRapesOoze():void {
	...
	if(rand(2) == 0)
	{
		...
		else outputText("Shrugging, you decide to go with the closest thing available and walk towards its head, pulling out your " + cockDescript(0) + " as you do so.  You kneel over the thing's head and stroke your penis lightly to ensure it is fully ready,  ", false);
		//Balls ftw
		if(player.balls > 1) outputText("anointing the slime's face with your " + sackDescript() + " as you do.  ", false);
		//Ball-less bitches
	...
	else
	{
		...
		if(player.cockTotal() > 1)
		{
			...
			outputText(" of your" + multiCockDescriptLight() + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  ", false);
		}
		//Single dicks
		else outputText("You shrug at the lack of obvious orifices and decide to go for the most common one, pulling out your " + cockDescript(0) + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  ", false);
		...
		if(player.cocks[0].cockLength >= 24) outputText("By the time you finally bottom out, your " + cockDescript(0) + " is nearly halfway through its torso.", false);
		else if(player.cocks[0].cockLength >= 48) outputText("By the time you finally bottom out, your " + cockDescript(0) + " reaches nearly into the slime's neck.", false);
		else if(player.cocks[0].cockLength >= 60) outputText("Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + cockDescript(0) + ".", false);
		...
		if(player.cockTotal() > 1) outputText("your " + multiCockDescriptLight() + " explodes into orgasm, your load filling the space inside the creature and spraying in thick streams across its back.", false);
		...
		outputText("  The thing seems to clamp down on you even tighter as you pull out, and you relish the sensation.  Its tight ass leaves your " + cockDescript(0) + " almost perfectly clean afterward, a glob of white still stuck inside the creature as it changes back into its original, amorphous state.  The cloud disperses as the slime slinks off.", false);
	}
	player.orgasm();
	dynStats("sen", 3);
}
...
private  femaleRapesOoze():void {
	...
	{
		outputText(" your " + vaginaDescript(0) + " growing wet and your " + cockDescript(0) + " growing firm as you imagine it inside you.  ", false);
	}
	else
	{
	...
	if(player.cockTotal() > 0)
	{
		if(player.cockTotal() == 1)
		{
			outputText("  Your " + cockDescript(0) + " grows hard as you are penetrated, and with the creature handling the movement you reach down to jack yourself off.", false);
		}
		else
		{
	...
	if(player.cockTotal() > 0)
	{
		if(player.cockTotal() > 1)
		{
			outputText("Your " + multiCockDescriptLight() + " explode into simultaneous orgasm, releasing ", false);
		...
		{
			outputText("Your " + cockDescript(0) + " explodes into orgasm and releases thick streams of semen onto both you and the creature, your mind blanking at the release.  You vaguely notice your ejaculate sinking into the creature's body and getting absorbed as you give yourself to pleasure.  ", false);
		}
	}
...
private  oozeButtRapesYou():void 
{
	...
	if(player.cockTotal() > 0)
	{
		...
		//Multi!
		else outputText("Your " + multiCockDescript() + " quickly grows erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub at a different cock with each hand as it pumps you, and quickly approach an orgasm.  ", false);
	}
	//Vaginas!
...
private  oozeRapesYouVaginally():void
{
	...
	if(player.cor <= 20)
	{
		...
		if(player.cockTotal() > 0)
		{
			...
			if(player.cockTotal() > 1)
			{
				outputText("Your " + multiCockDescript() + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, ", false);
			...
			//Single dick
			else outputText("Your " + cockDescript(0) + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, rapidly stroking your " + cockDescript(0) + " in time with the creature's movements.  ", false);
		}
		...
		{
			//Multi
			if(player.cockTotal() > 1) outputText("Your " + multiCockDescript() + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.", false);
			//Single
			else outputText("Your member climaxes as well, and you pull a hand to cover your eyes and face as your jism explodes onto yourself.  Your other hand slides to the base of your " + cockDescript(0) + " and squeezes as you coat your chest in semen.", false);
		}
		player.orgasm();
		dynStats("sen", 4);
	}
	...
	else
	{
		...
		{
			//multi
			if(player.cockTotal() > 1) outputText("Your " + multiCockDescript() + " climaxes as well, and you open your mouth as you spray rope after rope of hot, thick jism onto your face and chest.  You relish its salty taste, continuing to stroke yourself even as you come.  ", false);
			//single cock
			else outputText("Your own member finally reaches its climax as well, and you open your mouth as jism explodes onto your face and chest.  Your hands slide to the base of your " + cockDescript(0) + " and squeeze as you release thick ropes of ejaculate onto yourself, coating your face and chest as you relish its salty taste.  ", false);
		}
...
public  rapeOozeWithMilk():void {
	...
	if(player.gender == 1) 
	{ 
		outputText("caressing your " + multiCockDescriptLight(), false);
		if(player.balls > 0) 
		{
