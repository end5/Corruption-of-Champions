...
//*Victory Rape: PC Penis goes in Vagina (done)
		private  winDriderPCDickInSpiderCunt():void
		{
			...
			else outputText("Your " + cockDescript(x) + " seems to bulge inside her, giving itself over to the silken caresses of her glorious vagina.", false);
			...
			outputText("While you're distracted by the repeated contrast of tension and relief emanating from " + sMultiCockDesc() + ", the drider leans up and kisses your neck, humming against your throat as you squirt the last of your orgasmic goop into her vagina.  You slump into her arms, spent by the effort of fucking her so fast and cumming so hard.  As you lie there, her lips sucking hard enough to leave hickeys on your neck with every crushing, passionate kiss, you feel the patterns of glossy fluid tingling on your " + player.skin() + ".  A jolt of fear runs up your spine, and you try to push away from her envenomed affections.  It doesn't work!  You can barely lift your arms, let alone pull out of the amorous arachnid's embrace!\n\n", false);
			...
			else outputText("No, that's not right.  You're not some rutting beast, you're the champion.  Sure, you've got your " + cockDescript(x) + " locked hilt-deep inside her squirming slit, but that doesn't mean you're some kind of " + player.mf("stud", "slut") + " who jumps on a cunt without a second thought, does it?  You blink your eyes while you try to think.  She does have a point though, if you hadn't been in such a hurry for some sex, you wouldn't be sagging into her tits and feeling so awfully relaxed.\n\n", false);
			...
			outputText("A plaintive whining, almost exactly like that of an animal, vocalizes subconsciously.  Your " + cockDescript(x) + " feels afire, constantly leaking pre-cum while you're held immobile in your spidery mistress' arms; you lick your lips and start to beg, mumbling out your desire to cum for her, anything for her, so long as she lets your dick orgasm again and again.  You want to climax so bad, but you want her to control your cock so much more.  It's always getting you in trouble, and it feels so good to let her use you as her cum-drooling mount.\n\n", false);
			...
			outputText("You mewl out an affirmation, forgetting how to speak with all the venom pumping through you and the mind-crushing levels of arousal pouring out of your " + cockDescript(x) + ".\n\n", false);
...
//Part 2 of the above
		private  winDriderPCDickInSpiderCunt2():void
		{
			...
			outputText("A low growl rips through your throat as you pull back and plaster her face white, coating the caked-on residue of a few previous loads with fresh moisture.  You shift position and force your " + cockDescript(x) + " between her two pairs of breasts, sandwiching it in there for the titty-fuck of a lifetime.  Pistoning back and forth, humping the glorious orbs relentlessly, you give yourself over completely to your sore dick, letting your world dissolve into the feelings of each of the spider-woman's orifices around your member.\n\n", false);

			outputText("<b>Even later...</b>\n", false);

			outputText("You collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz.  You've completely emptied out every speck of sperm from your " + ballsDescriptLight() + ", and you even kept going for a while after that.  It wasn't until " + sMultiCockDesc() + " went soft at last that you found enough peace to lie down.  ", false);
...
//*Victory Rape: Ride Ovi Vaginal (done)
		private  victoryVSDriderRideOviVaginal():void
		{
			...
			if (monster.HP < 1) {
				...
				if (player.hasCock()) outputText(" and " + sMultiCockDesc() + " twitch", false);
			...
			if (player.hasCock()) {
				outputText("  Your own " + multiCockDescriptLight() + " retaliate", false);
...
//*Victory Rape: Ride Ovi Anal sex-insenitive (done)
		private  victoryVSDriderRideOviAnal():void
		{
			...
			if (player.hasCock()) outputText("while your " + multiCockDescriptLight() + " dribbles white, wasted goo onto the black and " + monster.hairColor + " striped carapace below", false);
...
//*Loss Vaginal Impregnation and forced BJ - Symphonie (done)
		private  loseVSDriderAndVaginallyImpreggleOvi():void
		{
			...
			if (player.hasCock() && monster.nipplesPierced > 0) outputText("  Chains brush over " + sMultiCockDesc() + " as her chest passes your crotch.", false);
			outputText("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.", false);
			if (player.biggestTitSize() > 3) outputText("  Venom dribbles down the cleavage of your " + biggestBreastSizeDescript() + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.", false);
			outputText("\n\n", false);
			...
			if (player.hasCock()) outputText("  Even your " + cockDescript(0) + " spurts some pre-cum of its own.", false);
...
//*Loss Ride & Impregnate Butt (done)
		private  loseVSDriderAndGetFuckedANDAnalImpreggegity():void
		{
			...
			if (player.lust > 99) outputText("Unable to resist the desire coursing through your body, you strip off your " + player.armorName + " and start stroking " + sMultiCockDesc() + ", displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.", false);
				...
				if (player.cockTotal() > 1) outputText("s", false);
				outputText(" to the air.  She squeezes " + oMultiCockDesc() + " in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.", false);
			}
			...
			if (monster.nipplesPierced > 0) outputText(", the crossed chains digging into your " + chestDesc(), false);
			...
			outputText("Lines of webbing burst from the arachnid abdomen, snapping into tree limbs before pulling taut.  The drider's eight legs clitter and clack, four of them folding around your back while your world rotates 90 degrees.  The two of you are pulled into the trees by the sticky strands, hanging sideways, almost upside-down, in the leaves.  The four-eyed monster-girl blinks asynchronously and smiles, shifting you slightly to bring " + oMultiCockDesc() + " against her moist lips.  You exhale, an intense, animalistic thrill shivering through you from the feeling of having your " + cockDescript(x) + " lodged in a woman's entrance.  She leans down and bites into your neck, releasing a surge of toxin into your bloodstream.  It boils like fire, making you twitch and squirm in her many-limbed clutches.  The pain fades quickly, but in its place it leaves behind a molten, liquid lust, filling " + sMultiCockDesc() + " to its absolute maximum size, as hard as it can possibly be.\n\n", false);
			...
			outputText("You look at her with fear, but your " + hipDescript() + " start to twitch against her, rubbing your " + player.cockHead(x) + " up and down the slippery slit.  A slow exhale of pleasure escapes the spider's purple lips while her legs squeeze tighter against you, trying to push your " + cockDescript(x) + " home.  The drider gasps when at last, you slip between her juicy outer labia, slurping deep into her cunny with a lurid squelch.  Finally held within her depths, your muscles all slacken, save for those you need to pump your hips against her, futilely trying to fuck her hot quim even though you have zero leverage.\n\n", false);
			...
			outputText("  Once it gets deep enough to push on your prostate, an upsurge of pleasure hits you hard enough to make " + sMultiCockDesc() + " dribble.  You moan against the drider's neck, pulsing pathetically inside the spider-woman's slit.  You know you'll soon be giving her exactly what she wants - all of it - and there's nothing you can do about it.\n\n", false);
...
//Summary: what goes up must come down
		public  birthSpiderEggsFromAnusITSBLEEDINGYAYYYYY():void
		{
			...
			if (player.hasCock()) outputText(SMultiCockDesc() + " stays limp, but drools a heavy flow of semen, puddling up a nice thick batch of your pleasure alongside your goo-pile.  ", false);
			if (player.hasVagina()) {
		...
		public  driderPregVagBirth():void
		{
			...
			if (player.biggestTitSize() >= 2) {
				outputText("One drider girl cautiously makes her way over to you.  Her legs tickle your stomach as she climbs up to one of your " + breastDescript(0) + ".  Grasping the flesh tightly in her tiny hands she begins to suckle at your " + nippleDescript(0) + ".", false);
...
//Win Drider Male Anal (OH BABY)
		private  buttFuckADriderOhBaby():void
		{
			...
			outputText("\"<i>Oh, you want that hole, do you? How very kinky,</i>\" the drider chides, but you aren't fooled by her tone – you can see her anus relaxing, preparing itself for your arrival.  With a grin, you grab your " + cockDescript(x) + " and push it against her tight hole, leaning into the penetration until you feel it slip past her sphincter and into her hot, snug passage.  As you slide more and more of your cockmeat into her, you reach up from her cheeks and grope her breasts, squeezing her pert nipples between your fingers as you finally bottom out.  Since you can go no further up her butt, you start to piston your hips forward, slapping your hips to her ass with short, forceful strokes.\n\n", false);
