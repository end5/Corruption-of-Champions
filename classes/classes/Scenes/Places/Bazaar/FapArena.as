...
private  fapArenaPageII():void {
	...
	else if(player.cor > 33) outputText("You nervously grab hold of the rod on your right side.  In your stress, you can't help but give it a little squeeze.  You tremble as your left neighbor grabs " + oMultiCockDesc() + " with an expert hand.  Most people are as nervous as you, their whole naked body trembling in stress.\n\n", false);
	else {
...
private  fapResults(place:number = 3):void {
	...
	if(place == 3) {
		...
		outputText("Your left partner is very professional.  At the very moment your " + cockDescript(x) + " starts shooting its goo, your cock-milker yells in a thundering voice: \"<i>NUMBER " + (num-1) + " SCORING!</i>\"  For a moment, lost in your pleasure, you pay little attention to the world outside of your " + cockDescript(x) + ".  Then you remember the game and shout in a whorish, pleasure-filled voice: \"<i>NUMBER " + num + " CUMMING!</i>\"", false);
		if(player.hasVagina()) {
	...
	//[else if the player doesn't lose, but doesn't win either - he cums neither first nor last]
		...
		outputText(" the floor with your sexual juices and yell in a whorish voice, \"<i>NUMBER " + (num+4) + " CUMMING!</i>\"; you're almost immediately followed by the scoring announcement from your left neighbor.  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + cockDescript(x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.", false);
		if(player.hasVagina()) {
			outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ", false);
			if(player.wetness() >= 4) outputText("large puddle", false);
			else if(player.wetness() >= 2) outputText("puddle", false);
	...
	//[else, the player wins]
		...
		outputText("You mentally keep track of all the people still in play.  They are fewer and fewer.  You just have to hold on a little longer.  Four... three... two... that's it.  You made it.  Phew, you have won just in time, your " + cockDescript(x) + " was starting to twitch on its own.  Unhindered by the fear of losing, you completely give up to your orgasm and sigh in utter satisfaction and relief as you release one of the most abundant cumshots you've ever had.  Lost in climax, you carelessly ", false);
		if(player.cumQ() >= 1000) outputText("soak", false);
		else if(player.cumQ() >= 500) outputText("pollute", false);
		else outputText("wet", false);
		outputText(" the floor with your sexual juices and yell in a whorish voice: \"<i>NUMBER " + num+ " CUMMING!</i>\"  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + cockDescript(x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.", false);
				if(player.hasVagina()) {
			outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ", false);
			if(player.wetness() >= 4) outputText("large puddle", false);
			else if(player.wetness() >= 2) outputText("puddle", false);
	...
	//[else, corr >50]
	else outputText("Your body tenses and " + sMultiCockDesc() + " hardens at the perspective of a collective anal orgy.  You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\" along with the others.  ", false);
	//end of global condition about PC's corr, display the following
	...
	if(place <= 1) {
		...
		if((player.cockTotal() > 2) || (!tent && player.cockTotal() > 1)) {
			...
			outputText("own, unhindered by the tight colon holding your first " + cockDescript(x) + ".", false);
		}
	...
	//[else if you didn't win]
		...
		//[1/2 chance of this]
		if(rand(2) == 0) outputText("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order weren't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths.  Nevertheless, your " + cockDescript(x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n", false);
		else outputText("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + cockDescript(x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + cockDescript(x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n", false);
...
private  tentacleFapSpecial(place:number):void {
	...
	if(player.cocks[x].cockLength >= 40 && player.cocks[x].cockType == CockTypesEnum.TENTACLE) {
		...
		if(place <= 2) {
			outputText("Needless to say, this interference creates a general disruption in the whole butt-fuck wave.  The whole train of peckers and puckers starts moving irregularly, grinding against your " + cockDescript(x) + " at the most unexpected moments, and the pleasure you're getting from your extra tentacle cock doesn't help.  Everyone's butt and dick is affected to some extent; everyone's pleasure is increased because of your audacious initiative, and loud, reluctant moans are heard.", false);
