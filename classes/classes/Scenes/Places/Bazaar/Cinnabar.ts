...
private  cinnabarNonHugeDickings():void {
	...
	if(player.hasCock() && !player.hasVagina()) outputText("You gasp and moan when you feel her other hand wrap around your " + cockDescript(0) + ", pulling and tugging on it until it starts to leak a trail of squeezed-out man-slime on the mattress.  ", false);
	...
	if(player.hasCock() && !player.hasVagina()) {
		outputText("  Her soft pads grip your " + cockDescript(0) + " tightly while she  presses on your prostate, milking your cum from you with professional skill.  Rather than coming in its usual spurts, it drips in a long, liquid chain of white-goo that drops into a submissive puddle on the bed.", false);
		if(player.cumQ() >= 250) {
...
private  fuckCinnabarHugerAndCorruptedEr():void {
	...
	outputText(", squirting juices from the callous, cunt-piercing thrust.  The " + player.cockHead(x) + " of your " + cockDescript(x) + " can be seen just below her neck", false);
	...
	else outputText(", and only now do you realize you blew past her cervix long ago, the tight ring hugging your " + cockDescript(x) + " in a clutching embrace.", false);
	outputText("\n\n", false);
	...
	outputText("  A half-second later your " + cockDescript(x) + " joins it, your internal muscles clenching and convulsing, heaving the jizz from your body in one huge, gooey blast.\n\n", false);
...
private  cinnabarHuger():void {
	...
	outputText(" to prod at your " + assholeDescript() + ", sliding in to push your prostate.  She hits it hard enough to make you pump cum with enough force to distort her already-ballooned body, and you go into an orgasmic frenzy, dragging your " + cockDescript(x) + " back to her entrance before jamming in all ", false);
...
private  cinnabarMultiCockPortalFuckFest():void {
	...
	outputText("  Your " + player.legs() + " go wobbly from the delicate touches dancing through your " + multiCockDescriptLight() + ", making it hard to stand.  Cinnabar slides ", false);
	...
	outputText(".  Slippery-soft warmth wreathes your " + cockDescript(x) + ", the plush black love-tunnel contracting slightly to happily squeeze you every few seconds.  The darker, sinfully tight star of Cinnabar's anus glazes with your pre-cum, her ass-cheeks wobbling perfectly in your hands with each rhythmic penetration.", false);
	...
	outputText("  The " + cockDescript(z) + " in her mouth feels so good it nearly makes you jump, the long pink tongue licking around it while a wet muzzle buries itself in your crotch.  Gods, it's too much!\n\n", false);
	
	outputText("You feel yourself cumming, that tell-tale warmth in your gut mixing with a few involuntary muscle-contractions that bounce your " + multiCockDescriptLight() + " inside Cinnabar.  You turn your head to watch her, " + num2Text(player.cockTotal()) + " penises pumping from holes in the wall to ravage her body completely and totally, her eyes rolled back in complete bliss.   Heat blooms in your " + multiCockDescriptLight() + ", the pumping gouts of seed deep into all of the slut's holes simultaneously.  She screams into your " + cockDescript(z) + ", vibrations making your orgasm that much stronger.  It drags on and on, the spit-roasted rat shaking while she's completely taken in every way", false);
	...
	outputText("An incessant licking reaches your ears, getting your attention.  You stand up, careful about your " + multiCockDescriptLight() + " while you look towards the bed.  The portals are closed down, and Cinnabar is bent double, folded in half like a contortionist", false);
