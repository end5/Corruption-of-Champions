...
package classes.Items
{
	...
	public final class ConsumableLib extends BaseContent
	{
		...
		public const EXTSERM:Consumable			= new HairExtensionSerum();
...
//		public const KITGIFT:SimpleConsumable = mk("KitGift","KitGift", "a kitsune's gift", m.kitsunesGift, "A small square package given to you by a forest kitsune.  It is wrapped up in plain white paper and tied with a string.  Who knows what's inside?", 0);
		...
		public const OVIELIX:Consumable = new OvipositionElixir();
		...
		public const PEPPWHT:SimpleConsumable = mk("PeppWht","PeppWht", "a vial of peppermint white", function(player:Player):void{getGame().peppermintWhite(player)}, "This tightly corked glass bottle gives off a pepperminty smell and reminds you of the winter holidays.  How odd.", 120);
		...
		public const SDELITE:SimpleConsumable = mk("SDelite","Sucb.Delite", "a bottle of 'Succubi's Delight'", curry(m.succubisDelight, true),"This precious fluid is often given to men a succubus intends to play with for a long time.");
		public const SENSDRF:SimpleConsumable = mk("SensDrf","Sens. Draft", "a bottle of sensitivity draft", m.sensitivityDraft, "This carefully labelled potion is a 'Sensitivity Draft', and if the diagrams are any indication, it will make your body more sensitive.", 15);
		...
		public const VITAL_T:SimpleConsumable = mk("Vital T","Vitality T.", "a vitality tincture", m.vitalityTincture, "This potent tea is supposedly good for strengthening the body.");
