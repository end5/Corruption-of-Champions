	...
	{	
		...
		*/
		 ...
		 */
		public  constructor(mainClass:any, debug:boolean = false)
		{
			this.debug = debug;
		...
		private  initAvailableKeysList():void
		{
			...
			*/
			...
			for (const button in controlMethods)
			{
		...
		CONFIG::debug 
		{
			public  catchGlobalError(event:UncaughtErrorEvent):void
			{
				...
				else if (event.error instanceof ErrorEvent)
				{
		...
		private  setupExitKey():void
		{
			...
			var stopMonkahTempFunc:() => void = function(callFunc:() => void):() => void
			{ 
