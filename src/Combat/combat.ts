...
function wait(): void {
    ...
    if (monster.effects.findByType(StatusAffects.PCTailTangle) >= 0) {
        ...
        else outputText("The creature continues probing at your asshole and has now latched " + num2Text(player.breastRows.totalNipples()) + " more suckers onto your nipples, amplifying your growing lust.  You must escape or you will become a mere toy to this thing!");
...
export function fantasize(): void {
    ...
    else if (player.breastRows.biggestTitSize() >= 6 && rand(2) == 0) {
    ...
    else if (player.breastRows.biggestLactation() >= 6 && rand(2) == 0) {
    ...
    if (player.effects.findByType(StatusAffects.LethicesRapeTentacles) >= 0) {
        ...
        if (player.effects.getValue3Of(StatusAffects.LethicesRapeTentacles) != 0) {
            ...
            if (tentaround == 1) {
                ...
                outputText(" " + num2Text(player.breastRows.totalNipples()) + " tendrils encircle your [nipples]");
                if (player.breastRows.hasFuckableNipples()) outputText(", threatening to slide inside them at a moment’s notice");
                else {
                ...
                outputText(".");
                if (player.breastRows.hasFuckableNipples()) outputText(" Your [nipples] are similarly entered.");
...
export function tease(justText: boolean = false): void {
    ...
    if ((futa || bimbo) && player.gender == 3) {
        ...
        // Breast jiggle!
        if (player.breastRows.biggestTitSize() >= 2) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 4) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 8) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 15) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 30) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 50) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 75) choices[choices.length] = 5;
        if (player.breastRows.biggestTitSize() >= 100) choices[choices.length] = 5;
        // Pussy Flash!
        if (player.vaginas.length > 0) {
        ...
        // Adj special!
        if (player.vaginas.length > 0 && player.buttRating >= 8 && player.hipRating >= 6 && player.breastRows.biggestTitSize() >= 4) {
        ...
        // 8 Pec Dance
        if (player.breastRows.biggestTitSize() < 1 && player.tone >= 60) {
    ...
    // VANILLA FOLKS
        ...
        // Breast jiggle!
        if (player.breastRows.biggestTitSize() >= 2) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 4) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 8) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 15) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 30) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 50) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 75) choices[choices.length] = 1;
        if (player.breastRows.biggestTitSize() >= 100) choices[choices.length] = 1;
        // Pussy Flash!
        if (player.vaginas.length > 0) {
            choices[choices.length] = 2;
    ...
    if (player.pregnancyIncubation <= 216 && player.pregnancyIncubation > 0) {
        ...
        if (player.breastRows.biggestLactation() >= 1) choices[choices.length] = 13;
    ...
    if (player.breastRows.hasFuckableNipples()) {
        ...
        if (player.breastRows.biggestTitSize() >= 3) choices[choices.length] = 15;
    ...
    // 24 Poledance - req's staff! - Req's gender!  Req's TITS!
    if (player.weaponName == "wizard's staff" && player.breastRows.biggestTitSize() >= 1 && player.gender > 0) {
    ...
    // 25 Tall Tease! - Reqs 2+ feet & PC Cunt!
    if (player.tallness - monster.tallness >= 24 && player.breastRows.biggestTitSize() >= 4) {
    ...
    // 27 FEEDER
    if (player.perks.findByType(PerkLib.Feeder) >= 0 && player.breastRows.biggestTitSize() >= 4) {
        choices[choices.length] = 27;
        choices[choices.length] = 27;
        choices[choices.length] = 27;
        if (player.breastRows.biggestTitSize() >= 10) choices[choices.length] = 27;
        if (player.breastRows.biggestTitSize() >= 15) choices[choices.length] = 27;
        if (player.breastRows.biggestTitSize() >= 25) choices[choices.length] = 27;
        if (player.breastRows.biggestTitSize() >= 40) choices[choices.length] = 27;
        if (player.breastRows.biggestTitSize() >= 60) choices[choices.length] = 27;
        if (player.breastRows.biggestTitSize() >= 80) choices[choices.length] = 27;
    }
    // 28 FEMALE TEACHER COSTUME TEASE
    ...
    if (player.vaginas.length > 0 && player.lactationQ() >= 500 && player.breastRows.biggestTitSize() >= 6 && cowScore(player) >= 3 && player.tailType == TailType.COW) {
    ...
    switch (select) {
        ...
        case 15:
            // Req's tits & Pussy
            if (player.breastRows.biggestTitSize() > 1 && player.vaginas.length > 0 && rand(2) == 0) {
            ...
            else if (player.breastRows.biggestTitSize() > 1 && rand(2) == 0) {
    ...
    if (auto) {
        ...
        if (breasts) {
            ...
            if (player.breastRows.biggestTitSize() >= 7) {
                bonusChance += .5;
                bonusDamage += 1;
            }
            if (player.breastRows.biggestTitSize() >= 12) {
                bonusChance += .5;
                bonusDamage += 1;
            }
            if (player.breastRows.biggestTitSize() >= 25) {
                bonusChance += .5;
                bonusDamage += 1;
            }
            if (player.breastRows.biggestTitSize() >= 50) {
                bonusChance += .5;
                bonusDamage += 1;
            }
            if (player.breastRows.hasFuckableNipples()) {
...
export function runAway(callHook: boolean = true): void {
    ...
    // Big tits doesn't matter as much if ya can fly!
        ...
        if (player.breastRows.biggestTitSize() >= 35) escapeMod += 5;
        if (player.breastRows.biggestTitSize() >= 66) escapeMod += 10;
    ...
    // FAIL FLEE
        ...
        // Nonflyer messages
            ...
            if (player.breastRows.biggestTitSize() >= 35 || player.buttRating >= 20 || player.hipRating >= 20) {
                // FOR PLAYERS WITH GIANT BREASTS
                if (player.breastRows.biggestTitSize() >= 35 && player.breastRows.biggestTitSize() < 66) {
                    if (player.hipRating >= 20) {
                ...
                else if (player.breastRows.biggestTitSize() >= 66) {
                    if (player.hipRating >= 20) {
