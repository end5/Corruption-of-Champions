...
export class PregnancyStore extends Object {
    ...
    public addPregnancyEventSet(pregType: number, ...pregStage): void {
        const pregVector: Vector.<int> = new Vector.<int>(pregStage.length + 2);
        pregVector[0] = pregType; // First element is the pregnancy type
        for (const i = 0; i < pregStage.length; i++) pregVector[i + 1] = pregStage[i];
    ...
    public addButtPregnancyEventSet(buttPregType: number, ...buttPregStage): void {
        const pregVector: Vector.<int> = new Vector.<int>(buttPregStage.length + 1);
        pregVector[0] = buttPregType; // First element is the butt pregnancy type
        for (const i = 0; i < buttPregStage.length; i++) pregVector[i + 1] = buttPregStage[i];
        pregVector[pregVector.length - 1] = -1; // Make last element -1 to ensure there is always a match
    ...
    public get event(): number {
        const pregType: number = type;
        if (pregType == 0) return 0; // Not pregnant
        const incubationValue: number = incubation;
        let pregEventVector: Vector.<int> = null;
        for (const i = 0; i < _pregnancyEventValue.length; i++) {
            pregEventVector = _pregnancyEventValue[i];
            if (pregEventVector[0] == pregType) {
                for (const j = 1; j < pregEventVector.length; j++) { // Skip element zero, the pregnancy type
    ...
    public get buttEvent(): number {
        const pregType: number = buttType;
        if (pregType == 0) return 0; // Not pregnant
        const incubationValue: number = buttIncubation;
        let pregEventVector: Vector.<int> = null;
        for (const i = 0; i < _buttPregnancyEventValue.length; i++) {
    ...
    public eventTriggered(): number {
        const currentStage: number = event;
        const lastNoticed: number = game.flags[_pregnancyTypeFlag] & PREG_NOTICE_MASK;
    ...
    public buttEventTriggered(): number {
        const currentStage: number = buttEvent;
        const lastNoticed: number = game.flags[_buttPregnancyTypeFlag] & PREG_NOTICE_MASK;
        if (currentStage * 65536 == lastNoticed) return 0; // Player has already noticed this stage
