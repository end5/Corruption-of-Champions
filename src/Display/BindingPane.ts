...
export class BindingPane extends ScrollPane {
    ...
    public constructor(inputManager: InputManager, xPos: number, yPos: number, width: number, height: number, uiscrollwidth: number) {
        ...
        *  was literally on crack, I SWEAR TO GOD. */
        ...
        const blank: MovieClip = new MovieClip();
    ...
    private InitContentObjects(): void {
        ...
        const _textFormatLabel: TextFormat = new TextFormat();
        _textFormatLabel.size = 20;

        const helpLabel: TextField = new TextField();
        ...
        for (const i = 0; i < _functions.length; i++) {
            ...
            const newLabel: BindDisplay = new BindDisplay(this.width);
            ...
            const genPrimaryCallback: () => void = function(funcName: string, inMan: InputManager): () => void {
            ...
            const genSecondaryCallback: () => void = function(funcName: string, inMan: InputManager): () => void {
                return function(): void {
    ...
    private UpdateContentObjects(): void {
        for (const i = 0; i < _functions.length; i++) {
            const currLabel: BindDisplay = _content.getChildByName(_functions[i].Name) as BindDisplay;
    ...
    private PopulateKeyboardDict(): void {
        ...
        const keyDescriptions: XML = describeType(Keyboard);
        const keyNames: XMLList = keyDescriptions.constant.name;
