...
export class ImageManager {
    ...
    private loadImageAtPath(imPath: string): void {
        ...
        const f: () => void = function(key: string): () => void {
        ...
        const req: URLRequest = new URLRequest(imPath);
        imgLoader.load(req);
    }
    ...
    private fileLoaded(e: Event, imPath: string): void {
        ...
        if (_allImagePaths.hasOwnProperty(imPath)) {
            ...
            const imId: string = _allImagePaths[imPath];
            ...
            const underscorePt: number = imPath.lastIndexOf("_");
            if (underscorePt != -1) {
                const decimalPt: number = imPath.lastIndexOf(".");
                const prefix: string = imPath.slice(0, underscorePt + 1);
                ...
                for (const k = 0; k < _imgListXML.ExtensionList.ExtensionType.length(); k++) {
                    // Programmatic extension concatenation! Woot.
                    const newPath: string = prefix + num + "." + _imgListXML.ExtensionList.ExtensionType[k];
    ...
    public getImageHeight(imageURL: string): number {
        ...
        const imageTarget: Image = _fqPathMap[imageURL];

        if (imageTarget == null) {
            return 1;
        }
        else {
            const ratio: number = imageTarget.width / imageTarget.height;
    ...
    private fixupListener(e: Event): void {
        ...
        if (e.target instanceof Loader) {
            ...
            const loader: Loader = e.target as Loader;
    ...
    private doFixup(e: Event): void {
        ...
        const imgRef: Loader = e.target.loader as Loader;
        const mainText: TextField = (mStage.getChildByName("mainView") as MovieClip).mainText as TextField;
        const scrollBar: UIScrollBar = (mStage.getChildByName("mainView") as MovieClip).scrollBar as UIScrollBar;

        const imgRefTopY: number = imgRef.getBounds(mainText).y; 							// 272
        const imgHeight: number = getImageHeight(imgRef.contentLoaderInfo.url); 			// 400
        const imgRefBottomY: number = imgRefTopY + imgHeight;
        const totalTextHeight: number = mainText.textHeight; 								// 264 -- Total displayed pixel height of text
        ...
        const txFormat: TextFormat = mainText.defaultTextFormat;
        let lineHeight: number = txFormat.size as int;
        lineHeight += 4;
        const padLines: number = Math.ceil((imgRefBottomY - totalTextHeight) / lineHeight);
