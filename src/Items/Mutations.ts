...
export class Mutations {
    ...
    public ceruleanPotion(player: Player): void {
        ...
        // All else
            ...
            if (player.effects.findByType(StatusAffects.RepeatSuccubi) >= 0) outputText("You look forwards to tonight's encounter.", false);
            // First timer huh?
        ...
        if (player.effects.findByType(StatusAffects.SuccubiNight) >= 0) {
            if (player.effects.getValue1Of(StatusAffects.SuccubiNight) < 3) player.effects.addValue(StatusAffects.SuccubiNight, 1, 1);
        }
        else player.effects.create(StatusAffects.SuccubiNight, 1, 0, 0, 0);
    }
    ...
    public incubiDraft(tainted: boolean, player: Player): void {
        ...
        if (player.perks.findByType(PerkLib.HistoryAlchemist) >= 0) rando += 10;
        ...
        if (rando < 50) {
            if (player.cocks.length == 1) {
                ...
                if (temp > 2) outputText("  You smile and idly stroke your lengthening " + cockDescript(game.player, 0) + " as a few more inches sprout.", false);
                if (tainted) dynStats("int", 1, "lib", 2, "sen", 1, "lus", 5 + temp * 3, "cor", 1);
                else dynStats("int", 1, "lib", 2, "sen", 1, "lus", 5 + temp * 3);
                if (player.cocks[0].cockType != CockTypesEnum.DEMON) outputText("  With the transformation complete, your " + cockDescript(game.player, 0) + " returns to its normal coloration.", false);
                else outputText("  With the transformation complete, your " + cockDescript(game.player, 0) + " throbs in an almost happy way as it goes flaccid once more.", false);
            }
            if (player.cocks.length > 1) {
                ...
                if (temp3 > 2) outputText("  You smile and idly stroke your lengthening " + cockDescript(player, temp2) + " as a few more inches sprout.", false);
    ...
    public minotaurCum(player: Player): void {
        ...
        if (player.hasVagina()) {
            ...
            if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_NORMAL) outputText("  Wetness builds inside you as your " + vaginaDescript(player, 0) + " tingles and aches to be filled.", false);
            else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_SLICK) outputText("  A trickle of wetness escapes your " + vaginaDescript(player, 0) + " as your body reacts to the desire burning inside you.", false);
    ...
    public minotaurBlood(player: Player): void {
        ...
        if (player.cocks.length > 0 && rand(2) == 0 && changes < changeLimit) {
            ...
            if (selectedCockValue != -1) {
                ...
                if (player.cocks.horseCocks() > 1) outputText("nother");
        ...
        if ((player.gender == 1 || player.gender == 3) && rand(1.5) == 0 && changes < changeLimit) {
            ...
            if ((player.cocks.horseCocks() + player.demonCocks()) < player.cocks.length) {
                ...
                // MULTICOCK
                    ...
                    if (player.cocks.horseCocks() > 1 || player.cocks.dogCocks() > 0) outputText("  Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.", false);
                    else outputText("  You feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your " + cockDescript(player, temp) + "'s root, tightening and pulling your " + cockDescript(player, temp) + " inside its depths.", false);
                    temp2 = player.increaseCock(temp, rand(4) + 4);
            ...
            // Players cocks are all horse-type - increase size!
                ...
                outputText("\n\n", false);
                if (temp2 > 2) outputText("Your " + cockDescript(player, temp) + " tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.", false);
                if (temp2 > 1 && temp2 <= 2) outputText("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged " + cockDescript(player, temp) + " from the pleasure of the growth.", false);
                if (temp2 <= 1) outputText("A slight pressure builds and releases as your " + cockDescript(player, temp) + " pushes a bit further out of your sheath.", false);
                changes++;
            }
    ...
    public succubiMilk(tainted: boolean, player: Player): void {
        ...
        if (player.cor >= 35 && player.cor < 70) {
            ...
            else if (player.cocks.length > 0) outputText("  You feel a building arousal, but it doesn't affect your cock.", false);
        }
        if (player.cor >= 70) {
            ...
            if (player.gender == 2 || player.gender == 3) {
                ...
                if (player.cocks.length == 1) outputText("  You feel a strange sexual pleasure, but your " + multiCockDescript(game.player) + " remains unaffected.", false);
                else outputText("  You feel a strange sexual pleasure, but your " + multiCockDescript(game.player) + " remain unaffected.", false);
            }
        }
        ...
        // Increase pussy wetness or grow one!!
            ...
            if (player.vaginas.length > 0) {
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING) {
                    if (player.vaginas.length == 1) outputText("Your " + vaginaDescript(player, 0) + " gushes fluids down your leg as you spontaneously orgasm.", false);
                    else outputText("Your " + vaginaDescript(player, 0) + "s gush fluids down your legs as you spontaneously orgasm, leaving a thick puddle of pussy-juice on the ground.  It is rapidly absorbed by the earth.", false);
                    player.orgasm();
                    if (tainted) dynStats("cor", 1);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS_DROOLING) {
                    if (player.vaginas.length == 1) outputText("Your pussy feels hot and juicy, aroused and tender.  You cannot resist as your hands dive into your " + vaginaDescript(player, 0) + ".  You quickly orgasm, squirting fluids everywhere.  <b>You are now a squirter</b>.", false);
                    if (player.vaginas.length > 1) outputText("Your pussies feel hot and juicy, aroused and tender.  You cannot resist plunging your hands inside your " + vaginaDescript(player, 0) + "s.  You quiver around your fingers, squirting copious fluids over yourself and the ground.  The fluids quickly disappear into the dirt.", false);
                    player.orgasm();
                    if (tainted) dynStats("cor", 1);
                }
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLICK) {
                    if (player.vaginas.length == 1) outputText("You feel a sudden trickle of fluid down your leg.  You smell it and realize it's your pussy-juice.  Your " + vaginaDescript(player, 0) + " now drools lubricant constantly down your leg.", false);
                ...
                if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS_NORMAL) {
                    if (player.vaginas.length == 1) outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + vaginaDescript(player, 0) + " felt much wetter than normal.", false);
                    else outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + vaginaDescript(player, 0) + " were much wetter than normal.", false);
                }
        ...
        if (rando >= 90) {
            if (player.skinTone == "blue" || player.skinTone == "purple" || player.skinTone == "indigo" || player.skinTone == "shiny black") {
                if (player.vaginas.length > 0) {
                    ...
                    if (player.clitLength > 3 && player.perks.findByType(PerkLib.BigClit) < 0) outputText("  After some time it shrinks, returning to its normal aroused size.  You guess it can't get any bigger.", false);
                    if (player.clitLength > 5 && player.perks.findByType(PerkLib.BigClit) >= 0) outputText("  Eventually it shrinks back down to its normal (but still HUGE) size.  You guess it can't get any bigger.", false);
    ...
    public caninePepper(type: number, player: Player): void {
        ...
        if (type <= 0 && crit > 1 && player.skinType == SKIN_TYPE_FUR && player.faceType == FACE_DOG && player.earType == EARS_DOG && player.lowerBody == LOWER_BODY_TYPE_DOG && player.tailType == TAIL_TYPE_DOG && rand(2) == 0 && player.effects.findByType(StatusAffects.DogWarning) >= 0) {
            ...
            if (temp == 0) {
                ...
                if (player.perks.findByType(PerkLib.MarblesMilk) >= 0) outputText("All you know is that there is a scent on the wind, it is time to hunt, and at the end of the day you need to come home for your milk.", false);
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cumMultiplier < 2 && rand(2) == 0 && changes < changeLimit) {
                ...
                if (player.perks.findByType(PerkLib.MessyOrgasms) >= 0) temp = 2;
                if (temp < player.cumMultiplier + .05 * crit) {
                    ...
                    if (crit > 1) outputText("  A bit of milky pre dribbles from your " + multiCockDescriptLight(game.player) + ", pushed out by the change.", false);
                }
                changes++;
            }
            // Oversized pepper
    ...
    public succubisDelight(tainted: boolean, player: Player): void {
        ...
        if (changes < changeLimit && rand(2) == 0 && player.cocks.length > 0) {
            if (player.cumMultiplier < 6 && rand(2) == 0 && changes < changeLimit) {
                ...
                if (player.perks.findByType(PerkLib.MessyOrgasms) >= 0) temp = 6;
            ...
            public function ovipositionElixer(player:Player):void
            {
                ...
                if (player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
                    if (player.effects.findByType(StatusAffects.Eggs) >= 0) {
                        ...
                        if (player.effects.getValue2Of(StatusAffects.Eggs) == 0) {
                            //1 in 2 chance!
                            if (rand(3) == 0) {
                                player.effects.addValue(StatusAffects.Eggs,2,1);
                        ...
                        if (rand(2) == 0) {
                            ...
                            player.effects.addValue(StatusAffects.Eggs,3,rand(4) + 1);
                            changes++;
                        }
                    }
                }
    ...
    */
        ...
        if (!large) {
            ...
            if (player.cocks.length > 0) {
                ...
                if (player.cocks.length == 1) {
                    ...
                    // Display the degree of thickness change.
                    if (temp3 >= 1) {
                        if (player.cocks.length == 1) outputText("  Your " + multiCockDescriptLight(game.player) + " spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                        else outputText("  Your " + multiCockDescriptLight(game.player) + " spread rapidly, swelling as they grow an inch or more in girth, making them feel fat and floppy.", false);
                    }
                    if (temp3 <= .5) {
                        if (player.cocks.length > 1) outputText("  Your " + multiCockDescriptLight(game.player) + " feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                        else outputText("  Your " + multiCockDescriptLight(game.player) + " feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
                    }
                    if (temp3 > .5 && temp2 < 1) {
                        if (player.cocks.length == 1) outputText("  Your " + multiCockDescriptLight(game.player) + " seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                        if (player.cocks.length > 1) outputText("  Your " + multiCockDescriptLight(game.player) + " seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                    }
                    dynStats("lib", 1, "sen", 1, "lus", 20);
                }
    ...
    public purePearl(player: Player): void {
        ...
        if (player.perks.findByType(PerkLib.PurityBlessing) < 0) player.perks.create(PerkLib.PurityBlessing, 0, 0, 0, 0);
    }
    ...
    */			/*
                ...
                if (player.cocks.length > 0) cockEvent = 1042;
                itemSubMenu = true;
    ...
    /*		}
            ...
            private function growPlusBalls():void {
                ...
                //1 in 4 BIG growth.
                if (rand(4) == 0) {
                    outputText("You feel a trembling in your " + ballsDescriptLight(player) + " as the chemicals start to go to work.  You can tell they're going to be VERY effective.\n");
                    player.ballSize += 2 + rand(4);
                    outputText("They shift, stretching your " + sackDescript(player) + " tight as they gain inches of size.  You step to steady yourself as your center of balance shifts due to your newly enlarged " + ballsDescriptLight(player) + ".  ");
                }
                else {
                    player.ballSize += 1 + rand(2);
                    outputText("You feel your testicles shift, pulling the skin of your " + sackDescript(player) + " a little bit as they grow to " + ballsDescriptLight(player) + ".  ");
                }
            ...
            private function growPlusBreasts():void {
                clearOutput();
                player.slimeFeed();
                outputText("You sink the needle into the flesh of your " + allBreastsDescript(player) + " injecting each with a portion of the needle.\n\n");
                if (player.breastRows.length == 1)
            ...
            private function growPlusClit():void {
                ...
                player.clitLength++;
                outputText("Your " + clitDescription(player) + " stops growing after an inch of new flesh surges free of your netherlips.  It twitches, feeling incredibly sensitive.");
            ...
            private function growPlusNipples():void {
                clearOutput();
                player.slimeFeed();
                outputText("You sink the needle into each of your " + nippleDescription(player, 0) + "s in turn, dividing the fluid evenly between them.  Though each injection hurts, the pain is quickly washed away by the potent chemical cocktail.\n\n");
            //Grow nipples
                ...
                if (!player.breasts.hasFuckableNipples() && rand(4) == 0) {
                    ...
                    //Talk about if anything was changed.
                    if (nowFuckable) outputText("Your " + allBreastsDescript(player) + " tingle with warmth that slowly migrates to your nipples, filling them with warmth.  You pant and moan, rubbing them with your fingers.  A trickle of wetness suddenly coats your finger as it slips inside the nipple.  Shocked, you pull the finger free.  <b>You now have fuckable nipples!</b>\n\n");
                }
                inventory.itemGoNext();
            }
    ...
    */ /*
    ...
    /*
            ...
            private function reductoBalls():void {
                ...
                outputText("You feel your scrotum shift, shrinking down along with your " + ballsDescriptLight(player) + ".  Within a few seconds the paste has been totally absorbed and the shrinking stops.");
                dynStats("lib", -2, "lus", -10);
                inventory.itemGoNext();
            }
            ...
            private function reductoButt():void {
                clearOutput();
                outputText("You smear the foul-smelling paste onto your " + buttDescription(player) + ".  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n");
                if (player.buttRating >= 15) {
                    ...
                    player.buttRating -= 3;
                    outputText("You feel much lighter as your " + buttDescription(player) + " jiggles slightly, adjusting to its smaller size.");
                }
                else {
                    player.buttRating -= 1 + rand(3);
                    if (player.buttRating < 1) player.buttRating = 1;
                    outputText("After a few seconds your " + buttDescription(player) + " has shrunk to a much smaller size!");
                }
                dynStats("lib", -2, "lus", -10);
                inventory.itemGoNext();
            }
            ...
            private function reductoClit():void {
                clearOutput();
                outputText("You carefully apply the paste to your " + clitDescription(player) + ", being very careful to avoid getting it on your " + vaginaDescript(player, 0) + ".  It burns with heat as it begins to make its effects known...\n\n");
                player.clitLength /= 1.7;
                //Set clitlength down to 2 digits in length
                player.clitLength = int(player.clitLength * 100) / 100;
                outputText("Your " + clitDescription(player) + " shrinks rapidly, dwindling down to almost half its old size before it finishes absorbing the paste.");
                dynStats("sen", 2, "lus", 10);
                inventory.itemGoNext();
            }
            ...
            private function reductoCock():void {
                ...
                if (player.cocks.length == 1) {
                    outputText("Your " + cockDescript(game.player, 0) + " twitches as it shrinks, disappearing steadily into your " + (player.cocks.hasSheath() ? "sheath" : "crotch") + " until it has lost about a third of its old size.");
                    player.cocks[0].cockLength *= 2 / 3;
                    player.cocks[0].cockThickness *= 2 / 3;
                }
                //MULTI
                else {
                    outputText("Your " + multiCockDescriptLight(game.player) + " twitch and shrink, each member steadily disappearing into your " + (player.cocks.hasSheath() ? "sheath" : "crotch") + " until they've lost about a third of their old size.");
                    for (var i:int = 0; i < player.cocks.length; i++) {
            ...
            private function reductoNipples():void {
                clearOutput();
                outputText("You rub the paste evenly over your " + nippleDescription(player, 0) + "s, being sure to cover them completely.\n\n");
                //Shrink
                if (player.nippleLength / 2 < 0.25) {
                    ...
                    outputText("Your " + nippleDescription(player, 0) + "s get smaller and smaller, stopping when they are roughly half their previous size.");
                    player.nippleLength /= 2;
                }
    ...
    */
    ...
    public useMarbleMilk(player: Player): void {
        ...
        if (player.effects.getValue2Of(StatusAffects.Marble) < 30 && player.effects.getValue3Of(StatusAffects.Marble) == 0) outputText("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n", false);
        // [before the player is addicted, Addiction < 50]
        else if (player.effects.getValue3Of(StatusAffects.Marble) <= 0) outputText("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n", false);
        else if (player.effects.getValue3Of(StatusAffects.Marble) > 0) {
            // [player is completely addicted]
            if (player.perks.findByType(PerkLib.MarblesMilk) >= 0) outputText("You gulp down the bottle's contents; it's no substitute for the real thing, but it's a nice pick me up.\n\n", false);
            else {
                // [player is no longer addicted]
                if (player.perks.findByType(PerkLib.MarbleResistant) >= 0) outputText("You gulp down the bottle's contents; you're careful not to get too attached to the taste.\n\n", false);
                // [player is addicted]
        ...
        else player.effects.create(StatusAffects.BottledMilk, 12, 0, 0, 0);
    }
    ...
    public laBova(tainted: boolean, enhanced: boolean, player: Player): void {
        ...
        if (player.hasVagina()) {
            if (player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS_LOOSE && changes < changeLimit && rand(2) == 0) {
                ...
                if (player.effects.findByType(StatusAffects.CuntStretched) < 0) player.effects.create(StatusAffects.CuntStretched, 0, 0, 0, 0);
                // Reset the timer on it to 0 when restretched.
                else player.effects.setValue(StatusAffects.CuntStretched, 1, 0);
                player.vaginas[0].vaginalLooseness++;
                changes++;
                dynStats("lus", 10);
            }
    ...
    public gooGasmic(player: Player): void {
        ...
        if (player.vaginalCapacity() < 9000) {
            if (player.effects.findByType(StatusAffects.BonusVCapacity) < 0) player.effects.create(StatusAffects.BonusVCapacity, 9000, 0, 0, 0);
            else player.effects.addValue(StatusAffects.BonusVCapacity, 1, 9000);
    ...
    /*
            public function extensionSerum(player:Player):void
            {
                ...
                if (flags[kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] > 0 && player.hairType != 4) {
                    ...
                    outputText("\n\n<b>Somehow you know that your " + hairDescription(player) + " is growing again.</b>", false);
                }
    ...
    public coal(player: Player): void {
        ...
        // Males go into rut
            ...
            if (player.effects.getValue1Of(StatusAffects.BonusACapacity) < 80) {
                if (player.effects.findByType(StatusAffects.BonusACapacity) < 0) player.effects.create(StatusAffects.BonusACapacity, 0, 0, 0, 0);
                ...
                for (const k = 0; k < player.breastRows.length; k++) {
                    ...
                    if (player.breastRows[k].breastRating > temp2) {
                        ...
                        if (player.breastRows[k].breastRating > 10) {
                            ...
                            else outputText("  The change moves down to your " + num2Text2(k + 1) + " row of " + breastDescript(game.player, 0) + ". They shrink greatly, losing a couple cup-sizes.", false);
                        }
                        // Small change
                        else {
                            ...
                            else outputText("  Your " + num2Text2(k + 1) + " row of " + breastDescript(game.player, k) + " gives a tiny jiggle as it shrinks, losing some off its mass.", false);
                        }
    ...
    public neonPinkEgg(pregnantChange: boolean, player: Player): void {
        ...
        if (changes < changeLimit && rand(5) == 0) {
            ...
            // Libido over 60? FUCK YEAH!
                ...
                outputText(".  ", false);
                if (player.gender == 1) outputText("Compression tightens down on " + sMultiCockDesc(game.player) + " as it strains against your " + player.armorName + ".  You struggle to fight down your heightened libido, but it's hard – so very hard.", false);
                else if (player.gender == 0) outputText("Sexual hunger seems to gnaw at your " + assholeDescript(player) + ", demanding it be filled, but you try to resist your heightened libido.  It's so very, very hard.", false);
        ...
        if (player.sens < 60 && changes < changeLimit && rand(3) == 0) {
            ...
            // (BIG boost 1/3 chance)
                ...
                else if (player.gender == 2) outputText("your " + vaginaDescript(player, 0) + " and " + clitDescription(player), false);
                ...
                // oh god genderless
                else outputText("you " + assholeDescript(player), false);
        ...
        if ((player.balls > 0 || player.cocks.length > 0) && player.cumQ() < 3000 && rand(3) == 0 && changeLimit > 1) {
            ...
            if (player.balls > 0) {
                ...
                // (medium cum quantity) < 250
                    ...
                    if (player.cocks.length > 0) outputText("  In the process, you brush " + sMultiCockDesc(game.player) + " and discover a bead of pre leaking at the tip.", false);
                }
                // (large cum quantity) < 750
                    ...
                    if (player.cocks.length > 0) outputText(SMultiCockDesc(game.player) + " leaks and dribbles pre-cum down your " + legs(player) + " as your body's cum production kicks up even higher.", false);
                ...
                // (XL cum quantity) < 2000
                    ...
                    if (player.cocks.length > 0) outputText("A thick trail of slime leaks from " + sMultiCockDesc(game.player) + " down your " + leg(player) + ", pooling below you.", false);
                ...
                // (XXL cum quantity)
                    ...
                    if (player.cocks.length > 0) outputText("pre-cum explodes from " + sMultiCockDesc(game.player) + ", running down your " + leg(player) + " and splattering into puddles that would shame the orgasms of lesser " + mf(player, "males", "persons") + ".  You rub yourself a few times, nearly starting to masturbate on the spot, but you control yourself and refrain for now.", false);
                ...
                if (player.cocks.length > 0 && (player.gender != 3 || rand(2) == 0)) {
                    ...
                    if (rand(2) == 0) {
                        ...
                        if (player.cocks.length > 1) {
                            ...
                            if (player.cocks.length > 2) outputText("es rub ", false);
    ...
    public kangaFruit(type: number, player: Player): void {
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cocks.kangaCocks() < player.cocks.length && (type == 1 && rand(2) == 0) && changes < changeLimit) {
                ...
                if (player.cocks.hasSheath()) outputText("your sheath", false);
    ...
    public sweetGossamer(type: number, player: Player): void {
        ...
        if (player.cocks.length > 0 && changes < changeLimit && rand(4) == 0) {
            ...
            if (temp == 1) {
                ...
                if (player.cocks.length == 1) outputText("It's", false);
        ...
        // Drider butt
        if (type == 1 && player.perks.findByType(PerkLib.SpiderOvipositor) < 0 && player.isDrider() && player.tailType == TAIL_TYPE_SPIDER_ADBOMEN && changes < changeLimit && rand(3) == 0 && (player.hasVagina || rand(2) == 0)) {
    ...
    /*
            public function applyLustStick(player:Player):void
            {
                ...
                if (player.effects.findByType(StatusAffects.LustStickApplied) >= 0) {
                    player.effects.addValue(StatusAffects.LustStickApplied, 1, 12 + rand(12));
                    ...
                    player.effects.create(StatusAffects.LustStickApplied, 24, 0, 0, 0);
                    outputText("You carefully open the sweet-smelling tube and smear the lipstick over your lips.  ", false);
                    if (player.cocks.length > 0) outputText("It tingles a little, but the drugs have little to no effect on you now.", false);
    ...
    */
    ...
    public broBrew(player: Player): void {
        ...
        if (player.perks.findByType(PerkLib.BimboBody) >= 0) {
            ...
            outputText("\n\n", false);
            if (player.perks.findByType(PerkLib.BimboBrains) >= 0) outputText("<b>(Lost Perks - Bimbo Brains, Bimbo Body)\n", false);
        ...
        // (No dick)
            ...
            if (player.balls == 0) outputText("  Two rounded orbs drop down below, filling out a new, fleshy sac above your " + legs(player) + ".  Sweet!  You can probably cum buckets with balls like these.", false);
            outputText("\n\n", false);
            player.cocks.createCock();
    ...
    public ectoplasm(player: Player): void {
        ...
        if (player.sens >= 20 && rand(3) == 0 && changes < changeLimit) {
            outputText("\n\nWoah, what the... you pinch your " + skinFurScales(player) + " to confirm your suspicions; the ghostly snack has definitely lowered your sensitivity.", false);
            dynStats("sen", -2);
        ...
        // Legs
        if (changes < changeLimit && player.perks.findByType(PerkLib.Incorporeality) < 0 && (player.skinTone == "white" || player.skinTone == "sable") && player.hairType == 2) {
            ...
            outputText("\n\nAn otherworldly sensation begins in your belly, working its way to your " + hipDescription(player) + ". Before you can react, your " + legs(player) + " begin to tingle, and you fall on your rump as a large shudder runs through them. As you watch, your lower body shimmers, becoming ethereal, wisps rising from the newly ghost-like " + legs(player) + ". You manage to rise, surprised to find your new, ghostly form to be as sturdy as its former corporeal version. Suddenly, like a dam breaking, fleeting visions and images flow into your head, never lasting long enough for you to concentrate on one. You don't even realize it, but your arms fly up to your head, grasping your temples as you groan in pain. As fast as the mental bombardment came, it disappears, leaving you with a surprising sense of spiritual superiority.  <b>You have ghost legs!</b>\n\n", false);
    ...
    public foxTF(enhanced: boolean, player: Player): void {
        ...
        if (player.cumQ() < 5000 < 2 && rand(3) == 0 && changes < changeLimit && player.cocks.length > 0) {
            temp = 2 + rand(4);
            // Lots of cum raises cum multiplier cap to 2 instead of 1.5
            if (player.perks.findByType(PerkLib.MessyOrgasms) >= 0) temp += rand(10);
    ...
    public eatEmberEgg(player: Player): void {
        ...
        if (player.perks.findByType(PerkLib.Dragonfire) >= 0) {
            if (player.effects.findByType(StatusAffects.DragonBreathCooldown) >= 0) player.effects.remove(StatusAffects.DragonBreathCooldown);
                ...
                if (player.effects.findByType(StatusAffects.DragonBreathBoost) < 0) player.effects.create(StatusAffects.DragonBreathBoost, 0, 0, 0, 0);
            }
    ...
    */
    ...
    public fishFillet(player: Player): void {
        ...
        if (player.effects.findByType(StatusAffects.FactoryOverload) >= 0) dynStats("cor", 0.5);
        dynStats("cor", 0.1);
    ...
    public trapOil(player: Player): void {
        ...
        if (player.sens < 70 && player.cocks.length > 0 && rand(3) == 0 && changes < changeLimit) {
        ...
        if (player.cocks.longestCockLength() >= 3.5 && player.cocks.length > 0 && rand(2) == 0 && changes < changeLimit) {
            outputText("\n\nYou flinch and gasp as your " + multiCockDescriptLight(game.player) + " suddenly become");
            ...
            if (player.cocks.length == 1) outputText("it seems");
        ...
        // Testicle Reduction:
        if (player.balls > 0 && player.cocks.length > 0 && (player.ballSize > 1 || player.effects.findByType(StatusAffects.Uniball) < 0) && rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.ballSize < 1 && player.effects.findByType(StatusAffects.Uniball) < 0) {
                outputText("  You whimper as once again, your balls tighten and shrink.  Your eyes widen when you feel the gentle weight of your testicles pushing against the top of your [hips], and a few hesitant swings of your rear confirm what you can feel - you've tightened your balls up so much they no longer hang beneath your " + multiCockDescriptLight(game.player) + ", but press perkily upwards.  Heat ringing your ears, you explore your new sack with a careful hand.  You are deeply grateful you apparently haven't reversed puberty, but you discover that though you still have " + num2Text(player.balls) + ", your balls now look and feel like one: one cute, tight little sissy parcel, its warm, insistent pressure upwards upon the joining of your thighs a never-ending reminder of it.");
                ...
                player.ballSize = 1;
                player.effects.create(StatusAffects.Uniball, 0, 0, 0, 0);
            }
    ...
    public ringtailFig(player: Player): void {
        ...
        if (player.sens < 80 && rand(3) == 0 && changes < changeLimit) {
            outputText("\n\nThe wrinkled rind suddenly feels alarmingly distinct in your hands, and you drop the remnants of the fruit.  Wonderingly, you touch yourself with a finger - you can feel even the lightest pressure on your " + skinFurScales(player) + " much more clearly now!");
        ...
        if (player.faceType != FACE_RACCOON_MASK && player.faceType != FACE_RACCOON && rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.faceType == FACE_HUMAN || player.faceType == FACE_SHARK_TEETH || player.faceType == FACE_SNAKE_FANGS || player.faceType == FACE_BUNNY) {
                ...
                else outputText("A dark, almost black mask shades the " + skinFurScales(player) + " around your eyes and over the topmost portion of your nose, lending you a criminal air!  <b>You now have a raccoon mask!</b>");
            }
    ...
    public mouseCocoa(player: Player): void {
        ...
        if (player.earType != EARS_MOUSE && changes < changeLimit && rand(4) == 0) {
            ...
            outputText(", like they're being pinched, and you can distinctly feel the auricles taking a rounded shape through the pain.  Reaching up to try and massage away their stings, <b>you're not terribly surprised when you find a pair of fuzzy mouse's ears poking through your " + hairDescription(player) + ".</b>");
        ...
        if ((player.skinType != SKIN_TYPE_FUR || (player.skinType == SKIN_TYPE_FUR && (player.hairColor != "brown" && player.hairColor != "white"))) && changes < changeLimit && rand(4) == 0) {
            // from skinscales
            if (player.skinType != SKIN_TYPE_FUR) {
                outputText("\n\nYour " + skinFurScales(player) + " itch");
