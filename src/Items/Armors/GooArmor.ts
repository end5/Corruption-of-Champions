...
export class GooArmor extends Armor {
    ...
    public useText() { // Produces any text seen when equipping the armor normally
        ...
        if (game.player.vaginas.length > 0) outputText("[vagina]");
        if (game.player.vaginas.length > 0 && game.player.cocks.length > 0) outputText(" and ");
