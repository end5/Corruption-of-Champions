...
export class BimboLiqueur extends Consumable {
    ...
    public canUse(): boolean {
        if (game.player.perks.findByType(PerkLib.FutaForm) < 0) return true;
        outputText("Ugh.  This stuff is so, like... last year.  Maybe you can find someone else to feed it to?\n\n");
        return false;
    }

    public useItem(): boolean {
        if (game.player.perks.findByType(PerkLib.BroBody) >= 0) {
            ...
            if (game.player.breastRows.biggestTitSize() < 7) {
            ...
            if (game.player.perks.findByType(PerkLib.BroBrains) >= 0) outputText(", Bro Brains");
            outputText(")\n");
            outputText("(Perks Gained: Futa Form, Futa Faculties)\n");
            game.player.perks.remove(PerkLib.BroBody);
            game.player.perks.remove(PerkLib.BroBrains);
            game.player.perks.create(PerkLib.FutaFaculties, 0, 0, 0, 0);
            game.player.perks.create(PerkLib.FutaForm, 0, 0, 0, 0);
            ...
            if (game.player.hairColor != "platinum blonde") {
                outputText("The first change that you notice is to your " + hairDescription(player) + ".  It starts with a tingling in your scalp and intensifies ");
            ...
            if (!game.player.vaginas.length > 0) {
                ...
                if (game.player.isTaur()) {
                    ...
                    if (game.player.vaginas[0].vaginalWetness < VaginaWetness.SLICK)
                        game.player.vaginas[0].vaginalWetness = VaginaWetness.SLICK;
                }
                else {
            ...
            if (game.player.breastRows[0].breastRating < 10) {
                ...
                outputText(", surprised at how large and rounded your expanding breasts have become while fresh tit-flesh continues to spill out around your needy fingers.  They feel so supple and soft, but when you let them go, they still sit fairly high and firm on your chest.  The newer, more generous, " + breastCupOfRow(game.player, 0) + " cleavage has you moaning with how sensitive it is, pinching a nipple with one hand ");
            }
            else {
                game.player.breastRows[0].breastRating += 5 + rand(5);
                outputText(", admiring how sensitive they're getting.  The big breasts start getting bigger and bigger, soft chest-flesh practically oozing out between your fingers as the squishy mammaries sprout like weeds, expanding well beyond any hand's ability to contain them.  The supple, " + breastCupOfRow(game.player, 0) + " boobs still manage to sit high on your chest, almost gravity defying in their ability to generate cleavage.  You pinch a nipple with one hand ");
            }
            dynStats("sen", 20);
            ...
            if (game.player.tone > 0) {
                ...
                if (game.player.str >= 30) {
                    if (game.player.str >= 90) dynStats("str", -10);
                    if (game.player.str >= 70) dynStats("str", -10);
                    if (game.player.str >= 50) dynStats("str", -10);
                    dynStats("str", -5);
            ...
            if (game.player.perks.findByType(PerkLib.BimboBody) < 0) {
                outputText("<b>(Bimbo Body - Perk Gained!)\n");
                game.player.perks.create(PerkLib.BimboBody, 0, 0, 0, 0);
            }
            if (game.player.perks.findByType(PerkLib.BimboBrains) < 0) {
                outputText("(Bimbo Brains - Perk Gained!)\n"); // int to 20.  max int 50)
                game.player.perks.create(PerkLib.BimboBrains, 0, 0, 0, 0);
