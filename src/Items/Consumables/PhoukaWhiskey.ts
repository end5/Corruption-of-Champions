...
export class PhoukaWhiskey extends Consumable {
    ...
    public phoukaWhiskeyAcceptable(player: Player): number { // This function provides a single common test that can be used both by this class and the PhoukaScene class
        ...
        // Double pregnancy
        const wombBabyLikesAlcohol: boolean = (player.pregnancyType == PregnancyStore.PREGNANCY_SATYR) || (player.pregnancyType == PregnancyStore.PREGNANCY_FAERIE);
        const colonBabyLikesAlcohol: boolean = (player.buttPregnancyType == PregnancyStore.PREGNANCY_SATYR);
        if (wombBabyLikesAlcohol && colonBabyLikesAlcohol) return 3;
        if (!wombBabyLikesAlcohol && !colonBabyLikesAlcohol) return -3;
        return -4;
    }

    public phoukaWhiskeyDrink(player: Player): number { // This function provides a single common test that can be used both by this class and the PhoukaScene class
