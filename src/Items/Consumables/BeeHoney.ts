...
export class BeeHoney extends Consumable {
    ...
    public useItem(): boolean {
        ...
        const pure: boolean = (value == PURE_HONEY_VALUE);
        const special: boolean = (value == SPECIAL_HONEY_VALUE);
        ...
        // Bee Legs
        if (changes < changeLimit && player.lowerBody != CoC.LOWER_BODY_TYPE_BEE && player.lowerBody != CoC.LOWER_BODY_TYPE_CENTAUR && rand(4) == 0) {
        ...
        // Venom Increase
        if (changes < changeLimit && player.tailType == CoC.TAIL_TYPE_BEE_ABDOMEN && player.tailRecharge < 15 && rand(2)) {
        ...
        if (changes < changeLimit && player.wingType == CoC.WING_TYPE_BEE_LIKE_SMALL && rand(4)) {
            changes++;
        ...
        if (changes < changeLimit && (player.wingType == CoC.WING_TYPE_NONE || player.wingType == CoC.WING_TYPE_SHARK_FIN) && rand(4)) {
            changes++;
        ...
        if (special) { // All the speical honey effects occur after any normal bee transformations (if the player wasn't a full bee morph)
            if (!player.cocks.length > 0) {
                ...
                outputText("\n\nYour " + cockDescript(player, 0) + " suddenly becomes rock hard and incredibly sensitive to the touch.  You pull away your " + player.armorName + ", and start to masturbate furiously as it rapidly swells in size.  When the change finally finishes, you realize that your " + cockDescript(player, 0) + " has both grown much longer and wider!  <b>");
                ...
                outputText("\n\nThe effects of the honey don’t seem to focus on your groin this time, but you still feel your " + cockDescript(player, 0) + " grow slightly under your " + player.armorName + ".");
                player.cocks[0].cockLength += 0.1 * rand(10) + 1;
                player.cocks[0].cockThickness += 0.1 * rand(2) + 0.1;
                dynStats("sen", 3);
            }
            if (player.cor >= 5) {
