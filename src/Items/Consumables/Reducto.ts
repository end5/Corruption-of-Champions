...
export class Reducto extends Consumable {
    ...
    public useItem(): boolean {
        ...
        const rdtBreasts: () => void = (game.player.breastRows.length > 0 && game.player.breastRows.biggestTitSize() > 0 ? reductoBreasts : null);
    ...
    private reductoBalls(): void {
        clearOutput();
        outputText("You smear the foul-smelling paste onto your " + sackDescript(game.player) + ".  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n");
    ...
    private reductoBreasts(): void {
        ...
        if (rand(2) == 0 && game.player.breastRows.biggestTitSize() >= 1) {
    ...
    private reductoClit(): void {
        clearOutput();
        outputText("You carefully apply the paste to your " + clitDescription(game.player) + ", being very careful to avoid getting it on your " + vaginaDescript(game.player, 0) + ".  It burns with heat as it begins to make its effects known...\n\n");
    ...
    private reductoCock(): void {
        ...
        if (game.player.cocks[0].cockType == CockTypesEnum.BEE) {
            outputText("The gel produces an odd effect when you rub it into your " + cockDescript(game.player, 0) + ".  It actually seems to calm the need that usually fills you.  In fact, as your " + cockDescript(game.player, 0) + " shrinks, its skin tone changes to be more in line with yours and the bee hair that covered it falls out.  <b>You now have a human cock!</b>");
    ...
    private reductoNipples(): void {
        clearOutput();
        outputText("You rub the paste evenly over your " + nippleDescription(game.player, 0) + "s, being sure to cover them completely.\n\n");
        // Shrink
        if (game.player.nippleLength / 2 < 0.25) {
            ...
            outputText("Your " + nippleDescription(game.player, 0) + "s get smaller and smaller, stopping when they are roughly half their previous size.");
