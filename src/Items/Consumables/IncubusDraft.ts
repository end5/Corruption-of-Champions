...
export function demonChanges(player: Player): void {
    ...
    if (player.tailType != TailType.DEMONIC && player.horns > 0) {
        if (player.tailType != TailType.NONE) {
            outputText("\n\n", false);
            if (player.tailType == TailType.SPIDER_ADBOMEN || player.tailType == TailType.BEE_ABDOMEN) outputText("You feel a tingling in your insectile abdomen as it stretches, narrowing, the exoskeleton flaking off as it transforms into a flexible demon-tail, complete with a round spaded tip.  ", false);
    ...
    if (player.horns == 0 || (rand(player.horns + 3) == 0)) {
        if (player.horns < 12 && (player.hornType == HornType.NONE || player.hornType == HornType.DEMON)) {
            outputText("\n\n", false);
            if (player.horns == 0) {
            ...
            if (player.hornType == HornType.NONE) player.hornType = HornType.DEMON;
            player.horns++;
            player.horns++;
            dynStats("cor", 3);
        }
        // Text for shifting horns
        else if (player.hornType > HornType.DEMON) {
            outputText("\n\n", false);
    ...
    // remove fur
    if ((player.faceType != FaceType.HUMAN || player.skinType != SkinType.PLAIN) && rand(3) == 0) {
        ...
        // De-fur
            ...
            outputText("\n\n", false);
            if (player.skinType == SkinType.FUR) outputText("Your skin suddenly feels itchy as your fur begins falling out in clumps, <b>revealing inhumanly smooth skin</b> underneath.", false);
            if (player.skinType == SkinType.SCALES) outputText("Your scales begin to itch as they begin falling out in droves, <b>revealing your inhumanly smooth " + player.skinTone + " skin</b> underneath.", false);
            player.skinType = SkinType.PLAIN;
            player.skinDesc = "skin";
        }
    }
    // Demon tongue
    if (player.tongueType == TongueType.SNAKE && rand(3) == 0) {
    ...
    if (player.skinType == SkinType.PLAIN && rand(4) == 0) {
        // Males/genderless get clawed feet
        if (player.gender <= 1) {
            if (player.lowerBody != LowerBodyType.DEMONIC_CLAWS) {
                outputText("\n\n", false);
                ...
                player.lowerBody = LowerBodyType.DEMONIC_CLAWS;
            }
        }
    ...
    // Grow demon wings
    if (player.wingType != WingType.BAT_LIKE_LARGE && rand(8) == 0 && player.cor >= 50) {
        // grow smalls to large
        if (player.wingType == WingType.BAT_LIKE_TINY && player.cor >= 75) {
            outputText("\n\n", false);
            ...
            player.wingType = WingType.BAT_LIKE_TINY;
            player.wingDesc = "tiny, bat-like";
            outputText("bat-like demon-wings!", false);
        }
        else if (player.wingType == WingType.BEE_LIKE_SMALL || player.wingType == WingType.BEE_LIKE_LARGE) {
            outputText("\n\n", false);
        ...
        // No wings
        else if (player.wingType == WingType.NONE) {
            outputText("\n\n", false);
