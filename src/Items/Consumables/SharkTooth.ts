export function sharkTooth(type: number, player: Player): void {
    ...
    // Transformations:
    // Mouth TF
    if (player.faceType != FaceType.SHARK_TEETH && rand(3) == 0 && changes < changeLimit) {
        outputText("\n\n", false);
    ...
    // Tail TF
    if (player.tailType != TailType.SHARK && rand(3) == 0 && changes < changeLimit) {
        changes++;
        ...
        player.tailType = TailType.SHARK;
    }
    // Hair
    ...
    // Skin
    if (((player.skinTone != "rough gray" && player.skinTone != "orange and black striped") || player.skinType != SkinType.PLAIN) && rand(7) == 0 && changes < changeLimit) {
        outputText("\n\n", false);
    ...
    // FINZ R WINGS
    if (player.wingType != WingType.SHARK_FIN && changes < changeLimit && rand(3) == 0) {
        outputText("\n\n", false);
        ...
        player.wingType = WingType.SHARK_FIN;
        player.wingDesc = "";
        changes++;
    }
    if (changes == 0) {
