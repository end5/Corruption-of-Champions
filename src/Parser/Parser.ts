...
export class Parser {
    ...
    */
    ...
    private convertSingleArg(arg: string): string {
        ...
        const capitalize: boolean = isUpperCase(arg.charAt(0));
    ...
    private convertDoubleArg(inputArg: string): string {
        ...
        const capitalize: boolean = isUpperCase(aspect.charAt(0));
        ...
        if (thing != null) {
            if (thing instanceof Function) {
                ...
                const indice: number = Number(aspectLower);
                if (isNaN(indice)) {
    ...
    private evalConditionalStatementStr(textCond: string): boolean {
        ...
        const expressionResult: Record<string, any> = isExp.exec(textCond);
        if (!expressionResult) {
            const condArg: any = convertConditionalArgumentFromStr(textCond);
            if (condArg != null) {
        ...
        const condArg1: any = convertConditionalArgumentFromStr(condArgStr1);
        const condArg2: any = convertConditionalArgumentFromStr(condArgStr2);
    ...
    private parseConditional(textCtnt: string, depth: number): string {
        ...
        const ret: any[] = new Array("", "", "");	// first string is conditional, second string is the output
    ...
    private getSceneSectionToInsert(inputArg: string): string {
        ...
        if (sceneName in this.parserState) {
            ...
            const tmp2: string = recParser(tmp1, 0);			// we have to actually parse the scene now
    ...
    private parseButtonTag(textCtnt: string): void {
        ...
        const buttonName: string = stripStr(arr[1]);
        const buttonFunc: string = stripStr(arr[0].substring(arr[0].indexOf(' ')));
    ...
    public recursiveParser(contents: string, parseAsMarkdown: boolean = false, prettyQuotes: boolean = true): string {
        ...
        if (parseAsMarkdown) {
            ...
            const regexPCloseTag: RegExp = /<\/p>/gi;
