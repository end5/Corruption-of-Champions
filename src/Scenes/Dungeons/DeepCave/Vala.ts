...
export class Vala extends Monster {
    ...
    public valaSpecial2(): void {
        ...
        if (this.effects.findByType(StatusAffects.Milk) >= 0) {
            this.effects.addValue(StatusAffects.Milk, 1, 5);
            ...
            this.effects.create(StatusAffects.Milk, 5, 0, 0, 0);
        ...
        dynStats("lus", this.effects.getValue1Of(StatusAffects.Milk) + player.lib / 20);
        combatRoundOver();
    }
    // Masturbation
    ...
    public valaCombatDialogue(): void {
        if (this.effects.findByType(StatusAffects.Vala) < 0) {
            ...
            this.effects.create(StatusAffects.Vala, 0, 0, 0, 0);
        }
        else {
            this.effects.addValue(StatusAffects.Vala, 1, 1);
            if (this.effects.getValue1Of(StatusAffects.Vala) == 1) outputText("\"<i>If you won't fuck Bitch, you must not be a master,</i>\" she realizes, the fight invigorating her lust-deadened brain. \"<i>You get to be a pet for the masters, too!</i>\"", false);
            else if (this.effects.getValue1Of(StatusAffects.Vala) == 2) outputText("\"<i>If the masters like you, maybe they will let Bitch keep you for herself! Won't you like that?</i>\"", false);
            else if (this.effects.getValue1Of(StatusAffects.Vala) == 3) outputText("\"<i>We obey the masters. They fed Bitch until she became big enough to please them. The masters love their pets so much, you'll see.</i>\"", false);
            else if (this.effects.getValue1Of(StatusAffects.Vala) == 4) outputText("\"<i>Thoughts are so hard. Much easier to be a toy slut. Won't you like being a toy? All that nasty memory fucked out of your head.</i>\"", false);
            else if (this.effects.getValue1Of(StatusAffects.Vala) == 5) outputText("\"<i>Bitch has given birth to many of the masters' children. She will teach you to please the masters. Maybe you can birth more masters for us to fuck?</i>\"", false);
    ...
    public constructor() {
        ...
        // this.plural = false;
        this.vaginas.createVagina(false, VAGINA_WETNESS_SLICK, VAGINA_LOOSENESS_GAPING_WIDE);
        ...
        let lust: number = 30 + game.flags[kFLAGS.TIMES_PC_DEFEATED_VALA] * 10;
        if (lust > 80) lust = 80;
        this.bonusHP = 350;
        this.lust = lust;
