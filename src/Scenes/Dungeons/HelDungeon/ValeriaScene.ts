...
export function gooArmorBeatsUpPC(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.breastRows.biggestTitSize() > 1 && player.lactationQ() > 0) outputText("and milk ");
...
export function takeGooArmorAndWearIt(): void {
    ...
    // (PC regains HP)
    Inventory.takeItem(player.setArmor(ArmorLib.GOOARMR), playerMenu);
    // ArmorLib.GOOARMR.equip(player,true,false);
