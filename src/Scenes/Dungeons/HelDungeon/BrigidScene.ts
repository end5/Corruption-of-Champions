...
function brigitPostQueenDeathBadEndII(): void {
    ...
    if (player.gender <= 1 || player.pregnancyIncubation > 0) {
        ...
        if (player.cocks.length > 1 || player.cumQ() > 1000) outputText("  " + SMultiCockDesc(game.player) + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
    ...
    // (else if not visibly pregnant F or H)
        ...
        // hymen check]
        cuntChange(player, 15, false, false, false);
        ...
        if (player.cocks.length > 0) outputText(sMultiCockDesc(game.player) + " bobbing up and down, drooling pre-cum, as ");
        ...
        if (player.cocks.length > 0) outputText(" as the phoenix's cunt slides onto your [cock smallest]");
