...
export class BrigidScene {
    ...
    private brigitPostQueenDeathBadEndII(): void {
        ...
        if (player.gender <= 1 || player.pregnancyIncubation > 0) {
            ...
            if (player.cockTotal() > 1 || player.cumQ() > 1000) outputText("  " + SMultiCockDesc(game.player) + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
        ...
        // (else if not visibly pregnant F or H)
            ...
            if (player.hasCock()) outputText(sMultiCockDesc(game.player) + " bobbing up and down, drooling pre-cum, as ");
