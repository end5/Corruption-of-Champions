...
const CENTAUR_KILLED: number = 1;
const CENTAUR_RELEASED: number = 2;
const CENTAUR_FUCKED: number = 3;
...
const HORZGOG: number = 0;
const DOGGECOCK: number = 1;
const TENTACOCK: number = 2;
...
function femFuckHerII(cockType: number): void {
    ...
    if (cockType == TENTACOCK) {
        ...
        outputText("\n\nYou nervelessly slide off the pole into the ground, earning a fresh bukkake of creamy cum to plaster your " + skinFurScales(player) + ".  While you recover, your demonic lover's bulk shifts away, and you soon here the telltale clopping of her moving away.  It sounds like she's heading towards the south - the exit.  You missed your chance to do anything about that demon...");
    }
...
function maleLoss(hpVictory: boolean): void {
    ...
    outputText(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got so far thinking you were some kind of " + mf(player, "stud", "queen bitch") + ".</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
    if (player.tallness >= 7 * 12) outputText(", even though your [feet] still touch the ground");
    ...
    outputText("\n\nSlurping hard, you lick and suck the dick in your throat, so busy tending to the omnibus that you barely the notice the throbbing ache in your loins.  You cast lusty, inviting eyes around at every feminine form you see during journey to the stables, located in some backwater, seemingly forgotten part of the fortress.  You quiver with need and explode in passion, moaning into the Omnibus's cock as your " + cockDescript(game.player, y) + " spends its lust into the oozing horse-pussy, wasting your champion-seed to father nothing more than a batch of shiftless imps.");
