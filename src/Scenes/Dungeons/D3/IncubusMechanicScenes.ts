...
const MECHNIC_NO_SELECTION: number = 0;
const MECHANIC_PAID: number = 1;
const MECHANIC_SUCKED: number = 2;
const MECHANIC_FOUGHT: number = 3;

const MECHANIC_KILLED: number = 1;
const MECHANIC_RELEASED: number = 2;
const MECHANIC_FUCKED: number = 3;
const MECHANIC_FUCKED_YOU: number = 4;
...
export function meetAtElevator(): void {
    if (flags[kFLAGS.D3_MECHANIC_LAST_GREET] == MECHNIC_NO_SELECTION) {
        ...
        if (player.effects.findByType(StatusAffects.IncubusBribed) >= 0) outputText(", a rolled-up hentai magazine tucked neatly into the chest-pocket of his overalls");
...
const MECHANIC_DEMON_COCK: number = 0;
const MECHANIC_HORZ_GOG: number = 1;
const MECHANIC_DOG_COCK: number = 2;
...
function rideMechanicsCockII(cType: number): void {
    ...
    if (player.effects.findByType(StatusAffects.IncubusBribed) >= 0) outputText(" Nothing but hentai mags for you to drain your balls with....");
    outputText("</i>\"");
    ...
    else outputText(" When you rise, it amazes you just how many inches of cock are sliding out of your well-used snatch. The " + cockHead(monster) + " eventually slips free, and you watch in awe at how his musky semen runs out from your gaped, thoroughly creamed lips. By the time you dress, he's vanished, but you recall hearing him panting something about seeing you at camp.");
...
function maleLossToMechanic(hpVictory: boolean): void {
    ...
    if (player.cocks.length > 1) {
        ...
        let allDemon: boolean = false;
        if (player.cocks.demonCocks() == player.cocks.length) allDemon = true;
...
function errybodyelseLossToMechanic(hpVictory: boolean): void {
    ...
    // Genderless HP Loss
        ...
        if (player.breastRows.hasFuckableNipples()) outputText(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
        else {
            outputText(" pinching and tugging on");
            if (player.breastRows.totalNipples() > player.breastRows.totalBreasts()) outputText(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
