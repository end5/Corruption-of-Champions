...
function exportSaveData(): void {
    ...
    let foundFreeFile: boolean = false;
    const baseFileName: string = "CoC_EndExport_";
    let currFileName: string;
    let attemptNum: number = 1;

    while (!foundFreeFile) {
        currFileName = baseFileName + String(attemptNum);
        const sharedObject: SharedObject = SharedObject.getLocal(currFileName, "/");
...
function goFight(): void {
    ...
    if (player.wingType != 0) {
        m.effects.create(StatusAffects.PhysicalDisabled, 0, 0, 0, 0);
        m.effects.create(StatusAffects.AttackDisabled, 0, 0, 0, 0);
    }
...
export function defeated(hpVictory: boolean): void {
    ...
    if (hasLethicite && player.effects.findByType(StatusAffects.MetMarae) >= 0 && player.effects.findByType(StatusAffects.FactoryOverload) < 0) {
...
function queenHer(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 2) outputText(" cupping a tit");
...
function woofwoof(): void {
    ...
    if (player.effects.findByType(StatusAffects.DungeonShutDown) >= 0) outputText("Your predecessor destroyed our welcoming facility. Otherwise I’m sure a certain imp would have revealed the truth to you long ago, before joining you with our breeding machines.");
    else {
...
function HowFuckedIsThePlayerOMeter(): number {
    ...
    // Marae corrupt +50
    if (player.effects.findByType(StatusAffects.FactoryOverload) >= 0) levelOfFuckedness += 50;
    ...
    if (player.effects.findByType(StatusAffects.WandererDemon) >= 0) levelOfFuckedness += 10;
    ...
    if (player.effects.findByType(StatusAffects.CampRathazul) >= 0) levelOfFuckedness -= 5;
...
function purgeShit(): void {
    ...
    const levelOfFuckedness: number = HowFuckedIsThePlayerOMeter();
    ...
    if (levelOfFuckedness <= 50) {
        ...
        if (player.effects.findByType(StatusAffects.FactoryOverload) < 0) outputText(" Marae reclaimed the full network of her roots and single-handedly purged the last holdouts of corruption from the lands, choking it out of her world like weeds from a garden.");
        ...
        outputText("\n\n");
        if (player.effects.findByType(StatusAffects.FactoryOverload) < 0) outputText("Marae is a wonderful aid in your quest, always able to manifest in her newly purified roots and offer words of wisdom or encouragement. Her magical might is a rock you can lean upon when times are tough. You aren’t sure how long you could keep this up without her.");
        ...
        if (player.effects.findByType(StatusAffects.FactoryOverload) < 0) outputText(" Those few pockets that escaped your armies could not escape Marae’s all-encompassing roots. Mareth is purified once more.");
