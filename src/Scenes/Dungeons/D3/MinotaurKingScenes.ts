...
function mechanicalbullhue(): void {
    ...
    if (player.lust >= 80) outputText(" - not that you needed much help -");
    ...
    if (player.isLactating()) outputText(", tricking your full nipples into spraying their milk everywhere");
...
function titfuckCowslut(): void {
    ...
    if (player.cumQ() <= 500) outputText(" Most fall short, but one daring squirt manages to coat Excellia’s tongue.");
    else if (player.cumQ() <= 1500) outputText(" Most manage to fall on Excellia’s tongue, painting it a nearly white save for a few off-color splotches. The rest puddles in the nape of her neck, giving you time to enjoy the cow-print tongue you’ve created.");
    else {
        ...
        if (player.cumQ() >= 5000) outputText(" And her breasts? They’re basically white save for their peaks.");
    }
...
function sloppySeconds(): void {
    ...
    else if (player.cocks.length > 2) outputText(" Your other penises are enjoying sliding between her perspiring thighs nearly as much.");
    ...
    if (player.hasKnot()) outputText(" There’s not much point in it; your knot does that much and more, trapping your load inside as effectively as the remainder of the Minotaur King’s.");
    outputText(" Your");
...
export function hailToTheKingBaby(hpVictory: boolean, pcCameWorms: boolean): void {
    ...
    outputText("\n\n<i>“Atta " + mf(player, "boy", "girl") + ",”</i> the triumphant beast-man croons. <i>“This is where you belong.”</i>");
    ...
    if (player.perks.findByType(PerkLib.BimboBrains) >= 0 || player.cor >= 66) outputText(" You’re long past the point of having a gag reflex and glad for it.");
    else outputText(" Miraculously, your gag reflex is completely absent. Maybe it’s too drenched in his heavenly pre to protest?");
    ...
    if (player.breastRows.biggestTitSize() >= 5) outputText(", jiggling your tits wildly");
    ...
    else outputText(", shaking your whole body with the force of his passion");
...
function leavethem(): void {
    ...
    if (player.perks.findByType(PerkLib.BimboBrains) >= 0) outputText("Ummm, like, I’m not </i>that<i> horny. Besides, I can fuck all I want once I take you out, cutie!");
