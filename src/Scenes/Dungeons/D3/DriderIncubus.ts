...
export class DriderIncubus extends AbstractSpiderMorph {
    ...
    private performPhysicalAttack(): void {
        if (_combatRound >= 3 && (_combatRound % 6 == 0 || _combatRound == 3)) stunningSpear();
            ...
            const opts: any[] = [bite, spiderMorphWebAttack, kick, kick, doubleStrike, doubleStrike];
    ...
    private goblinAI(): void {
        const opts: any[] = [goblinHandjob, goblinTongueGrapple, bootyTwerking, webNipplechain];
    ...
    private spearStrike(): void {
        ...
        const damage: number = (str + weaponAttack) * 0.40;
    ...
    private bite(): void {
        ...
        if (player.effects.findByType(StatusAffects.Stunned) >= 0 || (player.spe <= 1 && player.effects.findByType(StatusAffects.Web) >= 2)) {
            ...
            // Dodge
            if (combatMisdirect()) outputText(" You misdirect his venomous strike!");
            else if (combatEvade()) outputText(" You evade his venomous strike!");
            else if (combatMiss() || combatFlexibility()) outputText(" You avoid his venomous strike!");
            else {
                // Hits
    ...
    private kick(): void {
        ...
        else if (combatMiss() || combatEvade() || combatFlexibility()) {
            outputText(" You twist out of the way at the last moment, evading with ease.");
        }
        else {
            let damage: number = (str + weaponAttack + 25) - rand(player.tou);
    ...
    private doubleStrike(): void {
        ...
        const damage: number = str + weaponAttack + 10 - rand(player.tou);
