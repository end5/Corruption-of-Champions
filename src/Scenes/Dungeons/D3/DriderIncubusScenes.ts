...
export function spooderbuttGetsANewCockSleeve(hpVictory: boolean, pcCameWorms: boolean): void {
    ...
    if (hpVictory) outputText(" breath through such a thick covering.");
    else outputText(" pant needily under such a thick covering.");
...
export function spooderbuttGetsANewCockSleeveII(): void {
    ...
    outputText("\n\n<i>“I must say, I’m surprised, Kinariel. I did not think you’d manage to vanquish such a troublemaker all by your lonesome.”</i> Long-nailed fingers dig into your chin and twist your head around for a better view. <i>“And I’m disappointed. I had hoped you would prove a decent challenge. It has been so long since I’ve gotten to let loose in a fight.”</i> She tuts at you. <i>“Such a shame. Still, you have the aura of a breeder about you. You may yet serve me. Kinariel, put " + mf(player, "him", "her") + " down.”</i>");
...
export function spooderbuttGetsANewCockSleeveIII(): void {
    ...
    if (humanScore(player) < 5) outputText(", no matter how mutated");
    ...
    outputText(".");
    if (player.effects.findByType(StatusAffects.FactoryOverload) >= 0) outputText(" You couldn’t even save one of the last pure spirits in this place, Marae.");
    ...
    if (player.lust >= 90) outputText(" and horny");
    outputText(", but unwilling to admit as much to your long-time foe.");
    outputText("\n\n<i>“Good " + mf(player, "boy", "girl") + ".”</i> Lethice strokes your hair, ignoring the tears at the corners of your eyes. <i>“You deserve a reward I rarely give.”</i> She breaks your gaze, allowing you to watch her lift long black skirt, almost like something a nun would wear, if a nun had holes cut in the top to display her rigid, pierced nipples. Higher and higher, the fabric ascends up her perfectly smooth legs until a dewy, pink slit is revealed. <i>“You will be permitted to lick me as a free " + mf(player, "man", "woman") + ", before you go in the submission tanks.”</i>");
    ...
    outputText("\n\n<i>“If you do a good job, I’ll instruct them to maintain as much of your personality as possible.”</i> Lethice’s voice rings with as much truth as it does authority. <i>“Displease me and you shall be an empty-headed husk, fit only for reproduction.”</i> She giggles, <i>“But there’s no way a </i>good " + mf(player, "boy", "girl") + "<i> like you would fail to please, is there?”</i>");
...
export function beatTheSpooderbutt(hpVictory: boolean): void {
    ...
    // Lust
        ...
        if (!m.goblinFree) outputText(", but he pushes the goblin off his cock all the same");
    ...
    if (player.cocks.length > 0) {
        ...
        if (m.goblinOiled) outputText(" oiled-up");
...
export function fuckMitzi(): void {
    ...
    if (player.cocks.length > 1) {
        ...
        if (player.cocks.length == 2) outputText(" second");
        else outputText(" smallest");
    ...
    if (player.cocks.biggestCockArea() > 100) outputText(", her throat bulging with its unholy size");
    ...
    // Hugedix
    if (player.cocks.biggestCockArea() > 250) {
    ...
    // Back to errybody
    outputText("\n\n");
    if (player.cocks.biggestCockArea() <= 250) outputText("Bottoming out");
...
export function titfuckMitziSlow(): void {
    ...
    if (player.cocks.biggestCockArea() <= 20) outputText(", tiny");
    outputText(" tower of meat");
    if (player.cocks.biggestCockArea() <= 20) outputText(", nearly obscuring the whole thing");
    else if (player.cocks.biggestCockArea() >= 100) outputText(", barely able to support it");
    outputText(".");
    if (player.cocks.length > 1) {
    ...
    if (player.cocks.biggestCockArea() >= 100) outputText(" Sometimes she even plants a few more kisses on your [cockHead biggest] or just below. Other times she licks you like a lollipop - at least the parts not buried in bust.");
...
export function actuallyRideDriderDick(): void {
    ...
    if (player.clitLength > 3 || player.cocks.length > 0) outputText(" one of");
    outputText(" his nipples");
    if (player.clitLength > 3 || player.cocks.length > 0) {
        outputText(" while still trying to");
        if (player.clitLength > 3) outputText(" pump your [clit]");
...
export function buttfuckDrider(): void {
    ...
    if (player.cocks.biggestCockArea() >= 1200) outputText(" You feel like your mammoth member is going to tear him apart at this rate! The drider’s mouth twists into a silent ring of shock, unable to give voice to his reaction as you bury yourself inside him.");
    else if (player.cocks.biggestCockArea() >= 500) outputText(" Your animalistically-proportioned prick stretches the virginal passage to its limit, making the drider roar in a mix of surprise and unexpected pleasure.");
    else outputText(" Your prick sinks deep into the drider’s ass, gliding through his virginal hole.");
