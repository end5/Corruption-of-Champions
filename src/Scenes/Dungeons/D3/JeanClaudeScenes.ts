...
export function gogoFuckTheseBasilisksNorth(): void {
    clearOutput();
...
export function gogoFuckTheseBasilisks(): void {
    clearOutput();
    ...
    if (demonScore(player) > 4) outputText("  You can’t be mistaken for anything but a demon, after all.");
...
export function capturedByBasilisks(): void {
    clearOutput();
...
function capturedByBasilisksRun(): void {
    clearOutput();
...
function capturedbyBasilisksTaunt(): void {
    clearOutput();
...
export function beatDaFuckhugeBasilisk(hpVictory: boolean): void {
    clearOutput();
...
export function fuckhugeBasiliskFuckedYouUp(hpVictory: boolean): void {
    clearOutput();
...
export function basiliskBadEnds(): void {
    if (player.perks.findByType(PerkLib.BasiliskWomb) >= 0) bwombBadEnd();
    else regularBadEnd();
}

function regularBadEnd(): void {
    clearOutput();
    ...
    if (player.skinAdj != "smooth" || player.skinAdj != "latex" || player.skinAdj != "rubber") {
        ...
        if (player.skinType == SkinType.FUR) outputText("  Your fur drops away in ragged clumps as you drink.");
...
function bwombBadEnd(): void {
    ...
    if (player.cocks.length === 0) outputText("  His other dick bats into your stomach, thrusting upwards towards your [chest] as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.");
