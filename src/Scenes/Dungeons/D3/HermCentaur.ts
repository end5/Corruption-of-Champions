...
export class HermCentaur extends Monster {
    ...
    protected performCombatAction(): void {
        ...
        if (_chargingArouse) arouseSpellCast();
            ...
            // Selections
            const opts: any[] = [feminineMusk, aphrodisiacSquirt];
    ...
    private feminineMusk(): void {
        ...
        dynStats("lus+", 8 + (player.lib / 10) + (player.sens / 10));
    }
    ...
    private arouseSpellCast(): void {
        ...
        if (this.lust >= this._lustAtChargeStart + 10) {
            ...
            dynStats("lus+", 20 + (player.lib / 6) + (player.sens / 6));
        }
    }
    ...
    private hypnoCock(): void {
        ...
        if (player.inte * (2 / _hypnoCockUses) > rand((player.lib / 3) + (player.sens / 3) + (player.cor / 3))) {
            ...
            dynStats("lus+", 20 + 2 * _hypnoCockUses + 2 + rand((player.lib / 10) + (player.sens / 10)));

            outputText("Down it bobs, slowly hanging lower and lower... SMACK!  Up it goes, taking your bedazzled eyes along for the ride.  \"<i>That's a good " + mf(player, "boy", "girl") + ",</i>\" the dick's director whispers, \"<i>Just follow the tempo and let it fill your mind, oozing inside you with each thump.</i>\"");
