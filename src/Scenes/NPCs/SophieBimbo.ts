...
*/
export function sophieSprite(): void {
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00282] > 0 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00283] == 0) spriteSelect(83);
    else spriteSelect(53);
}

export function bimboSophie(): boolean {
    return (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00282] > 0 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00283] == 0 && flags[kFLAGS.SOPHIE_DEBIMBOED] == 0);
}

export function sophieIsInSeason(): boolean {
    return (flags[kFLAGS.SOPHIE_HEAT_COUNTER] > 551 && !SophieScene.pregnancy.isPregnant);
}

export function sophiePregChance(): void {
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] != 0) return;

    // 25% + gradually increasing cumQ bonus
    if (rand(4) == 0 || player.cumQ() > rand(1000)) {
        SophieScene.pregnancy.knockUpForce(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.INCUBATION_HARPY);
    }
}
export function sophieCapacity(): number {
    return 179;
}

// *Summary:  Sophie is defeated, give draft.
export function bimbotizeMeCaptainSophie(): void {
    sophieSprite();
    outputText("", true);
    ...
    if (monster.lust > 99) outputText("lust", false);
    ...
    outputText("After a few moments of torrential, slimy output, Sophie's quivering cunt spasms and her legs give out.  She slumps over, ass in the air and utterly exposed as she giggles, \"<i>Like, wow!  You made me cum so hard, " + mf(player, "mister", "miss") + "!  Can I, like, come home with you?  I don't really want to compete with these bitches and you're super " + mf(player, "handsome", "hot") + "!  We can have lots of like, sex and stuff!</i>\"\n\n", false);
...
function acceptBimboSophie(): void {
    sophieSprite();
    outputText("", true);
    ...
    if (SophieScene.pregnancy.isPregnant) SophieScene.pregnancy.knockUpForce(SophieScene.pregnancy.type, int(SophieScene.pregnancy.incubation * 1.75));
    outputText("You nod, and Sophie springs forward on her powerful legs, the strength undiminished by the effects of her bimbo transformation.  She tackles you in a bear-hug, kissing your " + chestDesc(game.player) + " over and over as her momentum carries you both off the side of the mountain.  You tumble in the air, screaming as you fall", false);
    ...
    outputText("You grudgingly direct her glide towards your campsite, and the two of you make good time.  Of course, Sophie's wet, drooling cunny makes an absolute mess of your " + player.armorName + " and " + legs(player) + " during the trip, soaking them with her copious pussy-cream.  She's exhausted and randy once you get to camp, and the landing is quite rough as a result.  You take a nasty tumble in the dirt, but it doesn't damage you significantly.  Sophie appears just as lucky - she's sitting on her plump rump with her legs spread wide, cluelessly scratching at her downy blond feathers.\n\n", false);
...
export function sophieCampLines(): void {
...
export function approachBimboSophieInCamp(output: boolean = true): void {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0) {
        if (output) {
            ...
            else if (SophieScene.pregnancy.isPregnant) {
                switch (SophieScene.pregnancy.event) {
                    case 3: // Medium Bump*
    ...
    if (player.inv.hasItem(ConsumableLib.DEBIMBO)) {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0 && !SophieScene.pregnancy.isPregnant && flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1) addButton(1, "Farm Work", sendToFarm);
...
// Appearance:
function sophieBimboAppearance(): void {
    clearOutput();
...
function bimboSophieSexMenu(): void {
    clearOutput();
    outputText("How will you have sex with your feather-brained slut?");
    menu();
    if (player.cocks.length > 0) {
        // BUTT FUCKS
        if (player.cocks.cockThatFits(sophieCapacity()) >= 0 && player.lust >= 33) addButton(2, "Anal", buttFuckBimboSophie);
        else outputText("\n\n<b>You're too big to butt-fuck her.</b>", false);
        // TIT FUCKS!
        if (SophieScene.pregnancy.event >= 2) addButton(5, "Titty-Fuck", sophiePreggoTitJobs);
        else addButton(5, "Titty-Fuck", fuckBimboSophiesTits);

        // VAG FUCKS!
        if (player.cocks.cockThatFits(sophieCapacity()) >= 0) {
            if (sophieIsInSeason()) {
        ...
        if (player.cocks.biggestCockArea() > sophieCapacity() && sophieIsInSeason()) {
    ...
    if (flags[kFLAGS.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR] > 0 && (player.gender > 0) && !SophieScene.pregnancy.isPregnant) {
        if (player.inv.hasItem(ConsumableLib.OVIELIX) || Inventory.inv.hasItemInStorage(ConsumableLib.OVIELIX)) {
...
function tellSophieToVisitIzma(): void {
    clearOutput();
...
export function bimboSophieLustStickSurprise(): void {
    outputText("\n", false);
    ...
    if (select == 0) {
        ...
        sophieSprite();
        if (player.cocks.length > 0) {
            outputText("While standing there, drinking a sip of water, strong hands pull down your " + player.armorName + ".  Plump lips wrap around " + oMultiCockDesc(game.player) + " and smother it in plush, warm slipperiness.  You look down at your bimbo harpy and sigh while " + oMultiCockDesc(game.player) + " grows harder in her lips.  She pops off and asks, \"<i>Like, wanna fuck?</i>\"  You sigh and try to resist the images of yourself mounting the harpy that flood your mind.  It might be time to fuck her soon...", false);
            dynStats("lus", 15);
        }
        else {
            dynStats("lus", 10);
        ...
        if (player.cocks.length > 0) {
            dynStats("lus", 15);
            outputText(SMultiCockDesc(game.player) + " seems to wiggle inside your " + player.armorName + ".  Damn, she feels really good on you!  Maybe you should screw the slutty bimbo soon?  ", false);
        }
...
export function daughterCheckup(): void {
    clearOutput();
...
// Unlock resistance:
export function unlockResistance(): void {
    if (player.cocks.length > 0) {
...
// Get oral as female.
function bimboSophieLicksRugs(): void {
    sophieSprite();
    outputText("", true);
    // *Summary - lean back on a rock and spread your legs.  She licks you off with chance of compelling you to sixtynine her.
    outputText("You sit back on a nearby rock and spread your " + legs(player), false);
    if (player.balls > 0) outputText(", lifting your heavy sack out of the way", false);
    outputText(" to give Sophie a perfect view of your " + vaginaDescript(player) + ".  Sophie's dull, somewhat glazed eyes brighten at the sight of your box, and she leans forward intently, dipping a digit into herself as she asks, \"<i>Ooh, can I, like, lick it for you?  It looks soooo yummy!</i>\"\n\n", false);
    ...
    // (ginormous clitties)
        ...
        outputText("You shudder as the harpy resumes her oral onslaught, slurping your " + clitDescription(player) + " deep into her throat in one smooth plunge.  She hums in delight, licking over the feminine fuck-stick's lower portions while she tries to get her fat, soft puss-pillows closer to your juicy snatch.  You rock your hips forward to thrust your " + clitDescription(player) + " further inside your bimbo's neckhole.  She gurgles happily and lashes at your " + vaginaDescript(player) + ", her tongue tracing trails of hot, sexual pleasure through your depths as her molten-hot throat milks you like no tomorrow.\n\n", false);
    }
    ...
    if (player.cocks.length > 0) outputText(", or thick, juicy dick", false);
    ...
    if (player.cocks.length > 0) outputText("and hard ", false);
...
function bimboSophieSixtyNineAfterCunnilingus(): void {
    sophieSprite();
    outputText("", true);
    // NON WANG
    if (player.cocks.length === 0 || player.cocks.cockThatFits(sophieCapacity()) == -1) {
        ...
        outputText("At long last, the bimbo's bald cunt climaxes from your work, splattering you with a flood of musky girl-spunk.  You still need her pussy so bad that you can't stop.  You lick, lap, and swallow with inhuman eagerness until the box atop you is emptied of everything save your saliva.  Then, and only then, do you finally feel release.  Your " + vaginaDescript(player) + " climaxes for Sophie, soaking her fingers and wringing them as if your life depended on it.  She giggles, coming down from her own orgasm and feeling quite pleased with the effects her words have had on you.\n\n", false);

        outputText("The harpy gets up, but only long enough to turn around and lay atop you, face to face");
        if (SophieScene.pregnancy.event >= 2) outputText(", egg-swollen belly smushing between you");
    ...
    // SixtyNine Continuation - DUDES
        ...
        outputText("You throw yourself at Sophie, compelled by thoughts of pleasure and her delicious-looking pussy.  Imagining yourself placing your lips on her cunt and slurping with reckless abandon, you tackle her, hard.  She takes the hit like a champ, tumbling with you through the dirt and dust until you land on your " + buttDescription(player) + ", bouncing to a stop with the harpy's feathery thighs clenching around your head.  ");
        if (SophieScene.pregnancy.event >= 2) outputText("Her gravid belly bumps up against your own, somehow untouched by the rough-and-tumble fall.  ");
        outputText("How did she do that?  Your questions fade away as you look up and spot your target - the cum-slicked delta of Sophie's cunt, so moist it seems to fog the air, beckoning you to plunge your tongue inside and taste it.\n\n", false);
        ...
        if (SophieScene.pregnancy.event >= 2) outputText(" and the way her egg-stuffed womb jiggles");
...
function buttFuckBimboSophie(): void {
    sophieSprite();
    outputText("", true);
    let x: number = player.cocks.cockThatFits(sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    ...
    if (player.cocks.length > 1) outputText("Those cocks look", false);
    else outputText("That cock looks", false);
    outputText(" delic... de... yummy!</i>\"  You slap her ass, grab her tail feathers, and pull the harpy's winking asshole down onto your " + cockDescript(game.player, x) + ". Her voices rises, not in anger, but in delight, and your " + cockHead(player, x) + " pulses pleasantly in the first few inches of the harpy's rectum.  The bimbo relaxes and sinks down, giggling and asking, \"<i>Is-ooh-is my butt, like, good for you?  It's soooo tight, and it's making my cunny all wet and stuff!</i>\"\n\n", false);

    outputText("You squeeze her shuddering rump hard enough to feel her flesh bulging out around your fingers, the soft cheeks nearly subsuming your hands, and then you pull her hard, dragging your " + cockDescript(game.player, x) + " the rest of the way inside her, all the way to the ", false);
    if (!player.cocks.hasSheath()) outputText("hilt", false);
    ...
    if (SophieScene.pregnancy.event >= 2) outputText("  Just like her breasts, her gravid tummy sways a little as well, plush with the weight of the egg you've given her.");
    ...
    if (player.balls > 0) outputText("over your " + ballsDescriptLight(player), false);
    ...
    outputText("\"<i>So... good.  Why didn't anyone tell me that butt-sex is like, super-duper fun?</i>\"  The breathless bimbo rocks her ass back and forth, and you let her have her 'fun'.  She's wiggling and squirming, impaled on a cock and loving it.  You simply relax and let her go to town, the bimbo's fingers running through her sodden box in time with the butt-jiggling contortions of her body.  Her anus milks your " + cockDescript(game.player, x) + " like a machine, drawing out thick streamers of pre-cum to make her ass almost as sloppy as her pussy.  It's hard to stand still with her working you so efficiently, and after a few moments of allowing your bimbo to pleasure you with her plump rump, you tire of waiting.\n\n", false);

    outputText("Pushing her away, you unsheath most of your " + num2Text(Math.round(player.cocks[x].cockLength)) + " inches from Sophie's stretched asshole, leaving just your " + cockHead(player, x) + " to hold the rectal portal open.  She whines in disappointment until you lunge forward and stuff her full of dick so suddenly that she squirts dribbles of lube.  You grab her hair with one hand and ride her ass hard and fast, each thrust punctuated by happy squeals of submissive pleasure from your airheaded anal toy.  Sophie cheers, \"<i>Fuck me " + mf(player, "stud", "babe") + ", fuck my ass!  Unh, y-yeah like that.  Mmmm, yeah, pull my hair and tell me what a bad girl I've been!</i>\"\n\n", false);
    ...
    outputText("You keep butt-fucking her hard and fast, ignoring the sprinkles of sticky, maternal harpy-juice that splatter on your " + legs(player) + " as you fuck her.  In between throaty moans and screams, she bellows out \"<i>YES!</i>\" as loudly as possible, then lapses into quiet, exhausted grunts.  Yet, her vagina does not stop its prodigious leaking; if anything it's dripping and squirting out even heavier flows of submissive girl-spooge.  The harpie's anal ring is convulsing around you, reduced to pointless trembling from the hard fucking you're giving her.  Looking back with lidded eyes, she begs, \"<i>C-cum... cum in my ass... it'll... it'll feel soo good, babe.  Please?  Like, gimme your spooge!</i>\"\n\n", false);
    ...
    outputText("  Seed spews from your " + cockDescript(game.player, x) + " into your bimbo's blubbery bottom with an audible gurgle that brings a blush to her cheek.  You pull her hair harder and bounce your " + hipDescription(player) + " off her bottom a few times, injecting the rest of your cum inside her while you rut at her now-sloppy anus.", false);
    if (player.cumQ() >= 500) {
    ...
    outputText("  With a few final clenches of pleasure, your dick's flow of cum trickles down to a dribble.  You sigh and muse that you must've completely emptied your " + ballsDescriptLight(player) + " by now, and you slump back against a rock to watch your jizz flood out of Sophie's ass.  She pushes some inside her pink nether-lips with a triumphant smile, clearly pleased with herself.\n\n", false);
    ...
    if (SophieScene.pregnancy.event >= 2) {
...
// TitFuck
function fuckBimboSophiesTits(): void {
    sophieSprite();
    outputText("", true);
    let x: number = player.cocks.cockThatFits(sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    ...
    if (SophieScene.pregnancy.event >= 2) outputText("Her swollen tummy pushes against your [legs] as she assumes the position, the weighty egg-distorted lump pressing against you.  ");
    outputText("She wobbles forward and her massive mounds easily slide to either side of " + oMultiCockDesc(game.player) + ", completely engulfing it.  Still holding on to her nipples, you pull them together and ", false);
    ...
    outputText(" in warm, smooth skin.  The smell of your " + cockDescript(game.player, x) + " mere inches from her nose seems to startle Sophie from her trance, and she breaks into a smile so big you'll worry it'll break her face.\n\n", false);
    ...
    outputText("Suddenly, Sophie's puckered lips are on your " + cockHead(player, x) + ", sucking it softly while her tongue tickles the sensitive slit of your urethra.  She pops off for a breath with a lusty look in her eyes and spit drooling from the corner of her puffy lips to lubricate her tits.  A golden lip print sits around the circumference of your " + cockDescript(game.player, x) + ", marking just how much she's managed to smother in oral pleasure.  Pulling her hands up, she presses her palms into the heavy weight of her breasts, nearly vanishing from view as she drags them up your " + num2Text(Math.round(player.cocks[x].cockLength)) + "-inch length.  Her drugged lipstick makes your " + cockHead(player, x) + " tingle deliciously, and you sigh, happy to have such glorious udders growing from your bleach-blonde bitch.\n\n", false);

    outputText("Full, slippery lips slide back around you again, further staining your " + cockDescript(game.player, x) + " in their amber gloss.  It's enough to make you swoon and flare, growing thicker inside her smothering mounds while she pumps at you with her tits.  Sophie croons in between sucking on your dick-tip, saying, \"<i>Mmm, such a yummy cock!  Let my tits like, milk out all that yummy cum from your cock, okay babe?</i>\"\n\n", false);

    outputText("You demur and start pumping your hips.  The bimbo shakes her platinum-blonde feathers in irritation, but she doesn't dare stop you now.  You grab her tits as well, lifting them out of her grip as you begin to fuck them in earnest, your cock cresting through the massive cleavage with each stroke.  Sophie smiles dopily, rapidly forgetting her unhappiness at being so thoroughly manhandled, and she returns to worshipping your " + cockHead(player, x) + " each time it comes anywhere near her mouth.  Spit and golden lipstick drool down her cleavage, and soon your rod looks like a golden spear of lust, trembling in the pale valleys of your bimbo slut's mountainous peaks.\n\n", false);
    ...
    outputText("One of the dumb blonde's hands circles under her bouncing, jiggling chest to stroke your " + cockDescript(game.player, x) + ", jacking you off with the spit, sweat, and drugged gloss as lubricants.  She jerks you hard and fast, smearing it ", false);
    if (player.balls > 0) outputText("all over your balls whenever she needs a break, and you feel your " + ballsDescriptLight(player) + " growing heavier by the moment.", false);
    ...
    outputText("  " + SMultiCockDesc(game.player) + " thickens and bloats, a rush of warmth rushing up the base as your internal muscles clench tight, squeezing out a globby rope of jism.  It explodes from your cum-slit, taking Sophie full on the face.\n\n", false);

    outputText("You're just getting started.  Sophie smirks and opens her mouth, letting her tongue slide out to taste your jizm as you launch the next rope, so long it trails from her hair to her chin and so thick it rolls in drops from the side of her face.  She hums in delight and squeezes her breasts tight around your " + cockDescript(game.player, x) + ", the next blast backing up inside you from the pressure.  A moment of buildup later, and your seed germinates and blooms over the harpy's tits, soaking them with spunk.  Droplets of the stuff splatter onto the ground around the both of you, but you just keep coming.  In your mind you have visions of yourself knocking up harpy after harpy, the slutty bird-girls mounting your fountaining fuck-stick long enough for their bellies to bloat with seed before moving on.  Like this, you could inseminate a whole tribe...\n\n", false);
    ...
    if (SophieScene.pregnancy.event >= 2) outputText("her swollen midriff is glazed, ");
...
export function sophieEggApocalypse(): void {
    sophieSprite();
    outputText("", true);
    // First time
...
// [Daddy]
function beBimboSophiesSugarDaddy(): void {
    sophieSprite();
    outputText("", true);
    let x: number = player.cocks.cockThatFits(sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    if (player.inv.hasItem(ConsumableLib.OVIELIX)) player.inv.consumeItem(ConsumableLib.OVIELIX);
    else if (Inventory.inv.hasItemInStorage(ConsumableLib.OVIELIX)) Inventory.inv.consumeItemInStorage(ConsumableLib.OVIELIX);
...
// [Mommy]
function beBimboSophiesSugarMommy(): void {
    sophieSprite();
    outputText("", true);
    if (player.inv.hasItem(ConsumableLib.OVIELIX)) player.inv.consumeItem(ConsumableLib.OVIELIX);
    else if (Inventory.inv.hasItemInStorage(ConsumableLib.OVIELIX)) Inventory.inv.consumeItemInStorage(ConsumableLib.OVIELIX);
...
// Butts McGee
// =====================================================
function rideDatSophieVag(nice: boolean = false): void {
    clearOutput();
    sophieSprite();
    let x: number = player.cocks.cockThatFits(sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    ...
    if (player.cocks.length > 1) {
        y = player.cocks.cockThatFits2(sophieCapacity());
        if (y < 0) {
            y = player.cocks.biggestCockIndex();
            if (y == x) y = player.cocks.smallestCockIndex();
        }
    }
    ...
    if (nice) {
        ...
        if (SophieScene.pregnancy.event >= 2) outputText("  Her pregnancy-imbued middle doesn't really help, either.");
        ...
        if (player.cocks.length >= 2) outputText("they grow");
        ...
        outputText("\n\n\"<i>So then,</i>\" you coo, reaching down and grabbing your " + cockDescript(game.player, x) + ".  You give it a few strokes, enjoying the feeling of your cock between your fingers, and then angle it so the tip is rubbing directly against Sophie's gushing cunt, the bird herself begging you with her eyes.  You let go of her other breast to rub a downy-feathered thigh, and she bites her lip as you chuckle.");

        outputText("\n\n\"<i>Ready?</i>\" you ask her, prodding your tip against her entrance.  She opens her mouth to answer - and you shove in all at once, enjoying the way her voice hikes up a couple of octaves as she moans in sheer pleasure.  Her soppy cunt squeezes down on your " + cockDescript(game.player, x) + ", the internal muscles desperately trying to work you, but you give them little room to do so; you instead pull right back out, watching as Sophie's arms - freed from your grip - reach out to try and tug you back in.");
        ...
        outputText("\n\nYour hips turn into a staccato of smacks and slaps as you help Sophie thrust against your constant pounding, her walls trying their very damnedest to milk your " + cockDescript(game.player, x) + " of its seed, and her feathery arms run across your back as she lets out another squawky moan - and then her body seizes up.");
    ...
    if (player.cocks.length == 1) {
        ...
        // {if (cocks = 1) and (cumMedium = true)}
            ...
            if (SophieScene.pregnancy.event >= 2) outputText(" (or at least all over her channel, since her womb is bound up with your egg)");
        ...
        // {if (cocks >= 2) and (cumHigh = true)}
            ...
            if (player.cocks.length == 3) outputText(", along with your third length, spittling and spraying onto her belly.");
            else if (player.cocks.length > 3) outputText(", along with the rest of your cocks, your entire mass of lengths getting your seed all over her.");
    ...
    if (SophieScene.pregnancy.event >= 2) outputText("resting your head on her gravid tummy and smiling to yourself.");
...
export function sophieGoesIntoSeason(): void {
    sophieSprite();
...
export function sophieSeasonExpiration(): void {
    sophieSprite();
...
export function sophieGotKnockedUp(): void {
    sophieSprite();
...
function sophieImpregnationSex(): void {
    let x: number = player.cocks.cockThatFits(sophieCapacity());
    const y: number = player.cocks.cockThatFits2(sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    clearOutput();
    sophieSprite();
    ...
    if (player.cocks.length > 1) outputText("one of your lengths");
    ...
    outputText("\n\nYou're up in a second, and a moment later, you've got the bimbo pushed over on all fours, your " + cockDescript(game.player, x) + " already pressing against her mound");
    if (player.cocks.length > 1) outputText(", the rest of your lipstick-smeared shafts slipping and sliding between the soft, lube-slicked thighs");
    ...
    outputText("\n\nYour hands find their way to just above the flare of the bird-brained girl's waist and begin to pull her down with each piston forward you make, forcing her to meet you halfway.  You pound Sophie's twat like a jackhammer, fluids squirting out with each booty-shaking cunt-stuffing thrust you dish out.  Your " + cockDescript(game.player, x) + " feels so sensitive, so hard that her slit feels like a molten pleasure-sleeve");
    if (player.cocks.length > 1) {
        outputText(", and the extra");
        if (player.cocks.length > 2) outputText("s between her thighs are");
    ...
    outputText("\n\nEvery ounce of strength in your body diverts to stuffing your " + cockDescript(game.player, x) + " as far into Sophie's sodden, drug-inducing pussy as possible, ramming so hard into her that she gives a chirp of half-pleasure, half-pain.  Your dick");
    ...
    if (player.cocks.length > 1) outputText(" over your extra, thigh slapping cock");
    ...
    if (player.cocks.length > 1) outputText("  Ropes of creamy spunk drizzle over the ground and the underside of Sophie's tits, but the lust-drunk harpy doesn't seem to mind any more than you do.");
    ...
    else outputText("running down your " + cockDescript(game.player, y) + " in miniature rivers");
    ...
    outputText("\n\nLicking her lips, Sophie chants, \"<i>Just one more, in my pussy, please?  Please, cum in my pussy again!</i>\"  She goes on like this, asking, begging, and even pleading as she pivots to place her plump, cum-soaked mound back on your " + cockDescript(game.player, x) + ".  The silken vice fits you as snugly as a tailor-made glove.  She immediately begins to bounce it up and down, looking over her shoulder to watch your face as the waves of sensation overpower your resistance, breaking down your conscious thoughts.  You cum promptly, squirting a long rope of pleasure in on top of all the sperm you've already deposited.  She bounces, forcing more of that ecstasy through your " + cockDescript(game.player, x) + ", laboring messily atop you in a display lascivious enough to make a whore blush.  Every upstroke of her velvet walls along your shaft earns her all the cum your accelerated reproductive system can offer up since the last downstroke, weak spurts of little bearing, but still an extra dose of cream for her twat.");

    outputText("\n\nYou fall back, eyes rolling wildly, moaning uncontrollably.  Consciousness for you becomes little more than the feelings tingling through your " + cockDescript(game.player, x) + " and the ensuing orgasms.  At some point, you lose track of time and pass out.");
    sophiePregChance();
    player.orgasm();
    dynStats("lib", 1);
...
function postSophieImpreg(): void {
    clearOutput();
    sophieSprite();
...
function giantCocksImpregnateSophie(): void {
    clearOutput();
    ...
    outputText("\n\nAbruptly, Sophie leans away, flat onto her back.  Your " + cockDescript(game.player, x) + " drops squarely between her tits, the tip just inches from her puckered lips.  Sophie licks them, eyeballing your tremendous dong as you lower yourself.  You can feel your immense penis pleasantly tingling atop the slutty girl's curvy frame, every curve, bit of feathery fluff, and beautiful, flawless patch of skin.  Sophie slurps an over-sized drop of pre-cum up as it rolls out of your urethra and swallows it noisily.  She opens her lips as wide as she can and kisses you right around your spunk-hole.  A surge of lust immediately rockets through you, centered on your sensitive " + cockHead(player, x) + ".  Syrupy pre splatters onto her face, and she wiggles back, her whole body stroking you off right up until your " + cockHead(player, x) + " is nestled snugly against her gold-glittered nether-lips, absorbing more of her lust-inducing chemicals.");
...
function postSophieTooBigImpreg(): void {
    clearOutput();
    sophieSprite();
...
export function sophieMoveInAttempt(): void {
    clearOutput();
    sophieSprite();
...
// Yes*
function yesSophieSleepWith(): void {
    clearOutput();
    sophieSprite();
...
function noSophieSleepWith(): void {
    clearOutput();
    sophieSprite();
...
function moveSophieInRepeat(): void {
    clearOutput();
    sophieSprite();
    outputText("You go up to the feathery bimbo and idly question if she still wants to share your bed with you");
    if (!SophieScene.pregnancy.isPregnant) outputText(" even though she's not pregnant any more");
...
export function sophieFenCraftedSex(morning: boolean = false): void {
    let x: number = player.cocks.cockThatFits(sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    sophieSprite();
    // {Morning Sex}
    if (morning) {
        outputText("\nOoooh, that feels good.  There's a slippery, warm snugness wrapping around your " + cockDescript(game.player, x) + ", accompanied by a quick, 'schluck-schluck-schluck' in steady beat.  Your heart beats faster as your ardor rises, matched lock step with your rising erection");
        if (player.cocks.length > 1) outputText("s");
    ...
    if (player.breastRows.biggestTitSize() >= 1) outputText("own");
    ...
    if (SophieScene.pregnancy.isPregnant) {
...
export function fuckYoPregnantHarpyWaifu(morning: boolean = false): void {
    ...
    // REGULAR FUCK
        ...
        clearOutput();
        sophieSprite();
        outputText("You go ahead and take off your [armor], letting Sophie take in the view of [eachCock].  She immediately flounces over and drops to her knees in front of you.  Her hands wraps around " + cockDescript(game.player, x) + ", ");
    ...
    outputText(" proving the veracity of her statements many times over.  You can't SEE the wetness, but your " + cockDescript(game.player, x) + " certainly feels it, twitching inside Sophie's silky slit when the waves of wetness roll over it.");
...
// Alternate Intro/Outro for consensual fucking when it isn't morning?
function sophiePreggoTitJobs(): void {
    const x: number = player.cocks.biggestCockIndex();
    clearOutput();
    sophieSprite();
    ...
    outputText("\n\nSuddenly, the amber nubs are withdrawn, and you stagger dizzily for a moment, realizing you hadn't been breathing this whole time.  Sophie drops to her knees in front of you, supporting you by your [butt].  Her hands squeeze you affectionately as she kisses against your " + cockHead(player, x) + ".");
    if (player.cocks.length > 1) {
        outputText("  Then, she fishes the ");
        if (player.cocks.length == 2) outputText("other one");
    ...
    else outputText("your " + cockHead(player, x) + "");
    outputText(" with her lips, already pursed into a plump, cock-sucking doughnut hole of pleasure.  The noise she makes when your " + cockDescript(game.player, x) + " ");
...
// Hog Wild*
function goHogWildOnSophieBewbs(): void {
    clearOutput();
    sophieSprite();
    const x: number = player.cocks.biggestCockIndex();
    outputText("You don't care about her promises- there's a pair of soft tits right there just waiting for a dick to fuck them, and nothing is going to stop you from spooging all over them however you want!  Sophie barely has a chance to react before you're tossing her on her back and mounting her chest.  Your [butt] rubs on the top of her pregnant belly as you settle your " + cockDescript(game.player, x) + " into the perfect position.");
    if (player.cocks.length > 1) {
        outputText("  Your other cock");
        if (player.cocks.length > 2) outputText("s splay");
    ...
    if (player.cocks[x].cockLength <= 30) outputText(cockHead(player, x) + " pushes into through her startled cock-pillows, muffling her more effectively than any gag.  She immediately sucks on it, a happy look in her eyes.  The bimbo seems to want nothing more than to lie there and polish your rod.");
    else outputText(" immense rod smacks across her face, too long to try to jam down her throat.  It completely covers her face, smothering her features with rigid, veiny dickflesh.  She immediately begins to lavish the underside with licks and long wet kisses, aided by her plus-sized bimbo lips.  When her eyes peek out from under your " + cockDescript(game.player, x) + ", they seem placid and happy.  It seems the slut wants nothing more than to lie there and service your cock.");
    outputText("\n\nGrabbing hold of Sophie's plump nipples, you immediately use them for leverage, tugging them together above your dick.  The tiny, pebbly joysticks are almost rock-hard already!  With your yanks merging her tits together around your " + cockDescript(game.player, x) + " and shooting explosions of sensation through her brain, Sophie struggles to stay coherent enough to keep orally attending to your maleness.  Her tongue's licks quickly begin to devolve into spastic, instinctive strokes, lashing around your underside with lusty fury.  They still feel good, so you don't really mind.  The space under Sophie's ass sloshes around muddily, proof of just how much the harpy is enjoying this tryst, even if you've turned the tables on her.");

    outputText("\n\nSophie's tits bounce off your crotch, hard.  Your [hips] have begun their march back and forth, bouncing off your pregnant tart's immense belly and back into her voluptuous tits.  The jiggling udders feel so good pressed around your " + cockDescript(game.player, x) + " that you can't even comprehend why anyone would want to wait for such pleasure.  You fuck her tits hard, so violently that Sophie begins to give up short gasps of pain, stopping her fellatio as the pain and pleasure mix confusingly in her head.  She grabs her jugs from the side, pressing them into you to take some of the tension from her abused nipples.");

    outputText("\n\nThe effects of her honeyed lipgloss are becoming more and more apparent to you, visible to the extreme in the thick, exceedingly quick pulsations that are running through your " + cockDescript(game.player, x) + ".  It's twitching happily, bouncing in the harpy's fat tits.  Her soft breasts have the perfect amount of give, and the golden fluid that slicks them makes them easily as slippery as a wet pussy, perhaps even more so.  Pre-cum is dribbling from your tip");
    ...
    outputText(".  Your [butt] is squished up against her belly, both of you sweating and slipping all over each other.  Bulging in her breasty embrace, your " + cockDescript(game.player, x) + " flexes, and your " + cockHead(player, x) + " thickens.");
    ...
    outputText("  You push your " + cockHead(player, x) + " and get a kiss from her as you finish cumming, oozing one last dollop onto her fat lips.  She whimpers in the most exciting way as you withdraw, wiping your cock off on her swollen belly, marking her as yours once more.");
...
// Let Her*
function letSophieMilkYoDick(): void {
    clearOutput();
    sophieSprite();
    const x: number = player.cocks.biggestCockIndex();
    ...
    // On top of her.
        ...
        else if (player.cumQ() < 800) outputText(" moans when your heavy deposits splatter into her hair, dripping through the downy blond feathers in a messy web.  Throughout it all, she sucks and licks at you, polishing your " + cockDescript(game.player, x) + " until she's sure every single drop of jizz has been expelled onto her curvy frame.");
        else {
...
// Sophie Births Egg*
export function sophieBirthsEgg(): void {
    sophieSprite();
...
export function sophiesEggHatches(): void {
...
export function sophieKidMaturation(): void {
    ...
    if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] > 0) {
        ...
        if (flags[kFLAGS.SOPHIE_FAMILY_INCEST] == 1) {
            ...
            outputText(".  She beams a radiant smile at you and turns around, shaking her rump at you enticingly, \"<i>So, when do you want to try me out, " + mf(player, "Daddy", "'Mom'") + "?</i>\"");
...
// [yes incest plz]
function yesToSophieIncest(): void {
    clearOutput();
    sophieSprite();
...
// [nono]
function noToSophieIncest(): void {
    clearOutput();
    sophieSprite();
...
export function sophieAndIzmaPlay(): void {
    outputText("\n");
    flags[kFLAGS.TIMES_SOPHIE_AND_IZMA_FUCKED]++;
    outputText("<b>Something odd happens that ");
    if (game.time.hours < 6 || game.time.hours > 20) outputText("night");
    else if (game.time.hours < 12) outputText("morning");
    else if (game.time.hours == 12) outputText("noon");
...
// (LEGGO MY EGGO)
function interruptSophieIzma(): void {
    clearOutput();
...
function watchIzmaSophie(): void {
    ...
    if (player.cocks.length > 0) outputText(", from me OR my Alpha");
    ...
    if (player.cocks.length > 0) outputText("stroke yourself through bliss");
