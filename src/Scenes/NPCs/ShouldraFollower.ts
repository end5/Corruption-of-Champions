...
export function followerShouldra(): boolean {
    return flags[kFLAGS.SHOULDRA_FOLLOWER_STATE] == 1;
}
function shouldraSleeping(changes: number = 0, reset: boolean = false): boolean {
    // Adjust sleep timer
    if (reset) {
...
// Initial Question
export function initialShouldersRecruitment(): void {
    clearOutput();
    spriteSelect(67);
...
export function morningShouldraAlert(): void {
    spriteSelect(67);
...
// corrupt vala
export function shouldraMeetsCorruptVala(): void {
    spriteSelect(67);
    clearOutput();
...
function shouldraTongueLicksPCs(): void {
    ...
    if (flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
        ...
        if (player.tongueType > TongueType.HUMAN) outputText("some of ");
    ...
    if (player.perks.findByType(PerkLib.Flexibility) >= 0) {
        ...
        if (flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
            ...
            outputText("\n\nShe sits you down and lets her dildos fall to the ground, forgotten for the moment.  \"<i>How's this one go?</i>\" she asks, leaning down in an awkward attempt to get your face in your " + vaginaDescript(player, 0) + ".  You start to explain the proper 'technique' to what she's trying to do, but before you can even finish your thought she's stuck a leg up in the air and has your mouth to your own snatch.  You doubt you'll ever get used to having her inside your thoughts.");
        }
...
export function shouldraDream1(): void {
    outputText("\n");
    spriteSelect(67);
...
function shouldraYappin(): void {
    ...
    if (flags[kFLAGS.SHOULDRA_TALK_NEEDED] == 1) {
        ...
        if (flags[kFLAGS.SHOULDRA_PLOT_LEVEL] == 1) {
            ...
            if (player.perks.findByType(PerkLib.Incorporeality) >= 0) outputText(" - and I mean <b>really</b> possess someone -");
...
export function shouldraFollowerScreen(): void {
    spriteSelect(67);
    clearOutput();
...
function shouldraTalkMenu(): void {
    ...
    let blowShitUp: () => void = null;
    let shrinkShit: () => void = null;
...
function shouldraGroPlus(): void {
    ...
    let balls: () => void = null;
    const breast: () => void = shouldraGrowsYoTits;
    let clit: () => void = null;
    let cock: () => void = null;
...
export function shouldersWarnings(): boolean {
...
export function nightTimeShouldraRapesThePC(): void {
...
export function shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark(): void {
    outputText("\n");
    spriteSelect(67);
    const choices: any[] = [];
    let select: number;
    ...
    if (player.effects.getValue1Of(StatusAffects.Exgartuan) == 1 && 9999 == 9999) {
    ...
    choices[choices.length] = 7;
    // 8 (Tel'Adre unlocked)
    if (player.effects.getValue1Of(StatusAffects.TelAdre) >= 1) choices[choices.length] = 8;
    // 9 (is preggers)
    ...
    if (player.perks.findByType(PerkLib.MaraesGiftStud) >= 0 || player.perks.findByType(PerkLib.MaraesGiftFertility) >= 0 || player.perks.findByType(PerkLib.MaraesGiftProfractory) >= 0 || player.perks.findByType(PerkLib.MaraesGiftButtslut) >= 0) choices[choices.length] = 16;
    ...
    if (player.perks.findByType(PerkLib.Incorporeality) >= 0) choices[choices.length] = 18;
    // 19 (possessed by Boobgartuan)
    if (player.effects.getValue1Of(StatusAffects.Exgartuan) == 2) choices[choices.length] = 19;
...
export function shouldraAndExgartumonFightGottaCatchEmAll(): void {
    // 40% chance to occur when player has both Shouldra and Cock Exgartuan and is going to sleep
    // Unlocks Argue 2
    spriteSelect(67);
    outputText("Your bedroll looks mighty comfortable after a hard day's ");
...
export function exgartumonAndShouldraFightPartII(): void {
...
export function exgartumonAndShouldraFightPartIII(): void {
...
export function exgartuMonAndShouldraShowdown(): void {
...
function keepShouldraAndKickOutExgartuan(): void {
    ...
    outputText("\n\n\"<i>I AM THE GODDAMN DEVIL OF DICKINGS!  I'LL FUCKING RAVAGE YOU!</i>\" Exgartuan shouts, surging forward at the bent over ghost.  In his rage, he managed to somehow forget just what a ghost can do, passing right through the happy phantom.  Before he can correct himself and throw you around the room any further, Shouldra's form turns to a fog and permeates your [cock].  You fall to your back as the war rages on in your titanic dong, violently thrashing around every which direction as two struggling voices emanate from within.  Your cock crashes into things all over your tent, knocking one thing over before bashing into the next.  You're afraid to even lay your hands on it!");
...
export function keepShouldraPartIIExgartumonsUndeatH(): void {
    clearOutput();
    spriteSelect(67);
    ...
    outputText("While slipping into your bedroll to enjoy a peaceful night's sleep, a chill runs down your spine.  You look around to-");
...
// Intro
export function shouldraBakeryIntro(): void {
    clearOutput();
    spriteSelect(67);
