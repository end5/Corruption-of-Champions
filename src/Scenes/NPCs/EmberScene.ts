...
export class EmberScene implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        // Ember fuck cooldown
        if (player.effects.getValue1Of(StatusAffects.EmberFuckCooldown) > 0) {
            player.effects.addValue(StatusAffects.EmberFuckCooldown, 1, -1);
            if (player.effects.getValue1Of(StatusAffects.EmberFuckCooldown) < 1) player.effects.remove(StatusAffects.EmberFuckCooldown);
        }
        // Ember napping
        if (player.effects.findByType(StatusAffects.EmberNapping) >= 0) {
            player.effects.addValue(StatusAffects.EmberNapping, 1, -1);
            if (player.effects.getValue1Of(StatusAffects.EmberNapping) <= 0) player.effects.remove(StatusAffects.EmberNapping);
        }
        if (followerEmber() && player.effects.findByType(StatusAffects.EmberNapping) < 0) {
            // Mino cum freakout - PC partly addicted!
            if (flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 1 && player.perks.findByType(PerkLib.MinotaurCumAddict) < 0 && flags[kFLAGS.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 0) {
    ...
    public timeChangeLarge(): boolean {
        if (player.effects.findByType(StatusAffects.EmberNapping) < 0 && followerEmber() && player.effects.findByType(StatusAffects.EmberFuckCooldown) < 0) {
            // Ember get's a whiff of fuckscent and knocks up PC!
            if (player.vaginas.length > 0 && player.inHeat && player.pregnancyIncubation == 0 && rand(10) == 0 && (flags[kFLAGS.EMBER_GENDER] == 1 || flags[kFLAGS.EMBER_GENDER] == 3)) {
...
function emberSexMenu(output: boolean = true): void {
    ...
    let catchAnal_: () => void = null;
    let pitchAnal: () => void = null;
    let blowEmber: () => void = null;
    let getBlown: () => void = null;
    let eatOut: () => void = null;
    let getEatenOut: () => void = null;
    let penetrateHer: () => void = null;
    let getPenetrated: () => void = null;
    ...
    if (player.vaginas.length > 0 && player.lust >= 33) getEatenOut = getEatenOutByEmbra;
...
export function getSomeStuff(): void {
    ...
    Inventory.takeItem(WeaponLib.DRGNSHL, TelAdre.armorShop);
}
...
function masturbateOntoAnEgg(): void {
    ...
    // Female Version (Z)
        ...
        if (player.vaginas.wetness() <= 3) outputText("few drops");
        else if (player.vaginas.wetness() < 5) outputText("squirt");
...
function talkToEmberAboutDragonzzz(): void {
    ...
    if (choice == 0) {
        ...
        // Low affection:
        if (emberAffection() <= 25) outputText("\n\nSnorting, Ember cracks an amused smile as " + emberMF("he", "she") + " chuckles. \"<i>What, does the 'Champion' think " + mf(player, "him", "her") + "self worthy of courting me?  That's a good one!</i>\"  " + emberMF("He", "She") + " giggles openly to make " + emberMF("his", "her") + " lack of interest in you known... yet, it seems rather forced.");
...
function discussExplorationWithEmber(): void {
    ...
    if (choice == 0) {
        ...
        if (subChoice == 0) {
            ...
            if (rand(3) == 0 && player.effects.getValue1Of(StatusAffects.MetWhitney) <= 1) outputText(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
            // (if Fetish Cult is encounterable)
            else if (rand(3) == 0 && player.effects.findByType(StatusAffects.FetishOn) >= 0) outputText("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
            // (If Beautiful Sword has not been acquired)
            else if (rand(3) == 0 && player.effects.findByType(StatusAffects.TookBlessedSword) < 0 && player.effects.findByType(StatusAffects.BSwordBroken) < 0) outputText("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
            else if (rand(3) == 0 && player.effects.findByType(StatusAffects.CampRathazul) < 0) outputText("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
...
export function emberBitchesAtYouAboutLustiness(): void {
    ...
    if (player.vaginas.length > 0) outputText("your " + vaginaDescript(player) + " slick and wet with moisture, ready to fuck");
...
function bloodForTheBloodGod(): void {
    ...
    if (flags[kFLAGS.DRANK_EMBER_BLOOD_TODAY] == 1 && !game.debug) {
...
function emberTFs(): void {
    ...
    // Gain Dragon Head
    if (changes < changeLimit && rand(3) == 0 && player.faceType != FaceType.DRAGON && flags[kFLAGS.EMBER_ROUNDFACE] == 0) {
        ...
        player.faceType = FaceType.DRAGON;
        changes++;
    }
    ...
    // Gain Dragon Ears
    if (changes < changeLimit && rand(3) == 0 && player.earType != EarType.DRAGON) {
        player.earType = EarType.DRAGON;
    ...
    // Gain Dragon Tongue
    if (changes < changeLimit && rand(3) == 0 && player.tongueType != TongueType.DRACONIC) {
    ...
    // Gain Dragon Tail
    if (player.tailType != TailType.DRACONIC && changes < changeLimit && rand(3) == 0) {
        // (If no tail)
        if (player.tailType == TailType.NONE) outputText("\n\nA sudden dull, throbbing pain in your " + buttDescription(player) + " forces your hands to it; you can feel an ominous lump over your tail bone, swelling bigger and bigger with every heartbeat.  All of a sudden, it seems to explode, jutting out and around until it hovers near your ankles, the skin under your flesh hard and scaly.  <b>You now have a dragon tail flicking at your back, flexible as a whip.</b>");
        // (If tail)
     ...
     */
    // Grow Dragon Wings
    if (player.wingType != WingType.DRACONIC_LARGE && changes < changeLimit && rand(3) == 0) {
        if (player.wingType == WingType.NONE) {
            ...
            player.wingType = WingType.DRACONIC_SMALL;
            player.wingDesc = "small, draconic";
        }
        // (If Small Dragon Wings Present)
        else if (player.wingType == WingType.DRACONIC_SMALL) {
        ...
        else if (player.wingType == WingType.SHARK_FIN) {
    ...
    if (dragonScore(player) >= 4 && rand(3) == 0 && player.gender > 0) {
        ...
        if (player.cocks.length > 0 && flags[kFLAGS.EMBER_GENDER] >= 2) {
            ...
            if (player.vaginas.length > 0 && flags[kFLAGS.EMBER_GENDER] == 3) outputText(" even while ");
        }
        if (player.vaginas.length > 0 && (flags[kFLAGS.EMBER_GENDER] == 3 || flags[kFLAGS.EMBER_GENDER] == 1)) outputText("taking that hard, spurting cock inside your own " + vaginaDescript(player, 0));
...
function emberIsAnEggFactory(): void {
    ...
    if (emberAffection() <= 25) {
        ...
        Inventory.takeItem(ConsumableLib.DRGNEGG, Camp.returnToCampUseOneHour);
    }
    // (Medium Affection)
    else if (emberAffection() < 75) {
        outputText("\n\nEmber gasps");
    ...
    // (High Affection)
        ...
        if (player.tongueType > TongueType.HUMAN) outputText(" and out... and out...");
        ...
        if (player.tongueType > TongueType.HUMAN) outputText("which is quite far indeed, ");
        ...
        if (player.tongueType > TongueType.HUMAN) outputText("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
...
function catchAnal(): void {
    ...
    outputText(" length of " + emberMF("his", "her") + " prick between your " + buttDescription(player) + " cheeks, letting you feel the refreshing coolness of " + emberMF("his", "her") + " pre dripping from the tip.  \"<i>This... isn't going to work,</i>\" " + emberMF("he", "she") + " growls.");
    ...
    if (player.vaginas.length > 0) outputText("your cunt gushing feminine lubricant to glaze Ember's belly");
...
function suckEmberCock(): void {
    ...
    if (player.vaginas.length > 0) outputText("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
    ...
    outputText("\n\nYou close your eyes and submit to the dragon's gentle ministrations, savoring the close contact as " + emberMF("his", "her") + " cool tongue glides across your " + skinFurScales(player) + ".  Ember takes " + emberMF("his", "her") + " time, making sure you're absolutely spotless.  Once you've been cleaned and licked to " + emberMF("his", "her") + " satisfaction, " + emberMF("he", "she") + " finally settles down; getting off the stump to lay on the floor and pull you atop " + emberMF("him", "her") + "; hugging you close.");
...
function stickDickInKnifeDrawer(): void {
    ...
    outputText("\n\nSatisfied at that, Ember extends " + emberMF("his", "her") + " tongue, slowly wrapping it around your " + cockDescript(game.player, x) + ", an act that makes you shiver as the inhumanly long, prehensile appendage twines around your member like a slick-skinned snake.  " + emberMF("He", "She") + " coils it around your shaft expertly, gripping you into a makeshift cock-sleeve.");

    outputText("\n\nLooking down at Ember, you see the dragon wreathed in happiness; " + emberMF("he", "she") + "'s so focused on " + emberMF("his", "her") + " task that " + emberMF("he", "she") + " doesn't even notice you staring.  Using " + emberMF("his", "her") + " coiled tongue " + emberMF("he", "she") + " begins jerking you off; the sensation is familiar and yet so alien... it only takes a few strokes before your " + cockDescript(game.player, x) + " is glistening with Ember's saliva, and in a particularly pleasurable tug, you moan.  A bead of pre escapes your " + cockHead(player, x) + " and slowly slides down your shaft, until it makes contact with Ember's tongue.");

    outputText("\n\nThe moment " + emberMF("he", "she") + " tastes you, " + emberMF("his", "her") + " eyes seem to glow.  In a whip-like motion, Ember pulls " + emberMF("his", "her") + " tongue back into " + emberMF("his", "her") + " awaiting mouth, engulfing your " + cockDescript(game.player, x) + ".  You gasp as the cool, slick flesh surrounds you, heedless of the sharp teeth whose hard surfaces you can occasionally feel grazing you.  You trust your dragon too much to believe " + emberMF("he", "she") + " would hurt you.");

    outputText("\n\nEmber sucks you hard, slurping around your shaft with " + emberMF("his", "her") + " prehensile tongue and cooing with pleasure.");
    if (flags[kFLAGS.EMBER_GENDER] == 1 || flags[kFLAGS.EMBER_GENDER] == 3) outputText("  " + emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your " + feet(player) + ", smearing small beads of Ember's barely contained pleasure on your " + skinFurScales(player) + ".");
...
function stickItInEmbersButt(): void {
    ...
    outputText(".  You force yourself to remain still, giving the horny dragon a chance to recover from your " + cockDescript(game.player, x) + " impaling " + emberMF("his", "her") + " back passage, savoring the feeling of " + emberMF("his", "her") + " cool muscles squeezing and rippling around your shaft.");
    ...
    outputText(emberMF("His", "Her") + " lusty moans each time " + emberMF("he", "she") + " comes down on your " + cockDescript(game.player, x) + " send a thrill up your spine, and the deep purring emanating from " + emberMF("his", "her") + " chest is audible enough to give you an idea of how much Ember is enjoying this, despite " + emberMF("his", "her") + " initial reluctance.");
...
function slurpDraggieCunnies(): void {
    ...
    if (player.tongueType > TongueType.HUMAN) outputText("inhuman ");
    ...
    if (flags[kFLAGS.EMBER_GENDER] == 1 || flags[kFLAGS.EMBER_GENDER] == 3) outputText(", heedless of the steady rain of pre dribbling into your " + hairDescription(player) + ".  You're confident she'll crack soon...");
    ...
    if (player.vaginas.length > 0) outputText("moist cunt");
...
function getEatenOutByEmbra(): void {
    ...
    if (player.vaginas.wetness() >= 4) outputText("spattering");
    ...
    if (player.vaginas.wetness() >= 4) outputText("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as " + emberMF("his", "her") + " face is completely covered in femcum.");
...
function penetrateEmbrah(): void {
    ...
    if (player.cocks[x].cockType == CockTypesEnum.DRAGON) {
        ...
        outputText("\n\nEmber takes your " + cockDescript(game.player, x) + " in her hand and begins gently stroking you to full mast.  \"<i>I hope you know how to use this,</i>\" Ember remarks.");
        outputText("\n\nYou assure her that you know exactly what you're doing. ");
    }
    outputText("\n\nDeciding it's better for the dragon to psyche herself up first, you wait patiently; it doesn't look like it will be long before she makes the first move.  Ember's hands roam all over your " + cockDescript(game.player, x) + " and you think you can almost hear a faint purring emanating from her chest.  Slowly one of her hands reaches down to massage her moistened netherlips.  Ember gasps in pleasure as she continues masturbating both of you in a daze, until she suddenly snaps out of her trance.");
...
function getPenetratedByEmberLastSexSceneWoooo(): void {
    ...
    outputText(" dick between Ember's legs, and you lick your lips hungrily.  Your " + vaginaDescript(player) + " throbs eagerly as you tell Ember you have certain itches you need to scratch... is Ember enough of a " + emberMF("man", "herm") + " to scratch them for you?");
    ...
    if (player.tongueType == TongueType.HUMAN) outputText("yours ");
    ...
    outputText("\n\nEmber leans over you, panting, " + emberMF("his", "her") + " hot breath mere inches from your " + face(player) + ", gazing at you through half-lidded, lust-addled eyes.  \"<i>You feel so good... so hot... so sexy...</i>\" Ember says mid-pant.  You pant right back, reaching up and wrapping your arms around " + emberMF("his", "her") + " neck, staring back at " + emberMF("him", "her") + " with the same expression and telling " + emberMF("him", "her") + " you think " + emberMF("he", "she") + " feels just as good.");
    ...
    if (player.perks.findByType(PerkLib.HistoryWhore) >= 0 || player.perks.findByType(PerkLib.HistorySlut) >= 0) outputText("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
...
export function emberRapesYourHeatness(): void {
    ...
    if (player.vaginas.length > 0 && player.inHeat && (flags[kFLAGS.EMBER_GENDER] == 1 || flags[kFLAGS.EMBER_GENDER] == 3))
...
function fuckOffEmberIWantANap(): void {
    ...
    player.effects.create(StatusAffects.EmberFuckCooldown, 12, 0, 0, 0);
    doNext(playerMenu);
}
...
function getKnockedUpByEmbrahBroBaby(): void {
    ...
    outputText("face.  \"<i>But if you must know, I do love you.</i>\"   With a final sigh Ember sets down on your bedroll for a quick nap.");
    ...
    player.effects.create(StatusAffects.EmberFuckCooldown, 36, 0, 0, 0);
...
function breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding(): void {
    ...
    if (player.vaginas.length > 0) outputText("  The tip of her tail gently teases your slick [vagina] with tiny strokes.");
    ...
    outputText("\n\nEmber jumps off you momentarily to inhale your musk, stroking you until a bead of pre forms on your " + cockHead(player, x) + ".  Ember's eyes grow wide, as if she had spotted a pearl; and she wastes no time in extending her tongue to lick the bead away.  \"<i>Hmm, delicious...  You're ready, and so am I!</i>\"");

    outputText("\n\nAiming your cock upwards; Ember straddles you and lowers herself on your pulsing " + cockDescript(game.player, x) + ", hissing in pleasure as your hot flesh finally makes contact with her drooling fuckhole");
    ...
    outputText("her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones.");

    // (If Ember has a dick)
    if (emberHasCock()) outputText("  Ember's cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.");
    ...
    outputText("her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll.");
    ...
    if (player.vaginas.length > 0) outputText("your neglected cunt splashing fluid down your [legs] and ");
    outputText("your " + cockDescript(game.player, x) + " gushing into the dragon's ready womb.");
    ...
    outputText("\n\nEmber purrs and pulls you into another kiss, holding you close with her tail, legs and arms; intent on keeping you right where you are.  Well, it's not like you could go anywhere else with her pussy effectively keeping your " + cockDescript(game.player, x) + " hostage inside its depths.");
    ...
    player.effects.create(StatusAffects.EmberFuckCooldown, 36, 0, 0, 0);
    player.effects.remove(StatusAffects.Rut);
    player.orgasm();
    dynStats("sen", -2);
...
export function giveBirthToEmberKids(): void {
    ...
    if (flags[kFLAGS.EMBER_OVIPOSITION] == 0) {
        ...
        outputText("\n\nEmber calmly smiles at you and points to your [chest].  You follow " + emberMF("his", "her") + " finger to see the little dragon nursing from your " + nippleDescription(player, 0) + ". \"<i>Sorry.  ");
        ...
        outputText("\n\nYou sigh softly, stroking your newborn's head even as it industriously sucks away at your " + nippleDescription(player, 0) + ".  Speaking of which, you ask Ember what you've had - a boy?  A girl?  Both?");
    ...
    player.effects.create(StatusAffects.EmberNapping, 5, 0, 0, 0);
    player.genderCheck();
}
...
function highAffectionEmberLustFuck(): void {
    ...
    if (flags[kFLAGS.EMBER_GENDER] == 1) {
        ...
        outputText("\n\nYou get yourself in position and align your shaft with the dragon's puckered hole.  You hump a few times experimentally; each time your " + cockHead(player, x) + " bumps against his tight ass and threatens to push past his sphincter, the dragon gasps.  You would be worried if his gaze wasn't so lusty and expectant.");
        ...
        outputText("\n\nYou look into his eyes and smile, slowly pressing into his tight boy-pussy and spreading his cheeks with your girth.  Ember moans, arches his back and growls with delight, ass already clenching eagerly around your invading " + cockDescript(game.player, x) + ".  A few more humps and you feel yourself go as far inside his ass as possible.");
...
function stayWithEmberAfterLustFuck(): void {
    ...
    // else if Ember is female:
        ...
        outputText("\n\nShe breaks the kiss and lightly strokes your shaft, smiling at you.  Then she aligns the tip of your " + cockDescript(game.player, x) + " with her pussy.");
        menu();
...
function penetrateWithEmber(clear: boolean = true): void {
    ...
    if (player.tailType > TailType.NONE) outputText("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
    ...
    if (player.wingType > WingType.NONE) outputText("stopping momentarily to stroke along your " + player.wingDesc + " wings before continuing up and ");
    ...
    outputText("listening to her moan softly in pleasure from your actions, feeling her netherlips rippling around your " + cockDescript(game.player, x) + ".  Your nursing is suddenly interrupted when you're pulled off Ember's breasts and into a deep kiss.  The dragon-girl shoves her tongue inside your mouth, exploring it in detail");
