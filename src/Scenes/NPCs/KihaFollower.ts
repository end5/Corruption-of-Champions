...
export function kihaFriendlyGreeting(output: boolean = true): void {
    ...
    if (output && flags[kFLAGS.KIHA_TALK_STAGE] == 6 && player.cor <= 50) {
        ...
        if (game.time.hours < 20) outputText("clouds", false);
...
function lieThere(): void {
    ...
    outputText("[pg]\"<i>Oh, " + mf(player, "he", "she") + " belongs to you, is that it?</i>\"", false);
    ...
    outputText("[pg]\"<i>Oh, like " + player.short + " would even look twice at those floppy things!</i>\" Kiha growls, grabbing her own D-cups defensively.  \"<i>" + mf(player, "He", "She") + " OBVIOUSLY prefers smaller, perkier boobs.  Don't you, " + player.short + "?</i>\"", false);
    outputText("[pg]You start to stammer an answer, but before you know it, Hel's put a foot on your chest, pushing you just a little deeper into the still-hot mud.  \"<i>How the hell would YOU know what " + player.short + " likes, huh?  <i>I</i> know exactly what makes " + mf(player, "him", "her") + " tick!</i>\"", false);
    ...
    outputText("[pg]Suddenly, Hel rises from the dirt beside you, her tail flaming dangerously.  \"<i>Oh, FUCK YOU!</i>\"  Hel cries, stamping back to stand beside Kiha.  \"<i>How the FUCK would you know ANYTHING about what " + mf(player, "he", "she") + " likes?  You think you know?  I'll fucking SHOW YOU!</i>\"", false);
    ...
    outputText("[pg]\"<i>See?</i>\"  Kiha shouts, grabbing at the stain you've left on her thighs, \"<i>" + player.short + " obviously likes me better - " + mf(player, "he", "she") + " dropped " + mf(player, "his", "her") + " cum RIGHT next to my vag.  " + mf(player, "He", "She") + " probably wants to knock me up even, don't you, " + player.short + "?</i>\"", false);
    outputText("[pg]\"<i>Oh, is that right?</i>\"  Hel laughs, giving her ample hips a shake forceful enough to dislodge your spooge, hitting Kiha full on the face.  \"<i>" + player.short + " put a load right in my asscheeks - " + mf(player, "he", "she") + " gave me the last AND the dirtiest load.  What do you get? Oh, that's right, a vag shot.  How unique!</i>\"", false);
...
function lovinHugKiha(): void {
    ...
    else if (player.vaginas.length > 0) outputText("[pg]Your [vag] tingles as it's exposed to the air.  The lusty dragon-maid dips a digit inside your depths before commenting, \"<i>Not bad, I guess.</i>\"", false);
    ...
    else if (player.vaginas.length > 0) doNext(lovingHugsGirlFuckSex);
...
function lovingHugDickings(): void {
    ...
    outputText("[pg]");
    if (horseScore(player) > 4 || dogScore(player) > 4 || catScore(player) > 4) outputText("Growling");
...
function lovingHugsGirlFuckSex(): void {
    ...
    outputText("[pg]Kiha mutters, \"<i>Gonna... gonna...</i>\" into your pussy, but before she can finish, you slam yours down on her face, grinding yourself off on her nose while you take her to orgasm.  Your body burns with lust, and once you taste the warrior-woman's burst of girl-honey on your tongue, you moan and buck your " + hipDescription(player) + " harder, frigging your [clit] off on her nose until the pleasure makes you seize, juices ");
    if (player.vaginas.wetness() < 3) outputText("dripping ");
    else if (player.vaginas.wetness() < 4) outputText("running ");
    else if (player.vaginas.wetness() < 5) outputText("flooding ");
...
function hangOutWithKiha(): void {
    ...
    // Hang Out 3
        ...
        if (!followerKiha()) {
            ...
            if (game.time.hours < 21) outputText("particularly cloudy day above you, with great pink and purple clouds drifting across the demonically-tainted horizon");
            ...
            if (game.time.hours < 21) outputText("day around your camp - thick pink and purple clouds are rolling lazily over the wasteland, shimmering behind the great maw of the portal");
        ...
        if (game.time.hours < 21) outputText("day");
        ...
        if (game.time.hours < 21) outputText("clouds");
...
function boneTheShitOutofKihaHolesWithHorsecock(): void {
    ...
    else if (player.vaginas.length > 0) outputText("fondles your [vag]");
    ...
    if (player.cocks[x].cockType.Index > 0) {
        ...
        outputText("[if (isTaur = true) \"throwing your hooves onto the rock, looming over Kiha\" else \"fondling your " + cockDescript(game.player, x) + " with one hand\"]");
    }
    outputText(", the bobbing motions flinging ropes of pre-cum from your " + cockDescript(game.player, x) + " every which way.  ");
    ...
    else outputText(cockHead(player, x) + " of your " + cockDescript(game.player, x));
    ...
    outputText(".  Of course you're big, you think, you're the alpha, the " + mf(player, "stud", "breeder") + ", the one in charge.  You ");
...
function kihaPlaysWithBigassCocksFemDomAhoy(): void {
    ...
    outputText("[pg]Kiha comments, \"<i>That's it, that's a good " + mf(player, "boy", "girl") + ",</i>\" just in time to watch the first globules of semen blast into the air.  You clench and unload another geyser of sperm into the air before the first rains down over Kiha, splattering over her hair and face");
...
function kihaGirlGirlSex(): void {
    ...
    if (player.vaginas.wetness() < 4) outputText("Trickles");
...
function savinTheAnalForKiha(): void {
    ...
    outputText("[pg]\"<i>Gah!</i>\"  the dragoness gasps as she feels your hardening " + cockDescript(game.player, x) + " pressing into her butt, barely restrained by your [armorName].  \"<i>Oh, of all the things to obsess over!</i>\"  she groans, but to your delight Kiha wiggles her hips in your grip, grinding her soft butt against your prick.  You give her a sharp little spank, and laugh as Kiha draws a sharp breath and clenches her derriere - just in time for you to free the beast, your " + cockDescript(game.player, x) + " flopping down between her butt cheeks.");
    ...
    if (player.vaginas.length > 0) outputText(", just as yours are");
    outputText(".");
    ...
    outputText("[pg]Kiha spits, her glob of saliva hitting the bottom of your cock with a warm, wet smack.  You shudder at the sudden heat on your " + cockDescript(game.player, x) + ", but it turns into a writhe of pleasure as Kiha grabs your prick between two of her big, scaly toes, her claws hanging menacingly around your girth.  She grins, looking over you as she pushes your prick back down against your belly.  Her supple, leathery soles press down on your rod, slowly sliding up and down your length, grinding her saliva into your cock, spreading her lubricant around.");
    ...
    outputText(".  Her movements are slow, sensual, teasing as she advances on you.  Her large, dusky breasts brush the ground, stiff nipples coming to rest on either side of your abused prick, dabbling into the pool of pre and spit she's left around the base of your cock. Laying atop you, the dragoness takes her breasts and wraps them around your shaft, enveloping your " + cockDescript(game.player, x) + " in warm, soft titflesh.  You let out a low, animalistic moan as she drags her tits along the entire length of your prick, keeping them pressed tightly together and spitting again and again onto you.  Her tits are as good as a wet pussy, not too tight but deliciously wet. You desperately want to cum, to smear her big tits with your seed, and the mere thought causes a long trickle of pre to leak out of your " + cockHead(player, x) + ", adding itself to the sodden mess your lover's turned your shaft into.  Kiha grins devilishly at you, her long tongue flicking out to taste your seed.");
    outputText("[pg]\"<i>Mmm, not bad.  And so much... you must be dying to stick your cock in, aren't you?  Do you want that " + cockDescript(game.player, x) + " inside me, doofus; to feel my tight little asshole swallow you up? I think you're about ready...</i>\"");
    outputText("[pg]Kiha shifts, straddling your [hips] and spreading her ass cheeks wide.  She lowers herself, slowly, moving down just enough to let your sensitive " + cockHead(player, x) + " brush the tight ring of her pucker.  You both gasp at once, a little lightning bolt of pleasure connecting you for a fraction of a second.  Kiha groans, slowly sliding down, just enough so that your head pokes through her clenched sphincter, brushing against the hot, moist walls of her anal passage.");
    ...
    if (player.breastRows.biggestTitSize() >= 3) outputText(" as she palms your " + biggestBreastSizeDescript(player), false);
    ...
    outputText(" and you already so sensitive from edging again and again.  Kiha slowly rolls her hips, dragging out an inch of your cock at a time before her gripping asshole swallows it back up; when she moves, though, she's careful never to seperate the two of you, always holding each other close as she gently rides your " + cockDescript(game.player, x) + ".  You grin as she finally allows you to move, your hips reaching up to meet hers as she begins making short, quick bounces, still holding you tight enough to nip your ear and trace kisses along your neck.");
    outputText("[pg]You move together, pressing your cock deep into Kiha's hot bowels before withdrawing nearly to the tip.  Again and again your " + cockDescript(game.player, x) + " slides in and out of Kiha's asshole, slowly stretching her until the going gets easy.  She starts to pick up the pace, pulling you up from the ");
    if (followerKiha()) outputText("rock");
    else outputText("tree");
...
function fuckKihasVagInCamp(): void {
    ...
    if (player.skinType != SkinType.SCALES) outputText("leave long, red scratches on your back.");
...
export function kihaBitchesOutCorruptPCs(): void {
    ...
    if (!followerKiha()) {
        ...
        else if (player.vaginas.length > 0) outputText("[vag]");
...
function dominateKihasFaceWithStuffAndStuffOrSomethingIDunnoWhyImStillWritingThis(): void {
    ...
    outputText("[pg]\"<i>If you want have your way with me, you'll need to earn it, just like anyone else,</i>\" Kiha explains.  She narrows her eyes at you and questions, \"<i>The question is, are you " + mf(player, "man", "woman") + " enough to take what you want?  I wouldn't want you to get hurt.</i>\"");
    outputText("[pg]That seems like a challenge.  Will you rise to it, or will you back down?");
...
export function pcWinsDomFight(): void {
    ...
    else if (player.vaginas.length > 0) outputText("release your lathered up " + vaginaDescript(player, 0));
    ...
    if (select == 0 || !player.vaginas.length > 0) {
        ...
        else outputText("[pg]\"<i>Ha!  The Champion still has some fight in " + mf(player, "him", "her") + "!  Keep me here if you can, wimp!  I WILL pin you down and make you beg to plant your seed in my womb!  Just wa-OOhhOhhh...</i>\"");
    }
    ...
    if (select == 0) outputText("works her way on top of your pelvis and impales herself on your " + cockDescript(game.player, x) + ", grinning like a mad fool as she does so.  \"<i>Told you!</i>\"  She remarks. \"<i>Was that really the best you could do?  With the hair pulling?</i>\"  She sighs.  \"<i>I guess I have to teach you a thing or two...</i>\"  Lowering herself down in between savage slams of her rump against your pelvis, the cool and arrogant woman takes the " + skin(player) + " of your neck and pierces it with her canines, causing you to cry out as the sting of her bite flows through you.  Humming her approval, she carries on with the coarse treatment of your entrapped cock, mashing and working forth a crescendo of skin on skin slapping noises.");
    // Kiha ensnares the PC between her legs for cunnilingus (universal):
    else if (select == 1) outputText("quickly wraps her legs around your head and rolls your body into a belly down prone.  \"<i>Eat it " + mf(player, "big boy", "you delectable slut") + "!  I've got you in a hold you can't possibly escape from!  Lap it up bitch!</i>\"  For the time being you comply, if only to give you time to find a way out of this as you probe her cunt with your tongue.  \"<i>Yeeeaahhhah, just like that.  Good " + mf(player, "boy", "girl") + ", lick your mistress's pussy!</i>\"  She coos authoritatively, her thigh tensing up with every stroke.  The dragon woman even has the nerve to play with your hair as she keeps you within her tight embrace.");
    ...
    // Random event two of three: Footjobs and missionary, oh my!
        ...
        if (player.tongueType == TongueType.SNAKE || player.tongueType == TongueType.DEMONIC) {
            ...
            // Demon:
            if (player.tongueType == TongueType.DEMONIC) outputText("taking to her pointed mammary like a boa to prey as you lather up each teat separately.  The hot-tempered dragon girl squirms, completely at your mercy while you have your fun.");
            // Snake:
    ...
    // Random Scene three of three: Standing dominant cunnilingus, with a reversal:
        ...
        if ((player.cocks.length > 0 && rand(2)) || !player.vaginas.length > 0) {
            ...
            outputText("[pg]Helpless to offer any resistance, and perhaps unwilling to resist, even if she could, Kiha lays back and accepts that you have won this \"<i>battle</i>\", licking, suckling and kissing your " + clitDescription(player) + " as you grind against her.  The sensation of her long, reptilian tongue pressing against and sliding sensuously over your nether-lips and clit is incredibly intense and satisfying, and it isn't long until you bellow a " + mf(player, "manly", "girlish little") + " cry and cum hard, your muscles tensing with pleasure as you drench the dragoness' face in lady-cum.  She readily licks it all up as it flows, moaning as she swallows your sexual essence.");
        }
    }
