...
export class Raphael implements TimeAwareInterface {
    ...
    public timeChangeLarge(): boolean {
        if (checkedRussetRogue++ == 0 && game.time.hours == 6 && flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] >= 0 && player.keyItems.has("Camp - Chest") >= 0 && player.gems >= 5 && player.effects.getValue1Of(StatusAffects.TelAdre) >= 1) {
            ...
            if (flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] == 0) { // Countdown to finale not currently engaged!
                ...
                if (RaphaelLikes()) { // Not yet met!  MEETING TIEM!
                    if (flags[kFLAGS.RAPHAEL_MET] == 0) {
                        ...
                        if (player.armor == ArmorLib.R_BDYST && (flags[kFLAGS.RAPHAEL_DRESS_TIMER] > 1 && flags[kFLAGS.RAPHAEL_DRESS_TIMER] <= 4)) {
                            outputText("<b>\nSomething unusual happens that morning...</b>\n");
                    ...
                    // shot down yet?
                    if (player.armor == ArmorLib.R_BDYST && flags[kFLAGS.RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] == 0) {
                        outputText("<b>\nSomething unusual happens that morning...</b>\n");
                        doNext(RaphaelEncounterIIDressFollowup);
                        return true;
                    }
...
export function RaphaelLikes(): boolean {
    ...
    if (player.breastRows.biggestTitSize() >= 12) return false;
    // ({If player has below C cup breasts}
    if (player.breastRows.biggestTitSize() < 3) return false;
    ...
    if (player.balls > 0 || player.cocks.length > 0) return false;
    // (For now:
...
function meetRaphaelPtII(): void {
    ...
    // [Talk] [Slap] [Swoon]
    simpleChoices("Talk", RaphaelFirstMeetingTALK, "Slap", RaphaelFirstMeetingSLAP, "Swoon", RaphaelFirstMeetingSWOON, "", null, "", null);
}
...
function RaphaelDressPtII(): void {
    ...
    outputText("All over your body where the gossamer isn't reinforced with jacket or corset, the density of the delicate velvet web varies. Although the silk hugs across your " + vaginaDescript(player, 0) + " and through the crack of your ass as a triple layer that guards against prying eyes, the surface of your hips and legs is clearly seen through the transparent motif of flowers swirling across the lace. The cheeks of your " + buttDescription(player) + " feel equally exposed despite the presence of four sweeping rosebranches stitched across them, but at least the jacket trails past your lower back and partly covers your buttocks with its parted tailflaps.", false);
    // ({If player has tail}
    if (player.tailType > TailType.NONE) outputText("  Your tail peeks out through the cut.", false);
    outputText("  You still can't help but feel that anyone standing behind you is given a generous glimpse of your ornate ass, however. The same goes for your " + breastDescriptOfRow(game.player, 0) + "; cupped, lifted and presented as they are to the outside world by grasping silk. Their ample curve and tender flesh are clearly visible through the red lace. The only thing saving their modesty is the tactical application of a sea of organic patterns across the lower half, with the curl of two roses covering your " + nippleDescription(player, 0) + "s. The ensemble comes with a pair of red stiletto high heels, but you're not sure you're ready for them. Wearing them would only perk up your noticeable posterior even more. That your " + breastDescriptOfRow(game.player, 0) + " contrast above a slender waist is enough for now. Maybe on special occasions.\n\n", false);

    outputText("You blush as the wind breezes by, and with the exception of the upper jacket, feel like you're wearing nothing at all. This sensation is only aggravated when you can't help but slip a finger across your inner thigh and feel it glide up effortlessly across the textile. It's like you've only become more sensitive for wearing it. Much to your amazement, the triple layer across your " + vaginaDescript(player, 0) + " doesn't provide quite as much protection as you assumed earlier; at least not so much against roving fingers. You find the fabric across your womanhood has a hidden opening to it. Rubbing through it is enough to part the velvet folds and set your finger upon your own. It's not apparent, but anyone aware of this shameful split would have easy access to your depths without even disrobing you. You feel nude.\n\n", false);
    ...
    Inventory.takeItem(ArmorLib.R_BDYST, playerMenu);
}
...
function RaphaelEncounterIIDressFollowup(): void {
    ...
    // ({If player does not meet the first encounter requirements:}
        ...
        // ({If player has no legs, or a centaur body.}
        if (player.lowerBody == LowerBodyType.NAGA || player.lowerBody == LowerBodyType.CENTAUR || player.lowerBody == LowerBodyType.GOO || player.lowerBody == LowerBodyType.PONY)
            outputText("\"<i>You're missing half your body!</i>\" He refers to your morphed legs.\n\n", false);
        // ({If player has above E cup breasts}
        if (player.breastRows.biggestTitSize() >= 7) outputText("\"<i>Your female curves... replaced with such... udders!</i>\" He looks at your bosom. \"<i>No woman could be elegant with such monstrosities up front!</i>\"\n\n", false);
        // ({If player has below C cup breasts}
        if (player.breastRows.biggestTitSize() < 3) outputText("\"<i>Your female curves... gone!</i>\" He looks at your bosom. \"<i>It's hard to tell you apart from a little girl!</i>\"\n\n", false);
        // ({If player has grown less than girly hips}
        ...
        // ({If player has grown ANY cock and balls}
        if (player.balls > 0 || player.cocks.length > 0) outputText("\"<i>What is that bulge below the tight outfit I gave you?</i>\" The fox inspects your groin. \"<i>No, never mind. I don't want to know.</i>\"\n\n", false);
        // (For now:
        // ({If player has lost all gender}
...
function RaphaelPicnicSkill(): void {
    ...
    // {Fencing practice variables: Speed 30-39 Must play out at least once}
        ...
        outputText("You gasp.  For the first time, Raphael forbids you something with harsh tone.  It's quite the departure.  You hang limply from his grasp, your " + breastDescriptOfRow(game.player, 0) + " squashed against his chest like dough.\n\n", false);
    ...
    // {Fencing practice variables: Speed 40-49, Must play out at least once}
        ...
        outputText("<i>\"I think you have found the proper posture now.\"</i> he comments austerely, holding you by the " + breastDescriptOfRow(game.player, 0) + " once more to make sure the angle of your back is correct. You push your rear a bit deeper into the fox's loins without needing to be guided in, enjoying the heat pulsing off the seat upon his manhood and how shamelessly you're grinding into him.  You actually begin to wonder if he himself doesn't notice and you struggle to contain the urge to wiggle around and bring attention to it.\n\n", false);
...
function RaphaelPostFenceSex(): void {
    ...
    outputText("When Raphael notices your attention, the time seems right for one of his one-liners.  'Do not fear the blade', 'look how the length stands firm upon the hilt', a lecture on the art of parrying, or the like is not forthcoming, however.  The fox says nothing instead and merely smiles knowingly at you from the side, knowing silent action is enough.  With his one remaining free arm, he claws around the silk of your womanhood and does indeed part the subtle opening of your interlapping folds. For a moment you gasp as his hot, slick cock falls freely into the denuded skin of your quaking " + vaginaDescript(player, 0) + ".  It shouldn't come as a surprise that he knows of the secret opening in the clothes: he gifted them to you after all, perhaps planning it all along.\n\n", false);
...
function postRaphaelCoitus(): void {
    ...
    if (flags[kFLAGS.RAPHAEL_RAPIER_TRANING] == 4) {
        ...
        Inventory.takeItem(WeaponLib.RRAPIER, playerMenu);
    }
...
function RaphaelThieverySmex(): void {
    ...
    outputText("Finally the fragile tension between you breaks.  Raphael undoes your zipper and roughly pulls your bodysuit down, revealing your " + allBreastsDescript(player) + ". You're stunned; the fine, abrasive texture of his nimble fingers' pads run in eager circles across your sensitive neck, from whence moans have begun to flow. His soft fur brushes across your body as the fox moves downwards.  Gingerly, he puts his maw down on your right breast and suckles once, but in the span of an agonizingly long time.  The " + nippleDescription(player, 0) + " is sucked into his toothy mouth, which he puts down on the nub carefully.  The little points scrape by sensually, before your nipple slips through them.\n\n", false);

    outputText("You gasp when he spreads your legs by placing his in between and parting them. When he fondles you down there, fingers rubbing into your flushed  " + vaginaDescript(player, 0) + ", you suddenly feel the wind passing through the hot bare inners of your parted folds.  Raphael has opened the silken opening of your suit. It shouldn't come as a shock.  He gifted them after all, perhaps planning for it all along.  Another surprise follows as you can feel something hot and rigid standing off his body and lying on top of yours. Raphael's vulpine cock is resting in the hollow of your bellybutton, through the fly of his pants.  The bright red, smooth tip rides up your tummy admiringly.  It lacks the mushroom shaped dome that human men have and instead, his cock is pointed and tapered, much like the weapons he prefers.  You can also feel a subtle, but noticeable canine bulb at the base, throbbing against your sensitive loins.\n\n", false);
...
// Intro
export function orphanageIntro(): void {
    clearOutput();
    if (RaphaelLikes()) {
        ...
        else if (game.time.hours >= 19 && game.time.hours <= 21) {
            // Repeat Approaches
...
function kissRaphaelFirstTimeOrphanage(): void {
    ...
    cuntChange(player, 5, true, true, false);
...
function cunnilingusWithRaphael(): void {
    ...
    outputText("\n\nWith his paws, Raphael raises up your butt and crotch just a few inches into the air, as if elevating it for a special occasion.  It encourages you to bend your knees and spread your legs, opening yourself up and presenting your womanhood fully.  Lacking all shame and still looking you into the eyes, the fox then does the lewdest thing.  Maintaining eye contact and without blinking, he rolls out his long red tongue and brushes it against your " + vaginaDescript(player, 0) + ".  The sensation of the bawdy rough thing lapping up your sensitive gash's gathered dew takes you completely by surprise and you let your head fall back and moan.  It feels like he's worshipping you by lapping through the trembling fruit.  At the very zenith of his lashes, he takes extra time to twirl around your " + clitDescription(player) + " in a vortex of swirling tastebuds.");
...
function girlOnTopOfRedFoxesOhMy(): void {
    ...
    outputText("\n\nRaphael's cock is something else. It must be the bone running through it that gives it its firmness.  Instead of stuffing yourself with a nondescript roll of meat, you can clearly feel him penetrating you with his well defined and rock-hard shaft.  With the faintest twitch of the hip, you can change its angle and run it through in an entirely different manner and you eagerly start experimenting with what feels best.  Eventually you settle on jutting your ass backwards and pressing the middlemost of his solid length against the front wall of your [vagina] while the tip bottoms out against the back.  The position does not go unnoticed to Raph, and he trails his paws along your " + buttDescription(player) + ".  You begin to bounce around on top him, jamming him into you more firmly and more securely with every rut in.  Raphael supports you by either grabbing you by the ass or cupping your tits, playing with them with his sandy paws.  He sinks in heavily, smoothly, with every gulp of your tightening cunt accompanied by a little wet slush around his hard manhood.");

    outputText("\n\nDesperate for that little bit more, you reach down and start fondeling your " + clitDescription(player) + " too.  Your button brushing in against Raphael's soft white pubes tickles.");
