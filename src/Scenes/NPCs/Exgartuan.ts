...
private  drinkFountainEndowment():void {
	...
	if(rand(3) == 0 && player.totalCocks() > 0) {
		outputText("\n\nYour " + multiCockDescriptLight() + " feels tighter inside your " + player.armorName + ", even when flaccid.  You shudder and realize you've probably gained more than a few inches in total length, and who knows how your thickness has changed.", false);
...
public  exgartuanBored():void {
	...
	if(player.statusAffectv1(StatusAffects.Exgartuan) == 1 && player.cockArea(0) >= 100) {
		...
		if(select == 0) {
		...
		else if(select == 4) outputText("You feel your " + cockDescript(0) + " slowly expand and contract, air escaping its urethra and tickling your nether regions.  Did... did your dick just sigh?  Your demon is growing impatient.", false);
			...
			outputText("Your " + cockDescript(0) + " has started rubbing against your thighs as a familiar voice chimes in, \"<i>I've got a hot, steaming batch of cum ", false);
		...
		switch(select) {
			case 0:
			 	outputText("You feel an ominous jiggling in your " + breastDescript(0) + " as the demon inside you stirs back to wakefulness.  The sound of a yawn being muffled by your " + player.armorName + " confirms it.", false);
				break;
			case 1:
...
public  exgartuanArmorShift():void {
	...
	if(player.armor == armors.BEEARMR) {
		...
		outputText("Your leather armor shifts, pressing tightly against your upper " + player.legs() + " and molding itself around your " + cockDescript(0) + " to prominently display it.", false);
...
public  exgartuanBeeRape():void {
	...
	outputText("\"<i>You sure know your knots!   Speaking of knots, I got an idea...</i>\" exclaims the demon inside your " + cockDescript(0) + ".  You wonder at what the demon could have in mind this time as you finish spreading the bee-bitch's legs in preparation.  The scent wafting out from her genitals is unreal, making you simultaneously hungry and incredibly horny.  You pull up the ginormous flanged tip with both hands and rub it over her honey-slicked crotch, mixing your demonic pre-seed with her honey as you smear it all over her downy inner thighs.\n\n", false);
...
private  boobgartuanSurprise2():void {
	...
	if(flags[kFLAGS.BOOBGARTUAN_SURPRISE_COUNT] == 0) {
		...
		//[if corruption <30]
		if(player.cor < 33) outputText("Your " + chestDesc() + " brush against the imp's enormous scrotum, causing him to eventually tumble over from the sensation, blocking the impeding entourage of imps behind you.", false);
		//[if corruption ≥30 & <70]
...
private  boobgartuanSurprise3():void {
	...
	//[if occurrence ≥8]
	else outputText("Before another thought can cross your mind, your " + chestDesc() + " fly up and smack you in the gob! \"<i>" + player.short + ", it's that time again,</i>\" the eager demoness teases you.  You think she's grown to enjoy the torture she puts you through.  \"<i>If you insist on going day in and day out without spending quality time with these tits, then I'll just insist on instructing you otherwise.</i>\" Your beautiful bosom launches forward, throwing your mug right back in the dirt.", false);
	...
	outputText("  You finally bear down and grasp onto... nothing?  <b>Your nipples are nowhere to be found!</b>  The passion is beginning to flatline as you confusedly grope around your " + chestDesc() + ".  \"<i>What's wrong, champion?</i>\" Exgartuan asks knowingly, \"<i>Is this too hard for you?</i>\" The tips of your fingers comb around your areolas until you discover an inward bend in your " + player.skin() + " where your nipples should be.  Apparently, Exgartuan isn't through having fun with you; the blasted seductress managed to suck your teats inward!  Before you can fathom just how she pulled it off without your knowledge, your humble howitzers start to tingle.  You feel an anxious quiver work its way up your spine before a familiar need begins to course through your being: the need to be milked!", false);
	//[if corruption <50]
	...
	//[if not lactating]
	if(player.biggestLactation() < 1 || player.lactationQ() == 0) outputText("  <b>Your " + chestDesc() + " have started to lactate profusely.</b>", false);
	...
	outputText("Your " + chestDesc() + " begin to rumble and quake, filling you with a mix of anxiety, dread, and passion.  One moment later, an enormous force bursts forth against your hands; your nipples have popped back out, soaking and ready to - wait a second...", false);
...
public  exgartuanNagaStoleMyMasturbation():void {
	...
	//[else]
		...
		outputText("  Your " + cockDescript(0) + " makes good on its end of the bargain by pumping out its own never-ending supply of lubricant, making your master massage all the more slick and smooth along its surface.  You lay off for a moment to admire your work, your demonic dong throbbing happily now that its full of blood and as tall and thick as a tree.  The corrupt warmth and comfort flowing through your body pleases you to no end.\n\n", false);
