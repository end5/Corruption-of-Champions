...
export class MarbleScene implements TimeAwareInterface {
    ...
    // trace("Marble Stats: Aff-"+player.effects.getValue1Of(StatusAffects.Marble)+" Add-" + player.effects.getValue2Of(StatusAffects.Marble) + " isA-" + player.effects.getValue3Of(StatusAffects.Marble) + ".");
    ...
    */
    ...
    public timeChange(): boolean {
        ...
        trace("\nMarble time change: Time is " + game.time.hours + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
            ...
            // Marble find items check
                ...
                if (player.effects.findByType(StatusAffects.MarbleItemCooldown) < 0) {
                    if (rand(10) == 0) {
                        ...
                        player.effects.create(StatusAffects.MarbleItemCooldown, (24 + rand(24)), 0, 0, 0);
                        player.effects.create(StatusAffects.MarbleHasItem, rand(10), 0, 0, 0);
                        needNext = true;
                    }
                }
            }
        ...
        if (player.effects.findByType(StatusAffects.MarbleItemCooldown) >= 0) {
            player.effects.addValue(StatusAffects.MarbleItemCooldown, 1, -1);
            if (player.effects.getValue1Of(StatusAffects.MarbleItemCooldown) < 1) player.effects.remove(StatusAffects.MarbleItemCooldown);
        }
        if (player.effects.findByType(StatusAffects.Infested) < 0) flags[kFLAGS.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION] = 0;
        if (player.effects.findByType(StatusAffects.MarblesMilk) >= 0 && player.perks.findByType(PerkLib.MarblesMilk) < 0) {
            // Decrement time remaining by 1
            player.effects.addValue(StatusAffects.MarblesMilk, 1, -1);
            // Remove the status and stat boosts when time runs out on the milk
            if (player.effects.getValue1Of(StatusAffects.MarblesMilk) <= 0) {
                needNext = true;
                dynStats("str", (-1 * player.effects.getValue2Of(StatusAffects.MarblesMilk)), "tou", (-1 * player.effects.getValue3Of(StatusAffects.MarblesMilk)));
                player.effects.remove(StatusAffects.MarblesMilk);
                // Text for when Marble's Milk effect wears off:
                // [addiction is 10 or less]
                if (player.effects.getValue2Of(StatusAffects.Marble) <= 10) outputText("\nYou feel the euphoria from drinking Marble's milk fade from you. Only now that it's gone do you notice that it was actually making you tougher.\n", false);
                // [addiction is 11-30]
                else if (player.effects.getValue2Of(StatusAffects.Marble) <= 30) outputText("\nYou feel a slight sense of loss as the euphoria from Marble's milk fades.  You kinda want to drink more, but the desire is not overpowering.\n", false);
                // [addiction is 31-50, player is not addicted]
                else if (player.effects.getValue2Of(StatusAffects.Marble) <= 50) outputText("\nYou shiver slightly as the euphoria from Marble's milk fades.  You really feel like suckling her breasts again.\n", false);
                // IF ADDICTED
                if (player.effects.getValue3Of(StatusAffects.Marble) > 0) {
                    ...
                    if (player.effects.findByType(StatusAffects.BottledMilk) >= 0) {
                        ...
                        player.effects.create(StatusAffects.MarbleWithdrawl, 0, 0, 0, 0);
        ...
        if (player.effects.getValue3Of(StatusAffects.Marble) > 0 && player.perks.findByType(PerkLib.MarbleResistant) < 0 && player.perks.findByType(PerkLib.MarblesMilk) < 0 && player.effects.getValue2Of(StatusAffects.Marble) > 25) {
            // If player does not have marble's milk or bottled milk, go into withdrawl
            if (player.effects.findByType(StatusAffects.MarblesMilk) < 0 && player.effects.findByType(StatusAffects.BottledMilk) < 0) {
        ...
        if (player.effects.getValue3Of(StatusAffects.Marble) <= 0) {
            if (player.effects.getValue2Of(StatusAffects.Marble) > 60) player.effects.setValue(StatusAffects.Marble, 2, 60);
        }
        // Withdrawl removal if you get unaddicted.
        if (player.effects.findByType(StatusAffects.MarbleWithdrawl) >= 0) {
            if (player.effects.getValue2Of(StatusAffects.Marble) <= 25) {
            ...
            else if (player.effects.findByType(StatusAffects.BottledMilk) >= 0) {
                outputText("\nYou no longer feel the symptoms of withdrawal.\n", false);
                needNext = true;
                player.effects.remove(StatusAffects.MarbleWithdrawl);
                dynStats("tou", 5, "int", 5);
        ...
        if (player.effects.findByType(StatusAffects.BottledMilk) >= 0) {
            player.effects.addValue(StatusAffects.BottledMilk, 1, -1);
            if (player.effects.getValue1Of(StatusAffects.BottledMilk) <= 0) player.effects.remove(StatusAffects.BottledMilk);
        }
        if (game.time.hours > 23) {
            flags[kFLAGS.MARBLE_PLAYED_WITH_KIDS_TODAY] = 0;
            if (player.effects.findByType(StatusAffects.Marble) >= 0 && player.effects.getValue2Of(StatusAffects.Marble) > 0) marbleStatusChange(0, -1);
        }
        return needNext;
    }
    ...
    public timeChangeLarge(): boolean {
        ...
        if (player.effects.getValue3Of(StatusAffects.Marble) > 0 && player.effects.getValue2Of(StatusAffects.Marble) < 25 && player.perks.findByType(PerkLib.MarblesMilk) < 0 && player.perks.findByType(PerkLib.MarbleResistant) < 0 && game.time.hours == 6) {
            spriteSelect(41);
            ...
            if (player.effects.getValue3Of(StatusAffects.Marble) == 1) {
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) < 30) {
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) >= 30 && player.effects.getValue1Of(StatusAffects.Marble) < 90) {
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) >= 90) {
            ...
            // Ashamed Marble
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) >= 30 && player.effects.getValue1Of(StatusAffects.Marble) < 70) {
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) >= 70) {
        ...
        if (player.effects.getValue3Of(StatusAffects.Marble) > 0 && player.effects.getValue2Of(StatusAffects.Marble) >= 100 && player.perks.findByType(PerkLib.MarblesMilk) < 0 && player.perks.findByType(PerkLib.MarbleResistant) < 0 && game.time.hours == 6) {
            spriteSelect(41);
            ...
            if (player.effects.findByType(StatusAffects.MarblesMilk) >= 0) {
                player.effects.remove(StatusAffects.MarblesMilk);
            }
            ...
            if (player.effects.getValue3Of(StatusAffects.Marble) == 1) {
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) < 49) {
                ...
                if (player.effects.getValue1Of(StatusAffects.Marble) >= 50 && player.effects.getValue1Of(StatusAffects.Marble) < 79) {
                ...
                // High affection
                if (player.effects.getValue1Of(StatusAffects.Marble) >= 80) {
            ...
            // Marble didn't like you addicted
                ...
                // Affection < 80, type 2:
                else if (player.effects.getValue1Of(StatusAffects.Marble) < 80) {
        ...
        if (checkedMarbleMilk++ == 0 && game.time.hours == 6 && player.perks.findByType(PerkLib.MarblesMilk) >= 0) {
            ...
            // Marble isn't at camp
                ...
                if (player.effects.findByType(StatusAffects.NoMoreMarble) < 0) {
                    postAddictionFarmMornings();
                    timeQ++; // We can get rid of this: threshhold--;
                }
            }
            doNext(playerMenu);
            return true;
...
export function marbleBreastSize(): string {
    if (flags[kFLAGS.MARBLE_PURIFIED] > 0) {
        ...
        if (player.effects.getValue4Of(StatusAffects.Marble) > 30) return "HH cup";
        else return "G cup";
    }
}
...
function apologizetoWalkingTitsIMEANMARBLE(): void {
    ...
    else outputText("It isn't too hard to figure out that Marble isn't really trying, and you easily catch her off guard on one of her double backs.  She doesn't even notice you until you peek out from between the stalks next to her, reaching out and getting a handful of her backside.  \"<i>Clever " + mf(player, "boy", "girl") + "...</i>\" she says.");
...
function rapeMarbleInHerRoom(): void {
    ...
    if (player.cocks.length > 0) {
        outputText("Chuckling to yourself, you free your " + multiCockDescriptLight(game.player) + " from your " + player.armorName + ".  You spend a moment to ask the helpless cow-girl if she is ready, her only response being a whimper, before ", false);
        ...
        if (player.cocks[0].cockThickness < 3) {
            ...
            if (player.cocks[0].cockLength <= 8) {
                outputText("forcing your " + cockDescript(game.player, 0) + " in as far as it will go.  ", false);
            } else {
                outputText("forcing your " + cockDescript(game.player, 0) + " in to the hilt.  ", false);
            }
            // the raping proper
        ...
        // now if the player doesn't fit
            ...
            outputText("attempting to push your " + cockDescript(game.player, 0) + " inside her.  Of course, the girth of your " + cockDescript(game.player, 0) + " makes this a rather difficult operation, and it becomes clear after a few moments that it just isn't going to fit.  Instead, you contend yourself by rubbing yourself between her ample ass cheeks, occasionally stroking your " + multiCockDescriptLight(game.player) + " in pride.\n\n", false);
            // is the player corrupt enough to get the fantasy?
            if (player.cor >= 33)
                marbleRapeCorruptFantasy();
...
export function encounterMarbleExploring(): void {
    ...
    // [affection >=30]
        ...
        if (player.effects.getValue2Of(StatusAffects.Marble) < 40) {
...
function drinkMarbleMilk(): void {
    ...
    if (player.earType > EarType.HUMAN) outputText(" and occasionally scratching behind your ears", false);
    outputText(".  ", false);
    ...
    if (player.effects.getValue2Of(StatusAffects.Marble) >= 50) {
...
export function encounterMarbleExploring2(): void {
    ...
    // [addiction >30]
    if (player.effects.getValue2Of(StatusAffects.Marble) > 30) {
    ...
    // [affection <60]
    if (player.effects.getValue1Of(StatusAffects.Marble) < 60) {
...
function turnDownMarbleSexFirstTime(): void {
    ...
    player.effects.setValue(StatusAffects.Marble, 1, 50);
    // (increase player inte)
    dynStats("int", 4);
    doNext(Camp.returnToCampUseOneHour);
}
...
function AcceptMarblesSexualAdvances(): void {
    ...
    standardSex(true);
    if (player.effects.findByType(StatusAffects.FuckedMarble) < 0) player.effects.create(StatusAffects.FuckedMarble, 0, 0, 0, 0);
...
export function addictedEncounterHappy(clearS: boolean = true): void {
    ...
    if (player.effects.findByType(StatusAffects.MalonVisitedPostAddiction) < 0) {
        ...
        player.effects.create(StatusAffects.MalonVisitedPostAddiction, 0, 0, 0, 0);
    }
    // Return visits
    else {
...
function playerAgreesToDrinkMarbleMilkBottled(): void {
    ...
    if (player.effects.findByType(StatusAffects.BottledMilk) >= 0) {
        player.effects.addValue(StatusAffects.BottledMilk, 1, (3 + rand(6)));
    }
    else player.effects.create(StatusAffects.BottledMilk, 8, 0, 0, 0);
    doNext(Camp.returnToCampUseOneHour);
}
...
function playerRefusesToDrinkBottledMilk(): void {
    ...
    else player.effects.create(StatusAffects.BottledMilk, 3, 0, 0, 0);
    doNext(Camp.returnToCampUseOneHour);
}
...
function withdrawalDelay(): void {
    if (player.effects.findByType(StatusAffects.BottledMilk) >= 0) {
        ...
        player.effects.create(StatusAffects.BottledMilk, 3, 0, 0, 0);
    }
...
export function marbleEncounterAddictedNonWithdrawlAshamed(): void {
    ...
    // [affection >= 30]
    if (player.effects.getValue1Of(StatusAffects.Marble) >= 30) outputText("worriedly ", false);
    outputText("for a moment before it dawns on her that you aren't shaking.\n\n", false);
    // [affection >= 30]
    if (player.effects.getValue1Of(StatusAffects.Marble) >= 30) outputText("\"<i>Sweetie, w", false);
...
function extendedMurbelFarmTalkz(): void {
    ...
    switch (flags[kFLAGS.MURBLE_FARM_TALK_LEVELS]) {
        case 0:
            ...
            else if (race(player) == "cow-morph" || race(player) == "cow-boy") {
            ...
            // [if PC is a dogmorph]
            else if (race(player) == "dog-morph") outputText("  \"<i>I've seen lots of dog-morphs before, are you from Barkersvile?</i>\"");
            // [if PC is a centaur]
        ...
        case 3:
            ...
            if (player.effects.getValue1Of(StatusAffects.Marble) >= 60) outputText("  \"<i>Even though the start of the trip didn't turn out all that well, I think I found what I was looking for.</i>\"");
        ...
        default:
            ...
            if (player.effects.findByType(StatusAffects.CampMarble) < 0) outputText(", except that mission of yours that's so important to you");
            outputText(".</i>\"");
            ...
            if (player.effects.findByType(StatusAffects.CampMarble) < 0) {
                ...
                if (player.effects.getValue3Of(StatusAffects.Marble) == 2) outputText("  I really just wanted someone who loved me and loved nursing from my breast.  I had no idea what that would do to you.");
                // [if addiction quest is active, and Marble is happy with her milk]
                else if (player.effects.getValue3Of(StatusAffects.Marble) == 1) outputText("  But I think that I've found the one who will change that.  Right sweetie?");
...
export function marbleSex2Continued(genders: number): void {
    ...
    // (player is male)
    if (genders == 1) {
        outputText("Marble leans back and invites you to come.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you are on top of her body. She puts her hands around you and lies back completely.  You carefully line up your " + cockDescript(game.player, 0) + " and thrust into her warm snatch.  ", false);
        ...
        outputText(".  Deep within her, your " + cockDescript(game.player, 0) + " explodes.  Within moments, Marble feels her orgasm too, her grip intensifying immensely.  You hear her give a sigh that sounds almost like a moo and her grip relents.  The two of you lay there panting, your " + cockDescript(game.player, 0) + " still inside her.\n\n", false);
    }
    // (player is female, I hope I did this right, I'm not familiar with the exact mechanics of female/female toys)
    else {
        outputText("Marble grins at you and reaches into the drawers of the mini-dresser next to her bed, pulling out a long, double-ended dildo. She slips one half into her own hole and invites you over to her.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you slip your " + vaginaDescript(player, 0) + " onto the other end of the dildo.  Sighing, Marble lies down underneath you and begins to rock back and forth.  You bring your own ministrations to the mix in response. Soon, the two of you are panting and moaning against each other with the dildo sliding around between you.  Suddenly, she locks her arms around you tightly and ", false);
        // [player is between 4 and 5 feet in height]
    ...
    if (player.effects.findByType(StatusAffects.FuckedMarble) < 0) {
        ...
        player.effects.create(StatusAffects.FuckedMarble, 0, 0, 0, 0);
    }
    // (repeat sex)
    else {
...
function marbleMilkSex(newpage: boolean = true): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.length == player.cocks.horseCocks() + player.cocks.dogCocks() + player.cocks.normalCocks()) {
            outputText("Marble seems pleased at the sight of your " + multiCockDescriptLight(game.player) + " and she pushes you back onto her bed.  She removes her own skirt and stops for a moment at her tail.  Giggling slightly, she uses her ribbon-tied tail to brush at your " + multiCockDescriptLight(game.player) + " before climbing on top of you and slipping her legs to your sides.\n\n", false);
        }
        ...
        if (player.cocks.length > 1) outputText("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your " + multiCockDescriptLight(game.player) + ".  Before you have a chance to answer, Marble grabs a hold of your central " + cockDescript(game.player, 0) + " and says \"<i>I'll bet it's this one.</i>\"  ", false);
        outputText("Without much hesitation, Marble lifts herself up, and impales her moist lips upon your " + cockDescript(game.player, 0) + ".  The two of you gasp as you sink into her interior.  You feel each inch slowly flow inside her until ", false);
        // [dick(0) is less than 8 inches long]
        if (player.cocks[0].cockLength < 8) outputText("every bit of your " + cockDescript(game.player, 0) + " is deep inside.  ", false);
...
export function marbleStatusChange(affection: number, addiction: number, isAddicted: number = -1): void {
    ...
    if (player.perks.findByType(PerkLib.MarblesMilk) < 0 && player.perks.findByType(PerkLib.MarbleResistant) < 0) {
        player.effects.addValue(StatusAffects.Marble, 1, affection);
        player.effects.addValue(StatusAffects.Marble, 2, addiction);
    }
    if (isAddicted != -1) player.effects.setValue(StatusAffects.Marble, 3, isAddicted);

    trace("Marble Affection: " + player.effects.getValue1Of(StatusAffects.Marble));
...
function applyMarblesMilk(): void {
    ...
    if (player.effects.findByType(StatusAffects.MarblesMilk) < 0) {
        player.effects.create(StatusAffects.MarblesMilk, 12, 0, 0, 0);
        if (player.str + 5 > 100) {
            str = 100 - player.str;
            if (str < 0) str = 0;
        }
        ...
        player.effects.setValue(StatusAffects.MarblesMilk, 2, str);
        player.effects.setValue(StatusAffects.MarblesMilk, 3, tou);
    }
    else {
        player.effects.addValue(StatusAffects.MarblesMilk, 1, 12);
    }
    // Prevent duration from going to high.
    if (player.effects.getValue1Of(StatusAffects.MarblesMilk) > 36) player.effects.setValue(StatusAffects.MarblesMilk, 1, 36);
...
I will refer to it as player.effects.getValue4Of(StatusAffects.Marble), replace it with what you will use.
...
export function interactWithMarbleAtCamp(): void {
    ...
    let sexEvent: () => void = null;
    let playtime: () => void = null;
    let marbleGoEvent: () => void = null;
    let goEventString: string = "";
...
function marbleTalkOverhaul(): void {
    ...
    if (player.cor < 50 && player.effects.getValue4Of(StatusAffects.Marble) < 60) addButton(1, "Just Talk", talkWithMarbleAtCamp);
...
function marbleKidsPlaytime(): void {
    ...
    if (select == 1) {
        if (flags[kFLAGS.MARBLE_KIDS] == 1 && flags[kFLAGS.MARBLE_BOYS] == 1) {
            ...
            outputText("Your daughter immediately clops up to you and jumps into your arms.  \"<i>" + mf(player, "Daddy, daddy", "Mommy, mommy") + "!  Play with me!</i>\"  You laugh at the girl in your arms, so young and yet full of energy.  She proceeds to rope you into a grand adventure, where you play the parts of the one who searches for a hero to save their camp, and the monster that has to be stopped, and also the monster's hostage that needs to be saved, while she is the great bovine saviour that carries out this grand quest.");
            outputText("\n\nThe game finally ends with the bovine saviour victorious over the horde of ravenous monsters  - somewhere along the line it was decided that hostages were no fun and single combat wasn't impressive enough, and you exhausted your theatrical skills to provide a bevy of satisfyingly scary faces and growls - and the people of the land (you) rejoicing.  You're left a bit tired afterwards, but you certainly don't regret having played with your child.");
        }
    ...
    if (select == 5) {
        ...
        outputText("\n\n\"<i>" + mf(player, "Daddy", "Mommy") + "!  Is this it?  Is this it?</i>\"");
        ...
        outputText("\n\nYour kids are just a little bit tired from all the work and it doesn't seem they'll pester Marble much more today.  One of them even grabs you, complaining, \"<i>" + mf(player, "Daddy", "Mommy") + ", I'm sleepy!</i>\"  You pat her head and smile, telling her that you'll be home soon enough.");
...
function talkWithMarbleAtCamp(): void {
    ...
    if (player.effects.findByType(StatusAffects.MetMarae) >= 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 1) {
    ...
    else if (player.effects.findByType(StatusAffects.FoundFactory) >= 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 2) {
    ...
    else if (player.effects.findByType(StatusAffects.DungeonShutDown) >= 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 3) {
    ...
    else if (player.effects.findByType(StatusAffects.MaraeComplete) >= 0 && player.effects.findByType(StatusAffects.FactoryOverload) < 0 && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 4) {
    ...
    else if (player.effects.getValue1Of(StatusAffects.TelAdre) >= 1 && flags[kFLAGS.MARBLE_TELADRE_STORY] == 0) {
    ...
    if (player.effects.findByType(StatusAffects.MetMarae) < 0) {
    ...
    else if (player.effects.findByType(StatusAffects.MetMarae) >= 0 && player.effects.findByType(StatusAffects.FoundFactory) < 0) {
    ...
    else if (player.effects.findByType(StatusAffects.FoundFactory) >= 0 && player.effects.findByType(StatusAffects.DungeonShutDown) < 0)
    ...
    else if (player.effects.findByType(StatusAffects.DungeonShutDown) >= 0 && player.effects.findByType(StatusAffects.MaraeComplete) < 0 && player.effects.findByType(StatusAffects.MetCorruptMarae) < 0)
    ...
    else if (flags[kFLAGS.DISCOVERED_DUNGEON_2_ZETAZ] == 0 || player.effects.getValue1Of(StatusAffects.TelAdre) == 0)
    ...
    else if (flags[kFLAGS.DISCOVERED_DUNGEON_2_ZETAZ] == 0 && player.effects.getValue1Of(StatusAffects.TelAdre) >= 1)
...
function marbleGathered(): void {
    ...
    if (flags[kFLAGS.MARBLE_PURIFICATION_STAGE] != 1 && player.effects.getValue1Of(StatusAffects.MarbleHasItem) <= 4)
        Inventory.takeItem(ConsumableLib.VITAL_T, Camp.returnToCampUseOneHour);
    // Tanned Leather clothes, armor, def: 5 (three days)
    else if (flags[kFLAGS.MARBLE_PURIFICATION_STAGE] != 1 && player.effects.getValue1Of(StatusAffects.MarbleHasItem) <= 7)
        Inventory.takeItem(ArmorLib.LEATHRA, Camp.returnToCampUseOneHour);
    ...
    else Inventory.takeItem(ConsumableLib.LABOVA_, Camp.returnToCampUseOneHour);
    player.effects.remove(StatusAffects.MarbleHasItem);
}
...
function marbleCampSexNew(): void {
    ...
    if (!player.isNaga()) {
        ...
        if (player.perks.findByType(PerkLib.MarblesMilk) >= 0 && player.gender > 0 && rand(2) == 0) {
            ...
            // ( [Male/Herm]
            if (player.cocks.length > 0) {
                outputText("She chuckles as her hand reaches your " + multiCockDescriptLight(game.player) + ".  \"<i>You seem quite ready and eager for something, sweetie.</i>\"");
            }
            else {
                outputText("She chuckles as her hand reaches your " + vaginaDescript(player, 0) + ".  ");
                // [Vaginal wetness 0-1]
                if (player.vaginas.wetness() < 2) outputText("\"<i>Oh, sweetie, this just won't do!  It's like you're not looking forward to this at all!</i>\"");
                // [Vaginal wetness 2-3]
                else if (player.vaginas.wetness() < 4) outputText("\"<i>It seems you're quite in need of something down here, sweetie...</i>\"");
                // [Vaginal wetness 4]
                else if (player.vaginas.wetness() < 5) outputText("\"<i>My, my, sweetie; this is so wet... are you sure we should stimulate it more?</i>\"");
                // [Vaginal Wetness 5]
            ...
            if (player.cocks.length > 0) {
                ...
                // [If balls] : A
                if (player.balls > 0) outputText("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your " + ballsDescriptLight(player) + " with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\"");
                // ([If pussy] :
                if (player.vaginas.length > 0) outputText("  From the base of your cock, her hand moves even lower, gently rubbing your " + clitDescription(player) + ".  \"<i>This seems to also require some attention...  Maybe later, sweetie, if you do a good job nursing from me.  But right now, I want to see you shoot off,</i>\" she says, as her hand moves back to your " + multiCockDescriptLight(game.player) + ".");
                outputText("\n\nShe starts tugging at and stroking your " + cockDescript(game.player, 0) + ", eager to make you cum now.  You buck your hips again and she responds more agressively this time, pushing you back down with her breasts as her touch starts to become unbearable.  \"<i>You're close, aren't you, sweetie?  It's okay, I want to see you cum.</i>\"");
            ...
            // [Female]
                ...
                outputText("\n\nYou're at the edge already, and the fingering of your " + vaginaDescript(player, 0) + " as well as the rubbing motion against your [clit] soon send you over it.  Thrashing in Marble's embrace, you release your feminine juices all over her hand.");
                if (player.vaginas.wetness() >= 4) outputText("  Messy as it is, Marble never stops, wanting to make the orgasm as intense as possible.");
                // ([Squirter]
                if (player.vaginas.wetness() > 4) outputText("  Soon, you're shooting your femcum all over her arm, and she giggles at the intensity and visibility of your orgasm.");
        ...
        // ORAL
            ...
            if (player.cocks.length > 0) outputText("a tent growing in your " + player.armorName);
            else if (player.vaginas.length > 0) outputText("a moistness growing under your " + player.armorName);
            ...
            if (player.cocks.length > 0) {
                ...
                outputText("your " + clitDescription(player) + " quivers in arousal");
            }
            outputText(".\n\n");
            ...
            // [if cock]
            if (player.cocks.length > 0) {
                outputText("  Her lips tightly enclose your " + cockDescript(game.player, 0));
                if (player.cocks.length > 1) outputText(" while she strokes another");
                outputText(", acting as an organic cock-ring.  She looks up to you as you watch her lips undulating under the repeated pulsations of your spooge-pumping " + cockDescript(game.player, 0) + ".  These cocksuckers are teasing you in the most infuriating way, and you savagely shove more of your prick down her throat, stretching her jaw and making her moan in delight, although her cries of pleasure are muffled by the pecker stuffing her mouth.");
            }
            // [if vagina]
            else outputText("  Her flexible, yet firm tongue really feels like a dong and it keeps provoking you with its prolonged digging. With a mighty thrust, you shove her against your " + vaginaDescript(player) + ", forcing the entirety of her oral muscle inside.");
            ...
            if (player.cocks.length > 0) {
                ...
                if (player.cumQ() >= 500) outputText("  Your " + cockDescript(game.player, 0) + " keeps spouting more seed inside her, efficiently distending her belly");
            ...
            // [if vagina]
            else outputText("\n\nYour " + vaginaDescript(player, 0) + " quivers and vibrates before releasing a vigorous discharge of fem-spunk, splattering Marble's lips and mouth; she drinks all your juices with an insatiable cum-thirst.  Her tongue explores your insides as if to make sure there's no girlcum left to absorb.");
            ...
            if (player.cor + player.effects.getValue4Of(StatusAffects.Marble) < 66) {
                outputText("The two of you find a secluded place in the camp and slip off your clothes.\n\n", false);
            }
            else if (player.cor + player.effects.getValue4Of(StatusAffects.Marble) < 132) {
            ...
            if (player.cocks.length > 0) {
                outputText("\n\nShe moves down to your crotch and slips your " + cockDescript(game.player, 0) + " between her breasts.  She smiles at you and says, \"<i>This is what you were hoping for, right?</i>\" before squeezing them together tightly and pushing your " + cockDescript(game.player, 0) + " in and out.  You sigh in pleasure and smile down at Marble as she pumps you between her breasts.  But it seems Marble is unsatisfied with only that; she quickens her pace and starts moving each of her breasts up and down opposite each other.", false);
                // Multiple dick side note
                if (player.cocks.length > 1) {
                    outputText("  Yet still unsatisfied, Marble engulfs and rub the rest of your " + multiCockDescriptLight(game.player) + " within her ample mounds.  ", false);
                }
            }
            // If Player has a vagina, Marble titfucks that
            if (player.vaginas.length > 0) {
                outputText("\n\nShe slides down your body to rub and caress your " + vaginaDescript(player, 0) + " with her breasts, sometimes pushing one of her " + marbleNips() + " inside your " + vaginaDescript(player, 0) + " and pulling a small gasp from you.  ", false);
                // If clit is too small to be titfucked...
                if (player.clitLength < 2) {
                    outputText("  Marble runs down each of your legs once with her breasts before unexpectedly plunging two of her fingers inside your " + vaginaDescript(player, 0) + ".", false);
                }
                else {
                    outputText("  Marble then pulls your " + clitDescription(player) + " between her breasts and does her best to pump and stimulate it as though it were a cock.", false);
                }
            }
            // If Player is genderless, Marble improvises
            if (player.gender == 0) {
                outputText("  She then says, \"<i>I think I've found a way to make you cum, sweetie,</i>\" with a sly expression.  She moves down from your upper body and starts rubbing and caressing your lower body too. It is extremely arousing, but it doesn't seem to be enough to push you over the edge.  Marble stands up, kisses you and rubs a single finger against the smooth skin of your groin.  You are so close to going over the edge.  Then one of her fingers pushes up against your " + assholeDescript(player) + ", causing you to go rigid from the new stimulation.", false);
            }
            // Sweet release
            ...
            if (player.vaginas.length > 0) {
                ...
                if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.", false);
                else if (player.vaginas[0].vaginalWetness <= VaginaWetness.SLICK) outputText("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.", false);
    ...
    // NAGAS
        ...
        outputText(".  \"<i>Why, you shameless " + mf(player, "pervert", "hussy") + "!</i>\" the cow-girl says in mock-offense, winking at you.");
        ...
        if (player.gender > 0) {
            ...
            if (player.vaginas.length > 0) outputText(vaginaDescript(player));
            ...
            if (player.cocks.length > 0) outputText("grabs your shaft");
            else outputText("penetrates your pussy");
            outputText(", while using her long tongue to lick the ");
            if (player.cocks.length > 0) outputText("tip");
            ...
            if (player.cocks.length > 0) outputText("  [EachCock] unleashes a load of semen right into her mouth, which she swallows with pink-faced, wanton gusto.");
            if (player.vaginas.length > 0) outputText("  Your [pussy] runs and squirts as you moan, drooling your juices into the little valley where Marble's breasts press against your scales.");
        }
        else {
            outputText("\n\nWhen she reaches where your genitals would be - if you had any - you're panting with pleasure.  Marble gives you a smile and whispers, \"<i>Don't worry sweetie, I know just how to make you cum.</i>\"  She then hugs your body tightly, reaching her arms around and squeezing and slapping your [butt], while slipping a finger into your " + assholeDescript(player) + ".  You gasp at the surprise, but are soon panting even harder.  The fact that she is still rubbing her breasts on your abdomen even as she teases you helps even more, and you can't help but coil your lower body around Marble, rubbing every part of it on her.  In moments, you cry out in bliss as your unusual anal orgasm washes over you.");
        }
...
function marbleNips(): string {
    ...
    if (player.effects.getValue2Of(StatusAffects.MarbleSpecials) == 0) player.effects.setValue(StatusAffects.MarbleSpecials, 2, 1);
    if (player.effects.getValue2Of(StatusAffects.MarbleSpecials) == 1) return "nipples";
...
function rapeDAHMARBLEZ(): void {
    ...
    player.effects.create(StatusAffects.MarbleRapeAttempted, 0, 0, 0, 0);
}
...
export function marbleBadEndFollowup(): void {
    ...
    let morph: string = "human";
    if (player.lowerBody == LowerBodyType.CENTAUR) morph = "centaur";
    if (dogScore(player) >= 4) morph = "dog-morph";
    if (horseScore(player) >= 3) {
        if (player.lowerBody == LowerBodyType.CENTAUR) morph = "centaur-morph";
        else morph = "equine-morph";
    }
    if (mutantScore(player) >= 5) morph = "corrupted mutant";
    if (minoScore(player) >= 4) morph = "minotaur-morph";
    if (cowScore(player) >= 5) {
        morph = "cow-";
        if (player.gender <= 1) morph += "boi";
        else morph += "girl";
    }
    if (beeScore(player) >= 4) morph = "bee-morph";
    if (goblinScore(player) >= 5) morph = "goblin";
    if (humanScore(player) >= 5 && morph == "corrupted mutant") morph = "somewhat human mutant";
    if (player.lowerBody == LowerBodyType.CENTAUR) morph = "centaur";
    if (player.lowerBody == LowerBodyType.CENTAUR) morph = "centaur";
...
function marbleCuddlin(): void {
    ...
    if (!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
        ...
        if (player.tallness <= 78) outputText("pulls your head on top of her chest.  She idly strokes your " + hairDescription(player) + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.", false);
...
function marbleNightSexIntro(clear: boolean = true): void {
    ...
    else if (player.gender == 1 && player.cocks.cockArea(x) > marbleCuntCapacity()) {
        outputText("You lie down next to her, but you can't help wishing that the two of you could have sex.  Suddenly, you feel Marble's hand grab a hold of your " + cockDescript(game.player, x) + ".  She sighs and says, \"<i>Sweetie, I'm afraid that you won't fit inside me.  Do you think you could get a little smaller, just for me?</i>\"  After a moment of getting comfortable, the two of you go to sleep.", false);
        doNext(playerMenu);
    }
    ...
    if (player.gender == 3) {
        if (player.cocks.cockArea(x) <= marbleCuntCapacity()) {
            ...
            outputText("Marble says, \"<i>Well sweetie, since that " + cockDescript(game.player, x) + " of yours is too big to fit inside me, let's put that " + vaginaDescript(player, 0) + " to use.</i>\"\n\n", false);
...
function marbleNightSexDudes(): void {
    ...
    flags[kFLAGS.MARBLE_LUST] = 0;
    outputText("", true);
    outputText("You gently lift yourself up and slide the tip of your " + cockDescript(game.player, x) + " into her waiting hole.  Ever so slowly, you push yourself further and further in.  Each inch gained brings gentle moans and coos from Marble, encouraging you onward.  ", false);
    // Can the PC's main cock go all the way?
    // If (cock 0 is longer than 8 inches)
    if (player.cocks[x].cockLength > 8) {
        outputText("There is only one thing that will stop your advance: when you reach the end of the line.  \"<i>Keep going sweetie, there is room left.  Make sure you fill me all the way.</i>\"  With a groan, your " + cockDescript(game.player, x) + " hits the furthest part of Marble's confines and you can go no further.\n\n", false);
    }
    ...
    if (player.cumQ() < 250) {
        outputText("Marble cries out in joy as your " + cockDescript(game.player, x) + " unleashes its load within her insides.  ", false);
    }
    else {
    ...
    if (player.cocks.length > 1) {
        ...
        if (player.cocks.length > 2) {
            outputText("the rest of your " + multiCockDescriptLight(game.player), false);
        }
        else {
...
function marbleNightSexChicks(): void {
    ...
    if (flags[kFLAGS.MARBLE_DICK_TYPE] == 0) {
        ...
        outputText("The two of you cry out in orgasm as your " + vaginaDescript(player, 0) + " clamps down on one end of her dildo and her womanhood clamps down on the other end.  ", false);
        // Does this PC have a cock too?
        // If (PC has at least 1 cock)
        if (player.cocks.length > 0) {
            outputText("At the same time, your " + multiCockDescriptLight(game.player) + " sprays its own load onto Marble's chest and stomach.  ", false);
        }
    ...
    // PC is female, Marble is a herm
        ...
        player.slimeFeed();
        outputText("You gently lift yourself up into the air and guide your " + vaginaDescript(player, 0) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + vaginaDescript(player, 0) + " onto her quivering " + marbleCock() + ", gaining inch by inch ", false);
        // is she satisfied?
        ...
        outputText("The two of you cry out in orgasm as your " + vaginaDescript(player, 0) + " clamps down on her " + marbleCock() + " as it fills you with its seed.  ", false);
        // Does this PC have a cock too?
        if (player.cocks.length > 0) {
            outputText("At the same time, ", false);
            if (player.cocks.length > 1) outputText("each of ", false);
            outputText("your " + multiCockDescriptLight(game.player) + " sprays its own load onto Marble's chest and stomach.  ", false);
        }
        outputText("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ", false);
...
function marbleNomNoms(): void {
    ...
    if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0) {
        ...
        if (player.effects.findByType(StatusAffects.SlimeCraving) >= 0 || player.lib + player.cor - player.inte > 35) {
...
export function MarbleDigsDraftsYo(): void {
    ...
    if (player.effects.getValue4Of(StatusAffects.Marble) > 50) outputText("big ", false);
...
function MarblePEggEffects(): void {
    ...
    if (player.effects.getValue4Of(StatusAffects.Marble) > 50) {
...
export function marbleOvulatesLikeMadDawg(): void {
    ...
    if (flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] < 100 && pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
        ...
        let newIncubation: number = pregnancy.incubation - (40 + rand(30));
...
function marbleAppearance(): void {
    ...
    if (player.effects.getValue4Of(StatusAffects.Marble) <= 20) outputText("Two small horns grow from her forehead, similar in size and appearance to those on a young female bovine.  ", false);
    else if (player.effects.getValue4Of(StatusAffects.Marble) <= 50) outputText("Two medium sized horns grow from her forehead, similar in size and appearance to those on a female bovine.  ", false);
    ...
    if (player.effects.getValue4Of(StatusAffects.Marble) <= 15) outputText("She has two pillowy breasts, each supporting a 0.4-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n", false);
    else if (player.effects.getValue4Of(StatusAffects.Marble) <= 25) outputText("She has two large breasts, each supporting a 0.6-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + ".\n\n", false);
    else if (player.effects.getValue4Of(StatusAffects.Marble) <= 35) outputText("She has two basketball-sized breasts, each supporting a 0.8-inch milk-seeping " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n", false);
    ...
    if (player.effects.getValue4Of(StatusAffects.Marble) <= 50) outputText("She has a pussy, with a 0.5 inch clit.\n\n", false);
    else if (player.effects.getValue4Of(StatusAffects.Marble) <= 75) outputText("She has a cunt, with a 0.6 inch clit.  You can see moisture gleaming from it.\n\n", false);
    ...
    if (player.effects.getValue1Of(StatusAffects.MarbleSpecials) == 0) outputText("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.", false);
...
function giveMarbleTailjobRelease(): void {
    ...
    if (player.cor > 70) {
        ...
        if (player.tongueType == TongueType.SNAKE || player.tongueType == TongueType.DEMONIC) outputText("  Your tongue runs along the length of the end of your tail, tasting both Marble's feminine secretions and her semen.  She gives you a smoldering gaze as you lick her juices up.  You grin at her.");
        // (Demon Tongue)
        if (player.tongueType == TongueType.DEMONIC) outputText("  You decide to put on a show for Marble, moving your tail as you drop out a large piece of your inhumanly long tongue, licking up her secretions sensuously while staring at her.  She quickly blushes under your gaze.");
...
function milkMarble(): void {
    ...
    if (player.cocks.cockThatFits(marbleCuntCapacity()) >= 0 && (player.isBiped() || player.isNaga() || player.isGoo())) addButton(1, "Fuck Her", milkMarbleFuckDatCowPussy);
    if (player.vaginas.length > 0 && flags[kFLAGS.MARBLE_DICK_TYPE] > 0 && (player.isBiped() || player.isGoo())) addButton(2, "Take Her", milkMarbleTakeHerDick);
...
function milkMarbleCunnilingling(): void {
    ...
    const pLust: number = int(20 + ((player.lib + player.cor) / 10));
    dynStats("lus", pLust);
    dynStats("sen", 1);
...
function milkMarbleFuckDatCowPussy(): void {
    ...
    if (player.tallness < 68) {
        outputText("[pg]\"<i>Oh sweetie.</i>\"  She pulls you into a deep hug against her chest.  \"<i>It's too much!  Hah, hahahahaha!  No more joking around.  I need you inside me now.</i>\"  She carefully carries you over to the bench and lays you down on your back, then straddles the bench above your " + cockDescript(game.player, x) + ".  \"<i>Are you ready for me?</i>\" she calls out in a deep husky voice, breasts heaving, tail swishing, and all the while the milker machine continues to hum above her.");
    }
    else {
    ...
    if (player.perks.findByType(PerkLib.MarblesMilk) >= 0) {
        ...
        dynStats("lib", .3);
        if (player.cor < player.effects.getValue4Of(StatusAffects.Marble)) dynStats("cor", 1);
        dynStats("tou", .1);
        dynStats("str", .1);
        dynStats("sen", -2);
        marblePreggoChance(1);
    }
    else if (player.effects.getValue4Of(StatusAffects.Marble) > 30) {
        ...
        outputText("[pg]Finally the two of you cry out together in orgasmic pleasure, and you spray out your seed into her passage, mixing with a rush of her own fluids into a slurry of sex juices sloshing around your " + cockDescript(game.player, x) + ".  The two of you settle down, with Marble laying down on you contentedly for a moment.");
...
function milkMarbleOnTheBar(): void {
    ...
    if (player.cocks.cockThatFits(marbleCuntCapacity()) > 0) outputText("[pg]You idly consider grabbing and pulling on her tail, so you could have a chance to take charge, though Marble would probably try and take control in that case for sure...");

    menu();
    addButton(0, "Follow Tail", milkMarbleBarFollowTail);
    addButton(1, "Cunnilingus", milkMarbleBarCunnilingling);
    if (player.cocks.cockThatFits(marbleCuntCapacity()) >= 0) addButton(2, "Pull Tail", milkMarbleBarPullTail);
...
function milkMarbleBarFollowTail(): void {
    ...
    if (x >= 0) {
        outputText("[pg]Her ribbon adorned, bovine appendage slowly unwraps from your wrist and instead searches around your crotch for something else.  Eventually the tuff comes into contact with your " + cockDescript(game.player, x) + ", and runs its fur across your length lovingly for a few times, then curls around your base and pulls you in towards Marble's waiting womanhood.");
        ...
        if (y >= 0) outputText("  You find your " + cockDescript(game.player, y) + " slides up comfortably in between her cheeks, and squeeze them together a bit to give it a nice tight valley of soft flesh to run through.");
        ...
        outputText("[pg]Everything centers onto three things: the feel of aroused cunt surrounding your shaft, rippling and squelching with each slow plunge and retreat from its depths, the soft springy ass and hip curves under your hands that runs oh so pleasantly under your hands, and of course, Marble's tail curled around the base of your " + cockDescript(game.player, x) + ".  The sensation of her thin appendage, soaked and sweat yet still soft with fur, constantly quivering and twitching in time with the movements of your two bodies, draws your entire focus.");
    }
    else {
    ...
    if (x >= 0) {
        ...
        const pLust: number = int(10 + player.lib / 10);
        dynStats("lus", pLust);
    }
    doNext(Camp.returnToCampUseTwoHours);
}
...
function milkMarbleBarPullTail(): void {
    ...
    outputText("[pg]You ignore her protests and smack her ass once, then roughly penetrate her with your " + cockDescript(game.player, x) + " while yelling at her to take it.");
    ...
    if (flags[kFLAGS.MARBLE_DICK_TYPE] == 0) {
        ...
        if (player.tongueType > TongueType.HUMAN) outputText("You take full advantage of your inhumanly long tongue to fill her up, running across her passage all the way to the entrance to her womb and then back to her lower lips.");
...
function milkMarbleLeaveAfterBar(): void {
    ...
    const pLust: number = int(5 + player.lib / 20);
    dynStats("lus", pLust);
    flags[kFLAGS.MARBLE_LUST] += 5;
    doNext(Camp.returnToCampUseTwoHours);
}
