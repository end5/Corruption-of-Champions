...
export function heliaCapacity(): number {
    return helCapacity();
}
export function heliaAnalCapacity(): number {
    return helAnalCapacity();
}
export function helAffection(diff: number = 0): number {
...
export function isHeliaBirthday(): boolean {
    return date.month == 7;
...
export function heliaFollowerIntro(): void {
...
export function helFollowersIntro(): void {
    clearOutput();
    ...
    // If Rath is in Camp
    else if (flags[kFLAGS.HEL_INTROS_LEVEL] < 3 && player.effects.findByType(StatusAffects.CampRathazul) >= 0) {
    ...
    // If Bimbo Sophie is at Camp:
        ...
        outputText("\n\n<i>\"Yay!  My special little cutie is back!</i>\"  Sophie exclaims, giggling as she wraps her feathered arms around you.  \"<i>Is my " + mf(player, "hunk", "babe") + " interested in like, some 'fun'?\"</i>");
...
export function angryHelAndIzzyCampHelHereFirst(): void {
    clearOutput();
...
export function heliaFollowerMenu(display: boolean = true): void {
    if (display) {
        clearOutput();
        spriteSelect(68);
    }
    if (flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 2) {
        ...
        if (flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 1) {
            ...
            simpleChoices("Dungeon", goToHeliaDungeon, "", null, "", null, "", null, "Not Yet", notYet);
        }
    }
}
...
function sparWithHeliaFirebuttsAreHot(): void {
    ...
    startCombat(new Hel());
    monster.effects.create(StatusAffects.Sparring, 0, 0, 0, 0);
    // No gems.
    monster.XP = 1;
    monster.gems = 0;
}
...
export function loseToSparringHeliaLikeAButtRapedChump(): void {
    clearOutput();
    // If HP loss)
...
// PC Whips Hel's Ass
export function PCBeatsUpSalamanderSparring(): void {
    clearOutput();
    // (If HP loss)
...
function talkToHel(): void {
    ...
    if (flags[kFLAGS.FOLLOWER_HEL_TALKS] == 0) {
        ...
        outputText("\n\n<i>\"Sounds good to me, " + mf(player, "bud", "babe") + ",\"</i> Hel grins, leading you by the arm to a rock near her hammock.  The two of you sit yourselves down, with Hel locking her fingers through yours in her lap.  <i>\"So what's on your mind, lover mine?\"</i>");
...
function hugASmokeyTail(): void {
    ...
    if (game.time.hours >= 21) {
        ...
        outputText("\n\nWith a bit of effort, Hel turns around in your embrace and starts to fiddle with your [armor], slowly pulling it off, leaving your bare flesh pressed against her own.  You breathe in the woodsmoke scent of her hair; rub your " + skinFurScales(player) + " along her smooth flesh; gasp lightly as her long tail wraps lovingly around your [legs], drawing you even closer, letting your face rest against her yielding chest.");
        ...
        if (player.tailType > TailType.NONE) outputText("s");
    ...
    // [If PC is 6-8ft Tall]
        ...
        outputText("\n\n<i>\"Oh, [name],\"</i> Hel sighs happily, holding you tight.  <i>\"My wonderful, " + mf(player, "handsome", "beautiful") + " [name].\"</i>");
    }
    // [If PC is <6ft Tall]
    else {
...
export function sleepyNightMareHel(): void {
...
function heliaCampPossession(): void {
    clearOutput();
...
// "Rough" Sex (Well, it's in the menu, if not in spirit) -- Naga Coil (Male w/ Naga Lower Body)
// Male naga shit
function inCampHelNagaLuv(): void {
    clearOutput();
    ...
    outputText("\n\nUnable to move much in your serpentine embrace, Hel begins to rapidly contract her vaginal and anal muscles, squeezing and grinding against your " + cockDescript(game.player, x) + " and tip.  Content to close your eyes and bask in the pleasures of your lover, ready to lose yourself in pleasure.  You relax, leaning back on yourself as you buck your hips into your lover's sweet hot box, flick your tail inside her ass, and play your tongue around her mouth, tasting her warm saliva and the aftertaste of beer.");

    outputText("\n\nYou cum easily, only a small grunt escaping your lips as Hel's delicious cunt brings you over, pulling your " + cockDescript(game.player, x) + " deep into her and tightening your hold around her.  The sudden squeeze is the last hurrah as you empty your ");
...
// "Rough" Sex -- Naga Coil (Female w/ Naga Lower Body)
function nagaCoilForHelCampWithGirls(): void {
    clearOutput();
...
function centaurMountsCampHel(): void {
    let x: number = player.cocks.cockThatFits(heliaCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    clearOutput();
    ...
    // if no boobs:
    if (player.breastRows.biggestTitSize() < 3) outputText(" but your body is too big, and you must content yourself to endure Hel's ministrations");
    ...
    outputText("\n\nAfter a few mind-hazing minutes of salamander hand-job, Hel takes the " + cockHead(player, x) + " of your cock into her mouth, slurping up the tip of your " + cockDescript(game.player, x) + ".  Your breath catches in your throat as the first inches of your cockmeat are engulfed in the hot, wet embrace of Helia's mouth.  Her lizard-tongue wastes no time in flicking across your " + cockHead(player, x) + ", the forked tip playing across your urethra, drawing forth a thick dollop of pre that soon smears her cheeks with white.  She pumps your cock into her mouth, licking and sucking on your crown as her pistoning hands force thicker globs of sticky precum out of you, eagerly swallowing every drop - but only after savoring it, rolling each drop around in her mouth until her entire gob is a sticky mess, thick webs of seed coating her lips each time she swallows another appetizing taste.");
    ...
    outputText("\n\nYou start forward as a rough tail-slap lands on your flank.  You reflexively step forward, your equine muscles acting on instinct...  which only serves to rock Hel off and down again on your " + cockDescript(game.player, x) + ", her impromptu harness moving her like a swing. You arch your back as the movement hammers Hel down on your cock like a free-swinging onahole, but the moment of pleasure lurches out of control when she gives you another hard tail-swat, and another; your insticts carry you forward until you reach a full canter. Shudders and twitches wrack your body as your own motions rock Hel on and off your [cock], slamming her back down on your shaft again and again.");
...
function femtaurPlusCampHel(): void {
    clearOutput();
    outputText("You trot around your lover, your powerful centaur frame brushing lightly against her. Hel grins, reaching out to pet your flanks and running her claws across your " + skinFurScales(player) + ".  \"<i>You've got a helluva body,</i>\" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around camp.  She gives you a gentle nudge toward a certain pile of your gear, and it doesn't take but a moment for you to notice she's urging you toward the special toys Whitney gave you for your equine-like body's needs.");
...
function heliaFollowerTentafuck(): void {
    clearOutput();
...
// Boxing
function boxWithInCampHel(): void {
    clearOutput();
    ...
    outputText("\n\nYou toss your [armor] aside and assume your own battle stance, your powerful muscles tightening beneath your " + skinFurScales(player) + ", ready to spring into action.  You move slowly, cautiously; you and Hel begin to circle each other like predators observing their prey, each watching the other for any momentary lapse, any split-second vulnerabilities.  Suddenly, Hel lashes out, a quick one-two jab at you, narrowly blocked in your guard.  You grind your feet into the ground, deflecting her inhumanly-strong blows as best you can.  She launches a third strike, but you duck aside, spinning into her guard and delivering a hard jab to her gut.  The salamander recoils, swinging her fiery tail at you as she gets some distance, staggering back from the hit.  You exchange another quick flurry of blows, punches connecting, blocked, or dodged in rapid succession.  The fight drags on, neither of you able to land a telling blow, your strikes deflected or dodged, your prowess in battle nearly matched.");
    fatigue(5 + rand(20));
...
function takeABath(): void {
    ...
    // {Repeat Intro (If you taught Hel to swin; else, play 1st Time)}
        ...
        if (player.armorName == "goo armor" && player.lust >= 33 && 9999 == 0) {
...
// Threesomes
export function heliaThreesomes(): void {
    clearOutput();
...
function heliaAndVapula(): void {
    ...
    outputText("\n\nYou feel hands wandering across your body, Hel slowly, sensuously, pulling off your [armor].  Soon, the two of you are bare, clothes and weapons discarded all around the succubus.  You push Hel over, grinding your hard " + cockDescript(game.player, x) + " between her legs.  As you grope the salamander girl's body, you gesture at Vapula and tell her to come closer so that you can stroke her, too.  You begin caressing each other, spending a few moments in languid pleasure until your " + cockDescript(game.player, x) + " looks ready to explode.");
    ...
    outputText("\n\nThis libertine orgy cannot last indefinitely, and even your girls are trembling convulsively, ready to reach their climax. You manage to slide your " + cockDescript(game.player, x) + " up the reptile girl's twat a few more times before blasting her with your spooge.  You fill Hel's interior, pouring cum like a torrent into her womb");
...
function campHelAndKihaThreeSome(): void {
    clearOutput();
...
function girlsThreesomeHelAndKiha(): void {
    clearOutput();
...
function dudeHeliaAndKihaThreeSome(): void {
    clearOutput();
    ...
    outputText("\n\nTenderly, Kiha plants a kiss in the nape of your neck, her long forked tongue tickling your " + skinFurScales(player) + ".  You turn and return her kisses, locking lips with the dragoness, your tongues entwining to the rhythm of the feet servicing your [cock].");
    ...
    outputText("\n\nKiha gives the salamander a grave look, but soon softens, nuzzling her chin in your " + hairDescription(player) + ".  Hel sighs, but leans down to give you each a quick kiss on the head; her foot never misses a beat, though, and combined with Kiha's sensual massaging, you realize you're dangerously close to the edge again.  You grit your teeth and let loose an animalistic growl, the only forewarning the distracted lizards get before the first burst issues forth from your [cockHead], smearing the sole of Hel's foot with creamy seed.  The salamander makes a happy gasp as another squirt of cum smears her clawed toes, a particularly strong arc fountaining so high that it smears her thighs.");
...
function helXValeriaFemalePC(): void {
    clearOutput();
    outputText("In the blink of an eye, Valeria slithers around Hel's body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a pair of thick blue pricks out of her groin, each shimmering in the sunlight as Hel strokes Valeria's new appendages, coaxing out a drop of slimey pre from both tips.  Valeria's face reappears on Hel's flat belly a moment after she's done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick's knot inflates enticingly.");

    outputText("\n\nThe Valeria-covered Hel takes you by the shoulder and bends you down, pushing you up against a nearby rock to expose your [butt] and the slick slit of your cunt.  You dig your fingers into the stone as Hel moves behind you, her clawed hands running roughly along your thighs, hips, and ass, groping and caressing your tender skin.  Her fiery tail brushes your [legs], coiling around before reaching up to tease your [clit], her tip running circles around your bud, sending thunderbolts of pleasure arcing through your body.  A look over your shoulder shows the gooey salamander spreading her legs and stroking her twin blue rods with one hand, the other squeezing your [butt], pulling your cheeks apart to reveal your [asshole].");
    ...
    outputText("\n\nYou grit your teeth and try to relax, urging your body to allow the foreign intruders in.  Your efforts seem futile, though, as both cocks slip right in, seemingly without effort, the gooey pricks becoming so thin and so slick that your body can't deny them - only to inflate again once they've gained a foothold inside you.  You cry aloud as Valeria expands inside you, her cocks growing... and growing... thickening and thickening until both your holes stretch pleasurably, forced to take more and more goo until you feel as if you're going to burst, filled with more cock than you ever could have taken before.  Hel, too, cries out as the armor-goo pushes a pair of mirror cocks into her, mimicking her movements and the cunt-stretching size of the members she's giving you.");

    outputText("\n\nBoth of you take a moment to recover, trying to let your bodies adjust to the titanic cyan pillars that are forcing your holes agape.  Soon, though, Hel adjusts her grip on your [hips] and starts to move, slowly pulling back the few inches of length inside you, withdrawing until only the very tip and flare remain within.  You groan lewdly as Hel pushes back in, slowly, gently filling you back up with her cocks, pouring gallons of goo into your womb and rectum until her hips finally push into your [butt], both your mouths locked in silent 'O's of pleasure as Valeria's goo stretches all four holes wide.");
    ...
    outputText("\n\nFinally, the orgasm subsides, and you're left trembling in a pool of goo and femcum, lube leaking freely out of your cunt and into Valeria.  Sighing contentedly, Hel reaches around to give you a long kiss before she pulls out... or tries to, anyway.  The knot, however, remains firmly in place.");
...
function heliaAnalTrainingPrompt(): void {
    clearOutput();
...
// Give Her Ale
function giveHeliaAnalTraining(): void {
    clearOutput();
...
function heliaGapeSceneChoices(): void {
...
function noGapeSlutForHelia(): void {
    clearOutput();
...
function yesMakeHeliaAGapeSlut(): void {
    clearOutput();
...
export function heliasBirthday(): void {
    clearOutput();
    ...
    outputText("\n\nThe heat of your lover's body pressed to yours, the feeling of her supple leathery scales caressing your " + skinFurScales(player) + " makes your mind skip a beat before you mumble, \"<i>Uh, Saturday?</i>\"");
...
// Stay Home
function stayHomeFromHeliaParty(): void {
    clearOutput();
...
function goWithHelia(): void {
    ...
    */
    ...
    outputText("\n\nKiri slips out of the crowd to plant a quick kiss on your cheek, followed by Hakon vigorously shaking your hand, great claws scoring your " + skinFurScales(player) + " as he thanks you again for the rescue, and for ");
...
// Hakon & Kiri
function heliaHakonAndKiri(): void {
    clearOutput();
    ...
    outputText("\n\n\"<i>How're you holding up, " + mf(player, "son", "kid") + "?</i>\" Hakon says, clapping you on the shoulder and passing you another beer.  \"<i>Hel tells me you're quite a handful.  Treating my girl right, I hope?</i>\"");

    outputText("\n\nYou falter at that, not quite sure how to address your lover's father in that regard, but the old salamander just laughs and gives you another rough shoulder-clap.  \"<i>Just teasing, " + mf(player, "boy", "girl"));
...
// Hel & Fox Twins
function heliaAndFoxTwins(): void {
    clearOutput();
...
function helAndSluttyHarpyMale(): void {
    clearOutput();
...
function helAndSluttyHarpyFemale(): void {
    clearOutput();
...
function helAndSluttyHarpyGenderless(): void {
    clearOutput();
