...
export class Urta implements TimeAwareInterface {
    ...
    */
    ...
    public timeChange(): boolean {
        ...
        // Urta egg freak out
        if (flags[kFLAGS.URTA_EGG_FORCE_EVENT] > 0 && game.time.hours > 6 && game.time.hours < 18 && flags[kFLAGS.URTA_EGG_FORCE_EVENT] < 12) {
...
export function urtaSprite(): void {
    if (urtaDrunk()) spriteSelect(84);
    else spriteSelect(1);
}

export function urtaCapacity(): number {
    let bonus: number = flags[kFLAGS.TIMES_RUT_FUCKED_URTAS_CUNT] * 5;
    if (bonus > 40) bonus = 40;
    if (flags[kFLAGS.URTA_TENTACLE_GAPED] > 0) return 500;
    return 60 + bonus;
}

export function urtaLove(love: number = 0): boolean {
...
export function urtaAvailableForSex(): boolean { return urtaFuckbuddy() && Scylla.scyllaAction != Scylla.SCYLLA_ACTION_FUCKING_URTA && flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] == 0; }

export function urtaFuckbuddy(): boolean { // Returns true if Urta is either the player's fuckbuddy or lover
...
export function urtaJustFriends(): boolean { return flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] == 0 && flags[kFLAGS.URTA_PC_LOVE_COUNTER] == -1; }

export function urtaAtBar(): boolean { // Is Urta physically at the Wet Bitch?
    return (!UrtaQuest.urtaBusy() && flags[kFLAGS.AMILY_VISITING_URTA] != 1 && game.time.hours < 15 && flags[kFLAGS.KATHERINE_UNLOCKED] != 3);
}

export function urtaDrunk(): boolean {
    // Preg = no drinking!
    if (pregnancy.isPregnant) return false;
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00146] == -1) return false;
    if (game.time.hours > 12 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00146] < 1) return true;
    if (game.time.hours > 8 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00146] >= 1) return true;
    else return false;
}
...
export function knockUpUrtaChance(): void { // Moved here from UrtaPregs since it needs to be accessed from outside Urta
    // GTFO IF PREGGO
...
export function urtaBarDescript(): boolean {
    outputText("\n\n", false);
    // Urta PISSED
    if (flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] > 1) {
...
export function urtaBarApproach(): void {
    urtaSprite();
...
function drinkUrtasBoozePtTwo(): void {
    ...
    if (player.vaginas.length > 0 && player.cocks.length == 0) outputText("squat down and ", false);
...
function drinkUrtasBoozeLetHer(): void {
    ...
    outputText("\"<i>Mmmm I shhoulda ushed a condom, you're gonna be a messssss!</i>\" she cries as she climbs atop her sexual peak, mounting it just like she's mounting your " + assholeOrPussy(player) + ".  You're spread wide, the equine member rammed tightly ", false);
    if (player.vaginas.length > 0) outputText("against your cervix as the tip presses firmly against it", false);
    else outputText("into your rectum", false);
    outputText(", unloading a thick batch of cream directly into your ", false);
    if (player.vaginas.length > 0) outputText("waiting womb", false);
    ...
    else if (player.vaginas.length > 0) outputText("  You briefly wonder if she's capable of knocking you up, and what the baby would look like.", false);
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.vaginas.length > 0) outputText("Your pussy dribbles and clenches, leaking Urta's whiteness as she claims it for her own.", false);
    ...
    if (player.vaginas.length > 0) cuntChange(player, 60, true);
    else buttChange(player, 60, true);
    doNext(Camp.returnToCampUseOneHour);
}
...
function dualUrtaMasturbation(): void {
    ...
    if (player.vaginas.length > 0) {
        if (player.vaginas[0].vaginalWetness > VaginaWetness.SLICK) outputText("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.", false);
        else outputText("Quickly engorging, your lower lips begin to part around your " + clitDescription(player) + ".", false);
        outputText("  ", false);
    }
    ...
    if (player.vaginas.length > 0) outputText("Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n", false);
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas[0].vaginalWetness == VaginaWetness.SLAVERING) outputText(" and gushing everywhere", false);
        outputText(".  ", false);
    }
...
// [FUCK HER]
export function urtaFuckHer(afterBefriending: boolean = false): void {
    urtaSprite();
    flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY]++;
    flags[kFLAGS.TIMES_FUCKED_URTA]++;
    flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] = 5;
    ...
    if (player.cocks.length == 0) {
        ...
        outputText("Urta nods, numbed by the unexpected and forward nature of your offer.  She looks up hopefully and pries her hands away from the twitching behemoth in her loins, waiting to see how you intend to deal with it.  You approach her and turn, straddling your " + legs(player) + " over her, watching her expression as you lower yourself down, ", false);
        if (player.tailType == TailType.HORSE || player.tailType == TailType.DOG) outputText("swishing your tail across her taut nipples", false);
        else outputText("making her wait", false);
        outputText(".  Eventually the blunted tip of Urta's horse-cock presses against your " + assholeOrPussy(player) + ".  Agonizingly, you force yourself to relax and lower yourself down on the mottled pink and black shaft.\n\n", false);
        ...
        if (player.vaginas.length > 0) {
            ...
            buttChange(player, 60, true, true, false);
        }
        outputText("  Urta babbles, \"<i>Oh godsh yesh thank you so much-ohmigod it's SO HOT inside you.</i>\"\n\n", false);

        outputText("You smirk over your shoulder and start bouncing up and down on her, slapping her balls with your " + buttDescription(player) + " each time you bottom out on the thick shaft.  You can feel her ring of prepuce rubbing your inner walls, pressing on nerves normally ignored.  Urta grunts underneath you and you feel her flare growing wider inside you, stretching you further and further with every bounce atop the equine-endowed slut.  Suddenly warmth is flooding into your ", false);
        ...
        if (player.cocks.cockArea(x) < urtaCapacity()) {
            ...
            if (cocks > 1 && y >= 0) outputText(", " + cockDescript(game.player, x) + ", and " + cockDescript(game.player, y) + ".", false);
            else outputText(" and " + cockDescript(game.player, x) + ".", false);
            outputText("  Both her hands are locked tightly around her behemoth as it unloads onto the alley floor and walls.  The juicy tunnel clamps down on top of you, squeezing your " + cockDescript(game.player, x) + " as if Urta's life depended on it.  It's as if every ounce of her being was focusing on clamping down on the invader, massaging, squeezing, and milking it, while the rest of her body goes entirely limp.", false);
            if (cocks > 1 && y >= 0) outputText("  Your " + cockDescript(game.player, y) + " feels like it's about to explode inside her asshole when her sphincter clamps down tightly, acting like the world's tightest cock-ring.", false);
            outputText("\n\n", false);
        ...
        // (TOO BIG)
            ...
            if (cocks == 1) {
                ...
                outputText("You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your " + legs(player) + " across her and begin grinding your " + multiCockDescriptLight(game.player) + " against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your " + multiCockDescript(game.player) + ".  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n", false);
                outputText("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + multiCockDescript(game.player) + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ", false);
            ...
            if (cocks > 1) {
                ...
                outputText("You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your " + legs(player) + " across her and begin grinding each of your " + multiCockDescriptLight(game.player) + " against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your " + multiCockDescriptLight(game.player) + ".  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n", false);

                outputText("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + multiCockDescriptLight(game.player) + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ", false);
                if (player.cumQ() < 250) outputText("You join in, though your own contribution seems meager by comparison.", false);
                else if (player.cumQ() < 500) outputText("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.", false);
                else outputText("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.", false);
...
// [Under Table BJ]
export function blowUrtaUnderTable(): void {
    urtaSprite();
    player.slimeFeed();
    flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY]++;
    urtaLove(1);
    ...
    if (player.breastRows.biggestTitSize() >= 2) outputText("dragging between your " + allBreastsDescript(player), false);
    else outputText("dragging across your chest", false);
    outputText(" as it begins to soften.  You wipe as much of her leavings from your " + face(player) + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, punishment for forcing you to take her so deep.\n\n", false);
...
// [Public Buttfucking]
export function takeUrtaInTheButtPublically(): void {
    urtaSprite();
    outputText("", true);
    ...
    outputText("You carefully shift your position, climbing in front of Urta.  With care, you grab the sensitive member just below the flare and begin rubbing it over your " + buttDescription(player) + " and ready ass-hole, lubing yourself up and trying to relax.  The impatient fox grabs you by your " + hipDescription(player) + " and pulls you down, forcing herself against your pucker.  ", false);
    ...
    // [DOESN'T FIT]
        ...
        tooBig = true;
        outputText("It's far too big for you, but that doesn't stop Urta from pushing it inside you.  You can feel yourself stretched around the invader, and it hurts enough that you almost scream out in pain, but inexorably, you're drawn down until your " + buttDescription(player) + " is resting on Urta's thighs.  Her thick prick keeps you stretched uncomfortably wide, and you can feel every twitch it makes.", false);
        // (STRETCH TEXT HERE)
        if (!buttChange(player, 60, true)) outputText("  ", false);
    ...
    if (player.vaginas.length > 0) {
        outputText("Heat spreads through your loins as your " + vaginaDescript(player, 0) + " engorges with blood, allowing your " + clitDescription(player) + " to ", false);
        if (player.clitLength < 2) outputText("peep out", false);
        else outputText("grow to its full size", false);
        outputText(".  ", false);
        if (player.vaginas[0].vaginalWetness < VaginaWetness.SLICK) outputText("Moisture beads on your lips, signaling just how turned on this total violation is making you.", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.", false);
    ...
    // (NIPPLES:
    outputText("Your " + nippleDescription(player, 0) + "s tent up under your " + player.armorName, false);
    if (player.breastRows.hasFuckableNipples() && player.breastRows.biggestLactation() >= 1) outputText(", leaking milky lubricants and aching to be penetrated.", false);
    else if (player.breastRows.hasFuckableNipples()) outputText(", aching to be penetrated.", false);
    else if (player.breastRows.biggestLactation() >= 1.5 && player.breastRows.biggestLactation() < 2.5) outputText(", leaking thin streams of milk that rapidly become visible to everyone around you.", false);
    else if (player.breastRows.biggestLactation() >= 2.5) outputText(", spurting thick streams of milk that soak your " + player.armorName + " and are visible to everyone in the bar.", false);
    else outputText(".", false);
    outputText("\n\n", false);

    outputText("Soft-furred hands grip your " + hipDescription(player) + " firmly and begin to lift you up, making you gasp as her flare is dragged through your hind-end, eliciting what Urta must think are cute gasps.  You hear a 'murrrr' of contentment in your ear and Urta releases you, twitching inside you as your weight slowly carries you back down into her lap.  Once your " + buttDescription(player) + " smacks into her thighs she grabs you again, dragging you back up her massive pole until you're looking down on the tables around you.  She stops and withdraws her support, guiding you back down her shaft and moaning when you take her ring of prepuce half-way down.\n\n", false);
    ...
    else outputText("you find yourself sliding your hands under your " + player.armorName + " and pinching your " + nippleDescription(player, 0) + "s, putting on a show for the crowd.", false);

    outputText("  Urta starts breathing hard into your ear, and you start flexing your " + legs(player) + " to help her lift you up.  Grateful for the effort, the fox-girl increases the pace by helping to pull you back down each time.  By now it's starting to feel pretty good, and her pre-cum is making your " + assholeDescript(player) + " so wet and slippery that you can slide quite quickly along the column of horse-meat.\n\n", false);
    ...
    if (player.breastRows.biggestTitSize() >= 1) {
        outputText("  Your " + allBreastsDescript(player) + " are jiggling and bouncing wildly, ", false);
        if (player.breastRows.hasFuckableNipples()) outputText("dripping lubricants", false);
        else if (player.breastRows.biggestLactation() > 1) outputText("spurting milk", false);
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length > 1) {
            ...
            outputText("  Steady drizzles of cum begin dripping from your " + multiCockDescriptLight(game.player) + " as she pounds away, rubbing hard against your prostate.", false);
        }
    }
...
// [PUBLIC FACIAL]
export function getAPublicFacialFromUrta(): void {
    urtaSprite();
    player.slimeFeed();
    flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY]++;
    urtaLove(2);
...
function optionalUrtaBukkake(): void {
    ...
    outputText("An hour later you're totally drenched in cum.  It's dripping off you from your hair down to your " + feet(player) + ", and you love it.  You lick up the mixed spooge until you start to feel full, and a blushing barmaid comes forward to hand you a towel.  You give her a salty kiss on the lips, sending her running away before you towel off and dress yourself.", false);
    doNext(Camp.returnToCampUseOneHour);
}
...
export function goBackToUrtasForLuvinz(): void {
    urtaSprite();
    outputText("", true);
...
export function goBackToUrtasForLuvinzII(): void {
    ...
    if (flags[kFLAGS.PC_SEEN_URTA_SEX_TOYS] == 0) {
        ...
        if (game.giacomo > 0) outputText("  You recognize some of the toys as Giacomo's wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.", false);
    ...
    if (player.vaginas.length > 0 && player.cocks.length > 0) outputText("hard and wet as you've ever been", false);
    else if (player.cocks.length > 0) outputText("hard as you've ever been", false);
    else if (player.vaginas.length > 0) outputText("wet as you've ever been", false);
    ...
    if (player.vaginas.length > 0) femaleRides = rideUrtaTenderFemale;
    ...
    if (player.isGoo() && player.skinType == SkinType.GOO) addButton(4, "Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms);
}
...
function rideUrtaTenderFemale(): void {
    ...
    if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("growing moistness in your loins", false);
    else if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("sopping wetness in your loins", false);
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length == 1) {
            ...
            if (player.cocks[0].cockLength > 15 && player.breastRows.biggestTitSize() >= 5) outputText("jiggling tits", false);
            else outputText("bellies", false);
            outputText(", glazing them in slippery goo.", false);
        }
        else {
            outputText("Each of your own " + multiCockDescriptLight(game.player) + " cums as well, blasting seed over both your ", false);
            if (player.cocks[0].cockLength > 15 && player.breastRows.biggestTitSize() >= 5) outputText("jiggling tits", false);
            else outputText("bellies", false);
            outputText(", glazing them in slippery goo.", false);
        }
...
function dudeFuckingUrtasCooch(): void {
    ...
    outputText("You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your " + multiCockDescriptLight(game.player) + " and start lining yourself up to fuck her doggie-style.  ", false);
    if (cocks > 1 && y >= 0) outputText("Your secondary " + cockDescript(game.player, y) + " wobbles directly behind her back-door and Urta's eyes widen a bit when she realizes you're going to double-penetrate her.  ", false);
    if (cocks > 2) outputText("The remainder of your " + multiCockDescriptLight(game.player) + " fan out, ready to rub themselves against her butt-cheeks once you push inside.  ", false);
    ...
    outputText("Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your " + cockDescript(game.player, 0) + "'s " + cockHead(player) + " with slippery moisture.  ", false);
    if (cocks > 1 && y >= 0) outputText("Your second " + cockNoun(player.cocks[y].cockType) + " soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ", false);
    ...
    outputText("Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your " + cockDescript(game.player, x) + " seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ", false);
    if (cocks > 1 && y >= 0) outputText("The intensity and pressure in her back-door is even worse, and your " + cockDescript(game.player, y) + " is nearly in pain from her asshole's tight, pulsating grip.  ", false);
    ...
    if (cocks > 1 && y >= 0) outputText("The " + cockDescript(game.player, y) + " you've crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ", false);
    if (cocks > 2) outputText("Her butt and tail are covered with the stuff as each of your remaining " + multiCockDescriptLight(game.player) + " splatters her with spooge, covering her fur in white sludge.  ", false);
...
function fuckUrtasVagButTooBig(): void {
    ...
    let vaginal: () => void = null;
    if (player.vaginas.length > 0) vaginal = rideUrtaTenderFemale;
...
function tenderTakeItUpTheAssFromUrta(): void {
    ...
    outputText("Her cheeks color when you give her a nod and begin lowering your " + buttDescription(player) + " towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your " + assholeDescript(player) + ".  ", false);
    ...
    if (cocks > 0) {
        ...
        if (cocks > 1) outputText("each of ", false);
        outputText("your " + multiCockDescriptLight(game.player) + " droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.", false);
        if (player.cocks[0].cockLength >= 15) {
            outputText("  Urta winks and guides ", false);
            if (cocks > 1) outputText("one of ", false);
            outputText("your " + multiCockDescriptLight(game.player) + " between her tits", false);
            ...
            outputText(".", false);
            outputText("  She begins giving you a slow tit-fuck, trying to encourage you to pick up the pace and start riding her.  It does the trick as your " + hipDescription(player) + " start rocking on their own in reaction to her efforts.  She licks harder, and you reward her by lifting yourself up and sliding back down.  Her massive girth squeezes a squirt of pre-cum from your prostate with every few strokes, splattering it on her muzzle.", false);
        }
        outputText("\n\n", false);
    }
    else {
        outputText("For a few moments you sit like that, trying to adjust to the pressure in your backside.  After a few moments you can feel your muscles relaxing, allowing Urta to rest more comfortably inside you.  Flexing your " + legs(player) + ", you pull yourself up off of her until the flared tip is just barely inside you, then you plunge back down.  Urta's mouth hangs open and her tongue lolls out, and you can feel the warmth of her leaking fluid inside of you, letting you know just how good your " + assholeDescript(player) + " feels to her.\n\n", false);
    }
    outputText("You bounce on top of your fox-lover's equine rod, spearing your " + assholeDescript(player) + " on it over and over.  ", false);
    ...
    if (player.cumQ() > 750) outputText("  You really are a messy, " + mf(player, "boy", "girl") + " aren't you?", false);
    outputText("</i>\"\n\n", false);
...
function comfortUrtaAfterTellinTruuf(): void {
    ...
    if (((player.lowerBody == LowerBodyType.CENTAUR && player.cocks.length > 0) ||
...
// [EAT THE BITCH OUT]
export function eatUrtaOutNomNomPussy(): void {
    urtaSprite();
    ...
    outputText("Sucking, licking, slurping, you judge the successfulness of your oral efforts by just how much fluid you can get to soak into your " + face(player) + ".  The bar's noises are completely indecipherable, muffled as they are by the furred thighs clamped around your head and blocking your ears.  The fragrant musk of her box fills every breath you take as you eat her out in a frenzy, tonguing her with wild desire and need as your world narrows to the gash between her legs.\n\n", false);

    outputText("Eventually, the vice-like pressure around your head releases, and you stagger back, blinking in confusion and allowing her balls to drop back into their normal place.  Her cock quivers on your " + hairDescription(player) + ", depositing a thick stream of warmth down the back of your head.  Her thighs are twitching and her balls pulsating as she cums, unloading a thick batch of herm-cream down your back.  You jerk to the side and stroke her as she unloads, reaching around to the top of the table and grabbing an empty glass.  Guiding the flared tip into the receptacle, you allow the unknowing hermaphrodite to fill the glass with her seed.\n\n", false);

    outputText("Finished at last, Urta's member rapidly deflates, the scent of her cum on your " + hairDescription(player) + " and her juice on your " + face(player) + " lingers.  You slide up into the booth next to your blissed out lover.", false);
    // [Give Glass] [Drink Glass] [Set Aside]
    simpleChoices("Give Glass", giveUrtaCumGlass, "Drink Glass", drinkUrtasCumGlass, "Set Aside", setAsideUrtaCumGlass, "", null, "", null);
}
...
// [Under Table BJ]
export function blowUrtaUnderTheTableLuv(): void {
    urtaSprite();
    player.slimeFeed();
    flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] = 2 + rand(3);
    urtaLove(1);
    ...
    outputText("Thumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ", false);
    if (player.breastRows.biggestTitSize() >= 1) outputText("dragging between your " + allBreastsDescript(player), false);
    else outputText("dragging across your chest", false);
    outputText(" as it begins to soften.  You wipe as much of her leavings from your " + face(player) + " as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta's member a firm squeeze, scolding it for its forceful treatment of your mouth.\n\n", false);
...
function rideUrtasCoochLoveyDovey(): void {
    ...
    outputText("Her cries are muted as you launch yourself over her, brutalizing her eager lips with a ferocious kiss.  You roughly tongue-fuck her as you adjust yourself, balancing yourself with one hand while your other guides her trembling shaft closer and closer to the entrance of your " + vaginaDescript(player, 0) + ".  You sigh happily and rub the pre-filled bulb against your opening, ", false);
    if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("growing moist", false);
    else if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("growing wetter and wetter as you lubricate the tip of her condom", false);
...
function rideUrtaInButtAtHomeLove(): void {
    ...
    outputText("Urta recovers enough to meet your gaze and licks her lips hungrily.  Her hands release your " + hipDescription(player) + " and rump, sliding up your sides and in towards your " + allBreastsDescript(player) + ".  You shiver as she circles your " + nippleDescription(player, 0) + ", teasing the flesh around them while managing to avoid providing them with any stimulation.  She looks up at you expectantly with a patient look, as if waiting on something from you.\n\n", false);

    outputText("You shift your hips and are rewarded by a caress over your left " + nippleDescription(player, 0) + ", but it stops almost immediately.  Catching on, you flex your " + legs(player) + " and lift yourself up until her flare is the only thing connecting you before sliding back down the pulsating shaft, squeezing your muscles around it.  Urta's hands reward you by pinching and tugging on your " + nippleDescription(player, 0) + "s, ", false);
    if (player.breastRows.hasFuckableNipples()) outputText("even slipping a finger inside their slippery depths, ", false);
    ...
    if (player.vaginas.length > 0) {
        outputText("  Neglected and unused, your " + vaginaDescript(player, 0) + " ", false);
        if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("dampens", false);
    ...
    outputText("The lovemaking increases in tempo, slowly working its way to a feverish crescendo.  You're bouncing atop the fox wildly, and your lover cums hard.  As full as you felt with her inside you, it's nothing compared to being stuffed with her cock AND cum.  Each cum-filled contraction swells her member from base to flare, allowing you to feel it stretch your insides while it travels up your " + assholeDescript(player) + ".  It dumps the load into the bubble of cum inside you, filling you with a creamy jizz balloon.\n\n", false);

    // (GENDERLESS ORGAZM)
    if (player.gender == 0) {
        outputText("You cry out with pleasure, feeling the closest thing you can to an orgasm.  Your muscles clench around the invading rod, milking it while the rest of your body trembles.  The strength drains from your " + legs(player) + " and arms and you twitch there, completely impaled on your lover's thick cock, happily and completely penetrated.\n\n", false);
    }
    // (EVERYBODY ELSE)
    else {
        outputText("You cry out with pleasure, feeling your body cum from the invading warmth in your " + assholeDescript(player) + ".  Shivering as the strength drains from your arms and " + legs(player) + ", you let the intruding and still cumming cock support your body.", false);
        if (player.vaginas.length > 0) {
            outputText("  Your " + vaginaDescript(player, 0) + " clenches desperately, peaking without a single touch, but hungry for something to milk.", false);
            if (player.vaginas[0].vaginalWetness > VaginaWetness.WET && player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.", false);
            else if (player.vaginas[0].vaginalWetness == VaginaWetness.SLAVERING) outputText("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.", false);
        }
        if (player.cocks.length > 0) {
            ...
            if (player.cumQ() >= 750 && player.cumQ() < 1250) outputText("  By the time you're done the balloon is the size of a basketball and looking dangerously close to bursting.", false);
                ...
                if (player.cumQ() >= 1700) outputText("  You fill it fuller and fuller, until it's bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your " + cockDescript(game.player, 0) + " continues to pump even more onto her.  So much for not making a mess!", false);
            }
        }
        outputText("\n\n", false);
    }
...
function oralFiestyUberExplosionUrta(): void {
    ...
    if (player.gender == 2 || (player.gender == 3 && rand(2) == 0)) {
        ...
        outputText("The musky scent of Urta's forearm-sized horse-prick wafts into your nostrils as it wobbles just below your " + face(player) + ", visibly twitching and throbbing from all the extra blood.  She whines plaintively, nuzzling her moist nose against your ", false);
        if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("equally wet", false);
        ...
        outputText("Urta eagerly gets to work, stabbing her tongue through your folds and into your " + vaginaDescript(player, 0) + ".  Her efforts bring her muzzle flush against you as she pauses to slurp your pussy-juices over your " + clitDescription(player) + ".  Her lips feel like liquid heat as they wrap around the incredibly sensitive bud, suckling it gently in between excursions into your passage.  Urta's slender fingers grab hold of your " + buttDescription(player) + " to support her as she tongue-fucks your " + vaginaDescript(player, 0) + " with abandon.\n\n", false);
        ...
        outputText("Not far from cumming yourself, you grind your " + hipDescription(player) + " on Urta's face, smearing her fur with your fragrant pussy-juices.  Her own moans of pleasure turn into noisy wet blubbering that vibrates through your " + vaginaDescript(player, 0) + ", getting you off on the spot.  You squeal around the cock in your ", false);
        ...
        if (player.vaginas[0].vaginalWetness > VaginaWetness.DROOLING) outputText("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.", false);
        ...
        outputText("The fox giggles and returns to cuddling and licking you.  You rest with a happy smile plastered on your " + face(player) + " from the experience.  After all the snuggling, you both rise and get dressed.  Urta finds a new dress in the closet and with a long goodbye kiss, you go your separate ways.\n\n", false);
        doNext(Camp.returnToCampUseOneHour);
        player.orgasm();
        dynStats("sen", -1);
        return;
    ...
    // (DUDEZILLAZ)
    outputText("Urta sits up and practically screams, \"<i>Really,</i>\" and squeals excitedly.  She claps and lies back as you climb into the bed, stopping only to yank off her cock-sheath and toss it into a nearby waste-bin.  You swing your " + legs(player) + " over her and get into position, aligning ", false);
    if (player.balls > 0) {
    ...
    outputText("Muffled, excited moans reach your ears as you work on your lover's pole.  The fox is clearly enjoying herself, and her sounds of pleasure cut off as she rams your " + cockDescript(game.player, x) + " deep into her throat.  She gurgles noisily while her tongue slides as far down your shaft as possible, licking at your ", false);
    if (player.balls > 0) outputText(ballsDescriptLight(player), false);
    else if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
    else outputText("taint", false);
    outputText(" while her spittle froths around her lips.  She manages to start humming deep within her throat, vibrating your " + cockDescript(game.player, x) + ".  The obstruction in your mouth stifles your moan, but Urta hears the startled gurgle and slides away, then plunges back down, humming the entire time in between gasps for air.\n\n", false);
...
export function urtaHomeLuvLuvinsMenu(): void {
    clearOutput();
    urtaSprite();
    ...
    if (player.isGoo() && player.skinType == SkinType.GOO) addButton(6, "Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms);
...
function urtasCoochiNeedsFuckedLove(): void {
    ...
    if (player.hasKnot(x)) {
        ...
        outputText("You rock back and forth, unable to pull out yet stimulating the sensitive bulb of your knot.  Your " + cockDescript(game.player, x) + " dumps its gooey cargo directly into your lover's stopped-up sex with barely audible squishing noises.  ", false);
        if (player.cumQ() < 250) { }
        ...
        else if (player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + cockNoun(CockTypesEnum.HUMAN) + " stops spewing inside her.  ", false);
        else {
            ...
            if (player.cumQ() < 3000) outputText("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ", false);
                ...
                if (player.cumQ() > 5000) outputText("Not content with such an easy release, your " + cockDescript(game.player, x) + " unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ", false);
            }
        }
        outputText("You finally feel your " + cockNoun(CockTypesEnum.HUMAN) + "'s knot shrinking and pull free with a loud and messy pop.  ", false);
    ...
    // [HORSE]
        ...
        if (player.cumQ() < 250) { }
        ...
        else if (player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + cockNoun(CockTypesEnum.HUMAN) + " has finished spewing your seed inside her.  ", false);
        else {
            ...
            if (player.cumQ() > 5000) outputText("Not content with such an easy release, your " + cockDescript(game.player, x) + " unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ", false);
        }
    ...
    // [Everybody Else]
        ...
        outputText("Your " + cockDescript(game.player, 0) + " clenches, filling her convulsing cunt as your seed begins pumping into her womb.  Urta cries, \"<i>oooooooOOOOHHhhh YESSSS... It's so warm and wet.  Yes, lover!  Keep goo-oh-OH-innnngg!!</i>\"\n\n", false);

        outputText("You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your " + cockDescript(game.player, 0) + " dumps its gooey cargo deep into your lover's canal with barely audible squishing noises.  ", false);
        if (player.cumQ() < 250) { }
        else if (player.cumQ() < 700) outputText("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ", false);
        else if (player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + cockNoun(CockTypesEnum.HUMAN) + " stops spewing inside her.  ", false);
        else {
            outputText("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your " + cockDescript(game.player, 0) + ".  ", false);
            if (player.cumQ() < 3000) outputText("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ", false);
            ...
            if (player.cumQ() > 5000) outputText("Not content with such an easy release, your " + cockDescript(game.player, 0) + " unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ", false);
        }
        outputText("You finally feel your " + cockNoun(CockTypesEnum.HUMAN) + " deflating, and you pull free with a messy slurp.  ", false);
        if (player.cumQ() >= 2000) outputText("Spunk rushes out in a torrent, splattering everywhere.  ", false);
        outputText("Urta's glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n", false);
    }
    // [RESUME]
    outputText("Urta calms down at last, though her member is still throbbing weakly as she peels her condom off.  The bloated cum-bubble at the tip is the size of a basketball, and she struggles to get it tied off and disposed of.  The fox tosses it into a waste-basket already full of the things and gives you a toothy smile, \"<i>I think somebody's " + cockNoun(CockTypesEnum.HUMAN) + " loves my pussy almost as much as I love you!</i>\"\n\n", false);
...
// [Back Room]
export function scyllaAndUrtaSittingInATree(): void {
    urtaSprite();
    outputText("", true);
    ...
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00147] == 0 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00145] > 0) {
        ...
        // Butans
        if (player.cocks.cockThatFits(urtaCapacity()) != -1 && player.lowerBody != LowerBodyType.CENTAUR) {
...
function urtaAndScyllaBoningLikeBitchesSober(): void {
    clearOutput();
...
function fuckUrtaWhileScyllaSucksSober(): void {
    ...
    outputText("  The velvet glove clasping your " + cockDescript(game.player, x) + " tightens without warning, accompanied by a full-body quake.  Urta's eyes roll back as some invisible force works through her, culminating in a muscular clench that you feel will pop your " + cockDescript(game.player, x) + " clean off your body.  You grunt, grabbing hold of the fox's tits for stability and wince from the almost painful tightness of her snatch.  The moment seems to drag on forever, but then, the vixen slumps back into your arms, allowing you to support her sagging weight.");
    ...
    outputText("\n\nMoaning, Urta whimpers, \"<i>S-so full... I - ungh - I... I can't hold it!  C-cuuuummmiiiiinnng!</i>\"  Her hips jerk forward hard and fast enough that it nearly pulls the gushing pussy off your " + cockDescript(game.player, x) + ", but Scylla isn't so lucky.  Her nose is squished almost flat into the fox's sweaty middle-fur, chin deep in ballsack.  She doesn't seem to mind; in fact, when drizzles of frothing fluid appear at the crinkled corners of the nun's ruby lips, her eyes roll back, exposing the whites.  Meanwhile, Urta's less-than-colt-sized dong swells, but not as a return to its former glory.  No, if anything, her musky dick is still shrinking.  The sheer size of the cum-globs pumping through her urethra are visibly stretching her horse-cock, giving the illusion of increased size.");
...
function dockWithUrtaUnderScyllasSoberSupervision(): void {
    clearOutput();
    ...
    if (player.vaginas.length > 0) outputText("half's ");
    ...
    outputText("\n\nDuring all this, Scylla's hands are stroking along your " + cockDescript(game.player, x) + " with delight that borders on rapture");
...
function watchTwoHotBitchesAndJerkIt(): void {
    ...
    outputText("You open your " + player.armorName + " and sit down on a crate no more than a few feet away from the orally fixated couple, fishing out " + sMultiCockDesc(game.player), false);
    if (player.vaginas.length > 0) outputText(" and exposing your " + vaginaDescript(player, 0), false);
    ...
    outputText("You stroke " + sMultiCockDesc(game.player), false);
    if (player.vaginas.length > 0) outputText(" and touch your " + vaginaDescript(player, 0), false);
    ...
    else outputText("\"<i>You're the besht " + mf(player, "man", "woman") + " a girl could ashk for.</i>\"", false);
...
function makeUrtaSitOnYourLapWithScylla(): void {
    ...
    if (player.lowerBody == LowerBodyType.NAGA) outputText(", snaking your tail around her legs", false);
    else if (player.lowerBody == LowerBodyType.GOO) outputText(", enveloping her legs with your goo", false);
    ...
    if (x != -1 && y != -1) outputText("Oooh, and in both holes?  You naughty " + mf(player, "boy", "girl") + ", you!", false);
    ...
    outputText("Scylla's neck is pulled tight as Urta's urethra is stretched wide by huge bulges of cum.  You can actually feel them sliding past the fingers you've dug into her sheath.  No sooner than one squeezes past is another on its way.  You wonder how Scylla can handle such incredible volumes and the only answer you can think of is magic.  Whatever the case, her swallowing gets noisier and Urta starts bouncing even faster atop your " + cockDescript(game.player, x), false);
    if (y != -1) outputText(" and " + cockDescript(game.player, y), false);
    ...
    if (y > -1) outputText("  Meanwhile, your other " + cockNoun(player.cocks[y].cockType) + " utterly fills her womb, stuffing it so completely that the fox's belly rounds out.", false);
...
export function chastityBeltFun(cameFromSexMenu: boolean = true): void {
    ...
    outputText("You take the mug and tip on your mouth, purposely letting some spill, forming rivulets of water across your " + skin(player, true, false) + ", " + (player.breastRows.hasBreasts() ? "past your breasts and down" : "down your chest and over ") + " your belly.  With a look of mock disappointment, you tell Urta you seem to have spilled some water, maybe she should take care of it?  Urta nods her head and immediately starts to head for a closet to fetch you a towel.  But you quickly stop her, by telling her that you want her to lick you clean.\n\n");
...
function friendsTalkExpack(): void {
    clearOutput();
    urtaSprite();
...
export function urtaDialogueMenu(): void {
...
function urtaDiscussesSelf(): void {
    ...
    if (urtaJustFriends())
        ...
        simpleChoices("Infertility", UrtaQuest.infertilityQuestions,
...
export function flirtWithUrta(): void {
    clearOutput();
...
function urtaFertilityDiscussion(): void {
    clearOutput();
...
function urtaFertilityToggle(): void {
    clearOutput();
    if (flags[kFLAGS.URTA_FERTILE] == 1) {
...
function urtaPregOthersToggle(): void {
    clearOutput();
    if (flags[kFLAGS.URTA_PREG_EVERYBODY] == 1) {
...
function urtaDiscussesEdryn(): void {
    ...
    const discussRomance: () => void = (urtaJustFriends() ? null : urtaDiscussesEdrynRomance);
...
function urtaDiscussesTheWatch(): void {
    ...
    choices("Membership", urtaDiscussesWatchMembership, "Role", urtaDiscussesWatchRole, "Crime", urtaDiscussesCrimeAndPunishment, "Reach", urtaDiscussesTheWatchsReach, "", null,
        "", null, "", null, "", null, "", null, "Back", urtaDialogueMenu);
}
...
function urtaDiscussesFamily(): void {
    ...
    if (!urtaLove()) {
        ...
        UrtaQuest.talkWithUrtaAboutFamFam();
    }
}
...
function urtaDiscussionTeaseAfterRomance(): void {
    ...
    if (!urtaLove())
        simpleChoices("Hidden BJ", blowUrtaUnderTable, "Urta's Place", goBackToUrtasForLuvinz, "", null, "", null, "", null);
    else {
        if (pregnancy.type == PregnancyStore.PREGNANCY_PLAYER)
            simpleChoices("Her Place", goBackToUrtasForLuvinz, "", null, "Eat Out", eatUrtaOutNomNomPussy, "", null, "", null);
        else simpleChoices("Her Place", goBackToUrtasForLuvinz, "Suck Off", blowUrtaUnderTheTableLuv, "Eat Out", eatUrtaOutNomNomPussy, "", null, "", null);
    }
    dynStats("lus", 25);
}
// [=Employment=]
...
function urtaDiscussesEdrynRomance(): void {
    ...
    if (!urtaLove()) {
        if (!urtaOpenAboutEdryn()) {
            ...
            if (player.vaginas.length > 0) outputText("Until you, she was the only woman who didn't freak out and insist it was handjobs or nothing when she saw how big I was.  ");
        ...
        // Subsequent Regular Variant:
            ...
            if (player.effects.getValue1Of(StatusAffects.Edryn) >= 5) {
...
export function spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs(): void {
    clearOutput();
...
function urtaDiscussessKatherine(): void {
    clearOutput();
    const affection: number = flags[kFLAGS.KATHERINE_URTA_AFFECTION];
...
function urtaKathSexWhenever(): void {
...
function urtaKathSexEncourage(): void {
...
// Eggs Option
export function giveTheFoxSomeEggs(): void {
    clearOutput();
    ...
    if (player.vaginas.length > 0) wetness += player.vaginas.wetness() * 100;
...
export function urtaChewsOutPC(newScreen: boolean = true): void {
    ...
    if (flags[kFLAGS.URTA_FERTILE_EGGS] == 0) {
        ...
        if (player.lib > 49 && player.vaginas.length > 0) {
...
function repeatUrtaEgging(): void {
    clearOutput();
...
// [Condomless]
function condomlessUrtaInHouseSmex(): void {
    clearOutput();
...
function pegUrtasAssMessy(): void {
    ...
    outputText("  Scooting forward, you edge your " + cockHead(player, x) + " up to her anus.  Her body heat washes over you as you enjoy the feel of her pucker's folded flesh pressing down on your most sensitive point, lubricated by the copious female fluid that drains from the sodden gash above.");
    outputText("\n\nYou begin to gently rock your [hips] forward, giving little, gentle pushes against the resistant muscle.  At first, you make no progress, simply forcing her anal ring to dilate, little by little.  Then, after a particularly vigorous stroke, your " + cockHead(player, x) + " slides through the clinging ring and into the tight embrace of her slick anus.  You stop to allow her to adjust, admiring her ");
    ...
    if (urtaDrunk()) outputText("h");
    outputText("o goooo-ooooooh yes!</i>\"  Her ass clutches and relaxes to your tempo, hugging your " + cockDescript(game.player, x) + " tight");
    ...
    outputText(", kissing your " + cockHead(player, x) + " every time it peeks past her tits, relishing the taste of her own lust all over it.");
    ...
    outputText("  Climbing out of the alabaster-painted bed is no easy task, but the two of you manage, laughing and leaning on each other's arms.  Urta leads you to a cabinet in another room, and bends over to open it.  You barely notice her pulling out mops, buckets, and cleansers.  Instead, your vision is riveted to her asshole, still gaped lewdly in the shape of your " + cockDescript(game.player, x) + ".  When she comes up with an armload of cleaning supplies, you're ");
...
function UrtaGetsPeggedCleanUp(): void {
    clearOutput();
...
function fillMeUpPleaseUrta(): void {
    clearOutput();
    ...
    if (player.tongueType > TongueType.HUMAN) outputText("prodigious ");
    ...
    if (player.vaginas.wetness() > 3) outputText("  You're so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.");
    ...
    if (player.vaginas.wetness() > 3) outputText(", though it's a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk");
    ...
    if (player.breastRows.biggestTitSize() >= 4) outputText(" and your [chest] rest heavily upon your cum-stuffed form");
    outputText(".");
...
function cleanUpAfterUrtaCreamInjectsYou(): void {
    clearOutput();
    if (player.cor >= 50) outputText("You start to duck out, but the disapproving glare the fox gives you makes it clear that you won't be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ");
...
function lippleBondUrtaScylla(): void {
    clearOutput();
...
function nagaOnUrtaMess(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        // [if (isSquirter = true)]
        if (player.vaginas.wetness() >= 4) outputText("  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.");
    }
    // if (cocks > 0)
    if (player.cocks.length > 0) {
...
function lickOutUrtaAtHome(): void {
    clearOutput();
    const x: number = player.cocks.cockThatFits(urtaCapacity());
    if (x < 0) {
...
export function slurpFawkesCocksForFunAndInflation(): void {
    clearOutput();
    ...
    outputText("\n\nThe waitress approaches as Urta releases you, glancing around the bar at the commotion, \"<i>Quite a scene ya caused, " + mf(player, "cutey", "girly") + ",</i>\" she looks at your stomach, \"<i>Would you like anything to, uh... wash that down with?</i>\"");
...
export function urtaAndRaphaelSurprise(): void {
    clearOutput();
...
function raphaelAndUrtaPartTwo(): void {
    clearOutput();
    ...
    if (player.tailType > TailType.NONE) outputText("lift your tails and ");
...
function urtaGooTesticleVoreRuinedOrgasms(): void {
    clearOutput();
...
function urtasRuinedOrgasmsFromGooPartII(): void {
    clearOutput();
    ...
    else outputText(mf(player, "boy", "girl"));
...
function urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice(): void {
    clearOutput();
    // First Time
...
function ignoreUrtasCollar(): void {
...
function wearZeCollar(): void {
    clearOutput();
    if (flags[kFLAGS.URTA_PETPLAY_DONE] == 0) {
        outputText("You reach down into the trashcan, unclasp the collar and slip it on.  You tighten it until it fits snugly against your " + skinFurScales(player) + ", but isn't otherwise uncomfortable.  With a satisfying <b>click</b>, the clasp snaps shut, so you know there's no going back.  Urta's mouth drops as she watches you do this, completely at a loss for words.  Her cock, on the other hand, hardens, knowing just what to do.");

        outputText("\n\nYou sink to your knees and then onto your hands in front of her, brazenly displaying your submission to the vixen goddess before you.  She leans back slightly, stunned by this action.  It takes her a minute to recover and pick her jaw up off the floor.  You dutifully wait, silently, until Urta issues a command.  She seems to sense this and clears her throat, clearly embarrassed and nervous.  \"<i>Oh, um, good " + mf(player, "boy", "girl") + ",</i>\" she murmurs, patting your head.  You rub your face into her palm, which brings a smile to Urta's face.");
        ...
        if (player.gender == 3) outputText(" and ");
        if (player.vaginas.length > 0) outputText("wetter");
    ...
    // Second Time
        ...
        if (player.tailType > TailType.NONE) outputText(", tail wagging");
    ...
    if (player.tailType == TailType.NONE) {
        ...
        if (player.tailType == TailType.NONE && player.ass.analWetness > 0) outputText("  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.");
...
function doggyStyle(): void {
    ...
    if (player.tailType > TailType.NONE) outputText("asshole");
    ...
    if (player.breastRows.totalNipples() == 2) outputText("the ");
    else outputText("an ");
    ...
    if (player.tailType == TailType.NONE) outputText(", while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won't be empty for long, however");
    ...
    if (player.breastRows.biggestTitSize() <= 5) outputText("resting on your folded arms");
    ...
    if (player.vaginas.length > 0) outputText(" unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover's balls");
    outputText(".");
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.vaginas.length > 0) outputText("  It then travels along to your ");
    }
    if (player.vaginas.length > 0) outputText("[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner's balls and legs in sticky, clear fluids.");
...
function getAPetReward(): void {
    ...
    if (player.breastRows.biggestTitSize() < 1) outputText("onto your chest");
    ...
    if (player.breastRows.totalNipples() == 2) outputText("its twin");
    ...
    if (player.cocks.length > 0) outputText("dick");
    else if (player.vaginas.length > 0) outputText("cunt lips");
    ...
    if (player.vaginas.length > 0) {
        ...
        else if (player.vaginas.length > 0) outputText("  Meanwhile");
    ...
    outputText("tongue along the vixen's thick cumvein, which pulses lightly.  Urta groans and runs a hand through your " + hairDescription(player) + ", gripping it solidly.  You can't help as you're forced further onto the warm rod.");
...
// Walkies!
function urtaTakesPCOnWalkies(): void {
    clearOutput();
    ...
    if (player.gender == 3) outputText(" and ");
    if (player.vaginas.length > 0) outputText("your [vagina] moistens, ready to be on display");
    outputText(".");
    ...
    if (player.breastRows.biggestTitSize() >= 5) outputText("the breasts hanging from your chest and ");
    ...
    outputText("\n\nYou " + mf(player, "laugh", "giggle") + " and confirm you had a great time as well.  Urta gives you a sheepish look, before asking if you'd ever want to do that again.  You lean in for a kiss, and let that answer the question.");
    dynStats("lus", 15);
    flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] = 0;
