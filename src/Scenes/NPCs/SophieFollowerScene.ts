...
export function sophieFollower(): boolean {
...
// Un-Bimbo*
export function unbimboSophie(): void {
    clearOutput();
...
function yesDebimboSophie(): void {
    ...
    outputText("\n\n\"<i>Wha... what did you DO TO ME!?!?!</i>\" Sophie screams, a look of sheer feral rage splayed across her still-bimbofied face, full-lips drawn into a snarl beneath gleaming eyes and long golden hair.  Her shiny yellow feathers ruffle as she stomps up to you, putting her face right up to yours.  \"<i>You " + mf(player, "bastard", "bitch") + "!  WHAT DID YOU DO TO ME?</i>\"");
...
function bimboSophieAgain(): void {
    clearOutput();
...
function beatSophieAroundYouMonster(): void {
    clearOutput();
...
export function beatUpDebimboSophie(): void {
    clearOutput();
...
export function debimboSophieBeatsYouUp(): void {
    clearOutput();
...
// Let Her Go*
function letDebimboSophieGo(): void {
    clearOutput();
...
function whyIDidItToDebimboSophie(): void {
    clearOutput();
...
function apologizeToDebimboSophie(): void {
    clearOutput();
...
export function sophieDaughterDebimboUpdate(): void {
...
export function followerSophieMainScreen(): void {
    clearOutput();
    SophieBimbo.sophieSprite();
    // Sophie is in season
...
// [Sex]
function sexWithFollowerSophie(): void {
    clearOutput();
    menu();
...
// Sleep With:*
function sleepWithSophieToggle(): void {
    clearOutput();
    if (flags[kFLAGS.SLEEP_WITH] != "Sophie") {
...
function fuckFollowerSophie(): void {
    ...
    outputText(", her words still affect you.  Her pussy just feels... so... exquisite.  It's so hot and wet that you almost feel like you could cum right now.  Your [legs] wobble, but Sophie just pushes back even harder, holding you up by your " + cockDescript(game.player, x) + " and the friction of your [butt] on the boulder.  The harpy watches your expression as her hips continue their slow roll, feeling your fingers twitch and knead her immense backside when the pleasure begins to mount.  You try to stay composed, but it's just so gods-damned good!  Her silken slit is so warm, wet, and just plain... erotic.  Slowly, the point where you would normally orgasm approaches, but instead of the spasming clenches that signal release, your jizz stays bottled up inside you tighter than sperm in a goblin's womb.");

    outputText("\n\nSophie begins to grunt and move more forcefully.  Her slit batters hard up against you as she cries, \"<i>Fuck yes!  Oooh, yeah that feels good.  You're a good " + mf(player, "boy", "girl") + ", not cumming just yet.</i>\"  The compelling tone returns.  \"<i>Massage my butt and ");
    ...
    if (y >= 0) outputText("You shift to bring your " + cockDescript(game.player, y) + " up in place, just in time for Sophie's wiggling rectum to spread around it.  It slides on in without so much as a whimper.");
...
// [Next]
function sophieVagFollowerFollowup(): void {
    clearOutput();
    let x: number = player.cocks.cockThatFits(SophieBimbo.sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    const y: number = player.cocks.cockThatFits2(SophieBimbo.sophieCapacity());
...
// Blowjob*
// Sucking dicks.
function sophieFollowerGivesBlowjobs(): void {
    clearOutput();
    let x: number = player.cocks.cockThatFits(SophieBimbo.sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
...
// Blow Sixtynine*
function sophieBlowsSixtyNine(): void {
    let x: number = player.cocks.cockThatFits(SophieBimbo.sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    clearOutput();
    ...
    if (player.tongueType > TongueType.HUMAN && player.tongueType <= TongueType.DRACONIC) outputText("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
...
// Forceful Blowjob*
function forceSophieBlowjob(): void {
    let x: number = player.cocks.cockThatFits(SophieBimbo.sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    clearOutput();
    ...
    outputText("\n\nSophie grumbles, but one of her hands does nestle itself down between her feathery thighs, her other going to your groin to fish out your " + cockDescript(game.player, x) + ".  She looks up at you through angry eyebrows softened slightly by concealed lust.  \"<i>I'll suck you off... this time,</i>\" she declares, ");
...
export function sophieSmallDongTeases(): void {
    ...
    if (player.breastRows[0].breastRating >= 1) outputText(" and squeezes your " + breastCupOfRow(player, 0) + " breast roughy.");
    else outputText(" and lets the talons dig into your " + skinFurScales(player) + " just enough to hurt a little.");

    outputText("\n\nSophie licks her shining lips as she looks down at the little spit-soaked pole that's twitching between her legs, just inches away from her pussy.  \"<i>Oooh, does Momma Sophie's favorite girl" + mf(player, "-boy", "y-girl") + " want her little bitch-stick played with?</i>\"  She touches it, right below the [cockHead smallest], her warm fingertip sliding in slow, sensuous circles until you begin to twitch at her touches.  \"<i>You know what I think?</i>\" she purrs, bending over to look you face to face- an amazing feat of flexibility considering her foot is locked on your shoulder.  \"<i>I think you like it when I play with this tiny girl-cock.  I think some depraved, sick, nasty part of your brain makes you cum harder when I tease you about how feminine this bitch-cock is.</i>\"");
    ...
    // {BOTH END HERE}
    outputText("\n\nThe older harpy giggles, when it ends.  \"<i>I love sensitive " + mf(player, "lady-boys", "girls") + " like you...  You let me feel so powerful...</i>\"");
...
// Similar to bimbo sophie
export function sophieFollowerGoesIntoSeas(): void {
...
export function sophieFertilityExpired(): void {
...
export function sophieFertilityKnockedUpExpired(): void {
...
function sophieSpecial(): void {
    ...
    outputText(cockDescript(game.player, x) + " inside Sophie's schlong-milking passage, causing her to moan in satisfaction as the first of your virile cum hits her inner walls.");
    ...
    outputText("\n\nWith a grin, Sophie rises from your " + cockDescript(game.player, x) + ", ");
    ...
    outputText("\n\nWith a grunt, you manage to withhold from her command, and Sophie coos with delight at your resistance.  \"<i>Look at my " + mf(player, "boy", "girl") + ", all grown up now, resisting the pleasures of Momma Sophie's fertile, cum-thirsty cunny!  You shouldn't do that... though, maybe we could cum together if you do... Okay, baby, once you feel Momma Sophie's pussy squeeze you extra hard, you will cum and you won't try to hold back, okay?</i>\"  You grunt and nod, but already find it hard to hold back when you imagine how Sophie will look knocked up with your kids. Once you cum inside her a second time, there's probably no way it won't hold.");

    outputText("\n\nYou hear Sophie's shlicking vagina swallow your entire " + cockDescript(game.player, x) + " and release it repeatedly, her hips and soft, plump ass bouncing off of your [legs] as she continues to fuck herself on your " + cockDescript(game.player, x) + " harder and harder.  The way her inner folds lap and massage at your drug-infused cock is exquisite, and it takes every ounce of willpower to indeed stop yourself from cumming inside her repeatedly like you were broken down there.  Finally, Sophie slaps her hips into your hardness, moaning out loud as her cunt squeezes and tugs at your member violently, temporarily stopping the flow of cum out of your " + cockDescript(game.player, x) + ", but only for a split second.  It churns and boils in your [balls], raising through your " + cockDescript(game.player, x) + " to be fired into Sophie's wet, ravenous cunt and baby-wanting womb.  Sophie's own orgasm and yours blend into one and prolong one another as you release ");
    ...
    outputText("\n\n\"<i>Ahhh... ahhh... good " + mf(player, "boy", "girl") + "... very good... I'm definitely a momma now.  But... buuuut... You should squirt out more just to make sure... and as punishment for trying to resist when Momma Sophie ordered you to cum!</i>\"");
    ...
    outputText("\n\nShe says that, but every lift and fall of her hips is a torture on your thoroughly milked, overly sensitive " + cockDescript(game.player, x) + ".  Her pussy is trying to draw out every drop of fluid you have in your body, and soon, it will succeed at doing just that it seems...");
...
// Initial Intro
export function sophieFollowerIntro(): void {
...
// Get Out
function getOutSophieRecruitment(): void {
    clearOutput();
...
// Kiss Her
function kissSophieRecruitment(): void {
    clearOutput();
...
// If Sure
function sophieRecruitmentFinale(): void {
    clearOutput();
...
export function sophieIncestInHerCooterOrSomethingIDunno(): void {
    clearOutput();
    outputText("Approaching the harpy nest you call out for one of your daughters.  ");
    if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] > 1) outputText("The prospect of their " + mf(player, "father", "mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
    ...
    let daughterCap: number = flags[kFLAGS.SOPHIE_ADULT_KID_COUNT];
    ...
    if (daughterCap == 0) {
        ...
        if (flags[kFLAGS.DAUGHTER_ONE_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Like, how do I look, " + mf(player, "Daddy", "Mommy") + "?  Aren't my breasts so totally soft?  Don't you just want to suck and squeeze them?  Ooohhh!  " + mf(player, "Daddy", "Mommy") + ", can you please touch me?  I want you so bad!  My pussy gets sooo wet when I, like, think about you pounding my cunny!</i>\"");
        // normal speech:
        else outputText("\"<i>Don't you like how big and soft my breasts are, " + mf(player, "Daddy", "Mommy") + "?  Don't you just want to squeeze them and suck on them?  Ooohh, " + mf(player, "Daddy", "Mommy") + ", please, I want you so bad.  I get so hot and wet when I think of you!  I just want your [cock] inside me, pounding my hot cunt until I scream!</i>\"");
        ...
        if (flags[kFLAGS.DAUGHTER_ONE_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\n\n\"<i>Ohhh, oooohhhh!  " + mf(player, "D-Daddy", "M-Mommy") + "!  L-like, I can't take so much teasing!  Please, Gimme your h-haaard cock, " + mf(player, "Dad", "Mom") + "!<i>\"");
        // [normal speech:
        else outputText("\n\n\"<i>Ohhh, oooohhhh!  " + mf(player, "D-Daddy", "M-Mommy") + "!  Nnnnn... I can't take so much teasing!  Please, please give it to me, fuck me as hard as you can, " + mf(player, "Daddy", "Mommy") + "!<i>\"");
    }
    // second daughter:
        ...
        if (flags[kFLAGS.DAUGHTER_TWO_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + mf(player, "Daddy", "Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
        else outputText("\"<i>Come on, " + mf(player, "Daddy", "Mommy") + ", touch me, spank me like the bad girl I am, lusting after my " + mf(player, "Daddy", "Mommy") + " like this.  Give it to me hard, spank my big round ass and pound my hot tight cunt like I know you want</i>\"");
    ...
    // third daughter:
        ...
        outputText("\n\nYou clap your hands and applaud your daughter's work.  The curvy little harpy is working so hard to please her " + mf(player, "daddy", "mommy") + " and entice him into her nest...  ");
        if (flags[kFLAGS.DAUGHTER_THREE_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>L-like, did I do a good job, " + mf(player, "Daddy", "Mommy") + "?  I so totally did my best for you.  I know I'm not super sexy like Mom and my sisters... but, " + mf(player, "Daddy", "Mommy") + ".. I will like, still please you, like, totally the best that I can!</i>\"");
        else outputText("\"<i>W-well, what did you think, " + mf(player, "Dad", "Mommy") + "?  Did I do a good job?  I know I'm not as hot as Mom, but I promise I'll please you as best I can!</i>\"");
        ...
        if (flags[kFLAGS.DAUGHTER_FOUR_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Come on " + mf(player, "Daddy", "Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
        else outputText("\"<i>Just look at me, " + mf(player, "Daddy", "Mommy") + ", so curvy, so ready, so hot and wet for you.  Just let loose and show me your love, show me how much you love me, " + mf(player, "Daddy", "Mommy") + ", fuck me hard, make me scream and claim me as yours.</i>\"");
        ...
        // bimbo speech
        if (flags[kFLAGS.DAUGHTER_FOUR_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("  \"<i>Ahh... ohhh... oh, " + mf(player, "Daddy", "Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
        // [normal speech:]
        else outputText("  \"<i>Mmmm... Oh, " + mf(player, "Daddy", "Mommy") + ", yessss...  That's it, touch me, touch me and play with me, do whatever you want with me.</i>\"");
...
function phaseTwoOfIncest(daughter: number): void {
    clearOutput();
    let x: number = player.cocks.cockThatFits(SophieBimbo.sophieCapacity());
    if (x < 0) x = player.cocks.smallestCockIndex();
    daughter++;
    ...
    outputText("\n\nBetween your legs, [eachCock] flops free, your " + cockDescript(game.player, x) + " achingly hard and ready for your fuck-hungry daughter.  The mass of masculine flesh throbs with your arousal, the insatiable organ longing to be sheathed within the welcoming depths of your daughter's soaking cunt, her tight, velvety folds ready to accept your potent, virile essence.");

    if (flags[kFLAGS.DAUGHTER_FOUR_BIMBO] > 0 && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0)

        const bimbo: boolean = (((daughter == 4 && flags[kFLAGS.DAUGHTER_FOUR_BIMBO] > 0) || (daughter == 3 && flags[kFLAGS.DAUGHTER_THREE_BIMBO] > 0) || (daughter == 2 && flags[kFLAGS.DAUGHTER_TWO_BIMBO] > 0) || (daughter == 1 && flags[kFLAGS.DAUGHTER_ONE_BIMBO] > 0)) && flags[kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED] == 0);

    if (bimbo) outputText("\n\n\"<i>Yessss!  Please, " + mf(player, "Daddy", "Mommy") + "!  Give it to me!  Like, fuck my brains out till I can't think of anything but your, like, totally amazing cock!</i>\"");
    else outputText("\n\n\"<i>Yesssss! Please, " + mf(player, "Daddy", "Mommy") + ", I've been wanting this so badly!  Give it to me, fuck me like the stud you are and fuck my cunt full of spunk!</i>\"");
    outputText(" she croons, her words dripping with raw, passionate need for you and your incestuous love.  The sight of her so needy, so wanting, her body aflame with an unquenchable hunger for you is just too tempting.  Your own lust boils over as a thick dollop of pre-cum bursts from your " + cockHead(player, x) + " and splatters across your daughter's sodden snatch, adding to the lube already waiting to welcome you into her body.");

    outputText("\n\nTaking your sensitive, throbbing cock in hand, you give yourself a stroke and let out a groan at how ready you are; how hard you are for the incestuous fuck you're going to enjoy.  With as much grace as a fuck-hungry beast, you align yourself with your daughter's tender folds and plunge yourself in, your " + cockDescript(game.player, x) + " sliding into her slippery cunt easily thanks to all the sweet juices oozing from her.  The sudden hard thrust into her body sends shockwaves of pleasure through your over-aroused daughter, her legs wrapping around your waist as soon as you hilt yourself inside her.  Not waiting for her to get used to you, you start to buck your hips, pistoning your " + cockDescript(game.player, x) + " into her over and over as you keep her pinned to the ground.  Her bird-like cries of delight fill the air and mix with the hard, slippery slaps of your hips against hers, telling you all you need to know about her pleasure and how she loves to be fucked by her " + mf(player, "father", "mother") + ".");
    ...
    if (bimbo) outputText("  \"<i>Oh, " + mf(player, "Daddy", "Mommy") + ", oh, " + mf(player, "Daddy", "Mommy") + "...  So good!  My " + mf(player, "Daddy", "Mommy") + "'s such a hot, sexy " + mf(player, "stud", "herm") + ".  I so totally thought I was going to like, lose my mind for a second.  I love you, " + mf(player, "Daddy", "Mommy") + ".</i>\"");
    else outputText("  \"<i>Oh, " + mf(player, "Daddy", "Mommy") + ", oh, " + mf(player, "Daddy", "Mommy") + "... so good, you're such a fucking " + mf(player, "stud", "perfect breeder") + "!  I could just get addicted to you.  I love you, " + mf(player, "Daddy", "Mommy") + ",</i>\"");
