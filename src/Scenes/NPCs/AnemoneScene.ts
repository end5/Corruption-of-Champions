...
export class AnemoneScene implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (player.effects.findByType(StatusAffects.AnemoneArousal) >= 0) {
...
export function mortalAnemoneeeeee(): void {
    ...
    if (flags[kFLAGS.TIMES_MET_ANEMONE] == 0 || player.inv.hasItem(ConsumableLib.MINOCUM)) {
        ...
        outputText("The initial surprise subsides to wooly-headedness and a feeling of mild arousal as the stingers in her tentacles find exposed flesh.  In panic of drowning you pull free of the ropy mass and backpaddle away from the girl until your " + feet(player) + " reassuringly touch the shallows of the lakebed once again and you're far enough above water to be able to fight.\n\n", false);
    } else {
        flags[kFLAGS.TIMES_MET_ANEMONE]++;
...
// victory:
export function defeatAnemone(): void {
    outputText("", true);
    // Win by HP:
    ...
    if (player.lust >= 33) {
        ...
        if (player.vaginas.length > 0 && player.breastRows.biggestTitSize() >= 4 && player.armor instanceof LustyMaidensArmor) bikiniTits = (player.armor as LustyMaidensArmor).lustyMaidenPaizuri;
...
function victoryButtholeRape(): void {
    ...
    outputText("Quite soon you can hardly stand your own arousal and your lover's cock is nice and hard.  Straddling the anemone, you position your " + assholeDescript(player) + " over her colorful shaft and gradually lower yourself towards it.  The florid crown slips into your hole, delivering the expected shock, and a gasp from behind you is accompanied by the anemone's hands moving to your hips.", false);
    // [butt hymen removal]
    if (!buttChange(player, monster.cocks.cockArea(0), true)) outputText("  ", false);
    outputText("Despite your anticipatory stiffening you find yourself trembling and your muscles weakening, but by a superb effort you manage to concentrate and lower yourself gently, savoring the slow crawl of the warmth up your " + assholeDescript(player) + ".  You reach the base of the anemone's short shaft soon, and pause for a minute; looking over your shoulder at the anemone, you notice her biting her lower lip impatiently.  Making a mental note of her cute expression to fuel your imagination, you turn forward and, putting your hands down for support, you begin to rise and fall on her erect penis.\n\n", false);

    outputText("With your " + assholeDescript(player) + " heated up from the aphrodisiac already, the friction is enough to warm it to fever pitch.  Over and over you impale yourself on the girl's rod, dragging trails of venom and heat up and down your rectum.", false);
    ...
    outputText("Your blue lover, restless now and uncontent to simply lie there anymore, begins to use her own hands and hips to pump in time with you, doubling the pace of the fuck.  The fervid intensity of her strokes doesn't leave any time for the venom to disperse before the next thrust arrives, making it feel as though your " + assholeDescript(player) + " is filled with one huge, hot cock that nevertheless slides in and out even as it remains motionless.  The sensation pushes you to orgasm quickly, your " + assholeDescript(player) + " clamping down on the anemone's penis", false);
    if (player.cocks.length > 0) {
        outputText(" as " + sMultiCockDesc(game.player) + " twitches and ejaculates in a ", false);
...
function rapeAnemoneWithDick(): void {
    ...
    if (player.cocks.cockThatFits(36) >= 0) {
        const x: number = player.cocks.cockThatFits(36);
        ...
        outputText(".  Unfastening your garments, you stroke " + sMultiCockDesc(game.player) + " to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand over her, she leans forward and opens her mouth invitingly.\n\n", false);
        ...
        outputText(" pull apart her feathery labia, showing a velvety interior.  <i>\"Ok...\"</i> she says haltingly.  You accept the invitation in a hurry, kneeling down and holding onto her hips, then guiding your " + cockDescript(game.player, x) + " into her.\n\n", false);
        ...
        if (player.cocks.length > 1) {
            ...
            if (player.cocks.length == 2) outputText("s", false);
            outputText(" up against the feelers lining her pussy.  Unexpectedly, they also contain the venomous nematocysts of her tentacles and in that single stroke " + sMultiCockDesc(game.player) + " is throbbing hard and squeezing pre-cum all over her groin.  She reaches down and plays with it", false);
            if (player.cocks.length > 2) outputText(", and them,", false);
        ...
        // [(if PC boobs > A)
        if (player.breastRows.biggestTitSize() >= 1) outputText(" in time with your own", false);
        outputText("; here and there one of the bounces brush her gills away, giving you a tantalizing view of nipple.", false);
        // [(if balls)
        if (player.balls > 0) outputText("  As your thrusts get faster your " + sackDescript(player) + " starts to slap into the tickly and quite-venomous feelers fringing her pussy, getting hotter and hotter as the aphrodisiac builds in your " + ballsDescriptLight(player) + ".  Your body answers with swelling, causing your sack to feel bigger and tighter with every slap.", false);
        // [(if noballs and clit > 6")
        else if (player.clitLength >= 6) outputText("  As your thrusts into the blue girl arouse you, your " + clitDescription(player) + " begins to swell with blood.  Pretty soon it's so erect that a particularly violent thrust mashes it into the feelers at the base of her labia, delivering a shock that almost makes you collapse.  The anemone, her reverie interrupted as you temporarily stop pumping, looks down.  Noticing the massive girl-cock sticking out of your " + vaginaDescript(player, 0) + ", she reaches between her legs and gives the tip a flick, grinning with sadistic playfulness.  Your eyes cross at that, sending her into a spasm of giggling.  Irritated but aroused by the abuse of your " + clitDescription(player) + ", you move your thumb over her own tiny blue button and begin flicking it in revenge as you renew your pumping.", false);
        outputText("\n\n", false);
        ...
        outputText("After the last of your cum has been squeezed out, you pull your " + cockDescript(game.player, 0) + " out and rinse it off in the lakewater.  You gather your gear while the anemone holds her belly and smiles placidly, staring into the sky.", false);
    ...
    // Too-big male: -requires cockarea > 36
        ...
        outputText(".  Unfastening your garments, you stroke " + sMultiCockDesc(game.player) + " to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand ", false);
        ...
        outputText("You smile at how stunned she is by you, and waggle your erect " + multiCockDescriptLight(game.player) + " from side-to-side.  The anemone obediently watches it swing, like a hypnotist's pendulum; her mouth reflexively opens as all her conscious thought is temporarily sidetracked.  You push on the shaft with one hand and move the tip down relative to her body, watching bemused as the girl tries to keep her mouth in line with it until it goes too low for her neck and snaps her out of her daze.  She closes her mouth and looks at you quizzically.  <i>\"No?\"</i> she asks.  You answer by pushing forward slightly, bumping the head of your " + cockDescript(game.player, 0) + " against her <i>mons pubis</i>.  The skin on her face darkens a bit as she realizes your intention... which turns out to be a blush, by the looks of the shy glance she gives you next!  <i>\"Umm.\"</i>  She gives it a moment of concerned study before her natural instincts take over and the anemone's fingers", false);
        // [(HP defeat)
        if (monster.HP < 1) outputText(" move to the lips of her vagina and", false);
        outputText(" pull apart her feathery labia, showing a velvety interior.  <i>\"Ok...\"</i> she says, clearly unsure of the wisdom of this.  You motion for her to lay back and lift her pussy as high as possible to reduce the angle, then attempt to guide your " + cockDescript(game.player, 0) + " into her.", false);
        ...
        outputText("The first few inches are slow going, as you try to stretch the blue girl's roughly human-sized pussy around your superhuman girth.  She sports a worried expression as you struggle to push the crown of your " + cockNoun(CockTypesEnum.HUMAN) + " in without bending your shaft.  The girl's body proves more elastic than you'd expected, though; with each shove her cunt takes more width without suffering any obvious ill effect.  Eventually you get the head of your " + cockDescript(game.player, 0) + " into her, and you give an experimental thrust to test her depths. You slide surprisingly far into her, your huge " + cockNoun(CockTypesEnum.HUMAN) + " making a bump in her pelvis; the anemone has abandoned her worry and is blissfully tweaking her nipples.  Taking the anemone's legs under your arms, you begin pumping at her, savoring the sensation of having at least part of your unwieldy tool inside someone.  Your blue partner gives out cute moans as you fuck her, prompting a grin from you, but most of your attention is focused on maintaining the angle of your " + cockDescript(game.player, 0) + ".  As you focus on the thrusts, you gradually notice the bump sliding further up her pelvis on each push... it's nearly up to her stomach now!  You quickly make up your mind to test her unusual biology, choking up on your grip of her legs and pushing hard with each thrust in.  The anemone is now tracking the progress of the bump representing your " + cockNoun(CockTypesEnum.HUMAN) + " as intently as you are; as your head pushes up her chest between her breasts she presses them together against the ridge, massaging them against the shaft through her skin.", false);
        // [(cocklength > 60")
        if (player.cocks[0].cockLength > 60) outputText("  The imagery and the stimulation inspire you to increase your efforts, and you push even harder into her until your " + cockDescript(game.player, 0) + " slides its way into her throat, expanding her slender neck to twice the normal width.  The anemone's mouth gapes open reflexively as if to gag as the pressure forces her head back, but she doesn't stop moaning or rubbing herself against you.  This seems to be as deep as you can get; the tip of your cock is now right up against the base of her chin.", false);
        outputText("\n\n", false);

        outputText("Inordinately pleased at having gotten your " + cockDescript(game.player, 0) + " so far in, you begin thrusting grandly, alternating huge back-and-forth pumps with hip gyrations that press you against the sides of your partner.  The anemone", false);
        // [(dick > 60")
        ...
        outputText(" has clearly been enjoying the treatment; soon she begins twitching in orgasm and her pussy spasms against the base of your " + cockNoun(CockTypesEnum.HUMAN) + ", flicking its venomous feelers into it, as she kneads her breasts in her hands.  Her body attempts to convulse, bending your " + cockDescript(game.player, 0) + " slightly, while her neglected dick gives a little squirt of pearly semen which lands on the raised line in the center of her body and makes a little trail as it rolls down.  The pressure of her twisting and the sensation of her pussy lapping at your shaft with aphrodisiac is enough to send you over the edge as well.  Your " + cockDescript(game.player, 0) + " twitches as you unload into your blue partner", false);
        // [(big skeet)
        ...
        outputText("Eventually both you and she are spent and limp, and you draw your " + cockDescript(game.player, 0) + " out of her, making an extended, wet sucking noise.  As you pull up your gear and make your way up the beach, ", false);
        // [(normal/big skeet)
...
function rapeAnemoneWithPussy(): void {
    ...
    // [(breast0>C)
    if (player.breastRows.biggestTitSize() > 2) outputText("making sure to let your " + allBreastsDescript(player) + " rub up against hers, ", false);
    outputText("then pull apart from her and ", false);
    // [(goddamn centaur)
    if (player.lowerBody == LowerBodyType.CENTAUR) outputText("turn away, kneeling down to display your animalistic, musky pussy readily.", false);
    else outputText("recline back on your " + legs(player) + ". Spreading your thighs, you reach down with two fingers and pull apart your " + vaginaDescript(player, 0) + " welcomingly; it's the last act in your sexual performance.", false);
    outputText("\n\n", false);
    ...
    // [(no centaur)
    if (player.lowerBody != LowerBodyType.CENTAUR) outputText("You take her by the upper arms and pull her on top of you as you lie back in the sun-warmed shallows.  ", false);
    outputText("Her hair drapes over you as she lines her penis up with your " + vaginaDescript(player, 0) + ", delivering heat to your body, but this is dwarfed by the sensation of her entry as she pushes her cock in for the first time.  ", false);
    ...
    outputText("The overwhelming sensations drive you over the edge and your " + vaginaDescript(player, 0) + " contracts hungrily around the heat radiating from the anemone's cock.  As your orgasming pussy ", false);
    // (squirter)
    if (player.vaginas[0].vaginalWetness >= VaginaWetness.DROOLING) outputText("soaks her crotch with juice and ", false);
    ...
    if (player.lowerBody == LowerBodyType.CENTAUR) outputText(" on your back", false);
    // [(notits)
    else if (player.breastRows.biggestTitSize() < 1) outputText(" on your chest", false);
...
// loss rapes:
export function loseToAnemone(): void {
    spriteSelect(4);
    const x: number = player.cocks.cockThatFits(36);
    outputText("", true);
    ...
    if (player.gender == 0) {
        ...
        // [(lactating)
        if (player.breastRows.biggestLactation() > 1) outputText("  For a moment the examination stops at the dribble of milk leaking from your " + nippleDescription(player, 0) + "s.  With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
    ...
    if (player.cocks.length > 0) {
        outputText(images.showImage("anemone-male-fuck"), false);
        if (player.cocks.cockThatFits(36) >= 0) {
            outputText("Shivering, you slump before the anemone and begin trying to remove your " + player.armorName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you, taking the opportunity to run a hand up your " + cockDescript(game.player, x) + ".  ", false);
            ...
            // [(normal)
            if (player.lowerBody != LowerBodyType.CENTAUR) outputText(" into your lap,", false);
            // (shitty taur)
            else outputText(" downwards, onto your hams,", false);
            outputText(" delivering lances of venom into your lower body.  The tingle warms your groin and more pre-cum leaks out of " + sMultiCockDesc(game.player) + ".  Her eyes lock onto a glistening dollop and she drops down quickly, enfolding the head of your " + cockDescript(game.player, x) + " in her cool mouth.  Her tongue dances around the crown of your " + cockNoun(CockTypesEnum.HUMAN) + ", relieving it of the sticky pre.  Looking ", false);
            ...
            if (player.cocks.length > 1) {
                // [(cock2 doesn't exist)
                if (player.cocks.length == 2) outputText("Your other dick rubs", false);
                // (cock2 exists)
                else outputText("The rest of your " + multiCockDescriptLight(game.player) + " rub", false);
            ...
            if (player.vaginas.length > 0) {
                ...
                outputText(" into your " + vaginaDescript(player, 0) + ", sending a tingle through your lower lips and exposing your clitoris.", false);
                // [(if clit > 5")
                if (player.clitLength > 5) outputText("  Having achieved this, she pulls her hand out and wraps another bundle of tentacles around your " + clitDescription(player) + ", then begins jerking it off in time to her efforts on your " + cockNoun(CockTypesEnum.HUMAN) + ".  Your eyes roll back in your head and your mouth gapes involuntarily at the rough stimulation of your swollen chick-stick.", false);
            }
            outputText("\n\n", false);
            ...
            if (player.cocks.length > 1) {
                outputText(" as ", false);
                // [(dick2 = no)
                if (player.cocks.length == 2) outputText("your cocks shoot", false);
                else outputText("the rest of your " + multiCockDescriptLight(game.player) + " shoot", false);
            ...
            // [(multi)
            if (player.cocks.length > 1) {
                if (player.cocks.length == 2) outputText(" and your other " + cockDescript(game.player, 1) + " follows suit", false);
        ...
        // loss, too-big male (cock > 5" width or > 48" length):
            ...
            outputText("Shivering, you slump before the anemone and begin trying to remove your " + player.armorName + ".  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and begins to pull them free of you, but your " + multiCockDescriptLight(game.player) + " flops out and bops her in the nose!  As you fumble the rest of the fastenings and finish removing your gear, the blue girl watches mesmerized at the bobbing flesh in front of her, slick pre-cum leaking from the tip", false);
            // [(big skeet)
            if (player.cumQ() > 500) outputText(" in a steady stream", false);
            outputText(".\n\n", false);

            outputText("Almost reverently, she caresses the shaft of your " + cockDescript(game.player, 0) + ", stroking lightly up its enormous length.  She pulls it down to her eye level, inspecting the head from several angles.  Tentatively, she opens her mouth and pulls your " + cockNoun(CockTypesEnum.HUMAN) + " into it, trying to fit your expansive dickflesh into a hole that even to your lust-crazed mind looks much too small.  Despite her best efforts, she can't get more than the crown past her lips, though the reflexive motions of her tongue poking around and inside the opening make you shiver and push out more pre-cum.  The anemone eventually pops your " + cockDescript(game.player, 0) + " out of her mouth and frowns in frustration.  After a few seconds, she seems to reach a decision.  Moving your shaft out of the way, she walks around behind you.  She places one hand on your ", false);
            if (player.lowerBody != LowerBodyType.CENTAUR) outputText("waist", false);
            else outputText("flank", false);
            // [(not centaur)
            if (player.lowerBody != LowerBodyType.CENTAUR) outputText(" and pushes your shoulders down with the other", false);
            outputText(".  As she draws you backwards, you're forced to put your own ", false);
            if (player.lowerBody != LowerBodyType.CENTAUR) outputText("hands ", false);
            else outputText("forelegs knee-", false);
            outputText("down in front of you to keep from falling face-first.  ", false);
            // [(if non-centaur)
            if (player.lowerBody != LowerBodyType.CENTAUR) outputText("The head of your " + cockDescript(game.player, 0) + " dips into the lukewarm lakewater, sending a tingle down the shaft.  ", false);
            outputText("Behind you, the anemone has taken her blue, tentacled penis into her hand and is stroking it and fondling the tip, forcing her own pre-cum out and smearing it along her length.  Satisfied with its slipperiness, she edges forward until her cock is resting on your " + buttDescription(player) + ".  Drawing her hips back, she lines it up with your " + assholeDescript(player) + ", then thrusts forward while pulling back on your waist.  The wriggly feelers slip past your butthole and light up your insides with her potent venom.", false);
            buttChange(player, monster.cocks.cockArea(0), true);
            outputText("\n\n", false);

            outputText("Taking a moment to transfer her now-free hand to your other hip, the anemone girl then begins to pump her stiff pecker into your " + assholeDescript(player) + ", pausing after every few strokes to gyrate her hips a bit, massaging your prostate with her feelers and smearing venom into it.  The stimulation brings you to your limit in minutes; your dick twitches spastically", false);
            // [(if balls)
            if (player.balls > 0) outputText(" and your " + sackDescript(player) + " tightens up", false);
            outputText(".  This doesn't escape your blue lover's notice, and she quickly stops pumping.  Left at the edge of orgasm, you panic and look over your shoulder at her.  Judging by her grinning demeanour, she seems to be up to something diabolical.  You stare at her confusedly until you feel a new heat at the base of your " + multiCockDescriptLight(game.player) + ". Glancing down, you see that her tentacle-hair has wrapped around " + sMultiCockDesc(game.player) + " and is squeezing tightly!  Pleased with the arrangement, the anemone begins pumping and rubbing your prostate again, spreading new warmth through your " + assholeDescript(player) + ".  Your delayed climax finally arrives, but the <i>de facto</i> cockring", false);
            ...
            outputText("For nearly an hour the anemone continues her performance, even going so far as to caress your swollen " + multiCockDescriptLight(game.player) + " with her unoccupied tentacles.  Several more orgasms arrive and desert you without bringing any relief from the pressure on your ", false);
            // [(if balls)
            if (player.balls > 0) outputText(ballsDescriptLight(player) + " and ", false);
            outputText(multiCockDescriptLight(game.player) + ".  Eventually you get to the point where you can't take it anymore, and when you feel the next orgasm drawing close you straighten up and begin ", false);
            // [(man)
            if (player.lowerBody != LowerBodyType.CENTAUR) outputText("clawing at your tormentor's tentacles, trying to pry them from " + sMultiCockDesc(game.player) + " by main force.", false);
            // (horse)
            ...
            outputText("  Looking a bit irritated that you want to bring her fun to an end, the anemone nevertheless relents and releases her visegrip on your " + multiCockDescriptLight(game.player) + ".  As the joy of seeing the way to your release cleared overtakes you, the anemone avails herself of your distraction to grab your arms and pull you toward her while pushing your " + legs(player) + " out from under you.  The two of you fall backward into the shallow water as " + sMultiCockDesc(game.player) + " begins unloading its immense backup of semen in a high arc.  The ", false);
            // [(skeet amount)
            ...
            if (player.tallness < 48 && player.lowerBody != LowerBodyType.CENTAUR) outputText("fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.", false);
            else if (player.tallness < 84 && player.lowerBody != LowerBodyType.CENTAUR) outputText("catch the air and rain down on both your faces, splashing quietly where they hit water.", false);
            else {
                outputText(" land right on your", false);
                // [(if breasts)
                if (player.breastRows.biggestTitSize() >= 1) outputText(" breasts and", false);
                outputText(" face.  You hear the anemone giggling as you flinch from the white shower.", false);
            }
            outputText("  After several minutes of climax with you shooting more jism than you thought possible and the anemone banging out an accompaniment on your " + assholeDescript(player) + ", you finally begin to wind down.  The anemone, clearly turned on by the impressive amount of ejaculate, unloads her own blue cock into your asshole.  Her semen, lower in temperature than yours, forms a little pocket of cool inside your " + buttDescription(player) + ".  She idly swishes her tentacles in the", false);
            // [(big skeet)
            ...
            if (player.tallness < 48 && player.lowerBody != LowerBodyType.CENTAUR) outputText("She looks you over, then bends down and drinks up as much of the semen floating in the water as she can find nearby.", false);
            ...
            outputText("  Having fed, she grins mischievously and grabs your " + cockDescript(game.player, 0) + ", then tows your floating body to the shoreline with it.  She rolls you onto the sand and then swims for deep water, vanishing.", false);
    ...
    // loss rape, vaginal (only full females):
        ...
        // [(lactation)
        if (player.breastRows.biggestLactation() > 1) outputText("  For a moment it brightens again when she notices the dribble of milk leaking from your " + nippleDescription(player, 0) + "s. With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
...
function minoCumForAnemonieeeeez(): void {
    spriteSelect(4);
    outputText("The initial surprise subsides to wooly-headedness and a feeling of mild arousal as the stingers in her tentacles find exposed flesh.  In panic of drowning you pull free of the ropy mass and backpaddle away from the girl until your " + feet(player) + " reassuringly touch the shallows of the lakebed.  As you shake your head to clear the haze, you notice a few of your items have fallen out of your pouches and are floating in the water.  The anemone has picked one in particular up and is examining it; a bottle of minotaur cum.  Her eyes light up in recognition as the fluid sloshes back and forth and she looks beseechingly at you, cradling it next to her cheek.  \"<i>Gimme?</i>\" she asks, trying to look as sweet as possible.\n\n", false);
...
function anemoneButtPlugginz(): void {
    ...
    if (!player.isTaur()) {
        ...
        else outputText("your " + vaginaDescript(player, 0));
        outputText(" as y");
    }
    else outputText("Y");
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(48) >= 0) {
        ...
        outputText(" until your " + clitDescription(player) + " starts to fill with blood and peeks from its hood");
    }
    outputText(", prompting a giggle from her");
    if (player.cocks.length > 0 && player.cocks.cockThatFits(48) >= 0) outputText(" as you smear a dribble of pre-cum into the crack of her ass");
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(48) >= 0) outputText("cock");
    ...
    outputText("\n\n\"<i>Where's the hole?</i>\" you demand, pointing at your own " + buttDescription(player) + " for an example.");
...
function anemoneQuoteUnquoteAnal(): void {
    clearOutput();
    const dick: boolean = (player.cocks.cockThatFits(48) >= 0 && player.cocks.length > 0);
    let x: number = 0;
    outputText(images.showImage("anemone-bj"), false);
    if (dick) x = player.cocks.cockThatFits(48);
    outputText("You're in the mood for anal and anal you shall have.  ");

    if (!player.isTaur()) {
        outputText("Ever a purist, you stroke your ");
        if (dick) outputText(cockDescript(game.player, x));
    ...
    outputText("; her hair delivers tingles of venom where it brushes your skin even as her own hands dawdle playfully along your " + skin(player) + ".  Without another word, you force her head into your groin as you ram your distended ");
    ...
    if (dick) {
        ...
        if (player.cocks.length == 0) outputText("; she even tries to push them up herself with her hands, as if that would make your clit produce the semen inside them");
        outputText(".");
        if (player.cocks.length > 0) outputText("  Your clit remains unproductive; your male orgasm squirts out uselessly around her head, mingling with the lake water.  The girl's expression upon seeing this is pained, and she tries but fails to get free to follow the arcing white ropes, sending further spasms of pleasure through your sensitive distaff staff.");
        ...
        if (player.cocks.length > 0) outputText(", pointing to the squiggles of your seed floating lazily in the lakewater,");
...
function hotdogTheAnemone(): void {
    ...
    if (dick) {
        outputText("your swollen, painfully hard prick fountains with cum, coating the anemone's back and hair in white.  The venom in your system draws out your orgasm to incredible lengths, and " + sMultiCockDesc(game.player) + " ejaculates over and over until you feel lightheaded and woozy.");
...
function anemoneGetsLayedByBeePositor(): void {
    ...
    if (player.antennae > AntennaeType.NONE) outputText(" antennae");
    ...
    if (player.sens >= 50 || player.isTaur()) {
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cocks.length == 1) outputText("male part erupts");
...
export function anemoneKidBirthPtII(): void {
    clearOutput();
    spriteSelect(71);
    outputText("You awake and look skyward to the sun for a hint at the time.  What greets you is more of an eclipse; a shape impedes your view.  As your eyes adjust to the light, it resolves into an upside-down blue-eyed, blue-skinned face wreathed with snubby, shoulder-length tentacles of purple and green hue.  The silence continues as you stare into it, until you move to push yourself off the hard ground.  At the first sign of activity, the head disappears into the water barrel with a sloshing sound.  You push yourself to your " + feet(player) + " and look back toward it; the eyes, now right side-up, peek over the rim bashfully atop a set of blue fingers.");
...
export function anemoneBarrelDescription(): void {
    if (game.time.hours < 6) // (morning)
...
export function approachAnemoneBarrel(): void {
    let item: () => void = null;
    let weaponT: string = "Give Weapon";
    let weaponB: () => void = giveAnemoneWeapon;
    clearOutput();
    spriteSelect(71);
...
function placeInAnemone(slot: number): void {
    clearOutput();
...
function takeOutOfAnemone(): void {
    ...
    const itype: ItemType = ItemType.lookupItem(flags[kFLAGS.ANEMONE_WEAPON_ID]);
...
function tutorAnemoneKid(): void {
    ...
    if (flags[kFLAGS.HAD_KID_A_DREAM] > 0 && kidAXP() >= 40 && player.lust > 99) {
        ...
        // if KidXP >= 40 and lust > 99 after tutor and PC has only huge dicks of area >= 60 or hasn't got shit
            ...
            if (player.cocks.length > 0) outputText("something too enormous");
...
function kidASex(cont: boolean = true): boolean {
    ...
    // if KidXP >= 40 and lust > 99 after tutor and PC has wang of area <60, set that cock to x variable, also set biggest cock not yet used to y variable if multicocked
    else if (player.cocks.length > 0 && player.cocks.cockThatFits(60) >= 0) {
        x = player.cocks.cockThatFits(60);
        y = -1;
        temp = 0;
        ...
        if (player.balls > 0 || y >= 0) {
            ...
            if (y >= 0) outputText(cockDescript(game.player, y));
            else outputText(sackDescript(player));
    ...
    // if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox
        ...
        outputText("\n\nLaughable as her reaction is, the venom now coursing through your " + vaginaDescript(player, 0) + " ensures not a giggle escapes you; your hips begin writhing in her lap, trying to find purchase on the blue girl to better pump her shaft for its seed.");
        // [(if pc is loose)
        if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.GAPING) outputText("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
        outputText("  Emboldened, she picks up your legs haltingly, then begins to work herself in and out of your depths.");

        outputText("\n\nWith all the grace of a first-timer, the girl clumsily leans down to kiss you, but falls short and can only plant a smooch on your still-clad [chest].  Still, she continues pumping enthusiastically, worry and shame evaporating from her brow as you moan lustily instead of rebuking her temerity.  Pausing to support you with one hand as she spreads your lips wider with her fingers, she exposes your " + clitDescription(player) + " to the air.");
        // [(clitsize<6)]
...
export function kidABabysitsCows(): void {
...
export function repeatCowSitting(): void {
...
export function kidAWatchesSharks(): void {
    clearOutput();
...
export function goblinNightAnemone(): void {
    outputText("\n<b>That night...</b>");
    ...
    outputText(", " + mf(player, "studly", "gorgeous") + " " + race(player) + "; told us that " + mf(player, "he", "she") + "'d be able to knock us all up like " + mf(player, "he", "she") + " did her, and a few of the little goblin tramps outside the family have seen one in this camp.  We're going to get our babies.</i>\"  Kid A remains silent, but shakes her head uncertainly, holding her equipment closer to her chest.");
...
/*
 ...
 "You feel a pressure in your lower esophageal region and pull your garments down to check the area.  <b>Before your eyes a pair of feathery gills start to push out of the center of your chest, just below your neckline, parting sideways and draping over your " + nippleDescription(player, 0) + "s.</b>  They feel a bit uncomfortable in the open air at first, but soon a thin film of mucus covers them and you hardly notice anything at all.  You redress carefully."
