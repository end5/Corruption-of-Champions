...
function rapeAnemoneWithDick(): void {
    ...
    if (player.cocks.cockThatFits(36) >= 0) {
        ...
        // [(if PC boobs > A)
        if (player.breasts.biggestTitSize() >= 1) outputText(" in time with your own", false);
...
function rapeAnemoneWithPussy(): void {
    ...
    // [(breastrow0>C-cup)
    if (player.breasts.biggestTitSize() > 2) outputText("  You give special attention to the presentation of your " + breastDescript(game.player, 0) + ", removing your top with tantalizing slowness, letting each breast slip out and hang between you like fruit ripe to be taken, then making sure to rub them seductively to arouse the both of you further.", false);
    ...
    // [(breast0>C)
    if (player.breasts.biggestTitSize() > 2) outputText("making sure to let your " + allBreastsDescript(player) + " rub up against hers, ", false);
    ...
    if (player.lowerBody != LowerBodyType.CENTAUR) {
        ...
        // [(breasts>manly)
        if (player.breasts.biggestTitSize() >= 1) outputText(breastDescript(game.player, 0) + " and ", false);
    ...
    // [(notits)
    else if (player.breasts.biggestTitSize() < 1) outputText(" on your chest", false);
...
export function loseToAnemone(): void {
    ...
    if (player.gender == 0) {
        ...
        // [(lactating)
        if (player.breasts.biggestLactation() > 1) outputText("  For a moment the examination stops at the dribble of milk leaking from your " + nippleDescription(player, 0) + "s.  With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.cockThatFits(36) >= 0) {
            ...
            outputText(" delivering lances of venom into your lower body.  The tingle warms your groin and more pre-cum leaks out of " + sMultiCockDesc(game.player) + ".  Her eyes lock onto a glistening dollop and she drops down quickly, enfolding the head of your " + cockDescript(game.player, x) + " in her cool mouth.  Her tongue dances around the crown of your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ", relieving it of the sticky pre.  Looking ", false);
            ...
            if (player.vaginas.length > 0) {
                ...
                // [(if clit > 5")
                if (player.clitLength > 5) outputText("  Having achieved this, she pulls her hand out and wraps another bundle of tentacles around your " + clitDescription(player) + ", then begins jerking it off in time to her efforts on your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ".  Your eyes roll back in your head and your mouth gapes involuntarily at the rough stimulation of your swollen chick-stick.", false);
            }
            outputText("\n\n", false);
        ...
        // loss, too-big male (cock > 5" width or > 48" length):
            ...
            outputText("Almost reverently, she caresses the shaft of your " + cockDescript(game.player, 0) + ", stroking lightly up its enormous length.  She pulls it down to her eye level, inspecting the head from several angles.  Tentatively, she opens her mouth and pulls your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + " into it, trying to fit your expansive dickflesh into a hole that even to your lust-crazed mind looks much too small.  Despite her best efforts, she can't get more than the crown past her lips, though the reflexive motions of her tongue poking around and inside the opening make you shiver and push out more pre-cum.  The anemone eventually pops your " + cockDescript(game.player, 0) + " out of her mouth and frowns in frustration.  After a few seconds, she seems to reach a decision.  Moving your shaft out of the way, she walks around behind you.  She places one hand on your ", false);
            ...
            if (player.tallness < 48 && player.lowerBody != LowerBodyType.CENTAUR) outputText("fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.", false);
                ...
                // [(if breasts)
                if (player.breasts.biggestTitSize() >= 1) outputText(" breasts and", false);
    ...
    // loss rape, vaginal (only full females):
        ...
        // [(lactation)
        if (player.breasts.biggestLactation() > 1) outputText("  For a moment it brightens again when she notices the dribble of milk leaking from your " + nippleDescription(player, 0) + "s. With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
