...
export class Vapula implements TimeAwareInterface {
    public constructor() {
    ...
    public timeChangeLarge(): boolean {
        if (flags[kFLAGS.VAPULA_FOLLOWER] >= 2.5 && game.time.hours == 6 && flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0) {
            femaleVapulaRecruitmentPartII();
            return true;
        }
        if (game.time.hours == 2 && vapulaSlave() && flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0 && flags[kFLAGS.VAPULA_DAYS_SINCE_FED] >= 5 && (player.cocks.length > 0 || (player.keyItems.has("Demonic Strap-On") >= 0 && player.vaginas.length > 0))) {
            vapulaForceFeeds();
            return true;
        }
        return false;
...
export function vapulaSlaveFlavorText(): void {
    ...
    if (choice == 6) {
        if (flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || flags[kFLAGS.IZMA_NO_COCK] > 0 || !IzmaScene.izmaFollower()) choice = 7;
    ...
    if (choice == 9) {
        if (player.effects.findByType(StatusAffects.CampRathazul) < 0) choice = 10;
...
export function callSlaveVapula(output: boolean = true): void {
    ...
    if (player.vaginas.length > 0 && player.keyItems.has("Demonic Strap-On") >= 0) {
        if (player.lust >= 33) fFeed = chixFeedVapulaBlehblehIVantToZuckYourSpooo;
        else if (output && player.cocks.length === 0) outputText("\n\nYou aren't suitably aroused to feed Vapula right now.");
    }
...
function talkToVapulaForSomeReason(): void {
    ...
    let threesomeT: string = "";
    let threesomeB: () => void = null;
    let haremT: string = "";
    const haremB: () => void = toggleVapulaHaremFucks;
...
function feedVapulaACupOfJizz(): void {
    ...
    outputText(".  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(game.player, 0) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your " + cockDescript(game.player, 0) + " appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
    player.orgasm();
    dynStats("cor", .5);
...
function vapulaThreesomeMenu(): void {
    ...
    let amily: () => void = null;
    // Option: Ceraph
    let ceraph: () => void = null;
    // Option: Sophie
    let sophie: () => void = null;
    // Option: Jojo
    let jojo: () => void = null;
    // Option: Izma
    let izma: () => void = null;
    if (player.cocks.length > 0 || (player.vaginas.length > 0 && player.keyItems.has("Demonic Strap-On") >= 0)) {
        if (JojoScene.campCorruptJojo()) jojo = vapulaJojoThreesomes;
        if (AmilyScene.amilyFollower() && AmilyScene.amilyCorrupt() && player.cocks.length > 0) amily = vapulaAndAmilyThreesome;
        if (CeraphFollowerScene.ceraphIsFollower() && player.cocks.length > 0) ceraph = vapulaCeraphThreesome;
        if (SophieBimbo.bimboSophie()) sophie = vapulaSophieThreesomeSelect;
        if (IzmaScene.izmaFollower() && flags[kFLAGS.IZMA_NO_COCK] == 0) izma = vapulaAndIzmaThreeSome;
    }
...
function vapulaCeraphThreesomePleaseBoth(): void {
    ...
    outputText("\n\nYou push Vapula onto the floor, making her fall on her back, legs spread.  You then grab hold of Ceraph's shoulders and put her on all fours, her warm snatch facing Vapula's.  Already moist, the succubus and the omnibus naturally start caressing each other, kissing and groping each other's breasts.  You watch them giving in to their own depravity before joining the show; grabbing Ceraph's plump butt, you press the tip of your " + cockDescript(game.player, x) + " against her backdoor.  The unexpected intrusion makes the omnibus cry, her gasps of joy only enticing you to push further, even at the cost of stretching her anal ring.  A few seconds later you're all the way in, enjoying the boiling and tight recesses of her pucker.  You roughly seize Ceraph's fuck-pillows and proceed to relentlessly assault her ass, making her yell under the conjugated efforts of your throbbing junk and the hot succubus fingering her from below.  Completely dominated by the pleasure you're inflicting on her, your slut starts muttering dirty words, often interrupted by sudden screams, \"<i>Fuck... fuck... fuck my ass... " + mf(player, "Master", "Mistress") + "...feels so- AAAAAH! Oh FUCK YEAH! Harder, " + mf(player, "Master", "Mistress") + "! Fuck your bitch!</i>\"");

    outputText("\n\nAnnoyed by the noise she's making, you slap her firm rump repeatedly, your hands imprinting marks on her red ass-cheeks.  Your buttslut doesn't seem to care, completely blissful from being used by her rightful " + mf(player, "Master", "Mistress") + ".  She groans, grinds harder and squirms in Vapula's embrace in her efforts to impale herself further on your " + cockDescript(game.player, x) + ".  The sight of your sluts' love-tunnels mixing their fluids together as they squirt as well as the insane motion of Ceraph's wobbling ass sending the sweetest sensations to your rod prove too much for you; despite your best efforts to hold back, you blast Ceraph's ass with your baby-batter, the globs effectively packing her anal entrance");
    if (player.cumQ() >= 1500) outputText(" and swelling her belly until she looks ready to give birth to a full horde of imps");
    outputText(".");
    outputText("\n\nYou feel your ");
    if (player.balls > 0) outputText(ballsDescriptLight(player) + " twitch and your " + cockDescript(game.player, x) + " pulsate as they unload their");
    else outputText("your " + cockDescript(game.player, x) + " twitch and pulsate as it unloads its");
    outputText(" essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, \"<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>\"  Her voice goes so high pitched it trails off, letting you hear Vapula's soft moans below.  You quickly pull out your " + cockDescript(game.player, x) + ", still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, \"<i>What are you doing? Put it back! Put it back already!</i>\"");
    outputText("\n\nWith another slap you tell her to shut up and learn to share; her partner hasn't received any cock yet, and you intend to remedy it.  Grabbing Vapula's thighs this time, you lunge toward her wet fuck-hole and bury your " + cockDescript(game.player, x) + " to the hilt, ");
    if (player.balls > 0) outputText(ballsDescriptLight(player) + " hitting her butt-hole with a faint thud");
    ...
    if (player.cocks.length > 4) outputText("  Sadly, your extra junk does not have any hole left to fill.  It just hangs there, rubbing against Ceraph's backside and teasing her all the more.");
    ...
    // [if multicocks]
    if (player.cocks.length > 1) outputText("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
    ...
    outputText("  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your " + cockDescript(game.player, x) + " down your sluts' throats, pumping their heads alternatively around your shaft");
    if (player.cocks.length > 1) outputText("s");
    if (player.balls > 0) outputText(" as the girls keep fondling your " + ballsDescriptLight(player));
    ...
    outputText("\n\nSatisfied, you kiss both of your sluts.  \"<i>Thank you " + mf(player, "Master", "Mistress") + ".</i>\" they say in unison, before you pat their heads and send them on their way; they will probably need to rest in order to digest their heavy meal.");
    // lust set to 0
    player.orgasm();
    dynStats("cor", 1);
...
function vapulaAndCeraphButtfuckTrainYeehaw(): void {
    ...
    outputText("\n\nWithout any warning, you harshly push your " + cockDescript(game.player, x) + " into Ceraph's tight anal recesses, making her clench and tense in arousal.  She immediately responds by grasping Vapula's squeezable thighs and digs her own, broad prick inside the succubus' colon.  The totally unexpected rectal assault makes both hell-girls scream in surprise and pleasure mixed, their anal walls saturated with intense dick-friction.  You stand here comfortably, easily lodging your " + cockDescript(game.player, x) + " inside Ceraph's backdoor while letting her have a better grip on Vapula's buttocks.  The omnibus' pucker is insanely hot!  Your rod of flesh feels like it's going to melt under her demonic warmth, and her astonishingly narrow colon squeezes it in the most harsh and delightful way.  You decide to speed up the pace and start sliding back and forth, ");
    ...
    outputText(".\n\n");
    outputText("Between two mad thrusts, you order Ceraph to imitate you in every movement, treating Vapula the same way you treat her.  You start slapping her jiggly ass, sending ripples across her red shivering flesh.  Entranced by your violent ministrations, the omnibus complies to your orders, spanking Vapula as hard as she can.  Soon moans are heard all across the butt-fuck train; the more you slap and thrust, the harder Ceraph hits the purple sex-goddess, as if she wanted to prove she was a better servant, worthier of your " + cockDescript(game.player, x) + ".  You reach around and knead Ceraph's tits, playing with her nipples as you devastate her backdoor with ferocious dick assaults, and she hesitantly grasps Vapula's bouncy tits, unable to match your thrusts in intensity.  She jerks her head backwards and moans, almost forgetting her own prick impaling the poor succubus in front of her.  Seeing that the poor bitch has lost all control over her own body, you stretch your arms further and reach Vapula's breasts.  The succubus is literally drooling in ecstasy; forced to feel the mighty thrusts of a strong omnibus as well as yours, she is bent over submissively, her tongue hanging and her eyes rolling under the combined strength of your anal-dickings.  You pound harder, squeezing her tits at each cock-nudge and muffling her pleasure-filled shrieks when you can.");

    outputText("\n\nAs for Ceraph, the poor thing isn't even able to think anymore.  Her rhythmical moans have been gradually replaced with some incoherent dirty babble, \"<i>Oh fuckfuckfuck... good.. fuck...  YES! Fuck... yes... fuckfuckfu-aaaAAAAAH!</i>\"  You keep impaling the omnibus' butthole with your " + cockDescript(game.player, x) + ", pounding harder in hope the anal tearing will shut her up, but she only seems to get even more pleasure from such a violation of her insides.  Her mind isn't able to reason properly, torn between her ferocious butt-penetration and the pleasure her own cock is getting as it moves of its own volition in and out of Vapula's tight little pucker.");

    outputText("\n\nYou pump more and more, growing more impatient and vicious as you feel your climax approaching.  You slap Ceraph's face, you toy with her nipples, you pinch Vapula's butt, you block Ceraph's thrusts midway, making both demonesses whimper in frustrated desire.  All the while, you keep butt-fucking Ceraph at a crazy pace, yelling like a " + mf(player, "man", "herm") + " possessed as you claim her impossibly tight confines with unequaled intensity.  You ride the butt-fuck train, submitting both girls to your will as their anal entrances keep being assaulted over and over again.  Their warm love-tunnels are literally steaming in arousal and their feminine juices are being spilled in a free flow of lust.");
    // [if balls]
    if (player.balls > 0) outputText("  As you relentlessly smash your groin against Ceraph's plump rump, your " + ballsDescriptLight(player) + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
    outputText("  The entirety of your crotch ends up drenched with a various mix of sexual juices, and your " + cockDescript(game.player, x) + " makes squelching noises at each thrust in and out of your omnibus' little ass.");

    outputText("\n\nEventually, you feel your stamina decreasing as a pressure builds in your loins and you brace yourself for the imminent cumshot. With a savage, desperate last thrust, you thoroughly impale the purple demoness on your junk and paint her interior walls with an explosion of spooge.  A mighty spray of cum spurt out of your " + cockDescript(game.player, x) + ", completely stuffing Ceraph with your seed.");
...
function vapulaSophieThreeSome(): void {
    ...
    outputText("\n\nWithout ceremony you grab the avian slut's fuckpillows and eagerly sandwich your meat between them, pressing the tip of your " + cockDescript(game.player, x) + " against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it ");
    if (player.cocks.cockArea(x) > 75) outputText("you've filled the entirety of her colon");
    else outputText("your cock is already buried to the hilt, its mammoth girth bulging between her broad hips");
    outputText("; the bulge even grinds a bit against Vapula's own butt and the succubus reaches toward you, entranced with lust.  She wraps her arms around you, letting you feel her tender boobs being squeezed against your [chest] as she gives you a mad kiss filled with passion.  Your lips remain locked for a while as you explore each other's mouths with your tongues; you almost forget the burning hot and roughly tight hole.  It entraps your " + cockDescript(game.player, x) + " as it continues massaging with the harpy's vice-like anal muscles.  The bimbo keeps wriggling arond your rod in order to get more of it inside, pleasantly smashing her fat rear against your groin.");
    outputText("\n\nStill not breaking the kiss, you groan silently and pull out your " + cockDescript(game.player, x) + ", all smeared in pre-cum.  Sophie whimpers and wails softly until you thrust all the way back in, literally jamming her with your junk.  Her pupils widen, her mouth opens and her gigantic tits keep swinging in the dirt as her whole body trembles and shudders from the brutal anal violation.  Her cry of pleasure is so high-pitched you barely hear her subsequent moan; satisfied, you start thrusting at a more convenient rhythm, enjoying the lush depths of the avian slut as the succubus keeps making out with you.  You keep butt-fucking Sophie, unable to focus on the ministrations of the expert succubus or the extremely warm recesses of the harpy's interior.  You regularly slap her plush, fat buttocks, sending ripples across her whole fleshy body. Needless to say, the bimbo's pussy is completely drenched in fem-spunk, her wide pussy drooling trickles after trickles of her juices, betraying her utter arousal.");
    // [if 2nd cock]
    ...
    outputText(buttDescription(player) + ".");
    // [if third cock]
    if (player.cocks.length > 2) outputText("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
    ...
    if (player.cocks.length < 3) outputText("hastily pull out and ");
    outputText("wave ");
    if (player.cocks.length >= 3) outputText("one of ");
    outputText("your still pulsing dick");
    if (player.cocks.length >= 3) outputText("s");
...
function vapulaSophieThreesomeCunts(): void {
    ...
    outputText("\n\nWithout ceremony you grab the avian slut's fuckpillows and eagerly sandwich your dildo between them, pressing the tip of it against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it your strap-on is already buried to its leather hilt, its mammoth erection bulging between her broad hips; the bulge even grinds a bit against Vapula's own butt and the succubus reaches toward you, entranced with lust. She wraps her arms around you, her tender boobs against your own " + allBreastsDescript(player) + " as she gives you a mad kiss filled with passion. Your lips remain locked for a while as you explore each other's mouths with your tongue; you almost forget the tight hole wrapped around your dildo.  You are brought back to it by the bimbo wriggling around the rod rammed in her rear in an attempt to squeeze more of it into her, twisting your end around in your sopping " + vaginaDescript(player, 0) + " as she pleasantly mashes her fat rear against your groin.");
    ...
    outputText("\n\nVapula shifts her attention to your breasts, her long tongue running lasciviously over your " + nippleDescription(player, 0) + "s, tightly circling and gently teasing their tips until they are as sensitive and erect as any clitoris.  Her soft lips edge you further and further into your fuck daze, riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula's voracious lips.");
...
function vapulaAndAmilyThreesome(): void {
    ...
    outputText("\n\nThe little mousy looks down awkwardly, intimidated by the towering succubus.  \"<i>Y-yes " + mf(player, "Master", "Mistress") + "...</i>\"");
    ...
    if (player.balls > 0) {
        ...
        outputText("  Once your orgasm starts fading, you drop Vapula and tell her to imitate Amily; the dilligent succubus gets on her knees and quickly licks your " + cockDescript(game.player, x) + ", feeding her cum-thirst as she cleans your junk of its spooge.  Your two sluts work quickly, easily sucking up every bit of goo until your " + cockDescript(game.player, x) + " eventually shrinks to a half-erect state, your lust consumed.");
    }
...
function vapulaJojoThreesomes(): void {
    ...
    let tease: () => void = null;
    let train: () => void = null;
    if (player.vaginas.length > 0 || player.keyItems.has("Demonic Strap-On") >= 0) tease = NTRSomeJojos;
    if (player.cocks.length === 0) { } else {
        tease = NTRSomeJojos;
...
function NTRSomeJojos(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        outputText("\n\n\"<i>These hips... these thighs...</i>\"  Grabbing hold of Vapula's waist, you shove your " + cockDescript(game.player, x) + " between her legs and start grinding your junk against her vaginal entrance, teasing her in the most lustful manner. You keep grinding until your purple slut utters a lubricious moan.");
        outputText("\n\n\"<i>No, really, her whole body is perfect.</i>\"  You run your hands all over the succubus curves, pinching here and caressing here.  \"<i>And gods, that tight anus!  Oh, Jojo, if you knew how cock-hungry she is...</i>\"  With a wicked, wanton smile, you rail Vapula's ass with your " + cockDescript(game.player, x) + ", filling the entirety of her interior with your dickflesh as you watch the monk.  The poor mousy slut has completely given in to his lust; he has fallen on his knees and is now stroking himself feverishly, unable to break your gaze.  You start pounding Vapula's butt; with each thrust you grunt ostensibly, letting your mouse pet know how much you enjoy it.  As for Vapula, the slutty demoness is clearly enjoying her anal treatment, the sight of a poor bitch masturbating to her being all the more arousing to her.  She also stares at him with longing eyes, as if to provoke him with her nude, horny body being fucked by someone else.  Soon she is screaming in utter ecstasy and starts babbling meaningless dirty talk, \"<i>Fuck, fuck me... oh yessssssssssssss... more!  MORE!</i>\"  She's clearly acting as a naughty bitch!  You wonder if the presence of a shy creature is helping to release her inner wantonness... Shrugging it off, you decide to focus on the nice purple body impaling itself on your " + cockDescript(game.player, x) + " and pound her harder, your meat releasing streams of sex juices as it pounds in and out of Vapula's pucker.");
        if (player.cocks.length > 1) {
            ...
            // [if third cock]
            if (player.cocks.length > 2) outputText("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
            outputText("  ");
        }
        else outputText("\n\n");
...
function jojoButtFuckTrain(): void {
    ...
    outputText("\n\nYou seize your " + cockDescript(game.player, x) + " and press its tip against the mouse's sphincter, slowly pushing.  Startled by the sudden anal invasion, Jojo reflexively grabs Vapula's hips and inserts his rodent prick into her.  The simultaneous penetrations make your sluts squeak and moan in ecstasy, their bodies entranced as their anal walls provide them one thousand subtle sensations.  You grip his arms, using him as leverage to pull out, and slam it back home again.  Used like the fucktoy he truly is, Jojo doesn't resist; his dick moves of its own volition in and out of Vapula's interior as you relentlessly push him back and forth.  You secretly rejoice: his tight little ass is clearly adapting to your " + cockDescript(game.player, x) + " and it somehow feels even better than usual.  You pound hard, making him groan and beg for more; you eagerly give him what he wants and stuff him full of your dickflesh.");
    if (player.cocks.cockArea(x) > 100) outputText("  You can actually feel his internal organs being bumped against your meat as your " + cockDescript(game.player, x) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
    ...
    if (JojoScene.tentacleJojo()) {
        ...
        // [if vagina]
        if (player.vaginas.length > 0) outputText("  Last but not least, a soft cock-knock at the entrance of your " + vaginaDescript(player) + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
    }
    ...
    if (n1 >= 0) outputText("\n\nA deliciously depraved idea crosses your mind; using your dick muscles, you move your " + player, n1) + " toward Vapula.  It squirms for a moment as you look for her wet snatch, poking her everywhere with your tip and teasing her with your accidental ministrations, but at last you find her vaginal entrance.  Using the rubbery flexibility of your plant-like prick, you brutally shove it down her love-tunnel, trying to force inside as many inches as you can.  You can feel her instinctively altering her own body in order to accommodate more dickflesh inside her, and you keep pushing until your " + cockDescript(player, n1) + " is stretched to maximum capacity.  Somehow your tentacle rod is entirely buried in Vapula's warm depths, and her unbelievable tightness is driving you mad with lust.  It twists on its on volition inside her, its vegetable-like texture being more sensitive and receptive to pleasure than a normal prick and the sensations it sends to you are abnormally exquisite.  You almost forgot the asshole you were pounding with your " + cockDescript(game.player, x) + " but the way Jojo wriggles against it in order to fit more of your length inside him reminds you of your job.";)
    // [if PC has another tentacle cock, variable is n2]
    if (n2 >= 0) outputText("\n\nSince your sluts still have free holes left, you decide to fill some more fuck-holes with your junk.  Guiding  your " + cockDescript(player, n2) + " toward Vapula's mouth, you think you could use a proper blowjob from a trained succubus and wait for her to let out another moan before filling your junk completely and thrusting up and down brutally, literally mouth-fucking her and treating her lips as a tight cock-ring.  Her expert tongue wraps around your girth, squeezing it and clinging to it like it's the most precious thing in her life.  She keeps jerking back and forth due to the hard butt-fucking she's receiving, and the jolts are transmitted to your " + cockDescript(player, n2) + " in the most teasing manner.");
    // [if PC has another tentacle cock, variable is n3]
    if (n3 >= 0) outputText("\n\nSince Vapula is already sucking you off, why not Jojo?  You still have appropriate junk left after all.  You promptly send your wriggling " + cockDescript(player, n3) + " and push it between the mousey's lips.  However, the slut won't open them!  You pound him ragingly, slamming the entirety of your " + cockDescript(game.player, x) + " down his colon and bruising his firm butt until he lets out a deep howl of pleasure and pain mixed; you profit from the opening and stuff his mouth so full of your " + cockDescript(player, n3) + " you think he's going to choke.  However, his mouth seems to be accustomed to take monstrous lengths - probably a side-effect of his corruption - and he quickly accommodates to your size.  Although he can't provide a blowjob as sweet as Vapula, you can definitely tell he's skilled at sucking dick.  The cute little slut!  You ram his asshole faster to reward him; this increased butt-fucking pace seems to have the desired effect, and soon your " + cockDescript(player, n3) + " is throbbing in need as is bathes in the mousey's saliva.");
...
function vapulaAndIzma(girls: boolean = true): void {
    ...
    // Vapula/Izma threesome (as female)
        ...
        outputText("\n\nNodding in approval, you pull the dildo out of your tigershark's ass with a wet pop and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass and without warning stuff your still-dripping strap-on in her ass; unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn't reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma's cock, never letting her get completely soft.  You pump your dildo in and out of Vapula's plump rump, enjoying the pink end stroking your dripping " + vaginaDescript(player, 0) + " while she still sucks off your tigershark lover, letting her balls fill back up with cum.");
...
function teaseVapula(): void {
    ...
    // Requires: Vapula not fucking harem, teased the day before
        ...
        outputText("You smile as once again Vapula gets into position on her knees, and you buckle on the strap-on and stand over her.  This time you make the pink end grow long, closing your eyes as it inches up your tunnel, before slowly thrusting your " + hipDescription(player) + " backwards and forwards in front of your succubus, letting it shrink and grow so it climbs up and down your wet walls.");
        ...
        // [Hooves:
        if (player.lowerBody == LowerBodyType.HOOFED || player.isTaur()) outputText("\"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
        // [Naga:
...
export function vapulaForceFeeds(): void {
    ...
    // [if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
        ...
        outputText("\n\nShe works on it for what seems like hours. You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need. The naughty bitch!  She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your " + cockDescript(game.player, x) + ", clinging to it like an animal.");
        outputText("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, effectively burying all of your " + cockDescript(game.player, x) + " in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.");
        if (player.balls > 0) outputText("  Your " + ballsDescriptLight(player) + " churn");
        else outputText("  Your " + cockDescript(game.player, x) + " twitches");
        ...
        if (player.cumQ() >= 1500) outputText(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(game.player, x) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
        player.orgasm();
        dynStats("cor", 1);
        flags[kFLAGS.VAPULA_DAYS_SINCE_FED] = 0;
        flags[kFLAGS.VAPULA_TEASE_COUNT] = 0;
...
function spankVapulaLikeABoss(): void {
    ...
    else if (player.vaginas.length > 0) outputText(" and a teltale moistness to your nethers");
    outputText(".");
    ...
    if (player.perks.findByType(PerkLib.Sadist) >= 0) outputText("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
