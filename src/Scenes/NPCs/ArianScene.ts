...
function bargeInOnArian(): void {
    ...
    // (else) //if PC is a herm
        ...
        outputText("\n\nArian hides his face once more inside his covers and says in a whisper, \"<i>Yes....</i>\"");
        outputText("\n\nYou pull the covers off his face and say, \"<i>Well... we'll have to fix that then.</i>\"  You slip off his bed and begin stripping off your [armor].  Arian shyly does the same, stripping off his robes until he is laying in his bed, completely naked.");
        outputText("\n\nOnce you toss your [armor] on the floor, however, Arian's eyes widen as he realizes you're not entirely female; he eyes your " + multiCockDescriptLight(game.player) + " and the moistening pussy between your legs with equal parts wonder and arousal.");
...
function hermsLikeMaleArian(): void {
    ...
    menu();
    if (player.cocks.length > 0 && player.cocks.cockThatFits(50) >= 0) addButton(0, "Penetrate", giveArianAnal);
...
function youDontMindBeingGayForArian(): void {
    ...
    menu();
    if (player.cocks.cockThatFits(50) == -1) {
...
function arianStoryDialogue1(): void {
    ...
    // PC has met Marae:
    if (player.effects.findByType(StatusAffects.MetMarae) >= 0) outputText("Marae herself told you they showed up about, what, 20-30 years ago?");
...
function arianSexMenu(output: boolean = true): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.cockThatFits(50) >= 0) addButton(0, "Anal - Pitch", giveArianAnal);
        // Get Blown:
        // PC must have a cock.
        ...
        // PC must have a cock that fits (cock area 50 or less)
        if (player.cocks.cockThatFits(flags[kFLAGS.ARIAN_CAPACITY]) >= 0 && flags[kFLAGS.ARIAN_VAGINA] > 0)
        ...
        if (player.cocks.cockThatFits(50) >= 0 && player.cocks.cockThatFits2(50) >= 0 && flags[kFLAGS.ARIAN_VAGINA] > 0) addButton(8, "Double Pen", doublePenetrateArian);
        // Docking
        // ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.)
        // PC cock area must be <= 30.
        if (player.cocks.smallestCockArea() <= 30 && flags[kFLAGS.ARIAN_COCK_SIZE] < 3 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0) addButton(7, "Docking", arianDocking);
    }
    // Get Anal:
...
function giveArianAnal(): void {
    ...
    if (x == -1) {
        x = player.cocks.smallestCockIndex();
    }
    ...
    // Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
        ...
        outputText("\n\nArian's ass tightens around your " + cockDescript(game.player, x) + " as [Arian eir] ");
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
        ...
        outputText(".  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your " + cockDescript(game.player, x) + ", pushing you ever closer to the climax.");
        ...
        // (Medium Cum Amount)
        else if (player.cumQ() < 1000) outputText("\n\nArian's butt feels so good that you can't help but paint it in white.  Gob after gob of searing hot cum flows from your " + cockHead(player, x) + " and into the lizan's willing bowels.  You can't help but continue thrusting inside [Arian eir] deliciously slick insides, even as Arian's distended sphincter tries its best to hold you in place.  By the time you're done, Arian looks like [Arian ey]'s three months pregnant; after a few final thrusts, you're confident you've given [Arian em] all that you can muster for the moment and pull out of the lizan's hungry ass.");
        // (High Cum Amount)
        else {
            outputText("\n\nYou bury yourself as deep as you can into the lizan's behind and brace yourself, holding onto [Arian eir] hips as the first of many jets of cum finally escapes your throbbing " + cockDescript(game.player, x) + ".  You can feel the groaning lizan shudder with each blast that you pump into [Arian eir] inviting interior; each of your sticky ropes of cum filling [Arian em] up until [Arian eir] belly looks as big as a beach ball.  Arian's contracted sphincter tries its best to hold your prodigious load in, but it can't hope to contain it all; soon white jism explodes from around the seal of your cock.  Trails of your pleasure run down the lizan's legs to soak the bedsheets along with the lizan's own fluids.  One final jet pushes Arian off your shaft, and you gaze at the messy results of your recent activities.");
        }
    ...
    // Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
        ...
        if (flags[kFLAGS.ARIAN_VAGINA] > 0) outputText("her pussy clenches, spilling a flood of juices; ");
        ...
        outputText("\n\nYou almost blink in surprise; you were just throwing the term buttslut around as a joke, you didn't think Arian was really like that.  Still, there are far more important matters - like digging your way into the depths of Arian's greedy little ass with your " + cockDescript(game.player, x) + "!  You squeeze the lizan's scaly butt and begin to rut [Arian em] like an animal, thrusting your way in and out of [Arian eir] back passage with all the eagerness you can muster.");
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            ...
            if (flags[kFLAGS.ARIAN_VAGINA] > 0) outputText(", and her");
        }
...
function getButtWreckedByArian(): void {
    ...
    if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) {
        ...
        if (player.tailType == TailType.NONE) outputText("back");
    ...
    if (flags[kFLAGS.ARIAN_VAGINA] > 0) outputText(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
    else outputText(".");
...
function suckAriansDick(): void {
    ...
    if (flags[kFLAGS.ARIAN_VAGINA] > 0) outputText(" and tickle her cunt with a finger");
...
function doublePenetrateArian(): void {
    ...
    const x: number = player.cocks.cockThatFits(flags[kFLAGS.ARIAN_CAPACITY]);
    const y: number = player.cocks.cockThatFits2(flags[kFLAGS.ARIAN_CAPACITY]);
    outputText("You look over your feminine lizard lover, and feel your " + multiCockDescriptLight(game.player) + " starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?");
    ...
    outputText("\n\n\"<i>What do you think I'm trying to do!?  Now get in here and paint my womb white!</i>\" Arian snaps, bracing herself on her bed and allowing her ass and pussy both to suck you in with surprising force.  As soon as you're hilted within both holes, her ass clamps shut on your " + cockDescript(game.player, y) + ", while her pussy's contractions begin truly milking you for all you're worth.  Stuck as you are, you have no option but to sit back and enjoy her contractions as you feel yourself nearing the edge of an inevitable orgasm.");
    ...
    // (Low Cum Amount)
    if (player.cumQ() <= 250) {
        outputText("\n\nYour " + multiCockDescriptLight(game.player) + " explode inside Arian's eager holes, giving them the liquid warmth they so crave.  The massage that your two cocks are receiving only enhance the intense feeling, and you find yourself cumming more than usual.  They don't stop massaging you for more, even as you stop unloading.");

        outputText("\n\n\"<i>More, I need more for my eggs!</i>\" Arian demands, yet you are truly spent...");
    }
    // (Medium Cum Amount)
    else if (player.cumQ() <= 1000) {
        outputText("\n\nLiquid lust floods Arian's insides, as your " + multiCockDescriptLight(game.player) + " do their best to relieve ");
        if (player.balls == 0) outputText("themselves");
        else outputText("your [balls]");
        outputText(" of their load; a load Arian is not only pleased to accept, but also eager to relieve you of every single stray drop off.  The tightness of her ass, pressing down on your " + cockDescript(game.player, y) + ", her pussy milking on your " + cockDescript(game.player, x) + ".  How could anyone refuse such an invitation?  You let yourself go, stuffing the eager lizan with more cum than you thought yourself capable of producing.");

        outputText("\n\n\"<i>M-more,</i>\" Arian pleads, even as her belly starts to distend.");
    }
    // (High Cum Amount)
    else {
        outputText("\n\nYou cum with such force, that if Arian's ass wasn't clamping down on your " + cockDescript(game.player, y) + " so tightly, you'd be sure she was going to get pushed off.  \"<i>So much cum!  Yesssss!</i>\" Arian moans, as you quickly give her usually lithe belly a very blatant bump.  Even though your prodigious amount of cum is enough to completely fill the eager lizan-girl, her ass and pussy work overtime to ensure you're completely spent; and you have no desire to resist.");

        outputText("\n\nBy the time you're finished, Arian's belly is positively bulging.  \"<i>Ahhh... eggs... cum... yes...</i>\" she states in a stupor.");
    }
    outputText("\n\nArian's ass goes slack around your " + cockDescript(game.player, y) + ", and Arian slowly slides off your shaft to plop on her bed; eyes closed in bliss, as she takes a short nap.");
...
function giveArianAnItem(): void {
    ...
    if (flags[kFLAGS.ARIAN_S_DIALOGUE] < 1) {
        ...
        if (!player.inv.hasItem(ConsumableLib.VITAL_T)) {
    ...
    if (flags[kFLAGS.ARIAN_S_DIALOGUE] == 0 && arianHealth() >= 10) arianStoryDialogue1();
        ...
        if (player.inv.hasItem(ConsumableLib.VITAL_T)) addButton(0, "Vital Tinct", arianVitalityTincture);
        if (flags[kFLAGS.ARIAN_HEALTH] >= 20) {
            if (player.inv.hasItem(ConsumableLib.P_DRAFT)) addButton(1, "P. Incubi D", giveIncubusDraftToArian);
            if (player.inv.hasItem(ConsumableLib.P_S_MLK)) addButton(2, "P. Suc.Milk", succubiMilkForArian);
            if (player.inv.hasItem(ConsumableLib.LACTAID)) addButton(3, "Lactaid", giveArianLactaid);
            if (player.inv.hasItem(ConsumableLib.REDUCTO)) addButton(4, "Reducto", giveArianReducto);
            if (player.inv.hasItem(ConsumableLib.REPTLUM)) addButton(5, "Reptilum", giveArianReptilum);
        }
        addButton(9, "Back", arianHomeMenu);
    }
}
...
function arianVitalityTincture(): void {
    ...
    player.inv.consumeItem(ConsumableLib.VITAL_T);
    arianHealth(10);
    menu();
...
function succubiMilkForArian(): void {
    ...
    if (flags[kFLAGS.ARIAN_VAGINA] == 0) {
        ...
        if (flags[kFLAGS.ARIAN_BREASTS] == 2) {
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] >= 3) outputText(" and her exposed shaft begins to grow hard");
            outputText(".");
        }
        else {
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] == 3) outputText(" and you see her exposed shaft slowly rising to point at you");
            outputText(".");
...
function giveArianLactaid(): void {
    ...
    if (flags[kFLAGS.ARIAN_BREASTS] == 0) {
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] == 3) outputText(", her exposed cock growing hard as she flushes with arousal");
...
function giveArianReptilum(): void {
    ...
    if (flags[kFLAGS.ARIAN_VAGINA] > 0) outputText("her vagina doing its best to soak her bed with lizan femcum");
    outputText(".");
...
function imbueTalisman(): void {
    ...
    */
    ...
    menu();
    if (player.inv.hasItem(UseableLib.B_CHITN, 2) && player.inv.hasItem(UseableLib.T_SSILK)) addButton(0, "Shielding", arianSpellPlace, "Shielding Spell");
    if (player.inv.hasItem(ConsumableLib.GOB_ALE, 2) && player.inv.hasItem(ConsumableLib.S_GOSSR)) addButton(1, "Immolation", arianSpellPlace, "Immolation Spell");
    addButton(9, "Back", arianHomeMenu);
}
...
function arianSpellPlace(spell: string): void {
    ...
    if (spell == "Shielding Spell") {
        ...
        // Shielding Spell: 2x Black Chitin and 1x Tough Silk. - Increases defense for the duration of the battle.
        player.inv.consumeItem(UseableLib.B_CHITN, 2);
    ...
    if (spell == "Immolation Spell") {
        ...
        // Immolation Spell: 2x Goblin Ale and 1x Sweet Gossamer. - Deals damage over time.
        player.inv.consumeItem(ConsumableLib.GOB_ALE, 2);
        player.inv.consumeItem(ConsumableLib.S_GOSSR);
    }
...
export function sleepWithArian(newl: boolean = false): void {
    ...
    outputText("\n\nArian sighs in happiness at your close contact.  \"<i>It feels so good to have your " + skinFurScales(player) + " against my scales.  So warm...</i>\" [Arian ey] sidles up against you, [Arian eir] tail draping over your waist as [Arian ey] sinks into your embrace.  You just squeeze [Arian em] a little tighter and hold [Arian em] close, saying nothing aside from a quiet whisper to sleep well.  \"<i>Good night, [name],</i>\" [Arian ey] whispers back, before extending a hand toward the globe illuminating the tent and snapping [Arian eir] fingers, shutting down the light.");

    // (if AnalXP <33)
    if (flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
        outputText("\n\nA strange sensation, combined with a soft sound, stirs you from your sleep.  You realize that Arian is stirring in [Arian eir] sleep, softly mumbling to [Arian em]self as [Arian eir] tail gently swishes to and fro under the covers, sometimes accidentally running its warm length over your " + skinFurScales(player) + ".");
...
function TeaseHighAnalXPArian(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        outputText("\n\nIs that so, you ask?  Does [Arian ey] really want a big squishy belly, stuffed full of hot, salty " + mf(player, "man", "herm") + "-goo?");
...
export function wakeUpAfterArianSleep(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
            ...
            outputText("\n\nArian promptly nuzzles your " + multiCockDescriptLight(game.player) + ", rubbing [Arian eir] face all over ");
        ...
        if (flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
            ...
            else if (flags[kFLAGS.ARIAN_VAGINA] > 0) outputText("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
...
export function arianEggingEvent(): void {
    ...
    if (flags[kFLAGS.ARIAN_EGG_CHAT] == 0) {
        ...
        outputText("As you are about to enter Arian's tent, you hear a moan emanate from within.  Those aren't moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent.");
