...
function sheilaIsSorrySheRapedYou(): void {
    ...
    if (player.effects.findByType(StatusAffects.KnowsArouse) >= 0) addButton(2, "CastArouse", sheilaPologyCastArouse);
...
function sheilaFriendlyHerPeoples(): void {
    ...
    // (lib >=50)
        ...
        if (player.cocks.length > 0) outputText(", and you raise the kids at home?  Or put them with a nanny");
        outputText("?</i>\"");
...
function shielaXPThreeSexyTimePostSexStayII(): void {
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(32) >= 0) {
        ...
        // [(not horse)]
        if (!player.isTaur()) outputText(" just above one " + nippleDescription(player, 0));
        ...
        // [(single cox)
        if (player.cocks.length == 1) outputText("your dick twitches inside her");
        // (multicox)
        else outputText("she spies the first bead of pearly semen exiting");
        if (player.cocks.length == 2) outputText(" your lonely, unused prick");
        if (player.cocks.length > 2) outputText(" each of your lonely, unused pricks");
        outputText(".");
        ...
        // [(multi)]
        if (player.cocks.length > 1) outputText(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey white");
        outputText(".");
        // [(big skeet)
    ...
    // (if no cockFit 32 and yes vag)
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cocks.length == 1) outputText("prick quakes and spurts");
    ...
    // (else if no cockFit 32 and no vag)
        ...
        if (player.cocks.length > 0) outputText(", stroking your oversized tool despite her dismissal");
        ...
        if (player.cocks.length > 0) {
            ...
            // [(multicock)
            if (player.cocks.length > 1) outputText("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
        ...
        // (cock.length > 0)
        if (player.cocks.length > 0) outputText(", her face still covered in globby white smears");
        outputText(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [name].");
        // [(cock.length > 0)
        if (player.cocks.length > 0) outputText("  I can't believe you called me a goblin, you arsehole.");
        outputText("</i>\"");

        outputText("\n\nYou ");
        // [(cock.length > 0)
        if (player.cocks.length > 0) outputText("can't believe how much she acts like one sometimes... but you ");
        ...
        // [(cock.length > 0)
        if (player.cocks.length > 0) outputText(", even after Sheila finishes wiping her face clean and stares at you");
...
function sheilaXPThreeSexyTimeGuardDuty(): void {
    ...
    // (else lib >= 40)
        ...
        // [(cock.length > 0)
        if (player.cocks.length > 0) outputText("shift your pelvis forward");
        // (cock.length == 0)
...
function sheilaXP4Sex(display: boolean = false): void {
    ...
    menu();
    if (player.gender > 0) {
        if (player.cocks.length > 0) addButton(0, "69WithWang", consensualSheila69, true);
        if (player.vaginas.length > 0) addButton(1, "69WithCooch", consensualSheila69, false);
    }
    // [Consensual - Vaginal Sex] (req. cock <= 32 area):
    if (player.cocks.length > 0 && player.cocks.cockThatFits(32) >= 0) addButton(2, "Fuck Her", consentacleVagSexForKangarooSlutBitches);
    // da scene
...
function consensualSheila69(cock: boolean = true): void {
    ...
    if (cock) {
        if (player.cocks.cockThatFits(32) >= 0) {
    ...
    // (vag)
        ...
        outputText("  When you finally drop your bottoms, revealing your wet " + vaginaDescript(player, 0) + ", she smiles softly.");
        ...
        if (player.clitLength > 8) outputText(" and bring your " + clitDescription(player) + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
    ...
    if (cock && player.cocks.cockThatFits(32) < 0) outputText("  \"<i>[name], I'm telling you it won't fit!</i>\"");
    ...
    if (cock) {
        ...
        // (big)
        if (player.cocks.cockThatFits(32) < 0) outputText(" and guiding it to bend comfortably against her chest, drooling a trail of pre-cum as she slides it into position");
        outputText(".");
    }
    // (vag)
    else outputText("returning the favor and plunging into your " + vaginaDescript(player, 0) + " with her hand.");
    ...
    if (cock) {
        ...
        // [(small cock)
        if (player.cocks.cockThatFits(32) >= 0) outputText(" into her warm, waiting mouth");
    ...
    if (!cock) {
        ...
        // [(bigclit)
        if (player.clitLength > 8) outputText(" and stroking your " + clitDescription(player) + " rapidly with one hand");
        outputText(".");
    }
    // (smallcox)
    else if (player.cocks.cockThatFits(32) >= 0) outputText("taking you into her mouth once again and licking away a neglected drop of pre-cum.");
    ...
    if (!player.isTaur()) {
        ...
        if (cock && player.cocks.cockThatFits(32) < 0) outputText("shaking, teasing nipples and heaving chest");
    ...
    // (horse)
        ...
        if (cock && player.cocks.cockThatFits(32) < 0) outputText("hard nipples");
        else outputText("throat");
        outputText(".");
    }
    // merge horse with rider
    // [(big cox)
    if (cock && player.cocks.cockThatFits(32) < 0) outputText("\n\n\"<i>[name]!  I'm coming!</i>\" Sheila cries.");
    ...
    // [(big cox)
    if (cock && player.cocks.cockThatFits(32) < 0) outputText("pressing her lips and tongue to the base of your shaft as she masturbates it vigorously.");
    /// (small cox)
    ...
    if (cock) {
        ...
        // (else doesn't fit)
            ...
            if (player.cocks.length > 1) {
                ...
                if (player.cocks.length == 2) outputText(" spasms");
    ...
    // (else if vag-focus)
        ...
        if (player.vaginas.wetness() <= 2) outputText("trickle out, and are quickly sucked up by her greedy mouth");
        else if (player.vaginas.wetness() <= 4) outputText("gush freely, leaving her face slick");
    ...
    // [(smallcox)
    if (cock && player.cocks.cockThatFits(32) >= 0) outputText(", pulling free of her throat.  She immediately turns over as well and forces herself to her hands and knees, then gags and coughs up a large amount of your jism.  When she's dumped enough to clear her passageway, she collapses forward, smearing the mess onto her chest as she spends a few minutes to catch her breath.  Once she's got it, she turns back over and sits up, strands of your slime linking her breasts to the grass.");
    // (else not smallcox)
    ...
    if (cock && player.balls > 0) {
        ...
        outputText(" shape against the rest of her skin, is an imprint of your " + ballsDescriptLight(player) + ".  The kangaroo girl rolls her eyes at the juvenile display and resumes her harangue.  ");
    }
...
function consentacleVagSexForKangarooSlutBitches(): void {
    ...
    if (!player.breastRows.hasFuckableNipples()) outputText("squeezing and fondling");
    ...
    // [(if wangaroo)
    if (player.cocks[player.cocks.cockThatFits(32)].cockType == CockTypesEnum.KANGAROO) outputText("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
    ...
    if (player.balls > 0 || player.vaginas.length > 0) {
        ...
        else outputText(vaginaDescript(player, 0));
        outputText(" and tickling you as she writhes in pleasure.  ");
        // (if vag)
        if (player.balls == 0 && player.vaginas.length > 0) outputText("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
    }
    ...
    if (player.cor >= 90 || player.cor > flags[kFLAGS.SHEILA_CORRUPTION]) {
        ...
        if (player.cocks.length > 1) {
            ...
            if (player.cocks.length == 2) outputText("dick erupts");
    ...
    // (else if equal but below 90)
        ...
        if (player.cocks.length > 1) outputText(" or onto her thighs and belly");
        outputText(".");
    }
    ...
    // (dog cock)
        ...
        // [(multi)
        if (player.cocks.length > 2) outputText("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with whiteness. ");
    }
...
function sheilaMutualMasturbation(): void {
    ...
    if (player.vaginas.length > 0 && !player.isTaur()) choices[choices.length] = "vagina";
    if (player.breastRows.hasFuckableNipples()) choices[choices.length] = "nipplecunt";
    ...
    // [(if cock)
    if (target == "cock") {
        if (player.cocks.length == 1) outputText("prick begins");
    ...
    // (vag)
        ...
        if (player.vaginas.wetness() < 4) outputText("drooling");
    ...
    // (else no corruption warning triggered)
        ...
        outputText("\n\nSheila strokes your " + hairDescription(player) + " as you lie ");
...
function sheilaCorruptionWarningsChooseFondle(): void {
    ...
    if (player.cocks.length > 0) outputText("erect");
    else if (player.vaginas.length > 0) outputText("wet");
...
export function sheilaGotWhomped(): void {
    ...
    menu();
    if (player.lust >= 33) {
        if (player.cocks.length > 0 && player.cocks.cockThatFits(48) >= 0) addButton(0, "Fuck Pussy", rapeSheilasCooter);
        if (player.cocks.length > 0) {
            if ((!player.isTaur() && player.cocks.shortestCockLength() <= player.tallness * .35) || (player.isTaur() && player.tallness * 1.2 <= player.cocks.shortestCockLength() && player.cocks.shortestCockLength() <= player.tallness * 1.55)) {
                addButton(1, "Forced BJ", forcedSheilaOral, true);
            }
        }
        if (player.vaginas.length > 0) addButton(2, "Forced Lick", forcedSheilaOral, false);
    }
...
function rapeSheilasCooter(): void {
    ...
    // set sheila xp = -4
    flags[kFLAGS.SHEILA_XP] = -4;
    const x: number = player.cocks.cockThatFits(48);
    ...
    if (player.cor < 50) {
        ...
        if (player.cocks.cockArea(x) > 32) outputText(" and the pain of taking a cock slightly too large for her");
        ...
        if (player.cocks.length > 1) {
            outputText("  Your other ");
            if (player.cocks.length == 2) outputText("dick spits");
...
function forcedSheilaOral(dick: boolean = true): void {
    ...
    if (dick) {
        ...
        // [(cock width <= 6")
        if (player.cocks[player.cocks.shortestCockIndex()].cockThickness <= 6) {
        ...
        // [(dick length <= 8)
        if (player.cocks[player.cocks.shortestCockIndex()].cockLength <= 8) {
        ...
        if (player.cor >= 70 && player.effects.getValue1Of(StatusAffects.Infested) == 5) {
        ...
        if (player.effects.findByType(StatusAffects.HairdresserMeeting) >= 0 || flags[kFLAGS.LUMI_MET] > 0) {
...
function sheilaGetsRapedWithADildont(): void {
    ...
    // (cock)
    if (player.cocks.length > 0) outputText("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
...
function sheilaCowgirlRapesYou(): void {
    ...
    // (else if cock fit 32)
        ...
        // [(multicock)]
        if (player.cocks.length > 1) outputText("separating out and ");
...
function sheilaForcedOralGeneric(): void {
    ...
    else if (player.vaginas.length > 0) outputText("her fingers slip between your labia");
    ...
    // (else PC corr < sheila corruption)
        ...
        if (player.cocks.length > 0) outputText("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
        else if (player.vaginas.length > 0) outputText("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
    ...
    if (player.tongueType > TongueType.HUMAN) fatigue(15);
...
function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE(): void {
    ...
    else if (player.lowerBody == LowerBodyType.HARPY && player.tailType == TailType.HARPY) {
    ...
    // (bees with boots)
    else if (player.lowerBody == LowerBodyType.BEE) {
...
function normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP(): void {
    ...
    // [(if Jojo present)
    if (player.effects.findByType(StatusAffects.JojoNightWatch) >= 0 || player.effects.findByType(StatusAffects.PureCampJojo) >= 0) outputText("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
...
function sheilaGoesDemon(): void {
    ...
    if (player.tongueType > TongueType.HUMAN) outputText("extending your oral muscle until it's all the way to the back of her pussy.  ");
...
function askDemonSheilaAboutDahChange(): void {
    ...
    if (flags[kFLAGS.SHEILA_CITE] >= 1) {
        ...
        if (player.cocks.length > 0) outputText(", and you feel a monument of your own being erected");
...
function askDemonSheilaAboutHerColony(): void {
    ...
    // [(cock)
    if (player.cocks.length > 0) outputText("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
...
function talkToDemonSheilaAboutWhatNow(): void {
    ...
    if (player.cor < 50) {
        ...
        if (player.cocks.length > 0) outputText("my pussy");
    ...
    if (player.cocks.length > 0) outputText("give them to me");
...
function demonSheilaTalkAnotherQuestion(): void {
    ...
    if (player.lust <= 99) {
        ...
        // (cock)
        if (player.cocks.length > 0) outputText("slipping more of her finger and thumb around your penis");
        else if (player.vaginas.length > 0) outputText("parting your lips more");
...
function demonImpChildren(): void {
    ...
    if (player.cocks.length > 0) dynStats("lus", 10 + player.lib / 10, "resisted", false);
    // [Other Sex][Knock Up(cock only)][Leave]
    menu();
    addButton(0, "Other Sex", pregDemonSheilaOtherSex);
    if (player.cocks.length > 0) addButton(1, "Knock Up", pregDemonSheilaKnockUp);
...
function pregDemonSheilaKnockUp(): void {
    ...
    menu();
    if (player.cocks.cockThatFits(56) >= 0) addButton(0, "Next", missionaryForThePurposesOfCreatingImpsWithSheila);
...
export function loseToSheila(consensual: boolean = false): void {
    ...
    if (player.HP < 1 && !consensual) {
        ...
        if (player.cocks.length > 0 && player.cocks.cockThatFits(56) >= 0)
            choices[choices.length] = loseToNormalSheilaAndGetRidden;
        // Loss - tail-spade sounding for fuckhueg cock (for cockarea > 56)
        if (player.cocks.length > 0 && player.cocks.biggestCockArea() > 56)
...
function loseToNormalSheilaAndGetRidden(): void {
    ...
    outputText("\n\n");
    // [(multi)
    if (player.cocks.length > 1) outputText("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [cockFit 56].  ");
    ...
    // [(multi)
    if (player.cocks.length > 0) outputText(" and slipping the unused remainder of your manhood between her thighs");
    ...
    if (player.balls > 0) {
        ...
        outputText("\n\nSheila's pussy spasms around you and she looks into your eyes; you feel a wave of compulsion from them, and arousal flows into you from each place your " + skin(player) + " touches her body.  Unable to withstand her orgasm-fueled power, your [balls] pull close to your shaft and you let go of your climax, grabbing her hips and dumping semen into her cunt");
        if (player.cocks.length > 1) outputText(" and lap");
...
function tailSpadeSoundingForFuckHugeDongsWithDemonSheila(): void {
    ...
    if (player.cocks.length > 1) outputText("; unguided, the ");
    if (player.cocks.length == 2) outputText("other twitches and sprays");
    else if (player.cocks.length > 2) outputText("others twitch and spray");
    if (player.cocks.length > 1) outputText(" all over, coating her back, you, the grass, and anything ");
    if (player.cocks.length > 2) outputText("else they happen to point at");
    if (player.cocks.length == 2) outputText("else it happens to point at");
...
function clitSwellingDemonSheilaClitSoundingAnal(): void {
    ...
    outputText(" forcing your lips apart and lingering on your rapidly-swelling " + clitDescription(player) + ".  \"<i>Oh, my.  What's with this ");
    ...
    if (player.clitLength <= 6) {
        ...
        else if (player.breastRows.biggestTitSize() >= 1) outputText(" between your " + allBreastsDescript(player));
        ...
        else outputText(buttDescription(player));
        outputText(" as she shallowly pumps your " + clitDescription(player) + " in and out of her spade.  Your pelvis twitches uselessly; with her tail waving in the air instead of braced against her, controlling the pace of the fuck is beyond you.  \"<i>Naughty sheila,</i>\" she says, squeezing your wiggling hip with a hand.  You think privately that she could just as easily be talking about herself.  \"<i>Am I not going fast enough for you?  Let me speed up.</i>\"");
        ...
        if (player.cor < 50) {
            ...
            outputText("\n\nIrritated, she wraps her other frantic tail around the still-straining black one and squeezes, finally managing to get some of her climax past the ring of tight skin at the end; with the fluid forming a bridge, you feel the tension release as a wave of her black, slippery tail-cum forces her slit wide open and gushes out to soak your clit and then your pussy underneath.  \"<i>Ah!  Ah!  Finally!</i>\" she cries, great gouts of agitated slime sliding down your " + clitDescription(player) + " to bubble and lick at your pussy, as if trying to get inside.");
        }
        // (else PC corr>=50)
        else {
            outputText("her tail quakes, then pushes her pussy into yours.  \"<i>God, yes!</i>\"  A spatter of hot fluid hits your " + vaginaDescript(player, 0) + " as your demon lover climaxes, and another begins to drool around the tip of your clit, still suspended inside her writhing tail.  The sudden labial heat is too much for you, and you answer her with a climax of your own, ");
            if (player.vaginas.wetness() >= 4) outputText("drizzling");
    ...
    // (else if clit >6" initially)
        ...
        outputText(", then wraps her tails around your thickened clit.  They twist and pull, sending spirals of pleasure along the numerous nerves, and begin gently but insistently tugging you toward her tan butt.  Sheila directs the tip right between her cheeks, and you can feel her using it to rub the ring around her hole.  \"<i>I think this should be about the right size for you, love,</i>\" she teases, spearing herself on your " + clitDescription(player) + ".  Inch after inch slides into her slowly, maddening you with warmth, until ");
        ...
        if (player.clitLength <= 12) outputText("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
            ...
            // [(if virgin vagina)
            if (player.vaginas.hasVirginVagina()) outputText("  \"<i>What's this?</i>\" Sheila teases.  \"<i>Why, your virtue is still intact, little girl!  I wouldn't want anything to happen to it!</i>\"  She pulls her tails out a bit, contenting herself with simply teasing your lips and driving you mad for penetration.");
        ...
        outputText("  Suddenly she stops mid-stroke as her asshole quivers, sending shivers up your orgasm-sensitized clitoris.  \"<i>Fffuck!  So soon!  I'm coming too, [name]!</i>\"  Sheila sinks down, burying your " + clitDescription(player) + " into her asshole.");
        // (clit>12")
        if (player.clitLength <= 12) {
            ...
            if (player.vaginas.hasVirginVagina()) outputText("  <b>You've lost your virginity!</b>");
        ...
        if (player.clitLength > 12 && player.vaginas.hasVirginVagina()) {
    ...
    if (player.clitLength > 12 || player.cor < 50) {
        cuntChange(player, 12, false, false, false);
        player.slimeFeed();
    }
    player.clitLength++;
...
export function beatUpDemonSheila(output: boolean = true): void {
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(56) >= 0) addButton(0, "Missionary", missionaryForThePurposesOfCreatingImpsWithSheila);
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(56) >= 0) addButton(1, "AnalHateFuck", analHateFuck);
    // Win - [Big Dick+Thighs] (usable for dicks with cockarea > 56) (all 10 of them)
    // penis envy, go!
    if (player.cocks.length > 0 && player.cocks.biggestCockArea() > 56) addButton(2, "UseHerThighs", bigDickAndThighs);
    // Win - [Penetration, in Spades] - for self-loathing vaginas
    if (player.vaginas.length > 0) addButton(3, "Clit-Fuck", winAgainstDemoNSheilaForVaginas);
}
...
function missionaryForThePurposesOfCreatingImpsWithSheila(): void {
    ...
    if (player.cocks.length > 1) {
        outputText(" and its ");
        if (player.cocks.length == 2) outputText("brother");
    ...
    if (player.cocks.length > 1) outputText(" and coating her crotch with a glaze of white");
    ...
    if (player.cocks.length > 1 || player.cumQ() >= 500) outputText(" as the spare seed dribbles from her crotch");
    outputText(", then breaks away and looks up into your face.");

    outputText("\n\n\"<i>[name], you ");
    if (player.cumQ() >= 500 || player.cocks.length > 1) outputText("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
    ...
    // (else if RNG doesn't end scene)
        ...
        // (if vag)
        if (player.vaginas.length > 0) outputText(" as her imp spawn take turns doing the same to you from behind");
...
function sheilaAnalHateFuckAGoGo(): void {
    ...
    if (player.cor >= 80 && player.effects.findByType(StatusAffects.Infested) >= 0) {
        menu();
...
function bigDickAndThighs(): void {
    ...
    if (!player.isTaur()) {
        ...
        if (player.cocks.length > 2) outputText(" one at a time, practically stacking them atop each other");
        outputText(".");
    }
    // (horse)
    else {
    ...
    if (player.cor < 40) {
        ...
        if (player.cocks.length == 1) outputText("your dick with both");
    ...
    // (else corr >= 40)
        ...
        if (player.cocks.length > 1) {
            ...
            if (player.cocks.length == 2) outputText(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
    ...
    if (player.cocks.length > 2) outputText("relinquish your other pricks and ");
    if (player.cocks.length == 2) outputText("relinquish your other prick and ");
    ...
    if (sheilaCorruption() < 50) outputText("  Though her modest breasts can't even begin to encompass your girth, Sheila rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
        ...
        if (player.cocks.length > 1) outputText(" built for " + num2Text(player.cocks.length) + " pricks");
        outputText(".");
    }
    ...
    if (player.cocks.length > 1) outputText("topmost ");
...
function winAgainstDemoNSheilaForVaginas(): void {
    ...
    if (!player.isTaur()) {
        ...
        // [(tail or naga)
        if (player.tailType > TailType.NONE && player.isNaga()) outputText(" and raise your tail");
    }
