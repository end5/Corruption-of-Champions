...
public  nomOnMommaIzzysTits():void {
	...
	if(player.tallness > 78 || player.isTaur()) {
		...
		if(player.hasCock()) {
			...
			if(player.biggestCockArea() > 40) outputText("hard, barely constraining " + game.player.sMultiCockDesc() + ".", false);
			else outputText("visibly from " + game.player.sMultiCockDesc() + ".", false);
			outputText("  ", false);
		}
		if(player.hasVagina()) {
...
public  izzyGivesSmallWangsFreeOral():void {
	...
	else outputText("pose lewdly and trace a finger over your bulge as you inform Isabella just how happy you'd be to feel her tongue on your " + game.player.cockDescript(x) + ".", false);
	...
	outputText("Isabella goes to work immediately, undoing the lower portions of your " + player.armorName + " with strong, forceful motions that shake your " + assDescript() + " as she forcibly disrobes you.  Free at last, your " + game.player.cockDescript(x) + " flops out", false);
	...
	if(isabellaAccent()) outputText(".  The busty redhead gleefully squeals, \"<i>Oooh it's so cute!  Even ven it's hard like this, it looks sort of like something that vould go on a girl.</i>\"  She pulls on it, leading you around by your " + game.player.cockDescript(x) + " until you're in front of her chair, and then she pushes you back onto the seat, still warm from the cow-girl's bountiful ass.  She asks, \"<i>How long do you think it vill last, hrmm?  I vonder what its milk tastes like...</i>\"\n\n", false);
	else outputText(".  The busty redhead gleefully squeals, \"<i>Oooh it's so cute!  Even when it's hard like this, it looks sort of like something that would go on a girl.</i>\"  She pulls on it, leading you around by your " + game.player.cockDescript(x) + " until you're in front of her chair, and then she pushes you back onto the seat, still warm from the cow-girl's bountiful ass.  She asks, \"<i>How long do you think it will last, hrmm?  I wonder what its milk tastes like...</i>\"\n\n", false);
	
	outputText("The cow-girl pulls down on her neckline, giving you a tantalizing view of her cream and caramel cleavage.  She leans forwards and presses her milk-swollen tits against your " + player.legs()  + ", rocking up and down so that you can feel each of the soft orbs squeezing around you.  Isabella's tongue slides out... and out... and out, until you see at least seven inches of tongue hovering over your " + game.player.cockDescript(x) + ".  Her warm spittle drips from the pulsing, smooth pink exterior of her tantalizing tongue while it lashes back and forth, less than an inch away from your " + player.cockHead(x) + ".  Each drop of fallen cow-girl spit that lands on your " + player.cockHead(x) + " only turns you on more, until you're grunting and panting at her, begging like an animal with a needy expression on your face.\n\n", false);
	
	outputText("Isabella smirks knowingly and caresses the sensitive underbelly of your " + game.player.cockDescript(x) + " while she coos, \"<i>");
	...
	outputText("</i>\"  To emphasize her point, the well-endowed cow-girl leans down and shows you just how flexible she can be.  The hot, wet slipperiness of her oral organ turns sideways, mashing against the side of your " + player.cockHead(x) + ".  A split-second later, it slides down, and her tongue makes another loop around your " + game.player.cockDescript(x) + ".  The process continues on and on, girding your manhood in wide, spit-lubed cow-tongue until the entire thing is cocooned inside Isabella's velvet embrace.\n\n", false);
	...
	outputText("  The strength ebbs from your body while Isabella corkscrews her tongue around you, and the warm, sticky wetness that envelops your " + game.player.cockDescript(x) + " grows hotter and hotter.  The contentment you've been feeling melts away like ice-cream on a sunny day while you adjust to the sensation.  Your body craves more, and Isabella obliges, opening her lips wide to engulf you wholly with her mouth.\n\n", false);
	...
	if(player.sens < 50) {
		...
		outputText("The cow-girl returns to her task with gusto, snaring your " + game.player.cockDescript(x) + " with her tongue, but instead of going all-out with her corkscrew technique, she's pumping it, sliding her hot wetness up and down your shaft with practiced ease.  ", false);
		if(player.balls > 0) outputText("She cups your " + ballsDescriptLight() + " and begins caressing the twitching orbs, giving them gentle squeezes each time the pleasure forces them to involuntarily contract towards your groin.", false);
			...
			outputText(" of your " + game.player.cockDescript(x) + " to your " + assholeDescript() + ", teasing the sensitive skin with light touches of her fingernail.", false);
		}
		...
		outputText(" while her tongue spins and pumps your " + game.player.cockDescript(x) + " to an inevitable release.  Your eyes cross as you try to hold out, but in seconds the telltale warmth begins to build inside you.  Finally, you give in and submit, feeling the cum welling in the cow-girl's suckling fuck-hole.\n\n", false);
	...
	//(High sensitivity fail!) 
		...
		else if(player.cockTotal() > 1) outputText("her hand stroking and squeezing your neglected " + game.player.cockDescript(1) + " almost as an afterthought.", false);
		...
		if(player.isGoo()) player.slimeFeed();
		if(isabellaAccent()) {
			outputText("You sigh nervelessly as the cow-girl waggles her tongue at you teasingly, making your " + game.player.cockDescript(x) + " jump from the memory of pleasure.  The redhead moans, \"<i>Oooh I knew it!  You are a pervert!  I just vanted to do a little licking and you got me all vet with your salty... mmm... cream.</i>\"  She pauses to lick her lips again before giving a gentle moo of contentment.  At least she doesn't seem mad at you!\n\n", false);
		
			outputText("The tanned woman looks down at you with disappointment and says, \"<i>You should get going, my tiny-cocked, pervert friend.  Perhaps you vill have some sex and learn how not to submit at ze first hint of pleasure?</i>\"  You go red with indignation, but she fondles your half-limp " + game.player.cockDescript(x) + " the entire time, a knowing smile spread across her lips.  Isabella helps you get dressed and gives you a firm smack on the " + buttDescript() + " as she says goodbye, \"<i>Don't change too much " + player.mf("boy","maid") + "!  I just hope ven you come back you've learned how not to cum ven I touch your buttons!</i>\"\n\n", false);
		}
		else {
			...
			outputText("The tanned woman looks down at you with disappointment and says, \"<i>You should get going, my tiny-cocked, pervert friend.  Perhaps you will have some sex and learn how not to submit at the first hint of pleasure?</i>\"  You go red with indignation, but she fondles your half-limp " + game.player.cockDescript(x) + " the entire time, a knowing smile spread across her lips.  Isabella helps you get dressed and gives you a firm smack on the " + buttDescript() + " as she says goodbye, \"<i>Don't change too much " + player.mf("boy","maid") + "!  I just hope when you come back you've learned how not to cum when I touch your buttons!</i>\"\n\n", false);
		}
	}
	if(!isabellaFollower()) isabellaFollowerScene.isabellaAffection(2);
	player.orgasm();
...
public  isabellaRapesYouWithHerAss():void {
	...
	if(player.cockTotal() > 1) {
		...
		{
			outputText("  The bovine bitch reaches down to stroke your " + game.player.cockDescript(x) + ".  \"<i>So ready, ");
	...
	if(player.balls > 0) outputText(" or the tightening of your " + sackDescript(player) + " as it pulls your "+ ballsDescriptLight(), false);
	...
	if(player.cockThatFits(38) >= 0) {
		...
		if(player.balls > 0) outputText("Involuntary muscles contract, tugging your " + sackDescript(player) + " up against your crotch while your " + ballsDescriptLight() + " visibly contract, loading your body full of liquid lust that's about to explode.", false);
		...
		if(player.cumQ() >= 1000) {
			...
			if(player.cumQ() >= 1500) outputText(", but your " + game.player.cockDescript(x) + " won't let up.  She cries and moans, jism pouring out around your shaft", false);
	...
	//[TOOBIG]
		...
		if(player.balls > 0) outputText("Your " + sackDescript(player) + " pulls tight against your body, each of your " + ballsDescriptLight() + " quivering and pushing its load through you.", false);
		...
		outputText("You keep squirting until your body is completely empty, leaving your " + game.player.cockDescript(x) + " to twitch and clench, trying to unload phantom seed.\n\n", false);
...
public  IsabellaPostSpankFeedSex():void {
	...
	outputText("It feels so good, so very good, but you struggle with the pleasure.  It SHOULDN'T feel this good to be held down by and raped until you're sinking into sex-scented mud, yet your " + game.player.cockDescript(x) + " is twitching inside Isabella's muscular folds, growing so hard you feel like a nail being driven through butter.  The cow-girl's milk-fountains don't help, soaking your belly and " + game.player.chestDesc() + " with sweet, thickening cream and adding more whorls of white to the dirty slurry.  Isabella's back arches and she screams,");
...
public  defeatIsabella():void {
	...
	var lactation:() => void = null;
	if(player.biggestLactation() >= 1) lactation = victoryLactation69;
	var buttsex:() => void = null;
	var sixtyNine:() => void = null;
	if(player.gender > 0 && player.lust >= 33) sixtyNine = victoryAgainstIzzzzzySixtyNine;
	var bigTitFuck:() => void = null;
	var smallTitFuck:() => void = null;
	var vaginalSex:() => void = null;
	if(player.hasCock()) {
...
public  victoryLactation69():void {
	...
	if(player.hasCock()) outputText("squeeze " + game.player.sMultiCockDesc() + " gently, feeling the heavy, turgid mass pulsating with each beat of your heart.", false);
	...
	outputText("With your bellies filling, the conflict that started it all seems like a distant dream that's washing away with each burst of sexual relief and swallow of pearlescent milk.  Isabella's moos of pleasure vibrate your " + nippleDescript(0) + ", adding to your own", false);
	if(player.breastRows.length > 1) outputText(", while your other " + game.player.breastDescript(1) + " plasters her crimson hair into her scalp", false);
	...
	else if(player.hasCock()) outputText("  Of course, you're pumping at " + game.player.sMultiCockDesc() + " just as hard, but the sounds of your fapping are easily drowned out by the enthusiastic cow-girl.", false);
	else outputText("  Of course, you're fingering your " + assholeDescript() + " just as hard, but that muscular hole barely makes a sound as you violate it.", false);
	...
	outputText("After a few moments of guzzling Isabella's orgasmic cream, your own climax starts to build, radiating from your squirting " + nippleDescript(0) + " and ", false);
	if(player.hasCock()) outputText(game.player.multiCockDescriptLight(), false);
	else if(player.hasVagina()) outputText(vaginaDescript(), false);
	...
	//(Cuntnips) 
	if(player.hasFuckableNipples()) outputText("  Isabella thrusts her tongue at your " + nippleDescript(0) + ", burrowing it deep into the unusual passage to get at the last of your liquid bounty and inadvertently intensifying your pleasure.  Your arm gives out and you drop on top of her, burying her in tit while hers serve as your pillows.", false);
	//(Cock & Puss)
	if(player.gender == 3) {
		if(player.wetness() == 5) outputText("  The juices that splatter from your " + vaginaDescript() + " almost seems an afterthought, but when you cum, you cum <i>allll</i> over.", false);
		else if(player.wetness() > 3) outputText("  The juices that drip from your " + vaginaDescript() + " almost seems an afterthought, but when you cum, you cum all over.", false);
		else outputText("  The clenching of your " + vaginaDescript() + " almost seems an afterthought, but when you cum you cum all over.", false);
	}
	//(Cock)
	if(player.hasCock()) {
		outputText("  With a few more solid pumps " + game.player.sMultiCockDesc() + " goes rock hard", false);
		if(player.balls > 0) outputText(", your " + ballsDescriptLight() + " pulling tight against your groin", false);
	...
	if(player.gender == 2) {
		...
		outputText("  Your " + clitDescript() + " pulses and jumps with each brush of your thumb, and you nearly lose yourself in the hedonistic self-pleasure.", false);
	}
	//(butt)
	if(player.gender == 0) outputText("  The constant fingering of your " + assholeDescript() + " finally comes to fruition, sending bolts of pleasure from your anal passage.  The star clenches tight around your digits, contracting involuntarily around the invader.", false);
	outputText("\n\n", false);
...
public  PCVictoryOnIsabellaButtsex():void {
	...
	outputText("You shrug out of your " + player.armorName + " and expose " + game.player.sMultiCockDesc() + " to the warm, breezy air", false);
	if(player.lust < 70) {
		outputText(", allowing ", false);
		if(player.cockTotal() == 1) outputText("it ", false);
		else outputText("them ", false);
		outputText("to harden precipitously.  The turgid flesh of " + game.player.sMultiCockDesc() + " grows harder and harder, egged on by the sight of Isabella's dark pucker and juicy, leaking cunt", false);
	}
	else {
	...
	outputText("You advance and grab her tail, yanking the bovine appendage back to pull those glorious ass-cheeks closer to your " + game.player.cockDescript(x) + ".  ", false);
	if(player.cockTotal() > 1) {
		outputText("It's probably the best suited to the task, out of your " + game.player.multiCockDescriptLight() + "", false);
		if(y >= 0) outputText(", and your " + game.player.cockDescript(y) + " may as well go into that sloppy cow-cooch", false);
		outputText(".  ", false);
	}
	...
	outputText(", and she voices a moo of discomfort when you slap her moist backside.  You ignore it and pull harder on her tail, forcing her anus to devour your entire " + game.player.cockDescript(x) + " in a single stroke", false);
	...
	if(y>= 0) outputText("contracts around your " + game.player.cockDescript(y) + " and squeezes a flow of heated pussy-juice over the penis, cocooning it with enough warm heat and wetness to make you feel as if you're melting.", false);
	...
	outputText(" and feeling the cum build up inside your " + ballsDescriptLight() + ".", false);
	...
	outputText("  Heat wells up from your " + ballsDescriptLight() + ", signaling the time to teach the haughty broad a lesson, at last.\n\n", false);
	...
	if(y >= 0) outputText("Your " + game.player.cockDescript(y) + " emulates its brother, shooting ropes of semen into the cow-girl's drippy pussy.  ", false);
	...
	if(player.cumQ() >= 750) {
		...
		outputText(" with more spunk, enough to balloon her back-door and remove most of the sensation from your " + game.player.cockDescript(x) + ".", false);
		if(player.cumQ() < 2000) {
...
public  victoryAgainstIzzzzzySixtyNine():void {
	...
	outputText("You twist sinuously, sliding your " + hipDescript() + " over Isabella until your groin is hovering over her mouth and your " + player.legs() + " are brushing her ears.  She tentatively opens her mouth, spreading her dusky lips wide as her tongue licks them.  The cow-girl grabs hold of your " + assDescript() + ", knowing full well what's expected of her, and pulls up to ", false);
	if(player.gender == 2) outputText("nuzzle your " + vaginaDescript(0) + ", smearing your vulva with her lips even she spreads the folds to access your " + clitDescript(), false);
	else if(player.cockThatFits(38) != -1) outputText("swallow your " + game.player.cockDescript(x) + " whole, plying her smooth, spit-covered tongue over every inch of it", false);
	else outputText("lick your " + game.player.cockDescript(x) + ", fondling it with her long, smooth tongue in ways that no human could", false);
	outputText(".\n\n", false);
	...
	//(VAGOOZLES) 
	if(player.gender == 2) {
		outputText("Isabella returns to her task with gusto, muff-diving deep and hard to plant her exquisitely long tongue far inside you.  Her lips circle your " + clitDescript() + ", ", false);		
	...
	//(WANGS – no fit)
		...
		outputText(" numerous times before bobbing her head up and down.  She's using her tongue like some kind of onahole, and the warm, slippery pressure makes your " + game.player.cockDescript(x) + " want to melt with pleasure.  The cow-girl slides all the way to the tip", false);
	...
	//(Dick + No fit)
		...
		outputText("Even as you exult in victory, the humming pleasure-squeals of Isabella's orgasm vibrate through her lips and tongue, spreading down your shaft.  The effect is profound and immediate, and even as Isabella loses control of her oral onahole, orgasm wracks your " + game.player.cockDescript(x) + ".  It trembles and pulsates a second before the first load bursts from the tip and splatters on Isabella's thin, white blouse.  The next doesn't fly as far, and drops on her neckline.  After that, you pour a few bursts onto her face.", false);
		if(player.cumQ() > 250) {
	...
	if(player.cockTotal() > 1) {
		outputText("  Throughout it all you feel like a one-man bukkake show, spurting jism from your " + game.player.multiCockDescriptLight() + " over the cumming cow-girl.", false);
	}
	outputText("\n\n", false);
...
public  tooBigVictoryTittyFuckingFuntimesWithMilk():void {
	...
	outputText("", true);
	outputText("You toss aside your " + player.armorName + " to reveal your " + game.player.cockDescript(x) + " to the ", false);
	...
	outputText("Sighing blissfully, you grab hold of Isabella's breasts by the prominent nipples and pull them together, finishing the titillating encapsulation of your member.  She moans loudly at the forceful tug as she watches you violating her tits.  The " + player.cockHead(x) + " of your " + game.player.cockDescript(x) + " bursts from between the pillowy mounds to leak on the cow-girl's chest, and the bovine beauty can only manage a dark blush in response.  You're already leaking a steady trickle of pre-cum from your crown from the hot tightness of Izzy's tits; orgasm isn't too far.\n\n", false);
	
	outputText("The cow-girl utters a pleasured moo in response to the tight nipple tension.  Holding tightly, you roll the four tips between your fingers as you slowly begin to fuck the cow-girl, ignoring the wetness that begins to squirt between them.  Isabella's hips seem to squirm of their own volation, and the cow-girl's legs spread wide to grind her skirt-girded loins against your " + player.legs() + ".  You respond by turning your gentle thrusts into rough, pistoning motions that shake the jiggling melons violently around your " + game.player.cockDescript(x) + ", but it only seems to please the milk-spurting cow.\n\n", false);
	
	outputText("Isabella grinds against you in a frenzy, thrashing wildly against your " + player.legs() + ".  Her motions only exacerbate the tightness of the tit-sleeve milking your " + game.player.cockDescript(x) + ", and as you look down at the convulsing cow-girl's blissful expression, you feel a telltale welling of pressure in your " + ballsDescriptLight() + ".  You wait for her mouth to gape particularly wide before thrusting forward, ", false);
	...
	outputText("Your " + game.player.cockDescript(x) + " is laid out over a busty, milk-spewing cow-girl, pressing up all the way ", false);
	...
	if(!isabellaFollower()) isabellaFollowerScene.isabellaAffection(3);
	outputText("Shrugging, you wipe " + game.player.sMultiCockDesc() + " off on her lips and get dressed.  This cow is one marvelous cum-dump.", false);
	player.orgasm();
	cleanupAfterCombat();
}
...
public  vaginalProdNPokeIsabella():void {
	...
	outputText("In spite of the distracting milk-fountains, your main focus remains on her cunt, and how wonderful that hot little box feels around your " + game.player.cockDescript(x) + ".  ", false);
	if(player.totalCocks() > 1) {
		outputText("You wish you had room for ", false);
		if(player.totalCocks() > 2) outputText("another " + game.player.oMultiCockDesc(), false);
...
public  tinyVictoryTittyFuckingFuntimesWithMilk():void {
	...
	outputText(" cow-girl.  She squeals in delight at the sight of your " + game.player.multiCockDescriptLight(), false);
	...
	outputText("The cow-girl grabs hold of your " + game.player.cockDescript(x) + " for emphasis and begins to stroke it enthusiastically.  Having her large (but still feminine) hand wrapped around you makes your dick look positively tiny in comparison, ", false);
	...
	outputText("Pulling herself up to lean against your groin, Isabella holds her hefty breasts in her hands and presses them around your " + game.player.cockDescript(x) + ".  The squishy-soft mammaries utterly envelops your penis in their warm, sweat-lubricated embrace", false);
	if(player.cockTotal() > 1) {
