...
export function isabellaGreeting(): void {
    ...
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00257] == 0) {
        ...
        if (player.effects.findByType(StatusAffects.Marble) >= 0) outputText("She's like Marble - she has a human face with horns and cow-like ears.", false);
    ...
    // Camp Meeting – Was welcome tall, but not short yet!
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cocks[player.cocks.shortestCockIndex()].cockLength < 9) {
                ...
                if (player.cocks[player.cocks.shortestCockIndex()].cockLength < 20) outputText("large", false);
    ...
    // Camp Meeting – Standard Repeat
        ...
        if (player.cocks.length > 0) {
            outputText("  She takes a long sniff and glances between your " + legs(player) + " at your groin", false);
            if (player.cocks[player.cocks.shortestCockIndex()].cockLength >= 9) outputText(", sighing wistfully.", false);
            else {
...
export function nomOnMommaIzzysTits(): void {
    ...
    if (player.tallness > 78 || player.isTaur()) {
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cocks.biggestCockArea() > 40) outputText("hard, barely constraining " + sMultiCockDesc(game.player) + ".", false);
            else outputText("visibly from " + sMultiCockDesc(game.player) + ".", false);
            outputText("  ", false);
        }
        if (player.vaginas.length > 0) {
            if (player.cocks.length > 0) outputText("Even y", false);
            else outputText("Y", false);
            outputText("our " + vaginaDescript(player, 0) + " ", false);
            if (player.vaginas.wetness() < 3) outputText("glistens with ", false);
            else if (player.vaginas.wetness() < 5) outputText("drips ", false);
        ...
        if (player.horns > 0 && player.hornType > HornType.NONE) outputText("your horns", false);
        ...
        outputText("You get rather absorbed in your task and lose track of time as you pull harder, trying to see just how far her supply of the stuff goes.  A flood of creamy sweetness nearly drowns you in response, and you're forced to chug for a few seconds to keep up with the flood.  Even without your suction, the flow of milk is much stronger than before, and it slakes your thirst quite effectively.  Isabella's song-strengthened voice begins moaning out loud, and though you can't make out the exact words due to her thick accent and lapses into a strange tongue, the meaning is quite clear: \"<i>Good " + mf(player, "boy", "girl") + "... (unintelligable moans)... yes, keep drinking... (more moaning)</i>\"\n\n", false);

        outputText("You're roughly yanked away from the milk spouting breast and pulled up to Isabella's face.  The cow-girl's bronzed visage is flushed darker, and even one of the white patches on her neck is crimson-tinged with lust.  She licks the creamy milk from your lips with an exceptionally wide, smooth tongue and then passionately french-kisses you, squirting more of her milk onto your " + chestDesc(game.player) + " the entire time.", false);
        if (player.cocks.length > 0) outputText("  " + SMultiCockDesc(game.player) + " twitches and drips from the intensity of the kiss, but you had completely forgotten about your sexual needs while you were drinking.", false);
        else if (player.vaginas.length > 0) outputText("  Your " + vaginaDescript(player, 0) + " aches with need and desire from the intensity of the kiss, but you had completely forgotten it during the feeding.", false);
        ...
        if (player.wingType > WingType.NONE && player.wingType != WingType.SHARK_FIN) outputText(" and stroke your wings", false);
    ...
    // Voluntary Nursing (Small Characters)
        ...
        outputText("You stop and look at them, just look, leaving nothing in the air but a long, pregnant pause that seems to go on and on.  Isabella coughs, snapping you from your reverie – you're standing a foot away from those sweat-glazed orbs, and you jerk your head back.  If you leaned any further forward, her prominent, quad-tipped areola would be in your mouth.  The cow-girl laughs and scoops you up in her arms before you can hesitate further.  Your cheek is crushed against a white spot on the side of her left tit, and your " + buttDescription(player) + " comes to rest on the short fur that sprouts from her thick thighs.  Isabella coos, \"<i>Relax, " + player.short + ", and let Izabella sate your thirst.  You vill love it.</i>\"\n\n", false);
        ...
        outputText("Sometime later you burp loudly and snuggle against the perfect, soft chest in front of you, sighing with happiness as a hand strokes your " + hairDescription(player) + ".  Isabella croons soft nothings into your ear and you drift into a dreamless, restful slumber.\n\n", false);
        ...
        if (player.cocks.length > 0 && x >= 0) {
            ...
            if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00261] == 0) {
                ...
                outputText("She sleep-fucked you again!  You sigh and wipe your hand off on your " + leg(player) + ", bemused by the cow-girl who seems too shy to sleep with someone who's awake.  Still, you feel completely sated in every way.  It's going to be a good ", false);
                if (game.time.hours < 12) outputText("day", false);
                else if (game.time.hours < 4) outputText("afternoon", false);
...
export function izzyGivesSmallWangsFreeOral(): void {
    ...
    else outputText("pose lewdly and trace a finger over your bulge as you inform Isabella just how happy you'd be to feel her tongue on your " + cockDescript(game.player, x) + ".", false);
    outputText("  The cow-girl blushes hard enough to color her dusky cheeks with a hint of rose, but her chest heaves with barely-contained excitement.  She drops out of the chair onto her knees and licks her lips hungrily, like a child eying a favorite treat.  Her hands dart forward and grab you by the " + hipDescription(player) + ", dragging you into a breast-padded hug.\n\n", false);

    outputText("Isabella goes to work immediately, undoing the lower portions of your " + player.armorName + " with strong, forceful motions that shake your " + buttDescription(player) + " as she forcibly disrobes you.  Free at last, your " + cockDescript(game.player, x) + " flops out", false);
    ...
    outputText("  The strength ebbs from your body while Isabella corkscrews her tongue around you, and the warm, sticky wetness that envelops your " + cockDescript(game.player, x) + " grows hotter and hotter.  The contentment you've been feeling melts away like ice-cream on a sunny day while you adjust to the sensation.  Your body craves more, and Isabella obliges, opening her lips wide to engulf you wholly with her mouth.\n\n", false);
    ...
    if (player.sens < 50) {
        ...
        if (player.balls > 0) outputText("She cups your " + ballsDescriptLight(player) + " and begins caressing the twitching orbs, giving them gentle squeezes each time the pleasure forces them to involuntarily contract towards your groin.", false);
            ...
            outputText("She probes your " + vaginaDescript(player) + " with her fingers, running them over your engorged lips and giving your " + clitDescription(player) + " ", false);
            ...
            outputText(" of your " + cockDescript(game.player, x) + " to your " + assholeDescript(player) + ", teasing the sensitive skin with light touches of her fingernail.", false);
        }
        ...
        outputText(" while her tongue spins and pumps your " + cockDescript(game.player, x) + " to an inevitable release.  Your eyes cross as you try to hold out, but in seconds the telltale warmth begins to build inside you.  Finally, you give in and submit, feeling the cum welling in the cow-girl's suckling fuck-hole.\n\n", false);
        ...
        if (IsabellaFollowerScene.isabellaAccent()) {
            ...
            outputText("The feisty redhead happily helps you back into your " + player.armorName + " and gives you an unceremonious smack on the " + buttDescription(player) + " before saying her goodbyes, \"<i>Come back soon, " + player.short + "!  You are quite the " + mf(player, "man", "woman") + ", even if your tasty penis is tiny.  Oh don't look like that, it makes such tasty salt-milk!  I'll lick it up any time.  Now go, I'm sure you have much to do!</i>\"\n\n", false);
        }
    }
    // (High sensitivity fail!)
    else {
        outputText("You try to fight the heaven around your " + cockDescript(game.player, x) + ", but it's too much for your poor, sensitive body to endure.  Giving up, you relax, hips pistoning instinctively into her mouth as the warm tightness of an orgasm rises inside you.  Isabella's eyes stare up at your face, watching intently while she keeps her lips wrapped tightly ", false);
        if (player.cocks.hasSheath()) outputText("around your sheath", false);
        ...
        if (player.balls > 0) outputText("her hand stroking and gently squeezing at your " + ballsDescriptLight(player) + " as if she could milk a bigger load from them that way.", false);
        else if (player.cocks.length > 1) outputText("her hand stroking and squeezing your neglected " + cockDescript(game.player, 1) + " almost as an afterthought.", false);
...
export function IsabellaWinsAndSpanks(): void {
    ...
    if (player.tallness <= 78) {
        ...
        if (player.perks.findByType(PerkLib.Masochist) >= 0) {
            ...
            if (player.cocks.length > 0) outputText("squirt pre-cum ", false);
            else if (player.vaginas.length > 0) outputText("slick your thighs ", false);
        ...
        if (player.perks.findByType(PerkLib.Masochist) < 0) outputText("draws a scream from your lips, pushing you past the bounds of consciousness.  Isabella hefts your limp form like a wet noodle and grasses sway behind you as you're carried off.", false);
            ...
            if (player.vaginas.length > 0) {
                outputText("  Your " + vaginaDescript(player) + " quivers and ", false);
                if (player.vaginas.wetness() < 5) outputText("drips", false);
...
export function isabellaRapesYouWithHerAss(): void {
    ...
    if (player.cocks.cockThatFits(38) >= 0) {
        ...
        if (player.balls > 0) outputText("Involuntary muscles contract, tugging your " + sackDescript(player) + " up against your crotch while your " + ballsDescriptLight(player) + " visibly contract, loading your body full of liquid lust that's about to explode.", false);
        ...
        if (player.cumQ() >= 1000) {
            ...
            if (player.cumQ() >= 1500) outputText(", but your " + cockDescript(game.player, x) + " won't let up.  She cries and moans, jism pouring out around your shaft", false);
    ...
    // [TOOBIG]
        ...
        if (player.balls > 0) outputText("Your " + sackDescript(player) + " pulls tight against your body, each of your " + ballsDescriptLight(player) + " quivering and pushing its load through you.", false);
        ...
        outputText("You keep squirting until your body is completely empty, leaving your " + cockDescript(game.player, x) + " to twitch and clench, trying to unload phantom seed.\n\n", false);
        ...
        outputText(", pooling around your " + legs(player) + " on the ground.", false);
...
export function IsabellaPostSpankFeedSex(): void {
    ...
    outputText("The cow-girl lets some of the pressure off in order to tweak one of your " + nippleDescription(player, 0) + "s, but as you gasp, her tongue is forced into your mouth, smothering your ", false);
    if (player.tongueType == TongueType.HUMAN) outputText("smaller", false);
    ...
    outputText("Her pussy tightens, clamping down and feeling smaller and smaller.  It's inhuman, squeezing more than a clenched fist - only this grip is made of syrupy-slipperiness and velvet cushions.  You can't resist the pleasure any longer, and you arch your back, digging yourself deeper into the mud in order to push your " + cockDescript(game.player, x) + " a tiny bit further into Isabella's spasming embrace.  Spooge boils up from your " + ballsDescriptLight(player) + ", ", false);
...
export function defeatIsabella(): void {
    ...
    let bigTitFuck: () => void = null;
    let smallTitFuck: () => void = null;
    let vaginalSex: () => void = null;
    if (player.cocks.length > 0) {
        if (player.cocks.cockThatFits(monster.analCapacity()) != -1 && player.lust >= 33) buttsex = PCVictoryOnIsabellaButtsex;
        if (player.cocks.cockThatFits(monster.vaginalCapacity()) != -1 && player.lust >= 33) vaginalSex = vaginalProdNPokeIsabella;
        if (player.cocks.cockArea(player.cocks.biggestCockIndex()) > 70 && player.lust >= 33) bigTitFuck = tooBigVictoryTittyFuckingFuntimesWithMilk;
        if (player.cocks[player.cocks.shortestCockIndex()].cockLength < 9 && player.lust >= 33) smallTitFuck = tinyVictoryTittyFuckingFuntimesWithMilk;
    }
...
export function victoryLactation69(): void {
    ...
    if (player.cocks.length > 0) outputText("squeeze " + sMultiCockDesc(game.player) + " gently, feeling the heavy, turgid mass pulsating with each beat of your heart.", false);
    else if (player.vaginas.length > 0) outputText("rub your fingers over the puffy lips of your pussy, feeling your vulva spread to give the juice-lubed digits access.", false);
    ...
    else outputText("  Of course, you're fingering your " + assholeDescript(player) + " just as hard, but that muscular hole barely makes a sound as you violate it.", false);
    ...
    outputText("After a few moments of guzzling Isabella's orgasmic cream, your own climax starts to build, radiating from your squirting " + nippleDescription(player, 0) + " and ", false);
    if (player.cocks.length > 0) outputText(multiCockDescriptLight(game.player), false);
    else if (player.vaginas.length > 0) outputText(vaginaDescript(player), false);
    ...
    if (player.gender == 3) {
        ...
        else if (player.vaginas.wetness() > 3) outputText("  The juices that drip from your " + vaginaDescript(player) + " almost seems an afterthought, but when you cum, you cum all over.", false);
        else outputText("  The clenching of your " + vaginaDescript(player) + " almost seems an afterthought, but when you cum you cum all over.", false);
    }
    // (Cock)
    if (player.cocks.length > 0) {
        outputText("  With a few more solid pumps " + sMultiCockDesc(game.player) + " goes rock hard", false);
        if (player.balls > 0) outputText(", your " + ballsDescriptLight(player) + " pulling tight against your groin", false);
    ...
    // (Puss only)
    if (player.gender == 2) {
        outputText("  The constant fingering of your " + vaginaDescript(player) + " accomplishes its goal, making your velvet tunnel clench vise-tight against the intruding digit.  ", false);
        if (player.vaginas.wetness() >= 4) outputText("Juice squirts from your hole, splattering onto the dirt.", false);
        else if (player.vaginas.wetness() >= 2) outputText("Girlcum drips from the hole, absorbing into the dirt.", false);
        else outputText("Girlcum soaks your lips, filling the air with female lust.", false);
        outputText("  Your " + clitDescription(player) + " pulses and jumps with each brush of your thumb, and you nearly lose yourself in the hedonistic self-pleasure.", false);
    }
    // (butt)
    if (player.gender == 0) outputText("  The constant fingering of your " + assholeDescript(player) + " finally comes to fruition, sending bolts of pleasure from your anal passage.  The star clenches tight around your digits, contracting involuntarily around the invader.", false);
    outputText("\n\n", false);
...
export function PCVictoryOnIsabellaButtsex(): void {
    ...
    outputText("You shrug out of your " + player.armorName + " and expose " + sMultiCockDesc(game.player) + " to the warm, breezy air", false);
    if (player.lust < 70) {
        outputText(", allowing ", false);
        if (player.cocks.length == 1) outputText("it ", false);
        else outputText("them ", false);
        outputText("to harden precipitously.  The turgid flesh of " + sMultiCockDesc(game.player) + " grows harder and harder, egged on by the sight of Isabella's dark pucker and juicy, leaking cunt", false);
    }
    else {
        outputText(", the wind teasing over the hardness of your length", false);
        if (player.cocks.length == 1) outputText(".  It seems", false);
    ...
    outputText("You advance and grab her tail, yanking the bovine appendage back to pull those glorious ass-cheeks closer to your " + cockDescript(game.player, x) + ".  ", false);
    if (player.cocks.length > 1) {
        outputText("It's probably the best suited to the task, out of your " + multiCockDescriptLight(game.player) + "", false);
        if (y >= 0) outputText(", and your " + cockDescript(game.player, y) + " may as well go into that sloppy cow-cooch", false);
        outputText(".  ", false);
    }
    outputText("Isabella's eyes widen with from the sudden pressure of your " + cockHead(player, x), false);
    if (y >= 0) outputText(" and other " + cockHead(player, y) + " pressing against her holes", false);
    else outputText(" pressing against her dark pucker", false);
    outputText(", and she voices a moo of discomfort when you slap her moist backside.  You ignore it and pull harder on her tail, forcing her anus to devour your entire " + cockDescript(game.player, x) + " in a single stroke", false);
    if (y >= 0) outputText(" while her pussy sheaths your other member in dick-melting warmth", false);
    outputText(".\n\n", false);

    outputText("The over-endowed redhead shudders underneath you, her muscles clamping involuntarily with such force that you're immobilized mid-thrust.  Somehow she holds you like that, squeezing with enough force that your " + cockHead(player) + " feels like it could pop.  You slap her ass hard enough to leave a handprint on her plump posterior, the spreading shock pursing Isabella's lips just as it dilates her anal ring.  She grunts wordlessly, too far gone to give voice to her complaints.  You slam back into her sweltering embrace", false);
    if (y >= 0) outputText("s", false);
    outputText(" hard enough to make the clap echo off some distant landmark.\n\n", false);

    outputText("Milk audibly 'drip-drip-drip's in the dirt, disgorged by the cow's wobbling udders.  Isabella's loud grunts begin to change with each prick-sheathing thrust, adding a low whimper of lust to her voice until the cow-girl is letting out long moos of contented desire.  You let go of her tail and grab hold of her waist.  Fingers dig into the red lace of her corset with every pull, dragging the red-head's near-heifer-sized cheeks into your " + hipDescription(player) + ".  She still squeezes around your ", false);
    ...
    if (y >= 0) outputText("contracts around your " + cockDescript(game.player, y) + " and squeezes a flow of heated pussy-juice over the penis, cocooning it with enough warm heat and wetness to make you feel as if you're melting.", false);
    else outputText("spews out a few strands of girl-juice that cling to your " + legs(player) + ", hanging and stretching with every sticky slap against her body.", false);
    ...
    outputText(" and feeling the cum build up inside your " + ballsDescriptLight(player) + ".", false);
    ...
    outputText("  Heat wells up from your " + ballsDescriptLight(player) + ", signaling the time to teach the haughty broad a lesson, at last.\n\n", false);
    ...
    if (y >= 0) outputText("Your " + cockDescript(game.player, y) + " emulates its brother, shooting ropes of semen into the cow-girl's drippy pussy.  ", false);
    ...
    if (player.cumQ() >= 750) {
        ...
        outputText(" with more spunk, enough to balloon her back-door and remove most of the sensation from your " + cockDescript(game.player, x) + ".", false);
        if (player.cumQ() < 2000) {
...
export function victoryAgainstIzzzzzySixtyNine(): void {
    ...
    if (player.effects.findByType(StatusAffects.Edryn) < 0) outputText("plumpest, juiciest set of pussy lips you've ever had the chance to lick.", false);
    ...
    outputText("You twist sinuously, sliding your " + hipDescription(player) + " over Isabella until your groin is hovering over her mouth and your " + legs(player) + " are brushing her ears.  She tentatively opens her mouth, spreading her dusky lips wide as her tongue licks them.  The cow-girl grabs hold of your " + buttDescription(player) + ", knowing full well what's expected of her, and pulls up to ", false);
    if (player.gender == 2) outputText("nuzzle your " + vaginaDescript(player, 0) + ", smearing your vulva with her lips even she spreads the folds to access your " + clitDescription(player), false);
    else if (player.cocks.cockThatFits(38) != -1) outputText("swallow your " + cockDescript(game.player, x) + " whole, plying her smooth, spit-covered tongue over every inch of it", false);
    else outputText("lick your " + cockDescript(game.player, x) + ", fondling it with her long, smooth tongue in ways that no human could", false);
    outputText(".\n\n", false);
    ...
    // (VAGOOZLES)
    if (player.gender == 2) {
        outputText("Isabella returns to her task with gusto, muff-diving deep and hard to plant her exquisitely long tongue far inside you.  Her lips circle your " + clitDescription(player) + ", ", false);
        ...
        if (player.vaginas.wetness() < 3) outputText("coating her tongue", false);
        else if (player.vaginas.wetness() < 4) outputText("sloppily leaking everywhere", false);
    ...
    // (WANGS - fits)
    else if (player.cocks.cockThatFits(38) != -1) {
        outputText("Isabella resumes her task with gusto, wrapping her lips around your ", false);
        if (player.cocks.hasSheath()) outputText("sheath", false);
        else outputText("base", false);
        outputText(" and suckling until her cheeks hollow from the vacuum.  Meanwhile, her tongue lashes around " + cockDescript(game.player, x) + ", slurping even as it curls around your manhood.  It circles it slowly, gradually building a tight, tongue-based cocoon that squeezes even as it slides around you, throwing off hellish amounts of pleasure.", false);
        if (player.cocks.length > 1) {
            outputText("  She ignores your other dick", false);
            if (player.cocks.length > 2) outputText("s as they smear her cheeks, focusing on this one, perfectly-matched specimen.", false);
    ...
    // (WANGS – no fit)
        ...
        outputText(" numerous times before bobbing her head up and down.  She's using her tongue like some kind of onahole, and the warm, slippery pressure makes your " + cockDescript(game.player, x) + " want to melt with pleasure.  The cow-girl slides all the way to the tip", false);
    ...
    // (Vagoo)
    if (player.gender == 2) {
        outputText("Even as you exult in victory, the humming pleasure-squeals of Isabella's orgasm vibrate through her lips and tongue, straight up your " + clitDescription(player) + ".  Orgasm hits you hard enough to knock the strength from your arms, and you collapse on top of the cow-girl, reflexively grinding your " + hipDescription(player) + " into her face.  Your " + vaginaDescript(player) + " ripples and clenches, squeezing Isabella's tongue as if it could milk some kind of cum from it.  The mouthful of pussy you have doesn't stop you from giving voice to your climax, and as you ", false);
    ...
    // (Dick + No fit)
        ...
        outputText("Even as you exult in victory, the humming pleasure-squeals of Isabella's orgasm vibrate through her lips and tongue, spreading down your shaft.  The effect is profound and immediate, and even as Isabella loses control of her oral onahole, orgasm wracks your " + cockDescript(game.player, x) + ".  It trembles and pulsates a second before the first load bursts from the tip and splatters on Isabella's thin, white blouse.  The next doesn't fly as far, and drops on her neckline.  After that, you pour a few bursts onto her face.", false);
        if (player.cumQ() > 250) {
    ...
    if (player.cocks.length > 1) {
        outputText("  Throughout it all you feel like a one-man bukkake show, spurting jism from your " + multiCockDescriptLight(game.player) + " over the cumming cow-girl.", false);
    }
    outputText("\n\n", false);

    outputText("Finally and completely sated, you roll off, ", false);
    if (player.cocks.length > 0) outputText("twitching as your member disentangles itself from her tongue.", false);
...
export function tooBigVictoryTittyFuckingFuntimesWithMilk(): void {
    ...
    outputText("Your gaze is so intently focused on the quad-tipped areolae that you barely acknowledge her request.  A slight grunt escapes your lips while you fondle your shaft, squeezing it gently with both hands.  You take a half-step forward and aim your " + cockHead(player, x) + " between Isabella's breasts.  The first dollop of pre-cum oozes from the tip just in time to smear into the valley of cleavage, lubricating the sweaty skin even further.  Her body is hot from the recent combat, and the warm, wet embrace of the cow-girl's bosom feels absolutely heavenly as you slide home through the valley of chest.\n\n", false);

    outputText("Sighing blissfully, you grab hold of Isabella's breasts by the prominent nipples and pull them together, finishing the titillating encapsulation of your member.  She moans loudly at the forceful tug as she watches you violating her tits.  The " + cockHead(player, x) + " of your " + cockDescript(game.player, x) + " bursts from between the pillowy mounds to leak on the cow-girl's chest, and the bovine beauty can only manage a dark blush in response.  You're already leaking a steady trickle of pre-cum from your crown from the hot tightness of Izzy's tits; orgasm isn't too far.\n\n", false);

    outputText("The cow-girl utters a pleasured moo in response to the tight nipple tension.  Holding tightly, you roll the four tips between your fingers as you slowly begin to fuck the cow-girl, ignoring the wetness that begins to squirt between them.  Isabella's hips seem to squirm of their own volation, and the cow-girl's legs spread wide to grind her skirt-girded loins against your " + legs(player) + ".  You respond by turning your gentle thrusts into rough, pistoning motions that shake the jiggling melons violently around your " + cockDescript(game.player, x) + ", but it only seems to please the milk-spurting cow.\n\n", false);

    outputText("Isabella grinds against you in a frenzy, thrashing wildly against your " + legs(player) + ".  Her motions only exacerbate the tightness of the tit-sleeve milking your " + cockDescript(game.player, x) + ", and as you look down at the convulsing cow-girl's blissful expression, you feel a telltale welling of pressure in your " + ballsDescriptLight(player) + ".  You wait for her mouth to gape particularly wide before thrusting forward, ", false);
    ...
    outputText("Your " + cockDescript(game.player, x) + " is laid out over a busty, milk-spewing cow-girl, pressing up all the way ", false);
    ...
    else outputText("leans back to swallow your " + cockHead(player, x) + " and lick it, just before", false);
...
export function vaginalProdNPokeIsabella(): void {
    ...
    outputText("You step back to undress. Isabella takes the opportunity to reach for her sodden snatch, but you brush her questing fingers away with your " + foot(player) + ".  She glares at you a moment before she resumes her slutty gyrations, the need for satisfaction overwhelming her irritation at being denied.  You " + mf(player, "chuckle", "giggle") + " down at her as you pull out your " + multiCockDescriptLight(game.player) + ".  Wide-eyed, the red-head watches you stroke " + sMultiCockDesc(game.player) + " and licks her lips.\n\n", false);
    ...
    if (IsabellaFollowerScene.isabellaAccent()) {
        ...
        if (player.cocks.cockArea(x) > monster.vaginalCapacity()) outputText("Oh! The cock! It's too big for me!  It's... stretching me so mooo-uch!", false);
        else if (player.cocks.cockArea(x) > 13) outputText("Oh, such a nice cock you have!  It's rubbing my pussy so well!", false);
    ...
    outputText("In spite of the distracting milk-fountains, your main focus remains on her cunt, and how wonderful that hot little box feels around your " + cockDescript(game.player, x) + ".  ", false);
    if (player.cocks.length > 1) {
        outputText("You wish you had room for ", false);
        if (player.cocks.length > 2) outputText("another " + oMultiCockDesc(game.player), false);
...
export function tinyVictoryTittyFuckingFuntimesWithMilk(): void {
    ...
    outputText("Pulling herself up to lean against your groin, Isabella holds her hefty breasts in her hands and presses them around your " + cockDescript(game.player, x) + ".  The squishy-soft mammaries utterly envelops your penis in their warm, sweat-lubricated embrace", false);
    if (player.cocks.length > 1) {
