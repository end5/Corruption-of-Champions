...
export class SophieScene implements TimeAwareInterface {
        ...
        if (!sophieAtCamp()) { // Could be at the farm or still in the wild
            ...
            if (pregnancy.isPregnant) {
                if (pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
                    const triggeredEvent: number = pregnancy.eventTriggered();
...
export function sophieAtCamp(): boolean { // Whether she's a bimbo or not
...
export function sophiesDaughterDescript(): void {
...
export function sophiesPregnancyDescript(triggeredEvent: number): void {
    if (triggeredEvent == 1) {
...
// [Discovery]
export function meetSophie(): void {
...
// [Repeat Meeting]
export function meetSophieRepeat(): void {
    SophieBimbo.sophieSprite();
    outputText("", true);
    // (Pissed)
    ...
    if (player.cocks.length > 0) {
        ...
        if (rand(2) == 0 && !pregnancy.isPregnant) {
            ...
            outputText("  Sophie releases her talons and you drop smartly onto the far side.  She pants, \"<i>It's breeding time.  " + mf(player, "Boy", "Girl") + ", fertilize me; NOW.</i>\"\n\n", false);
...
function sophieLookingForDemons(): void {
    ...
    outputText(mf(player, "boy", "girl") + "?  You seem to see something you want,</i>\" observes the curvaceous bird-woman. \"<i>Come into my nest, " + player.short + "; it's been sooo long since I've been properly... fertilized.</i>\"  Sophie relaxes as she awaits your response.\n\n", false);
...
function cramANippleInIt(): void {
    ...
    if (player.breastRows.biggestLactation() < 2) {
        ...
        if (player.breastRows.totalNipples() > 2) {
            ...
            if (player.breastRows.totalNipples() < 5) outputText("before finishing the last one.", false);
            else outputText("and the next, continuing until you're quite empty.", false);
        }
        outputText("\n\n", false);
    }
    // (Medium PG)
    else if (player.breastRows.biggestLactation() < 3.5) {
        ...
        if (player.breastRows.totalBreasts() > 2) {
            if (player.breastRows.totalNipples() > 2) outputText("She drinks from each of the nipples that adorn your " + allBreastsDescript(player) + " until her belly is full and gurgling.", false);
            else outputText("She drinks from each breast in turn until her belly is full and gurgling.", false);
        }
        else if (player.breastRows.totalNipples() > 2) outputText("She drinks from each nipple in turn until her belly is full and gurgling.", false);
        outputText("\n\n", false);
    }
    // (High PG)
    else {
        ...
        if (player.breastRows.totalNipples()) outputText("other ", false);
        else outputText("next ", false);
        outputText("one and begins the cycle anew.", false);
        if (player.breastRows.totalNipples() >= 4) outputText("  With all your nipples, this goes on for quite some time.", false);
        outputText("\n\n", false);
    }

    outputText("Sophie pulls back with ", false);
    if (player.breastRows.biggestLactation() >= 3.5) outputText("an incredibly loud burp and blushes red.", false);
    else if (player.breastRows.biggestLactation() >= 2) outputText("a satisfied burp.", false);
...
function consensualHotSophieDickings(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 1) outputText("her breasts rubbing against your own", false);
    else {
    ...
    if (flags[kFLAGS.FUCKED_SOPHIE_COUNTER] == 0) {
        ...
        outputText(", or are you just falling in love with me?  It's been some time since I've had an admirer like you.  We had a word for " + mf(player, "boys", "sluts") + " like you – perverts.  I kid, I kid.  Perverts like you have such pent up, potent cum, and I know you love to give it to me.", false);
    }
    else {
...
function consensualSophieSexNoFit(): void {
    ...
    if (player.tallness <= 48) outputText("her breasts pressed into your face", false);
        ...
        if (player.breastRows.biggestTitSize() >= 1) outputText("own", false);
        else outputText("chest", false);
    }
    else outputText("her breasts pressing against your stomach", false);
    outputText(", your " + cockDescript(game.player, x) + " begins to grow along the bottom of the nest.  It rubs her lower lips delightfully as it lengthens underneath her.   Her eyes go wide with shock and she exclaims, \"<i>That's bigger than a minotaur's!  Just because I've laid a few hundred eggs in my time doesn't mean I can fuck a monstrous giant-cock.  You do realize that with a pecker like that you belong in some demonic zoo, rather than in a beautiful harpy's nest.</i>\"\n\n", false);
    ...
    outputText("Your body twists and squirms underneath her, aching for more stimulation, more aroused than it should be.  Your grunts and moans grow to a feverish intensity until Sophie leans back to place a finger on your lips.  She commands, \"<i>Shush now; I know the pollen in my lip-gloss makes it hard not to cum right away, but you need to wait until I'm ready for your seed.</i>\"  You whine plaintively but it's no use.  It feels like orgasm is so close that you could reach out and touch it, but it just won't come.  Sophie giggles cruelly and circles your bloated " + cockHead(player, x) + " with her fingertip, assuring you: \"<i>You can cum soon, I promise.  Just wait; once your tip is in my pussy you'll cum out all your seed.  Your harpy queen commands it!</i>\"  Her words seem... heavy, somehow; like they have some force or weight behind them.\n\n", false);

    outputText("Sophie stands up and begins to flap her wings, kicking up dust and debris and forcing you to shield your eyes as she lifts off the ground.  Her hands lock onto your " + cockDescript(game.player, x) + " and guide her over top of it.  Beads of wetness drip from her vagina and roll over your tender, sensitive skin as she lowers herself down, planting your " + cockHead(player, x) + " inside her egg-widened pussy lips.  Her primary wings flap in a frenzy, struggling to hold her aloft without the aid of her arms, but you barely notice.  Your " + cockDescript(game.player, x) + " is cumming and cumming HARD for the matronly harpy, filling her waiting womb with spunk.", false);
    if (player.cumQ() >= 1000) {
...
export function luststickApplication(hours: number = 4): void {
    // Immune to luststick?
...
export function sophieLostCombat(): void {
    SophieBimbo.sophieSprite();
    flags[kFLAGS.SOPHIE_FOLLOWER_PROGRESS] = 0;
...
export function sophieWonCombat(): void {
    SophieBimbo.sophieSprite();
    flags[kFLAGS.SOPHIE_FOLLOWER_PROGRESS] = 0;
    if (player.cocks.length > 0) {
...
function maleVictorySophieRape(): void {
    ...
    outputText("Warmth boils up in your loins, the telltale sign of your coming orgasm.   You plunge inside her and bend over, biting her nipples as you cum.  Jism boils out of your " + cockDescript(game.player, x) + ", creaming her pussy as Sophie has another blissful orgasm.  You doubt she's even conscious at this point, but you don't care one way or another.  ", false);
...
function maleVictorySophieRapeHUGE(): void {
    ...
    outputText("You press your " + cockDescript(game.player, x) + " forward, intent on abusing Sophie's hole with your gargantuan fuck-stick, but it won't fit.   In desperation, you slide your dick over her lips, stimulating her slippery gash and lubricating your " + cockDescript(game.player, x) + " with harpy-lust, but when you pull back and try to cram it inside... it still won't fit!  Sophie grunts underneath you and barks in irritation, \"<i>It won't work, " + mf(player, "boy", "girl") + "!  You're too big for harpy-snatch, even one as well-traveled as mine!</i>\"  You reach down and slap her ass punitively, and the jiggle that slowly travels up her large thighs gives you an idea.\n\n", false);

    outputText("Balling your hand into a fist, you push it into her large, sloppy hole and collect a handful of her fluids.  You pull it out and slather it over her muscled thighs and the veiny, unlubricated surface of your " + cockDescript(game.player, x) + ", then go back in for more.  In half a minute, her crotch and your cock are slicked with Sophie's prodigious pussy-fluid.  You drop your " + cockDescript(game.player, x) + " down into place between her legs and pull her feet together, sandwiching your dick-flesh in a vice of soft thigh-meat.  It's divine, and though Sophie seems rather unhappy with the situation, her half-hearted struggles only make her legs feel that much better.\n\n", false);

    outputText("Tears well up at the corners of her eyes as you use the harpy like a sex-toy.  It's so pathetic that you actually take pity on the poor matron.  You gather both her ankles in your right hand, freeing your left to reach under the flesh of your " + cockDescript(game.player, x) + " and push inside her vagina.  Sophie's eyes cross at the sudden large intrusion, but her hips rock instinctively as if she could somehow milk some cum from the intruder.  You reach in far enough to feel the barrier of her womb and gently push against it before you pull back out for the next penetration.  The harpy has stopped crying and is starting to squeeze and relax her thighs around your " + cockDescript(game.player, x) + ".  She pays back your mercy with as much pleasure as she can, and presses her breasts around your " + cockDescript(game.player, x), false);
    ...
    if (player.cocks[x].cockLength >= 40) {
        outputText("  She sucks the " + cockHead(player, x) + " inside her mouth and whirls her tongue around it ", false);
    ...
    outputText(".", false);
    if (player.cumQ() >= 500) {
        outputText("  Clenched tightly, you revel in the feeling of thick jizz-globules distorting your " + cockDescript(game.player, x) + " as they blast out.  It splatters wetly, making a total mess of your curvy victim", false);
    ...
    if (player.cocks[x].cockLength < 50) outputText("covered the underside of your " + cockDescript(game.player, x) + " in kisses during your orgasm, but the tingling oral drugs only made it feel better.", false);
    else outputText("has been running her drug-laced lip-gloss all over your " + cockHead(player, x) + " this entire time!  The tingling made your orgasm even better, but you know you've absorbed a lot of the stuff by now.", false);
    outputText("\n\n", false);

    outputText("Sophie leans back and scoops a handful of your white goo into her lubricant-soaked snatch and promptly falls asleep.  It figures she would do that.  " + SMultiCockDesc(game.player) + " stays hard, and you're sure it's only going to get worse for quite some time.  You glance down at the mother-harpy, considering a second fuck, but decide that it'd be better with a partner who was still awake.", false);
    // Apply harpy status.
...
function sophieVictoryPussyGrind(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 1) outputText("You pinch a " + nippleDescription(player, 0), false);
    ...
    if (player.vaginas[0].vaginalWetness >= VaginaWetness.SLAVERING) outputText("Juices squirt from the joined pussies", false);
    else if (player.vaginas[0].vaginalWetness >= VaginaWetness.SLICK) outputText("Juices soak the nesting material", false);
    else if (player.vaginas[0].vaginalWetness >= VaginaWetness.WET) outputText("Juice trickles from the paired pussies", false);
...
function fuckDatClit(): void {
    ...
    if (player.vaginas[0].vaginalWetness >= VaginaWetness.DROOLING) outputText("Juice dribbles down your " + legs(player) + " as your body enjoys the treatment.  ", false);
...
function tinyDickSupremeSophieLoss(): void {
    ...
    outputText("Her words sting, making your " + cockDescript(game.player, 0) + " wilt to an even smaller size, egging Sophie on to greater taunts.  She announces, \"<i>I'm still going to use your cum for fertilizer since any egg will be a girl anyway.  Isn't that nice?  " + mf(player, "Maybe she'll have no tits and be teased for the rest of her life, just like her trap of a father.", "Maybe she'll have nice big tits like her trap of a father.") + "  You're going to cum just as soon as the heat from my pussy hits you, I know it.  There's no way a little dick like that stands a chance at enduring real sex.</i>\"\n\n", false);
    ...
    outputText(" keep you immobilized for the domineering harpy.  Sophie snickers and poises her talons over your " + cockDescript(game.player, 0) + " while she asks, \"<i>Would you like to be a little girl?  I could do that.</i>\"  Her talons snap menacingly, but before you can do anything, she opens her claw-like foot and presses the soft underside against your member.  While you'd like to resist her, your " + cockDescript(game.player, 0) + " will have no such thing, and hardens nicely from the supple underskin of her foot-claw.\n\n", false);
    ...
    outputText("A tickle of moist body-heat wafts over your " + cockDescript(game.player, 0) + " and it twitches again.  You cry out in anguish and ecstasy, and the tiny prick twitches for a moment before unloading a blast of sticky, white goop onto your belly.  Sophie drops the rest of the way down, her huge cunt noisily slurping up your " + cockDescript(game.player, 0) + " like a bird devouring a worm a fraction of its size.  ", false);
...
function normalLossRapuuuuSophie(): void {
    ...
    outputText("  Sophie's hips pull you in more tightly as you spend the last of your white goo within her depths.   She murmurs, \"<i>Not bad for your first shot.</i>\"  Her voice shifts and buzzes as she suggests, \"<i>You're getting even harder now, little " + mf(player, "boy", "girl") + ".  Mhmmm, you'll cum again soon.</i>\"  The words burrow inside you and seem to squeeze more blood into your " + cockDescript(game.player, x) + ".  It gets so hard it's painful, and the tightness of her walls is almost too much to bear.  The bird-woman's face gives a tender, but knowing smile when you cum the second time, and the third time, and the fourth...", false);
    player.orgasm();
    dynStats("sen", 5);
    sophieFucked();
    luststickApplication(8);
...
function tooBigForOwnGoodSophieLossRape(): void {
    ...
    outputText("Hours later, you wake with an agonizing headache.   You sit up and fresh stabs of pain rock your world, nearly dropping you flat on your back again.  That harpy... she left you at the bottom of the mountain, and didn't even bother to cover your crotch back up!  Your " + cockDescript(game.player, x) + " is trembling, hard, and dripping pre-cum.  Its entire surface is covered in yellowish lip-prints, and judging from how tired you feel, Sophie found a way to make you cum numerous times.  With that many kisses covering you, you'll be hard for hours.  You get on your " + feet(player) + " and head back to camp.", false);
    player.orgasm();
    dynStats("sen", 5);
    sophieFucked();
