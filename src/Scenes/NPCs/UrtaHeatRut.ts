...
// [Approach Sober Urta]
export function approachSoberUrtaHeatRutProc(): void {
    clearOutput();
    Urta.urtaSprite();
    ...
    if (player.inHeat && player.inRut && player.cocks.length > 0 && player.vaginas.length > 0) outputText(" Males, females and herms alike stare at you with a mixture of puzzlement, longing and hostility, as if unsure whether they want to fight with you, fuck you senseless or be fucked senseless by you.");
    else if (player.inRut && player.cocks.length > 0) outputText(" Males stare at you with a hint of challenge in their eyes, whilst females eye you attentively. Herms don't seem sure whether they want to fight you or fuck you.");
    // (Player in heat
    else if (player.inHeat && player.vaginas.length > 0) outputText(" Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
    outputText("  You ignore them all, concentrating on Urta.");

    outputText("\n\nAs you approach, ");

    if (Urta.urtaLove()) outputText(" she opens her arms to embrace you, and smiles.");
    ...
    if (player.inRut && player.cocks.length > 0) outputText("  Urta is right next to you, and she has already proven herself a good lay");
    ...
    if (player.inHeat && player.vaginas.length > 0) outputText("  You... smell good, so I at least hope you're not here to tease me.");
    outputText("</i>\"  She leans closer to you, and ");
    if (player.inRut && player.cocks.length > 0) outputText("your eyes are quickly drawn to her full breasts as she whispers something to you. You're too hypnotized to care.");
    ...
    if (Urta.urtaLove()) outputText("your adorable, hot vixen");
    ...
    if (player.inRut && player.cocks.length > 0) outputText(" focus is on the hips of the fox-morph");
    ...
    if (player.inRut && player.inHeat && player.cocks.length > 0 && player.vaginas.length > 0) outputText(" put a baby in her, then let her put a baby in you");
    else if (player.inRut && player.cocks.length > 0) outputText(" put a baby in her so she can push it out of those beautiful hips");
    ...
    if (Urta.urtaLove()) outputText("  I'm really thankful you managed to control yourself and see me with this.  You know that if you need me, I'm always ready to help, love.</i>\"");
    ...
    if (player.inRut && player.inHeat && player.cocks.length > 0 && player.vaginas.length > 0) outputText("mixed genitals");
    else if (player.inRut && player.cocks.length > 0) outputText("[cock biggest]");
    ...
    if (player.inRut && player.inHeat && player.cocks.length > 0 && player.vaginas.length > 0) {
        ...
        // [Cock] [Pussy])
        menu();
        if (player.cocks.cockThatFits(Urta.urtaCapacity()) >= 0) addButton(0, "Cock", sateRutWithSoberUrta);
        else addButton(0, "Cock", sateRutWithSoberUrtaButHuegDicked);
        addButton(1, "Pussy", soberUrtaSatingPCHeat);
    }
    // Player in rut:
    else if (player.inRut && player.cocks.length > 0) {
        ...
        if (player.vaginas.length > 0) outputText("  Your " + multiCockDescriptLight(game.player) + " and [vagina] are filling your clothes with mixed juices");
        ...
        if (player.vaginas.length > 0) outputText(" and your pussy dripping and eager");
        outputText(".");
        menu();
        if (player.cocks.cockThatFits(Urta.urtaCapacity()) >= 0) addButton(0, "Next", sateRutWithSoberUrta);
...
export function approachDrunkenUrta(): void {
    clearOutput();
    Urta.urtaSprite();
    outputText("As you walk over to the table where your foxy herm-lover sits, you think you see eyes following you, your presence attracting an unusual amount of attention.");
    // Player in heat and rut:
...
function notInterestedInUburDrunkUrtaRuts(): void {
    ...
    if (player.inRut && player.cocks.length > 0) outputText("[EachCock] twitches in protest at you leaving a willing lay behind.  ");
    // [Medium lust increase]

    // [If the PC has a snake tongue]
    if (player.tongueType == TongueType.SNAKE) outputText("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
    // Rut:
    if (player.inRut && player.cocks.length > 0) outputText("Your " + multiCockDescriptLight(game.player) + " hardens in your pants as you still force yourself to leave.");
    // [Medium lust increase]
...
function interestedInUburDrunkurtaRuts(): void {
    ...
    if (player.inRut && player.inHeat && player.cocks.length > 0 && player.vaginas.length > 0) {
        ...
        if (!Urta.urtaLove()) outputText("under her skirt");
...
function drunkUrtaIntroPartDuex(chosenSex: number = 1, newl: boolean = true): void {
    if (newl) clearOutput();
    else outputText("\n\n");
    Urta.urtaSprite();
    ...
    // Player in both :
    if (player.inRut && player.inHeat && player.cocks.length > 0 && player.vaginas.length > 0) outputText("and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.");
    ...
    if (chosenSex == 1) outputText(multiCockDescriptLight(game.player));
    else outputText("[vagina]");
    ...
    menu();
    if (chosenSex == 1) {
        if (player.cocks.cockThatFits(Urta.urtaCapacity()) >= 0) addButton(0, "Next", drunkUrtaRidesARutPCsCock);
...
function watchDrunkRuturtaJerkoff(): void {
    ...
    if (!Urta.urtaLove()) outputText("pulls off her skirt");
...
function keepTeasingDatHornyFawkes(): void {
    ...
    outputText("slowly beginning to stroke and caress your " + multiCockDescriptLight(game.player) + " for the benefit of those who bear vaginas.");
...
function soberUrtaSatingPCHeat(): void {
    ...
    if (player.cocks.length > 0) {
        outputText(" by shoving your " + multiCockDescriptLight(game.player));
        if (player.cocks.length > 0 && player.balls > 0) outputText(" and [balls]");
    ...
    if (player.cocks.length > 0) outputText("[cock biggest]");
    else {
        outputText("clitoris, ");
    ...
    if (player.cocks.length > 0) outputText(" Cum rushes out of [eachCock] and onto the soft fur of Urta's abdomen, then starts leaking out slowly in time with her thrusts.");
    outputText(" Your cunt clamps down tighter onto Urta as you orgasm,");
    if (player.vaginas.wetness() >= 4) outputText(" pussy juice shooting out around her member, coating Urta's fur and soaking her balls and the base of her cock.");
    ...
    if (player.vaginas.wetness() >= 4) outputText("shooting out all over the place");
    ...
    if (player.cocks.length > 0) outputText("your own member responding alike to Urta's, shooting out semen, ");
    outputText("your [vagina] ");
    if (player.vaginas.wetness() >= 4) outputText("once again setting off to coat Urta in your juices.");
    ...
    outputText("\n\n\"<i>What?  ");
    if (!Urta.urtaLove()) outputText("You're still not satisfied?</i>\"");
    ...
    // [Hermaphrodite]
    if (player.cocks.length > 0) outputText("[eachCock] shoots its seed onto the floor of the alley, helplessly");
    ...
    outputText("\n\n\"<i>That felt amazing!  ");
    if (!Urta.urtaLove()) outputText("If only I had you by hand every time I need to blow a load.");
    ...
    outputText("\n\n\"<i>");
    // ([Regular Urta]
    if (!Urta.urtaLove()) outputText("Sure thing, I'm here to help,</i>\" she responds with a wink.");
...
function sateRutWithSoberUrtaButHuegDicked(): void {
    ...
    outputText("Urta observes your approaching " + cockDescript(game.player, x) + " with a hungry, eager expression. Without taking her eyes off of it, she flips up her ");
    if (!Urta.urtaLove()) outputText("skirt");
    ...
    outputText("\n\nYou stare at your " + cockDescript(game.player, x) + " in an uncomprehending fashion.  How are you going to find relief now?  You look up as Urta saunters over towards you in a seductive manner, erect cock jutting before her and a mischievous grin on her face.  \"<i>While I'll have to go and attend to this myself later, I can still give you a hand...</i>\" she purrs throatily.  She kneels before you, and you have to watch yourself to avoid having her twenty inches of cock slap your [leg].  Confidently, she reaches out to take hold of your " + cockDescript(game.player, x) + ", first with one hand, and then the other.");
    ...
    // Lover Urta Path/
        ...
        outputText("\n\nShe pulls her head free and gasps for breath, looking at you in admiration. \"<i>By Marae... how do you manage to do this for me, lover?</i>\" she asks, stunned.  But then, gamely, she returns her attention to your " + cockDescript(game.player, x) + ", licking, suckling and mouthing at the head.  As she does this, she continues her restless assault with her hands upon the rest of your lengthy shaft with her skilled hands.  She shuffles closer, allowing you to rest yourself between her DD-cups.  \"<i>Can you feel how fast my heart is beating through your cock?  You turn me on so very much,</i>\" she purrs.");
...
function sateRutWithSoberUrta(): void {
    ...
    outputText("\n\nWell, you don't intend to keep the lady waiting any longer then you have to, and you slide it home, allowing her testicles to drop once you are safely inside.  The globular organs bounce atop your " + cockDescript(game.player, x) + " and rock back and forth against your belly, jiggling with every movement the two of you make, Urta's cock sandwiched firmly between your bodies and held between her breasts.");

    outputText("\n\nUrta finally breaks the kiss and leers at you.  \"<i>Kinky " + mf(player, "boy", "girl") + "...</i>\" she teases you, \"<i>but I like that.</i>\"  She promptly starts to rock back and forth, seeking to simultaneously impale herself repeatedly upon your shaft and to grind her own huge cock between your bodies.");
    if (Urta.urtaLove()) outputText("  \"<i>You're the only one who has ever accepted every part of me,</i>\" she tells you, one hand brushing affectionately against your cheek.");

    outputText("\n\nYou simply moan back, your lust-addled mind only able to concentrate on the fact you are surrounded by her womanhood. You thrust back and forth, your " + cockDescript(game.player, x) + " surging deep inside her and then wetly pulling out, only to slide back home.  You nip and kiss her prominent nipples, making the vixen churr in delight, and she kisses you back.  Her front is plastered with pre now, her cockhead bubbling fluids like a fountain, and you can't resist bending down and playing with her cockhead.  She yips and growls with pleasure as you molest her hermhood, your tongue playing around the tip, lapping the sweet pre as it flows forth.  She surprises you by craning down her neck and starting to lap at it herself, her long, broad tongue curling expertly around its wide-flared tip.  The two of you make a game out of it, seeing who can better lick and lap, occasionally running your tongues over each other's whilst doing so.");
    ...
    if (Urta.urtaLove()) outputText("blows you a kiss with a flirty wink of her eye and then ");
    ...
    if (Urta.urtaLove()) outputText("  Her hips deliberately sway in a mesmerizing fashion, tail flicking from side to side to give you a tantalizing grip of her still-dripping snatch and the now-flaccid prick underneath.");
    ...
    if (urtaQuestDone()) {
        ...
        if (!Urta.urtaLove()) outputText("\"<i>And you had better learn to control yourself, " + mf(player, "mister", "missy") + ".");
        else outputText("\"<i>But you should learn to control yourself lover-" + mf(player, "boy", "girl") + ".");
...
function drunkenUrtaFucksPCInHeat(): void {
    ...
    if (player.cocks.length > 0) {
        outputText(", impatiently nudging aside your " + multiCockDescriptLight(game.player));
    ...
    if (player.vaginas.wetness() >= 4) outputText("As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta's face, making her pull herself free with a shocked shake of her head.");
    ...
    if (player.cocks.length > 0) outputText("nor even your " + multiCockDescriptLight(game.player));
...
function sateRutWithDrunkUrtaWithHugeDick(): void {
    ...
    else if (player.vaginas.length > 0) outputText("against your [clit]");
    ...
    if (player.cocks.biggestCockArea() < 150) outputText("compete with minotaurs or something?");
...
function drunkUrtaRidesARutPCsCock(): void {
    ...
    outputText("\n\nUrta lifts her balls and presses your already hard cock against her black pussy lips, justifying your decision.  Slowly, you feel your tip sink into her, then the rest of your member, as she releases a soft moan.  She keeps pressing down, forcing your " + cockDescript(game.player, x) + " inside her wet gash until you feel her butt pressed against your ");
    if (player.balls > 0) outputText("[balls]");
    else outputText("[hips]");
    outputText(", and her own balls resting comfortably on your abdomen.  She grits her teeth as you feel her pussy slowly tighten around and massage your " + cockDescript(game.player, x) + ", and you groan in response.  Urta starts to bounce up and down on your member, smacking her thighs and ass into your body, not minding her own giant equine member flopping around and occasionally smacking you over your chest.  She lets her tongue flick out into the air for a moment and moans in delight, before picking up the pace and starting to pump your cock in and out of her hungry passage furiously.");
    ...
    else outputText("recognize the tingling sensation from your " + cockDescript(game.player, x) + " to your spine");
    ...
    if (player.cumQ() >= 500 && player.cumQ() <= 1500) outputText("\n\nUrta seems positively full and stretched now, and your rut almost subsides for a moment, her stretched tummy imitating a pregnancy quite well.");
        ...
        outputText("\n\nUrta is stretched and completely full, so, holding your " + cockDescript(game.player, x) + " in her hand for a moment, she lifts herself off your cock, presses on her abdomen and allows some of your spunk to drip out of her, coating the table, your thighs and your ");
        if (player.balls > 0) outputText("[balls]");
        else if (player.vaginas.length > 0 && player.clitLength >= 3.5) outputText("cock-like clit");
        else if (player.vaginas.length > 0) outputText("pussy lips");
    ...
    outputText("\n\nYour " + cockDescript(game.player, x) + " feels sensitive and tender after the two past orgasms, but the horny nympho just forces it deep into her snatch, the sensitive head and shaft all the way down to your base again squeezed tighter as she starts to bounce up and down, trying to breathe life into your erection.  She kisses you furiously and passionately, exploring your mouth with her tongue as she keeps the stimulation on your groin, cooing, groaning and panting as if trying to breathe her own lust into you.");

    outputText("\n\n\"<i>Come on, come on, whatever it takes!  Squeeze those boobies, smack that ass, just get fucking hard!</i>\"");

    outputText("\n\nFollowing her plea, you give her breasts a rather rough squeeze, and you feel your " + cockDescript(game.player, x) + " miraculously harden inside her once again.");
    ...
    if (!Urta.urtaLove()) outputText("intense, unadultered lust");
    else outputText("possessiveness and a hungry, but somehow warm expression");
    outputText(".");

    outputText("\n\nI should make you do this every day!</i>\"");
    if (Urta.urtaLove()) outputText("  She suddenly seems a bit shy and surprised, before saying, \"<i>Make sure to remind me I said that later....</i>\"");
    ...
    if (Urta.urtaLove()) outputText("  You can rest later!  I'll even give you a massage, so keep giving it to me!");
    outputText("</i>\"");
    ...
    outputText(".");
    if (!Urta.urtaLove()) outputText("  She grins at your lack of resistance.");
    else outputText("  \"<i>You're mine, right?  Right now, you're mine and mine alone.  My wonderful virile breeder, all " + mf(player, "his", "her") + " seed just for me.</i>\"");
    ...
    outputText(".  You seem to slowly lose the sensations in most of your body.  The awareness of your surroundings slips away, as all your thoughts and feelings focus in your groin, struggling to keep up with the mad pace of breeding that Urta has kept so far.  It's hard to keep conscious as you feel another surge of ecstasy pass all over your body and focus on your " + cockDescript(game.player, x) + ", shooting off yet again inside her coaxing hole.  You hear the sound of your name being called and the milking, squeezing sensations that you barely manage to associate with her ravenous, insatiable pussy.");
    ...
    if (!urtaQuestDone()) {
        ...
        if (Urta.urtaLove()) outputText("  If only I could... uh, I guess I should just enjoy what I can.");
        outputText("</i>\"");
    }
    // [If Urta is fertile]
    else {
    ...
    outputText("\n\nShe ");
    if (!Urta.urtaLove()) outputText("winks at you as you get ready to leave.  I hope I'll get to play with you again soon.</i>\"");
    else {
        ...
        outputText("\n\nYou " + mf(player, "chuckle", "giggle") + " and she punches your shoulder playfully, but keeps her face from blushing more.  She seems pleased that she brought herself to that declaration.");
    }
    // [Both end here]
    outputText("\n\nSatisfied even if your body hurts all over and your " + cockDescript(game.player, x) + " seems empty, dry, burning, and almost ruptured.  You leave the house with a small smile on your face, and a strong stench of sexual fluids following you back to the camp.");
