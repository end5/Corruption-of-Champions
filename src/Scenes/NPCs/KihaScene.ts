...
function kihaRapesMen(): void {
    ...
    if (player.isNaga() || player.tailType == TailType.LIZARD) outputText("\"<i>Heh, if you had half as much dexterity as you do impudence, I'd let you do this for me.  Fat chance of that,</i>\" she remarks off-handedly.  ", false);
    ...
    if (player.balls > 0) outputText(", resting the heel of her foot against the swell of your " + ballsDescriptLight(player) + " and applying just enough pressure to feel the cum churning within", false);
    ...
    outputText("The gushing honey pooled and slathered along her tail and asscheeks while she was frigging herself gives you more than enough lubrication to part the dragon's silken passage, muscled sphincter clenching rhythmically, more to pull you deeper inside than in response to the sudden intrusion.  You cry out at the intense tightness sucking at your " + cockDescript(game.player, x) + " and reach up to grab the girl's breasts, but she smacks your hands away with a sharp blow.  \"<i>If you dirty my scales with your filthy mitts, I'll crush you between my thighs!</i>\" she gasps, running her own hands through her hair as her head rolls back in delight.  She rides you hard, bouncing off of your lap without regard for the bruising her athletic ass is giving your " + hipDescription(player) + ", but you can hardly feel anything beyond the vice-like tightness of her rectum.  You open your mouth to beg her to ease up, but she uses the opportunity to thrust her tail between your parted lips, the warm appendage still dripping with girl-cum.  You nearly gag at the insertion, but the shock is quickly subordinated to arousal as your mouth is flooded with the second-hand taste of her sweaty nectar. You reflexively begin sucking on the scaled tip, wrapping your tongue around the scaly invader and even rolling your jaw as you lap Kiha's cream from her posterior.\n\n", false);
    ...
    // [Lizard/naga tail:
    if (player.isNaga() || player.tailType == TailType.LIZARD) outputText(" while your tail curls around to find your own asshole.  The grip of the dragon's nethers so intense, you barely feel the pressure of your own tail, pushing in easily to soothingly stroke your overtaxed prostate", false);
    ...
    outputText("When you come to, your throat feels raw, the taste of her pussy still on your lips.  Surprisingly, your groin is wet, but not sticky, without a trace of semen anywhere.  She must've licked " + sMultiCockDesc(game.player) + " clean after you passed out.  Your " + hipDescription(player) + " are still sore from the wild woman's ride, so it takes you longer than usual to limp your way back to camp.", false);
    player.orgasm();
    cleanupAfterCombat();
}
// *Herm - Adj
function kihaRapesHerms(): void {
    ...
    outputText(", Kiha repositions herself so that her ashen-hued vulva presses moistly against your " + vaginaDescript(player, 0) + ", the plush lips of her nethers lapping at yours in a torrid embrace.  Rocking against you in an angular motion, the dragon's sweltering sex grinds against your own sensitive flesh, small shocks of overstimulation arcing through your hips as her stiff, pink pearl brushes against your " + clitDescription(player) + ".  You mirror her motions, rubbing into the girl with increasing urgency as your flesh engorges, nearly forgetting the throbbing imperative of your " + multiCockDescriptLight(game.player) + ".\n\n", false);

    outputText("You're so lost in the motion of your sinuous bodies gyrating against one another that when a stiffness slides between your cunnies, rather than thinking to look down, you merely writhe against it with unquestioning ardor.  When nubby ridges begin to caress your clenching womanhood, however, you draw a sharp intake of breath and blink back the lusty haze from your eyes.  Kiha's wriggling tail has slid between the breach of her ass and has slipped up, between the two of you, textured hardness rubbing both snatches with drool-oiled scales.  Gradually pumping her tail between the engulfing cunts, the dragoness throws her head back in eye-clenching pleasure.  \"<i>Oh, I'm so much better at this than you are!</i>\" she moans, taking care to flick the ridges of her plates against her sweet, hooded nub, massaging your " + clitDescription(player) + " almost as an afterthought.  \"<i>You're fucking lucky I took pity on a loser like you,</i>\" she exhales, her voice stifled by the rapid approach of her orgasm.\n\n", false);

    let dicked: boolean = false;

    // [If the player has an incubus draft:
    if (player.inv.hasItem(ConsumableLib.INCUBID)) {
...
function victoryDickKiha(): void {
    ...
    if (y != -1 && player.cocks.length > 1) {
        ...
        if (player.isNaga() || player.tailType == TailType.LIZARD || z >= 0) {
...
function rapeKihaWithWORDS(): void {
    ...
    // Conversation Level 2 (40- Corruption! PUREBABIES ONLY!) (Zed)
        ...
        if (silly() && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00281] > 0 && Lottie.lottieMorale() <= 33) outputText("  Is that what it feels like to be Lottie?", false);
    }
...
function analRapuzulaKiha(): void {
    ...
    if (player.balls > 0) outputText(" and threatening to crush your " + ballsDescriptLight(player), false);
