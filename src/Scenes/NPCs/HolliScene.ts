...
function fightHolli(): void {
    flags[kFLAGS.FOUGHT_HOLLI] = 1;
    startCombat(new Holli());
}

export function treeMenu(output: boolean = true): void {
    if (output) clearOutput();
    let ride: () => void = null;
    let fuck: () => void = null;
    let burnIt: () => void = null;
    if (flags[kFLAGS.FOUGHT_HOLLI] > 0) {
        ...
        if (player.perks.findByType(PerkLib.Dragonfire) >= 0 || player.perks.findByType(PerkLib.FireLord) >= 0 || player.perks.findByType(PerkLib.Hellfire) >= 0 || player.effects.findByType(StatusAffects.KnowsWhitefire) >= 0) {
...
function noPlzDontFuckWithFarm(): void {
    clearOutput();
...
function fertilizeHolli(cock: boolean = true): void {
    ...
    if (!cock && player.vaginas.length > 0) {
        ...
        if (player.perks.findByType(PerkLib.FerasBoonBreedingBitch) >= 0) odds += 10;
        if (player.perks.findByType(PerkLib.ElvenBounty) >= 0) odds += 2;
    }
    if (odds >= rand(101)) {
...
// Sprout Phase: (edited)
export function getASprout(): void {
    clearOutput();
...
export function fuckPlantGrowsToLevel2(): void {
    clearOutput();
...
export function flowerGrowsToP3(): void {
    clearOutput();
...
function torchP3Tree(): void {
    ...
    // [(nerdfire)
    if (player.effects.findByType(StatusAffects.KnowsWhitefire) >= 0) outputText("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
    // (bromancefire)
    else if (player.perks.findByType(PerkLib.FireLord) >= 0) outputText("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
    ...
    if (player.effects.findByType(StatusAffects.KnowsWhitefire) >= 0) outputText("magical will");
...
// Phase Four (edited)
export function treePhaseFourGo(): void {
    clearOutput();
...
// ojo Rolls Out -Z
export function JojoTransformAndRollOut(): void {
    clearOutput();
...
export function amilyHatesTreeFucking(): void {
    clearOutput();
...
// [Slapaho]
function slapAmilysWhoreFace(): void {
    clearOutput();
...
// (FUCK DAT TREE BITCH)
function fuckHolliInZeFlowerPuss(): void {
    clearOutput();
    flags[kFLAGS.HOLLI_FUCKED_TODAY] = 1;
    const x: number = player.cocks.biggestCockIndex();
...
function level4RideHollisTentacruels(): void {
    ...
    outputText("\n\nYou fold your arms impatiently across your [chest] and nod, waiting for her to get her phallic tendrils in position.  It takes you completely off guard when a half-dozen tentacles snap across your front and another bundle sweeps your [legs] out, spinning you to look face down at the ground, supported horizontally on a bed of taut, straining tentacles.  More loop over you, under you, and around you, cinching tight to your wrists and [feet].  They pull them out while other prehensile plants swing down to support you, occasionally looping around you, their moist skin easily gliding over your " + skinFurScales(player) + ".  All that, and she's not even fucking you yet!");
...
function begHolli4Watches(): void {
    clearOutput();
...
function domUpSomeHolli(): void {
    ...
    let domPowah: number = player.level;
...
function holliGetsDickDommed(): void {
    clearOutput();
    outputText("Tossing your [armor] aside, you expose your " + multiCockDescriptLight(game.player) + ".  Holli's eyes fixate on [oneCock], her attention rapturously contained by a single phallus.  You grip your maleness in hand and bounce it up and down into your other palm.  The solid slaps of genital impacts echo off the surrounding rocks, each sending a pleasant tingle of pleasure through the root of your manhood.  Before long, you're completely erect and throbbingly hard, bouncing unbidden with each and every thrum of your heart.");
...
function threatenHolli(): void {
    ...
    if ((player.gender > 0 && player.effects.findByType(StatusAffects.JojoNightWatch) >= 0 && player.effects.findByType(StatusAffects.PureCampJojo) >= 0) || flags[kFLAGS.ANEMONE_WATCH] > 0) {
...
function callDatJojo(): void {
    ...
    flags[kFLAGS.FOUGHT_HOLLI] = 1;
    startCombat(new Holli());
    monster.effects.create(StatusAffects.JojoIsAssisting, 0, 0, 0, 0);
}

export function defeatHolli(): void {
    clearOutput();
    flags[kFLAGS.FUCK_FLOWER_KILLED] = 1;
    // Win without Jojo -Z
    if (monster.effects.findByType(StatusAffects.JojoIsAssisting) < 0) {
...
// [yes gimme monk pls]
function recruitJojoToCamp(): void {
    clearOutput();
...
// [no want]
function dontRecruitJojoToCamp(): void {
    clearOutput();
...
// >Lose
export function enjoyYourBadEndBIYAAAATCH(): void {
    clearOutput();
    ...
    if (player.gender == 1 || (rand(2) == 0 && player.cocks.length > 0)) {
        ...
        else outputText("exploring the depths of your body and curling back against the surface of your " + skin(player) + ".  ");
...
function holliPlaysWithPenisesBadEnd(): void {
    clearOutput();
    ...
    outputText("\n\nBy the time the girl reaches her third orgasm, her partners are tense and near the breaking point; with an shudder and a stilling of the shaking branches, the silent one opens " + mf(player, "his", "her") + " mouth in mute ecstasy and ejaculates, ");
    if (player.cocks.length > 1) outputText("spraying the girl's stomach and tits with waste seed from " + mf(player, "his", "her") + " unused manhood and ");
...
function girlsGetANiceManToBadEnd(): void {
    clearOutput();
    ...
    else if (player.balls > 0 && player.ballSize <= 10 && player.gender == 3) outputText(multiCockDescriptLight(game.player) + " and [balls] that hang slightly over a [vagina].");
    // (if H and noballs)
    else if (player.gender == 3) outputText(multiCockDescriptLight(game.player) + " and a [vagina].");
...
function holliAndGenderlessSittingInATree(): void {
    clearOutput();
    ...
    if (player.skinType != SkinType.FUR && player.skinType != SkinType.SCALES) outputText(player.skinTone + "-colored");
    else outputText("covered in " + skinFurScales(player));
    ...
    if (player.skinType == SkinType.SCALES) outputText("a quiver of arousal shakes its scales");
    else if (player.skinType == SkinType.FUR) outputText("its fur begins to prick up with arousal");
    else outputText("a blush of lust colors its " + skinFurScales(player));
    outputText("...");
...
export function amilyComesBack(): void {
    clearOutput();
