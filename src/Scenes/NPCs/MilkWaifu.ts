...
export function milkSlave(): boolean {
    return flags[kFLAGS.MILK_NAME] instanceof String;
}

// Arriving at Camp
export function arriveWithLacticWaifuAtCamp(): void {
    clearOutput();
...
function nameZeMilkBath(): void {
    ...
    else if (mainView.nameBox.text == "" || mainView.nameBox.text instanceof Number) {
        clearOutput();
...
export function milkyMenu(): void {
    clearOutput();
...
// Appearance
function milkWriteFuAppearance(): void {
    clearOutput();
...
// Reducto
function superReductoUsage(): void {
    clearOutput();
    ...
    if (flags[kFLAGS.MILK_SIZE] == 0) {
        ...
        else if (player.effects.findByType(StatusAffects.CampMarble) >= 0) outputText(", enough to give Marble pause");
...
function milkBathTime(): void {
    ...
    else if (player.gender == 3) outputText(multiCockDescriptLight(game.player) + " and [vagina]");
...
function relaxWithMilkWaifu(): void {
    clearOutput();
    outputText("You sit in the tub for a while, letting the fragrant fluids soak into your " + skinFurScales(player) + ".  You spend the better part of an hour lounging, letting your cares float away in " + flags[kFLAGS.MILK_NAME] + "'s endless white bounty.  Finally though, you pull out the tub's plug and climb out, finding a towel nearby.  Thankfully, the milk doesn't seem to leave behind any residue, and you feel clean and refreshed, if a bit horny.");
    // (+Lust, -Fatigue)
...
function communalBath(): void {
    ...
    outputText("\n\nWithout prompting, " + flags[kFLAGS.MILK_NAME] + " shifts around to rub a little milk into your skin, gently bathing you.  Her dark fingers run through your hair, washing it thoroughly before moving down to your arms, [chest], and legs, cleansing every inch of your " + skinFurScales(player) + ".  You relax to " + flags[kFLAGS.MILK_NAME] + "'s touch, letting her massage the cares out of your weary muscles.  Around you, your followers begin to do much the same, taking turns bathing each other, scrubbing backs and extremities with the soothing milk of " + flags[kFLAGS.MILK_NAME] + "'s bounty.");
...
// [Milk Girl]
function pullInZeMilkGirl(): void {
    clearOutput();
...
// 	[Don't]
function dontFuckTheMilkWaifu(): void {
    clearOutput();
...
function fuckTheMilkWaifu(): void {
    clearOutput();
...
function beARugMunchingMilkDyke(): void {
    clearOutput();
...
// [Drink & Masturbate]
function drinkAndFapturbate(): void {
    clearOutput();
...
// [Finish Normally]
function finishMilkNormall(): void {
    clearOutput();
...
function titFuckDatMilkSlut(): void {
    clearOutput();
