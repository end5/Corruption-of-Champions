...
export function urtaPregAppearance(): void {
...
// In Urta Menu
export function urtaPreggoApproached(): void {
    clearOutput();
    menu();
    switch (Urta.pregnancy.event) {
...
// ☼☼
// Hospital
export function preggoUrtaGivingBirth(): void {
    clearOutput();
...
// [=Refuse=]
function IAintGotTimeForUrtaBirfs(): void {
    clearOutput();
...
export function getUrtaLetter(): void {
...
// [=Go=]
function goVisitUrtaBirfs(withUrta: boolean = false): void {
    clearOutput();
    Urta.pregnancy.knockUpForce(); // Clear Pregnancy
    flags[kFLAGS.URTA_PREGNANT_DELIVERY_SCENE] = 0;
    flags[kFLAGS.NEW_BABY_ANNOUNCED] = 0;
...
// =Yes=]
function stayAfterUrtaBirf(): void {
    clearOutput();
...
// [=Leave=]
function noNeedForTitMilkTodayUrta(): void {
    clearOutput();
...
// [=Drink=]
function drinkSomeUrtaPostBirthTitMilk(): void {
    clearOutput();
...
export function urtaPregooUpdates(): boolean {
...
// [=No=]
function noHelpForUrtaPreggoPC(): void {
    clearOutput();
...
// [=Yes=]
function getUrtaHelpPreggoPC(): void {
    clearOutput();
...
// [=Sex=]
function getUrtaSexWhenPreggoz(): void {
    clearOutput();
...
// [=Massage=]
function getAnUrtaMassageWhenPreggo(): void {
    clearOutput();
...
// [=Decline=]
function declineUrtaLevelSixPreggoHelps(): void {
    clearOutput();
...
// [=Accept=]
function acceptUrtaLevelSixPreggoHelps(): void {
    clearOutput();
...
// PC Gives Birth
export function PCGivesBirf(): void {
    outputText("\n");
    flags[kFLAGS.URTA_PREGNANT_DELIVERY_SCENE] = 0;
    flags[kFLAGS.NEW_BABY_ANNOUNCED] = 0;
...
export function urtaIsHappyAboutPregnancyAtTheBar(): void {
    clearOutput();
...
export function urtaIsAPregnantCopScene(): void {
    clearOutput();
    ...
    if (Edryn.pregnancy.isPregnant) outputText("  Edryn's pregnant belly isn't helping her, either.");
...
export function urtaSpecialDeliveries(): void {
    clearOutput();

    outputText("As you enter through the gates, Edryn clops up to you, a small basket swinging from the crook of one arm");
    if (Edryn.pregnancy.isPregnant) outputText(" and her pregnant belly jiggling from the motions");
    ...
    if (Edryn.pregnancy.isPregnant) outputText("as quickly as a pregnant mare can, anyway,");
...
export function talkToUrtaAboutHerHouse(): void {
    clearOutput();
...
// Pregnant Urta Sex
function preggoUrtaSmexOrSomething(): void {
    clearOutput();
...
// Belly Massage
function massagePregnantUrtasBelly(): void {
    clearOutput();
    ...
    if (player.tongueType == TongueType.SNAKE) outputText("forked ");
    ...
    if (player.tongueType > TongueType.HUMAN) outputText(", a task that is made far easier due to your tongue's ability to stretch to a surprising length");
...
function urtaMassageClimax(): void {
...
function urtaBoobWorshipScene(): void {
    clearOutput();
    ...
    if (player.faceType == FaceType.SHARK_TEETH || player.faceType == FaceType.SNAKE_FANGS || player.faceType == FaceType.SPIDER_FANGS) outputText(", always minding your sharp teeth");
    ...
    if (player.cocks.length > 0) outputText(", tasting a bit of yourself,");
...
// Nurse
function nurseFromMommaUrtasBooBees(): void {
    clearOutput();
    ...
    if (player.tongueType > TongueType.HUMAN) outputText("  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm's delight.");
...
// if Stroke:
function titWorshipAndStroke(): void {
    clearOutput();
...
// Milk
function milkPregnantUrta(): void {
    clearOutput();
    ...
    if (player.cocks.length > 0) outputText("Urta promptly stops stroking your dick, though whether it's because she wants to be helpful or because she's annoyed you've stopped is anyone's guess.  ");
...
// [=Stay=]
function stayForHorseDickings(): void {
    clearOutput();
...
function urtaPregCravings(): void {
    clearOutput();
    ...
    if (player.cocks.length > 2) outputText("The rest of your " + multiCockDescriptLight(game.player) + " are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
...
// [=Blow=]
function getBlownByPregnantUrta(): void {
    clearOutput();
    ...
    if (player.cocks.length > 1) {
        ...
        if (player.cocks.length > 2) outputText("s as they join");
...
// [=Hold On=]
function holdOnYouAintGettingYerCumYetYouNaughtyFox(): void {
    clearOutput();
    ...
    if (player.cocks.length > 1) {
        ...
        if (player.cocks.length > 2) outputText("s bulging ominously as they too prepare to deposit their loads on the willing fox-herm.");
    ...
    if (player.cumQ() < 250) {
        ...
        if (player.cocks.length > 1) {
            ...
            if (player.cocks.length == 2) outputText(" convulses");
    ...
    // (Medium Cum Amount)
        ...
        if (player.cocks.length > 1) {
            outputText("Your other cock");
            if (player.cocks.length == 2) outputText(" contributes");
            else outputText("s contribute");
            outputText(" by making a complete mess of her as ");
            if (player.cocks.length > 2) outputText("they randomly spatter");
    ...
    // (High Cum Amount)
        ...
        if (player.cocks.length > 1) {
            outputText("  Your other cock");
            if (player.cocks.length > 2) outputText("s shoot their loads");
        ...
        if (player.hasKnot(player.cocks.biggestCockIndex())) outputText(", just above your knot,");
...
function weirdUrtaCravingsConclusion(): void {
    clearOutput();
...
function urtaRaepsJoo(): void {
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length > 1) outputText("  [EachCock] tenses in unison");
...
function partTwoOfUrtaBoning(): void {
    clearOutput();
...
// [=Accept=]
function acceptUrtaRimJobbies(): void {
    clearOutput();
...
export function visitKidsFirstTime(): void {
    clearOutput();
...
function pickKidSceneHere(): void {
    clearOutput();
    // ANNOUNCE BEHBIES
    // Once per pregnancy?
...
// [=No=]
function noPlayTimeForKids(): void {
    clearOutput();
...
// [=No=]
function noBathTiemPlease(): void {
    clearOutput();
...
function urtaKidsText(male: string = "", female: string = "", plural: string = ""): string {
    if (urtaKids() == 1) {
...
// [=Yes=]
function giveTheKidsABath(): void {
    clearOutput();
...
// [=No=]
function noBathPleaseUrtaLian(): void {
    clearOutput();
...
// [=Yes=]
function getABackWashFromLianna(): void {
    clearOutput();
...
// Take the kids for a walk - Special thanks to Belin
// Randomly picked when selecting Kids option in Urta's house.
function takeTheKidsForWalk(): void {
    clearOutput();
    ...
    outputText(" along with your own, greedily licking up melting droplets.  There's just a bit of cone left as you return to Urta's house, and you quickly polish that off, upon entering.  \"<i>Oh, hey.  You're back.</i>\"  Lianna greets you in her usual inexpressive fashion.");
...
export function visitTheHouse(): void {
    clearOutput();
...
// Lianna
// Appearance
function liannasAppearance(): void {
    clearOutput();
...
function talkToLiana(output: boolean = true): void {
    if (output) {
        clearOutput();
...
// Her Job
function talkToLianaAboutHerJob(): void {
    clearOutput();
...
// Her Research
function talkToLianaAboutHerResearch(): void {
    clearOutput();
...
function talkAboutLiannasLubeSpray(): void {
    clearOutput();
...
function lubeSpraySideEffects(): void {
    clearOutput();
...
function sexWithLianna(): void {
    ...
    if (player.cocks.length > 1) outputText("  Your lower garments suddenly feel much tighter.");
    ...
    if (player.cocks.length > 0) addButton(1, "HarvestManly", liannaHandjobbies);
    addButton(4, "Back", visitTheHouse);
}

// [=Refuse=]
function refuseGenderlessInjection(): void {
    clearOutput();
...
// [=Accept=]
function acceptARandomGenderFromASkunk(): void {
    clearOutput();
...
// Dildo-syringe
// Needs vag.
function getEggsHarvested(): void {
    clearOutput();
...
// Handjob
// Needs cock.
function liannaHandjobbies(): void {
    clearOutput();
    ...
    if (player.cocks.length > 1) outputText("  \"<i>However I think milking just one is good enough.</i>\"");
