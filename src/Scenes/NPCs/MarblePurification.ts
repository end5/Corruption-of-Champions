...
export function giveMarblePureQuestLabova(): void {
    ...
    if (flags[kFLAGS.TIMES_GIVEN_MARBLE_PURE_LABOVA] == 0) {
        ...
        player.effects.addValue(StatusAffects.Marble, 4, -4);
        ...
        if (player.effects.getValue2Of(StatusAffects.MarbleSpecials) == 4) outputText("eight");
        else outputText("two");
    ...
    else if (player.effects.getValue4Of(StatusAffects.MarbleSpecials) <= 30) {
...
export function claraShowsUpInCampBECAUSESHESACUNT(): void {
    ...
    outputText("\n\nAt this point Rathazul suddenly interjects. \"<i>If " + player.short + " gets the milk by suckling on your breasts, " + mf(player, "he", "she") + " would probably contaminate it with " + mf(player, "his", "her") + " saliva and make it useless. I need it to be as pure as possible; hand-milking would be preferable, if you can do that, but if you must use the milkers at Whitney’s farm, that will suffice.</i>\"");
...
export function mediumIntelligenceDoesntDrinkCowCuntsTea(): void {
    ...
    outputText("\n\n\"<i>You really are a smart " + mf(player, "boy", "girl") + ",</i>\" the girl speaks, ignoring your question, \"<i>I shoould have known you wouldn’t be so easy to trick.</i>\"  You make your demand of her once more.  \"<i>Don’t worry about them, my darling.  They’ll be awake in an hour or so, but by that time we’ll be long goone.</i>\"");
...
export function cowCuntBadEndPartIII(): void {
    ...
    if (player.hairLength > 0) outputText("through your " + hairDescription(player));
...
export function cowCuntBadEndPartIV(): void {
    ...
    outputText("\n\nIt then becomes rather apparent exactly what Clara really wants to play with now, and you feel her fingers slip down and around your body.  She gets herself a handful of your " + buttDescription(player) + ", and then slips her fingers into your crevice.  That finger doesn't stop until it's pushed itself inside your " + assholeDescript(player) + " and triggered a response that makes Clara laugh with delight.  Feeling emboldened, the bovine girl thrusts another finger into your back entrance and starts wiggling them around.");
...
export function badEndDickThatFitsClara(): void {
    ...
    outputText("\n\nWell, she certainly isn’t wasting any time, as the very next instant she has lowered her opened labia onto the tip of your " + cockDescript(game.player, x) + ".  The passage is already wet and slick, thanks to her previous orgasm.  Immediately, an expression of both pain and pleasure crosses over the bovine woman’s face and she starts to pant.  You lift up your hands, bClara issues a sharp command that you aren’t to do anything at all while she has the sex she wants.  You can’t do much else but just lie back and take it.");
...
export function cocklessPCsGetAbusedByCowCunts(): void {
    ...
    if (player.tongueType > 0) {
        ...
        outputText("\n\n\"<i>Mhh, huh, ah, higher!</i>\"  You obey, and move up to Clara’s clit, and the hood around it.  She has already become heavily engorged, and the instant your " + tongueDescription(player) + " touches the bovine woman’s erect love button, she lets out a low shriek of pleasure!  \"<i>Woah!  Wah, mah, mooo!</i>\" she cries out as a torrent of fluid sprays from her womanhood and drenches your face.  Clara is unquestionably an inexperienced virgin, given her rapid and unexpected orgasm.  She spends several moments trying to catch her breath.");
        ...
        outputText("\n\nAbruptly, a large torrent of fluid comes rushing down the hole you’ve been pleasing.  Much to your dismay, the flood of fluid actually forces your " + tongueDescription(player) + " out of her passage before nearly drowning you in girl cum.  You cough and sputter for several seconds until air finally returns to your lungs.  What energy you had before now is now gone.");
        ...
        outputText("\n\nClara finally releases your head from her death grip, and lets you move around again.  \"<i>Wow, " + mf(player, "lover-boy", "lover-girl") + ".  I never imagined you’d be able to do that!  I guess the slave I got had an unexpected feature.  I’m still going to be changing you up to give me anoother kind of pleasure, but damn!  Look at what you made me doo!</i>\"  She indicates the large pool of vaginal fluids covering the floor.  The bedroll you woke up on is completely ruined.");
...
export function defeatClaraCuntInAFight(cheated: boolean = false): void {
    if (cheated) clearOutput();
        ...
        if (player.effects.getValue1Of(StatusAffects.ClaraCombatRounds) >= 10 && player.effects.findByType(StatusAffects.ClaraFoughtInCamp) >= 0) {
            clearOutput();
        ...
        else if (player.effects.findByType(StatusAffects.ClaraFoughtInCamp) < 0) {
    ...
    outputText("\n\n\"<i>Getting a mate, what doo you think?  I was going to take " + mf(player, "lover-boy", "lover-girl") + " here away to live with me,</i>\" comes the incredulous reply.");
    outputText("\n\n\"<i>Getting a... out of my sweetie?!  We're already mates, where do you go off thinking you can just come in here and steal " + mf(player, "him", "her") + "?</i>\"");
    outputText("\n\n\"<i>No sis, you had your chance.  " + player.short + " isn't addicted to you, that means that " + mf(player, "he", "she") + "'s up for grabs.  That's how things work for our race.</i>\"");
    ...
    outputText("\n\n\"<i>Admit it, you don't feel like '" + mf(player, "lover-boy", "lover-girl") + "' here actually cares for you.  How could they if they won't drink your milk?  You're a Lacta Bovine, giving your milk is the most important thing in the world to you.  This isn't love.  Without " + mf(player, "him", "her") + " actually being addicted to you, it's just a one sided infatuation.  I was doing you a favor by taking " + mf(player, "him", "her") + " away from you.</i>\"");
    ...
    outputText("\n\n\"<i>There, you see?  Now I'm sure you understand why me and " + player.short + " here belong toogether.  I won't deny the proper order of things, and I'll make sure that " + mf(player, "he", "she") + " leads a fulfilling life as my milk slave.  " + mf(player, "He", "She") + "'ll even by my first husband!</i>\"");
    ...
    outputText("\n\nThe younger cow-girl scoffs, \"<i>Haven't you been listening to me sis?  " + mf(player, "He", "She") + "'ll be addicted to me.  That.  Is.  Love.  How else can you show just how much you care?</i>\"");
    ...
    outputText("\n\n\"<i>Really?  Didn't you say that the reason you were purifying yourself was so that " + mf(player, "he", "she") + " would nurse from you again?  Sounds pretty selfish to me after what you were saying about doing things for who you 'love'.</i>\"");
    ...
    outputText("\n\n\"<i>Then if " + player.short + " loves you so much, why can't " + mf(player, "he", "she") + " change for your sake?  Why can't they become addicted to show just how much they love you?</i>\"");
    outputText("\n\nMarble swallows before saying, \"<i>Do you have any idea of what " + player.short + " is trying to do?  " + mf(player, "He", "She") + " is trying to defeat the demons!  To stop the corruption for good!</i>\"");
    outputText("\n\nAt this Clara bursts out laughing, \"<i>Oh, and save the world I take it?  This " + race(player) + "?  Don't be ridiculous.</i>\"");
...
// does not work with centaur, naga, drider, or goo bodies.  However, I don't particularly want to just say you can't do this scene if you have them.
{
    ...
    if ((player.cocks.length > 0 && x >= 0) && flags[kFLAGS.MARBLE_DICK_TYPE] > 0) {
        ...
        outputText("\n\nMarble looks to the side of her sister's head, and gives you a solemn nod.  You take that as an invitation to begin the violation and grab a good grip of Clara's expansive rear.  Then you prod your " + cockDescript(game.player, x) + " at the entrance to her anus, prompting a gasp and plea from the woman you're about to penetrate to stop.  Her sister doesn't heed her begging, and neither do you.");
    ...
    // else if (PC does not have a cock, Marble does)
        ...
        outputText("\n\nYou express your disappointment to Marble at her sister's unfinished performance, so the fucking continues.  Thankfully, it seems that the bad bovine did get the message and has started to tease and suck on your " + clitDescription(player) + " as well as continuing with her teases of your folds and depths.  You'd almost feel bad for doing what you two are doing to her if she wasn't evidently enjoying it as much as she is.  Muffled but loud gasps and moos are constantly being uttered into your " + vaginaDescript(player) + " by the bound girl between you and your lover.  Her breath upon your womanhood only serves to make you feel even better.");
    ...
    {
        outputText("\n\nEventually you roll her onto her back.  Then Marble sits down on her sister's face while you put your mouth to Clara's womanhood and raise your " + buttDescription(player) + " up into the air so that your mate can reach your own " + vaginaDescript(player) + ".");
...
export function purificationQuestFinal(): void {
    ...
    player.effects.setValue(StatusAffects.Marble, 4, 10);
    game.inCombat = false;
...
export function claraCampAddition(): void {
    ...
    if (flags[kFLAGS.CLARA_PURIFIED] > 0) {
        ...
        if (time.hours <= 7 || time.hours >= 20) outputText("Clara is laying sprawled out on the ground of her cage, snoring loudly.");
        else {
            temp = rand(4);
            switch (temp) {
                case 0:
