...
export function haveAKid(): void {
    ...
    outputText(" before coiling around the shaft with serpentine dexterity.  Her tail contracts and squeezes, undulating across your " + cockDescript(game.player, 0) + " until it finally massages a thick blob of pre from your " + cockHead(player) + ", dribbling out to stain the wasteland ground.");

    outputText("\n\nSuddenly, Helia grabs your shoulders and shoves you down.  You flop onto your back, tail-encased prick standing straight up like a flagpole as Hel straddles your [hips], vigorously fingering herself as her tail lines your cock up with her drooling womanhood.  Your breath catches as your " + cockHead(player) + " brushes her boiling juices, slickening your entry as she slides down your pole.  You grab Hel's flared hips, guiding her in as she envelopes your " + cockDescript(game.player, 0) + " in the roiling inferno inside her.  Her tail unravels from around your throbbing shaft as she descends, tantalizingly slowly; she takes the better part of a minute before her groin is cozily joined to yours, her hungry maw kissing the very base of your lusty prick.");
    ...
    if (player.breastRows.biggestTitSize() <= 2) outputText("supports her, pressing down on your [chest]");
...
function getAnotherDad(): void {
    ...
    if (player.effects.getValue1Of(StatusAffects.TelAdre) >= 1) addButton(0, "Mai", maiWouldBeTheBestInseminator);
    // [Spiderboy]
...
function dontTellMeAboutMai(): void {
    ...
    outputText("\n\n\"<i>Yeah, I know.  Forget about it, I'm fine.  Really.  It's just that this happened all so suddenly, you know?  This isn't exactly where I saw myself a year ago.  Not that I'm complaining,</i>\" she adds, turning back to you with a kiss, squeezing you with her tail.  \"<i>I just hope I'm ready, I guess.  Until the Pale Flame took, I'd never really thought about being a mother.  Never really wanted it.  But last night, it was like I was possessed, like I couldn't control myself.  I just... needed you.  Well, maybe not your sperm - not literally, I mean - but it was like my body screaming 'You've found " + mf(player, "him", "her") + ", " + mf(player, "he", "she") + "'s the one, this is a keeper!  Time to propagate!'  I-I can't help but feel like this wasn't entirely my choice, you know? Like I was acting on instinct, like an animal, not a person.  But at the same time, if I've made a mistake, then I have you here with me, to help me like you always do.  I love you with all my heart, [name]");
    // if HelLove is Yes:
    if (flags[kFLAGS.HEL_LOVE] == 1) outputText(", and I'm so very, very glad you love me back.");
    else outputText(", even if you can't say the same. And that's alright; it doesn't change my heart, and it never will. I love you.");
    outputText("</i>\"");
...
export function helspawnDiscoversBooze(): void {
    ...
    if (race(player) != "cat-boy" && race(player) != "cat-girl") outputText("a cat");
...
export function helSpawnChoosesAFightingStyle(): void {
    ...
    if (player.effects.getValue1Of(StatusAffects.Kelt) >= 100) {
    ...
    menu();
    if (player.effects.getValue1Of(StatusAffects.Kelt) >= 100) addButton(2, "Bow", snipermanders);
...
function snipermanders(): void {
    ...
    if (player.lowerBody == LowerBodyType.LIZARD) outputText(", even with your own lizard legs");
...
function swordAndBoardmander(): void {
    ...
    outputText("\n\n\"<i>Get " + mf(player, "him", "her") + ", " + flags[kFLAGS.HELSPAWN_NAME] + "!</i>\" Hel calls from the sidelines, sipping on a mug.  \"<i>Show " + mf(player, "him", "her") + " what a salamander can do!</i>\"");
...
function talkToHelspawn(): void {
    ...
    // Talk 3
    // {Needs Rath at camp]
    else if (temp <= 1 && player.effects.findByType(StatusAffects.CampRathazul) >= 0) {
    ...
    // {Isabella must be at camp}
        ...
        if (race(player) != "human") outputText(" and you yourself have suffered as well");
...
function sparHelspawn(): void {
    ...
    // If Sluttymander:
    if (flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) outputText("\n\n\"<i>Ready to get your shit kicked in, old " + mf(player, "man", "lady") + "?</i>\" she grins, drawing her weapon.");
...
export function hakonAndKiriComeVisit(): void {
    ...
    outputText("\n\n\"<i>Too right.  Put 'er there, " + mf(player, "son", "girl") + ",</i>\" Hakon says, extending a hand to you, grinning wide as he lets his daughter and granddaughter go.  You shake the old salamander's hand, nearly wincing as his powerful grip throttles your wrist vigorously, practically walking you back into the heart of the camp.");
