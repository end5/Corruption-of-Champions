...
export function returnToRathazulMenu(): void {
    if (player.effects.findByType(StatusAffects.CampRathazul) >= 0)
        campRathazul();
    else encounterRathazul();
}

export function encounterRathazul(): void {
    spriteSelect(49);
...
function rathazulMoveToCamp(): void {
    clearOutput();
...
function rathazulMoveDecline(): void {
    clearOutput();
...
export function campRathazul(): void {
    spriteSelect(49);
    if (flags[kFLAGS.MARBLE_PURIFICATION_STAGE] == 2 && player.effects.findByType(StatusAffects.MetRathazul) >= 0) {
        MarblePurification.visitRathazulToPurifyMarbleAfterLaBovaStopsWorkin();
        return;
...
function rathazulWorkOffer(): boolean {
    ...
    let purify: () => void = null;
    let debimbo: number = 0;
    let reductos: () => void = null;
    let lethiciteDefense: () => void = null;
    let dyes: () => void = null;
    if (player.inv.hasItem(ConsumableLib.BLACKEG) || player.inv.hasItem(ConsumableLib.L_BLKEG)) {
    ...
    // Item crafting offer
    if (player.inv.hasItem(UseableLib.GREENGL, 2)) {
        ...
        spoken = true;
        if (player.inv.hasItem(UseableLib.GREENGL, 5)) {
            showArmorMenu = true;
            totalOffers++;
        }
        else {
    ...
    // Item crafting offer
    if (player.inv.hasItem(UseableLib.B_CHITN)) {
        ...
        spoken = true;
        if (player.inv.hasItem(UseableLib.B_CHITN, 5)) {
            showArmorMenu = true;
            totalOffers++;
        }
        else {
    ...
    // SPOIDAH
    if (player.effects.findByType(StatusAffects.CampRathazul) >= 0 && player.inv.hasItem(UseableLib.T_SSILK) && flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00275] == 0) {
        showArmorMenu = true;
        spoken = true;
        totalOffers++;
    ...
    if (player.effects.findByType(StatusAffects.CampRathazul) >= 0) {
        if (flags[kFLAGS.RATHAZUL_DEBIMBO_OFFERED] == 0 && (SophieBimbo.bimboSophie() || player.perks.findByType(PerkLib.BimboBrains) >= 0 || player.perks.findByType(PerkLib.FutaFaculties) >= 0)) {
            ...
            if (player.inv.hasItem(ConsumableLib.SMART_T, 5) && player.gems >= 250) {
                totalOffers++;
                debimbo = 1;
            }
            else if (!player.inv.hasItem(ConsumableLib.SMART_T, 5)) outputText("  You should probably find some if you want that...");
...
function rathazulPurifyIncubiDraft(): void {
    clearOutput();
    if (player.gems < 20) {
    ...
    if (!game.debug)
        player.inv.destroyItems(ConsumableLib.INCUBID, 1);
...
function rathazulPurifySuccubiMilk(): void {
    clearOutput();
    if (player.gems < 20) {
        outputText("Rathazul says, \"<i>You do not have enough gems for that service.</i>\"");
        doNext(returnToRathazulMenu);
    ...
    if (!game.debug)
        player.inv.destroyItems(ConsumableLib.SUCMILK, 1);
...
function rathazulPurifySuccubiDelight(): void {
    clearOutput();
    if (player.gems < 20) {
        outputText("Rathazul says, \"<i>You do not have enough gems for that service.</i>\"");
        doNext(returnToRathazulMenu);
    ...
    if (!game.debug)
        player.inv.destroyItems(ConsumableLib.SDELITE, 1);
...
function rathazulPurifyLaBova(): void {
    clearOutput();
    if (player.gems < 20) {
        outputText("Rathazul says, \"<i>You do not have enough gems for that service.</i>\"");
        doNext(returnToRathazulMenu);
    ...
    if (!game.debug)
        player.inv.destroyItems(ConsumableLib.LABOVA_, 1);
...
function makeADeBimboDraft(): void {
    ...
    // Take items
    player.gems -= 250;
    player.inv.consumeItem(ConsumableLib.SMART_T, 5);
    statScreenRefresh();
    player.effects.addValue(StatusAffects.MetRathazul, 2, 1);
    Inventory.takeItem(ConsumableLib.DEBIMBO, returnToRathazulMenu);
}

export function rathazulArmorMenu(): void {
    spriteSelect(49);
    clearOutput();
    const beeArmor: () => void = (player.inv.hasItem(UseableLib.B_CHITN, 5) ? craftCarapace : null);
    const gelArmor: () => void = (player.inv.hasItem(UseableLib.GREENGL, 5) ? craftOozeArmor : null);
    let silk: () => void = null;
...
function craftSilkArmor(): void {
    ...
    // (Not enough webs:
    if (!player.inv.hasItem(UseableLib.T_SSILK, 5)) {
...
function commissionSilkArmorForReal(): void {
    ...
    player.gems -= 500;
    statScreenRefresh();
    player.inv.destroyItems(UseableLib.T_SSILK, 5);
    menu();
...
export function chooseArmorOrRobes(robeType: number): void {
    spriteSelect(49);
...
function craftOozeArmor(): void {
    spriteSelect(49);
    player.inv.destroyItems(UseableLib.GREENGL, 5);
...
function buyDye(dye: ItemType): void {
    spriteSelect(49);
    clearOutput();
...
function buyReducto(): void {
    spriteSelect(49);
    clearOutput();
    const cost: number = (flags[kFLAGS.AMILY_MET_RATHAZUL] >= 2 ? 50 : 100);
    if (player.gems >= cost) {
...
function growLethiciteDefense(): void {
    spriteSelect(49);
    clearOutput();
...
function growLethiciteDefenseYesYesYes(): void {
    spriteSelect(49);
    clearOutput();
...
function growLethiciteDefenseGuessNot(): void {
    spriteSelect(49);
    clearOutput();
...
export function craftCarapace(): void {
    spriteSelect(49);
    ...
    if (player.breastRows.biggestTitSize() >= 8) outputText("Your " + biggestBreastSizeDescript(player) + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.\n\n", false);
    player.inv.destroyItems(UseableLib.B_CHITN, 5);
    player.effects.addValue(StatusAffects.MetRathazul, 2, 1);
