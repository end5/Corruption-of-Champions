...
export function ceraphIsFollower(): boolean {
    return flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00286] > 0 || flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00287] > 0;
}

export function ceraphFollowerEncounter(forceCeraph: boolean = false): void {
    if (forceCeraph) {
        ceraphFollowerAppearance();
        return;
    }

    if (rand(24) == 0 && player.cocks.length > 0)
        catgirlEncounter();
...
export function ceraphSprite(): void {
    spriteSelect(87);
}

// [Actually Ceraph] -
export function ceraphFollowerAppearance(output: boolean = true): void {
...
function ceraphSexMenu(): void {
    clearOutput();
    let maleFuck: () => void = null;
    let femaleFuck: () => void = null;
    let hermFuck: () => void = null;
    ...
    if (player.cocks.length > 0 && player.vaginas.length > 0 && player.lust >= 33) {
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(100) >= 0) {
...
function followerCeraphRoleplay(): void {
    outputText("", true);
    outputText("You tell Ceraph you'd like to do a little roleplaying.  Her nipples turn hard under their latex bindings as she asks, \"<i>What will it be, " + mf(player, "Master", "Mistress") + "?  Shall I pretend you've just teased me into sexual submission, or would you like to switch things up and have your bottom play at being top again?  Or maybe... you'd like me to shapeshift into some other girl, and do all the dirty, depraved things she never would?</i>\"", false);
    ...
    let urta: () => void = null;
    let marbles: () => void = null;
    let dominika: () => void = null;
    if (flags[kFLAGS.TIMES_FUCKED_URTA] > 0 && (player.cocks.length > 0 || player.vaginas.length > 0) && player.lust >= 33) urta = ceraphUrtaRoleplay;
    if (player.cocks.length > 0 && player.cocks.cockThatFits(70) >= 0 && player.effects.findByType(StatusAffects.Marble) >= 0 && player.lust >= 33) marbles = sweetieNOOOO;
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00150] > 0 && player.lust >= 33 && player.cocks.length > 0) dominika = cerminika;
...
export function submissiveCeraphOffer(): void {
    spriteSelect(7);
    outputText("", true);
    ...
    outputText("Letting your eyes play over the demon's exotic, sculpted skin, you can't help but be tempted by her offer... Ceraph sees you mulling it over and produces a collar as she purrs, \"<i>No pressure... " + mf(player, "Master", "Mistress") + ".  You could always just take me here like usual, and perhaps, the next time you'll stop being so lucky...</i>\"\n\n", false);

    // [Display Rape Options + Collar Option]
    if (player.gender > 0) {
        outputText("Do you fuck her? (And if so, which of your body parts do you do it with?)", false);

        let dicking: () => void = null;
        let buttsmexing: () => void = null;
        // Dickings ahoyu!
        if (player.cocks.length > 0) {
            dicking = CeraphScene.maleFuckCeraphsPussy;
            if (player.cocks.cockThatFits(monster.analCapacity()) != -1) buttsmexing = CeraphScene.buttRapeCeraph;
            else outputText("  <b>There's no way you could fit inside her ass - you're too big.</b>", false);
        }
        let cunting: () => void = null;
        if (player.vaginas.length > 0) cunting = CeraphScene.rideCeraphsCockLikeaBAWSSexclamation11eleven;
...
function collarCeraph(): void {
    ...
    if (player.cor < 33) outputText("that you never expected making a demon into your slave would factor into your quest.  On one hand it seems wrong, but... she's a demon.  The fewer you have opposing you, the easier it will be to end their threat completely.", false);
        ...
        if (player.cocks.length > 0) outputText("cum-dump", false);
        else outputText("tongue-slave", false);
        outputText(".", false);
    }
    outputText("  Ceraph pulls herself up to her knees and kisses your " + feet(player) + ", a show of absolute submission and obedience.\n\n", false);

    outputText("The defeated demon explains, \"<i>Though I am now and forever your slut, your slave, your bitch... those in my harem cannot be abandoned.  I am sad to say I cannot live with you, " + mf(player, "Master", "Mistress") + ".</i>\"  She sees the look forming in your eyes and hastily adds, \"<i>Oh, I'll still be at your beck and call, but if I can't make it, I'll be sure to send you one of my pets.  Just rub this charm whenever you want my services, " + mf(player, "Master", "Mistress") + ", and I'll be there.</i>\"  Ceraph holds out a tiny onyx bar tipped with rubies.  The gems shine and glitter with their own inner light, while the black shaft seems to drink in everything around it, leaving behind darkness.\n\n", false);
    ...
    outputText(" her nodule-studded demon-dick with her free hand.  She whimpers, \"<i>Would my " + mf(player, "Master", "Mistress") + " prefer to carry " + mf(player, "his", "her") + " slave's token, or wear it as a belly-button piercing?</i>\"\n\n", false);
...
function carryCarephsToken(): void {
    ...
    outputText("Ceraph asks, \"<i>So, before my " + mf(player, "Master", "Mistress") + " leaves, would you like to fuck your new slut one of the old ways, one last time?</i>\"\n\n", false);
...
function getCeraphFollowerPiercing(): void {
    ...
    outputText("Ceraph asks, \"<i>" + mf(player, "Master", "Mistress") + ", before you go, would you like to fuck your slut one of the old ways, one last time?</i>\"", false);
    // [Display Rape Options + Collar Option]
    if (player.gender > 0) {
...
function cawkTawgle(): void {
    ...
    if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0) {
        ...
        outputText("She lets out a low hum, then suddenly arches her back as her hand sinks in an inch.  \"<i>O-oh-oh!</i>\" she moans, pulling it away with a small string of pre-cum tying it to her now-bulgeless panties.  Absently she licks the fluid from her fingers and asks, \"<i>Was there anything else, my " + mf(player, "Master", "Mistress") + "?</i>\"\n\n", false);
...
function CeraphHandsOutNewFetishesLikePervCandy(): void {
    ...
    if (flags[kFLAGS.PC_FETISH] == 0) {
        ...
        if (player.earsPierced == 0) outputText("pulls a pair of gleaming, green piercings from a pouch.  \"<i>Oh, don't worry " + mf(player, "Master", "Mistress") + "; you're going to love this so much.  These piercings are special, and they'll give you exactly what you want.</i>\"", false);
        else outputText("places her hands upon your pierced ears.  She whispers softly, \"<i>Don't worry " + mf(player, "Master", "Mistress") + ", I can easily use the piercings you already have.  It won't hurt.</i>\"", false);
        outputText("\n\n", false);
        // (NOT PIERCED)
        ...
        // (PIERCED)
        else outputText("The demon rubs your ears in her hands, numbing them slightly.  A gradual buzz builds behind your eyes, accompanied by a wave of dizziness.  You blink and try to shake your head, but as numb as you are, it's quite difficult.  After a few moments, the odd sensations pass, returning normal feeling to your ears and " + face(player) + ", much to your relief.  ", false);
        ...
        if (player.cocks.length > 0) outputText("cock bounce in time with your fluttering heartbeats", false);
        else if (player.vaginas.length > 0) outputText("vagina get wetter and wetter", false);
        else outputText("parted lips and confused expression", false);
        outputText(" as the new thoughts and desires settle themselves inside you.  She gives you a gentle pat and explains, \"<i>It's ok " + mf(player, "Master", "Mistress") + "; you're an exhibitionist now.  Would you like your piercing slave to give you even more?</i>\"\n\n", false);
    ...
    // *Fetish level = 1
        ...
        // If already pierced
        if (player.nipplesPierced > 0) outputText("She's all too happy to build up the suspense as she lays  her hands on your pierced nipples, giving them a gentle tweak that you can barely feel.  \"<i>Don't worry, " + mf(player, "Master", "Mistress") + ".  Imbuing your new fetish into you through these will be easy.  Just tell me all about which fetishes make you hottest while I do it, and see if you can guess your new kink.</i>\"\n\n", false);
        // Business as usual!
        outputText("The demon doesn't give you a chance to reply; instead, she focuses on ", false);
        if (player.nipplesPierced > 0) outputText("your " + nippleDescription(player, 0) + "s, circling her fingers all around the fleshy nubs.  Goosebumps run over your body in a wave, accompanied by a similar chill and a pressure behind your temples.  You shudder, but it quickly fades.", false);
        else outputText("aligning the business ends of the piercings with your sensitive nipple-flesh.  Your right " + nippleDescription(player, 0) + " is pierced in one smooth motion, nearly making you scream in pain.  As she fastens it on, you feel goosebumps spread over your body in a wave.  The second piercing doesn't seem to hurt as bad, but the sensation of spreading goosebumps is far more noticeable.", false);
        outputText("  Your eyes dart around, curious what fetish your demonic slave has given you this time.\n\n", false);

        outputText("Ceraph smiles down at you and whimpers, \"<i>I hope you're pleased with the new fetish " + mf(player, "Master", "Mistress") + ".  Just think about how similar being paralyzed is to being tied down and tell me if you like it.</i>\"\n\n", false);
    ...
    // *Fetish level = 2
        ...
        else if (player.vaginas.length > 0) outputText("feminine moisture drools from between your lips and your " + clitDescription(player) + " turns into a hard button.", false);
        else outputText("your body aches for release.", false);
        outputText("  With an amused grin, Ceraph yanks down your gear and ", false);
        // [dicks]
        if (player.cocks.length > 0) {
            outputText("grabs your " + cockDescript(game.player, 0), false);
        ...
        // [cunts]
            ...
            outputText(", getting ahold of the flesh around the base of your " + clitDescription(player) + ".  With practiced ease, she snaps the piercing closed, attaching the diamond stud to you while the pain fuzzes your vision red.\n\n", false);
...
function unfetishifyYourselfWithFollowerCeraph(): void {
    outputText("", true);
    spriteSelect(87);
    outputText("You ask Ceraph to remove one of the fetishes she generously donated earlier.  She sighs and nods, saying, \"<i>" + mf(player, "Master", "Mistress") + ", are you sure? It isn't that easy to do, and I love knowing my owner is aroused by my piercings!</i>\"\n\n", false);
...
function goThroughWithCeraphUnfetishification(): void {
    ...
    outputText("After a few moments, she seems to recover, and she asks, \"<i>Was there something else you needed me here for, " + mf(player, "Master", "Mistress") + ", or did you just want to waste my time?</i>\"\n\n", false);
...
function fuckFollowerCeraphsVagoo(): void {
    let x: number = player.cocks.cockThatFits(115);
    if (x < 0) x = 0;
    const y: number = player.cocks.cockThatFits2(115);
    outputText("", true);
    spriteSelect(87);
    ...
    outputText("You let Ceraph know that you'll be using her pussy.  She sighs and says, \"<i>Yes, " + mf(player, "Master", "Mistress") + ",</i>\" unable to hide the disappointment in her tone.  Ceraph shifts, the panties of her outfit fading away to reveal her dripping cunny", false);
    if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0) outputText(" and half-erect, throbbing cock", false);
    outputText(".  ", false);
    if (player.cor < 33) outputText("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, suddenly bound there by the abrupt appearance of those same panties, tied in a knot.  Supported by only her knees, her tits, shoulders, and her face, she asks, \"<i>Grab me and fuck me " + mf(player, "Master", "Mistress") + ", please grind your slave's face in the dirt!</i>\"\n\n", false);
    ...
    outputText("  Dragging the demon back, you bring her slutty, sodden puss up to your " + cockDescript(game.player, x), false);
    ...
    else outputText(", feeling the heat washing off her mons and onto your " + cockHead(player, x) + ".", false);
    outputText("  Yanking back, you bury all " + num2Text(Math.round(player.cocks[x].cockLength)) + " inches of your " + cockDescript(game.player, x) + " into Ceraph's unholy, warm snatch, ", false);
    if (player.cocks.cockArea(x) > 150) outputText("distorting her body around the sheer bulk of your massive member.\n\n", false);
    ...
    outputText("Somehow, she feels as tight around your " + cockDescript(game.player, x) + " as any virgin and three times as wet.  The gentle claps of genital against genital send ripples through Ceraph, smearing her cheek in the growing mud-puddle while her tits wobble dangerously inside the sheer latex bra.\n\n", false);

    outputText("\"<i>M-more!  Harder, " + mf(player, "Master", "Mistress") + "!</i>\" the demon pants as she crosses her legs behind your back, as if it could somehow stop you from pulling the whole way out.  You hold her one handed, just long enough to crack your palm against her tight, toned ass, and then you pick up the pace.  Now that you're fucking her faster, the slut isn't even trying to talk anymore.  Her hair is plastered against her scalp, stained brown by the mud, and her arms are slack in the restraints.  Ceraph gives every impression of being utterly resigned to being fucked like a toy, used without care for her own feelings or emotions.  Knowing that you've taken a powerful dominatrix and turned her into... this - it sends a chill up your back, invigorating your fast-pumping hips.\n\n", false);

    // (DOUBLE PENN!)
    if (y >= 0) {
        outputText("A wicked idea crosses your mind, and you reach down to grab your " + cockDescript(game.player, y) + ", aiming it at the flexing asshole just above the slave's squelching snatch.  Your next thrust plows it deep into Ceraph's anus, thankfully lubricated by the demon's constant squirting.  Growling at the mounting, doubling pleasure, you resume your tempo and slam both your cocks hilt-deep in Ceraph.  ", false);
        ...
        outputText("Your slave's reaction is to wiggle her backside at you and curl her tail about your waist, trying to pull you even further into her body.  Delirious and high on pleasure, you " + mf(player, "chuckle", "giggle") + " and pound away, heedless of anything but your own pending climax.", false);
        if (player.cocks.length >= 3) {
            outputText("  Sadly, your extra penis", false);
            if (player.cocks.length > 3) outputText("es have nothing to do but slide across her cheeks, dripping pre-cum all over her smooth skin.", false);
    ...
    outputText("With a shuddering explosion of warmth, you cum.  Ceraph groans loud and low, her pussy happily caressing your " + cockDescript(game.player, x) + " as it spews its potent load into her demonic womb.  ", false);
    if (y >= 0) outputText("Her asshole likewise gleefully devours the seed from your " + cockDescript(game.player, y) + ", flexing wildly from the fluid injection.  ", false);
    ...
    if (y >= 0) outputText("s", false);
    outputText(" with sloppy spooge.  Ceraph screams, \"<i>Yes!  Yes!  Fuck me!  Fill me!  Use me " + mf(player, "Master", "Mistress") + "!  Pump me full of cum while you grind my whorish face in the dirt!</i>\"  Her voice goes ragged, high pitched and screaming", false);
    if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0) outputText(", and her cock starts pumping more demonic spunk into the mud", false);
    outputText(".  It trails off, though her pussy continues teasing your " + cockDescript(game.player, x) + ", wringing the last of your seed from your " + ballsDescriptLight(player) + ".\n\n", false);
...
function followerCeraphTongueFucking(): void {
    ...
    outputText("Desirous of being pleasured by your demonic slave, you spread out your " + legs(player) + " to allow easy access to your " + vaginaDescript(player), false);
    ...
    outputText("Now breathing heavily, your " + hipDescription(player) + " begin trembling, aching to mount the invading member, fuck it, mate with it; anything to sate your growing desires.  Ceraph gives you a knowing wink and sidles forward, sliding the last several inches through your spread nether-lips into the velvety embrace of your " + vaginaDescript(player) + ".  You can feel it, squirming and rubbing inside you, twisting through your pussy with a slow, maddening purpose.  From time to time it brushes your cervix, but never hard, never painfully.  At the same time, it seems to always be in contact with your most sensitive places.  It makes you wonder if Ceraph has practiced this on herself at some point, and you briefly entertain the notion of the demon bent over, vigorously fucking her box with her perverted tongue-prick ravaging her purple pussy.\n\n", false);

    outputText("A jolt of pleasure blasts the image from your mind and nearly takes your " + legs(player) + " out from under you.  With a start, you realize Ceraph has opened her lips wide enough to slurp your " + clitDescription(player) + " into her mouth, and somehow, she's produced a second tongue to service it.  With the stimulation of a tentacular tongue constantly hitting your g-spot and a second oral organ ", false);
    if (player.clitLength >= 3) outputText("fellating", false);
    else outputText("licking", false);
    outputText(" at your " + clitDescription(player) + ", you start to shudder, trying to stave off what you know is coming.  You don't want Ceraph to get too uppity, thinking she can get you off this fast, but you're dangerously close, and her pumping, teasing mouth-cock is relentless.\n\n", false);
    ...
    if (player.vaginas.wetness() >= 5) outputText(", even though her cheeks are bulging and her throat struggles to devour all of the fountaining girl-spunk.", false);
    else if (player.vaginas.wetness() >= 4) outputText(", even though her cheeks are slightly bulged and she's gulping it down.", false);
    else if (player.vaginas.wetness() >= 3) outputText(", even though she has to gulp it down from time to time.", false);
    else if (player.vaginas.wetness() >= 2) outputText(", even though you produce enough for her to gulp.", false);
    ...
    else outputText("Shuddering, Ceraph returns her tongue to normal and slides it out of your tender quim with a knowing smile.\n\n\"<i>I'll never get tired of that, " + mf(player, "Master", "Mistress") + ",</i>\" she quips.\n\n", false);

    outputText("You pull her back to your " + vaginaDescript(player) + " to lick the last of your lady-spunk from your nethers, then send her on her way with a smile on your face.  Your expression widens when you see Ceraph stagger, still a bit shaky from her own orgasm.", false);
...
function ceraphTentacleGrape(): void {
    ...
    outputText("The two amethyst cocks wind their way over your " + legs(player) + " and lift you into the air with unholy strength, dangling you upside down while they crawl over your body, the smooth skin rubbing and stroking at your " + skinFurScales(player) + ".  They curl up and slide through your hands, allowing you to feel the inhuman warmth of Ceraph's passion.  Smiling, you indulge your slave, marvelling at the incredible degree of control she has over her shape-shifting.  Ceraph slides the two phallic tendrils between your loins and butt-cheeks, threading one in from the front and the other from the back.  They grind on your " + vaginaDescript(player) + " and " + assholeDescript(player) + ", teasing you, giving time for you to get as wet as possible.\n\n", false);

    outputText("A warm, sucking orifice aligns itself with your " + multiCockDescriptLight(game.player) + ", making obscene squelching noises as it dilates to take ", false);
    if (player.cocks.length == 1) outputText("all of your girth", false);
    ...
    outputText(" down your cum as it erupts from your " + multiCockDescriptLight(game.player) + ".  As it swallows every drop, you hazily wonder what she'll do with it all, but then the still-fucking tentacles move faster, spraying their cum out from your too-packed orifices to rain over both of you.  Your " + vaginaDescript(player) + " and " + assholeDescript(player) + " flutter and contract, involuntarily squeezing the purple-skinned invaders for even greater levels of sensations.  It's too much and too hard.  You black out with a moan of satiated pleasure.\n\n", false);

    outputText("You come to in a puddle of cum, both yours and Ceraph's.  The demoness is sitting down across from you, her appearance returned to normal.  She brightens when she wakes and kneels, saying, \"<i>Thank you for allowing me to serve you so... completely, " + mf(player, "Master", "Mistress") + ".  It was... thrilling.</i>\"\n\n", false);
...
export function catgirlEncounter(): void {
    outputText("", true);
    ...
    if (player.perks.findByType(PerkLib.Evade) < 0 && player.spe / 5 + rand(20) < 22) outputText("flies into you, knocking you to the ground.  After a moment, you find yourself faced with a pair of overeager cat-morphs grinning down at you.", false);
...
function fuckCeraphsCatgirls(): void {
    ...
    outputText("You smile at them and say that you'd be happy to have them for your pleasure; their horns suggest that they'll be quite a trip.  The two purr happily and instruct you to lie on your back to start the fun.  You relax as directed and the cat slaves unfasten your " + player.armorName + " from your body.  As they work, they make sure to gently stroke every inch of your newly exposed flesh with their soft furry hands as it's revealed; all the while moving closer and closer to your most personal parts.  When " + oMultiCockDesc(game.player) + " finally tastes the air, it gets even more attention.\n\n", false);

    // purrfect tit fuck
    outputText("You can't help but put your hands to their heads and start rubbing and scratching them behind their ears.  Suddenly, one of them steps back as the other moves down in front of your " + cockDescript(game.player, x) + " and pulls it inside her breasts.  In response, both your hands end up on her head, forcing it down onto your cock, and you feel your manhood start to vibrate as the catgirl begins to purr.  You cry out in pleasure at your shaft being massaged by soft-furred breastflesh while she hums into the tip.\n\n", false);
    ...
    if (player.cocks.length == 1) outputText("sliding her hand between the furry tits, down the base of your " + cockDescript(game.player, x) + ".", false);
    ...
    outputText("Thanks to all the stimulation from before, their expert tongues almost immediately bring your body to a shuddering orgasm, and " + sMultiCockDesc(game.player) + " ", false);
...
function ceraphUrtaRoleplay(): void {
    Urta.urtaSprite();
    outputText("", true);
    outputText("\"<i>Roleplay? My " + mf(player, "Master", "Mistress") + " is wonderfully exploitative with " + mf(player, "his", "her") + " pet's lewd body,</i>\" Ceraph purrs, lips curling into a sly smile. Holding your arms at your sides, you nod at the subjugated demon, indicating that she should strip you. Keeping her eyes averted, she obediently complies, removing your " + player.armorName + " piece by piece until you stand nude, in all your splendor. Turning upon her, you issue your curt command, briefly describing the form that she is to take. Surprisingly, she knows exactly who you're talking about. \"<i>Ah, the fox-bitch,</i>\" she muses, eyes flashing solid black again for a moment. \"<i>She's been such a thorn in my side for so long... letting you defile her will be a particularly intense pleasure, " + mf(player, "Master", "Mistress") + ".</i>\"\n\n", false);
    ...
    outputText("Sweeping her hands about in deference, Ceraph curtseys to you and raises her stolen face, eyes twinkling green behind her medium-length bangs. \"<i>With your permission, " + mf(player, "Master", "Mistress") + ", the final touch.</i>\" You nod, a grin already creeping at the sides of your mouth. Eagerly, the fiend takes hold of her demonic shaft- hard as much from the transformation as your lascivious stare- both hands wrapping around the demonic phallus gingerly. Licking a long, pink tongue over her inky lips, the shapeshifter begins to jerk herself off, sliding her palms up and down the pulsing, bumpy dick with quickening strokes. Her mouth hangs open and she rolls her eyes up as the frantic pace sends her furred chest wobbling and her lashing tail twitching frantically behind her. Gradually, you notice that her brutal pace seems to be lengthening the demon's organ, swelling firmness bloating it larger and longer, purplish hue darkening and darkening until it fades to a ruddy burgundy at the tip, fading to a velvet black at the base. \"<i>Oh " + mf(player, "Master", "Mistress") + ", your wish is my command</i>\" she gasps, her oily voice turning richer and huskier with every syllable, until it is an exact echo of Urta's. The resculpted horsecock throbbing in her hands lurches forward to its full 20 inches as her tip flares out, thick jets of ropey cum bursting from the fox-girl's equine member. As it jerks in her hands, a fuzzy, ebony sac drops from the puffy sheath of her jizzing cock, trembling balls dropping heavily into the scrotum. When she's finally done, the captain of Tel'Adre's City Guards stands before you, panting, her still-dripping cock in one hand, a tall bottle of whisky in the other, creamy pools of cum all around her.\n\n", false);
    ...
    // [Drink][Sober]
    let sober: () => void = null;
    if (player.cocks.length > 0) sober = ceraphUrtaRoleplaySober;
    let drunk: () => void = null;
    if (player.vaginas.length > 0) drunk = ceraphUrtaRoleplayDrunk;
...
function ceraphUrtaRoleplayDrunk(): void {
    ...
    outputText("Grasping her shoulders and lifting her gaze to yours, you stare into Urta's emerald eyes. With a signing breath, you whisper that she's never looked more beautiful, and press forward, your lips eagerly finding hers. She twists her head too far to the side, trying to compensate for a muzzle that's no longer there before giggling into your mouth and turning back too far, bumping her nose against yours. She lets out a brief bark of laughter and moistly kisses your forehead, running her hands unsteadily down your " + skinFurScales(player) + ". \"<i>So, you like me this way, huh? Well, now it's my turn. Bottom's up!</i>\" She pushes you backwards harder than she'd intended, knocking your head against the soft ground before grabbing your " + hipDescription(player) + " and flipping you onto your " + allChestDesc(game.player) + ". Looking back over your shoulder, you see the girl tweaking her pale, pink nipples which stiffly rise from the generous swell of her olive breasts. A warm, firm thwack between your ass cheeks tells you that neither the alcohol nor her first orgasm has affected the herm's raging hardness. As she slides her cock up and down, between the pillowy orbs of your rump, you can feel every contour of her twenty inch horsecock- from its bulging veins to the ringed lip of her fleshy sheath to the smooth, cool skin of her refilling scrotum, heavily slapping against your inner thighs. You squeeze your " + buttDescription(player) + " in time with her long strokes, stroking the shaft between your globes as she quickens the pace. She can't keep her hands off her new body, it seems, the guards-woman rubbing her palms over her breasts, belly, arms, and hips, feeling her flawless flesh as eagerly as she hotdogs your " + buttDescription(player) + ".\n\n", false);
    dynStats("lus", 125);
    // [Next]
...
function ceraphUrtaRoleplayDrunk2(): void {
    ...
    if (AmilyScene.amilyFollower() || MarbleScene.marbleFollower() || IzmaScene.izmaFollower()) outputText("  After I knock you up, try explaining the fox tails on your kids to those other bitches.", false);
...
function ceraphUrtaRoleplaySober(): void {
    Urta.urtaSprite();
    outputText("", true);
    outputText("You tell Urta to put the bottle down. She won't need that, not any more. She looks at you in confusion, setting the whiskey to one side, curling her tail between her legs to cover her throbbing member. Closing the distance between the two of you, she stiffens when you wrap an arm around the small of her back and bring the other hand up to her chin. She doesn't have to be ashamed any more, you explain, because you know the cure for her curse. The fox-morph's eyes light up, her mouth parting but not daring to speak or even breathe. Stroking a thumb along the line of her jaw, you close your eyes and nod slowly, pulling her into an embrace tight enough for you to feel the fluttering pulse of her body heat sinking through your " + skinFurScales(player) + ". You can tell by the wobbling of her lower lip that she is dying to ask how, but you merely brush the dappled-grey bangs from her eyes, staring into the guard's emerald irises. You can feel the soft intake of her breath as it catches in her throat and she leans toward you ever so slightly, blushing. You meet her halfway, obsidian-warm lips pressing against yours tentatively at first, before gaining confidence. She sinks deeper into the embrace, the tight tension knotting her back slowly easing as surrenders her self-conscious shame for unabashed passion, relishing the intimacy of your caress. When you draw back from the intoxicating fever of the fox girl, you whisper one word to her: \"<i>Love.</i>\"\n\n", false);
...
export function carephCorruptionSlaves(): void {
    outputText("", true);
    ...
    outputText("One of the imps steps forward and opens a letter, then begins reading.  \"<i>Lady Ceraph apologizes to her " + mf(player, "Master", "Mistress") + ", but she finds herself unable to service you.  So she has sent this human as an offering for the " + mf(player, "Master", "Mistress") + " to corrupt.  To this end, she has prepared a concoction for you.  Drinking this will provide you with what you need for the job, " + mf(player, "Master", "Mistress") + ".</i>\"\n\n", false);
    ...
    if (player.cor >= 75) {
        ...
        outputText("The imp recoils and offers a quick apology.  \"<i>No, of course not, " + mf(player, "Master", "Mistress") + ".  Forgive us, we did not wish to offend.</i>\"\n\n", false);
...
function makeCarephsLackeysLeave(): void {
    outputText("", true);
    outputText("You wave the imps away and tell them that you're not interested.  One of the imps protests, \"<i>But, " + mf(player, "Master", "Mistress") + "-</i>\"  You cut him off before he has a chance to finish, saying that you wanted Ceraph, not some human girl!  Then, you toss the potion away and tell them to take the girl away.\n\n", false);

    outputText("\"<i>Y-Yes, " + mf(player, "Master", "Mistress") + "...</i>\" the imps reply meekly, pulling on the collar to drag the girl away.", false);
...
function ceraphLackeyCorruption(): void {
    ...
    if (player.cocks.cockArea(player.cocks.biggestCockIndex()) < 48 && player.cocks.length > 0) {
    ...
    outputText(legs(player) + ".  The imps on her extremities laugh at the girl as she relaxes and her head slumps into the ground; you motion for the imps to release her and step back, then align yourself with her pussy.\n\n", false);

    // (if PC is above 60 cock area)
    if (player.cocks.cockArea(x) >= 60) {
    ...
    outputText("She strokes your head, giggling, \"<i>I hope you enjoyed our little tryst, " + mf(player, "Master", "Mistress") + ".  Lady Ceraph wasn't lying when she said you were one hell of a fuck.</i>\"  You lift your head in surprise; did she become a demon on purpose?\n\n", false);
...
function declineCeraphFauxCorruption(): void {
    ...
    outputText("\"<i>Sorry boys, " + mf(player, "Master's", "Mistress") + " orders.</i>\"  She extends her wings and flies away, and the horny imps follow suit, still busy masturbating.  A 'pit-pat-pat' sound follows them, the noise of their pre-cum hitting the dry dirt from on high.\n\n", false);
    doNext(Camp.returnToCampUseOneHour);
}
...
function acceptMoreCeraphFauxCorruption(): void {
    ...
    outputText("\"<i>Look at what happened to me... used and transformed, then forced to service a bunch of dirty imps...  Thank you, " + mf(player, "Master", "Mistress") + ",</i>\" she moans with a lewd smile.\n\n", false);

    outputText("Licking the cum off her body, she sashays towards you to give you a little peck on the cheek.  \"<i>Hmm, you're such a good " + mf(player, "Master", "Mistress") + ", I might have to leave Ceraph's harem and join yours instead.  See you around, hot stuff.</i>\"  She rounds up the tired imps and extends her wings, setting off alongside them.", false);
...
function sweetieNOOOO(): void {
    ...
    outputText("\"<i>Aaaah, not satisfied with me, " + mf(player, "Master", "Mistress") + "?</i>\" Ceraph huffs, feigning exasperation.  She pointedly runs a hand along her muscular thigh, up her taut belly, and around one of her perfectly-formed lilac breasts.  \"<i>And what did you have in mind for our... playtime?</i>\"\n\n", false);
    ...
    outputText("  She reaches up and takes a tentative grasp of her curved, demonic horns, straightening and molding them into more bovine models.  The spade-tip of her tail shrinks, then puffs out with hair, and the whole appendage droops as it becomes remarkably more cow-like.  Almost as an afterthought, she paces up to you and slowly strips you of your " + player.armorName + ".  She teases " + oMultiCockDesc(game.player) + " a bit before gliding back to her previous position.\n\n", false);
    ...
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00295] == 1) doNext(postUdderChoice);
        ...
        outputText("  \"<i>Now then, " + mf(player, "Master", "Mistress") + "... or, should I say, Sweetie,</i>\" she breathes, her sultry tones smoothing into an earthy, slightly drawn-out accent, \"<i>there's one more detail that she - sorry, I - don't have; would you like me to have... an udder?</i>\"\n\n", false);
...
function postUdderChoice(newl: boolean = false): void {
    ...
    if (player.balls > 0) outputText("your " + ballsDescriptLight(player) + " tightening", false);
    ...
    outputText(".  After a few more moments of observation, you turn away, planting your hands on your hips victoriously.  You simply thank the cowgirl, and she responds with a satisfied moan.  Suddenly, a massive splashing sound - as if gallons of liquid were suddenly released - reaches your ears, and the barn shimmers and fades into the familiar setting of your camp.  You feel a hand on your shoulder, and you glance down to see a creamy-colored hand, veins of purple creeping along the surface and inching up her fingers.  \"<i>Thank you, " + mf(player, "Master", "Mistress") + ",</i>\" Ceraph replies amiably, tittering to herself.  \"<i>You've got some kinks to work out, don't you?  I like that.</i>\"\n\n", false);
...
function cerminika(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        outputText("Dominika slides her body back, rubbing the softness of her folds against your " + cockDescript(game.player, 0) + ".  ", false);
        ...
        outputText("  You simply smirk at her, starting to bounce her into your lap and shoving your " + cockDescript(game.player, 0) + " deeper inside. Once more she bites her lip, fingers digging into your stomach. No matter how classy she wants to appear, you can quite easily feel how wet she is", false);
        ...
        outputText("You give her a carefully crafted retort that demonstrates your intellectual superiority, but she just gives you a fake smile and pats your cheek.  Eyes half-closed she slowly slides herself along you, enjoying the sensation of each inch of your " + cockDescript(game.player, 0) + " as it brushes along her walls.  \"<i>Mmmm,</i>\" she lets herself purr out, \"<i>Worth... the wait.</i>\"  Her toes curl slightly, one of her legs shifting back slightly to alter the angle you enter her at.  It seems to work how she wants it to, as a deep, satisfied breath accompanies the next downward slide of her hips.", false);
        ...
        outputText("Grabbing her shoulders, you surprise her once more by twisting and rolling on top of her.  \"<i>No!</i>\" she shouts immediately, knowing your plot, \"<i>You " + mf(player, "bastard", "bitch") + "!  No one puts me on my own back!</i>\"  You remind her that facts need more proof than the assumption that things will never change.  She spits at you, but you take solace in the fact that your hands are over her wrists and your " + cockDescript(game.player, 0) + " is shoved so deep in Dominika there's a chance she could spit up cum when you're done rather than swallow it.  Presuming, of course, that her vagina is connected to her stomach, which it probably is not.\n\n", false);
        ...
        outputText("It's surprisingly easy to spear yourself into the cabalist's plush posterior.  You had expected a woman of her nature to be tighter than she is, but the pussy juices soaking your " + cockDescript(game.player, 0) + " are sufficient for a slippery and enjoyable entry.  She grunts and gasps dissatisfaction, but there's a deepness in her breath that betrays the excitement building within.  At the same time, she seems curiously nervous, and shakes not just in pleasure.\n\n", false);

        outputText("\"<i>P-please,</i>\" she stammers out, even as her asshole clenches around you.  Though it was easy to force yourself into Dominika's puckered button, you're impressed at how well her colon squeezes your shaft.  There's no blown-out anal whore here, just a petite fucktunnel the perfect size to be broken by your " + cockDescript(game.player, 0) + ".  You part aside her bunghole, claiming her ass in your name – colonizing her colon.  The entire reason those cheeks exist is cushioning for your crotch as you pound away at her, the juices her cunt left on your cock squirting out as your prick dominates every inch of those fleshy walls.  \"<i>Please,</i>\" she gets out again, \"<i>I've held out... so long...</i>\" She turns her head to look at you.  Tucked just behind the part of her hair, two small horns push from her skull.  A moment of confusion takes you, before it is replaced by a knowing smirk.  Jeering at the corrupted sorceress, you mock her inability to defend herself.  Without her chastity belt to protect her from the pleasure of the demon realm, it is free to feed upon her body and devour what purity remained, leaving behind just another succubus.  \"<i>Don't... make me cum,</i>\" she gasps.\n\n", false);

        outputText("Under most circumstances you would have no opposition to denying a slut like her an orgasm, but considering the precipice she dangles over, you don't feel bad giving her a push.  Your " + cockDescript(game.player, 0) + " continues its obscene assault on her backside, splitting her juicy cheeks to either side as you bend her guts to your dick's will.  Dominika lets out a moan that seems as though she may have intended it to be a scream of frustration, but her body is too far gone into pleasure to protest.  Like ink running through paper a deep dark purple begins to spread from her tattoos.  The color runs into itself and grows larger, leaving behind no lines where it joins.  The tan of her skin begins to disappear under the corruption.  \"<i>You " + mf(player, "bastard", "bitch") + ",</i>\" she pants, shuddering as the horns grow larger.  \"<i>You would destroy my humanity... just for your pleasure?</i>\"\n\n", false);

        outputText("\"<i>Yes,</i>\" you answer calmly.\n\n", false);

        outputText("She wails in mixed frustration and pleasure, juices splattering against the ground as her hungry cunt grows hornier.  A darkness overtakes her eyes, and her nails grow long and sharp, and through it all your " + cockDescript(game.player, 0) + " continues to degrade her ass.  As though a heartbeat pumping blood through her body, each thrust spreads the color over her skin a little more.  Her feet push and scrabble lightly against the ground in tormented frustration, but she is helpless beneath you.  You go without stopping, only slowing when wings unexpectedly burst from her shoulder blades, dark and starry.\n\n", false);
        ...
        else outputText("  She shudders as your " + cockDescript(game.player, 0) + " fills her guts with warmth, hips seizing as she rides out her assgasm.", false);
        ...
        outputText("  The demoness laps at your " + cockDescript(game.player, 0) + ", cleaning it of spunk, sweat, and all the various filth it accumulates, sucking and dragging her fat, midnight lips across them.  Dominika purrs her happiness, her joy at being forced into such a whorish form.\n\n", false);

        outputText("\"<i>Was that good?</i>\" Ceraph asks in her own voice, immediately ruining the illusion. \"<i>", false);
        // [Player has huge dick:
        if (player.cocks.cockArea(0) >= 75) outputText("I doubt her ass could actually take you so well, but... I wanted my " + mf(player, "master", "mistress") + " to enjoy a perfect asshole.  Sue me.  ", false);
...
// (First Time):
export function encounterZetsuko(): void {
    outputText("", true);
    flags[kFLAGS.ZETSUKO_MET]++;
    let banana: () => void = null;
    let vag: () => void = null;
    let dick: () => void = null;
    // First time
    if (flags[kFLAGS.ZETSUKO_MET] == 1) {
        ...
        outputText("\"<i>Zetsuko apologizes for Mistress's absence, " + mf(player, "Master", "other Mistress") + ".  Mistress wasn't able to be here, so she sent Zetsuko instead.  She hopes Zetsuko will be to your liking...</i>\"\n\n", false);
        ...
        outputText("\"<i>Zetsuko was wondering if " + mf(player, "Master", "Mistress") + " could perhaps... loosen her jacket?</i>\" she says, struggling in her straitjacket a bit.  \"<i>Mistress tied it a little too tightly...</i>\"\n\n", false);
        ...
        outputText("\"<i>" + mf(player, "Master", "Mistress") + " isn't going to let Zetsuko out, is " + mf(player, "he", "she") + "? Hehe... Too smart... Zetsuko is a bad girl, yes... many bad habits, trying to trick " + mf(player, "Master", "Mistress") + ".  Mistress tries to teach her, but Zetsuko has far too much fun being bad.</i>\"\n\n", false);
        ...
        outputText("\"<i>Is Zetsuko's pussy to " + mf(player, "Master", "Mistress") + "'s liking?</i>\" she says, laying on her back and spreading her legs further to proudly display her freakish appendage to you, putting on a display as she begins to tongue her own clit, flicking her hood piercing gently.\n\n", false);
    ...
    // (Subsequent times):
        ...
        outputText("\"<i>Zetsuko apologizes for Mistress's absence, " + mf(player, "Master", "other Mistress") + ".  Mistress wasn't able to be here, so she sent Zetsuko instead.  Zetsuko was hoping perhaps today " + mf(player, "Master", "Mistress") + " would deign to loosen Zetsuko's bonds?</i>\"\n\n", false);
    ...
    if (player.cocks.length > 0) dick = fuckZetsukosTonguepussy;
    if (player.vaginas.length > 0) vag = getTonguedByZetsuko;
...
function fuckZetsukosTonguepussy(): void {
    outputText("", true);
    const x: number = player.cocks.biggestCockIndex();
    outputText("Your lust and curiosity get the best of you as the sight of that licentious tongue wriggling around between her legs sends a heat to your groin, blood rushing into your " + multiCockDescriptLight(game.player) + ".  Zetsuko lays flat on her back, grinning up at you as her pussy licks itself in anticipation, and as you lower yourself down, her tongue snakes out even more, beginning to wrap itself around " + oMultiCockDesc(game.player) + " and slathering it with her saliva-like juices.  Your sensitive member can feel that the surface of her tongue is covered in hundreds of tiny bumps and nubs that massage your shaft, and the multiple studs that line it rub you sensually as the bizarre organ coils around your " + cockDescript(game.player, x) + " like a snake, the very tip gently stimulating around the edges of your urethra.\n\n", false);

    outputText("You marvel at the incredible degree of control she seems to have over her tongue, watching as it begins to stroke and squeeze along your shaft, coating you in slick saliva from base to tip.  Its grip around you tightens up a bit, and you can feel it tugging you down gently, drawing your " + cockDescript(game.player, x) + " ever closer to her entrance.  Her vagina lips spread open as you approach, gaping wide like a hungry mouth, and as it does so, you can see that the inner surface of her pussy has a number of soft bumpy nodules lining it in a downward spiral pattern.\n\n", false);

    outputText("\"<i>Zetsuko can't wait to have " + mf(player, "Master's", "Mistress's") + " dick in her pussy...  she knows " + mf(player, "Master", "Mistress") + " will just love it...</i>\" she says, wiggling her hips a bit as she pulls you up against her entrance, her lips sucking on the tip gently.", false);
    if (player.cocks.cockArea(x) >= 100) outputText("  You wonder just how she plans to fit you inside, but there's not much you can do about it at this point.  You'll just have to trust that she knows what she's doing, crazy or not.", false);
    ...
    if (player.cocks.cockArea(x) >= 100) outputText("somehow ", false);
    ...
    if (player.balls > 0) outputText("slithering across your " + ballsDescriptLight(player) + ", ", false);
    else if (player.vaginas.length > 0) outputText("gently licking the outer edges of your labia, ", false);
    outputText("moving along to the back, ", false);
    if (player.balls > 0 || player.vaginas.length > 0) outputText("and ", false);
    outputText("teasing your " + assholeDescript(player) + " gently.  You gather up a mound of her soft tit flesh in each hand and begin squeezing along them lustfully, pushing on her pierced nipples with your thumbs gently while her tongue continues to lick and pleasure everything within its reach.\n\n", false);

    outputText("\"<i>Ooo-oohh! " + mf(player, "Master's", "Mistress's") + " cock tastes so good in Zetsuko's pussy!</i>\" she moans, rolling her eyes back as you feel her begin to \"<i>suck</i>\" on your " + cockDescript(game.player, x) + " deeply, rocking her hips back and forth.  The suction inside her drooling cunt feels like one of the most intense deepthroats you've ever felt, and you forget for a moment that it's coming from her vagina until Zetsuko's loudest moan yet snaps you back to reality.\n\n", false);
...
function getTonguedByZetsuko(): void {
    outputText("", true);
    let x: number = -1;
    if (player.cocks.length > 0) x = player.cocks.biggestCockIndex();
    ...
    outputText("The salacious organ slithers toward you, caressing the outer lips of your " + vaginaDescript(player, 0) + " and gently prodding your " + clitDescription(player) + " before receding, making what can only be described as a \"<i>come hither</i>\" motion.  You take a step forward, pressing your hips up against the heavy cushions of her ass, and her tongue immediately curls toward your groin, ", false);
    if (x >= 0) outputText("wrapping itself around your " + cockDescript(game.player, x) + " and squeezing momentarily before ", false);
    ...
    if (x >= 0) {
        outputText("In a somewhat impressive feat of contortion, Zetsuko brings her feet in toward her groin to stimulate your " + multiCockDescriptLight(game.player) + ", lifting ", false);
        ...
        if (player.cocks.length > 1) outputText("s", false);
        outputText(" with the soles of her feet.  Her toes are surprisingly dextrous, able to curl around and grip your " + cockDescript(game.player, x) + " lightly, gently pinching and tugging at the skin just under the head and manipulating your throbbing member", false);
        if (player.cocks.length > 1) outputText("s", false);
        outputText(" almost as skillfully as if she were using her hands!\n\n", false);
    }

    outputText("You slide your hips forward, curling her body back further and resting your weight on the pillowy cushions of her bottom, riding down onto her tongue and shivering deliriously.  You can feel it plumbing your depths, slithering into you and filling every nook and cranny as it swells and flows along your walls to take up every last bit of room you have.  The degree of control she has over it is simply incredible, becoming thicker to stretch your " + vaginaDescript(player, 0) + " and then suddenly retracting to swirl itself into a spiralling tendril that drills your hole lewdly.\n\n", false);

    outputText("\"<i>Oooh, Mistress's pussy is delicious!</i>\"\n\n", false);

    outputText("Grabbing hold of her thighs, you begin to rock yourself back and forth across her tongue, moaning powerfully as you feel it begin to thrust in and out of you now.  The numerous studs that line the center of the organ begin to press firmly against the roof of your " + vaginaDescript(player, 0) + ", stimulating your G-spot and flicking your " + clitDescription(player) + " each time her tongue dives back inside you.  Your fingers dig into her flesh like a vice now as you grind yourself down on the indecent tendril, moaning salaciously as the waves of your climax begin to overtake you.\n\n", false);

    outputText("Your " + vaginaDescript(player, 0) + " cinches tight around Zetsuko's tongue, and it continues to wriggle inside you while you ride out your orgasm.", false);
    if (x >= 0) {
        outputText("  " + SMultiCockDesc(game.player) + " twitches powerfully between her feet, ", false);
    ...
    if (player.vaginas.wetness() >= 4) outputText(" and overflowing to dribble down her bountiful ass cheeks and stomach", false);
...
function giveFollowerBodyBits(): void {
    ...
    if (player.gender > 0 || player.breastRows.biggestTitSize() > 0) {
        ...
        let wang: () => void = null;
        let smallestWang: () => void = null;
        let vag: () => void = null;
        let breasts: () => void = null;
        let breasts2: () => void = null;
        let breasts3: () => void = null;
        if (player.cocks.length > 0) {
            outputText("\nYou can give her your [if (cocks == 1) penis|biggest penis or smallest penis].");
            wang = ceraphFollowerCockTaking;
            if (player.cocks.length > 1) smallestWang = createCallBackFunction(ceraphFollowerCockTaking, true);
        }
...
function ceraphFollowerCockTaking(smallest: boolean = false): void {
    ...
    if (smallest) x = player.cocks.smallestCockIndex();
    const y: number = x + 1;
    ...
    outputText("\n\nCeraph's fingers feel unexpectedly hot as they brush your " + cockDescript(game.player, x) + ", as if she had just come out of a heated tub.  You don't have long to ponder that little detail as they pinch into a tight ring", false);
    if (player.cocks[x].cockThickness >= 4) outputText(", compressing your massive girth into the rigid shaft", false);
    outputText(", slowly closing.  It should hurt but it doesn't, it just feels warmer and warmer, and then with a sudden 'PYOING!', Ceraph is holding your " + cockDescript(game.player, x), false);
    if (player.balls > 0 && player.cocks.length == 1) {
        outputText(" and balls", false);
        // Gotcher balls!
        player.balls = 0;
        player.ballSize = 1;
    }
    if (player.cocks.length == 1) player.effects.remove(StatusAffects.Infested);
...
function ceraphFollowerTitTaking(rowNum: number = 0): void {
    spriteSelect(87);
    clearOutput();
    const x: number = rowNum;
...
function portalFuckWithFollowerCeraph(): void {
    spriteSelect(87);
    clearOutput();
    const x: number = player.cocks.cockThatFits(100);
    const y: number = x + 1;
    // FIRST TIME:
...
function layEggsInSlaveCeraph(): void {
    ...
    // [if (cocks > 0)
    if (player.cocks.length > 0) outputText("  Ceraph glances at the bulge in your [armor] quickly before she demurely lowers her eyes.  A wicked smile plays across her beautiful face as she responds, \"<i>Yes, [master].</i>\"");
    ...
    outputText("\n\n\"<i>Stop!</i>\" Ceraph cries as you jerk the bottoms back and forth savagely, \"<i>I'll take them off, " + mf(player, "Master", "Mistress") + "!  I'm sorry!</i>\"");
...
function sumissivenessToCeraphFollower(): void {
    ...
    outputText("\n\n\"<i>That's a better place for a useless little " + mf(player, "boy", "girl") + " like you, down in the dirt where you belong,</i>\" the demoness declares, pacing back and forth.  She crouches next to you and whispers in your ear, \"<i>The safe word is 'apple', you pitiful pig.</i>\"");
    outputText("\n\nYou nod meekly, awed by the demonic dominatrix's imposing, self-assured aura of command.");
    outputText("\n\n\"<i>Bend over, slut,</i>\" Ceraph commands.");
    outputText("\n\nYou start to stand up, but Ceraph again lashes out with her whip, striking you across the back four or five times.  It's hard to keep count through the pain.  Stinging welts rise on your " + skin(player) + ", and a heady thrill runs through your groin.  You're totally stiff by the time she finishes the punishment, and it's plainly visible to the cruel hermaphrodite above.");
    ...
    outputText("\n\nCeraph smirks and asks, \"<i>Does my subby slut like it when I play rough?  Or did you just get hard when you realized how right I am?  There's no shame in enjoying your station, pet.  After all, animals are there to please their owners.</i>\"  She strokes your " + hairDescription(player) + " with an unfamiliar gentleness.  It's like a pleasant balm after the rough whipping, and you find yourself leaning into it, accepting the demeaning affection just to feel something that doesn't hurt.");

    outputText("\n\n\"<i>Good " + mf(player, "boy", "girl") + ",</i>\" the purple-skinned dom coos as she begins to undress you, keeping you on the ground the whole time.  \"<i>Let's get you out of these pesky clothes.  Good pets are naked, and I can see how <b>hard</b> you're trying to be a good pet.</i>\"  Her hand dances across ");
    ...
    if (player.cocks.length > 0) outputText("pre-cum has begun to form on your [cockHead biggest]");
...
function barkToCeraphOnce(dog: boolean = true): void {
    ...
    if (player.tailType > TailType.NONE) outputText(", wagging your tail excitedly");
    outputText(".");
    ...
    if (player.cocks.length > 0) outputText("milked ");
    ...
    else if (player.vaginas.length > 0) outputText("to your achingly empty pussy");
    if (player.cocks.length > 1) outputText("es");

    outputText(".  Almost immediately, you squirt out a huge dollop of ");
    if (player.cocks.length > 0) outputText("pre-cum");
    else outputText("lady spunk");
    outputText(".  It dangles heavily enough to ");
    if (player.cocks.length > 0) outputText("make your [cock biggest] bob below");
    ...
    if (player.cocks.length > 0) outputText("rams straight into your inner, cock-milk reserve");
    ...
    outputText("\n\n");
    if (player.cocks.length > 0) outputText("Dick-milk");
    ...
    if (player.cocks.length > 0) outputText("sliding");
    else outputText("pouring");
    outputText(" smoothly out of it as your ");
    if (player.cocks.length > 0) outputText("inner reservoir is squeezed");
    ...
    outputText(".  ");
    if (player.cocks.length > 0) outputText("Ironically, [eachCock] has begun to lose some of its stiffness, and you grow limper and limper as more of your cum leaks from you");
    ...
    if (player.cocks.length > 0) outputText("the 'pressure'");
    else outputText("a need within you");
    outputText(" feels released, you're still horny");
    if (player.cocks.length > 0) {
        if (player.cocks.length == 1) outputText(", even with a soft cock");
    ...
    if (player.cocks.length > 0) outputText("semen");
    ...
    if (player.cocks.length > 0) outputText("prostate is getting pushed up against each time");
    ...
    if (player.cocks.length > 0) outputText("prostate is");
    else outputText("vaginal walls are");
    outputText(" being smashed flat.  Warmth explodes inside your intestines, slowly spreading out around the flaring, demonically enhanced dick as it orgasms.  You can hear your owner's lady-spunk splattering into the ground behind you as she whimpers, \"<i>G-g-good... " + mf(player, "boy", "girl") + "...</i>\", her dick firmly spraying its thick, tainted milk ");
    if (player.cocks.length > 0) outputText("straight onto your sore prostate");
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length > 1) outputText("each of your cocks");
    ...
    outputText("\n\n\"<i>Good, " + mf(player, "boy", "girl") + ".</i>\" her voice says, slowly fading into the wind.");
...
function barkOrMeowTwiceToCeraph(dog: boolean = true): void {
    ...
    if (player.tailType > TailType.NONE) outputText("tail");
    ...
    if (select == 0) {
        ...
        outputText(".  ");
        if (player.cocks.length > 0) outputText("It bounces against your tummy");
    ...
    // Corrupt Jojo or Amily
        ...
        // Both
        if (AmilyScene.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 2 && JojoScene.campCorruptJojo()) {
            ...
            outputText(" sexual scent is more than distracting.  Ceraph squirts the two of you with water a half-dozen times to keep you from fucking, and each time the shame is greater.  You really are a pet, one that can't even keep " + mf(player, "his", "her") + " most basic needs in check.");
        }
        // Mice finisher
        outputText("\n\nYou go for an exhausting walk around the nearby wasteland with your companion");
        if (AmilyScene.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 2 && JojoScene.campCorruptJojo()) outputText("s");
        ...
        if (AmilyScene.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 2 && JojoScene.campCorruptJojo()) outputText("look around uncertainly before darting back to their usual places, knowing you'll give the command if you want them to take care of your desires.");
    ...
    // Generic
        ...
        if (player.cocks.length > 0) outputText("goblins");
        else outputText("imps");
        outputText(" seem intrigued by the sight of your compulsively ");
        if (player.cocks.length > 0) outputText("erect maleness");
        ...
        if (player.cocks.length > 0) outputText("her");
        else outputText("his");
        outputText(" hand beneath your freely drooling ");
        if (player.cocks.length > 0) outputText("erection");
        else outputText("pussy");
        if (player.cocks.length > 1) outputText("s");
        outputText(" to collect a palmful of slippery ");
        if (player.cocks.length > 0) outputText("boy");
        else outputText("girl");
        outputText("-honey.  ");
        if (player.cocks.length > 0) outputText("She");
        else outputText("He");
        outputText(" holds it up in front of her nose and sniffs it, sighing, \"<i>" + mf(player, "He", "She") + " certainly does seem to be well tamed, miss demon.  You're lucky you caught " + mf(player, "him", "her") + " before I did.  With ");
        if (player.cocks.length > 0) outputText("cum this nice, I'd be milking " + mf(player, "him", "her") + " dry every time " + mf(player, "he", "she") + " could get it up");
        else outputText("a scent like this, I'd be fucking " + mf(player, "him", "her") + " every chance I could get");
        ...
        if (player.cocks.length > 0) outputText("pre-cum rockets");
        ...
        if (player.cocks.length > 0) outputText("goblin");
        else outputText("imp");
        ...
        if (player.cocks.length > 0) outputText("pre-ejaculate");
