...
// Intro
export function shouldraGreeting(): void {
    spriteSelect(66);
    outputText("", true);
...
// Victory Scenarios
// Ghost Girl Defeated
export function defeatDannyPhantom(): void {
    spriteSelect(67);
    outputText("", true);
    // defeat by damage)
    if (monster.HP < 1) {
...
// (let her in)
function littlePigLittlePigLetMeIn(): void {
    spriteSelect(67);
    outputText("", true);
...
function sharkbustersVictory(): void {
    spriteSelect(67);
    outputText("", true);
    ...
    if (player.breastRows.biggestTitSize() >= 1) outputText(", " + allBreastsDescript(player) + " squishing and squashing against her own barely-clad bosom", false);
...
function sharkyEctoginas(): void {
    ...
    // [if breasts present]
    if (player.breastRows.biggestTitSize() >= 1) outputText("and caking your cleavage with the excess lubrication", false);
    outputText(". After the cloud of arousal disperses from the ghost-girl's normally intelligent mind, she can't help but notice how uncomfortable you must be, squirming about with her gigantic pinky shoved up your " + vaginaDescript(player, 0) + ".\n\n", false);
...
function ghostIzmaPenis(): void {
    spriteSelect(67);
    outputText("", true);
    outputText("You swiftly disrobe and lay your " + player.armorName + " to the side, revealing your stiffening " + multiCockDescriptLight(game.player) + " in all of its glory. The ghost-girl's eyes brighten at the sight, and she even allows herself a little swoon (which looks absolutely silly, given her current tough shark-like vessel) as she regards your manhood.  ", false);
    outputText("She staggers forward, dropping to her knees in front of you", false);
    // [if testicles present]
    if (player.balls > 0) outputText(" and cradling your " + ballsDescriptLight(player) + " with almost reverent gentleness, running her coarse shark-skinned fingers across your sack as gently as she can manage", false);
    outputText(".\n\n", false);
...
function ghostGooGurlzDuckfaces(): void {
    spriteSelect(67);
    outputText("", true);
    ...
    else if (player.cocks.length > 0) slimeyPenorsAhoy();
    else slimeGinaFlation();
}
// Slime x Ghost Girl - Vagina Scene (Female Inflation)
function slimeGinaFlation(): void {
    spriteSelect(67);
    // [if vagina]
    ...
    if (player.breastRows.biggestTitSize() < 1) outputText("Your frame is jostled about as the slime under you shrinks.", false);
    else outputText("Your frame is jostled about as the slime under you shrinks, your goo-filled gut pushing your " + allBreastsDescript(player) + " up into your overburdened jaw.", false);
    outputText("\n\n", false);
    ...
    if (player.breastRows.biggestTitSize() < 1) outputText("see your " + nippleDescription(player, 0) + " wiggling sporadically.", false);
    else outputText("see your " + allBreastsDescript(player) + " trembling, " + nippleDescription(player, 0) + " twitching eagerly.", false);
...
function slimeyPenorsAhoy(): void {
    spriteSelect(67);
    ...
    outputText("She falls into spell-chanting- a strange-sounding thing, considering her current body- and you're forced to wait impatiently until her incantation is complete. She ends the chant with a gesture towards you, cluing you in on the recipient of the spell. With a wry smile, you spread your arms, and she happily accepts the offer. Neither of you waste time with foreplay, both of you too aroused to care about much at all. The spectral slime girl happily hops up and impales herself upon your " + cockDescript(game.player, x) + ". She twitches eagerly as she wraps her legs around your waist, pinning herself to your still-standing frame", false);
    // ([if boobs present]
    if (player.breastRows.biggestTitSize() < 1) outputText(" while squishing her small boobs into your own " + allBreastsDescript(player), false);
    ...
    if (player.balls > 0) outputText("Your " + legs(player) + " are forced apart by your burgeoning cum-sack", false);
...
function genderlessGhostBuster(): void {
    ...
    if (player.breastRows.biggestTitSize() < 1) outputText("two new additions bud on your chest.", false);
    else if (player.breastRows.biggestTitSize() <= 8) outputText("your " + allBreastsDescript(player) + " begin to bounce all around.", false);
    else outputText("your " + allBreastsDescript(player) + " swell slightly to make room for a sudden increase in milk production.", false);
    outputText("As the ghost continues darting around the room, ", false);
    // [if flat to E-cup]
    if (player.breastRows.biggestTitSize() <= 8) outputText("your new EE-cup breasts settle", false);
    ...
    if (!player.breastRows.hasFuckableNipples()) outputText("new ", false);
    ...
    if (player.breastRows.biggestTitSize() < 1) outputText("your tits, along with the elongated nipples, receding back into a flat chest.", false);
    else if (player.breastRows.biggestTitSize() <= 8) outputText("your boobflesh shrinking back down into its normal size, nipples included.", false);
...
// Penis Scene
function penisLossThatIsntALoss(): void {
    spriteSelect(67);
    ...
    if (player.inte < 50) outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
    else if (player.inte < 90 || player.cocks.cockThatFits(80) == -1) outputText("You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
    else {
        x = player.cocks.cockThatFits(80);
        ...
        outputText("She moves to leave in a huff, but you stop her with a single telepathic demand. She's yours, you remind her, and since she's gone through the trouble of trying to get you off, she might as well finish the job. Even as her lips purse in a stubborn pout, a dominating wave of influence sweeps over her, and regardless of herself, she begins unsteadily moving towards you. Resolving to meet halfway, you stride over to her and push her to your knees. As she watches, you shed your " + player.armorName + " and hoist your " + cockDescript(game.player, x) + " up to her lips, leaving a fairly clear implication of your expectations. The ghost girl's eyes widen at the sight of your manhood, and she cringes as a new wave of arousal sweeps through her. She gives a little whimper at her inability to quell her overwhelming libido, but you simply laugh mockingly and gesture at your maleness.  \"<i>Ghost blowjob,</i>\" you mutter snidely.\n\n", false);

        outputText("You don't even give her the chance to act of her own volition, simply reaching out, taking ahold of her head, and slamming her onto your " + cockDescript(game.player, x) + ".  She takes it surprisingly well, unsurprisingly easily, taking most of the shaft in the first thrust alone.  With not as much as even a sputter, she impatiently double-handedly grabs your " + buttDescription(player) + " and thrusts her head forward.  Before you know it, your entire " + cockDescript(game.player, x) + " is wrapped in a fleshy tunnel of love.", false);
        if (player.cocks.length > 1) {
            outputText("  Mindful of ", false);
            if (player.cocks.length == 2) outputText("your other dick", false);
            else outputText("the rest of " + multiCockDescriptLight(game.player), false);
            outputText(", the mentally dominated girl quickly clutches ", false);
            if (player.cocks.length == 2) outputText("your other shaft and pumps it vigorously.", false);
            else if (player.cocks.length == 3) outputText("your other two shafts and pumps them vigorously.", false);
        ...
        if (player.cocks.length == 2) outputText("Taking her hand away from your other dick, the ", false);
        else if (player.cocks.length >= 3) outputText("Taking her hands away from your other dicks, the ", false);
...
function letShouldraIn(): void {
    spriteSelect(67);
    outputText("", true);
    outputText("The ghost girl squeals in excitement as you allow her partial control- only enough for her to feel your penis as if it were her own.  With an ecstatic, victorious scream, she impales herself upon your dick, taking the entire thing down her throat like it's nothing.  Your jet-stream of cum rockets into her gob, blasting her head straight off your dick to slam into your " + chestDesc(game.player) + " with an \"<i>Oof.</i>\"\n\n", false);
    ghostBJFinisher(true);
}
// deny her
function kickShouldraOut(): void {
    spriteSelect(67);
    outputText("", true);
    outputText("Reminding her that this is a punishment for underestimating you, you firmly deny her request for entry.  Her bright yellow eyes glaze over in her disappointment, but she dare not go against your wishes.  The ghost girl turns back around and mechanically goes back to her work.  As soon as she slides her puckered lips down your twitching shaft, however, you're already past your point of endurance, and a jet of cum lifts her head right off of your dick.  The back of her head slams into your " + chestDesc(game.player) + " with an \"<i>Oof.</i>\"\n\n", false);
    ghostBJFinisher(false);
}
// back to this thing okay
function ghostBJFinisher(inside: boolean): void {
    spriteSelect(67);
    const x: number = player.cocks.cockThatFits(80);
    outputText("Like a backed up geyser, your gigantic testicles push hot, thick strands of seed out of your belaboured " + cockDescript(game.player, x) + " straight up in the air.  The cum splatters on both you and the ", false);
...
// Vagina Scene
function ghostGinaWinSexings(): void {
    ...
    outputText("Against your will, and against all of your better instincts, the ghost-girl, as aroused as you are uncomfortable, moves both hands to your sex, gripping it and giving it a squeeze. Your entire body locks up, causing you to fall backwards onto the pavement as a devastating orgasm rocks through your system. A strangled cry is the only reaction you can give to the explosive sensations. The ghost-girl, however, not satisfied with the suddenness of the climax, begins feverishly stroking your clitoris with one hand, the other reaching into your love-tunnel and stimulating the rosy flesh inside. You arch your back and attempt to dig small holes into the road with your " + feet(player) + " as shock-waves run through your system over and over, never giving you a chance to recuperate. You hear, as if from a great distance, a mix of frenzied laughter and orgasmic cries of pleasure ringing inside your head. Finally, mercifully, you fade into unconsciousness, leaving the ghost girl to her constant stimulation.\n\n", false);

    outputText("You awake some time later in a prodigious puddle of girly spunk. Your attempt to sit up is impeded by a clunk against the ground.  A clunk? You prop yourself up on your elbows and glance down to see a large metal pipe, stuck in your vagina presumably some time during your black-out. Reaching down, you extricate a full ten inches of piping from your cunt, and shake your head at the tenacity of your unworldly bodily invader. You are happy to feel a tightening in your crotch, however, as the magic finally wears off, returning your womanliness to a " + vaginaDescript(player, 0) + ". You also note the absence of your gigantic clitoris, and give yourself a little stroke to confirm; it's reverted to normal size as well. All's well that ends well, you suppose.\n\n", false);
    outputText("Her pleasure dealt with, the ghost girl appears head-first from your belly, flowing out and reconstituting herself into the normal, brown-eyed girl you first encountered.  Almost as an afterthought, she smiles, moving to her clothing and swiftly dressing, then gives you a little wave before wandering away, rounding a corner and disappearing from sight. You resolve to find her again- her magic was pretty fun!", false);
...
// Hermaphrodite Scene
function hermaphroditeGhostsCumEctoplasm(): void {
    spriteSelect(67);
    let x: number = player.cocks.biggestCockIndex();
    outputText("", true);
    ...
    outputText("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ", false);
    if (player.inte < 50) outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.", false);
    else if (player.inte < 90 || player.cocks.cockThatFits(80) == -1) outputText("You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts for just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
    else {
        x = player.cocks.cockThatFits(80);
        outputText("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n", false);
    ...
    outputText("Taking complete control of your body as if it was never actually yours, she removes your " + player.armorName + " and throws them carelessly to the ground. Grabbing up your " + cockDescript(game.player, x) + " in both hands and fondling it awkwardly, she strikes you as obviously inexperienced with the male anatomy, despite her levels of intelligence. She continues to grope your lengthening member, admitting, \"<i>Even with the rare male bodies I have possessed, I rarely got to play with their...</i>\"\n\n", false);
    ...
    if (player.balls > 0) outputText("Stopping her double-handed stroke technique, she slips your hand under your obtrusive " + ballsDescriptLight(player) + " and gently pushes them aside to get a better look at your previously unnoticed female sex.  ", false);
    outputText("She chuckles malevolently, and you groan, mind cycling through the possible outcomes of the girl's impulses. Your lips curl unbidden in an evil smile, then begin mouthing an archaic incantation. You wince inwardly, bracing for the worst. She finishes and goes back to the vigorous stroking, and you are left to wonder what the spell was meant to do- that is, until you feel a tingling beginning at the base of your " + cockDescript(game.player, x) + ", slowly climbing up the shaft until your entire length is covered in the sensation.\n\n", false);
...
// Loss Introduction
export function loseToShouldra(): void {
    spriteSelect(67);
    outputText("", true);
    ...
    if (player.gender == 1) {
        ...
        else if (player.cocks.biggestCockArea() >= 200) shouldraGiantCockLoss();
...
// Penis Scene
// [If you have a penis]
function ourDadTaughtUsNotToBeAshamedOfOurDicks(): void {
    spriteSelect(67);
    const x: number = player.cocks.biggestCockIndex();
    // [first encounter]
    ...
    outputText("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your " + player.armorName + " and throwing them carelessly to the ground. She takes your " + cockDescript(game.player, x) + " in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  \"<i>Most of my hosts have been women,</i>\" she explains as she continues to grope your lengthening member.  \"<i>And even then, I rarely got to play with their...</i>\"\n\n", false);
...
// Penis With Worm Infestation Scene
function loseToShouldraWithWorms(): void {
    spriteSelect(67);
    // [first encounter]
    if (flags[kFLAGS.SHOULDRA_WORM_SCENE_COUNTER] == 0) outputText("She abruptly gasps in excitement, ", false);
    else outputText("She 'ooo's softly as she recalls your anatomy, ", false);
    outputText("and you feel your hands moving unbidden towards your crotch. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes control. She heaves your body off the ground, removing your " + player.armorName + " and throwing them carelessly to the ground. She takes your " + cockDescript(game.player, 0) + " in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy. \"<i>Most of my hosts have been- WHAT THE FUCK!?</i>\"\n\n", false);
...
function ghostGinaLosses(): void {
    ...
    outputText("Against your will, and against all of your better instincts, the ghost-girl, as aroused as you are uncomfortable, moves both hands to your sex, gripping it and giving it a squeeze. Your entire body locks up, causing you to fall backwards onto the pavement as a devastating orgasm rocks through your system. A strangled cry is the only reaction you can give to the explosive sensations. The ghost-girl, however, not satisfied with the suddenness of the climax, begins feverishly stroking your clitoris with one hand, the other reaching into your love-tunnel and stimulating the rosy flesh inside. You arch your back and attempt to dig small holes into the road with your " + feet(player) + " as shock-waves run through your system over and over, never giving you a chance to recuperate. You hear, as if from a great distance, a mix of frenzied laughter and orgasmic cries of pleasure ringing inside your head. Finally, mercifully, you fade into unconsciousness, leaving the ghost girl to her constant stimulation.\n\n", false);
    outputText("You awake some time later in a prodigious puddle of girly spunk and attempt to slowly sit up, only to be impeded by a clunk against the ground.  A clunk?  You prop yourself up on your elbows and glance down to see a large metal pipe, stuck in your vagina presumably some time during your black-out. Reaching down, you extricate a full ten inches of piping from your cunt, and shake your head at the tenacity of your unworldly bodily invader. You are happy to feel a tightening in your crotch, however, as the magic finally wears off, returning your womanhood back to a " + vaginaDescript(player, 0) + ". You also note the absence of your gigantic clitoris, and give yourself a little stroke to confirm; it's reverted to normal size as well. All's well that ends well, you suppose.\n\n", false);
...
// Hermaphrodite Scene
function loseToShouldraAsHerm(): void {
    spriteSelect(67);
    ...
    outputText("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your " + player.armorName + " and throwing them carelessly to the ground. She takes your " + cockDescript(game.player, 0) + " in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  \"<i>Most of my hosts have been women,</i>\" she explains as she continues to grope your lengthening member.  \"<i>And even then, I rarely got to play with their...</i>\"\n\n", false);
    ...
    if (player.balls > 0) outputText("Stopping her double-handed stroke technique, she slips your hand under your obtrusive " + ballsDescriptLight(player) + " and gently pushes them aside to get a better look at your previously unnoticed female sex.  ", false);
    outputText("She chuckles evilly, and you groan, mind cycling through the possible outcomes of the girl's impulses. Your lips curl unbidden in an evil smile, then begin mouthing an archaic incantation, and you wince inwardly, bracing for the worst. She finishes and goes back to the vigorous stroking, and you are left to wonder what the spell was meant to do—that is, until you feel a tingling beginning at the base of your " + cockDescript(game.player, 0) + ", slowly climbing up the shaft until your entire length is covered in the sensation.\n\n", false);
...
// Genderless Scene
function genderlessShouldrasLossRapes(): void {
    spriteSelect(67);
    outputText("Excited to find out what manner of host you are, the ghost-girl swiftly assumes complete control of your body and cups a hand against your crotch to feel... nothing. Confused, she quickly wriggles out of your " + player.armorName + " and goes in for another grope, feeling nothing but skin. \"<i>Son of a... please don't tell me you don't have any sexual bits I can play with!</i>\" she huffs, and you chuckle sheepishly, not knowing what to tell her. She groans unhappily and falls back onto your " + buttDescription(player) + ", muttering a string of curse words under your breath. \"<i>That's it!</i>\" she fumes. \"<i>I'm going to get off, whether you want me to or not!</i>\" She resumes cursing and wiggling your hips for a bit longer, then stops suddenly, a smirk playing across your face. \"<i>Whether you want me to or not...</i>\"\n\n", false);
    ...
    else outputText("your " + allBreastsDescript(player) + " swells slightly to make room for a sudden increase in milk production.", false);
    outputText("  As the ghost continues darting around the room, ", false);
    if (player.breastRows.biggestTitSize() <= 8) outputText("your new EE-cup breasts settle", false);
...
// Penis With Exgartuan Scene
function ghostGartuanLossSceneOrSomeShit(): void {
    spriteSelect(67);
    const x: number = player.cocks.biggestCockIndex();
    outputText("Your entire body shivers in delight as the ghost girl regards your excessively large " + cockDescript(game.player, x) + ". She places your hand over the bulge in your " + player.armorName + " and titters in glee as your member twitches and hardens, just a tad. She strips eagerly, but is halted by a familiar booming voice coming from the half-erect member. \"<i>WHO THE HELL IS THIS BITCH?</i>\" a perplexed Exgartuan yells grumpily. \"<i>Living in peoples' bodies is MY THING!</i>\"\n\n", false);
...
function superCoolPaladinSexStuffDudeGasmJizzMonkies(): void {
    spriteSelect(66);
    outputText("", true);
...
// (yup)
function courtCrazyGirlsDotCom(): void {
    spriteSelect(66);
    outputText("", true);
...
// (two days later)
export function paladinModeFollowup(): void {
    spriteSelect(67);
    outputText("\n", false);
    ...
    if (game.time.hours < 6 || game.time.hours >= 23) outputText("\n\nYou rage fap and consider going back to bed.", false);
    player.orgasm();
    doNext(playerMenu);
}
...
function slimeGhostGalForHerms(clearOut: boolean = false): void {
    ...
    if (player.breastRows.biggestTitSize() > 0) outputText("both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.");
    ...
    if (game.time.hours < 11) outputText("morning");
    else if (game.time.hours < 17) outputText("afternoon");
