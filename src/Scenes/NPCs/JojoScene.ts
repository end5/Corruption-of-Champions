...
function jojoMutationOfferYes(): void {
    ...
    // [Female]
        ...
        if (player.breastRows.biggestLactation() < 2) outputText(" while your tits cover his head, nearly suffocating him", false);
...
function mutateJojo(): void {
    ...
    if (player.breastRows.biggestTitSize() < 1) outputText("bodies", false);
    ...
    if (player.vaginas.length > 0) outputText("caressing your aching twat ", false);
...
export function useTentacleJojo(): void {
    ...
    const clitSucking: boolean = false;
    ...
    outputText("You drop it, letting it smack into the floor with a wet-sound 'SPLAT' that creates a puddle of pre-cum on the ground.  With your hands free, it is easy to quickly remove your " + player.armorName + " and toss it back towards your tent.  By the time you turn back around, his snake-like member is curling up your thigh, towards your " + assholeOrPussy(player) + ".  ", false);
    if (player.vaginas.length > 0) outputText("You feel it split in half, dividing in order to take you from both ends.  ", false);
    ...
    outputText("Looking very much like a starving man looking at a steak, Jojo smiles serenely and lashes out with his shoulder-tentacles, grabbing your arms and pinning them to your waist.  They wrap about your middle and lift you off the ground with ease.  Suspended in front of him, you're effortlessly rotated in place until your " + assholeDescript(player) + " is in the air and your legs splayed apart, with the mutant-mouse's primary tentacle getting ever closer to your " + assholeDescript(player), false);
    if (player.vaginas.length > 0) outputText(" and " + vaginaDescript(player, 0), false);
    outputText(".\n\n", false);
    // REAL SECKS STARTS HERE
    if (player.cocks.length > 0) {
        outputText("Upside down, you're forced to watch as his tail-tentacles curl out from between his legs, and extend towards your vulnerable " + multiCockDescriptLight(game.player) + ".  All three of them pause a few inches away for what seems an eternity, revealing circular dripping orifices filled with slimy wriggling cilia.\n\n", false);
        if (player.cocks.length == 1) {
            // (1dick+balls =
            if (player.balls > 0) {
                outputText("One of them lashes forwards, ", false);
                if (player.cocks.cockArea(0) > 60) outputText("laboriously stretching around", false);
                else if (player.cocks.cockArea(0) >= 20) outputText("engulfing", false);
                else outputText("easily swallowing", false);
                outputText(" your " + cockDescript(game.player, 0) + " in a tight vise of pleasure.  The other two tentacles slide under, each taking a " + ballDescript(player) + " partially inside their 'mouths' and massaging them softly.", false);
                outputText("\n\n", false);
            }
            // (1 dick no balls =
            else {
        ...
        if (player.cocks.length == 2) {
            ...
            if (player.balls > 0) {
                ...
                outputText(" your " + multiCockDescriptLight(game.player) + " in tight vises of pleasure.  The remaining tentacle slides underneath, alternatively taking each of your " + ballsDescript(player) + " partway into its 'mouth' and massaging them with its thousands of wriggling stimulators.\n\n", false);
            }
            // (2 dicks no balls)
            else {
                outputText("Two of them lash forward, ", false);
                ...
                outputText(" your " + multiCockDescriptLight(game.player) + " in tight vises of pleasure.  The remaining tentacle slides underneath, seeking ", false);
                if (player.vaginas.length > 0) {
                    outputText("out your " + clitDescription(player) + " and sucking it inside.", false);
                    // (If big clit –
                    if (player.clitLength >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.", false);
                }
        ...
        if (player.cocks.length >= 3) {
            ...
            if (player.cocks.cockArea(0) > 60) outputText("laboriously working to swallow", false);
            else if (player.cocks.cockArea(0) >= 20) outputText("engulfing", false);
            else outputText("easily swallowing", false);
            outputText(" your " + multiCockDescriptLight(game.player) + " in tight vises of pleasure.  The sensation is overwhelmingly pleasurable, like having three cocks being licked and sucked by a thousand tongues.", false);
            // (if extra:
            if (player.cocks.length > 3) outputText("  The remaining " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "'s pulse and throb with jealous desire, until one of the tentacles slips off and captures it.  They take turns, slipping off and capturing each of your offerings until you wish they'd just keep sucking forever.", false);
            outputText("\n\n", false);
        }
    }
    // (0 dicks but clit)
    if (player.cocks.length == 0 && player.vaginas.length > 0) {
        outputText("One of Jojo's tail-tentacles slides underneath his body, shooting forwards to seek out your " + clitDescription(player) + ".  It pauses an inch or two away and opens up a horrific orifice full of wriggling tentacles and slick fluids.  The next moment it lunges forwards and sucks it inside.", false);
        // (If big clit –
        if (player.clitLength >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.", false);
    ...
    // Virgin taking
    if (player.vaginas.hasVirginVagina() || player.ass.analLooseness == 0) {
        outputText("barely slipping inside your virginal opening", false);
        if (player.vaginas.hasVirginVagina() && player.ass.analLooseness == 0) outputText("s", false);
    }
    // not virgin taking
    else {
        // With cunt
        ...
        // Just butt
            ...
            if (player.ass.analLooseness >= 4) outputText(" practically falling into your oversized cum-receptacle", false);
                ...
                else outputText("forcing its way inside your " + assholeDescript(player), false);
            }
        }
    }
    // ON WITH THE PENETRATION
    outputText(".  Your " + mouthMuzzle() + " opens with a wordless cry of surprise ", false);
    if (player.vaginas.length > 0) outputText("and pleasure ", false);
    outputText("as Jojo's tentacle", false);
    if (player.vaginas.length > 0) outputText("s begin", false);
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.length == 1) outputText("  The tentacle on your " + multiCockDescriptLight(game.player) + " seems appreciative, and begins sucking to swallow down all your pre.", false);
        if (player.cocks.length > 1) outputText("  The tentacles on your " + multiCockDescriptLight(game.player) + " seem appreciative, and begin sucking to swallow down all your pre.", false);
    }
    if (player.vaginas.length > 0) outputText("  Your " + vaginaDescript(player, 0) + " spasms around the invader, clenching uncontrollably as it's plowed by the thick vine-prick.", false);
    outputText("\n\n", false);
    outputText("You feel the bindings around your waist sliding and twisting to free up some of their length.  The pair of tentacle-cocks rear up before your " + allBreastsDescript(player) + " like hungry snakes eyeing a mouse.\n\n", false);
    ...
    // (NEITHER - JUST NIPS)
    else if (player.breastRows.biggestTitSize() < 4 && rand(2) == 0) {
        nippleSucking = true;
        outputText("The heads split apart, dividing into quarters and revealing a wet pink opening with a writhing tongue in the center.  They latch on and suck hard, each pulling your entire " + nippleDescription(player, 0) + " into their interior.  Gasping at the sensations, you can only moan as the tentacular tongue twists itself around your nipple.  ", false);
        if (player.breastRows.biggestLactation() >= 1) outputText("A squirt of milk escapes and is quickly sucked down by the hungry tentacles.  ", false);
    ...
    else if (player.breastRows.biggestTitSize() < 4 && player.breastRows.biggestLactation() < 2) {
        mouthFucking = true;
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.cocks.length <= 2 && player.balls == 0) {
            outputText("The tentacle locked around your " + clitDescription(player) + " bulges out a bit as your clit throbs from the orgasm, before it sucks harder, making your clitty even larger.  You squeal from the raw sensation until the tentacle pops off, satisfied, leaving your clit looking like an angry red cock.  ", false);
            player.clitLength += .25;
        }
        outputText("Both tentacles pull out, still dripping whiteness that puddles in your holes.", false);
    }
    else {
        outputText("tentacle in your " + assholeDescript(player) + " thickens perceptibly.  Bulges of cum work their way down the main tentacle, spreading your " + assholeDescript(player) + " wide before stuffing you full of mouse cream.  You can feel your bowels filling, expanding you until you look mildly pregnant.  Your body spasms around them, locked in the throes of orgasm.  The tentacle pulls out, still dripping whiteness that puddles in your well-used hole.", false);
    }
    // Dick milkings ahoy!
    if (player.cocks.length > 0) {
        if (player.cocks.length == 1) {
            outputText("The tentacle working over your " + multiCockDescript(game.player) + " clamps down and wriggles deliciously around you.  You can't help but cum from the assault, feeding the tentacle your hot spoogy load.  It drinks it down eagerly", false);
            if (player.cumQ() >= 100) {
        ...
        if (player.cocks.length > 1) {
            outputText("The tentacles working over your " + multiCockDescript(game.player) + " clamp down and wriggle deliciously around you.  You can't help but cum from the assault, feeding the tentacles your hot spoogy load.  They drink it down eagerly", false);
            if (player.cumQ() >= 100) {
    ...
    if (player.breastRows.biggestLactation() >= 2 && nippleSucking) {
        ...
        outputText("  Your " + allBreastsDescript(player) + " finally feel emptied; it's a relief.\n\n", false);
        milkPuddle = true;
    }
    // Titfucking breastgasm
    if (titFucking) {
        outputText("The titfucking tentacles squeeze tighter against your " + allBreastsDescript(player) + " before they cum again, much harder than before.  Each time it pops free of your flesh, a wave of spunk flows out with it, rolling off your chest to drip into your already soaked face and mouth.  ", false);
        ...
        if (player.hairLength > 0) outputText("He keeps unloading it until you feel it in your " + hairDescription(player), false);
...
function milkJojoFirst(): void {
    ...
    if (player.cocks.length > 0) outputText("It starts to turn your way before it stops and rises, seeming to sniff at the rodent directly above.", false);
    ...
    if (player.cocks.length > 0) {
        // (Too big dick:
        if (player.cocks.cockThatFits(40) < 0) outputText("You release your cum all over Jojo's back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It's a good look for him.", false);
    ...
    else if (player.vaginas.length > 0) outputText("You climax and clamp down on your fingers, squeezing your hand as your own cries of pleasure mix with your cock-toy's milked-out moans.", false);
...
function repeatMilkJojo(tentacle: boolean = false): void {
    ...
    // TENTACLE JOJO MILKING (edited)
        ...
        outputText("Jojo sags down in the harness while his members finish releasing their load.  The leftovers from his orgasm easily fill the stall with at least a foot of cum.  You sigh at the realization that both you and your clothes are utterly drenched.  The harness unceremoniously dumps Jojo into the puddle of his own making, and as he rises, he hugs your " + legs(player) + " and says, \"<i>Thank you.  Did... did I cum enough?  I'll do more next time, I promise!</i>\"\n\n", false);
...
function amilyTeachingJojoBJ(): void {
    ...
    if (player.cocks.length > 0) outputText(" Do you surprise them both by allowing Amily to demonstrate on you?");
    else outputText(" Do you want to watch them fuck?");

    menu();
    if (player.cocks.length > 0) addButton(0, "Demonstrate", BJRidesGETYOUONE);
...
function BJRidesGETYOUONE(): void {
    ...
    if (player.cocks.biggestCockArea() > 50)
...
function amilyAndJojoFuck(): void {
    ...
    if (player.vaginas.length > 0) outputText(" It isn't until you notice how wet your crotch is that you come to the realization that you've masturbated yourself through a few orgasms as well.");
...
export function jojoFollowerMeditate(): void {
    ...
    if (player.effects.getValue1Of(StatusAffects.Meditated) > 0) {
        ...
        if (player.cor > 80)
            ...
            cleanse -= 1;
        dynStats("cor", cleanse - player.cocks.countCockSocks("alabaster"));
...
export function defeatedJojo(hpVictory: boolean): void {
    ...
    if (player.lust > 33 && player.gender > 0) {
        ...
        doYesNo(postCombatRape, cleanupAfterCombat);
    }
    else {
        finishCombat();
    }
...
function jojosFirstRape(): void {
    ...
    if (player.gender == 1) {
        ...
        if (player.cocks.length == 1) {
            outputText("You grin and press your " + cockDescript(game.player, 0) + " against him, making him squeal in protest.  You press on, eager to violate his puckered asshole, reveling in the crushing tightness.  His muscles quiver nervelessly as you pound him raw, his muted protests getting weaker as you notice a rapidly swelling bulge under him.  You reach around and begin jerking him off as you fuck him, fantasizing about pouring him full of corruptive demon power, making him your slave.  The dirty thoughts make your balls feel full; a pulsing, squeezing tightness builds in your nethers as your " + cockDescript(game.player, 0) + " flexes and bulges inside your prey.  You cum hard, pressing his muzzle into the dirt as you pump glob after glob of cum up his ass, violating him to his core.\n\n");
        ...
        if (player.vaginas.wetness() < 2) {
            ...
            outputText("but the feeling of your " + vaginaDescript(player, 0) + " grinding against his hardening cock robs him of any will.  ");
        }
...
function jojosSecondRape(): void {
    ...
    if (player.gender == 1) {
        ...
        if (player.cocks.averageCockThickness() >= 3) outputText("It makes little difference to your " + cockDescript(game.player, 0) + ", he still feels tight as a virgin's cunt to you.  ");
        outputText("You grab him by the waist and fuck him powerfully, slamming your " + cockDescript(game.player, 0) + " in brutally hard and fast.  You notice his hands are in his crotch, feverishly masturbating his disproportionately large cock like a slut.  You start leaking pre-cum like a sieve, realizing you're doing it, really doing it - making this virtuous mouse into a wanton slut!  You squeeze him tightly as you cum into his bowels, his belly distending slightly as your orgasm goes on and on.  Trails of cum run down his fur as it becomes more than his ass can handle.");
        if (player.cocks.length > 1) outputText("  Your remaining equipment showers him with jizz, more than you ever thought you could produce.");
    ...
    if (player.gender == 2 || player.gender == 3) {
        ...
        if (player.breastRows.biggestLactation() >= 1 && player.breastRows.biggestLactation() < 2) outputText("Milk squirts from  your nipples, spraying him down with small droplets of your creamy tit-treat.  ");
        if (player.breastRows.biggestLactation() >= 2 && player.breastRows.biggestLactation() < 3) outputText("Streams of milk spray from your nipples in time with your pussy contractions, hosing the poor mouse down and puddling on his fur.  ");
        if (player.breastRows.biggestLactation() >= 3) outputText("Gouts of milk erupt from your nipples, spraying continually as you cum.  The poor mouse is soaked from head to toe, your cream utterly drenching the monk.  ");
        if (player.vaginas.averageVaginalWetness() == 5) outputText("Your " + vaginaDescript(player, 0) + " drenches him with your squirting girl-cum, mixed with his own seed.");
        if (player.cocks.length == 1) outputText("Jizz sprays onto his chest from your " + cockDescript(game.player, 0) + ".  ");
        if (player.cocks.length == 2) outputText("A hail of jizz splatters over Jojo from your " + multiCockDescriptLight(game.player) + ".  ");
        if (player.cocks.length == 3) outputText("A multitude of thick cum-streams splatter over Jojo from head to waist as your " + multiCockDescriptLight(game.player) + " hose him down.  ");
...
function jojosThirdRape(): void {
    ...
    if (player.gender == 1) {
        ...
        if (player.cocks.length == 1) {
            outputText("presenting your " + cockDescript(game.player, 0) + " to him.  ");
        }
        else outputText("presenting your " + multiCockDescriptLight(game.player) + " to him.  ");
        ...
        if (player.cocks.length == 1) outputText("to ram your " + cockDescript(game.player, 0) + " deep into his maw.  You roughly grab his ears, facefucking him hard, his tongue working hard to please.  ");
        if (player.cocks.length > 1) outputText("to cram two cocks deep into his maw, making his jaw stretch obscenely.  You roughly grab his ears, facefucking him hard, his tongue working hard to please you.  ");
        outputText("One of Jojo's paws is buried in his groin, stroking his " + cockDescriptShort(monster, 0) + " with feverish intensity.  The hornier he gets, the more his throat seems to relax, allowing you to push deeper.  The glazed, lust-addled look on his face is so hot, you can't hold back any longer.  ");
        if (player.cocks.length == 1) outputText("Your " + cockDescript(game.player, 0) + " clenches tightly, erupting ");
        if (player.cocks.length > 1) outputText("Your twin dicks clench tightly, erupting ");
    ...
    if (player.gender == 2) {
        ...
        outputText(", beckoning him towards your " + vaginaDescript(player, 0) + ".  He looks disheartened, but obediently kneels before you, his whiskers tickling your " + vaginaDescript(player, 0) + ", his wet nose bumping your clit, his tongue taking long licks between your lips.  ");
        if (player.breastRows.biggestTitSize() >= 2) {
            outputText("You sigh and knead your breasts in pleasure.  ");
        }
        if (player.breastRows.biggestLactation() >= 1.5 && player.breastRows.biggestTitSize() > 2 && player.breastRows.mostBreastsPerRow() >= 2 && player.breastRows.length >= 1) outputText("Every sensual peak within you is mirrored with small spurts of milk from your nipples.  It eventually trickles down to Jojo's tongue, spurring his efforts on.  ");
        ...
        if (player.vaginas.averageVaginalWetness() > 1 && player.vaginas.averageVaginalWetness() <= 3) outputText("creaming his tongue liberally with girlcum.");
        if (player.vaginas.averageVaginalWetness() == 4) outputText("creaming all over him with your slobbering pussy.");
        if (player.vaginas.averageVaginalWetness() == 5) outputText("splattering him with girlcum from your " + vaginaDescript(player, 0) + ".");
    ...
    if (player.gender == 3) {
        ...
        outputText(", beckoning him towards your " + vaginaDescript(player, 0) + ".  He looks disheartened, but obediently kneels before you, his whiskers tickling, his wet nose bumping your clit, his tongue taking long licks between your lips.  ");
        if (player.cocks.length == 1) outputText("You sigh as your " + cockDescript(game.player, 0) + " droops over his head.  ");
        if (player.cocks.length > 1) outputText("You sigh as your " + multiCockDescriptLight(game.player) + " pile atop his head.  ");
        if (player.breastRows.biggestTitSize() >= 2) {
        ...
        if (player.vaginas.averageVaginalWetness() <= 1) outputText("squeezing him tightly into your " + vaginaDescript(player, 0) + ".");
        if (player.vaginas.averageVaginalWetness() > 1 && player.vaginas.averageVaginalWetness() < 4) outputText("creaming his tongue liberally with girlcum.");
        if (player.vaginas.averageVaginalWetness() == 4) outputText("creaming all over him with your slobbering pussy.");
        if (player.vaginas.averageVaginalWetness() >= 5) outputText("splattering him with you girlcum from your " + vaginaDescript(player, 0) + ".");
        if (player.cocks.length > 0) outputText("  Thick runners of your pre dribble down his neck, sticking to his fur.  ");
        outputText("\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,");
        if (player.isBiped()) outputText(" spreading your legs and");
        outputText(" shaking your rump.  ");
        if (player.hasLongTail() > 0) outputText("Your tail swishes back and forth teasingly.  ");
...
function jojosFourthRape(): void {
    ...
    if (player.gender == 1) {
        ...
        if (player.cocks.length == 1) outputText("You touch his head softly and guide him to your " + cockDescript(game.player, 0) + ", smiling as he licks his lips with anticipation.  You gasp at the feeling of his soft lips and wet tongue as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take as much of you as possible into his mouth, panting warmly and wetly around your " + cockDescript(game.player, 0) + ".  You twitch in pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ");
        else outputText("You touch his head softly and guide him to your " + multiCockDescriptLight(game.player) + ", smiling as he licks his lips in anticipation.  You gasp at the feelings of his soft lips and wet tongue on your heads as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take in two shafts, struggling to swallow them as deep as possible, panting wetly around you.  You twitch with pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ");
        // Too thick for him
        if (player.cocks.averageCockThickness() > 3 || (player.cocks.dogCocks() > 0 && player.cocks.averageCockThickness() > 3)) outputText("Jojo is forced to back off of your " + cockDescript(game.player, 0) + " from time to time to come up for air, barely opening his jaw wide enough to take your girth back inside his mouth.  ");
        // Too long for him
        if (player.cocks.averageCockLength() > 10 || (player.cocks.horseCocks() > 0 && player.cocks.averageCockLength() > 10)) outputText("He struggles not to gag on your " + cockDescript(game.player, 0) + "'s length, opening his throat as far as he can.  ");
    ...
    if (player.gender == 2 || player.gender == 3) {
        ...
        outputText("You touch his head softly, guiding him to your " + vaginaDescript(player, 0) + ", lips breaking into a grin as he licks his mouth with desire.  You gasp at the feeling of his wet nose as it bumps against your groin, shooting thunderbolts of pleasure from your " + clitDescription(player) + ".  He attacks with his tongue, thrusting strikes practically attacking your " + vaginaDescript(player, 0) + " with his long, practically serpentine, tongue.  You shudder, knowing a visible sign of corruption when you see it, moisture slicking the mouse's face.\n\n");
        outputText("Jojo moans into your folds as his " + cockDescriptShort(monster, 0));
        ...
        if (player.vaginas.averageVaginalWetness() == 4) outputText("Your legs trap his muzzle in your " + vaginaDescript(player, 0) + ", slicking his muzzle with girlcum as you spasm with bliss.  ");
        if (player.vaginas.averageVaginalWetness() == 5) outputText("Your legs squeeze him against your " + vaginaDescript(player, 0) + ", girlcum erupting over his face and soaking him as you bliss out with orgasm  ");
        if (player.cocks.length == 1) outputText("Your " + cockDescript(game.player, 0) + " drizzles cum in his hair, some dripping off by his ear.  ");
        if (player.cocks.length > 1) outputText("Your " + cockDescript(game.player, 0) + "s drizzle him with cum, covering his head with thick white streams of your jizz.  ");
        outputText("Twitching weakly with pleasure, you fall back.\n\n");
        // Fux!
        outputText("The pleasure was magnificent... but you want MORE.  You push yourself up, feeling pleased as you spy Jojo stroking his cum-slick " + cockDescriptShort(monster, 0) + ", thick streams of cum leaking from the tip.  You pounce on him, pushing him down on the ground, and position your " + vaginaDescript(player, 0) + " over him, allowing him to scent your sex.  With a triumphant smile, you sink down onto him, impaling yourself on his " + cockDescriptShort(monster, 0) + ".  ");
        if (player.vaginas.averageVaginalLooseness() == 0) outputText("He is painfully large, so much so that you feel more pain than pleasure.  ");
        if (player.vaginas.averageVaginalLooseness() == 1) outputText("He stretches you around him like a latex glove, pulling your " + vaginaDescript(player, 0) + " taught with his " + cockDescriptShort(monster, 0) + ", the sensation riding a razor's edge between pleasure and pain.  ");
        if (player.vaginas.averageVaginalLooseness() == 2) outputText("His " + cockDescriptShort(monster, 0) + " stuffs you completely, filling your " + vaginaDescript(player, 0) + " to capacity.  ");
        if (player.vaginas.averageVaginalLooseness() == 3) outputText("His " + cockDescriptShort(monster, 0) + " fits you perfectly, burying deep inside your folds.  ");
        if (player.vaginas.averageVaginalLooseness() == 4) outputText("You easily accomadate his member into your " + vaginaDescript(player, 0) + ".  ");
        if (player.vaginas.averageVaginalLooseness() == 5) outputText("His " + cockDescriptShort(monster, 0) + " slips inside your " + vaginaDescript(player, 0) + " with little resistance, easily sinking in to the hilt.  You muse to yourself, \"<i>If only he were thicker...</i>\"  ");
        outputText("You ride him slowly, gyrating your hips in tiny grinding circles while you run your hands through his fur.  His hips bounce you gently with tiny twitching thrusts, cum pooling out of your " + vaginaDescript(player, 0) + " as it continues to drip from him.  ");
        outputText("He gradually ups the tempo, and you are forced to go along for the ride as you begin to bounce on his " + cockDescriptShort(monster, 0) + ".  You grab fistfuls of his fur and hang on as he begins pounding your " + vaginaDescript(player, 0) + ", his huge balls slapping against you.  Cum squirts from your pussy with each of his violent thrusts, more pouring deep inside you continually.  Jojo squeals with glee and slams his hips into yours a final time, triggering an eruption of seed in your channel.  You feel it pouring into your womb, slowly distending your belly with every shuddering pump of cum.  You orgasm helplessly, fingering your " + clitDescription(player) + " the whole time.  ");
        // Futacawk here
        if (player.cocks.length > 0) {
            // Single Cock
            if (player.cocks.length == 1) {
                // Horsefun!
                if (player.cocks.horseCocks() == 1) outputText("Your " + cockDescript(game.player, 0) + " feels a building pressure, the whole thing pulsating wildly with each of your heartbeats, most noticably the tip, which flares out wildly.  Powerful contractions wrack your sheath and " + cockDescript(game.player, 0) + " as pre practically fountains from it.  ");
                // DogFun!
                if (player.cocks.dogCocks() == 1) outputText("Your " + cockDescript(game.player, 0) + " feels an intense pressure, and begins bulging out obscenely above your sheath.  The knot thickens gratuitiously, filling as it pulses with need.  Cum drips from your pointed tip as it continues to bulge wider, filling you with unbearable pressure.  ");
                // Else
                if (player.cocks.normalCocks() == 1) outputText("Your " + cockDescript(game.player, 0) + " twitches, muscle contractions slowly working their way up from the base.  ");
                // CUMSPLOISION
...
export function loseToJojo(): void {
    ...
    if (game.monk == 2 || game.monk == 3) {
        ...
        if (player.cocks.length == 1) player.lib = 15;
        if (player.vaginas.length == 1) player.lib += 10;
        if (player.cocks.length > 1) player.lib += 5;
        if (player.cocks.horseCocks() > 0) player.lib += 3;
        if (player.cocks.dogCocks() > 0) player.lib += 2;
        if (player.breastRows.biggestLactation() >= 1) player.lib += 2;
        game.monk = 0;
    }
    else {
...
function corruptJojoSexMenu(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 2) addButton(6, (player.breastRows.biggestLactation() > 1 ? "Suckle" : "Breasts"), corruptJojoBreasts); // All ya need is bewbs
...
function corruptJojoBJCruel(): void {
    ...
    if (player.cocks[x].cArea() < 10)
        outputText("You grip the fur on his head tightly in one hand and pull his mouth over your " + cockDescript(player, x) + ", thrusting into his muzzle with little concern for letting him catch his breath. You shove your length down his throat and start sawing away, making the mouse's eyes roll back from breathlessness. You can feel the muscles of his throat grip and spasm around your cock flesh as he chokes on the length, his thin lips trembling around your.  ");
    else if (player.cocks[x].cArea() < 36)
        outputText("You grip the fur on his head tightly in one hand and pull his mouth over your " + cockDescript(game.player, x) + ", thrusting into his muzzle with little concern for letting him catch his breath. The girth of your " + cockDescript(player, x) + " nearly dislocates his jaw. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth.  ");
    else outputText("You grip the fur on his head tightly in one hand and pull his mouth over your " + cockDescript(player, x) + ", thrusting against his muzzle with your " + cockDescript(player, x) + ". You can feel his buck teeth scratching against the top and bottom of your " + cockDescript(player, x) + "'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your huge erection away, and since you can't fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length.  ");
    outputText("His eyes turn to you in fear and his body shudders for lack of breath, but it does nothing more than stoke the fires of your lust. You groan in pleasure as your balls draw up tight, churning with your corrupted seed, and in a rush you feed it to him, your orgasm overtaking you as surge after hot surge of cum flares through your flesh and into his throat.  " + (player.vaginas.length > 0 ? "A sympathetic orgasm hits your pussy, causing a surge of feminine juices to splash against his chest and dribble down your thighs lewdly.  " : ""));
...
function corruptJojoBJGentle(): void {
    ...
    if (player.cocks[x].cArea() < 10)
        outputText("He opens his mouth to protest, but you never give him the chance, sliding your " + cockDescript(player, x) + " between his lips and down his throat. You can feel the muscles of his throat grip and spasm around your " + cockDescript(player, x) + " as he chokes on the length, his thin lips trembling around your girth as his tongue slides across your vein-lined underside.  " + (player.breastRows.biggestTitSize() >= 2 ? "Your hands lift to massage your breasts and tug at your nipples, and you can see him watching transfixed as you fuck his throat.  " : ""));
    else if (player.cocks[x].cArea() < 36)
        outputText("He opens his mouth to protest, but you never give him the chance, forcing your " + cockDescript(player, x) + " between his lips and nearly dislocating his jaw with the girth of it. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth.  " + (player.breastRows.biggestTitSize() >= 2 ? "Your hands lift to massage your breasts and tug at your nipples, and you can see him watching transfixed as you fuck his throat.  " : ""));
    else outputText("He opens his mouth to protest, only to have your " + cockDescript(player, x) + " mute him. You can feel his buck teeth scratching against the top and bottom of your " + cockDescript(player, x) + "'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your huge erection away, and since you can't fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length.  " + (player.breastRows.biggestTitSize() >= 2 ? "His eyes move from your massive member to your bouncing breasts above with a look of wanton desire that makes you laugh softly.  " : ""));
    outputText("His eyes turn to you in fear and awe, pleading for release, and a slip of your foot to his own straining erection lets you know how in need of an orgasm he is, but this time is yours. You groan in pleasure as your balls draw up tight, churning with your corrupted seed, and in a rush you feed it to him, your orgasm overtaking you as surge after hot surge of cum flares through your flesh and into his throat.  " + (player.vaginas.length > 0 ? "A sympathetic orgasm hits your pussy, causing a surge of feminine juices to splash against his chest and dribble down your thighs lewdly.  " : ""));
    outputText("Your orgasm seems to last forever, filling his belly with your corrupted essence, causing his stomach to bulge slightly with the sheer volume of it. You pull away at last, letting him gasp for breath and fall to the ground, curling around his bloated belly.  ");
    if (player.breastRows.biggestTitSize() >= 2)
...
function corruptJojoAnalCruel(): void {
    ...
    outputText("You press your body up behind him" + (player.breastRows.biggestTitSize() >= 2 ? ", mashing your breasts against his back" : "") + " and hiss into his ear, \"<i>Hello slut...</i>\" You keep hold of the base of his tail, hiking it up to lift his ass enough that he has to go to his toes to stay standing. You listen to him whimper softly as he feels your stirring loins press against the cleft of his oh-so-fuckable ass.  ");
    if (player.cocks[x].cArea() < 10)
        outputText("You saw your swelling erection between his ass cheeks a few times, and then with little warning, you shove yourself deep into his body, making the mouse gasp out as you fill his well used rear. You groan in pleasure as you feel his anal ring grip in flutters along your " + cockDescript(player, x) + " as you spear in and out of him, fucking your slut toy with wild abandon.  ");
    else if (player.cocks[x].cArea() < 36)
        outputText("You press the mouse hard against the tree, inhaling his scent and sliding your " + cockDescript(game.player, x) + " between his firm cheeks.  There is little in the way of tenderness as you thrust deep into his body. You can hear him groan as your " + cockDescript(player, x) + " forces his intestines to shift to accommodate you.  ");
    else outputText("You grin as your mouse slut cries out with your " + cockDescript(player, x) + " spearing into his bowels. You can feel the weight of the tree against your " + cockDescript(player, x) + " as you force his belly to bulge out vulgarly to accommodate the enormous girth.  ");
...
function corruptJojoAnalGentle(): void {
    ...
    else if (player.cocks.biggestCockArea() < 36)
        outputText("You slide your thick and drooling cockhead beneath the mouse's balls, working the musky drool of your pre-cum against the well used crinkle of his ass before forcing the thick vein-lined length of your " + cockDescript(player, x) + " into him. You watch as inch after thick, vulgar inch disappears into his body, grinning as his face contorts in a mix of pain and pleasure from it, and then start to fuck him in earnest, watching as his belly bulges with each thrust of your massive prick.  ");
    else outputText("You force your " + cockDescript(player, x) + " against the mouse's ass and watch as he shakes his head, silently begging you not to do it. You smile and grip his hips, then press forward hard, forcing his body to adapt to your girth, stretching his ass and belly dangerously. You can barely get more than a foot of your " + cockDescript(player, x) + " into him before bottoming out against his diaphragm, so you just fuck him with what you can, churning his insides with each thrust.  ");
...
function corruptJojoBreasts(): void { // Should only be available to players with biggestBreastSize > 2
    ...
    if (player.breastRows.biggestLactation() > 1) { // For suckling the scene is the same regardless of player's gender
    ...
    else if (player.breastRows.biggestTitSize() <= 5) {
        outputText("He leans in to nuzzle and kiss at your breasts, his hands moving to caress the soft and full orbs in gentle worship. His kissing and licking slowly circles in on your nipples, bringing them to firm points that send jolts of warm pleasure through your body when he at last takes them into his mouth. You reach down between your legs, " + (player.cocks.length > 0 ? "taking hold of your shaft and masturbating it lazily as he works.  " : "slipping your fingers into your slit as you lazily masturbate with the pleasure he brings.  "));
    }
    else {
...
function whisperJojobait(): void {
    ...
    if (player.cocks[x].cockType == CockTypesEnum.TENTACLE) {
        outputText("His rodent tail wraps around your waist as you get into position, causing your " + cockDescript(player, x) + " to writhe even harder, searching for the hole Jojo's tail is pulling you towards.  As soon as you're close enough, your " + cockDescript(player, x) + " pushes into Jojo, twisting around to widen the mouse's hole even further.  Jojo squirms as you brutally stretch him out, stiffening once his hole is stretched to the max.  After admiring the now obscenely gaping hole of your mouse slut, you begin to grind your member around, causing Jojo to scream in ectasy as your " + cockDescript(player, x) + " goes ballistic inside of his hungry bowels.\n\n");
    }
    // [Small penis (7 inches or less)]
    else if (player.cocks.cockArea(x) < 13) {
        outputText("His hole flexes constantly, as if hungry for your " + cockDescript(player, x) + ". Jojo's tail wraps around your waist as you get into position, and you sink your " + cockDescript(player, x) + " into his hungry mouse hole.  The sensation of Jojo's hole quivering around your " + cockDescript(player, x) + "makes you smile as you begin thrusting in and out of him.  Jojo groans beneath you like a whore in heat, his ass moving in time to meet your thrusts.  The sound of your bodies clapping together is an applause as you fuck the little mousey without reservation.\n\n");
    }
    // [Medium penis (8-12 inches)]
    else if (player.cocks.cockArea(x) < 25) {
        outputText("His tail possessively wraps itself around your waist as you tease him with your " + cockDescript(player, x) + ", smearing your leaking pre all over his stretched tail hole.  You slide yourself into him with a sigh, feeling his hole quiver around your invading sex organ.  Jojo wails like a whore in heat beneath you, grunting as you begin to piston pump your swollen sex organ in and out of his greedy mousey hole.\n\n");
    }
    // [Large penis (13 inches and up)]
    else {
        outputText("Jojo's tail wraps around your waist as you get into position behind him, tightening possessively and trying to bring you closer.  You oblige the slut, sliding your " + cockDescript(player, x) + ", up the mouse's spread cheeks, teasing his ass with the underside.  When you slip your oversized sex organ into his hungry hole, you feel a cringe ripple through his entire body.  You can soon feel the earth beneath Jojo's stomach as the slow invasion of your " + cockDescript(player, x) + " is halted by the mouse's diaphragm; you're unable to fit more than a foot of its length into Jojo's overstuffed tail hole.  You withdraw slowly before suddenly shoving your hips forward, knocking the wind out of the little mousey.  Despite being stretched and filled beyond his limits, the mouse releases a pleased groan and begs for more.\n\n");
    }

    outputText("You feel the pressure building as you saw your " + cockDescript(player, x) + " in and out of Jojo's tail hole, brutally fucking the mouse.  Jojo moans in both ecstasy and pain, releasing a shrill squeak with every thrust.  He claws at the ground, both hungry for more and desperate for release.  The tip of his tail unwraps from your rapidly thrusting hips and slides down your " + buttDescription(player) + ", spurring you to jackhammer his insides faster. The tip slips into your " + assholeDescript(player) + ", working your prostate as you abuse the mouse's.\n\n");
    ...
    // Without Fertility/Lots of Jizz Perk]
        ...
        outputText("Jojo's balls begin to shrink as he shoots his own seed, your weight forcing his body flat against the ground once his sac has shrunk to normal.  His tail still slides around inside your " + buttDescription(player) + ", spurring you to reward him with a few post-orgasm thrusts.  His bowels are hot and wet from your load, and you grind your " + cockDescript(player, x) + " around with a look of supreme bliss on your face.  Jojo groans as you pull out, releasing a stream of creamy white that slides down to his now normal sized balls. Well... normal for Jojo.\n\n");
...
/* The old code for sex with corrupt Jojo
        ...
        private function masturbateJojo():void {
            ...
            //Male
            if (player.gender == 1 && player.breastRows.biggestTitSize() < 2) {
                storage = player.cocks.biggestCockIndex();
                //MALE SOFT BJ
                if (selection == 0) {
                    outputText("As if on command, Jojo slips into your camp from the jungle's shadows, dropping to his knees with a timid look of fear in his eyes. You step forward and caress your fingers through the fur between his shell-like ears, whispering softly to him, \"<i>It's all right, my beautiful slut, it will all be over soon.</i>\"");
                    outputText("  He whimpers as you say this, feeling the corruption flowing off of your body like an alluring musk, drawing him deeper into your service.  ");
                    if (player.cocks[storage].cArea() < 10)
                        outputText("He opens his mouth to protest, but you never give him the chance, sliding your " + cockDescript(player, storage) + " between his lips and down his throat. You can feel the muscles of his throat grip and spasm around your cock flesh as he chokes on the length, his thin lips trembling around your girth as his tongue slides across your vein-lined underside.  ");
                    else if (player.cocks[storage].cArea() < 36) outputText("He opens his mouth to protest, but you never give him the chance, forcing your " + cockDescript(player, storage) + " between his lips and nearly dislocating his jaw with the girth of it. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth.  ");
                    else outputText("He opens his mouth to protest, only to have your " + cockDescript(player, storage) + " mute him. You can feel his buck teeth scratching against the top and bottom of your " + cockDescript(player, storage) + "'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your huge erection away, and since you can't fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length.  ");
                ...
                if (selection == 1) {
                    ...
                    outputText("He shakes his head as you say this, trying to marshal up the strength to resist you. You draw your teeth back in a snarl of anger at this resistance and punch the mouse in the gut, dropping him to his knees gasping for breath.  ");
                    if (player.cocks[storage].cArea() < 10)
                        outputText("You grip the fur on his head tightly in one hand and pull his mouth over your " + cockDescript(player, storage) + ", thrusting into his muzzle with little concern for letting him catch his breath. You shove your length down his throat and start sawing away, making the mouse's eyes roll back from breathlessness. You can feel the muscles of his throat grip and spasm around your cock flesh as he chokes on the length, his thin lips trembling around your.  ");
                    else if (player.cocks[storage].cArea() < 36)
                        outputText("You grip the fur on his head tightly in one hand and pull his mouth over your " + cockDescript(player, storage) + ", thrusting into his muzzle with little concern for letting him catch his breath. The girth of your " + cockDescript(player, storage) + " nearly dislocates his jaw. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth.  ");
                    else outputText("You grip the fur on his head tightly in one hand and pull his mouth over your " + cockDescript(player, storage) + ", thrusting against his muzzle with your " + cockDescript(player, storage) + ". You can feel his buck teeth scratching against the top and bottom of your " + cockDescript(player, storage) + "'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your huge erection away, and since you can't fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length.  ");
                ...
                if (selection == 2) {
                    ...
                    if (player.cocks[storage].cArea() < 10)
                        ...
                        outputText("You slide your thick and drooling cockhead beneath the mouse's balls, working the musky drool of your pre-cum against the well used crinkle of his ass before forcing the thick vein-lined length of your " + cockDescript(player, storage) + " into him. You watch as inch after thick, vulgar inch disappears into his body, grinning as his face contorts in a mix of pain and pleasure from it, and then start to fuck him in earnest, watching as his belly bulges with each thrust of your " + cockDescript(player, storage) + ".  ");
                    else outputText("You force your " + cockDescript(player, storage) + " against the mouse's ass and watch as he shakes his head, silently begging you not to do it. You smile and grip his hips, then press forward hard, forcing his body to adapt to your girth, stretching his ass and belly dangerously. You can barely get more than a foot of your " + cockDescript(player, storage) + " into him before bottoming out against his diaphragm, so you just fuck him with what you can, churning his insides with each thrust.  ");
                    outputText("You pound away at the mouse's tight body for as long as you can, then feel your orgasm hit you hard, your balls drawing up tight as your seed churns and pulses through you and into the mouse's ass, filling his belly with your lust and corruption. You watch his belly swell with the seed in a beautifully vulgar display.  ");
                ...
                if (selection == 3) {
                    ...
                    if (player.cocks[storage].cArea() < 10)
                        outputText("You saw your swelling erection between his ass cheeks a few times, and then with little warning, you shove yourself deep into his body, making the mouse gasp out as you fill his well used rear. You groan in pleasure as you feel his anal ring grip in flutters along your " + cockDescript(player, storage) + " as you spear in and out of him, fucking your slut toy with wild abandon.  ");
                    else if (player.cocks.biggestCockArea() < 36)
                        outputText("You press the mouse hard against the tree, inhaling his scent and sliding your " + cockDescript(player, storage) + " between his firm cheeks.  There is little in the way of tenderness as you thrust deep into his body. You can hear him groan as your " + cockDescript(player, storage) + " forces his intestines to shift to accommodate you.  ");
                    else outputText("You grin as your mouse slut cries out with your " + cockDescript(player, storage) + " spearing into his bowels. You can feel the weight of the tree against your " + cockDescript(player, storage) + " as you force his belly to bulge out vulgarly to accommodate the enormous girth.  ");
                    outputText("You thrust away at your squirming and mewling mouse, taking out your pleasure on him with little concern for his own enjoyment, not that this is really a problem, as before you manage to cum, you feel him tense as he 'fertilizes' the tree you have him pressed against. The feel of his orgasm milks you to your own explosion within his belly, emptying your balls with a low groan of relief.  ");
            ...
            if (player.gender == 1 && player.breastRows.biggestTitSize() >= 2) {
                ...
                if (selection == 0) {
                    ...
                    outputText("He whimpers as you say this, feeling the corruption flowing off of your body like an alluring musk, drawing him deeper into your service.  ");
                    if (player.cocks[storage].cArea() < 10)
                        outputText("He opens his mouth to protest, but you never give him the chance, sliding your " + cockDescript(player, storage) + " between his lips and down his throat. You can feel the muscles of his throat grip and spasm around your " + cockDescript(player, storage) + " as he chokes on the length, his thin lips trembling around your girth as his tongue slides across your vein-lined underside. Your hands lift to massage your breasts and tug at your nipples, and you can see him watching transfixed as you fuck his throat.  ");
                    else if (player.cocks[storage].cArea() < 36)
                        outputText("He opens his mouth to protest, but you never give him the chance, forcing your " + cockDescript(player, storage) + " between his lips and nearly dislocating his jaw with the girth of it. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth. Your hands lift to massage your breasts and tug at your nipples, and you can see him watching transfixed as you fuck his throat.  ");
                    else outputText("He opens his mouth to protest, only to have your " + cockDescript(player, storage) + " mute him. You can feel his buck teeth scratching against the top and bottom of your " + cockDescript(player, storage) + "'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your " + cockDescript(player, storage) + " away, and since you can't fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length. His eyes move from your massive member to your bouncing breasts above with a look of wanton desire that makes you laugh softly.  ");
                ...
                if (selection == 3) {
                    ...
                    if (player.cocks[storage].cArea() < 10)
                        ...
                        outputText("You press the mouse hard against the tree, inhaling his scent and sliding your " + cockDescript(player, storage) + " between his firm cheeks.  There is little in the way of tenderness as you thrust deep into his body. You can hear him groan as your " + cockDescript(player, storage) + " forces his intestines to shift to accommodate you.  ");
                    else outputText("You grin as your mouse slut cries out with your " + cockDescript(player, storage) + " spearing into his bowels. You can feel the weight of the tree against your member as you force his belly to bulge out vulgarly to accommodate the enormous girth.  ");
                    outputText("You thrust away at your squirming and mewling mouse, taking out your pleasure on him with little concern for his own enjoyment, not that this is really a problem, as before you manage to cum, you feel him tense as he 'fertilizes' the tree you have him pressed against. The feel of his orgasm milks you to your own explosion within his belly, emptying your balls with a low groan of relief.  ");
                    outputText("You pull out of Jojo's ass once your orgasm has subsided and wipe your " + cockDescript(player, storage) + " off on the fur of his back, then walk away to leave him to his own devices.  ");
                }
            }
            //CuntBOOOOOI
            if (player.gender == 2 && player.breastRows.biggestTitSize() < 2) {
            ...
            if (player.gender == 3) {
                ...
                if (selection == 0) {
                    ...
                    outputText("He whimpers as you say this, feeling the corruption flowing off of your body like an alluring musk, drawing him deeper into your service.  ");
                    if (player.cocks[storage].cArea() < 10)
                        outputText("He opens his mouth to protest, but you never give him the chance, sliding your " + cockDescript(player, storage) + " between his lips and down his throat. You can feel the muscles of his throat grip and spasm around your " + cockDescript(player, storage) + " flesh as he chokes on the length, his thin lips trembling around your girth as his tongue slides across your vein-lined underside. Your hands lift to massage your breasts and tug at your nipples, and you can see him watching transfixed as you fuck his throat.  ");
                    else if (player.cocks.biggestCockArea() < 36)
                        outputText("He opens his mouth to protest, but you never give him the chance, forcing your " + cockDescript(player, storage) + " between his lips and nearly dislocating his jaw with the girth of it. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth. Your hands lift to massage your breasts and tug at your nipples, and you can see him watching transfixed as you fuck his throat.  ");
                    else outputText("He opens his mouth to protest, only to have your " + cockDescript(player, storage) + " mute him. You can feel his buck teeth scratching against the top and bottom of your " + cockDescript(player, storage) + "'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your " + cockDescript(player, storage) + " away, and since you can't fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length. His eyes move from your massive member to your bouncing breasts above with a look of wanton desire that makes you laugh softly.  ");
                ...
                if (selection == 2) {
                    ...
                    if (player.cocks[storage].cArea() < 10)
                        ...
                        outputText("You slide your thick and drooling cockhead beneath the mouse's balls, working the musky drool of your pre-cum against the well used crinkle of his ass before forcing the thick vein-lined length of your " + cockDescript(player, 0) + " into him. You watch as inch after thick, vulgar inch disappears into his body, grinning as his face contorts in a mix of pain and pleasure from it, and then start to fuck him in earnest, watching as his belly bulges with each thrust of your massive prick.  ");
                    else outputText("You force your " + cockDescript(player, 0) + " against the mouse's ass and watch as he shakes his head, silently begging you not to do it. You smile and grip his hips, then press forward hard, forcing his body to adapt to your girth, stretching his ass and belly dangerously. You can barely get more than a foot of your " + cockDescript(player, 0) + " into him before bottoming out against his diaphragm, so you just fuck him with what you can, churning his insides with each thrust.  ");
                    outputText("You pound away at the mouse's tight body for as long as you can, then feel your orgasm hit you hard, your balls drawing up tight as your seed churns and pulses through you and into the mouse's ass, filling his belly with your lust and corruption. You watch his belly swell with the seed in a beautifully vulgar display.  ");
                    outputText("His eyes glaze over from the intensity of the act, his teeth tightly grit, and then you can hear a keening groan from him as he falls over the edge into his own orgasm, his untouched mouse cock bouncing and jerking on his belly as his thick seed is sprayed across his chest and face lewdly. He blushes deep at the visible proof that he enjoyed what you did to him and trembles beneath you.  ");
                    outputText("You can't help but laugh at the scene, and draw out of his ass with a groan of pleasure. You watch as he crawls back into the jungle in shame, leaving a trail of your cum the whole way.  ");
                }
                ...
                if (selection == 3) {
                    ...
                    outputText("You cry out in pleasure as your orgasm floods through your body, causing your juices to splash out around your mouse slut's cock, and your own " + multiCockDescriptLight(player) + " to explode with thick splashes of your hot cum across his chest and belly.  ");
...
function beeEggsInCorruptJojo(): void {
    ...
    if (player.gender > 0) {
        ...
        if (player.gender == 3 || player.cocks.length > 1) outputText("are");
        else outputText("is");
...
export function meditateInForest(): void {
    ...
    dynStats("str", .5, "tou", .5, "int", .5, "lib", -1, "lus", -5, "cor", (-1 - player.cocks.countCockSocks("alabaster")));
...
export function jojoTalkYourOrigin(): void // Prob tack on some interaction count or something to unlock this
{
    ...
    else if (player.perks.findByType(PerkLib.HistoryFighter) >= 0) {
    ...
    else if (player.perks.findByType(PerkLib.HistoryHealer) >= 0) {
    ...
    else if (player.perks.findByType(PerkLib.HistoryScholar) >= 0) {
    ...
    else if (player.perks.findByType(PerkLib.HistorySlut) >= 0) {
    ...
    else if (player.perks.findByType(PerkLib.HistorySlacker) >= 0) {
    ...
    else if (player.perks.findByType(PerkLib.HistorySmith) >= 0) {
    ...
    else if (player.perks.findByType(PerkLib.HistoryWhore) >= 0) {
...
export function jojoTalkSandCave(): void {
    ...
    if (flags[kFLAGS.MET_MILK_SLAVE] == 1) {
        ...
        else if (flags[kFLAGS.MILK_NAME] instanceof String) {
...
export function apparantlyJojoDOESlift(): void {
    ...
    if (player.perks.findByType(PerkLib.ControlledBreath) < 0 && player.perks.findByType(PerkLib.CleansingPalm) < 0 && player.perks.findByType(PerkLib.Enlightened) < 0) {
        ...
        if (flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] == 5) {
            ...
            if (player.breastRows.biggestTitSize() == 0) outputText(" rises and falls");
            ...
            player.perks.create(PerkLib.ControlledBreath, 0, 0, 0, 0);
        }
    }
    // {after the PC has gained the controlled breath perk}
    else if (player.perks.findByType(PerkLib.ControlledBreath) >= 0 && player.perks.findByType(PerkLib.CleansingPalm) < 0 && player.perks.findByType(PerkLib.Enlightened) < 0) {
        ...
        if (flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] == 10) {
            ...
            player.perks.create(PerkLib.CleansingPalm, 0, 0, 0, 0);
        }
    }
    // {after the PC has gained the Cleansing Palm attack}
        ...
        if (flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] >= 16 && player.inte >= 70) {
            ...
            player.perks.create(PerkLib.Enlightened, 0, 0, 0, 0);
        }
    }
    // {after PC has gained the Enlightened Perk}
        ...
        const enlightenedBlurbs: any[] = new Array();
