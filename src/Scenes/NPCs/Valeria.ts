...
function followersValeriaSex(display: boolean = true): void {
    ...
    if (player.cocks.length > 0) penetrate = penetrateValeria;
    const getFucked: () => void = valeriaGetFucked;
    const gooFlation: () => void = gooFlation;
    let dominated: () => void = null;
    if (player.gender > 0) dominated = valeriaSexDominated;
    const dickToggle: () => void = valeriaDickToggle;
...
function valeriaGetFucked(): void {
    ...
    if (player.breastRows.biggestTitSize() > 3) outputText("bumping over your [chest] and stopping for a brief moment to tickle your sensitive tits with her goopy bottom before ");
    ...
    if (player.cocks.length > 0) outputText(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
    outputText(".");
    ...
    if (!player.cocks.length > 0 && player.vaginas.length > 0) {
        ...
        if (!player.breastRows.hasFuckableNipples()) outputText("  Valeria swirls her heels around your sensitive [nipples], letting the tips slip into her gooey feet.  She rocks her heels back and forth, popping your nips in and out of her gooey body, feeling like little pinches and squeezes on your defenseless flesh.");
        ...
        if (player.breastRows.hasFuckableNipples()) outputText("and [chest] ");
...
function valeriaSexDominated(): void {
    ...
    if (player.vaginas.length > 0 && (!player.cocks.length > 0 || rand(2) == 0)) {
        ...
        outputText(mf(player, "boy", "girl") + " ready to cum?</i>\" she withdraws herself from you, leaving you feeling suddenly very, very empty.  \"<i>I didn't say you could cum yet, slut! I'm not done with you yet.</i>\"");
    ...
    // (PC has Dick)
        ...
        outputText("\n\nValeria begins to use her goo to peel back your clothes, soon revealing your defenseless, half-erect  package.  She makes a show of licking her lips as tendrils of goo wrap tightly around [eachCock] like a warm, wet onahole. You submit to your gooey bonds, and seeing your lack of resistance, Valeria smiles down at you and squeezes your " + multiCockDescriptLight(game.player) + " tighter.");
        outputText("\n\nYou gasp with pleasure as she starts to stroke [eachCock], jerking you off as she looms over you, grinning wickedly.  \"<i>Oh, you like that do you?</i>\" the armor-goo asks.  \"<i>Well then, I might just let you get off, too... If you're a good " + mf(player, "boy", "girl") + "</i>\"  She starts to increase her tempo, making you squirm and writhe as she wanks your " + multiCockDescriptLight(game.player) + ", licking her lips as little bubbles of pre start to form.");
        ...
        outputText(" " + mf(player, "boy", "girl") + " ready to cum? Well, go on then - we've got time for more.</i>\"");
        outputText("\n\nYou cum, ropes of thick, white jizz shooting out of your cock and into the goo's waiting body.  Laughing, the goo-girl absorbs your cum, growing larger and larger as you feed her until she towers over you, her expanding breasts and belly now hanging over you.  \"<i>Oh, that's good. Good, " + mf(player, "boy", "girl") + ", good.  Yes, let it all out, just like that... just like that,</i>\" she coos, soaking your cum up until your orgasm finally passes.");
        ...
        outputText("\n\n\"<i>Oh, that's good... good, " + mf(player, "boy", "girl") + ", good.  Yes, let it all out, just like that... just like that,</i>\" she moans, soaking your juices up until your orgasm finally passes.  Sated, she withdraws around your foot, leaving you a quivering mess on the ground.");
...
function talkWithValeria(): void {
    ...
    if (race(player) != "human") outputText(" were");
...
function flirtWithValeria(): void {
    ...
    if (player.cocks.length > 0) outputText("brushing along the length of your rapidly-hardening cock");
...
function acceptValeriasNeeds(): void {
    ...
    if (player.effects.findByType(StatusAffects.SlimeCraving) >= 0) outputText(" after all, you have the same ones");
...
export function valeriaAndGooThreeStuff(): void {
    ...
    */
    if (player.cocks.length > 0) {
        ...
        outputText(" around ");
        if (player.cocks.length == 1) outputText("it as it distends");
...
function valeriaGooRapeII(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 3) outputText("the valley of your cleavage");
    ...
    // Fuckable nipples
    if (player.breastRows.hasFuckableNipples() && player.breastRows.biggestTitSize() >= 5) {
        ...
        outputText(". Valeria takes this as an invitation, and " + num2Text(player.breastRows.totalNipples()) + " phallic objects greedily push at your cunt-nipples, eagerly sliding themselves inside your welcome chest-pussies. Looking down, you can see the navy-blue outlines of the phallic tendrils waving and writhing as they push their way deep, embedding themselves tit-deep in blissful warmth.");
    }
    // Dick
    if (player.cocks.length > 0) {
        outputText("\n\nValeria's voice calls out once more, \"<i>You didn't think I'd forget ");
        if (player.cocks.length == 1) outputText("this little guy");
        else outputText("these little guys");
        outputText(", did you? ");
        if (player.cocks.biggestCockArea() >= 50) outputText("...Maybe big guy is a bit more appropriate. Whatever. ");
        ...
        if (player.cocks.length > 1) outputText("s");
        outputText(". You inadvertantly clench your muscles in a way that makes your " + multiCockDescriptLight(game.player) + " so very swollen, in turn making it that much easier for the gelatinous fiend to have her way with your cumslit");
        if (player.cocks.length > 1) outputText("s");
        outputText(". You feel that warm, wet fluid slowly peel it open and push inside. It doesn't hurt at all, but it is a little weird having your " + multiCockDescriptLight(game.player) + " slowly stuffed in such a way. The warmth travels the whole way down your dick");
        if (player.cocks.length > 1) outputText("s");
    ...
    if (player.breastRows.hasFuckableNipples() && player.breastRows.biggestTitSize() >= 5) {
        ...
        if (player.breastRows.length > 1) outputText("top row of breasts");
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.length > 1) outputText("do not");
        else outputText("does not");
        outputText(" differ greatly from Valeria's other liquid attentions. ");
        if (player.cocks.length > 1) outputText("They start");
    ...
    if (player.breastRows.biggestTitSize() >= 5 && player.breastRows.hasFuckableNipples()) outputText(", pausing to handle your hefty, swelling tits");
    ...
    if (player.breastRows.biggestTitSize() >= 5 && player.breastRows.hasFuckableNipples()) {
    ...
    if (player.breastRows.biggestTitSize() >= 5 && player.breastRows.hasFuckableNipples()) player.effects.setValue(StatusAffects.GooStuffed, 4, 1);
    cleanupAfterCombat();
}
...
export function birthOutDatGooSlut(): void {
    ...
    if (player.skinType == SkinType.FUR) outputText("furry ");
    outputText("bitch in heat. Gods, there's so much!");
    if (player.effects.getValue4Of(StatusAffects.GooStuffed) > 0) {
        outputText("\n\n");
        if (!player.breastRows.hasFuckableNipples()) outputText("Stiffening up");
        ...
        if (player.breastRows.length > 1) outputText(" and the next row down");
        if (player.breastRows.length > 2) outputText(", and so on");
    ...
    player.effects.remove(StatusAffects.GooStuffed);
