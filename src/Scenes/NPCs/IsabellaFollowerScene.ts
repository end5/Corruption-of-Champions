...
public  isabellaMoovesInGreeting():void {
	...
	if(flags[kFLAGS.ISABELLA_TIMES_OFFERED_FOLLOWER] == 0) {
		...
		else outputText("squeeze against your " + game.player.chestDesc() + ", the four hard nipples capping each milk-flooded breast pressing on your own.", false);
...
public  isabellaMorningWakeupCall():void {
	...
	outputText("  Her massive, malleable tongue moulds around your " + game.player.cockDescript(x) + " as she works to enhance your pleasure.  A trickle of warm, creamy milk escapes the heavy jugs resting on your " + player.legs() + ", showing you just how much she enjoys dispensing oral pleasure.  The tangled, crimson mass of hair slowly begins to bob up and down upon your loins, and Isabella's big brown eyes look up at you with radiant happiness, the corners of her mouth curling into a cock-filled smile.\n\n", false);
	
	outputText("You groan out loud at the sight of the sexy cow-girl's mischievous gaze.  She's face-fucking your " + game.player.cockDescript(x) + " without any difficulty and laying languid, lascivious licks over every part of your maleness simultaneously.  Your cock is utterly, helpless bound up by her thick oral organ.  It twitches inside her mouth-muscle's embrace, helplessly releasing rivulets of pre-cum that disappear into Isabella's throat as fast as you can produce them.  You can see her cheeks hollowing as she begins to pull the tongue-wrapped dick deep into her maw, the suction making your " + game.player.cockDescript(x) + " grow dangerously sensitive to the cow-girl's whims.\n\n", false);
	
	outputText("\"<i>Oh fuck!</i>\" you groan without meaning to, lifting your " + hipDescript() + " to press your ", false);
	if(player.balls > 0) outputText(ballsDescriptLight() + " against her chin, the cum-bloated orbs bouncing and shuddering with your " + game.player.cockDescript(x) + " as release nears.", false);
	else {
		outputText(game.player.cockDescript(x) + " the whole way into her mouth, the slippery, hot cow-girl's lips sealing tightly around your ", false);
		if(player.hasSheath()) outputText("sheath", false);
		else outputText("base", false);
		outputText(".", false);
	}
	outputText("  Isabella brushes her hair back to watch your reaction; her shining eyes never leave your face as she kicks her efforts into overdrive, pumping and stroking your " + game.player.cockDescript(x) + " with nothing more than her tongue's moist heat.\n\n", false);
	...
	if(player.cockTotal() > 1) {
		...
		if(player.cockTotal() > 2) outputText("the rest of your " + game.player.multiCockDescriptLight(), false);
		else outputText("the one penis she's left untouched", false);
		outputText(", spraying across your belly and " + game.player.chestDesc() + ".  Isabella, ever attentive, grabs the twitching cock", false);
	...
	if(flags[kFLAGS.ISABELLA_MORNING_FELLATIO_COUNT] == 1) {
		...
		if(isabellaAccent()) outputText("Yum!  You cum so hard for momma Izabella, don't you?</i>\"  The sated cow-girl gives your " + game.player.cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...", false);
		else outputText("Yum!  You cum so hard for momma Isabella, don't you?</i>\"  The sated cow-girl gives your " + game.player.cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...", false);
	}
	else {
...
private  repeatGermanBratwurstInCamp():void {
	...
	outputText("You ask Isabella if she would mind helping you blow off some pressure before you go back out.  She glances down at " + game.player.sMultiCockDesc() + " and ", false);
	if(flags[kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING] < 5 && flags[kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING] > 0) {
		...
		if(isabellaAccent()) outputText("gleefully muses, \"<i>Oooh, I see.  Of course I vill help you, " + player.short + "!</i>\"  She traces a finger around " + game.player.oMultiCockDesc() + "' outline and continues, \"<i>It vould be mein pleasure to squeeze it all out into mein caboose.  Zat is what you vant, nein?", false);
		else outputText("gleefully muses, \"<i>Oooh, I see.  Of course I'll help you, " + player.short + "!</i>\"  She traces a finger around " + game.player.oMultiCockDesc() + "' outline and continues, \"<i>It would be my pleasure to squeeze it all out into my caboose.  That is what you want, no?", false);
	}
	outputText("</i>\"\n\n", false);
	...
	outputText("You nod, catching a whiff of her musky pussy's aroma without meaning to.  It's not like you could avoid it, with her body mere inches in front of your " + player.face() + ", shaking so perfectly and so closely that an occasional droplet catches on your nose.  Your hands clamp around the chair with an iron grip as you fight not to rise and rape her tender, teasing asshole on the spot.  " + game.player.SMultiCockDesc() + " is trickling and dribbling, bouncing with your heartbeats as your " + player.legs() + " shiver and shake with denied lusts.  You NEED to do something... to fuck, thrust, or just fap... anything - your " + game.player.multiCockDescriptLight() + " can't take it anymore!\n\n", false);
	
	outputText("Drawn up by your dick, you let go of your grip on the cow-crafted chair and start to rise into Isabella.  She expertly drops her bouncy butt into your " + game.player.chestDesc() + ".  It knocks you back down into place while she scolds, ", false);
	...
	outputText("The curvaceous cow-girl's stance shifts slightly, her teasing, shaking dance coming to a stop for just a moment as she adjusts to rest her rump against your " + game.player.cockDescript(x) + ".  ", false);
	...
	if(player.cockArea(x) <= 38) {
		if(isabellaAccent()) outputText("\"<i>Oh my, does mein behind feel so good zat your cute, delectable cock vill squirt all over me before you even get to feel ze best part?</i>\" teases Isabella as she flexes one side and then the other, forcing your " + game.player.cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  \"<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don't stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>\"", false);
		else outputText("\"<i>Oh my, does my ass-job feel so good that your cute, delectable cock is gonna squirt all over me before you even get to feel the best part?</i>\" teases Isabella as she flexes one side and then the other, forcing your " + game.player.cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  \"<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don't stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>\"", false);
		//(if libido <=15:  
		...
		outputText("  She doesn't even let you answer.  Her back-and-forth flexing abruptly halts as she switches to an up and down motion, hotdogging your " + game.player.cockDescript(x) + " in the tight vice of her spotted backside.\n\n", false);
		
		outputText("You grab hold of the chair again with both hands, struggling not to cum while Isabella's ass slides over your member.  It's so hot, wet, and sensual that you're trembling from the effort of holding still, but you manage, knowing the longer you hold back, the better the finish will be.  She slowly pumps your " + game.player.cockDescript(x) + " in her anal valley while you whine and moan, your face taking on a more lurid, sexual expression with every passing moment.  When at last you feel you can hold back no longer, you groan, \"<i>G-gonna... gonna-cum-soon!</i>\"\n\n", false);
		...
		outputText("Your " + game.player.cockDescript(x) + " writhes in bliss for what seems like forever, but before you know it, you're coming back down to Earth.  Isabella whispers, \"<i>Good " + player.mf("boy","girl") + ",</i>\" to you and stands up, ignoring the cum that drips from her dark pucker to stain " + game.player.sMultiCockDesc() + ".  She helps you up, and her hand tussles your hair affectionately.  Then she turns towards her side of camp, walking a little bow-legged as she leaves.", false);
	}
	//(Big dicks) 
	else {
		if(isabellaAccent()) outputText("\"<i>Oh my, does mein ass feel zat good on ze huge, pervy thing you call a dick?</i>\" teases Isabella as she flexes one side then the other, forcing your " + game.player.cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, \"<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you'll waste your cum all over my ass?</i>\"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your " + game.player.cockDescript(x) + " from base ", false);
		else outputText("\"<i>Oh my, does my ass-job feel that good on that huge, pervy thing you call a dick?</i>\" teases Isabella as she flexes one side then the other, forcing your " + game.player.cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, \"<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you'll waste your cum all over my ass?</i>\"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your " + game.player.cockDescript(x) + " from base ", false);
		...
		outputText("You shudder, trying to hold still and prolong the pleasure, but " + game.player.sMultiCockDesc() + " is jerking up and down, defiantly grinding itself through the hot, slippery valley of Isabella's ass.  The " + player.cockHead(x) + " bounces above like a flag-pole in a hurricane, and then you can bear it no longer.  You're cumming... cumming so very hard.  Your whole body locks tight, back arching slightly from the strain.  Isabella slides down to the bottom of your convulsing pole and resumes rolling your " + game.player.cockDescript(x) + " between her bronzed ass-cheeks, causing your eruptions of seed to form lazy 'S' shapes in the air before splattering down on her heaving bottom.", false);
...
private  tentacleBoneFollowerIzzy():void {
	...
	outputText(game.player.multiCockDescriptLight() + " did all the work; your groin feels sore from the sheer effort deployed in order to engage in a maximally effective penetration.  As you keep pumping, grinding and teasing her with your wriggling rods, you feel a pressure growing in your crotch and all the way up your urethras; you accelerate the thrusting pace even as ", false);
	...
	outputText("you feel your climax coming closer and closer.  Isabella's body is literally swooning in pleasure and her face displays all signs of an intense incoming climax.  The sight of your lover's bliss is enough to launch you over the edge; with an amazingly inhuman might, you cum. The spunk boils furiously as it spurts out of " + game.player.sMultiCockDesc() + ". You give in to your powerful orgasm as you coat her with white goo, filling her throat", false);
	if(t4 >= 0) {
		outputText(", ", false);
	...
	outputText("You keep holding her with " + game.player.sMultiCockDesc() + " for a moment, both of you enjoying the powerful embrace as you literally dick-hug her.  The entirety of her body is dripping with your jism but she doesn't seem to care.  Eventually, you put her down, letting her splat in the puddle of juices that formed below your junk-prison.  At last, you unfold your arms.\n\n", false);
	player.orgasm();
...
private  getIzzyBurped():void {
	...
	if(flags[kFLAGS.ISABELLA_PROBOVA_BURP_COUNT] == 1) {
		...
		if(player.hasCock() && player.longestCockLength() < 18) {
			outputText("  A pressure builds in your crotch, at first subtle, then incredibly obnoxious to the point where you're forced to undo the lower portion of your " + player.armorName + " to seek some sort of relief.  Your shock is total as you stare at your former " + game.player.cockDescript(0) + ", now measuring at least 18 inches long – a condition that Isabella has already noticed and is clearly formulating a gripe about.", false);
...
private  fuckIsabellaInTheBarn():void {
	...
	outputText("\n\nStripping out of your [armor], you sidle up behind the distracted bovine and grab your " + game.player.cockDescript(x) + ", giving it a few quiet strokes until it's nice and hard.  Then, with a grin, you let it pop free of your grasp so that it flops down between Isabella's gropable cheeks, quickly buried beneath her yielding flesh.");
