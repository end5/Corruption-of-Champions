export function appearance(e: MouseEvent = null): void {
	...
	/*if(player.lowerBody == LowerBodyType.CENTAUR) race = "centaur";
	if(player.lowerBody == LowerBodyType.PONY) race = "pony-kin";
	//determine sheath
	...
	{
		if(player.lowerBody == LowerBodyType.CENTAUR) race = "centaur-morph";
	...
	if(player.lowerBody == LowerBodyType.NAGA) race = "naga";
	if(player.lowerBody == LowerBodyType.CENTAUR) race = "centaur";
    ...
    // Face
    if (player.faceType == FaceType.HUMAN || player.faceType == FaceType.SHARK_TEETH || player.faceType == FaceType.BUNNY || player.faceType == FaceType.SPIDER_FANGS || player.faceType == FaceType.FERRET_MASK) {
        ...
        else if (player.faceType == FaceType.FERRET_MASK)
        ...
        if (player.skinType == SkinType.PLAIN) outputText("  Your face is an adorable cross between human and ferret features, complete with a wet nose and whiskers.  The only oddity is your lack of fur, leaving only [skin] visible on your ferret-like face.", false);
    ...
    else if (player.faceType == FaceType.RACCOON_MASK) {
        // appearance for skinheads
        if (player.skinType != SkinType.FUR && player.skinType != SkinType.SCALES) {
            outputText("  Your face is human in shape and structure, with " + skin(player));
    ...
    else if (player.faceType == FaceType.RACCOON) {
    ...
    // horse-face
    if (player.faceType == FaceType.HORSE) {
        if (player.skinType == SkinType.PLAIN || player.skinType == SkinType.GOO)
    ...
    // dog-face
    if (player.faceType == FaceType.DOG) {
        if (player.skinType == SkinType.PLAIN || player.skinType == SkinType.GOO)
    ...
    // cat-face
    if (player.faceType == FaceType.CAT) {
        if (player.skinType == SkinType.PLAIN || player.skinType == SkinType.GOO)
            outputText("  You have a cat-like face, complete with a cute, moist nose and whiskers.  The " + skin(player) + " that is revealed by your lack of fur looks quite unusual on so feline a face.", false);
        if (player.skinType == SkinType.FUR)
    ...
    // Lizard-face
    if (player.faceType == FaceType.LIZARD) {
        if (player.skinType == SkinType.PLAIN || player.skinType == SkinType.GOO)
    ...
    if (player.faceType == FaceType.DRAGON) {
    ...
    if (player.faceType == FaceType.KANGAROO) {
    ...
    // M/F stuff!
    outputText("  It has " + faceDesc(player) + ".", false);
    // Eyes
    ...
    else if (player.eyeType == EyeType.BLACK_EYES_SAND_TRAP)
    ...
    // not bald
        ...
        else if (player.earType == EarType.KANGAROO)
    ...
    else if (player.tongueType == TongueType.DRACONIC)
    ...
    // Horns
    // Demonic horns
    if (player.hornType == HornType.DEMON) {
        if (player.horns == 2)
    ...
    // Super lizard horns
    if (player.hornType == HornType.DRACONIC_X4_12_INCH_LONG)
    ...
    // Antlers!
    if (player.hornType == HornType.ANTLERS) {
        if (player.horns > 0)
    ...
    // WINGS!
    if (player.wingType == WingType.BEE_LIKE_SMALL)
    ...
    if (player.wingType == WingType.BAT_LIKE_TINY)
    ...
    if (player.wingType == WingType.BAT_LIKE_LARGE)
    ...
    if (player.wingType == WingType.FEATHERED_LARGE)
    ...
    if (player.wingType == WingType.DRACONIC_SMALL)
    ...
    else if (player.wingType == WingType.DRACONIC_LARGE)
    ...
    else if (player.wingType == WingType.GIANT_DRAGONFLY)
    ...
    else if (player.armType == ArmType.SPIDER)
    ...
    if (player.lowerBody == LowerBodyType.PONY)
    ...
    if (player.tailType == TailType.DOG)
    ...
    if (player.tailType == TailType.DEMONIC)
    ...
    if (player.tailType == TailType.LIZARD) {
    ...
    else if (player.tailType == TailType.DRACONIC) {
    ...
    else if (player.lowerBody == LowerBodyType.FERRET) outputText("  Two furry, digitigrade legs form below your [hips].  The fur is thinner on the feet, and your toes are tipped with claws.", false);
    ...
    else if (player.lowerBody == LowerBodyType.DOG)
    ...
    else if (player.lowerBody == LowerBodyType.DEMONIC_HIGH_HEELS)
    ...
    else if (player.lowerBody == LowerBodyType.DEMONIC_CLAWS)
    ...
    else if (player.lowerBody == LowerBodyType.BUNNY)
    ...
    else if (player.lowerBody == LowerBodyType.HARPY)
    ...
    else if (player.lowerBody == LowerBodyType.KANGAROO)
    ...
    else if (player.lowerBody == LowerBodyType.CHITINOUS_SPIDER_LEGS)
    ...
    else if (player.lowerBody == LowerBodyType.FOX)
    ...
    else if (player.lowerBody == LowerBodyType.DRAGON)
    ...
    else if (player.lowerBody == LowerBodyType.RACCOON)
    ...
    if (player.lowerBody == LowerBodyType.NAGA && player.gender > 0) {
        outputText("\nYour sex", false);
    ...
    if (player.vaginas.length > 0) {
        if (player.gender == 2 && player.lowerBody == LowerBodyType.CENTAUR)
