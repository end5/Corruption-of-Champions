...
export function pumpkinFuckEncounter(): void {
    clearOutput();
...
export function pumpkinFuck(): void {
    clearOutput();
...
// [Next]
export function pumpkinFuckPartII(): void {
    clearOutput();
    ...
    outputText("\n\nThe goddess circles you, idly tracing black, claw-like nails over your exposed " + skin(player) + ".  When did she get so close?  She drops her hand low, the dagger-sharp protrusions tenderly cupping your ");
    ...
    outputText(".  \"<i>");
    if (race(player) != "human") outputText("I know what you were, virile suitor.  ");
...
export function pumpkinFuckPartIII(): void {
    clearOutput();
    ...
    player.perks.create(PerkLib.FerasBoonAlpha, 0, 0, 0, 0);
    dynStats("cor", 30);
    doNext(Camp.returnToCampUseOneHour);
}

export function mountPumpkin(): void {
    clearOutput();
    flags[kFLAGS.PUMPKIN_FUCK_YEAR_DONE] = date.fullYear;
    // >Yes [Player is pregnant, has low vaginal wetness, or has low fertility and is not in heat]
    if (player.pregnancyIncubation > 0 || (player.vaginas.wetness() < 2 && player.totalFertility() < 30)) {
        ...
        cuntChange(player, 50, true, false, false);
    ...
    // Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.
        ...
        outputText("\n\nYour [hips] slap into the orange 'skin' wetly, the omnipresent ooze immediately soak your " + skin(player) + " with corrupted lubricant.  You hardly mind, finding your body already setting into motion as you steady your hands to either side of the heavy fruit.  The lascivious fruit doesn't show any signs of minding the fast fuck you're giving it.  Indeed, the once weakly-moving vines have stilled, their restless tremors silenced by the firm smacks of your groin against vibrant rinds and swollen lips.  You hump away at your immobile lover, spellbound by its fecund fruit-cock, too drunk on lust and pleasure to question the strangeness of it all.");
...
// >Next
export function pumpkinMountingLevelII(): void {
    clearOutput();
    ...
    if (player.breastRows.biggestTitSize() >= 2) outputText(", her free hand working your [fullChest] with glee");
...
export function mountPumpkinsIII(): void {
    clearOutput();
    ...
    player.perks.create(PerkLib.FerasBoonBreedingBitch, 0, 0, 0, 0);
    dynStats("cor", 30);
    doNext(Camp.returnToCampUseOneHour);
}
...
// Introduction
export function feraSceneTwoIntroduction(): void {
    clearOutput();
...
// Leave
export function leaveFerasWonderland(): void {
    clearOutput();
...
// Explore Das Trees
export function exploreFerasWonderland(): void {
    clearOutput();
    flags[kFLAGS.FERAS_GLADE_EXPLORED_YEAR] = date.fullYear;
    outputText("Straightening to your full height, you take a few experimental sniffs and begin to walk back towards the middle of the meadow, brushing aside garish pink blooms that rise up past your waist.  That smell is getting stronger now.  How could you have missed it when you first arrived? It's heavenly, like a combination of every sweet treat you like with an undercurrent of passionate sexual musk blended in underneath.  You close in, " + mf(player, "[hips] swaggering excitedly", "[hips] sashaying excitedly") + ", " + skin(player) + " flushing.  A few baby blue sunflowers bump your shoulders as you muscle past them to hone in your new favorite aroma.  You've just got to know what bloom could possibly release such a heavenly scent!");
...
// Leave
export function leaveAfterFlowerHuffing(): void {
    clearOutput();
...
// Fuck the Lips
export function fuckDemCorruptFlowerLips(): void {
    clearOutput();
    ...
    else outputText("loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your " + skin(player) + ".  You're not really sure if sap is the appropriate name for such a substance, but you're far beyond caring at this point.");
...
// Get a Boosted Dick!
export function dickBoostingFuntimes(): void {
    clearOutput();
    ...
    outputText(" as you do, whispering, \"<i>Good " + mf(player, "boy", "girl") + ",</i>\" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  \"<i>Don't stop yet, [name].  You're going to be doing this a lot, so I'm giving you lots of practice.</i>\"");
    ...
    player.perks.create(PerkLib.FerasBoonSeeder, 0, 0, 0, 0);
    menu();
    addButton(0, "Next", postSeederUpgrade);
}

export function postSeederUpgrade(): void {
    clearOutput();
...
export function stickAStamenInIt(): void {
    clearOutput();
    flags[kFLAGS.FERAS_TRAP_SPRUNG_YEAR] = date.fullYear;
...
// Gape
export function gapeMePleaseFera(): void {
    clearOutput();
    ...
    if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.LEVEL_CLOWN_CAR) outputText(", spreading its well-widened lips as wide as they've ever been");
    else if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.GAPING) outputText(", spreading your lips well beyond the widest they've ever been and still farther.");
    else if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.LOOSE) outputText(", meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.");
    ...
    // STEAL VIRGINITY!
    cuntChange(player, 90000, true, true, false);
    ...
    if (player.lowerBody == LowerBodyType.HUMAN) outputText("toe");
    else if (player.tailType > TailType.NONE) outputText("tail");
    ...
    if (player.vaginas.wetness() < 4) outputText(" even though you're not a squirter");
    ...
    // (cum!)
    // Minimum size to 4
    player.perks.create(PerkLib.FerasBoonWideOpen, 0, 0, 0, 0);
    // Boost fertility by a bunch
    player.fertility += 20;
    if (player.fertility > 50) player.fertility = 50;
    // Raise wetness to at least 3.
    if (player.vaginas[0].vaginalWetness < VaginaWetness.SLICK) player.vaginas[0].vaginalWetness = VaginaWetness.SLICK;
    if (player.vaginas[0].vaginalLooseness < VaginaLooseness.GAPING_WIDE) player.vaginas[0].vaginalLooseness = VaginaLooseness.GAPING_WIDE;
    // Add five corruption.
    player.orgasm();
    dynStats("cor", 3);
    // [Next]
...
export function gapeByFeraEpilogue(): void {
    clearOutput();
...
// Tight
export function tightLikeAToigar(): void {
    clearOutput();
    ...
    if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.LEVEL_CLOWN_CAR) outputText(", slipping past your capacitive entrance with ease");
    else if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.GAPING) outputText(", sliding into your welcoming entrance fairly easily");
    else if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.LOOSE) outputText(", slowly working its way into your decently unstretched entrance");
    ...
    outputText("\n\nFera coos, \"<i>Atta girl, are you feeling it?  Can you feel the way your pussy is tightening up?  You can, I know, and your womb is getting thirstier and thirstier, just aching to squeeze the cum out of a helpless male or herm that you've taken the time to beat into their place, right, my pretty?</i>\"  Her fingers reach down to your black-stained cleft and rub around the hardening gunk that paints your crotch, making it look like someone painted liquid latex all across your " + skin(player) + ".  At the same time, the tendril lunges back in to your spunk-spouting cunt, plugging it just before it starts to unload once more, its stalk undulating between your legs with each thrust, rising up and straightening with each hip-shaking thrust it delivers to your increasingly supple, stained box.");
    ...
    // Maximum size to 2
    player.perks.create(PerkLib.FerasBoonMilkingTwat, 0, 0, 0, 0);
    if (player.vaginas[0].vaginalLooseness > VaginaLooseness.LOOSE) player.vaginas[0].vaginalLooseness = VaginaLooseness.LOOSE;
...
export function tightnessEpilogue(): void {
    clearOutput();
...
// Grow a Dick!
export function growADickForFera(): void {
    clearOutput();
    ...
    if (player.tailType > TailType.NONE) outputText(" and press your tail to the ground");
...
export function afterFeraGivesYouACawk(): void {
    clearOutput();
