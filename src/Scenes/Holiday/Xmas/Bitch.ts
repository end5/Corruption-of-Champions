...
export function xmasBitchEncounter(): void {
    outputText("", true);
    spriteSelect(9);
...
// [Decline]
export function declineXmasPresent(): void {
    spriteSelect(9);
    outputText("", true);
...
// [Open Present]
export function openXmasPresent(): void {
    spriteSelect(9);
    outputText("", true);
    ...
    if (player.cor >= 90 || game.monk >= 5 || player.effects.findByType(StatusAffects.Exgartuan) >= 0 || AmilyScene.amilyCorrupt() || flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00283] > 0 || flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00282] > 0 || flags[kFLAGS.NIAMH_STATUS] > 0) {
        ...
        outputText("Before you can utter a single word of confusion or protest, the elf moans and the cock erupts, spurting a rope of cum into your hair.  The next blast takes you across the nose, then on your lips, then your chin, and finally onto your " + allBreastsDescript(player) + ".  Shocked and dripping, you stand dumbfounded as the elf plants a kiss on your lips, tears off the box, and runs away with her cock flopping and buzzing in time with each step.  There's no way to catch her in this darkness.\n\n", false);
        ...
        Inventory.takeItem(ConsumableLib.COAL___, playerMenu);
        flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] = date.fullYear;
    }
        ...
        if (rand(2) == 0) Inventory.takeItem(ArmorLib.SS_ROBE, playerMenu);
        else Inventory.takeItem(ArmorLib.SSARMOR, playerMenu);
        flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] = date.fullYear;
    }
        ...
        Inventory.takeItem(ConsumableLib.SDELITE, playerMenu);
        flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] = date.fullYear;
    }
}

// [Unwrap the elf]
export function unwrapElfyPresent(): void {
    spriteSelect(9);
    outputText("", true);
...
export function futaDickXmasElfClr(): void {
    clearOutput();
    dickXmasElfGo();
}
export function futaVagXmasElfClr(): void {
    clearOutput();
    vagFuckXmasElf();
}
...
export function dickXmasElfGo(): void {
    ...
    if (player.cocks.cockArea(0) < 80) {
        ...
        if (player.cumQ() >= 250) outputText("  The spunk quickly fills her womb and flows back out her entrance, frothing and bubbling as it drips over your " + hipDescription(player) + ".  ", false);
        ...
        outputText("Aside from your " + cockDescript(game.player, 0) + ", your body has gone completely numb from the waist down.  You glance down and see your " + legs(player) + " twitching and convulsing under the elf's assault.   She's so hot inside, it's like her pussy's candy-coated in liquid pleasure.   You fuck her harder and harder, squeezing her hips tightly and roughly fucking her magical pussy.   Somehow you're on the edge again, about to unload.  You start counting in your head, trying to resist another orgasm.  You know if you do you'll never be able stop.  You doubt you'll even be able to think.\n\n", false);

        outputText("Your effort at holding out totally fails – you start counting in time with each thrust, tying the numbers into the feel of the velvet cock-sleeve as it massages your " + cockDescript(game.player, 0) + ".   The elf smiles condescendingly and taunts, \"<i>I can see you've already lost the ability to resist the pleasure.  Just give in, and enjoy the night.  I'll be riding you 'til morning.</i>\"\n\n", false);
        ...
        outputText("You snap out of it and the elf is smirking down at you, stationary.  You feel totally numb – your " + cockDescript(game.player, 0) + " has all of your sensation, all of your sense of touch.   On their own, your " + hipDescription(player) + " continually rise and fall, pumping into her with noisy explosions of pleasure that leave you gasping and writhing.  Every time you start to think something, the vise-like tunnel slides down you and squeezes your " + cockDescript(game.player, 0) + ", interrupting any attempt at thought.  You give up on thinking and start drooling all over yourself, your world fading into a heaven of constant pleasure and repeated orgasms.\n\n", false);
        ...
        if (player.breastRows.biggestTitSize() >= 2) outputText("making them jiggle", false);
        ...
        outputText(", a gentle pressure that builds higher and higher until you feel about to explode.  You need to cum, and you squirm in the elf's grasp, trembling and shuddering as one of her hands slips over a particularly sensitive spot. A bead of pre-cum rolls out of your " + cockHead(player) + " and starts sliding down the shaft, followed by another, and another, and another.\n\n", false);
        ...
        outputText("Thick ropes of your orgasmic goop splatter into her hair, and each successive blast comes out with less and less force, until you're leaking a steady stream of cum that rolls down your " + cockDescript(game.player, 0) + " and soaks the pointy-eared woman completely.   As you release your prodigious load the scent of cinnamon fills the air, and you dumbly wonder if it's some side effect of the magic she used on you.  Regardless, cum keeps leaking out over the elf and onto you, and you eventually lose consciousness from perpetual orgasm.", false);
    }
...
// [FEMALE SCENE]
export function vagFuckXmasElf(): void {
    outputText("She pushes you down onto your bedroll and whispers, \"<i>Time to enjoy your present.</i>\"\n\n", false);
...
// MANTASTIC FOLLOWUP:
export function xmasFuckFollowup(): void {
    hideUpDown();
    outputText("", true);
...
// FEMTASTIC FOLLOWUP:
export function xmasFuckFollowupFems(): void {
    hideUpDown();
    outputText("", true);
...
export function xmasSmart(): void {
    hideUpDown();
