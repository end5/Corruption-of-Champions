...
export function candyCaneTrapDiscovery(): void {
    clearOutput();
...
// LEAVE
export function leaveBeforeMeetingCandyCaneBun(): void {
    clearOutput();
...
// INVESTIGATE
export function investigateCandyCaneBun(): void {
    clearOutput();
...
// NO
export function declineCandyCaneCawks(): void {
    clearOutput();
...
// YES
export function helpWithTheCandyCane(): void {
    clearOutput();
...
export function getAChristmasChicken(): void {
...
// [Leave]
export function leaveXmasChicken(): void {
    clearOutput();
...
// [Talk]
export function talkToXmasChicken(): void {
    clearOutput();
...
// [Decline]
export function declineChickenBlowjobs(): void {
    clearOutput();
...
// [Accept]
export function acceptChristmasBJs(): void {
    // (male)
    if (player.gender == 1) {
...
export function acceptChristmasWomanBJ(): void {
    clearOutput();
...
export function encounterKamiTheChristmasRoo(): void {
...
export function approachKamiTheChristmasRoo(): void {
    clearOutput();
    if (flags[kFLAGS.KAMI_ENCOUNTER] == 1) {
        ...
        outputText("\n\nThe note reads: \"<i>\<i>Hey there " + mf(player, "stud", "babe") + "\</i>.  My name's Kami.  Wanna come out the back and fuck?  It can be my special present to you.</i>\"  She signed it with a winking smiley face");
...
export function skipOutAfterOpeningKamisBox(): void {
    clearOutput();
...
export function chooseLetsGoKami(): void {
    clearOutput();
...
export function giveKamiTheChristmasRooAnal(): void {
    ...
    if (player.cumQ() < 1000) outputText("  \"<i>Mmm... just like that, babe,</i>\" Kami moans submissively, her asshole clenching one last time before you pull out and climb off your satiated conquest, your " + cockDescript(game.player, x) + " still dripping creamy white spunk.");
    // {if cum quantity ≧ heavy}
    else
        outputText("  One of Kami's hands flies from her dick to her suddenly swollen belly as your mighty seed gushes forth.  The unexpectedly heavy girl sinks further into the ground, your load swishing back up and loosening her tunnel around you and forcing you to remove your still-spewing " + cockDescript(game.player, x) + " before something unfortunate happens.  Semen starts tumbling out as soon as you dismount your kangaroo conquest, but judging by the slow pace, she's going to look heavily pregnant for a while.");
...
export function takeItRooButtStyle(): void {
    clearOutput();
...
export function KamiBlowJob(): void {
    clearOutput();
...
export function getWinterPudding(): void {
    clearOutput();
    if (player.gems < 35) {
...
export function polarPete(): void {
    clearOutput();
...
// Unwrap
export function unwrapPolarPete(): void {
    clearOutput();
    ...
    player.orgasm();
    Inventory.takeItem(ConsumableLib.PEPPWHT, Camp.returnToCampUseOneHour);
}

// Leave
export function leaveDisFukkinPolarBear(): void {
    clearOutput();
...
export function peppermintWhite(player: Player): void {
    clearOutput();
...
export function nieveHoliday(): boolean {
    return date.month == 11 || date.month == 0;
}

export function snowLadyActive(): void {
    clearOutput();
    hideMenus();
...
export function nieveBuilding(): void {
    clearOutput();
    // First Step: The Body
...
// Gems
// Add to previous text if possible?
export function nieveGemEyes(): void {
    clearOutput();
    if (player.gems >= 9) {
...
// Snowwoman
export function nieveSnowWoman(): void {
    clearOutput();
...
// Snowman
// Add to existing text if possible, rather than a new window?
export function nieveSnowMan(): void {
    clearOutput();
...
// Reqs NIEVE_STAGE == 3
export function findACarrot(): void {
    clearOutput();
...
export function nieveMF(Man: string = "", Woman: string = ""): string {
    if (flags[kFLAGS.NIEVE_GENDER] == 1) return Man;
    else return Woman;
}

// IT'S ALIVE!
export function nieveComesToLife(): void {
    clearOutput();
...
export function yesKeepNieve(): void {
    clearOutput();
...
export function noNoKeepNieve(): void {
    clearOutput();
...
// Appearance Screen
export function approachNieve(): void {
    clearOutput();
...
// Sex Menu
export function nieveSexMenu(): void {
    clearOutput();
...
export function lickNieve(): void {
    clearOutput();
    ...
    if (!silly()) {
        ...
        // (Normal/Snake Tongue:
        if (player.tongueType != TongueType.DEMONIC) outputText("It doesn't get far, but elicits squeals of surprise and pleasure from your elemental slave who's already grasping at the snow surrounding her for support.");
        // Demon Tongue:
        else if (player.tongueType == TongueType.DEMONIC) outputText("Her eyes widen as you snake your impressive, demonic tongue into her. She grunts and squeals with pleasure, hands grasping piles of snow, wishing they were bedsheets.");
...
// For male Nieve
// Any gender
export function suckNieveOff(): void {
    clearOutput();
...
// Any Gender
// Nieve's cock is 9x1.5
export function nieveFucksYou(): void {
    clearOutput();
...
export function takeNieveAnal(): void {
    ...
    buttChange(player, 9, true, true, false);
...
// Vaginal
export function takeNieveVaginal(): void {
    clearOutput();
...
export function nieveIsOver(): void {
    clearOutput();
    hideMenus();
    // Nieve Not Completed
...
function fixNieveGender(arg: number = 1): void {
    flags[kFLAGS.NIEVE_GENDER] = arg;
    fixNieve();
}
function fixNieveMouth(arg: number = 1): void {
...
export function returnOfNieve(): void {
    clearOutput();
