...
// Introduction: -McGirt
export function datTurkeyRumpMeeting(): void {
...
// Shoo her Off -McGirt
export function shooTurkeyAway(): void {
    clearOutput();
...
// Let her Approach
export function letZeTurkeyApproach(): void {
...
export function stuffDatTurkeyWithSpoo(): void {
    clearOutput();
...
export function spitRoastThatTurkey(): void {
    clearOutput();
...
export function thatllDoTurkey(): void {
    clearOutput();
    ...
    if (player.perks.findByType(PerkLib.PilgrimsBounty) < 0) {
...
// Let her Approach:
export function femaleLetZeTurkeyGalApproach(): void {
    clearOutput();
...
// Appetiser:
export function turkeyAppetizer(): void {
    clearOutput();
...
// Main Course:
export function femaleAndTurkeyMainCourse(): void {
    clearOutput();
...
// DESERT
export function haveTurkeyGirlDesertInYourCunt(): void {
    clearOutput();
    ...
    if (player.perks.findByType(PerkLib.Cornucopia) < 0) {
        outputText("\n\n(<b>Gained Perk: Cornucopia - increases vaginal and anal capacities by 30.</b>)");
        player.perks.create(PerkLib.Cornucopia, 0, 0, 0, 0);
    }
...
export function turkeyGirlTwoTheTurkeningBySavinWhatADickInAButt(): void {
    clearOutput();
...
export function helloAgain(): void {
    clearOutput();
...
// Gravy Boat
export function gravyBoatDatTurkturk(): void {
    clearOutput();
...
// Season Her
export function seasonHerDatTurkeyBitch(): void {
    clearOutput();
    ...
    outputText("\n\nInstantly, you can see her pale flesh reddening, burning with desire.  Her fem-slime practically pours out of her over-excited pussy as her breath quickens, chest heaving her tits up and around your " + multiCockDescriptLight(player) + ".  Perfect! You pull your hand back out of her love-tunnel, slapping both her butt cheeks to wipe off the last of the lusty cream on her bare derriere, which flushes as red as her hair.  \"<i>Gobble!</i>\" she screams as you swat her ass, slamming her big hips down on your face, trying to get you to eat her needy box again even as she's panting and twitching, playing with her tits and your cock with shameless abandon.");

    outputText("\n\n\"<i>Gobbles, gobble,</i>\" you command, poking a finger into her drooling slit.");

    outputText("\n\n\"<i>Gobble!</i>\" she answers, rolling off of you and hiking her legs up, knees sinking into her bust to show off her slit for you, so eager and inviting.  You rise to your [feet], grabbing the lusty fowl behind her hooked knees and pulling her in to line up with your " + multiCockDescriptLight(player) + ".  \"<i>G-gobble!</i>\" she begs, fluttering her plumage against your [legs], staring up at you imploringly as her skin further reddens, so hot with desperate lust you can practically feel the sexual hunger radiating off of her.  Time to finish this turkey off in style!");
...
// Dessert
export function turkeyDesertBitches(): void {
    clearOutput();
    ...
    if (player.breastRows.biggestTitSize() > 1) outputText(", cupping a tit");
...
export function pigSlutRoastingGreet(): void {
    clearOutput();
    ...
    outputText("\n\n\"<i>Hey there " + mf(player, "handsome", "cutie") + "! We've got quite a unique special going on right now!</i>\" she exclaims in a cheery voice.");
...
export function getARoastPiggueOinkOinkOinkMotherfucker(): void {
    clearOutput();
    player.gems -= 1;
...
export function morePigRoast(): void {
    clearOutput();
...
export function tenderizeDatPigSloot(): void {
    clearOutput();
    ...
    if (player.tailType > TailType.NONE) outputText("You even bring your tail from behind your back to join in, the unfamiliar texture causing the sausage slut to let out surprised, high-pitched squeaks. ");
...
export function tenderizeHerHardcore(): void {
    clearOutput();
...
export function hamRoastTenderizeHerSoftcore(): void {
    clearOutput();
...
export function spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl(): void {
    clearOutput();
...
export function spankEpilogueForRizzles(): void {
    clearOutput();
...
export function getDoneEatingOutPigSlut(): void {
    clearOutput();
    ...
    if (player.breastRows.biggestTitSize() < 20) {
        outputText(" Your boobs also grow, gaining size as it seems that every part of your body wants to join in on the growth.");
        if (player.breastRows.biggestTitSize() < 3) growTits(player, 3, player.breastRows.length, false, 1);
        if (player.breastRows.biggestTitSize() < 6) growTits(player, 2, player.breastRows.length, false, 1);
        if (player.breastRows.biggestTitSize() < 12) growTits(player, 2, player.breastRows.length, false, 1);
        if (player.breastRows.biggestTitSize() < 20) growTits(player, 2, player.breastRows.length, false, 1);
        changed = true;
    }
    outputText("\n\nYou sigh");
