...
export function maleTooBigVictorySex(): void {
    ...
    outputText("\n\nExhausted at last, you pat your " + game.player.cockDescript(x) + " affectionately.  You'd wipe it off on the witch's hair, if it wasn't messier than the " + skin(player) + " you plan to clean.  She begins to lick her fingers and clean the stuff off her face.  You just laugh, and get dressed.  There's still much to do.");
    player.orgasm();
    if (game.inDungeon) {
        if (game.inCombat)
            cleanupAfterCombat();
...
export function fuckTwoPregWitches(): void {
    ...
    const y: number = player.cockThatFits2(50);
