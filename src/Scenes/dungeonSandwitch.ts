...
export function forceDildos(): void {
    ...
    if (player.vaginas.length > 0) player.cuntChange(10, true, true, false);
...
export function explainYourSelfToZeSandBitch(): void {
    ...
    if (player.cor > player.inte || monk >= 5 || player.effects.findByType(StatusAffects.Exgartuan) >= 0 || amilyScene.amilyCorrupt() || flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00283] > 0 || flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00282] > 0 || flags[kFLAGS.NIAMH_STATUS] > 0) {
...
export function grabTheMilkGirl(): void {
    ...
    if (player.vaginas.length > 0 && player.breasts.biggestTitSize() >= 3) addButton(1, "Lady Fuck", ladyFucks);
