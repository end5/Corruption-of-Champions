...
export function dungeonSuccubusForceFeed(): void {
    ...
    outputText("The succubus tentatively takes a " + nippleDescript(0) + " into her mouth, sighing at the tangy taste of your sweat-drenched skin.  Her lips gently suckle, wrapping around the perky hardening nub as it fills with arousal and milk.  You feel something let go inside your " + breastDescript(game.player, 0) + " and the succubus smiles, now working to free more of your trapped cream.\n\n", false);
...
export function incubusLossRape(): void {
    ...
    // Tentacle gangbang
        ...
        if (player.vaginas.length > 0 && (player.cocks.length == 0 || rand(2))) {
            ...
            if (player.cor < 80) outputText("You feel your " + legs(player) + " lifted and pulled tight as countless demonic cocks encircle your body, binding and constraining you further.  You whimper as a demonic tentacle probes your back door while a thicker one lines itself up just below your " + clitDescript() + ".  ", false);
...
export function incubusVictoryRapeSex(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.gender == 3) {
            if (player.balls > 0) outputText(multiCockDescriptLight(player) + ", " + ballsDescriptLight() + ", and " + vaginaDescript(0), false);
            else outputText(multiCockDescriptLight(player) + " and " + vaginaDescript(0), false);
        }
        else {
            if (player.balls > 0) outputText(multiCockDescriptLight(player) + " and " + ballsDescriptLight(), false);
    ...
    // (VAGINAL - Requires Vagina)
        ...
        if (player.gender == 3) {
            if (player.balls > 0) outputText(multiCockDescriptLight(player) + ", " + ballsDescriptLight() + ", and " + vaginaDescript(0), false);
            else if (player.balls > 0) outputText(multiCockDescriptLight(player) + " and " + vaginaDescript(0), false);
        }
        else {
        ...
        outputText("Relaxing the muscles in your " + legs(player) + ", you let a few inches of his length slip inside you, every nub and nodule of his corrupted prick filling the walls of your love-canal with inhuman pleasures that make your knees weak.  A particularly delightful bump brushes your " + clitDescript() + ", causing your " + legs(player) + " to finally give out. The incubus' nubbly cock plunges entirely inside you.\n\n", false);
...
export function demonBadEnd(): void {
    ...
    // [HERM ENDING]
        ...
        outputText(multiCockDescript(player) + " and " + vaginaDescript(0) + " grow wet and ready, both starting to leak fluids as the succubus' hand traces your inner thigh.  She teases, \"<i>Oh my! You're so wet and ready and I haven't even touched your moist little cum-receptacle.  And that throbbing cock!  How obscene!  You're a slut aren't you?  Who else would be so turned on by the idea of cumming until your humanity is splattered between my legs?</i>\"\n\n", false);
