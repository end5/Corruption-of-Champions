...
export function masturbateMenu(): void {
    menu();
    if (player.cocks.length > 0 && (player.cocks[0].cockType == CockTypesEnum.BEE)) {
        clearOutput();
    ...
    if (player.ovipositor.canOviposit() && player.breastRows.hasFuckableNipples() && player.lust >= 33 && player.breastRows.biggestTitSize() >= 21) {
...
function fappingItems(menus: boolean = true): boolean {
    ...
    if (player.keyItems.has("Fake Mare") >= 0 && player.cocks.length > 0 && player.isTaur()) {
...
export function masturbateGo(): void {
    clearOutput();
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0 && player.effects.getValue2Of(StatusAffects.Exgartuan) == 0) {
        if (player.isNaga() && rand(2) == 0 && player.effects.getValue1Of(StatusAffects.Exgartuan) == 1)
            Exgartuan.exgartuanNagaStoleMyMasturbation();
        else Exgartuan.exgartuanMasturbation();
        return;
    }
    if (player.cocks.countCockSocks("gilded") > 0 && flags[kFLAGS.GILDED_JERKED] < player.cocks.countCockSocks("gilded")) {
        gildedCockTurbate();
        return;
    }
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length == 1) {
            outputText("You stroke your " + cockDescript(player));
            if (player.lib < 45)
    ...
    if (player.vaginas.length > 0) {
        if (player.vaginas.length == 1) {
            ...
            if (player.lib < 45)
                outputText("You touch and play with your " + vaginaDescript(player) + ", ");
            else if (player.lib < 70)
                outputText("You slap your pussy softly, ");
            else outputText("You touch your enflamed and aroused " + vaginaDescript(player) + ", ");
            switch (player.vaginas[0].vaginalWetness) {
                ...
                case VaginaWetness.DROOLING:
                    outputText("slicking your fingers in the juices that constantly dribble from " + vaginaDescript(player) + "  "); break;
                default:
        ...
        if (player.vaginas.length > 1) {
            ...
            else outputText("Touch your enflamed and aroused " + vaginaDescript(player) + "s, ");
            switch (player.vaginas[0].vaginalWetness) {
                case VaginaWetness.DRY:
                    outputText("expertly arousing your female parts.  "); break;
                case VaginaWetness.NORMAL:
                ...
                case VaginaWetness.SLICK:
                    outputText("smiling as your fingers become coated in your slick fluids.  "); break;
                case VaginaWetness.DROOLING:
                    outputText("slicking your fingers in the juices that constantly dribble from " + vaginaDescript(player) + "s  "); break;
                default:
    ...
    if (player.cocks.length == 1) {
        ...
        if (player.cocks[0].cockThickness < 1.8)
            outputText("You easily wrap a hand around your " + cockDescript(player) + " and start masturbating.  ");
        else if (player.cocks[0].cockThickness < 3) {
            outputText("You have some difficulty fitting your hand around your " + cockDescript(player) + ", relishing the feelings of your ");
            ...
            outputText("You use both hands to grip your " + cockDescript(player) + ", feeling the throbbing of your " + (player.hasKnot() ? "knot" : "member") + " as you begin to masturbate.  ");
        }
        else outputText("You grasp onto your " + cockDescript(player) + " with both hands, but fail to encircle it fully as you begin to masturbate.  ");
        // If length > 12 proud
        // if length > 16 looong strokes (maybe tittyfucK?)
        // if length > 20 selfsuck
        if (player.cocks[0].cockLength < 12) {
            if (player.cocks.normalCocks() >= 1)
                outputText("You stroke quickly, pleasuring your sensitive dick, darting down to fondle the base of your cock.  ");
            else if (player.cocks.horseCocks() >= 1)
                outputText("You stroke quickly, reveling in your sensitive horseflesh, darting down to fondle your sensitive sheath.  ");
            else if (player.cocks.dogCocks() >= 1)
            ...
            else if (player.cocks.tentacleCocks() >= 1)
            ...
            else if (player.cocks.demonCocks() >= 1)
            ...
            else if (player.cocks.catCocks() >= 1)
                outputText("You stroke quickly, feeling the tiny 'barbs' of your " + cockDescript(player) + " sliding through your fingers, even darting down to circle the sensitive skin around your sheath.  ");
            else if (player.cocks.lizardCocks() >= 1)
                outputText("You stroke quickly, pleasuring your sensitive " + cockDescript(player) + ", sliding fingers over each ridge and bump that covers its knotty length.  ");
            else if (player.cocks.anemoneCocks() >= 1)
                outputText("You stroke quickly, gasping as your fingers are stung repeatedly with the aphrodisiac-laced tentacles around the base of your " + cockDescript(player) + " and under its crown.  ");
            else if (player.cocks.displacerCocks() >= 1)
            ...
            if (player.cocks.normalCocks() >= 1)
                outputText("You delight in teasing the crown of your " + cockDescript(player) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ");
            else if (player.cocks.horseCocks() >= 1)
                outputText("You delight in teasing the sensitive flared tip of your " + cockDescript(player) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and ripple with each stroke, responding to every touch.  ");
            else if (player.cocks.dogCocks() >= 1)
                outputText("You delight in teasing the pointed tip of your " + cockDescript(player) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ");
            else if (player.cocks.tentacleCocks() >= 1)
                outputText("You delight in teasing the over-sized mushroom-like tip of your " + cockDescript(player) + ", caressing it after every stroke as you squeeze out dollops of pre to smear along its slimy length.  It writhes and twists in your hands of it's own volition, lengthening and shortening with each set of strokes.  ");
            else if (player.cocks.demonCocks() >= 1)
                outputText("You delight in teasing the larger bumps that form a ring around the crown of your " + cockDescript(player) + ", watching as they twitch and spasm in time with the dollops of pre you're squeezing out and smearing over the entire length.  ");
            else if (player.cocks.catCocks() >= 1)
                outputText("You delight in teasing the sensitive nubs all along your " + cockDescript(player) + ", circling the cock-tip at the end of each stroke to gather pre and slather it over your entire length.  Each of the tiny 'barbs' provides bursts of pleasure with each stroke, driving you on.  ");
            else if (player.cocks.lizardCocks() >= 1)
                outputText("You delight in teasing the rounded bulbs that cover your " + cockDescript(player) + ", circling them with your fingertips before sliding up to the urethra and gathering a drop of pre.  You smear it over your sensitive reptile skin and revel in the pleasure radiating through you.  ");
            else if (player.cocks.anemoneCocks() >= 1)
                outputText("You delight in grabbing hold of the tiny, stinging tentacles around the base and squeezing them between your " + cockDescript(player) + " and hand.  Aphrodisiac pours into your blood as you release them and stroke along the length, gathering dollops of pre to coat yourself with and 'accidentally' bumping the other tentacles at the crown.  ");
            else if (player.cocks.displacerCocks() >= 1)
                outputText("You delight in teasing the opened tip of your " + cockDescript(player) + ", rubbing it at the end of each stroke, watching it squeeze out dollops of pre that you smear over it to tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ");
            else outputText("You delight in teasing the crown of your " + cockDescript(player) + ", rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ");
        }
        else if (player.cocks[0].cockLength < 26) {
            if (player.cocks.normalCocks() >= 1)
                outputText("The head of your " + cockDescript(player) + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
            else if (player.cocks.horseCocks() >= 1)
                outputText("The flared tip of your " + cockDescript(player) + " wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ");
            else if (player.cocks.dogCocks() >= 1)
                outputText("The pointed tip of your " + cockDescript(player) + " angles towards your face as you masturbate, a dollop of pre slowly leaking from it.  ");
            else if (player.cocks.tentacleCocks() >= 1)
                outputText("The overly wide head of your " + cockDescript(player) + " bumps against your lips as you masturbate, waving back and forth like a snake as it searches for an orifice.  ");
            else if (player.cocks.demonCocks() >= 1)
                outputText("The purplish head of your " + cockDescript(player) + " bumps against your lips as you masturbate, flushing darkly with every beat of your heart.  ");
            else if (player.cocks.catCocks() >= 1)
                outputText("The slightly pointed tip of your " + cockDescript(player) + " bumps against your lips as you masturbate, flushing with blood as your spines grow thicker in your hand.  ");
            else if (player.cocks.lizardCocks() >= 1)
                outputText("The pointed, purple tip of your " + cockDescript(player) + " bumps against your lips while its knotted surface flushes near-purple and seems to grow thicker.  ");
            else if (player.cocks.anemoneCocks() >= 1)
                outputText("The tentacle-ringed tip of your " + cockDescript(player) + " brushes against your lips, making them tingle with artificial heat while you stroke it.  ");
            else if (player.cocks.displacerCocks() >= 1)
            ...
            if (player.breastRows.hasFuckableNipples() && player.breastRows.biggestTitSize() >= 3) {
            ...
            if (player.breastRows.canTitFuck() && player.breastRows.biggestTitSize() > 3 && !nippleFuck) {
            ...
            if (player.cocks.normalCocks() >= 1)
                outputText("The head of your " + cockDescript(player) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ");
            else if (player.cocks.horseCocks() >= 1)
                outputText("The flared tip of your " + cockDescript(player) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with horsemeat.  ");
            else if (player.cocks.dogCocks() >= 1)
                outputText("The head of your " + cockDescript(player) + " wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your " + cockDescript(player) + ".  ");
            else if (player.cocks.tentacleCocks() >= 1)
                outputText("The bulbous, mushroom-like head of your " + cockDescript(player) + " pushes against your face eagerly, smearing pre-cum over your lips as it seeks entrance to the nearest orifice.  You're unable to resist opening your mouth to suck it down, filling your mouth with slick rubbery cock-tentacle.  ");
            else if (player.cocks.demonCocks() >= 1)
                outputText("The tainted swollen head of your " + cockDescript(player) + " pushes against your face, smearing sweet pre-cum over your lips.  You're unable to resist opening wide and taking in the demonic member, submitting wholly to the desire to pleasure your corrupted body-parts.  ");
            else if (player.cocks.catCocks() >= 1)
                outputText("The pointed tip of your " + cockDescript(player) + " pushes against your face, smearing pre-cum over your lips and tickling you with the many barbs.  You're unable to resist opening wide and taking in the " + cockDescript(player) + ", half-humming half-purring in contentment.  ");
            else if (player.cocks.lizardCocks() >= 1)
                outputText("The slightly-pointed tip of your " + cockDescript(player) + " pushes against your face, smearing pre-cum over your eager lips.   You can't resist opening wide and slipping it inside as your hands caress the reptilian bulges along your length.  ");
            else if (player.cocks.anemoneCocks() >= 1)
                outputText("The rounded, tentacle-ringed tip of your " + cockDescript(player) + " slides against your face, smearing pre-cum over your lips and stinging them with aphrodisiacs that make you pant with lust.  You can't resist opening wide to greedily slurp it down, and in seconds tiny, tingling stings are erupting through your oral cavity, filling you with lust and pleasure.  ");
            else if (player.cocks.displacerCocks() >= 1)
            ...
            if (player.breastRows.hasFuckableNipples() && player.breastRows.biggestTitSize() >= 3) {
                ...
                if (player.cor > 60) {
                    if (player.cocks.normalCocks() >= 1)
                        ...
                        outputText("The sweet scent of your " + cockDescript(player) + " fills your nostrils as inch after inch of tentacle forces its way down your throat.  You struggle briefly with it, wrestling to keep it from pushing the whole way into your gut, swooning with the pleasure that fighting with your own cock induces.  ");
                    else if (player.cocks.demonCocks() >= 1)
                        outputText("The spicy demonic odor your " + cockDescript(player) + " radiates fills your nostrils as you force inch after inch of tainted meat down your throat.  You struggle briefly, but your " + cockDescript(player) + " quickly overwhelms your resistance and your gag reflex momentarily seems to disappear.  ");
                    else if (player.cocks.catCocks() >= 1)
                        ...
                        outputText("The salty, dry odor of your " + cockDescript(player) + " fills your nostrils as you force inch after inch into your mouth, swallowing as much of the bulgy shaft as you can.  ");
                    else if (player.cocks.anemoneCocks() >= 1)
                        outputText("The stinging, aphrodisiac-laced " + cockDescript(player) + " slowly works its way down your throat as you swallow it deeper and deeper, gurgling happily as your throat grows more sensitive.  ");
                    else if (player.cocks.displacerCocks() >= 1)
                        outputText("The strong animal scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ");
        ...
        outputText("You grasp one of your " + multiCockDescriptLight(player) + " in each hand, ");
        if (player.cocks.averageCockThickness() <= 1.8)
        ...
        // Get into it
        // Play with it if <= 10"
        if (player.cocks.averageCockLength() <= 10)
        ...
        // Tittyfuck if D cup+
        else if (player.cocks.averageCockLength() < 20) {
            ...
            if (player.breastRows.hasFuckableNipples()) {
                ...
                if (player.cocks.dogCocks() > 0) {
                    ...
                    else if (player.cocks.dogCocks() != player.cocks.length) { // if you have something that will fit
                        nippleFuck = true;
                        autofellatio = true;
                        // UNFINISHED
                        if (player.cocks.normalCocks() > 0) titFuckSingle();
            ...
            if (player.breastRows.biggestTitSize() <= 3 && !nippleFuck) {
            ...
            if (player.breastRows.hasFuckableNipples()) {
                ...
                if (player.cocks.dogCocks() > 0) {
                    ...
                    if (player.breastRows.hasFuckableNipples()) {
                        ...
                        nippleFuck = true;
                        autofellatio = true;
                        if (player.cocks.normalCocks() > 0 || player.cocks.horseCocks() > 0) titFuckSingle();
                    }  // if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
                }
                else {
                    nippleFuck = true;
    ...
    else if (player.cocks.length >= 3) { // Three or more cocks
        // Grab it
        // Play with sheath if has one
        if (player.cocks.hasSheath()) {
            ...
            if (player.breastRows.hasFuckableNipples()) {
                ...
                if (player.cocks.dogCocks() > 0) {
                    ...
                    if (player.breastRows.hasFuckableNipples()) {
                        ...
                        nippleFuck = true;
                        autofellatio = true;
                        if ((player.cocks.normalCocks() > 0 || player.cocks.horseCocks() > 0) && player.cocks.length - player.cocks.dogCocks() == 1) titFuckSingle();
                        if (player.cocks.length - player.cocks.dogCocks() > 1) multiTitFuck();
                    } // if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
                }
                else {
                    nippleFuck = true;
        ...
        // Deepthroat yourself, maybe titfuck if equipped.
        if (player.cocks.averageCockLength() >= 20) {
            // autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas.length == 1) {
            ...
            // Big clits
                ...
                if (player.vaginas[0].vaginalWetness > VaginaWetness.DRY) outputText("glistening ");
            ...
            switch (player.vaginas[0].vaginalWetness) {
                ...
                case VaginaWetness.DROOLING:
                    outputText("Warm wetness runs down your legs in thick streams, pouring from your " + vaginaDescript(player) + ".  You smile sheepishly and stroke up your now slickened legs to your pussy-lips, parting them and letting your fingers dive inside the wet channel.  "); break;
                default:
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.length == 1) {
            if (player.lib < 30)
                ...
                outputText("You roll and begin to doze, your semi-erect " + cockDescript(player) + " flopping against you. ");
            else if (player.lib <= 80) {
                outputText("As you close your eyes and relax, your " + cockDescript(player) + " surges back to erectness, ensuring ");
                if (player.cor < 50)
                    outputText("your dreams will be filled with sex.");
                else outputText("you dream in a depraved kinky fantasia.");
            }
            else outputText("You groan and drift to sleep, your rigid " + cockDescript(player) + " pulsing and throbbing with continual lust.");
        }
        // Multi Cock
        else {
            if (player.lib < 30)
...
function genderlessMasturbate(): void {
    ...
    // Tit foreplay
    titForeplay();
    if (player.effects.findByType(StatusAffects.FappedGenderless) < 0) { // first time as a genderless person
        ...
        player.effects.create(StatusAffects.FappedGenderless, 0, 0, 0, 0);
    }
    // All times as a genderless person (possibly written for all genders perhaps not herm (not enough hands)) -
    outputText("Your " + assholeDescript(player) + " begins to twitch. It's practically crying out for attention.\n\n");
    outputText("You lay down on your side and reach gingerly behind yourself.  The palm of your hand comes to rest on your " + buttDescription(player) + ".  You slide your finger into your crack and find your " + assholeDescript(player) + ".  You run your finger slowly around the sensitive ring of your hole and feel a tingle emanating from it. A smile creeps across your lips as you begin to imagine what is about to happen.\n\n");
    ...
    if (player.ass.analWetness < 2) {
        ...
        outputText(" in a generous helping of saliva and head back for your " + assholeDescript(player) + ".\n\n");
    }
...
function titForeplay(): void {
    ...
    if (player.breastRows.length == 1 && player.breastRows.biggestTitSize() > 3) {
        ...
        if (player.lib < 45)
            outputText("You hesitantly run your hands across your front, caressing each of your " + num2Text(player.breastRows.totalBreasts()) + " breasts in turn. ");
        else if (player.lib < 70)
            outputText("You run your hands across your front, squeezing each of your " + num2Text(player.breastRows.totalBreasts()) + " breasts in turn. ");
        else outputText("You aggressively run your hands across your front, groping each of your " + num2Text(player.breastRows.totalBreasts()) + " breasts in turn. ");
        ...
        if (player.cor <= 25)
            ...
            outputText("Though a small part of you is horrified by your abnormal endowments, it doesn't stop you from mashing your " + allBreastsDescript(player) + " together");
    ...
    if (player.breastRows.hasFuckableNipples() && player.breastRows.biggestTitSize() > 3) {
        ...
        else if (player.breastRows.averageNipplesPerBreast() > 1)
            outputText("many ");
        ...
        if (player.vaginas.averageVaginalWetness() < 2)
            ...
            // Lactating?
            if (player.breastRows.averageLactation() > 0) outputText(" milky ");
            outputText("girlcum squirt out of your engorged nipples. ");
        }
        // Special extras for lactation
        if (player.breastRows.averageLactation() > 0 && player.breastRows.averageLactation() < 2)
        ...
        else if (player.breastRows.averageLactation() < 3)
    ...
    // If nipples aren't fuckable.
    else if (player.breastRows.length > 0 && player.breastRows.biggestTitSize() > 3) {
        outputText(", emitting ");
        ...
        if (player.breastRows.totalNipples() > 2) outputText("many ");
        ...
        if (player.breastRows.biggestLactation() > 1 && player.breastRows.biggestLactation() < 2)
            outputText("Droplets of milk dribble from each nipple, spattering milk onto your " + legs(player) + " and crotch.  ");
        else if (player.breastRows.biggestLactation() < 3 && player.breastRows.biggestLactation() >= 2)
            outputText("Thin squirts of milk spray from each nipple, spattering milk onto your " + legs(player) + " and crotch.  ");
        else if (player.breastRows.biggestLactation() >= 3) outputText("A constant stream of milk drizzles from each teat, soaking your " + legs(player) + " and crotch.  ");
    }
}
...
function titFuckSingle(): void {
    ...
    if(player.lib < 45) outputText("You grip your " + cockDescript(game.player, 0) + " and begin cautiously guiding it towards ", false);
    if(player.lib >= 45 && player.lib < 70) outputText("Shivering with anticipation, you place the tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
    if(player.lib >= 70) outputText("Without hesitation, you shove the tip of your " + cockDescript(game.player, 0) + " into ", false);
    //More than one row?
    ...
    if(player.breastRows.averageNipplesPerBreast() > 1) outputText("many ", false);
    ...
    if (player.lib < 45)
        outputText("You grip your " + cockDescript(player) + " and begin cautiously guiding it towards ");
    else if (player.lib < 70) {
        ...
        outputText("tip of your " + cockDescript(player) + " against the opening of ");
        /* Old method
                if(player.cocks.horseCocks() > 0) outputText("Shivering with anticipation, you place the flared tip of your " + horseDescript(0) + " against the opening of ", false);
                else if(player.cocks.normalCocks() > 0) outputText("Shivering with anticipation, you place the tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
                else if(player.cocks.dogCocks() > 0) outputText("Shivering with anticipation, you place the pointed tip of your " + dogDescript(0) + " against the opening of ", false);
                else if(player.cocks.tentacleCocks() > 0) outputText("Shivering with anticipation, you place the bulbous tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
                else if(player.cocks.demonCocks() > 0) outputText("Shivering with anticipation, you place the bump-encircled tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
                else if(player.cocks.catCocks() > 0) outputText("Shivering with anticipation, you place the tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
                else if(player.cocks.anemoneCocks() > 0 || player.cocks.displacerCocks() > 0) outputText("Shivering with anticipation, you place the wriggling tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
                else outputText("Shivering with anticipation, you place the tip of your " + cockDescript(game.player, 0) + " against the opening of ", false);
        */
    }
    else {
        ...
        switch (player.cocks[0].cockType) { // Different way of choosing cock to use, results in consistent description of cock[0]
            ...
            case CockTypesEnum.TENTACLE:
                ...
                break;
            default:
                outputText(cockHead(player) + " of ");
        }
        outputText("your " + cockDescript(player) + " into ");
        /* Old method
                if(player.cocks.normalCocks() > 0) outputText("Without hesitation, you shove the tip of your " + cockDescript(game.player, 0) + " into ", false);
                else if(player.cocks.horseCocks() > 0) outputText("Without hesitation, you shove the engorged flare of your " + cockDescript(game.player, 0) + " into ", false);
                else if(player.cocks.dogCocks() > 0) outputText("Without hesitation, you shove the pointed tip of your " + dogDescript(0) + " into ", false);
                else if(player.cocks.tentacleCocks() > 0) outputText("Without hesitation, you shove the over-sized mushroom that is your " + cockDescript(game.player, 0) + " into ", false);
                else if(player.cocks.demonCocks() > 0) outputText("Without hesitation, you shove the bump-ringed monstrosity that is your " + cockDescript(game.player, 0) + " into ", false);
                else if(player.cocks.anemoneCocks() > 0) outputText("Without hesitation, you shove the tentacle-laden mushroom that is your " + cockDescript(game.player, 0) + " into ", false);
                else if(player.cocks.displacerCocks() > 0) outputText("Without hesitation, you shove the opened top of your " + cockDescript(game.player, x) + " into ");
                else outputText("Without hesitation, you shove the " + cockHead(0) + " of your " + cockDescript(game.player, 0) + " into ", false);
        */
    }
    // More than one row?
    ...
    // More than 1 nip per boob?
    if (player.breastRows.averageNipplesPerBreast() > 1) outputText("many ");
    // Different based on looseness (again)
    if (player.vaginas.averageVaginalLooseness() < 2)
    ...
    if (player.breastRows.averageLactation() == 0) {
        if (player.vaginas.averageVaginalWetness() < 2)
            outputText("Y");
        else if (player.vaginas.averageVaginalWetness() < 4) {
            ...
            outputText("your " + cockDescript(player) + " and y");
            /* Old method
                        if(player.cocks.normalCocks() > 0) outputText("Slick juices dribble down the skin of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.horseCocks() > 0) outputText("Slick juices dribble down the mottled skin of your " + horseDescript(0) + " and y", false);
                        else if(player.cocks.dogCocks() > 0) outputText("Slick juices dribble down the red, shiny skin of your " + dogDescript(0) + " and y", false);
                        else if(player.cocks.tentacleCocks() > 0) outputText("Slick juices dribble down the rubbery skin of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.demonCocks() > 0) outputText("Slick juices dribble down the shiny purplish skin and nodules of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.lizardCocks() > 0) outputText("Slick juices dribble down the bumpy purple skin of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.catCocks() > 0) outputText("Slick juices dribble over the pink, spiny protrusions that cover your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.anemoneCocks() > 0) outputText("Slick juices dribble over the nearly transparent skin of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.displacerCocks() > 0) outputText("Slick juices dribble over the dusky purple of your " + cockDescript(game.player, 0) + " and y", false);
                        else outputText("Slick juices dribble down the skin of your " + cockDescript(game.player, 0) + " and y", false);
            */
        }
        else outputText("Slick juices squirt out from around your " + cockDescript(player) + " and y");
    }
    else {
        if (player.breastRows.averageLactation() < 2) {
            ...
            outputText("your " + cockDescript(player) + " and y");
            /* Old method
                        if(player.cocks.normalCocks() > 0) outputText("Rivulets of milky girlcum drizzle down the skin of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.horseCocks() > 0) outputText("Rivulets of milky girlcum drizzle down the mottled skin of your " + horseDescript(0) + " and y", false);
                        else if(player.cocks.dogCocks() > 0) outputText("Rivulets of milky girlcum drizzle down the red, shiny skin of your " + dogDescript(0) + " and y", false);
                        else if(player.cocks.tentacleCocks() > 0) outputText("Rivulets of milky girlcum drizzle down the rubbery skin of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.demonCocks() > 0) outputText("Rivulets of milky girlcum drizzle down the shiny purplish skin and nodules of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.catCocks() > 0) outputText("Rivulets of milky girlcum drizzle down the spiny, pink flesh of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.lizardCocks() > 0 || player.cocks.displacerCocks()) outputText("Rivulets of milky girlcum drizzle over the purplish, knotted flesh of your " + cockDescript(game.player, 0) + " and y", false);
                        else if(player.cocks.anemoneCocks() > 0) outputText("Rivulets of milky girlcum drizzle and drip from the many tiny tentacles of your " + cockDescript(game.player, 0) + " and y", false);
                        else outputText("Rivulets of milky girlcum drizzle over the sensitive skin of your " + cockDescript(game.player, 0) + " and y", false);
            */
        }
        else {
            outputText("Milky girlcum squirts out from around your " + cockDescript(player) + ", staining ");
            switch (player.cocks[0].cockType) { // Different way of choosing cock to use, results in consistent description of cock[0]
                case CockTypesEnum.ANEMONE:
            ...
            /* Old method
                        if(player.cocks.normalCocks() > 0) outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining it white.  Y", false);
                        else if(player.cocks.horseCocks() > 0) outputText("Milky girlcum squirts out from around your " + horseDescript(0) + ", staining its mottled skin white.  Y", false);
                        else if(player.cocks.dogCocks() > 0) outputText("Milky girlcum squirts out from around your " + dogDescript(0) + ", staining its shiny skin white.  Y", false);
                        else if(player.cocks.tentacleCocks() > 0) outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining its rubbery skin white.  Y", false);
                        else if(player.cocks.demonCocks() > 0) outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining its purplish-skin white.  Y", false);
                        else if(player.cocks.catCocks() > 0) outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining the pink kitty-skin white.  Y", false);
                        else if(player.cocks.lizardCocks() > 0 || player.cocks.displacerCocks() > 0) outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining the purple, knotty flesh white.  Y", false);
                        else if(player.cocks.anemoneCocks() > 0) outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining the odd aquatic shaft wide.  Y", false);
                        else outputText("Milky girlcum squirts out from around your " + cockDescript(game.player, 0) + ", staining its length white.  Y", false);
            */
        }
    }
    ...
    if (Math.round(player.cocks.cockArea(0)) >= player.vaginalCapacity()) {
        ...
        if (player.sens < 45)
            outputText("ou grunt with exertion as you attempt to stuff your " + cockDescript(player) + " into ");
        else if (player.sens < 70)
        ...
        if (Math.round(player.cocks.cockArea(0)) == player.vaginalCapacity()) {
            if (player.vaginas.averageVaginalLooseness() < 2)
                ...
                switch (player.cocks[0].cockType) { // Different way of choosing cock to use, results in consistent description of cock[0]
                    case CockTypesEnum.ANEMONE:
                        outputText("The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + cockDescript(player) + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  ");
                        break;
                    case CockTypesEnum.CAT:
                        outputText("The swollen tips of your bloated nipples stretch around the barbed tip of your " + cockDescript(player) + ", swallowing it like an enormous mouth.  ");
                        break;
                    case CockTypesEnum.DEMON:
                        outputText("The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + cockDescript(player) + ", swallowing it like an enormous mouth.  ");
                        break;
                    case CockTypesEnum.DISPLACER:
                        outputText("The swollen tips of your bloated nipple wrap around the outstretched head of your " + cockDescript(player) + ", convulsing with wet, squishing sounds as it wriggles inside you.  ");
                        break;
                    case CockTypesEnum.DOG:
                    case CockTypesEnum.FOX:
                        outputText("The swollen lips of your bloated nipple stretch around the pointed tip of your " + cockDescript(player) + " swallowing it like an enormous mouth.  ");
                        break;
                    case CockTypesEnum.HORSE:
                        outputText("The swollen lips of your bloated nipple stretch around the flared tip of your " + cockDescript(player) + " swallowing it like an enormous mouth.  ");
                        break;
                    case CockTypesEnum.HUMAN:
                        outputText("The swollen lips of your bloated nipple stretch around the tip of your " + cockDescript(player) + " swallowing it like an enormous mouth.  ");
                        break;
                    case CockTypesEnum.LIZARD:
                        outputText("The swollen tips of your bloated nipple wrap around the pointed tip of your " + cockDescript(player) + ", stretching oddly as it swallows the knot-covered appendage.  ");
                        break;
                    case CockTypesEnum.TENTACLE:
                        outputText("The swollen lips of your bloated nipple stretch around the rounded tip of your " + cockDescript(player) + ", swallowing it like an enormous mouth.  ");
                        break;
                    default:
                        outputText("tips of your bloated nipples wrap around the " + cockHead(player) + " of your " + cockDescript(player) + ", swallowing it like an enormous mouth.  ");
                }
                /* Old method
                                if(player.cocks.normalCocks() > 0) outputText("The swollen lips of your bloated nipple stretch around the tip of your " + cockDescript(game.player, 0) + " swallowing it like an enormous mouth. ", false);
                                else if(player.cocks.horseCocks() > 0) outputText("The swollen lips of your bloated nipple stretch around the flared tip of your " + horseDescript(0) + " swallowing it like an enormous mouth. ", false);
                                else if(player.cocks.dogCocks() > 0) outputText("The swollen lips of your bloated nipple stretch around the pointed tip of your " + dogDescript(0) + " swallowing it like an enormous mouth. ", false);
                                else if(player.cocks.tentacleCocks() > 0) outputText("The swollen lips of your bloated nipple stretch around the rounded tip of your " + cockDescript(game.player, 0) + ", swallowing it like an enormous mouth. ", false);
                                else if(player.cocks.demonCocks() > 0) outputText("The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + cockDescript(game.player, 0) + ", swallowing it like an enormous mouth. ", false);
                                else if(player.cocks.catCocks() > 0) outputText("The swollen tips of your bloated nipples stretch around the barbed tip of your " + cockDescript(game.player, 0) + ", swallowing it like an enormous mouth. ", false);
                                else if(player.cocks.lizardCocks() > 0) outputText("The swollen tips of your bloated nipple wrap around the pointed tip of your " + cockDescript(game.player, 0) + ", stretching oddly as it swallows the knot-covered appendage. ", false);
                                else if(player.cocks.anemoneCocks() > 0) outputText("The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + cockDescript(game.player, 0) + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  ", false);
                                else if(player.cocks.displacerCocks() > 0) outputText("The swollen tips of your bloated nipple wrap around the outstretched head of your " + cockDescript(game.player, 0) + ", convulsing with wet, squishing sounds as it wriggles inside you.");
                                else outputText("The swollen tips of your bloated nipples wrap around the " + cockHead(0) + " of your " + cockDescript(game.player, 0) + ", swallowing it like an enormous mouth. ", false);
                */
            }
            outputText("With each thrust, you bury your " + cockDescript(player) + " deeper into your greedy tit. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feeling of fullness where no such feeling should be possible.");
        }
        ...
        if (Math.round(player.cocks.cockArea(0)) > player.vaginalCapacity()) {
            if (player.vaginas.averageVaginalLooseness() < 2)
                ...
                switch (player.cocks[0].cockType) { // Different way of choosing cock to use, results in consistent description of cock[0]
                    case CockTypesEnum.ANEMONE:
                        outputText("The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + cockDescript(player) + " spreads them wider and fills them with artificial lust.  ");
                        break;
                    case CockTypesEnum.CAT:
                        outputText("The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    case CockTypesEnum.DEMON:
                        outputText("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    case CockTypesEnum.DISPLACER:
                        outputText("The swollen lips of your bloated nipple gape wide, but the wide head of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    case CockTypesEnum.DOG:
                    case CockTypesEnum.FOX:
                        outputText("The swollen lips of your bloated nipple gape wide, but the massive girth of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    case CockTypesEnum.HORSE:
                        outputText("The swollen lips of your bloated nipple gape wide, but the flared tip of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    case CockTypesEnum.HUMAN:
                        outputText("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    case CockTypesEnum.LIZARD:
                        outputText("The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + cockDescript(player) + " slowly spreads them even wider.  ");
                        break;
                    case CockTypesEnum.TENTACLE:
                        outputText("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(player) + " spreads them even wider.  ");
                        break;
                    default:
                        outputText("The swollen lips of your bloated nipple gape wide, but the " + cockHead(player) + " of your " + cockDescript(player) + " spreads them even wider.  ");
                }
                /* Old method
                                if(player.cocks.normalCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(game.player, 0) + " spreads them even wider. ", false);
                                else if(player.cocks.horseCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the flared tip of your " + horseDescript(0) + " spreads them even wider. ", false);
                                else if(player.cocks.dogCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the massive girth of your " + dogDescript(0) + " spreads them even wider. ", false);
                                else if(player.cocks.tentacleCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(game.player, 0) + " spreads them even wider. ", false);
                                else if(player.cocks.demonCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the tip of your " + cockDescript(game.player, 0) + " spreads them even wider. ", false);
                                else if(player.cocks.catCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + cockDescript(game.player, 0) + " spreads them even wider. ", false);
                                else if(player.cocks.lizardCocks() > 0) outputText("The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + cockDescript(game.player, 0) + " slowly spreads them even wider. ", false);
                                else if(player.cocks.anemoneCocks() > 0)  outputText("The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + cockDescript(game.player, 0) + " spreads them wider and fills them with artificial lust. ", false);
                                else if(player.cocks.displacerCocks() > 0)  outputText("The swollen lips of your bloated nipple gape wide, but the wide head of your " + cockDescript(game.player, 0) + " spreads them even wider. ", false);
                                else outputText("The swollen lips of your bloated nipple gape wide, but the " + cockHead(0) + " of your " + cockDescript(game.player, 0) + " spreads them even wider. ", false);
                */
            }
        ...
        if (player.vaginas.averageVaginalWetness() < 2) outputText("girl-lube quickly coats the whole length of your " + cockDescript(player) + " in a glistening layer of fuck juice. ");
            ...
            /* Old method
                        if(player.cocks.normalCocks() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling at your crotch. ", false);
                        else if(player.cocks.horseCocks() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling in and around your sheath. ", false);
                        else if(player.cocks.tentacleCocks() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, mixing with the tentacles own lubricants. ", false);
                        else if(player.cocks.demonCocks() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling around the base of your member. ", false);
                        else if(player.cocks.catCocks() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling around the spines near your base. ", false);
                        else if(player.cocks.lizardCocks() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling around the bulbs near your base. ", false);
                        else if(player.cocks.anemoneCocks() > 0)  outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling around the wriggling tentacles near your base. ", false);
                        else if(player.hasKnot()) outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling around the bulb near your base. ", false);
                        else  outputText("girl-lube drizzles down the length of your " + cockDescript(game.player, 0) + " in thick streams, pooling around the spines near your base. ", false);
            */
        }
        else outputText("girl-lube spurts out of your tortured nipple with each thrust of your " + cockDescript(player) + ", spattering your arms and face with your secretions. ");
    }
    if (Math.round(player.cocks.cockArea(0)) < player.vaginalCapacity()) {
        // Different noises based on sensitivity
        if (player.sens < 45)
        ...
        if (player.vaginas.averageVaginalLooseness() < 2)
            ...
            outputText("tip of your " + cockDescript(player) + " and begin to slide down its length, ");
            switch (player.cocks[0].cockType) { // Different way of choosing cock to use, results in consistent description of cock[0]
                case CockTypesEnum.ANEMONE:
            ...
            /* Old method
                        if(player.cocks.normalCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely. ", false);
                        else if(player.cocks.horseCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the flared tip of your " + horseDescript(0) + " and begin to slide down its length, swallowing it completely. ", false);
                        else if(player.cocks.dogCocks() > 0 || player.cocks.displacerCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + dogDescript(0) + " and begin to slide down its length, even swallowing your bulging knot without difficulty. ", false);
                        else if(player.cocks.tentacleCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely as it twists and pulses on its own, fucking your nipple.  ", false);
                        else if(player.cocks.demonCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely as each bump and nodule makes you quiver with unholy pleasures.  ", false);
                        else if(player.cocks.catCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely as each springy barb makes you quiver with pleasure.  ", false);
                        else if(player.cocks.lizardCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely as each of the bulgy knots along its length stretch the orifice further.  ", false);
                        else if(player.cocks.anemoneCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely and leaving a searing trail of desire in its path.  ", false);
                        else outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(game.player, 0) + " and begin to slide down its length, swallowing it completely. ", false);
            */
        }
        ...
        if (player.vaginas.averageVaginalWetness() < 2)
            ...
            if (player.cocks.normalCocks() > 0)
                outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(player) + " and hands, pooling on the ground below you. ");
            else if (player.cocks.hasSheath() > 0)
                outputText("girl-lube drizzles down the length of your " + cockDescript(player) + " in thick streams, pooling in and around your sheath. ");
            else if (player.cocks.tentacleCocks() > 0)
                outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(player) + " and hands, pooling on the ground below you. ");
            else if (player.cocks.demonCocks() > 0)
                outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(player) + " and hands, pooling on the ground below you. ");
            else outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(player) + " and hands, pooling on the ground below you. ");
        }
    }
}

function multiTitFuck(): boolean {
    const holeTotal: number = player.breastRows.totalNipples();
    ...
    if (fittableCocks > 2 && holeTotal < fittableCocks) {
        ...
        if (player.vaginalCapacity() >= 2 * player.cocks.cockArea(randomCock)) {
            ...
            if (2 * holeTotal >= fittableCocks)
                outputText("You giggle with glee as you realize that you will be able to jam every single one of your " + multiCockDescriptLight(player) + " into your abused nipples, thanks to double-penetration! ");
...
function multiNippleFuckPrep(randomCock: number): void {
    ...
    if (player.cocks[randomCock].cockType == CockTypesEnum.HUMAN) {
        ...
        // more than one?
        if (player.cocks.normalCocks() > 1)
            outputText("one of your " + num2Text(player.cocks.normalCocks()) + " " + cockDescript(player, randomCock) + "s ");
        else outputText("your " + cockDescript(player, randomCock) + " ");
    }
    ...
    if (player.cocks[randomCock].cockType == CockTypesEnum.HORSE) {
        ...
        if (player.cocks.horseCocks() > 1)
            outputText("one of your " + num2Text(player.cocks.horseCocks()) + " " + cockDescript(player, randomCock) + "s ");
        else outputText("your " + cockDescript(player, randomCock) + " ");
    }
    ...
    if (player.cocks[randomCock].cockType == CockTypesEnum.DOG) {
        outputText("pointed tip of ");
        // more than one?
        if (player.cocks.dogCocks() > 1)
            outputText("one of your " + num2Text(player.cocks.dogCocks()) + " " + cockDescript(player, randomCock) + "s ");
        else outputText("your " + cockDescript(player, randomCock) + " ");
    }
...
function orgazmo(selfSucking: boolean, nippleFuck: boolean): void {
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length == 1) {
            ...
            if (player.cocks.normalCocks() == 1)
                ...
                outputText("swelling, bulbous knot.  You feel it growing tighter and tighter until it's nearly twice the width of your " + cockDescript(player) + ".  The pressure is an agonizing pleasure that only builds higher and higher as you come closer and closer to orgasm.  ");
            else if (player.cocks.horseCocks() == 1)
                outputText("swollen equine sheath, slowly beginning to work its way up your shaft.  Pre-cum begins pouring from your " + cockDescript(player) + ", slicking your " + cockDescript(player) + " as you get ready to blow.  ");
            else if (player.cocks.tentacleCocks() == 1)
                outputText("wiggling vine-like cock, growing rapidly.  You feel the rubbery surface of your tentacle prick contracting as it nears release.  Thick bulges of fluids collect and travel along the length of your " + cockDescript(player) + ", the first of which is almost to your oversized tip.  ");
            else if (player.cocks.demonCocks() == 1)
                outputText("tainted man-meat, growing rapidly.  You feel the nodules close to the base swell and pulse, starting a rippling wave of pleasure that migrates upwards.  The ring of bumps around your crown double in size, flaring out as your " + cockDescript(player) + " prepares to unload.  ");
            else if (player.cocks.catCocks() == 1)
                outputText("full, cat-like sheath.  You feel it tingling and throbbing as the spines pulsate with arousal.  You feel the barbs at the bottom thickening as bulges of fluid migrate through your " + cockDescript(player) + " towards the tip.  ");
            else if (player.cocks.lizardCocks() == 1)
                outputText("swollen member.  You can feel it tingling and bulging strangly as it begins to contract and pulse.  Pre-cum leaks from your " + cockDescript(player) + " in a steady stream as each 'bulb' nearly doubles in size, and then one at a time, they deflate while your urethra dilates wide.  ");
            else if (player.cocks.anemoneCocks() == 1)
                outputText("base.  The tentacles surrounding your " + cockDescript(player) + " go nuts, constricting around it, inadvertently firing aphrodisiacs into the orgasmic flesh as they wring a potent, hip-jerking climax from you.  ");
            ...
            outputText("Pleasurable spasms overwhelm you as cum erupts from your " + cockDescript(player));
            ...
            if (selfSucking) {
                if (player.cumQ() < 25)
                    ...
                    outputText("Pulse after pulse of cum erupts from your " + cockDescript(player) + " into your mouth.  You swallow what you can but it's too much for you.  Cum runs down your cock to pool on you as your orgasm drags on.  ");
                else outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + cockDescript(player) + " as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  ");
            }
            else if (nippleFuck) {
                if (player.cumQ() < 25)
                ...
                else outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + cockDescript(player) + " as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ");
            }
            else {
                if (player.cumQ() < 25)
        ...
        if (player.cocks.length > 1) {
            ...
            if (player.cocks.dogCocks() > player.cocks.normalCocks() && player.cocks.dogCocks() > player.cocks.horseCocks()) { // Primary Dog
                let dogIndex: number;
                for (dogIndex = 0; dogIndex < player.cocks.length; dogIndex++) {
                    if (player.cocks[dogIndex].cockType == CockTypesEnum.DOG) break;
                }
                outputText("Your feel your knots bulging and swelling, growing tighter and tighter until they're nearly double the width of a " + cockDescript(player, dogIndex) + ".  The agonizing pressure builds higher and tighter with every passing second as you get closer and closer to orgasm.  ");
            }
            else if (player.cocks.horseCocks() > player.cocks.normalCocks()) { // Primary Horse
                ...
                outputText("You feel a pulsing in your sheath, slowly working its way up your " + cockDescript(player, horseIndex) + "s.  Pre-cum pours from your " + cockDescript(player, horseIndex) + "s, slicking the wobbly equine shafts as they get ready to blow.  ");
            }
            else { // Primary Normal
            ...
            if (nippleFuck) {
                ...
                else outputText("With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your " + multiCockDescriptLight(player) + " as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ");
            }

            // Doing the perverted sprinkler after dicks have popped out.
            outputText("Pleasure overwhelms your fragile mind as cum erupts from your " + multiCockDescriptLight(player) + ".  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ");
            if (selfSucking) {
                if (player.cumQ() < 25)
                    ...
                    outputText("Pulse after pulse of cum erupts from your " + cockDescript(player) + " into your mouth.  You swallow what you can but it's too much for you.  Cum runs down your " + cockDescript(player) + " to pool on you as your orgasm drags on.  Jizz rains over you the entire time from the rest of your \"equipment\".");
                else outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + cockDescript(player) + " as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  Your other 'equipment' rains jizz upon you the whole while, soaking you in a cum-puddle.");
            }
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas.length == 1) {
            ...
            // Wet orgasms
            if (player.vaginas[0].vaginalWetness == VaginaWetness.SLAVERING) outputText("A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your " + vaginaDescript(player) + ".  ");
        }
    ...
    if (player.breastRows.averageLactation() >= 2 || player.breastRows.hasFuckableNipples()) {
...
function titCum(cumQuantity: number = 3): void {
    ...
    if (player.breastRows.averageLactation() >= 2 && !player.breastRows.hasFuckableNipples()) {
        ...
        if (player.breastRows.averageLactation() <= 2.6) outputText("thin streams of milk spray out of your ");
        if (player.breastRows.averageLactation() > 2.6 && player.breastRows.averageLactation() < 3) outputText("thick gouts of milk spurt out of your ");
        if (player.breastRows.averageLactation() >= 3) outputText("huge torrents of milk blast out of your ");
        // different description based on size of nips
        if (player.breastRows.averageNippleLength() <= 1) outputText("erect nipples, ");
        if (player.breastRows.averageNippleLength() > 1 && player.breastRows.averageNippleLength() < 4) outputText("fat, puckered nipples, ");
        if (player.breastRows.averageNippleLength() >= 4) outputText("huge, swollen teats, ");
        if (player.breastRows.averageLactation() <= 2.6) outputText("spattering milk everywhere. ");
        if (player.breastRows.averageLactation() > 2.6 && player.breastRows.averageLactation() < 3) outputText("covering everything nearby. ");
        if (player.breastRows.averageLactation() >= 3) outputText("drenching the entire area.");
    }
    ...
    if (player.breastRows.hasFuckableNipples()) {
        ...
        // Lactating?
        if (player.breastRows.averageLactation() > 0) outputText("milk and ");
...
function deluxeDildo(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        outputText("  A drop of pink aphrodisiac leaks from the tip, offering you a hint of the pleasure you're in for.   You make sure to catch it on your crotch, letting the fluid seep into your " + vaginaDescript(player) + ".  Warmth radiates outwards, spreading to your thighs as your " + clitDescription(player));
        ...
        doNext(Camp.returnToCampUseOneHour);
        outputText("\n");
        cuntChange(player, player.vaginalCapacity() * 0.9, true);
    }
}
...
function onaholeUse(): void {
    ...
    if (player.effects.findByType(StatusAffects.PlainOnaholeUsed) < 0) {
        player.effects.create(StatusAffects.PlainOnaholeUsed, 0, 0, 0, 0);
        ...
        if (player.cocks.length > 1)
            outputText("With your free hand, you slap your " + multiCockDescriptLight(player) + " to 'attention' and ease the onahole over your cocks.  ");
...
export function deluxeOnaholeUse(): void {
    clearOutput();
    // Flag after first use!
    if (player.effects.findByType(StatusAffects.DeluxeOnaholeUsed) < 0) {
        player.effects.create(StatusAffects.DeluxeOnaholeUsed, 0, 0, 0, 0);
...
function onaholeMulticockContinuation(): void {
    outputText("You pull the sloppy toy from your dribbling dick and smile, shoving its slippery surface down on another of your " + multiCockDescriptLight(player) + ".  You rapidly work it around your cocks, orgasming until ");
    if (player.balls > 0)
...
function allNaturalOnaholeUse(): void {
    clearOutput();
    // First use!
    if (player.effects.findByType(StatusAffects.AllNaturalOnaholeUsed) < 0) {

        player.effects.create(StatusAffects.AllNaturalOnaholeUsed, 0, 0, 0, 0);
...
function stimBeltUse(): void {
    ...
    if ((player.keyItems.has("Self-Stimulation Belt") >= 0)) {
        ...
        cuntChange(player, 1, true, true, false);
    }
}
...
function allNaturalStimBeltUse(): void {
    ...
    if (player.keyItems.has("All-Natural Self-Stimulation Belt") >= 0) {
        // First time!
        if (player.effects.findByType(StatusAffects.UsedNaturalSelfStim) < 0) {
            // Flag as used!
            player.effects.create(StatusAffects.UsedNaturalSelfStim, 0, 0, 0, 0);
    ...
    // [Repeatable]
        ...
        if (player.cocks.countCockSocks("gilded") > 0 && flags[kFLAGS.GILDED_JERKED] < player.cocks.countCockSocks("gilded")) {
            ...
            if (flags[kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] > 10 && player.balls > 1 && rand(5) == 0) {
                ...
                outputText("\n\nWhile gasping for air, you scowl at your " + cockDescript(player) + " in disapproval. That's when your " + sackDescript(player) + " catches your attention. It's gleaming with your sex sheen and you watch one of your balls slowly slide off to one side.");

                outputText("\n\nYou think you can make it! You throw your other leg over your head and both feet come to rest on your back. You push your mouth towards your " + sackDescript(player) + ", slowly walking your toes down your back. You are only an inch from your " + ballsDescriptLight(player) + " now and your own aroma fills your nostrils, spurring your on. Then, finally, you are there! And it nearly knocks you out! The sensation of your own balls in your mouth is incredible--you can feel them churning in your mouth--and your [feet] start kneeding your back. You bring your hands up to massage your " + ballsDescriptLight(player) + " as well, making them take turns in your mouth. Completely intoxicated by your own scent you loose all track of time--there is only the bliss of sucking, licking, and massaging your own balls...");

                outputText("\n\nAbrubtly you realize that your " + sMultiCockDesc(player) + " have soaked your torso in precum--apparently you've been on the edge for some time. In one swift motion, you pick your head up and slam your mouth down over your " + cockDescript(player) + " leaving your hands to continue their ball massage.");

                // add cum quantity conditional text...
                outputText("\n\nThe release is immediate. You feel the contractions of your climax against your [face], your [feet] involuntarly start massaging your back, and you feel your " + cockDescript(player) + " expand and contract in your mouth as seed pumps through it into your stomach. It's more relaxing than anything else--each contraction makes you feel like you might be melting a little, like you might remain in this position forever.");

                outputText("\n\nYour [feet] and hands are still masagging their respective charges when you realize you are starting to go flaccid! You make an attempt to massage every last drop of cum from your " + ballsDescriptLight(player) + ", sucking on your " + cockDescript(player) + " continuously as it goes down. It slowly slips from your mouth once it's completely deflated, causing you to frown slightly. Still feeling great in this position, you go down on your " + sackDescript(player) + " again--alternating between licking and sucking them with your mouth and massaging them with your hands.");
...
function centaurNippleCunt(): void {
    ...
    if (flags[kFLAGS.PC_FETISH] > 0)
        outputText("panting lustily as you envision being caught masturbating your " + nippleDescription(player, 0) + "s.");
    else if (player.cor < 33)
        outputText("blushing a bit as you look down at your " + nippleDescription(player, 0) + "s.");
    else if (player.cor < 66)
    ...
    if (player.vaginas.length > 0 || player.cocks.length > 0) {
        ...
        if (player.cocks.length > 1)
            outputText("  You feel each of your " + multiCockDescriptLight(player) + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.");
        else if (player.cocks.length == 1)
            outputText("  You feel your " + multiCockDescriptLight(player) + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.");
        if (player.balls > 0) outputText("  Rocking slightly inside your sack, your " + ballsDescriptLight(player) + " repeatedly clench up against your body, ready to release.");
        outputText("\n\n");
    }
    // (ORGAZMO)
...
function centaurHugeCock(): void {
    ...
    const primary: number = player.cocks.longestCock();
    const plural: boolean = (player.cocks.length > 1);
    outputText("You shrug out of your " + player.armorName + ", not that it does much to impede your " + multiCockDescriptLight(player) + " as a centaur.   ");
    if (player.cor < 33)
    ...
    outputText("your " + multiCockDescriptLight(player) + " swells out underneath you, hanging down from your equine hindquarters as ");
    if (!plural) outputText("it grows full and hard.");
    else outputText("they grow full and hard.");
    if (plural)
        outputText("  After a moment they peek out from under your forelegs, proudly displaying your " + multiCockDescriptLight(player) + ".");
    else {
    ...
    if (player.tallness * 1.5 < player.cocks[player.cocks.longestCock()].cockLength) {
        outputText("  In no time flat you're rendered virtually immobile by the length of ");
        if (plural) outputText("all ");
        outputText("your " + multiCockDescriptLight(player) + ".");
    }
    outputText("\n\n");
    ...
    if (player.tallness * 1.5 >= player.cocks[player.cocks.longestCock()].cockLength) {
        ...
        outputText(cockDescript(player, primary) + " and lift up, pressing it tightly against your belly as you begin to stroke along the sensitive underside.  ");
        if (player.cumQ() > 500)
        ...
        if (player.cocks.length == 2)
            ...
            outputText("  Your other " + multiCockDescriptLight(player) + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.");
    ...
    // Get started (long)
        ...
        outputText(cockDescript(player, primary) + " and lift it up, curving it slightly as you begin to stroke the underside with long, fluid caresses.  You even manage to bend down and give it a lick, forcing you into a pleasurable shudder.");
        if (player.cumQ() > 500)
            outputText("  Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.");
        else if (player.cumQ() > 100)
            outputText("  A dribble of pre-cum starts to leak from the tip as you get more into the act.");
        ...
        // STROKE (long)
        outputText("You smear your pre-cum over the tip, rolling your palm across the sensitive tip in a way that makes it hard to stand.  Meanwhile your other hand is making great use of your incredible length, jacking you off with long fluid strokes.  In no time flat, you can feel your " + cockDescript(player) + " pulsing in your grip, growing harder in time with your fevered stroking.");
        if (player.cocks.length == 2)
            outputText("  Your other dick mimics its lucky brother's pleasure, even though it's been ignored in order for you to focus on your current 'toy'.");
        else if (player.cocks.length > 2)
            outputText("  Your other " + multiCockDescriptLight(player) + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.");
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.breastRows.hasFuckableNipples()) {
            ...
            outputText(" of your " + num2Text(player.breastRows.totalNipples()) + " " + nippleDescription(player, 0) + "s quiver, ");
            if (player.vaginas.wetness() < 4 && player.breastRows.biggestLactation() < 2)
                outputText("leaking");
            else if (player.vaginas.wetness() < 5 && player.breastRows.biggestLactation() < 3)
                outputText("dripping");
...
function centaurDudesGetHorseAids(): void {
    ...
    outputText("Seeing the toy's exposed, gaping genitals, you feel a stirring in your " + multiCockDescriptLight(player) + ".  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ");
    // [If small cock:
    if (player.cocks.cockArea(x) < 30)
    ...
    outputText("Satisfied the toy is ready for you, you clop back a few paces and surge forward.  You mount the toy easily, your belly running across its smooth, warm back until your chest bumps against the mare's head.  You grip her shoulders for support and start to buck your hips, your " + cockDescript(player, x) + " poking around in search of entrance.  Finally, you feel the tip of your prick ");
    if (player.cocks.cockArea(x) < 30)
        outputText("pressing against the tight ring of the toy's anus");
    else outputText("lining up with the toy's gaping cunt");
    outputText(".  You rear your " + hipDescription(player) + " and slam yourself into the mare's waiting hole.\n\n");

    outputText("The toy's passage seems to shift and contract around your " + cockDescript(player, x) + ", molding itself to perfectly sheathe you.  What a marvelous little toy!  You slide on up until you hilt yourself, your crotch pressed against the mare's wide ass as your " + chestDesc(player) + " squeezes against her back.  Now fully mounted, you begin to rut on the mare toy, slapping your hips");
    if (player.balls > 0)
        outputText(" and " + ballsDescriptLight(player));
    ...
    outputText("Finally able to get the release you weren't able to give yourself, you feel an orgasm mounting deep within you.  You let out a loud, equine whinny as you hump the toy faster and faster, pounding her with your " + cockDescript(player, x) + " until the pleasure overwhelms you.  You cry out as you cum, squirting your centaur-cum as far into the toy as you can shoot it");
    ...
    if (player.cocks.countCockSocks("gilded") > 0 && flags[kFLAGS.GILDED_JERKED] < player.cocks.countCockSocks("gilded")) {
        ...
        outputText(".\n\nSated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your " + cockDescript(player, x) + " is soft inside it.  You scamper off of her, dropping back to your four equine feet.  <b>You are startled to see glittering gems pouring from her ");
        if (player.cocks.cockArea(x) >= 30)
            outputText("horsecunt");
        else outputText("asshole");
        ...
        outputText("Sated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your " + cockDescript(player, x) + " is soft inside it.  You scamper off of her, dropping back to your four equine feet.  With a contented yawn, you disassemble the toy and haul it off back to your stash, leaking your cum the entire way.");
    }
...
function getHugeEggsInCawk(): void {
    ...
    if (player.effects.getValue1Of(StatusAffects.Exgartuan) == 1 && player.effects.getValue2Of(StatusAffects.Exgartuan) == 0 && flags[kFLAGS.TIMES_EGGED_IN_COCK] == 0) {
        ...
        if (ShouldraFollower.followerShouldra()) outputText(", with Shouldra's assistance,");
...
export function birthBeeEggsOutYourWang(): void {
...
function layEggsInYerTits(): void {
    clearOutput();
    if (player.effects.getValue1Of(StatusAffects.Exgartuan) == 2 && player.effects.getValue2Of(StatusAffects.Exgartuan) == 0) {
        ...
        if (player.ovipositor.eggs() < 20) {
            ...
            if (player.cocks.length > 0) outputText("  Your [chest] and ovipositor meanwhile are slowly being coated in white from your languid pleasure.");
    ...
    if (player.effects.findByType(StatusAffects.Eggchest) < 0) {
        player.effects.create(StatusAffects.Eggchest, 3 + rand(10), 1 + rand(4), 0, 0);
...
function tentacleSelfFuck(): void {
    ...
    if (y >= 0) outputText("  It loops around your " + cockDescript(game.player, y) + " while you fight with it, strangling the other phallus in tight coils of squeezing, floral friction.");
    ...
    outputText(" twist your wrist down.  That change in direction comes far easier than trying to stroke it.  Releasing sets of alien muscles that you had held instinctively, you let your " + cockDescript(game.player, x) + " do what it was made to do.  It slithers down sinuously, the purplish head pressing heavy and hot against your juicy mound just hard enough to encourage you to press back against it.  It slowly spreads your clinging tunnel around its obscene girth one fold at a time.");
    // Cunt change!
    cuntChange(player, player.cocks.cockArea(x), true, true, false);

    outputText("\n\nRipples of delight radiate along your " + cockDescript(game.player, x) + " as it buries itself as deeply into your velvet tunnel as possible.  The fat, purplish head stretches you out as it goes, just enough that the trailing stalk is comfortably ensconced in twat.  Tugging on the exposed portion, you find yourself pumping wildly on your length, squeezing it while paroxysms of ecstasy render your fine muscle control useless.  The dual sensations of being fucked and dishing out a hot dicking have overlapped into a tangled-up knot inside you.");
    if (y >= 0) {
        outputText("  Your " + cockDescript(game.player, y) + " is getting jacked off by the engorged cock-coil's motions and slowly leaks creamy pre over the jerking length.");
        if (player.cocks[y].cockType == CockTypesEnum.TENTACLE) {
            ...
            buttChange(player, player.cocks.cockArea(y), true, true, false);
        }
    }
    outputText("\n\nDelirious with excitement, you grab hold of your [chest]");
    if (player.breastRows.biggestTitSize() >= 1) outputText(", kneading the soft mammary");
    outputText(" and twisting your [nipple] as your body goes white-hot with pleasure");
    if (player.breastRows.hasFuckableNipples()) outputText(", even stuffing a finger inside a sloppy, dripping nipplecunt");
    outputText(".");
    ...
    if (player.cocks.tentacleCocks() >= 3) {
    ...
    if (player.cocks.tentacleCocks() >= 6) {
        outputText("\n\nThe excess green tools rise up above you.  They survey the view before them in a crude approximation of a sentient surveyor, seeming to take in the shifting, squiggling view of your ardent masturbation.  In truth, you're just trying to think of where to stick them.  A few droplets of liquid lust leak from their tips in sympathetic pleasure, and it gives you an idea for just what to do with them.  They stretch out towards your torso, veering to the sides at the last second.  Dripping onto your shoulders, the pulsating plant-pricks slowly push into your armpit, lubricating your " + skinFurScales(player) + " with their amorous liquid.");
        if (player.cocks.tentacleCocks() >= 7) outputText("  They pile in there, really squirming against each other as much as you, frotting against your sweat-slick flesh.  ");
        outputText("  You're fucking your armpits, and it feels divine, so good that your pits are swampy pits of sex within moments.");
    }
    // One more:
    else if (player.cocks.tentacleCocks() >= 4) {
        outputText("\n\nThe last green tool rises up above you.  It surveys the view before it in a crude approximation of a sentient surveyor, seeming to take in the shifting, squiggling view of your ardent masturbation.  In truth, you're just trying to think of where to stick it.  A droplet of liquid lust leaks from the tip in sympathetic pleasure, and it gives you an idea for just what to do with it.  It stretches out towards your torso but veers left at the last second.  Dripping onto your shoulder, the pulsating plant-prick slowly pushes into your armpit, lubricating your " + skinFurScales(player) + " with its amorous liquid.  You're fucking your armpit and it feels divine, so good that you quickly slick your pit and twist the spare cock around yourself so that it can double-fuck both sides.");
    }
    ...
    if (player.cocks.tentacleCocks() == 2)
        outputText(" and ass");
    ...
    if (player.cocks.tentacleCocks() >= 2) outputText(", while your [ass]'s interior is painted bright white");
    outputText(".");
    if (player.cocks.tentacleCocks() == 2) {
        outputText("  Your untended " + cockDescript(game.player, y) + " blows spunk over you ");
        if (player.cumQ() < 50)
        ...
        if (player.cocks.tentacleCocks() == 4)
            outputText("s swell");
        else outputText(" swells");
        outputText(" in your armpit");
        if (player.cocks.tentacleCocks() >= 5) outputText("s");
    ...
    if (player.cocks.tentacleCocks() == 2)
        outputText(" and ass");
    else if (player.cocks.tentacleCocks() > 2) outputText(", ass, and mouth");
    outputText(", leaving your orifice");
    if (player.cocks.tentacleCocks() > 1) outputText("s");
...
function tentacleGoesUpYerPooperNewsAtEleven(): void {
    ...
    for (const x = tentacle + 1; x < player.cocks.length; x++) { // Find the biggest tentacle cock you've got
    ...
    outputText("You eagerly reveal your flora pecker as it squirms and wriggles on its own, gently caressing the green surface here and there, its coloration changing as you tease yourself.  After toying with your tentacle dick for a while, you decide to get down to business; using your newly acquired shaft muscles, you expertly guide your ever-writhing " + cockDescript(player, tentacle) + " to your back, pointing it toward your buttocks.  You grind the tip against your [butt], making pre-cum flow from your mushroom-like head and smearing it against your " + skinFurScales(player) + ".  Using your own seminal fluid as a natural lube, you press the tip of your " + cockDescript(player, tentacle) + " in front of your own backdoor, stretching your anal opening little by little, careful not to tear your own insides.  This goes on for a while, until you suddenly lose all patience and roughly stuff your own " + cockDescript(player, tentacle) + " at full force inside your colon.");
    // [anal tightness check]
    buttChange(player, player.cocks.cockArea(tentacle), true, true, false);

    outputText("\n\nThe impetuousness of the act makes you cry in a mixture of pleasure and pain, your [asshole] being overloaded with intense sensations.  Fortunately the tender and rubbery texture of your " + cockDescript(player, tentacle) + " allows for more sensitivity, the subtle friction sending tingles from your crotch all the way up your spine.  You shiver from the sheer cocktail of raw pleasure you're inflicting on your own body.  Your " + cockDescript(player, tentacle) + " keeps squirming against your insides, making you quiver and giggle like a whore, until it lodges all the way inside your colon, adopting a more comfortable position.  You then proceed to ferociously fuck your own [asshole], stretching it a bit more at every thrust.");
    ...
    outputText("  You groan and lazily remove your " + cockDescript(player, tentacle) + " from your anus as you give in to your pleasure-induced drowsiness.");
    if (player.cocks.length > 0) {
        // Single Cock
        if (player.cocks.length == 1) {
            if (player.lib < 30)
...
function gildedCockTurbate(): void {
    ...
    for (const x = gildedCock + 1; x < player.cocks.length; x++) { // Find the biggest gilded cock you've got
    ...
    outputText("\n\nPressure builds, and tremors of pleasure run through your body as you stroke faster and faster.  You let out a small moan, as you pump the cocksleeve up and down your " + cockDescript(player, gildedCock) + ".  Your cock swells within the confines of the sleeve, throbbing against the metallic confines of the sheath.  You shudder, arching your back, as you climax, your " + Math.round(player.cumQ()) + "mLs of cum arcing up high into the air overhead.");
    ...
    outputText(" gems falls down upon your body instead of cum, bouncing off your " + skinFurScales(player) + ".");
...
function midasCockJackingGemsRoll(): number {
    const cockSocks: number = player.cocks.countCockSocks("gilded");
...
function onaholeRepeatUse(corrupted: boolean): void {
    ...
    if (corrupted) {
        ...
        outputText("Much to your annoyance and embarrassment, you feel the need to relieve yourself of your tension. Sighing in reservation, you remove the used onahole from your sack. Operating with a mind of its own, your " + cockDescript(player) + " springs to attention, anticipating the coming stimulation and release. Your member easily pushes past the opening in the toy and you begin working your penis with some vigor. Pleasure begins to pulse through your " + cockDescript(player) + " as you build yourself up. The force of the building fluids pushes against your inner organs, creating the paradoxical sensation of pain and pleasure. Pure instict takes over, and your hips begin to buck reflexively. With a sudden, sharp pinch at the base of your member, the need to cum takes over your body");
        ...
        if (gemsCreated > 0) {
            ...
            outputText(" of gems falls down upon your body instead of cum, bouncing off your " + skinFurScales(player) + ".\n\n<b>You force yourself to pick up the " + gemsCreated + " gems before passing out.</b>");
        }
        else {
