...
export function updatePregnancy(): boolean {
    ...
    if (player.inHeat) {
        ...
        dynStats("lib", -player.effects.getValue2Of(StatusAffects.Heat));
        if (player.lib < 10) player.lib = 10;
        statScreenRefresh();
        player.effects.remove(StatusAffects.Heat);
    ...
    if (player.pregnancyIncubation == 1) {
        ...
        if (player.effects.findByType(StatusAffects.Birthed) < 0) player.effects.create(StatusAffects.Birthed, 1, 0, 0, 0);
        else {
            player.effects.addValue(StatusAffects.Birthed, 1, 1);
            if (player.perks.findByType(PerkLib.BroodMother) < 0 && player.effects.getValue1Of(StatusAffects.Birthed) >= 10) {
    ...
    if (player.pregnancyIncubation > 1) {
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_COTTON) {
            ...
            if (player.pregnancyIncubation == 32 || player.pregnancyIncubation == 64 || player.pregnancyIncubation == 85 || player.pregnancyIncubation == 150) {
                displayedUpdate = true;
                // Increase lactation!
                if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.mostBreastsPerRow() > 1 && player.breastRows.biggestLactation() >= 1 && player.breastRows.biggestLactation() < 2) {
                ...
                if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.mostBreastsPerRow() > 1 && player.breastRows.biggestLactation() > 0 && player.breastRows.biggestLactation() < 1) {
                    outputText("\nDrops of breastmilk escape your nipples as your body prepares for the coming birth.\n", false);
                    player.boostLactation(.5);
                }
                // Lactate if large && not lactating
                if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.mostBreastsPerRow() > 1 && player.breastRows.biggestLactation() == 0) {
                    outputText("\n<b>You realize your breasts feel full, and occasionally lactate</b>.  It must be due to the pregnancy.\n", false);
                    player.boostLactation(1);
                }
                // Enlarge if too small for lactation
                if (player.breastRows.biggestTitSize() == 2 && player.breastRows.mostBreastsPerRow() > 1) {
                    outputText("\n<b>Your breasts have swollen to C-cups,</b> in light of your coming pregnancy.\n", false);
                    growTits(player, 1, 1, false, 3);
                }
                // Enlarge if really small!
                if (player.breastRows.biggestTitSize() == 1 && player.breastRows.mostBreastsPerRow() > 1) {
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_MARBLE) {
            ...
            if (player.pregnancyIncubation == 85) {
                ...
                else if (player.breastRows.biggestTitSize() <= 1) {
                ...
                else if (player.breastRows.biggestTitSize() <= 10) {
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_EMBER) {
            ...
            if (flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
                ...
                if (player.pregnancyIncubation == 250) pregText = "Your belly grows ever bigger, making your pregnancy noticeable; your belly also feels somewhat solid.  Ember casts pleased glances in your direction, whenever " + EmberScene.emberMF("he", "she") + " thinks you're not looking.";
    ...
    if (player.buttPregnancyIncubation > 1) {
        ...
        if (player.buttPregnancyType == PregnancyStore.PREGNANCY_DRIDER_EGGS) {
            if (player.buttPregnancyIncubation == 199) {
                outputText("\n<b>After your session with the drider, you feel so nice and... full.  There is no outward change on your body, aside from the egg-packed bulge of your belly, but your " + assholeDescript(player) + " tingles slightly and leaks green goop from time to time. Hopefully it's nothing to be alarmed about.</b>\n", false);
                displayedUpdate = true;
            }
            ...
            if (player.buttPregnancyIncubation == 120) {
                outputText("\n<b>Your belly is bulging from the size of the eggs growing inside you and gurgling just about any time you walk.  Green goo runs down your " + legs(player) + " frequently, drooling out of your pregnant asshole.</b>\n", false);
                displayedUpdate = true;
            }
        ...
        if (player.buttPregnancyType == PregnancyStore.PREGNANCY_SANDTRAP || player.buttPregnancyType == PregnancyStore.PREGNANCY_SANDTRAP_FERTILE) {
            ...
            if (player.buttPregnancyIncubation == 20) {
                ...
                outputText("\nSomething oily drips from your sphincter, staining the ground.  You suppose you should feel worried about this, but the overriding emotion which simmers in your gut is one of sensual, yielding calm.  The pressure in your bowels which has been building over the last few days feels right somehow, and the fact that your back passage is dribbling lubricant makes you incredibly, perversely hot.  As you stand there and savor the wet, soothing sensation a fantasy pushes itself into your mind, one of being on your hands and knees and letting any number of beings use your ass, of being bred over and over by beautiful, irrepressible insect creatures.  With some effort you suppress these alien emotions and carry on, trying to ignore the oil which occasionally beads out of your " + assholeDescript(player) + " and stains your [armor].\n");
                dynStats("int", -.5, "lus", 500);
                displayedUpdate = true;
            }
        }
    ...
    if (player.buttPregnancyIncubation == 1 && player.buttPregnancyType == PregnancyStore.PREGNANCY_SANDTRAP_FERTILE) {
        ...
        if (player.buttRating < 17) {
            ...
            if (player.buttRating < 13) {
                player.buttRating++;
                outputText("\nYou notice your " + buttDescription(player) + " feeling larger and plumper after the ordeal.\n", false);
            }
    ...
    if (player.buttPregnancyIncubation == 1 && player.buttPregnancyType == PregnancyStore.PREGNANCY_BEE_EGGS) {
        ...
        if (player.breastRows.averageLactation() >= 1 && player.breastRows.biggestTitSize() > 2) outputText("Seeking even greater sensation, your hands gather the honey and massage it into your " + breastDescriptOfRow(game.player, 0) + ", slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ", false);
        ...
        if (buttChange(player, 20, true)) outputText("\n", false);
        if (player.buttRating < 17) {
            // Guaranteed increase up to level 10
            if (player.buttRating < 13) {
                player.buttRating++;
                outputText("\nYou notice your " + buttDescription(player) + " feeling larger and plumper after the ordeal.", false);
            }
            // Big butts only increase 50% of the time.
            else if (rand(2) == 0) {
                player.buttRating++;
    ...
    if (player.pregnancyType == PregnancyStore.PREGNANCY_BUNNY && player.pregnancyIncubation == 1) {
        ...
        outputText("You sink into restful unconsciousness, marveling at how stretchy and sensitive your " + vaginaDescript(player, 0) + " feels now.", false);
        ...
        if (player.vaginalCapacity() < 300) {
            ...
            player.effects.addValue(StatusAffects.BonusVCapacity, 1, 10);
        }
        player.knockUpForce(); // Clear Pregnancy
        player.orgasm();
    ...
    if (player.pregnancyType == PregnancyStore.PREGNANCY_ANEMONE && player.pregnancyIncubation == 1) {
        ...
        outputText("Hurriedly you strip off your gear and sit down with your back against a rock.  Focusing yourself, you attempt to prepare for labor; you try to remember your recent partners and worry about what kind of monstrous infant you might have to force out of your " + vaginaDescript(player, 0) + ".  The first contraction comes and you push as hard as you can, to be rewarded with the feeling of something sliding out between your labia.  You attempt a few more pushes but nothing further seems forthcoming; curious, you look down at your crotch only to discover a blue stalk sticking proudly out of your vagina!\n\n", false);

        if (flags[kFLAGS.ANEMONE_KID] > 0) {
            outputText("As you take in the sight, small nodules around the tip begin to form and lengthen, until the little anemone is capped by a mop of wriggling blue-green tentacles.  Horrified, you grasp it at the base and give it a sharp pull.  The pain makes you lock up and nearly takes away your consciousness as its sticky surface releases its grip on your labia and " + clitDescription(player) + "!   It writhes and slips out of your pain-wracked hands, leaving them tingling.  As you lie there, stunned, it begins to inch back toward your " + vaginaDescript(player, 0) + ".  Footfalls sound next to you, and a blue hand picks up the squirming, cilliated creature.  Kid A gives you a shy smile, then turns to her barrel.  A quick splash and a filled waterskin later, she heads toward the stream, toting your grub-like offspring.");
            ...
            outputText("As you take in the sight, small nodules around the tip begin to form and lengthen, until the little anemone is capped by a mop of wriggling blue-green tentacles.  Horrified, you grasp it at the base and give it a sharp pull.  The ensuing pain in your labia and " + clitDescription(player) + " makes you lock up and nearly takes away your consciousness, and with " + multiCockDescript(player) + " in the way, you can't get any leverage on the pull at all!  The anemone detaches weakly, but writhes and slips out of your pain-wracked grip, leaving your hands tingling.  As you lie there, stunned, it begins to inch back toward your " + vaginaDescript(player, 0) + ".  Searching about weakly with the feelers, it touches along your thigh and searches out the entrance of your pussy.  When the tentacled crown brushes past your lips a venomous heat stirs your crotch and fills you with energy; shocked into sense, you look at the absurd creature.  You raise your arm to slap at it, but something stays your hand.  As if sensing your hesitation, it stands upright and holds itself at attention for inspection.  It would be easy to knock it away... and yet, the unprepossessing little thing looks so proud that you can't quite bring yourself to do so.");
            ...
            cuntChange(player, 20, true, true, false);
            outputText("\n");
            player.effects.create(StatusAffects.CampAnemoneTrigger, 0, 0, 0, 0);
            player.knockUpForce(); // Clear Pregnancy
            return true;
        }
        // [(if pc has 0-9 existing cocks)
            ...
            // [(if boobs)
            if (player.breastRows.biggestTitSize() >= 1) outputText(allBreastsDescript(player) + " and ", false);
            ...
            if (player.vaginas[0].vaginalWetness < VaginaWetness.SLICK) outputText("a trickle", false);
            else if (player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("a squirt", false);
    ...
    if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyStore.PREGNANCY_IMP) {
        ...
        if (player.effects.findByType(StatusAffects.BirthedImps) >= 0) player.effects.addValue(StatusAffects.BirthedImps, 1, 1);
        ...
        if (player.vaginas[0].vaginalLooseness == VaginaLooseness.TIGHT) player.vaginas[0].vaginalLooseness++;
        // 50% chance
        if (player.vaginas[0].vaginalLooseness < VaginaLooseness.GAPING_WIDE && rand(2) == 0) {
        ...
        if (player.breastRows.averageLactation() > 0 && player.breastRows.averageLactation() < 5) {
            outputText("  Your breasts won't seem to stop dribbling milk, lactating more heavily than before.", false);
            player.boostLactation(.5);
        }
        // Lactate if large && not lactating
        if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.mostBreastsPerRow() > 1 && player.breastRows.averageLactation() == 0) {
        ...
        if (player.vaginas[0].vaginalWetness == VaginaWetness.DRY) player.vaginas[0].vaginalWetness++;
        ...
        if (player.buttRating < 10 && rand(2) == 0) {
            ...
            player.hipRating++;
            outputText("\n\nAfter the birth your " + player.armorName + " fits a bit more snugly about your " + hipDescription(player) + ".", false);
        }
        outputText("\n", false);
        displayedUpdate = true;
    }
    ...
    if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyStore.PREGNANCY_MARBLE) {
        ...
        if (flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] < 100) {
            ...
            outputText("You push and heave with all your might, little else going through your mind. You somehow register when the head comes out, and soon the shoulders along with the rest of the body follow.  You lean back and pant for a while before feeling a pair of hands grab a hold of you. They slowly and clumsily feel up your body before finding your " + chestDesc(player) + " and a mouth quickly closes down on a " + nippleDescription(player, 0) + ".  You sigh softly, and drift off to sleep.", false);
            cuntChange(player, 20, true, true, false);
            ...
            outputText("\nYou feel a clenching sensation in your belly and something shifts inside.  Your contractions start a few moments later and you realize that it's time for your child to be born.  You cry out mildly in pain and lie down, letting your body start to push the baby out.  Marble rushes over and sees that it's time for you to give birth, so she picks you up and supports you as you continue pushing the child out of your now-gaping " + vaginaDescript(player, 0) + ".", false);
            ...
            // it's a boy!
            {
                ...
                if (player.hipRating < 10) {
                    player.hipRating++;
                    outputText("After the birth your " + player.armorName + " fits a bit more snugly about your " + hipDescription(player) + ".", false);
                }
            ...
            // it's a girl!
            {
                ...
                if (player.effects.getValue4Of(StatusAffects.Marble) >= 20) {
                    outputText("As the child contentedly drinks from your " + nippleDescription(player, 0) + ", Marble tells you, \"<i>Sweetie, somehow I know that our kids won't have to worry about the addictive milk.  It will only make them healthy and strong.</i>\"  You nod at her and put the child into the nursery.  ", false);
                }
                else {
    ...
    if (player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR) {
        ...
        cuntChange(player, 120, true, true, false);
        if (player.vaginas[0].vaginalWetness == VaginaWetness.DRY) player.vaginas[0].vaginalWetness++;
        if (player.gender == 1) player.gender = 3;
        if (player.gender == 0) player.gender = 2;
        player.orgasm();
    ...
    if (player.pregnancyIncubation == 1 && (player.pregnancyType == PregnancyStore.PREGNANCY_CENTAUR || player.pregnancyType == PregnancyStore.PREGNANCY_KELT)) {
        ...
        player.knockUpForce(); // Clear Pregnancy
        if (player.breastRows.averageLactation() > 0 && player.breastRows.averageLactation() < 5) {
            outputText("  Your " + allBreastsDescript(player) + " won't seem to stop dribbling milk, lactating more heavily than before.", false);
            player.boostLactation(.5);
        }
        outputText("  ", false);
        cuntChange(player, 100, true);
        if (player.vaginas[0].vaginalWetness == VaginaWetness.DRY) player.vaginas[0].vaginalWetness++;
        if (player.gender == 1) player.gender = 3;
        if (player.gender == 0) player.gender = 2;
        player.orgasm();
    ...
    if (player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS && player.pregnancyIncubation > 0) {
        ...
        if (player.pregnancyIncubation == 1) {
            ...
            if (player.effects.getValue2Of(StatusAffects.Eggs) == 0) {
                // light quantity
                if (player.effects.getValue3Of(StatusAffects.Eggs) < 10) {
            ...
            // Large egg scene
                ...
                if (player.vaginas[0].vaginalLooseness < VaginaLooseness.LOOSE) outputText("stretching your tight cunt painfully, the lips opening wide ", false);
                if (player.vaginas[0].vaginalLooseness >= VaginaLooseness.LOOSE && player.vaginas[0].vaginalLooseness <= VaginaLooseness.GAPING_WIDE) outputText("temporarily stretching your cunt-lips wide-open ", false);
                if (player.vaginas[0].vaginalLooseness > VaginaLooseness.GAPING_WIDE) outputText("parting your already gaping lips wide ", false);
                ...
                if (player.clitLength <= 2) outputText("your " + vaginaDescript(player, 0) + " by pulling your folds wide and playing with your clit.  Another egg pops free from your diminishing belly, accompanied by an audible burst of relief.  You make wet 'schlick'ing sounds as you spread the slime around, vigorously frigging yourself.  ", false);
                ...
                if (player.effects.getValue3Of(StatusAffects.Eggs) >= 11) outputText("Your swollen belly doesn't seem to be done with you, as yet another egg pushes its way to freedom.   The stimulation so soon after orgasm pushes you into a pleasure-stupor.  If anyone or anything discovered you now, they would see you collapsed next to a pile of eggs, your fingers tracing the outline of your " + vaginaDescript(player, 0) + " as more and more eggs pop free.  In time your wits return, leaving you with the realization that you are no longer pregnant.  ", false);
                outputText("\n\nYou gaze down at the mess, counting " + eggDescript() + ".", false);
                player.orgasm();
                dynStats("resisted", false);
            }
            outputText("\n\n<b>You feel compelled to leave the eggs behind, ", false);
            if (player.effects.findByType(StatusAffects.AteEgg) >= 0) outputText("but you remember the effects of the last one you ate.\n</b>", false);
            ...
            player.effects.create(StatusAffects.LootEgg, 0, 0, 0, 0);
            displayedUpdate = true;
            player.knockUpForce(); // Clear Pregnancy
        }
    }
...
export function eggDescript(plural: boolean = true): string {
    ...
    if (player.effects.findByType(StatusAffects.Eggs) >= 0) {
        descript += num2Text(player.effects.getValue3Of(StatusAffects.Eggs)) + " ";
        // size descriptor
        if (player.effects.getValue2Of(StatusAffects.Eggs) == 1) descript += "large ";
		/*color descriptor
		...
		5 - rubbery black -
		*/
        if (player.effects.getValue1Of(StatusAffects.Eggs) == 0) descript += "brown ";
        if (player.effects.getValue1Of(StatusAffects.Eggs) == 1) descript += "purple ";
        if (player.effects.getValue1Of(StatusAffects.Eggs) == 2) descript += "blue ";
        if (player.effects.getValue1Of(StatusAffects.Eggs) == 3) descript += "pink ";
        if (player.effects.getValue1Of(StatusAffects.Eggs) == 4) descript += "white ";
        if (player.effects.getValue1Of(StatusAffects.Eggs) == 5) descript += "rubbery black ";
        // EGGS
