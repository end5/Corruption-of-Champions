...
export function plantyLadySeeWhatHappens():void {
	clearOutput();
...
//[Let's fuck!]
export function plantLadyLetsFuck(from:number = 1.1):void {
	clearOutput();
	...
	Victory/Dominant:
	Male:     Pussy [V 1.1L, D 1.1]     Get BJ [V 1.2, D 1.2]     Anal     Cunnilingus    Suck dick [V 1.4, D
	1.4]              Get anal [V 1.5 and D 1.5]           Leave
	...
	Leave
	herm:    Pussy [V 1.1L, D 1.1]     Get BJ [V 1.2, D 1.2]     Anal [V 1.3, D 1.3]     Cunnilingus [V 1.7,
	D 1.7]   Suck dick   [V 1.4 and D 1.4]    Get anal [V 1.5, D 1.5]    Dick in vag [V 1.6, D 1.6]
	Get Oral [V 1.7, D 1.7]         Leave
	Genderless:    Cunnilingus [V 1.7, D 1.7]      Suck dick [V 1.4 and D 1.4]    Get anal [V 1.5, D 1.5]
	Leave

	Loss/Submissive
	Male:     Pussy [L and S 1.7]     Get BJ [L 1.2, S 1.2]    Anal  [L 1.5, S 1.5]    Cunnilingus [L 1.3, S 1.3]   Suck dick [L 1.1, S 1.1]
	Get anal [S 1.6 and L 1.6]   Leave
...
if(from == 1.1) {
    ...
    simpleChoices("Dominant",9999,"Submissive",9999,"",0,"",0,"",0);
}
}

//(Intro 1.2: Struggle) (leads to Intro: 1.2.1, or  1.2.2 scenes, depending on PC stats)
export function plantWomanStruggle():void {
clearOutput();
...
//(struggle: failure, or Intro 1.2.3) (< 40 STR)
    ...
    if(player.gender > 0) {
        ...
        if(player.gender == 3 || (player.vaginas.length + player.cocks.length > 1)) outputText("them")
        else outputText("it");
...
export function sexFailureIntro():void {
clearOutput();
...
export function declinePlantSmex():void {
clearOutput();
...
export function seriouslyGetLostPlantLady():void {
clearOutput();
...
startCombat(monster);// TODO create PlantGirl
monster.HP -= 20;
if(player.horns > 0 && player.hornType > HornType.NONE) monster.HP -= 15;
doNext(1);
}

//[Sex failure: Do it! (Trapped sex Intro (TsI))]
export function struggleFailDoItPlantSmex():void {
clearOutput();
...
//SHE RIDES UR DICK
if(player.cocks.length > 0) choices[choices.length] = trappedPlantSexPussy;
//SHE BLOWS YA
if(player.cocks.length > 0) choices[choices.length] = trappedPlantSexWantsToBlowYa;
//SHE WANTS UR DICK IN HER ASS
if(player.cocks.length > 0) choices[choices.length] = strugglePlantLadyFailAnal;
//SHE WANTS 69 - ANY GENDER
if(player.gender > 0) choices[choices.length] = failStruggle69;
//SHE FUCKS YER CUNT, YA GIT
if(player.vaginas.length > 0) choices[choices.length] = struggleFailPlantFucksPussy;
...
export function escapeFromPlantWomanAndDeclineSex():void {
clearOutput();
...
export function plantRapingAssTrappedSexIntro():void {
clearOutput();
...
export function plantRapingAssTrappedSexIntroExitOnly():void {
clearOutput();
...
export function fuckMyAssPlantLadyWhileImTrapped():void {
clearOutput();
...
if(player.cocks.length > 0) outputText("stroke");
else outputText("finger");
outputText(" you.");

outputText("\n\nAs she takes ");
if(player.cocks.length > 0) outputText("[oneCock]");
else if(player.vaginas.length > 0) outputText("your " + clitDescription(player));
...
if(player.cocks.shortestCockLength() < 48) {
    outputText("\n\nYour ");
    if(player.balls > 0) outputText("[balls] and ");
    outputText(cockDescript(player, player.cocks.shortestCockIndex()) + " flap");
    ...
    outputText(".  The creature twists and jerks at the head of your cock and works diligently to stimulate your rock hard " + multiCockDescriptLight(game.player) + ", intent on making you moan out for her.");
}
//PC has a vagina:
...
outputText(" to your ");
if(player.balls == 0) outputText(nippleDescription(player, 0));
...
export function trappedFlowerSexBlowjob():void {
clearOutput();
...
export function trappedFlowerSexBlowjobRejection():void {
...
export function trappedFlowerSexBlowjobAhoy():void {
clearOutput();
...
//(If PC has a cock(s):
else if(player.cocks.length > 0) outputText("wrap themselves around your " + multiCockDescriptLight(game.player) + " and begin to squeeze");
//(If PC has a pussy:
else if(player.vaginas.length > 0) outputText("take hold of you clitDescription(player) and tug hard");
//(If PC is genderless:
...
export function trappedPlantSexPussy():void {
clearOutput();
outputText("Staring down at your " + multiCockDescriptLight(game.player) + ", the plant woman's eyes become fixated and betray a sense of longing, of curiosity, and eagerness.  Approaching your well restrained body, she takes [oneCock] into her soft hand and brushes it's sensitive head against her smooth pelvis, fairly close to the creature's moist and sweet smelling pussy.  Proceeding to wrap its arms around you, the plant woman takes [oneCock] in-between her legs and against her clit, signaling the creature's intent to plunge its hungry cunt down on your dick.");
...
export function trappedPlantSexPussyGOOO():void {
clearOutput();
...
if(player.cocks[x].cockLength < 4) {
    ...
    if(player.cocks.length > 1) outputText("s gain");
    ...
    if(player.cocks.length > 1) outputText("es are now ");
...
//(if > 4 inches)
else outputText("\n\nShe presses her body against yours, causing your rock hard multiCockDescriptLight(game.player) to jut upward against her leafy stomach.");
outputText("\n\nRaising herself ever so slightly, the plant woman wraps her legs around and straddles you as she lowers her pussy onto the tip of " + cockDescript(game.player, x) + ".  The female perversion of nature slowly takes the entirety of your cock in her, causing your prick to sensually ease through her tight cunt like a mole crawling through the fertile earth.  The sensation is too much, causing you to shudder and twitch uncontrollably.  Your dominant partner rests her ");
//(If height is 36 inches to 72 inches:
else if(player.tallness < 72) outputText("raises her face to yours ");
...
else if(player.tallness < 96) outputText("forehead against your chest, sniffing you intensely as she takes in your musk.  The creature of flora and warped flesh moans and shivers as she acquires your " + mf(player, "manly","feminine") + " scent, and ");
...
outputText(" as she slowly begins to work her hips, grinding the innards of her hot and moist sex against your " +  cockDescript(game.player, x) + ".");
//(if one cock:
if(player.cocks.length == 1) outputText("  The plant creature begins to speed up her thrusting against your cock, eager to drive you over the edge with the tightness of her hole.");
else {
    //(If multi-cocked:
...
//(Multi-cocked:
if(player.cocks.length > 1) outputText("  Buried deep in the makeshift vaginas, [eachCock] thrusts forward and eagerly deposits the nourishment the creature craves.");
//Vagina:
if(player.vaginas.length > 0) outputText("  Cumming hard, your " + clitDescription(player) + " releases a liberal stream of juices down the vine that fucks you, dissolving into the appendage as you moan in furious release.");
...
export function trappedPlantSexWantsToBlowYa():void {
clearOutput();
outputText("The plant woman looks your body up and down, seemingly torn between what tickles her fancy and how she can take your body.  Her almost childlike darting of her eyes stops when she lays her sight on your multiCockDescriptLight(game.player), sending an aroused shiver through her body as she appears to finalize her decision.  Like a child walking to the local stream for a swim, she proceeds towards you in an almost skipping like fashion.  Reaching your naked body, she promptly sinks to her knees and takes your " + multiCockDescriptLight(game.player) + " into her hand");
//(if more than 4 cocks or PC cocks are more than 10 inches in diameter combined:
if(player.cocks.length >= 4 || player.cocks.totalCockThickness() >= 10) outputText("... or she tries to, as she fumbles with your freakish bundle of dicks.  Unable to fit them all into her hand she shrugs");
...
//[Nope, TsI: 1.4r]
export function trappedPlantSexDontBlowMe():void {
clearOutput();
outputText("It may seem she wants to take your [oneCock] and suck on it like a sugar cane, but you aren't keen on her choice of how to pleasure you.  Remaining flaccid, you stare down at the plant woman and tell her you aren't interest in a little oral love for your " + multiCockDescriptLight(game.player) + ", aware of the fact that she can't understand you.  Despite your words likely sounding like jibberish, the plant woman understands your intent and motions for you to make up your mind; given the slightly annoyed expression on her face, you had better make it quick.");
//(taken back to sex menu)
doNext(9999);
}

//[Blow me!, TsI: 1.4a]
export function trappedPlantSexBlowMe():void {
clearOutput();
...
if(player.cocks.longestCockLength() < 24) {
    outputText("  Now staring her straight in the eyes, your ");
    outputText(multiCockDescriptLight(game.player) + " stare");
    if(player.cocks.length == 1) outputText("s");
    ...
    outputText(" that will be tending to them today.  Eager to rise to said challenge, the plant woman attempts to take your " + multiCockDescriptLight(game.player) + " into her mouth and ");

    //(if cock(s) do not exceed 6 inches in total diameter:)
    if(player.cocks.totalCockThickness() < 6) {
        outputText(" manages to fit ");
        if(player.cocks.length == 1) outputText("it");
        else outputText("them");
        outputText(" into her mouth with little difficulty");
        if(player.cocks.length > 1) outputText(".  What a champ");
    }
    //if total cocks exceed 6 inches total diameter, but up to two cocks fall under 6 inches:
    else if(player.cocks.twoDickRadarSpecial(6)) {
    ...
    if(player.cocks.length > 2) {
        ...
        if(player.cocks[player.cocks.thinnestCockIndex()].cockLength < 12) outputText(" against her face, while planting her hands against your thighs");
    ...
    if(player.cocks.length >= 2) outputText("two ");
    ...
    if(player.cocks.length >= 4) outputText("upper shafts");
    else if(player.cocks.length >= 3) outputText("upper shaft");
    else outputText("[hips]");
    outputText(" as she better positions herself for the entirety of your ");
    outputText("cock");
    if(player.cocks.length > 1) outputText("s as they make");
    ...
    if(player.cocks.length > 1) outputText("s");
    outputText(" of your ");
    if(player.cocks.length > 1) outputText("two ");
    outputText("dick");
    if(player.cocks.length > 1) outputText("s");
    ...
    if(player.cocks.length == 1) outputText(".");
        ...
        outputText(", while she cheek fucks the remainder of your " + multiCockDescriptLight(game.player) + ".");
    }
    ...
    if(player.balls > 0) outputText("scrotum's");
        ...
        if(player.cocks.length == 1) outputText("cock's");
    ...
    if(player.cocks.length == 1) outputText("prop some of your length up on her average body to prevent it from straining towards the ground.");
    ...
    outputText("\n\nHaving to scoot back a fair bit to enjoy her catch, the plant woman kneels against the dirty ground and attempts to take your " + cockDescript(game.player, 0) + " into her mouth.  You feel her attempt to fit your rod into her little mouth; teeth accidentally scraping along your head, the buckling of jaws, and soft lips that are rebutted are all the signs you need to understand that your " + multiCockDescriptLight(game.player));
...
export function plantAnalGivingTheDick():void {
clearOutput();
outputText("The sight of this nice, ample ass and the thought of fucking a supple  little pucker piques your " + multiCockDescriptLight(game.player) + ", causing ");
if(player.cocks.length == 1) outputText("it");
else outputText("them");
outputText(" to become semi-erect.  As you reach her full ass, your prick");
if(player.cocks.length > 1) outputText("s poke");
...
outputText(" will hopefully penetrate is a winner in the tightness department, your " + multiCockDescriptLight(game.player) + " surge");
if(player.cocks.length > 1) outputText("s");
...
if(player.cocks.length > 1) outputText("\n\nThe rest of your " + multiCockDescriptLight(game.player) + " erupt as well, coating her butt and thighs in sticky jizz.  Like a true slut, the plant woman grabs your ejaculating prick and crams it into her fuck-hole, her pussy milking you of all the cum you can muster up.");
...
export function failStruggle69():void {
clearOutput();
...
else outputText(assholeDescript(player) + " as she dives nose first into the under crevice of your " + buttDescription(player) + ", rubbing the bridge of it between your cheeks as she giggles");
outputText(".");
...
if(player.cocks.length > 0) outputText(multiCockDescriptLight(game.player));
else if(player.vaginas.length > 0) outputText("[clit]");
...
//[No way!, TsI. 1.6r]
export function nowayStruggle69Plant():void {
clearOutput();
...
if(player.cocks.length > 0) outputText(multiCockDescriptLight);
...
export function struggle69Plant(pt:number = 1):void {
clearOutput();
...
if(pt == 2) {
    ...
    //PC pussy chosen:
    if(target == 2) outputText(", allowing her soft lips to surround your " + clitDescription(player) + ", 'trapping' it for her much anticipated assault.");
    //PC cock chosen:
    else outputText(".  Wasting no time with 'formalities,' she stuffs your " + cockDescript(game.player, 0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
}
//[69!, TsI: 1.6a, give her oral]
    ...
    //PC cock chosen:
    else outputText(" as she wastes no time with 'formalities,' stuffing your " + cockDescript(game.player, 0) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
}
...
if(target == 2) {
    ...
    if(player.cocks.length > 0) {
        outputText("\n\nIf that wasn't bad enough, the unsporting woman also takes to your " + multiCockDescriptLight(game.player) + " and strokes ");
        if(player.cocks.length == 1) outputText("it");
        else outputText("them");
    ...
    if(player.cocks.length > 0) {
        ...
        if(player.cocks.length > 1) outputText("s explode");
...
//Cock(s):
    ...
    if(player.balls > 0) outputText("testicles begin");
        ...
        outputText("cock");
        if(player.cocks.length == 1) outputText(" begins");
        else outputText("s begin");
    }
    outputText(" to churn ");
    if(player.balls == 0 && player.cocks.length == 1) outputText("its");
...
export function struggleFailPlantFucksPussy():void {
clearOutput();
...
//[No way!, TsI. 1.7r]
export function struggleFailPlantPussyFuckNoWay():void {
clearOutput();
...
//[Allow her, TsI. 1.7a]
export function struggleFailPlantPussyFuckAllow():void {
clearOutput();
...
export function victoryVsPlant():void
{
clearOutput();
//(Victory, V 1.1L):
...
export function dominateVsPlant():void {
clearOutput();
...
if(player.cocks.length > 1) {
    ...
    if(player.cocks.length == 2) outputText(" flaps");
...
if(player.balls == 0) outputText(multiCockDescriptLight(game.player));
else outputText(ballsDescriptLight(player));
outputText(" acknowledge");
if(player.balls == 0 && player.cocks.length == 1) outputText("s");
outputText(" her desire and continue to shoot thick streams of your seed into her");
if(player.cocks.length > 1) outputText(" as well as on her stomach and the ground beneath you");
if(player.vaginas.length > 0) outputText("; your vaginaDescript(player, 0) quivers with the need for release, letting loose with its own torrent of lady fluids as you moan in orgasm");
...
if(player.balls > 0) outputText("[balls] really hurt");
    ...
    if(player.cocks.length > 1) outputText("s really hurt");
}
...
//[Yes] - goofuck
export function corruptDomWinFuckGooGal():void {
clearOutput();
...
export function victoryOverPlantGalWithPCCockCorrupted(newl:boolean = false):void {
if(newl) clearOutput();
else outputText("\n\n");
outputText("After an eternity of ravaging her pussy, you feel your load building up pressure and getting ready to explode.  You tighten your grip on the plant woman's hair and with one last thrust fill her womb with your corrupted semen.  You jerk her hair back, forcing her head back towards your mouth as you begin to neck her.  \"<i>AuuughhhAHHHHHHHHHHHHHHH!</i>\" she screams out as you pull on her hair, with her pussy clamping down on your " + cockDescript(game.player, x) + ".  Her fuck hole ever so eager to receive your seed, her body convulses violently as you continue to fire more semen into the floral vixen's cervix.  The excretion is too much, and with a groan, you pass out on top of her.");
...
export function victoryDefeatPlantGirlGetBJMainSCREENON():void
{
clearOutput();
...
if(player.cocks.tentacleCocks() > 0 && (player.cocks.tentacleCocks() == player.cocks.length || rand(2) == 0)) {
    plantGirlPCWinGetTentacleBJ();
    return;
}
else {
    //[if cock size is 4-14 inches]
    if(player.cocks[player.cocks.biggestCockIndex()].cockLength < 14) outputText("takes the entirety of your [cock biggest] in her mouth and down her throat.  You lurch forward as your cock hits the back of her throat, moaning intensely at the plant girl's ability to take your whole penis in her mouth.");
    ...
    //[If Naga:
    if(player.isNaga()) outputText("\n\nWhile the plant woman occupies herself with your multicockDescriptLight(), you work your reptilian tail underneath your partner's kneeling body.  With a quick thrust upward, the tip impacts with her clit and brushes up her pelvis; the sudden touch causes the plant woman to moan out in pleasant surprise, allowing your [cock biggest] to perch itself on her tongue as she recoils.");
...
//______________________________________________________________________________
//(if Non-goo:
if(!player.isGoo()) outputText(".  She pulls your dick out of her mouth and closes her eyes as she gets ready to catch the rest of your load with her mouth.  Thick globs of semen erupt from your multiCockDescriptLight(game.player) and strike her in the face.  She herself orgasms as your torrent of semen washes over her head and chest, covering both liberally.");
//(If goo:
...
if(player.cocks[x].cockLength < 48) {
    ...
    else outputText(nippleDescription(player, 0));
    outputText(" and begins to squeeze tighter and tighter.  “Owww! Owww! Okay, Okay! I get it!” you cry out as you once again ram your " + cockDescript(game.player, x) + " down her throat.  The plant woman responds by massaging your ");
    if(player.balls > 0) outputText("balls");
...
export function beatPlantGirlDomLetsFuckAnal():void {
clearOutput();
...
if (inCombat) {
    ...
    if(player.cocks.length > 1) outputText(" grabs a of your penis and guides it to her vagina.");
    else {
    ...
    if(player.vaginas.length > 0) {
        if(player.cocks.length == 1 && player.balls > 0) outputText("  Not content with just teasing your balls, the");
        else outputText("  The");
    ...
    outputText("\n\nYou grunt and moan as your " + cockDescript(game.player, x) + " shoots its anal payload ");
    if(player.cocks.length > 1) outputText(" along with its vagina-clad brother");
    ...
    if(player.vaginas.length > 0) outputText("Your [vagina] obliges, and a few steams of lady spunk flow from your [clit] and intertwine with the fauna creature's fluids on the ground.");
...
//[Penetrate her]
export function nagaPenetrateWithTailDuringPlantGirlMeanAnalPCWin():void {
clearOutput();
...
export function pcBeatsPlantgirlAndCorruptAssFuckPt2(tail:boolean = false):void {
//Orgasm]
...
export function victorySuckPlantgirlsDick():void {
clearOutput();
...
//(Dominant intro, D 1.4:
    ...
    //PC has a dick:
    if(player.cocks.length > 0) outputText("take [oneCock] and poke the tip of your head against her pelvis; where a penis would be located if she were a guy... or pretty much everything else around here.");
...
if (inCombat) {
    ...
    if(player.cocks.length > 0) outputText("[eachCock] is rigidly erect");
    else if(player.vaginas.length > 0) outputText("your [vagina] is gazed over with your secretions");
    //Genderless:
...
export function beatPlantGirlAndTakePlantCockInButt():void {
clearOutput();
//(Victory, V 1.5:
if (inCombat)
...
if(player.cocks.length > 0) outputText("[eachCock] becomes rock solid");
if(player.gender == 3) outputText(" while ");
if(player.vaginas.length > 0) outputText("your pussy becomes incredibly wet");
...
if(player.cocks.length > 0) outputText("[eachCock] jerks upward, the last shot of blood filling and hitting them like a thunderous punch");
if(player.gender == 3) outputText(", while ");
if(player.vaginas.length > 0) outputText("your pussy leaks a bit of girl cum.  It slowly drools out onto her pelvis");
...
else if(player.vaginas.length > 0) outputText("Your vaginal muscles clenching up along with your sphincter, y");
else outputText("Y");
  outputText("ou ");
...
if(player.gender > 0) {
    ...
    if(player.cocks.length > 0) outputText("[oneCock]");
    if(player.vaginas.length > 0) outputText("your vulva");
}
//PC is genderless:
...
if(player.cocks.length > 0) outputText("strokes [oneCock] with her hands");
if(player.gender == 3) outputText(" as she also ");
if(player.vaginas.length > 0) outputText("massages your labia");
...
if(player.cocks.length > 0) outputText("quickens her pace of stroking your cock");
if(player.gender == 3) outputText(" and ");
if(player.vaginas.length > 0) outputText("fingers the interior of your pussy, causing you to roll your eyes into the back of your head as she sensually teases the inside of your sex");
...
if(player.vaginas.length > 0) outputText("vaginal muscles clamping down and drooling pussy juice all over the plant woman's lap");
...
export function defeatPlantgirlBlowAndButtfuckCorrupt():void {
clearOutput();
...
export function defeatPlantgirlBlowAndButtfuckPure():void {
//V 1.6 and D 1.6
clearOutput();
outputText("With your hands on your hips you " + mf(player, "walk","sway"));
...
outputText(clitDescription(player) + ".  The plant woman yelps in surprise as the vine makes contact with your labia, causing you to jump a little.  You turn to face the plant woman, who is blushing profusely and resting her hand on her pussy.  Looking at the vegetation, you deduce that she must be able to feel you along her vines and that these creepers may also function as penile appendages.  A grin spreads across your face as the realization hits you, and you bring the vine back to your sex and tease your vulva.  Soft moans escape the plant girl as the head of the perverted vegetation brushes past your clit, and you ");
//(pure:
if(player.cor < 50) outputText("giggle");
//Corrupted:
else outputText("smirk");
outputText(" at the woman's predicament.  Deciding to stop teasing the poor girl, you take the head of the creeper and push it into your pussy, edging it upwards at a gradual pace so that your partner can feel the contours of your " + vaginaDescript(player, 0) + ".");
...
export function vineAndCockWithPlantGirlPCWin(scene:number = 1):void {
...
[If you chose just the vine, V and D 1.6vp] You slip a hand to your
  ...
  (if female) breastDescriptOfRow(game.player, 0) and start to massage your hardening nippleDescriptAll().

(if herm)) member and begin to (fondle your cockDescript(game.player, 0)).
...
Both of you moan as your sexes touch one another, sensation running through your vulva like wildfire. Twitching ever so slightly, the plant woman grabs you by your hipDescription(player) as she angles her cock to breach the mouth of your vaginaDescript(player, 0).  Without wasting any more time you buck your hips downward, driving her member past your hungry lips and through the walls of your sex, flooding your body with endorphins and bringing on a sensation of giddiness.  Your partner squirms under you as her cock bottoms out inside of you, unable to control herself as her member takes the full blunt of your pussy. Grabbing her by the shoulders you begin to fuck the plant woman, bobbing your [butt] up and down against her lap; the melancholy of your intimate act ringing forth with the sounds of sloppy, wet sex. The nerves in your pussy light up as her member brushes back and forth inside of your fuck-hole, making you cry out occasionally as she “abuses” your cunt like it owes you money.  In response, you make the rough ride you're giving her faster and more savage.  Hands darting upward, the plant woman hungrily grabs your breastDescriptOfRow(game.player, 0),

(if breasts: pulling on your nippleDescription(player, 0) as she bucks her head back in utter bliss. You let out a pained cry and twist her nipples in response, soliciting a similar reaction from her. Both of	 you grin manically at each other, and you lower yourself to give her a quick kiss before rising back up into your straddling position, eager to abuse her cock until you get her sweet splooge inside of you)
...
(if herm: slips her hand down to your multiCockDescriptLight(game.player), taking (it/one at a time) into her hands, stroking (it/them) furiously as you pound her moist love canal.  You let up on the rough fucking for a  moment as she tends to your dick(s), taking in the euphoria of her soft, leaf like texture on your naughty bit(s). )
...
Eventually all good things must come to an end;  Your inevitable orgasm approaches, body trembling in anticipation at the thought of covering this creature in cum.  Sexual fluids rush out of you as you concentrate on pushing, forcing them out. Instinct takes hold as you mash your pelvis against the plant woman's lap. (Your multiCockDescriptLight(game.player)) explode(s),
...
[Goo anal]
With a soft giggle, you snap your attention to the gooey mass at the mouth of the plant woman's asshole, and pause briefly to focus.  Your lover stares up at you quizzically for but  brief moment, before her eyes go wide at the slimy intrusion at the opening of her tight pucker.  Lathering her insides up as you go, you direct your gooey body into her tight little asshole  and gradually solidify it into a sticky dildo of sorts. Weakly the creature groans, likely due to the fact that you are triple penetrating yourself with her penile vegetation while fucking both her cock and asshole.  Grinning devilishly, you start your gyrations once more.
(Leads to Climax)
...
(if vagina and/or cock: (vaginaDescript(player, 0)/   /and/ multiCockDescriptLight(game.player)/
...
(vaginaDescript(player, 0)/ multiCockDescriptLight(game.player)/ assholeDescript(0) ).
...
(multiCockDescriptLight(game.player)/ vaginaDescript(player, 0)/assholeDescript( ) )
against her face, slathering (it/them) down her forehead and past her nose as
PC has cock(s): (gravity takes your multiCockDescriptLight(game.player) and prods (it/them) against her lips)
...
Vc 1.7: cock: (If PC can use two cocks, use two cocks. If PC can't, just
...
(If dick instanceof greater than 44 inches, but not 60 inches: You slam your cock(s) against her
right hand. With a surprised giggle, she takes your (cock/one of your cocks) in her hand and begins to stroke you with unbridled speed and effort.  Now that that problem instanceof out of the way, you dive your face into her lap and burrow your tongue deep into her, sliding around like an eel in a shallow, narrow stream. (leads up to Vc 1.7 cock end)
(If dick instanceof greater than 60 inches: take the plant woman's legs and position them against
...
Vc 1.7 pussy end: Soon, you feel her muscles tensing up and figure that her orgasm will soon be arriving.  Not content to having her orgasm first, you treat her face like two pieces of kindling as you buck against her face, sliding your sex against her in an effort to "ignite" your own sexual release.  Gritting your teeth as you do so, your pussy tingles and becomes overwhelmed with the sensation of brushing your clitDescription(player) against the fauna-like texture of her face.  Pulling back on the plant girl's legs you bate your breath, orgasm flooding through your diaphragm and eager to escape from your mouth. With stifled, passionate moans, you (douse/PC is a squirt-er: spray) your fluids all over the plant creature's face, and she quickly reciprocates with her own barrage of pussy juice against your own face.  Hungrily, you lap and slurp it all up like a person dying of thirst.
You hold the contorted position you two are in for several  until you regain your strength. Apathetically flopping the fatigued woman to the ground, you proceed to grab your (armor) and head back to camp, pausing only to comment on how you look forward to the next time you get to "punish" the fauna beauty.
(if genderless. Vc 1.7g: prods the crack of your ass as she parts your [butt], and licks your assholeDescript(player) with her energetic tongue; oddly enough, it resembles the texture of flower petals, and having that sensation against your hole causes you to quiver.)  Her moist tongue slaps against your hole, leaving it nice and wet as she prods and lathers your hole with her tongue. )
