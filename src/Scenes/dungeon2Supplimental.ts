...
export function loseToImpMob(): void {
    ...
    if (player.biggestTitSize() > 1) {
        outputText("A sudden weight drops onto your chest as one of the demons straddles your belly, allowing his thick, tainted fuck-stick to plop down between your " + allBreastsDescript() + ".  The hot fluid leaking from his nodule-ringed crown  swiftly lubricates your cleavage.  In seconds the little devil is squeezing your " + game.player.breastDescript(0) + " around himself as he starts pounding his member into your tits.  The purplish tip peeks out between your jiggling flesh mounds, dripping with tainted moisture.", false);
        if (player.biggestLactation() > 1) outputText("  Milk starts to squirt from the pressure being applied to your " + game.player.breastDescript(0) + ", which only encourages the imp to squeeze even harder.", false);
        outputText("\n\n", false);
    }
    // (NIPPLECUNTS!)
...
export function impGangGetsRapedByMale(): void {
    outputText("", true);
    outputText("You walk around and pick out three of the demons with the cutest, girliest faces.  You set them on a table and pull aside your " + player.armorName + ", revealing your " + player.multiCockDescriptLight() + ".  You say, \"<i>Lick,</i>\" in a tone that brooks no argument.  The feminine imps nod and open wide, letting their long tongues free.   Narrow and slightly forked at the tips, the slippery tongues wrap around your " + game.player.cockDescript(0) + ", slurping wetly as they pass over each other in their attempts to please you.\n\n", false);
    ...
    outputText("The girly imp to your left, seeing how occupied your " + game.player.cockDescript(0) + " is, shifts his attention down to your ", false);
...
export function encapsulationPodAI(): void {
    ...
    // [Round 2 Action]
        ...
        if (player.cockTotal() > 0) {
            outputText("prods your " + game.player.cockDescript(0) + " for a second before it begins slithering around it, snake-like.  Once it has you encircled from " + player.cockHead() + " to ", false);
        ...
        if (player.cockTotal() > 1) {
            ...
            if (player.cockTotal() == 2) outputText(game.player.cockDescript(1) + " gets the same treatment, and soon both of your " + player.multiCockDescriptLight() + " are quite happy to be here.  ", false);
...
export function loseToThisShitPartII(): void {
    ...
    if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
        ...
        outputText(" and squeeze, forcing " + player.sMultiCockDesc() + " to inflate to readiness.  Deep inside your " + assholeDescript() + ", the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ".\n\n", false);
...
export function useValaOHYEAHKOOLAIDPTII(): void {
    ...
    if (player.gender == 1) {
        ...
        outputText("Finally spent, you step back and toss the girl off of your " + game.player.cockDescript(0) + ", letting her fall back into a slump against the wall, dangling from her manacles. A wicked thought crosses your mind and you step back to the alchemy table. Grabbing armfuls of hexagonal bottles, you flip the girl around, exposing her drooling, empty face and her sore, gushing breasts. You plug a bottle up to her lips and she begins sucking at it automatically, perhaps believing it's a dick in some corner of her ravaged mind. Vala quaffs the Ovi Elixir gratefully, then a second, then a third. By the time you've finished pouring the stuff down her throat, her body has already begun changing. The elixir has pounced on your seed and forced her ovulation, rapidly accelerating the speed of conception. Her tummy pouts, then bloats as your sperm impregnates the fairy in seconds. Her abdomen swells violently, and you suspect that force-feeding her so much so quickly may have resulted in twins or triplets. She moans in blissful pregnancy, her already disproportioned tits gurgling with even greater milky weight. If there was any doubt as to if she could stand under her own power before, it's gone now- even your strength wouldn't be able to move the breeding cow that you've turned her into. Knowing your luck, however, she'll have given birth by the time you check in next.", false);
    }
    // f2
    else if (player.gender == 2) {
...
export function cleansedValaFuckHerBrainsOut(): void {
    ...
    if (player.totalCocks() > 0) {
        ...
        outputText("Feeling your straining body, hard against hers, she runs a hand through her purple hair, spilling it across half of her face. \"<i>I want you to enjoy this as much as I will.</i>\" Vala reaches a hand between her legs and slides against your abdomen, guiding your " + game.player.cockDescript(0) + " into her pussy. The fae's labia practically breathes the heat of her uterus across your twitching shaft and you slide through her well-lubricated depths with a moist sigh, her inhuman snatch tight with her passionate yearning. She rocks back and forth barely an inch at a time, but it's enough to ignite a manic need in your loins. ", false);
        // [2 dicks-
        if (player.totalCocks() > 1) outputText("\"<i>Fairies don't exactly crap rainbows, but we're not quite like normal humans either,</i>\" she grins, taking your " + game.player.cockDescript(1) + " and pressing it against her pudgy butt, her pink sphincter loosening just enough to slip your cockhead inside the fairy's body before it clenches down. It almost feels like she's got a hand inside of her ass; the walls ripple with expert coordination, rings of muscle clenching and loosening with a dexterity that takes the breath from you. Your tip, sealed just inside her posterior, is milked with a tight grace that has your body twitching before you realize it.  ", false);
        // 3+ dicks-
        if (player.cockTotal() >= 3) outputText("Vala takes a deep breath, letting the feeling of your meat inside her gut stoke the fires of lust growing in her belly. \"<i>I think there's a little room left, if you're game,</i>\" she whispers, moving to your " + game.player.cockDescript(2) + ". She bites her lower lip and forces herself down on it, squeezing your two shafts together in her well-lubed love tunnel. The feeling of your straining lengths inside her triple-stuffed body is an ecstatic agony that robs you of words and your body thrusts helplessly, Vala's breeder hips grinding in fleshy ripples, her lower torso swollen with cock.  ", false);
...
export function faerieOrgyFuckMaleContinue(): void {
    ...
    outputText("You nearly forget to breathe as jizz pours from your body until the dick-shaped shell is entirely forced from your body. Deftly, Vala catches the device before it can spill its load and she pastes her own, much larger, petal on the base, sealing your spunk inside. The verdant shaft, now that it's off your cock, seems to be a dildo perfectly molded to resemble your " + game.player.cockDescript(0) + ". It's a soft pink color, like engorged genitals, and even gently pulses with your every heartbeat.", false);
...
export function releaseZForInfo(): void {
    ...
    if (player.cor > 66) {
        ...
        if (player.biggestTitSize() < 1) outputText("chest", false);
        else outputText(game.player.breastDescript(0), false);
...
export function malesZetazOver(): void {
    ...
    outputText(" you as you're thrown atop one of the tables and rolled onto your side.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you're completely immobilized from the neck down, and your " + player.legs() + " are kept spread to allow easy access to " + player.sMultiCockDesc() + " and " + assholeDescript() + ".\n\n", false);
    ...
    outputText("Zetaz hands the funnel to an underling with a knowing laugh and repositions himself over your " + player.legs() + ".  Warm pressure pushes at your " + assholeDescript() + ", forcing your clenching flesh to yield around the intruder.  Normally such an instant penetration would be irritating, or perhaps painful, but the sudden pressure on your prostate only serves to release a copious squirt of pre-cum.  An unwelcome moan slips past your lips and sends a titter of laughter through the mob.  As if losing wasn't bad enough – they all know you're getting off on having your " + assDescript() + " penetrated.  The worst part is that the humiliation is just making the situation hotter and " + player.sMultiCockDesc() + " harder.\n\n", false);
...
export function kinathisValaStuff(): void {
    ...
    else outputText(game.player.cockDescript(x));
