...
export function bigJunkDesertScene(): void {
    ...
    if (!player.isTaur()) {
        ...
        if (player.breastRows.biggestTitSize() >= 35) outputText("  Your " + allBreastsDescript(player) + " hang lewdly off your torso to rest on the desert sands, seeming to bury the dunes on either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The burning heat of the desert teases your " + nippleDescription(player, 0) + "s mercilessly as they grind in the sand.", false);
        // IF CHARACTER HAS A BALLS ADD SENTENCE
        if (player.balls > 0) outputText("  Your " + player.skinTone + sackDescript(player) + " rests beneath your raised " + buttDescription(player) + ".  The fiery warmth of the desert caresses it, causing your " + ballsDescriptLight(player) + " to pulse with the need to release their sperm through your " + multiCockDescriptLight(game.player) + ".", false);
    ...
    // FOR CENTAURS
        ...
        outputText("The impending erection can't seem to be stopped.  Your sexual frustration forces stiffness into your " + multiCockDescriptLight(game.player) + ", which forces the barrel of your horse-like torso to the ground.  Normally your erection would merely hover above the ground in between your centaurian legs, but your genitals have grown too large and heavy for your " + hipDescription(player) + " to hold them aloft.  Instead, you feel your body being forcibly pulled down at your hindquarters until you rest atop your " + multiCockDescriptLight(game.player) + ".", false);
        // IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
        if (player.breastRows.biggestTitSize() >= 35) outputText("  Your " + allBreastsDescript(player) + " pull your human torso forward until it also is forced to rest facedown, just like your horse half.  Your tits rest, pinned on the desert sand to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The burning heat of the desert teases your " + nippleDescription(player, 0) + "s incessantly.", false);
        // IF CHARACTER HAS A BALLS ADD SENTENCE
        if (player.balls > 0) outputText("  Your " + player.skinTone + sackDescript(player) + " rests beneath your raised " + buttDescription(player) + ".  The airy warmth of the desert teases it, causing your " + ballsDescriptLight(player) + " pulse with the need to release their sperm through your " + multiCockDescriptLight(game.player) + ".", false);
        // IF CHARACTER HAS A VAGINA ADD SENTENCE
        if (player.vaginas.length >= 1) {
            outputText("  Your " + vaginaDescript(player) + " and " + clitDescription(player) + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + buttDescription(player) + " above.", false);
            // IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
