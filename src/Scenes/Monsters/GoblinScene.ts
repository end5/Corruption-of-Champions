...
export function goblinRapesPlayer(): void {
    ...
    if (player.perks.findByType(PerkLib.BimboBrains) >= 0 || player.perks.findByType(PerkLib.FutaFaculties) >= 0) {
        ...
        if (player.vaginas.length > 0) {
            ...
            // --[If nipplecunts]
            if (player.breastRows.hasFuckableNipples()) outputText(", fingering your wet nipplecunts and pinching the now-puffy areolas tightly, sending uncontrollable waves of pleasure from your chest to your groin.", false);
            // -[If not]
            ...
            outputText("Your green slut counterpart stands up and steps over your crotch, positioning herself perpendicular to you. The warm wetness of her dripping cunt splashes your thighs when she works the free end of the double-dong into her own slick twat. Your pussy squelches against hers wetly as they meet in the middle, your " + clitDescription(player) + " pressed on by her nether lips. The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing against your " + clitDescription(player) + " again and again.\n\n", false);
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas[0].vaginalWetness <= VaginaWetness.NORMAL) outputText("The lips of your sex engorge, becoming almost as puffy as the goblin's.  ", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("Feminine lubricant soaks into the back of your " + player.armorName + ".  ", false);
        else outputText("It rapidly forms into a puddle as your " + vaginaDescript(player, 0) + " does its best to show just how ready you are.  ", false);
    }
    outputText("Your " + nippleDescription(player, 0) + "s poke upwards, giving the goblin easy targets.  She grabs hold of them ", false);
    if (player.breastRows.hasFuckableNipples()) outputText("slipping her thumbs inside the tender cunts and pinching against them tightly", false);
    ...
    if (player.cocks.length > 0 && (!player.vaginas.length > 0 || rand(2) == 0)) {
        ...
        if (player.cocks.cockArea(0) > monster.vaginalCapacity()) {
            ...
            else outputText("the base of your " + cockDescript(game.player, 0), false);
            outputText(" before she buries it deep into your " + assholeOrPussy(player) + ".  Her thighs, breasts, and body surround you with a shroud of warm green flesh that wriggles and twists around you with feverish lust.  The lips of her still-partially stretched cunt do their best to devour your member's head when the flexible little minx curves her hips back down to grind on it.\n\n", false);
        ...
        // [DICK FITS]
            ...
            if (player.balls > 0) outputText("cup your " + ballsDescriptLight(player) + " in one hand, squeezing gently as the drug begins to affect them", false);
    ...
    // [FEMALEZ]
        ...
        outputText("Later, the wet goblin audibly pops off the dildo.  She stumbles, bow-legged, before teasing your " + clitDescription(player) + " and yanking her toy free.  Your lips ", false);
        if (player.vaginas[0].vaginalLooseness <= VaginaLooseness.GAPING) outputText("gape apart momentarily", false);
...
export function gobboRapeIntro(): void {
    ...
    let buttseks: () => void = null;
    let feeder: () => void = null;
    let fitsFuck: () => void = null;
    let tooBig: () => void = null;
    let corruptTooBig: () => void = null;
    ...
    if (player.cocks.length > 0) {
        ...
        // Spidercondom
        if (player.tailType == TailType.SPIDER_ADBOMEN && player.cocks.cockThatFits(monster.vaginalCapacity()) >= 0)
...
function giveGoblinAMilkMustache(): void {
    ...
    outputText("She struggles against your grip, trying to get something, anything inside her needy pussy while yelling \"<i>Come on " + mf(player, "slut", "stud") + ", you know you want to - mmph!</i>\"  You cut her off by shoving her mouth onto your " + nippleDescription(player, 0) + ". She gasps involuntarily, filling her mouth with your milk. In an instant she freezes, then slowly swallows the milk in her mouth. She relaxes in your arms a moment later, gently suckling at your nipple. Her old lust-filled self is gone, replaced with a pliant girl who now wants nothing but your milk. You slowly lower your hand and start rubbing at her still-slick pussy. In response, she puts her hand on your other " + nippleDescription(player, 0) + ", playing with it and teasing you.\n\n", false);

    outputText("After a while, you feel the goblin fall asleep in your arms. Even then, she still continues suckling gently on your " + nippleDescription(player, 0) + ". You smile, satisfied, and gently lift the goblin off your chest. You pat her shoulder softly, and she stirs awake again. She gives you a bit of a dazed look before you give her a gentle push, and she starts walking away with a vacant, drooling stare.", false);
...
function gobboButtSecks(): void {
    ...
    outputText("After quickly removing your " + player.armorName + ", you tear off what little clothing the green-skinned woman is wearing.  With ease, you lift her off the ground and position her over your " + cockDescript(game.player, x) + ". The little goblin whore is so enthralled with you that her body quivers from excitement.\n\n", false);
    outputText("With a sneer, you take the crown of your " + cockDescript(game.player, x) + " and press it not against her drenched pussy lips, but her asshole!\n\n", false);
    ...
    outputText("Without any reservations, you slam the goblin whore onto your " + cockDescript(game.player, x) + ", virtually impaling her! The immediate tightness of her asshole nearly drives you over the edge! The more she squirms, the tighter her ass muscles squeeze you. You stand frozen in ecstasy for a moment, your tongue drooping out of your mouth and eyes rolling into the back of your head.\n\n", false);
    outputText("Protesting, the goblin squirms more, even going as far as to beat her fists into your chest; she's so feeble at the moment that her strikes actually tickle. Her puny assault is enough to knock you out of your carnal daze. You grin manically as you grasp her tiny waist and hammer her! Your " + cockDescript(game.player, x) + " quickly adjusts to her stretching asshole as you thrust harder and deeper!\n\n", false);
    outputText("\"<i>This isn't what I wanted!</i>\" she cries, \"<i>This isn't what I wanted!</i>\" If your " + cockDescript(game.player, x) + " expanding her ass wasn't enough, she can feel your ", false);
    if (player.balls > 0) outputText(sackDescript(player), false);
    else outputText(hipDescription(player), false);
...
function gobboGetsRapedFem(): void {
    ...
    // Goblin victory rape, female naga:
        ...
        // [Has fangs:
        if (player.faceType == FaceType.SNAKE_FANGS) outputText("You bare your fangs at her and the curiosity disappears. She turns to run, but your tail is faster than she is.", false);
        // [No fangs:
        ...
        outputText("That's proven very false in a moment though, as her eyes fly open and she yells out \"<i>WHOOOOOO!</i>\". You're startled at the sudden change in demeanor, but decide to roll with it, shoving your " + vaginaDescript(player, 0) + " into the evidently very energetic little creature.\n\n", false);
        ...
        if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("grows puffy and moist", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("drips with feminine moisture", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("slowly begins to soak your thighs", false);
        ...
        outputText("The goblin squeals, though in delight or pain you can't be sure.  You start spanking her harder and harder, turning her ass from green to red with the repeated abuse.  Her entire body begins convulsing and squirting, splattering your arm with her honey.  She got off on it!  Well, after that kind of show, she's going to get you off too - or else!\n\n", false);
        outputText("You pull her off your leg and shove her face into your " + vaginaDescript(player, 0) + ".  She thrashes against you, too lost to her own pleasure to realize what's going on.   Forcefully you put her plump little lips on your box and grind, letting her tongue slip into your folds.  Her eyes are little white slits, her pupils rolled up out of view, but there's enough of something in there that her tongue recognizes the taste and starts licking.  You tremble; it feels WAY better than it should.  Perhaps some of her potions have left a residue on her lips and tongue, but you don't care.\n\n", false);
        outputText("You cum on the drugged green bitch, ", false);
        if (player.vaginas[0].vaginalWetness > VaginaWetness.DROOLING) outputText("splattering her with your fluids", false);
        else if (player.vaginas[0].vaginalWetness > VaginaWetness.SLICK) outputText("coating her face with the proof of your pleasure", false);
        else if (player.vaginas[0].vaginalWetness > VaginaWetness.WET) outputText("leaving her sticky with your nether-juices", false);
        else outputText("leaving the taste of pussy on her tongue", false);
        outputText(".  ", false);
        if (player.breastRows.biggestLactation() >= 3.5) outputText("Milk explodes from your nipples, soaking the petite slut.  ", false);
        else if (player.breastRows.biggestLactation() > 2) outputText("Twin streams of milk soak the slut's hair, running down her face like white tears.  ", false);
        else if (player.breastRows.biggestLactation() >= 1) outputText("Milk dribbles from your nipples, falling into the little slut's hair.  ", false);
...
function rapeAGoblinCorruptTooBig(): void {
    ...
    outputText("You have a seat, legs wide, on the ground and hold the little goblin whore with her relatively tiny slit resting at the tip of your " + cockDescript(game.player, x) + ". Finally out of her stupor a look of extreme conflict crosses her face as her need for cock and cum goes to war with her survival instincts. On the brink of defeat, those instinct suddenly regroup and beat back her lust once she feels the pain of just the " + cockHead(player, x) + " of your " + cockDescript(game.player, x) + " starting to stretch out her " + vaginaDescript(monster) + ". She does all she can to resist, but with the way you're holding this is little more than flailing wildly and pushing against your " + cockDescript(game.player, x) + " with her feet, practically climbing it like the tree trunk it must look like from her perspective.\n\n", false);
    outputText("Both of you dripping with sweat from your respective exertions, you slowly begin to realize the combination of her furious struggling and the tightness of her " + vaginaDescript(monster) + " is going to keep you from the penetration you were so looking forward to. However, as you begin to consider finishing off in her throat, a darkness stirs and another idea crosses your mind.\n\n", false);
    outputText("The goblin relaxes a little when she feels you no longer pressing her down onto your " + cockDescript(game.player, x) + ". She absolutely thrills when you bring her tiny pussy to your lips and begin to have at it. Your tongue plays around both on the inside and outside of her " + vaginaDescript(monster) + " until you coax her little nub out from hiding. You wrap your lips around it and begin putting your corruption to task. As you drive the goblin slut closer and closer to orgasm, working over her clit with an expertise rarely found outside of the infernal ranks, bolts of corrupt energies travel from your tongue, through her clit, and deep into her core. As she cums, screaming, you pull off of her, admiring the outward signs of your demonic handiwork. What before was an average sized love button has swollen five times its size into a proud, engorged clit... and that's not the only thing you enlarged.\n\n", false);

    outputText("You reposition your fucktoy so that now she's facing away from you and again bring your " + cockDescript(game.player, x) + " into contact with her " + vaginaDescript(monster) + ". Again she struggles, but after her orgasm she doesn't have the strength to put up a decent fight; all her resistance does is arouse you further, now that she's fighting the inevitable. You begin to press her down onto yourself.\n\n", false);
    ...
    outputText("You ram home the rest of your " + cockDescript(game.player, x) + " left outside of your newly christened dick jockey. It's enough to orgasm the bitch, and you haven't even gotten started. You ", false);
    ...
    if (player.breastRows.biggestTitSize() >= 1) {
        ...
        if (player.breastRows.biggestLactation() >= 1) {
            ...
            if (player.breastRows.biggestLactation() < 3) outputText("weep streams of milk onto her features", false);
            else outputText("plaster her with gouts of fluid", false);
        }
    }
    outputText(". Her hair is matted with", false);
    if (player.breastRows.biggestLactation() >= 1) outputText(" milk and", false);
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas[0].vaginalWetness <= VaginaWetness.SLICK) outputText("juices ", false);
        else if (player.vaginas[0].vaginalWetness <= VaginaWetness.DROOLING) outputText("floods ", false);
    ...
    if (player.balls >= 2) outputText("Your " + ballsDescriptLight(player) + " are trying but, h", false);
    else outputText("H", false);
    outputText("er spasming " + vaginaDescript(monster) + " is clamping down so hard on your " + cockDescript(game.player, x) + " that it can't release.\n\n", false);
...
function manRapesGoblinTooBig(): void {
    ...
    outputText(" of your " + cockDescript(game.player, x) + " visibly pulses in excitement as you brush it against her already-slick folds.  She squirms under you, clearly enjoying the feeling of you pushing against her opening.  With painful slowness, you begin pushing forward, feeling her body stretch around your ", false);
    if (player.cocks[0].cockType == CockTypesEnum.HORSE) outputText("flare", false);
    ...
    outputText("Disgruntled with the tease, you pull out and slide it onto her torso, pinning her underneath your " + cockDescript(game.player, x) + " and smearing her face and body with her juices.  Her tits squish down enough that her erect little purple nipples barely poke out on each side.  The little slut looks relieved and more than a little turned on.  She licks her lips and speaks happily, \"<i>Thanks hun, I think you would have torn me in half!  Don't worry, I'm more than happy to get soaked with your cum,</i>\" as she wraps her arms and legs around you", false);
...
function gobboGetsRapedMaleFits(): void {
    ...
    if (player.cocks.cockArea(x) > monster.vaginalCapacity() * .8) {
        ...
        outputText("The familiar tightness of a coming orgasm grows in your groin, tightening as you near release. You pick up the pace, full out sprinting, letting the girl bounce and jiggle as she clings to you, supported entirely by your " + cockDescript(game.player, x) + ". ", false);
        if (player.balls > 0) outputText("Your " + ballsDescriptLight(player) + " tighten, releasing the seed of your orgasm.  ", false);
        ...
        if (player.cumQ() >= 100) outputText("  Your enhanced body easily stuffs her full of cream, pudging her belly out slightly, your seed staying embedded in her womb with nowhere to escape, her cunt plugged tightly with your " + cockDescript(game.player, x) + ".", false);
        if (player.cumQ() >= 500) outputText("  The orgasm is so potent that by the time you wind down, she looks to be sporting a pregnancy the size of a medicine ball.  Your cum is trapped inside her, unable to find any gap between her walls and your " + cockDescript(game.player, x) + ".", false);
        else if (player.cumQ() >= 250) outputText("  The orgasm is so potent that by the time you wind down, she looks heavily pregnant.  Your cum is unable to find any gap between her walls and your " + cockDescript(game.player, x) + ".", false);
        outputText("\n\n", false);
    ...
    // (FITS – Get ridden)
        ...
        outputText("You pick up the defeated goblin, looking her over.  She crosses her arms across her chest pitifully and asks, \"<i>What now?</i>\" with her eyes darting down when she thinks you won't notice.  You muse to yourself 'great minds think alike' and lower her down towards your " + cockDescript(game.player, x) + ".  The tip slips between her moist and parted folds, brushing against her entrance and taking some of her weight for you.  She goes cross-eyed and smiles happily, wrapping her legs as far around your " + hipDescription(player) + " as possible.\n\n", false);
        ...
        outputText("The familiar tightness of a coming orgasm grows in your groin, tightening as you near release.  You pick the pace, full out sprinting, letting the girl bounce and jiggle as she clings to you, supported entirely by your " + cockDescript(game.player, x) + ".  ", false);
        if (player.balls > 0) outputText("Your " + ballsDescriptLight(player) + " tighten, releasing the seed of your orgasm.  ", false);
        outputText("The howl of a powerful orgasm fills your ears as your cumming sets off the little green cock-sleeve.  One of her hands lets go, and starts rubbing her belly while she kisses and licks your belly-button.", false);
        if (player.cumQ() >= 250) {
        ...
        if (player.cumQ() >= 500) outputText("  The orgasm is so potent that by the time you wind down, she looks heavily pregnant and your cum squirts out of any gap it can find between her walls and your " + cockDescript(game.player, x) + ".", false);
        outputText("\n\n", false);
        outputText("You pant and stop, pulling the stuffed goblin off you and setting her on the ground, smiling in satisfaction as your cum ", false);
        if (player.cumQ() >= 250) outputText("pours out in a river", false);
        else outputText("leaks", false);
...
function goblinCondomed(): void {
    ...
    outputText("Impatiently, she thrusts her legs out and hooks her toes around your lower body, trying to pull you closer while still keeping her needy hole accessible. Her olive feet clench around your flesh, her soles firm and muscular on your " + skinFurScales(player) + " as she slides up and down the outsides of your " + hipDescription(player) + ". Dragging her heels across your thighs, the goblin pushes her feet together on either side of your " + cockDescript(game.player, x) + ", the balls of her jade skin pressing against ", false);
