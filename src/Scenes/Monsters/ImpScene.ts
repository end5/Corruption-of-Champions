...
export function impVictory(): void {
    clearOutput();
    const canFeed: boolean = (player.effects.findByType(StatusAffects.Feeder) >= 0);
    const canBikiniTits: boolean = (player.vaginas.length > 0 && player.breastRows.biggestTitSize() >= 4 && player.armor instanceof LustyMaidensArmor);
    ...
    if (player.lust > 33) {
        let maleRape: () => void = null;
        if (player.cocks.length > 0) {
            if (player.cocks.cockThatFits(monster.analCapacity()) == -1)
        ...
        else if (maleRape == null && !player.breastRows.hasFuckableNipples() && !canFeed && !canBikiniTits && !player.ovipositor.canOvipositBee()) {
        ...
        if (player.breastRows.hasFuckableNipples()) addButton(2, "NippleFuck", noogaisNippleRape);
    }
...
function rapeImpWithDick(): void {
    ...
    if (player.cocks.length == 1) {
        ...
        if (player.balls == 0 && player.vaginas.length > 0) outputText("  The cum seems to boil out from inside you as your " + vaginaDescript(player, 0) + " soaks itself.  With delicious slowness you fire rope after rope of cum deep into the imp's rectum.", false);
        if (player.balls == 0 && player.vaginas.length == 0) outputText("  The cum seems to boil out from inside you, flowing up your " + cockDescript(game.player, x) + ".  With delicious slowness, you fire rope after rope of cum deep into the imp's rectum.", false);
        if (player.cumQ() >= 14 && player.cumQ() <= 30) outputText("  Your orgasm drags on and on, until your slick jism is dripping out around your " + cockDescript(game.player, x) + ".", false);
        if (player.cumQ() > 30 && player.cumQ() <= 100) outputText("  Your orgasm seems to last forever, jizz dripping out of the imp's asshole around your " + cockDescript(game.player, x) + " as you plunder him relentlessly.", false);
        if (player.cumQ() > 100) outputText("  Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last.  The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your " + cockDescript(game.player, x) + " with each thrust.", false);
    ...
    if (player.cocks.length >= 2) {
        ...
        outputText("\n\nThe tight confines of the imp's ass prove too much for you, and you feel your orgasm build.", false);
        if (player.balls > 0) outputText("The cum seems to boil in your balls, sending heat spreading through your " + cockDescript(game.player, x) + " as your muscles clench reflexively, propelling hot spurts of jism deep into the imp's rectum.  Your other equipment pulses and dripples steady streams of its own cum.", false);
        if (player.balls == 0 && player.vaginas.length > 0) outputText("The cum seems to boil out from inside you as your " + vaginaDescript(player, 0) + " soaks itself.  With delicious slowness you fire rope after rope of cum deep into the imp's rectum.  Your other equipment drizzles small streams of jizz in sympathy.", false);
        if (player.balls == 0 && player.vaginas.length == 0) outputText("The cum seems to boil out from inside you, flowing up your " + cockDescript(game.player, x) + ".  With delicious slowness, you fire rope after rope of cum deep into the imp's rectum.  Your other equipment drizzles small streams of jizz in sympathy.", false);
        if (player.cumQ() >= 14 && player.cumQ() <= 30) outputText("  Your orgasm drags on and on, until your slick jism is dripping out around your " + cockDescript(game.player, x) + ".", false);
        if (player.cumQ() > 30 && player.cumQ() <= 100) outputText("  Your orgasm seems to last forever, jizz dripping out of the imp's asshole around your " + cockDescript(game.player, x) + " as you plunder him relentlessly.", false);
        if (player.cumQ() > 100) outputText("  Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last.  The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your " + cockDescript(game.player, x) + " with each thrust.", false);
        outputText("\n\nSatisfied at last, you pull him off just as he reaches his own orgasm, splattering his hot demon-cum all over the ground.   You drop the imp hard and he passes out, dripping mixed fluids that seem to be absorbed by the dry earth as fast as they leak out.", false);
...
function rapeImpWithPussy(): void {
    ...
    if (player.vaginalCapacity() < monster.cocks.cockArea(0)) {
        ...
        outputText("\n\nYou lose track of time as you languidly pump against the imp's " + cockDescriptShort(monster, 0) + ".  At long last you feel your " + vaginaDescript(player, 0) + " ripple and quiver.  Your " + legs(player) + " give out as you lose your muscle control and collapse against the small demon.  You gasp as his " + cockDescriptShort(monster, 0) + " erupts against you, splattering your chest with hot demonic cum that rapidly soaks into your skin.  You giggle as you rise up from the exhausted imp, feeling totally satisfied.", false);
    }
    // Big enough!
    else {
        outputText("  You sink down his " + cockDescriptShort(monster, 0) + " slowly, delighting in the gradual penetration and the tingling feeling of his dripping hot pre-cum.  At last you bottom out on his balls.", false);
        cuntChange(player, monster.cocks.cockArea(0), true);
        outputText("  Your lust and desire spurs you into movement, driving you to bounce yourself up and down on the " + cockDescriptShort(monster, 0) + ".  His exquisite member pushes you to the very height of pleasure, your " + vaginaDescript(player, 0) + " clenching tightly of its own accord each time you bottom out.  The tensing of the little demon's hips is the only warning you get before he cums inside you, hot demonic jizz pouring into your womb.  Your " + legs(player) + " give out, pushing him deeper as he finishes filling you.", false);
        outputText("\n\nThe two of you lay there a moment while you recover, at last separating as you rise up off his " + cockDescriptShort(monster, 0) + ".  Spunk drips down your legs, quickly wicking into your skin and disappearing.", false);
...
function sprocketImp(): void {
    ...
    outputText("and you find yourself dragged upward with him, soaring over the tops of the trees.  The cool rush of air does nothing to abate your arousal.  If anything, the cold shock only makes your body more aware of its own need.  After just a few seconds that feel like an eternity to your lust-filled being, the imp hurls you down into a tree.  You flail as you fall, barely catching yourself on the upper branches.  Your hands and " + legs(player) + " are tangled in the smooth wooden spiderweb below you, your mind torn between desire for the imp above and fear of the fall below.  You can see from the gleam in the horned creature's red eyes that he has you right where he wants you.\n\n", false);
    // New PG
    ...
    // Variable cocktext
    if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType == CockTypesEnum.DEMON || player.cocks[0].cockType > 4) outputText("your " + cockDescript(game.player, 0) + ", rubbing the tip of his prick against your own, ", false);
    else if (player.hasKnot(0)) outputText("your " + cockDescript(game.player, 0) + ", rubbing the tip of his prick against your point, ", false);
    else if (player.cocks[0].cockType == CockTypesEnum.HORSE) outputText("your " + cockDescript(game.player, 0) + ", rubbing the tip of his prick against your flared head, ", false);
    ...
    if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType == CockTypesEnum.DEMON || player.cocks[0].cockType > 4) outputText("shoves his shaft deeply into the slit in the head of your dick.  ", false);
    ...
    if (player.breastRows.length > 0 && player.breastRows.biggestTitSize() > 2) outputText(", fucking you so hard that your " + allBreastsDescript(player) + " bounce with each thrust.  ", false);
    ...
    if (player.cocks.length > 2) outputText("The rest of your " + multiCockDescriptLight(game.player) + " twitch and release their seed at the same time, creating a shower of spunk that rains down on both you and the imp, coating both of your bodies.  ", false);
    if (player.breastRows.biggestLactation() >= 1) outputText("At the same time, milk bursts from your " + nippleDescription(player, 0) + "s, splattering him in the face.  You feel a sick sort of triumph as you get him back for cumming inside you.  ", false);
    // Vagoooz
...
function centaurGirlOnImps(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        outputText("Your unexpected running momentarily paused the cocks inside you as their owners groped for holds on your " + hipDescription(player) + " and " + buttDescription(player) + ". With their positions relatively well established, they begin pounding at you again, causing you to nearly stumble in pleasure.\n\n", false);
        ...
        // [Has breasts:
        if (player.breastRows.biggestTitSize() > 1) outputText("  Your hands reflexively grab your " + chestDesc(game.player) + " and mash them about.", false);
        outputText("\n\n", false);

        outputText("The owner of the dog-cock in your " + vaginaDescript(player) + " manages to insert his knot as his balls empty inside you, but the cat-cock's body has no such luck and his grip on you falters. He slides out of your " + assholeDescript(player) + " but manages to grasp the fur of your back and straddle you, all while his cock continues to spray you down with jism.\n\n", false);

        // [Has breasts:
        if (player.breastRows.biggestTitSize() > 1) {
            outputText("He slides up to your torso and grasps your wildly flailing " + allBreastsDescript(player) + ", massaging them harshly. His ministrations are surprisingly crude, and you wonder how many times he's attempted to pleasure a woman.", false);
            // [Has fuckable nipples:
            if (player.breastRows.hasFuckableNipples()) outputText("  His fingers slide inside your " + nippleDescription(player, 0) + "s and start spreading and squishing them. Your femcum leaks out over his hands and soon your front is slick and shiny.", false);
            // All other nipples:
            ...
            if (player.breastRows.biggestLactation() >= 1 && player.lactationQ() < 50) outputText("  Milk dribbles and squirts from you as his desperate squishing continues, forming small puddles on the ground.", false);
            else if (player.breastRows.biggestLactation() >= 1) outputText("  Milk sprays from you as his desperate squishing continues, creating massive puddles of milk that you splash through as you continue moving.", false);
            outputText("\n\n", false);
        }
...
function centaurOnImpMale(vape: boolean = false): void {
    ...
    outputText("As your shadow falls over the imp, it looks between your " + legs(player) + " with a hint of fear.  ", false);
    if (player.cocks.cockArea(x) <= 15) {
        outputText("Relief washes over it followed by intense lust as is throws itself onto a mossy rock and eagerly presents its " + assholeDescript(monster) + ".   The sound of your hooves moving on either side of its body seems to send the creature into a frenzy as it begins humping the air while small mewling sounds escape its lips.  ", false);
        // <<Cor <50>>
        if (player.cor < 50) outputText("You slowly rub your " + cockDescript(game.player, x) + " between the creature's cheeks, letting your pre-cum oil the small hole, before slowly beginning the insertion.  Before you can get half-way the creatures drives its self back against you, impaling its " + assholeDescript(monster) + " around your " + cockDescript(game.player, x) + " and making inhuman sounds of ecstasy. The " + assholeDescript(monster) + " relaxes around your " + cockDescript(game.player, x) + ", taking it all in while its practiced muscles grip and jerk you off internally.\n\n", false);
        // <<Cor 50+>>
        else outputText("You position your " + cockDescript(game.player, x) + " against its dry anus and drive yourself inside of it using your powerful equine legs.  The creatures gives a loud shriek as its insides are forced open, and you feel the raw tightness trying to resist your intrusion.  Giving the creature no chance to relax you begin pistoning into it, grinning as the sounds of pain give way to grunts and yelps of pleasure. You cannot last long in the creature's hole, and soon spurts of cum begin shooting out and filling its bowels.\n\n", false);
        // <<GoTo I1>>
        ...
        // <<Cock: large, Cor <50>>
        if (player.cor < 50) {
            outputText("The imp's eyes widen and you see its apprehension as it attempts to turn and flee.  You make soothing sounds as you approach the skittish creature, while easily keeping pace with it.  Seeing little chance for escape, the creature turns toward you again and carefully begins making its way between your " + legs(player) + ", eyes wide in supplication.  Your smile seems to relax it, and lust fills its eyes again as it slowly starts massaging your " + cockDescript(game.player, x) + ".  Getting more and more confident, the creature is soon using both of its hands on your " + cockDescript(game.player, x) + ", and its wet and serpentine tongue is moving all over the length of your erection.  There is little chance of your " + cockDescript(game.player, x) + " fitting into its small mouth, but it does its best to pleasure you as it goes more and more wild.  ", false);
            // <<Thick large>>
            ...
            outputText("You cannot take the attention for long before your hooves are scraping at the ground and jets of sperm shoot out of your " + cockDescript(game.player, x) + " and down its waiting throat.\n\n", false);
            // <<GoTo I2>>
    ...
    if (player.cocks.length > 1 && player.cocks[player.cocks.biggestCockIndex()].cockLength >= 24) {
        outputText("As your shadow falls over it, it looks with a hint of fear between your legs, and then its eyes widen in a mixture of apprehension and lust.  Before you can even more the little creatures scrambles forward between your hooves and wraps its hands around your " + cockDescript(player, player.cocks.biggestCockIndex()) + ".  Its tongue begins to trail all along the length of it as its small hands stroke it intensely.\n\n", false);
        // << Cor <50>>
        if (player.cor < 50) {
            outputText("You slowly undulate your " + cockDescript(player, player.cocks.biggestCockIndex()) + " against the creature's mouth, delighting in its eager tongue.  ", false);
            // <<GoTo I3 then return>>
            centaurOnImpResults(3);
            outputText("The sounds beneath you quickly take on a more intense note and you feel massive amounts of cum splashing liberally over your hooves, belly, and " + cockDescript(player, player.cocks.biggestCockIndex()) + ".  The hot sensation sends you over the edge as you begin spilling yourself into the creature's eager mouth.\n\n", false);
            // <<GoTo I2>>
            centaurOnImpResults(2);
            // <<End>>
            player.orgasm();
        ...
        // << 1 or 2 cocks, Cor 50+>>
            ...
            outputText("With an evil smile you wait for your " + cockDescript(player, player.cocks.smallestCockIndex()) + " to be at its lips before you slide it forward into its waiting mouth.  Giving it little more than a moment to catch its breath you slide your " + cockDescript(player, player.cocks.smallestCockIndex()) + " further and down the creature's throat.  Though you cannot see the obscene bulge it is making in the creature's mouth-pussy you delight in the intense tightness beneath you.  The throat muscles are massaging your " + cockDescript(player, player.cocks.smallestCockIndex()) + " as the imp desperately scrambles for air, pulling at the tentacles you have forced into it.  It cannot even begin to close its jaw as you thrust deeper and deeper, as you try to intensify the sensations.\n\n", false);
            outputText("As the imp is focused on the tentacles cutting off its air, you position your " + cockDescript(player, player.cocks.biggestCockIndex()) + " against its " + assholeDescript(monster) + ".  Pausing only for a second for the pleasure of anticipation, you shove yourself deep inside the imp's " + assholeDescript(monster) + ", only making it a few inches before having to pull back and try again.  The creature's throat seems to be working overtime now as it tries to divide its attention between the two invaders.  Each thrust of your " + cockDescript(player, player.cocks.smallestCockIndex()) + " makes it a little bit deeper inside of the creature, and you wonder passionately if you can get the two to meet in the middle.\n\n", false);
            outputText("It is not long before you begin to feel the creature's struggles slowing down.  ", false);
            // <<Cor <80 >>
            if (player.cor < 80) {
                outputText("Feeling merciful you extract yourself from the creature, flipping it unto a nearby rock as it begins to regain consciousness.  Before it realizes what you are doing your " + cockDescript(player, player.cocks.biggestCockIndex()) + " is prodding at its " + assholeDescript(monster) + ", then sliding quickly between its cheeks.  The amount of slobber over you is more than enough lubricant.  You groan in pleasure as it gives a slight squeal, then proceed to finish yourself off in the once-tight orifice.\n\n", false);
                // <<Goto I1>>
                centaurOnImpResults(1);
                player.orgasm();
                cleanupAfterCombat();
            ...
            // <<Cor 80+>>
                ...
                outputText("You groan in pleasure and slide your " + cockDescript(player, player.cocks.biggestCockIndex()) + " even deeper down the creature's throat, until you can feel its head against your ", false);
                // <<if balls>>
                if (player.balls > 0) outputText(ballsDescriptLight(player) + ".\n\n", false);
                ...
                if (player.breastRows.hasFuckableNipples()) outputText("Shoving your fingers deep into your " + nippleDescription(player, 0) + "s", false);
                else outputText("With a fierce tug on your " + nippleDescription(player, 0) + "s", false);
                outputText("you begin to cum deep and directly into the imp's stomach and " + assholeDescript(monster) + ".  ", false);
                // <<cum multiplier: lots>>
                ...
                outputText("With on last grunt you begin extracting the tentacles back out, almost cumming again from the tightness around them.  You give your " + cockDescript(player, player.cocks.smallestCockIndex()) + " one last shake over the creature's face before trotting away satisfied and already thinking about the next creature you might abuse.", false);
                player.orgasm();
                cleanupAfterCombat();
                return;
            }
        ...
        // << 3+ cocks, Cor 80+>>
            ...
            outputText("With an evil smile you wait for the creature's mouth to touch one of your tentacles before the other two snake their way down and wrap themselves around the imp's thighs.  With a tug the creatures is pulled off of it's feet and upside down, its eyes widening in a mixture of fear and debased lust as it sees your " + cockDescript(player, player.cocks.biggestCockIndex()) + " undulating in front of it.  You slowly move the tentacle up as your other cocks forcefully tug its legs apart, and then playfully begin sliding yourself over the imp's small cheeks.\n\n", false);
            ...
            outputText("Licking your lips in anticipation you begin pushing your " + cockDescript(player, player.cocks.biggestCockIndex()) + " into the imp's " + assholeDescript(monster) + " while listening to the mewling sounds coming from beneath you.  You take your time as you push in, seeing no need to rush yourself as you feel the creature gaping more and more.  Once you bottom out you reach down and grab the creature's arms, securing it firmly against your belly as you break into a trot.  The sensation of the imp's " + assholeDescript(monster) + " bouncing around your " + cockDescript(player, player.cocks.biggestCockIndex()) + " is intense and you ride harder until you know you are close to the bring.  Quickly you slow down and drape the creature over a nearby boulder, using your hands and tentacles to pin it to the harsh surface, and then your mighty legs push you forward even deeper into the creature's bowels.  The shriek should be audible pretty far in this area, and you groan in debased pleasure thinking it might draw someone else for you to rape or be raped by.  Grunting slightly you begin pushing into the imp even harder just to generate more loud sex-noise.  ", false);
            // <<Breasts>>
            if (player.breastRows.biggestTitSize() >= 0) {
                outputText("One of your hands releases it and begins playing with your " + allBreastsDescript(player), false);
                // <<nips have pussies>>
                if (player.breastRows.hasFuckableNipples()) outputText(" and fingering your " + nippleDescription(player, 0) + "s", false);
...
function centaurOnImpFemale(vape: boolean = false): void {
    ...
    player.slimeFeed();
    player.knockUp(PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP);
    outputText("As the imp lays beaten its hands stroke its " + cockDescriptShort(monster, 0) + " as its eyes look over you in the hope that you might abuse it in some manner.  You lick your lips as you stare at the large member and you turn around to display your " + vaginaDescript(player, 0) + ".  ", false);
    // Not gaping?
    if (player.vaginas[0].vaginalLooseness <= VaginaLooseness.GAPING) {
        // Penetration for non-gape cases
        outputText("With a lascivious grin the imp hops forward, gripping your flanks as it drives its member forward into your " + vaginaDescript(player, 0) + ".  ", false);
        // <<If Pussy Virgin>>
        ...
        // Not virgin fucking flavors
            ...
            if (player.vaginalCapacity() < monster.cocks.cockArea(0)) outputText("It groans in delight at your incredible tightness and shoves itself forward even harder.  ", false);
            // [Increase size as needed]
            // <<At Dicksize>>
            if (player.vaginalCapacity() >= monster.cocks.cockArea(0) && player.vaginalCapacity() <= monster.cocks.cockArea(0) * 1.25) outputText("It makes a pleased sound as it slides deeply into your " + vaginaDescript(player, 0) + ".  ", false);
            // <<Bigger than dicksize>>
            if (player.vaginalCapacity() >= monster.cocks.cockArea(0) * 1.25) outputText("Its dick slides easily and slopping noises start sounding from your backside.  Part of you wishes that its large member was larger still, as your mind drifts to some of the monstrous cocks that have penetrated you in the past.  ", false);
        }
        // Ride around with him till he cums and falls off
        outputText("When the creature completely bottoms out inside of you, you begin trotting forward with a wicked grin.  The creature's hands grasp your flanks desperately, and its " + cockDescriptShort(monster, 0) + " bounces inside your " + vaginaDescript(player, 0) + ", adding to your sensation.  The movement is causing the imp to push himself even harder against you as it tries to not fall off, and it is all you can do to keep an eye on where you are going.  Soon you can feel the imp's sperm filling your " + vaginaDescript(player, 0) + " and overflowing even as your cunt-muscles try to milk it of all of its seed. Unsatisfied you begin to speed up as you use its " + cockDescriptShort(monster, 0) + " to bring about your own orgasm.  The small creature is unable to let go without hurting itself.  It hangs on desperately while you increase the pace and begin making short jumps to force it deeper into you.  The feeling of sperm dripping out and over your " + clitDescription(player) + " pushes you over and cry out in intense pleasure.  When you finally slow down and clear your head the imp is nowhere to be seen.  Trotting back along the trail of sperm you left behind you find only its small satchel.", false);
        cuntChange(player, monster.cocks.cockArea(0), true, true, false);
        player.orgasm();
        cleanupAfterCombat();
        return;
    ...
    // <<Gaping>>
        ...
        // <<Cor 50+, Breasts>>
        if (player.cor >= 50 && player.breastRows.biggestTitSize() >= 2) {
            outputText("You release the tree as you begin playing with your " + allBreastsDescript(player), false);
            // <<w/ nip-pussies>>
            if (player.breastRows.hasFuckableNipples()) outputText(" and shoving your fingers into your " + nippleDescription(player, 0) + ".  ", false);
            else outputText(".  ", false);
            // <</Breasts>>
        }
        outputText("The imp is pushing deeper and deeper and in moments you cry out again as you feel first its hooves and then its " + cockDescriptShort(monster, 0) + " tearing open your cervix and bottoming out in your womb.  ", false);
        // <<Asshole large+>>
        if (player.analCapacity() >= 35) {
            outputText("When the imp realizes it cannot go any further you feel its hands against your asshole, and your eyes go wide in realization of what it is planning on doing.  Lubed up by your now drooling juices, the fist pushes hard into your " + assholeDescript(player) + ", shoving past your ring-muscles.  ", false);
...
function centaurOnImpResults(iNum: number): void {
    ...
    if (iNum == 1) {
        ...
        if (player.cumQ() >= 250) {
            // <<no knot>>
            if (player.cocks[x].cockType != CockTypesEnum.DOG) outputText("Soon the amount is overflowing from the abused " + assholeDescript(monster) + ", dripping between you with no sign of stopping as you continue thrusting yourself into the imp.  ", false);
            // <<knot>>
            else outputText("Soon the abused " + assholeDescript(monster) + " is full to the brim, though your knot keeps any from escaping while more and more pumps in.  Soon the creature's belly is distending and the imp is gasping wordlessly. ", false);
            outputText("When your " + cockDescript(game.player, x) + " finally emerges a torrent of cum follows out of the distended hole and covering the back of the creature's legs.  ", false);
            // <<I1_1>>
            // <<2 cocks>>
            if (player.cocks.length == 2) outputText("Your other cock drenches the imp's back with its own secretions that immediately start dripping down its sides.  ", false);
            // <<3+ cocks>>
            if (player.cocks.length > 2) outputText("Your other cocks release their cum all over the creature's back and sides, leaving it a glazed mess.  ", false);
            // <</I1_1>>
            outputText("You leave him panting and lapping at a pool of your semen.", false);
        }// <</multiplier>>
        // <<cum multiplier: little-normal>>
        else {
            outputText("With a last thrust into the cum receptacle you begin slowing down, even as its own " + cockDescriptShort(monster, 0) + " spills its seed over the ground.  ", false);
            // <<I1_1>>
            // <<2 cocks>>
            if (player.cocks.length == 2) outputText("Your other cock drenches the imp's back with its own secretions that immediately start dripping down its sides.  ", false);
            // <<3+ cocks>>
    ...
    if (iNum == 3) {
        ...
        if (player.breastRows.biggestTitSize() >= 2) {
            ...
            if (player.breastRows.hasFuckableNipples()) {
                ...
                if (player.breastRows.biggestLactation() >= 1) outputText("Milk streams out from your " + nippleDescription(player, 0) + "s as if they had been recently filled with dripping cum.  ", false);
                else outputText("Your fingers slide faster and faster into your " + nippleDescription(player, 0) + "s even as the imp begins to stroke itself under you.  ", false);
            }
            // No pussies
            else {
...
function areImpsLactoseIntolerant(): void {
    outputText("", true);
    outputText("You advance on the masturbating imp, baring your " + allBreastsDescript(player) + " and swinging them from side to side. The little creature watches them, mesmerized as he masturbates his foot-long erection.\n\n", false);

    outputText("You sit down in front of the little creature and grab ahold of his hair. The imp squeals slightly in pain before his cries are silenced with a " + nippleDescription(player, 0) + ".  It fills his mouth as he yields, defeated. At once he starts to drink down as much of your milk as he can.\n\n", false);

    outputText("After a moment, he takes one of his hands off his large member and puts it against your " + biggestBreastSizeDescript(player) + " to steady himself as he continues to nurse. You give a pleased sigh and simply bask in the sensations of pleasure that being nursed gives you.  You ruffle the little imp's hair affectionately. \"<i>These creatures are so much nicer to be around when they just take their minds off their cocks,</i>\" you think as you see his other hand relax and stop rubbing his swollen, demonic member.\n\n", false);

    outputText("You feel the imp's mighty gulps start to slow down until he lets out a sigh of relief. While imps may be small, they're very hungry creatures. Your " + nippleDescription(player, 0) + " slips out of the imp's mouth, and you gently lay it down on the ground. It gives a few gentle burps before dozing off; you can see that the imp's erection has retracted, and its belly has expanded significantly. You smile to yourself and, feeling fully satisfied, you stand up.", false);
...
export function impGangabangaEXPLOSIONS(): void {
    player.slimeFeed();
    spriteSelect(18);
    ...
    // CENTAUR
    if (player.lowerBody == LowerBodyType.CENTAUR) {
        if (rand(2) == 0 && (player.cocks.length == 0 || player.gender == 3)) {
            // (First encounter)
            if (player.effects.findByType(StatusAffects.ImpGangBang) < 0) {
                outputText("The imps stand anywhere from two to four feet tall, with scrawny builds and tiny demonic wings. Their red and orange skin is dirty, and their dark hair looks greasy. Some are naked, but most are dressed in ragged loincloths that do little to hide their groins. They all have a " + cockDescriptShort(monster, 0) + " as long and thick as a man's arm, far oversized for their bodies. Watching an imp trip over its " + cockDescriptShort(monster, 0) + " would be funny, if you weren't surrounded by a horde of leering imps closing in from all sides...\n\n", false);
                player.effects.create(StatusAffects.ImpGangBang, 0, 0, 0, 0);
                ...
                outputText(".  The " + monster.short + " surges forward and grapples you. Imps grope your body and hump their " + cockDescriptShort(monster, 0) + " against your horse legs, smearing their sweat and pre-cum into your " + player.skinDesc + ". The rest of the " + monster.short + ", a dozen or more imps, all leer at you and laugh as they slap and pinch your body. The imps have sharp claws, tiny sharp teeth, and short horns on their heads. They scratch, claw, and bite at you with all of these weapons as they try to pull you down to the ground. One bold imp leaps forward and grabs your ", false);
                // (If the player has a cock)"
                if (player.cocks.length > 0) outputText(cockDescript(game.player, 0), false);
                // (If the player has breasts)
                else outputText(nippleDescription(player, 0), false);
                outputText(", twisting and pinching hard enough to make you yelp in pain. An imp leaps up and mounts you, grabbing your " + hairDescription(player) + " like reins. The long flesh of his " + cockDescriptShort(monster, 0) + " rubs against the small of your back. The " + monster.short + " stinks of sweat and pre-cum, its moist grip and obscene smirk leaves you with no doubt as to what they will do to you if you lose this fight.\n\n", false);
            }
            outputText("The horde drags you to your knees, grappling your legs and crawling over your horse-body to pin you down. You try to buck them off but there are too many to fight. The imps drag your arms behind your back, wrapping them around your rider. Another imp whips off his loincloth to reveal his pre-cum drooling " + cockDescriptShort(monster, 0) + " and tosses the cloth to the imps holding your arms. They quickly tie your arms back with the sweat-damp loincloth.  ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 1) outputText("Having your arms tied behind your back forces your chest out, making your " + allBreastsDescript(player) + " stand out. They bounce as you struggle.  ", false);
            outputText("The " + monster.short + " stroke themselves and rub their hands over your outstretched chest, smearing their pre-cum into your skin. The imp riding you bounces up and down, rubbing his sweaty " + ballsDescriptLight(monster) + " against your " + player.skinDesc + " while he yanks your hair.  ", false);
            // (Low Corruption)
            if (player.cor < 50) outputText("Your face flushes with humiliation. Your imp rider twists your " + hairDescription(player) + " hard and you whimper in pain. Imps rub their cocks along your " + hipDescription(player) + " while others stroke themselves and jeer at your helplessness.  ", false);
            // (High Corruption)
            ...
            outputText("You yelp in shock as you feel a sharp slap on your ass. You look back to see an imp pulling your tail up. He grins at you and slaps your " + hipDescription(player) + " again. He yanks your tail and slaps your ass one last time, then dives down to plant his face in your " + vaginaDescript(player, 0) + ". His inhumanly nimble tongue teases the folds of your pussy and flicks at your " + clitDescription(player) + ".  ", false);
            // (If the player has balls)
            if (player.balls > 0) outputText("The tongue slides over your " + sackDescript(player) + ", coating it with warm drool.  ", false);
            // (Low Corruption)
            if (player.cor < 50) outputText("You shake your hips, trying to escape the demonic tongue. The imp grips your " + hipDescription(player) + " and pulls his face further into your cunt, sliding his nimble tongue over your lips. You grit your teeth, trying to ignore the warmth spreading from your " + vaginaDescript(player, 0) + ".", false);
            // (High Corruption)
            else outputText("You let out a shuddering sigh as the heat from your cunt spreads into the rest of your body. Your " + hipDescription(player) + " tremble as the tongue slides over the folds of your " + vaginaDescript(player, 0) + ". The imp grips your flanks harder and dives his nimble tongue into your fuck-hole.", false);
            outputText("\n\n", false);

            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 1) {
                outputText("Hands slide over your " + allBreastsDescript(player) + ", dragging your attention back to the front of the mob. Two imps grope your " + biggestBreastSizeDescript(player) + ", mauling your flesh as they drag your tits around your chest. They lick your tit-flesh, slowly working their way up towards your " + nippleDescription(player, 0) + ". The imp rider drops your hair and reaches around you, shoving his cock against your back as he squeezes your " + biggestBreastSizeDescript(player) + ". Finally the imps reach your nipples, their tongues wrapping around and pulling at the tingling flesh.  ", false);
                // (Low Corruption)
                if (player.cor < 50) outputText("You can't escape the tongues lapping and pulling at your " + nippleDescription(player, 0) + ", matching the one in your cunt. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.", false);
                // (High Corruption)
                else outputText("The tongues squeezing and tugging your nipples match the tongue working your " + vaginaDescript(player, 0) + ", flooding your body with lust. You moan and arch your back, offering your tits to the imps. You can hear your pulse pounding in your ears as you pant with desire.", false);
                outputText("  Suddenly you feel tiny needle-sharp teeth pierce your " + nippleDescription(player, 0) + ". You scream as venom pumps into your tits, red-hot poison that makes your " + allBreastsDescript(player) + " feel as though they were being stung by bees. You moan in pain as your breasts start to swell, the imps continuing to pump demon-taint into them.\n\n", false);
                // Grow tits!
                growTits(player, 2, player.breastRows.length, false, 1);
                player.boostLactation(.3);
            }
            outputText("Dimly through your haze of lust and pain you see a large imp step forward from the mob. Four feet tall and broader and stronger looking than any imp you've seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a " + cockNoun(CockTypesEnum.HORSE) + " big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your face. You shudder as the hot fluid stings the sensitive skin of your lips. His " + ballsDescriptLight(monster) + " are each the size of your fist and slick with sweat. He slaps his sweaty cock-head against your cheek, nearly scalding you with the heat.  ", false);
            // (Low corruption)
            if (player.cor < 50) outputText("You yelp and twist your head to escape the heat.  ", false);
            // (End low corruption)
            outputText("He slowly rubs his shaft over your cheeks and along your lips, each ridge of his demonically-hot " + cockNoun(CockTypesEnum.HORSE) + " tugging at your lips. The hot pre-cum dribbles over your sensitive flesh and the musk makes your sinuses tingle. The big imp sneers as you whimper, and whips his bull-shaft back to slap your face. The other imps watch and stroke themselves as their master cock-whips you.\n\n", false);

            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("The big imp grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your " + allBreastsDescript(player) + " swell further at his touch. ", false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("The other imps reclaim your aching breasts, sucking your " + nippleDescription(player, 0) + " and mauling your " + allBreastsDescript(player) + " so hard their fingers disappear into your swelling flesh. ", false);
            outputText("The imp rubs his hands over your sides and flanks, his " + cockNoun(CockTypesEnum.HORSE) + " bobbing as he walks. The other imps watch their master as he moves around you. Only the imp sucking your " + vaginaDescript(player, 0) + " doesn't notice, his tongue thrusting deeply into your folds. The big imp grabs him by the neck and easily tosses him aside, his tongue dragging through your cunt as he's pulled away from you. The master imp takes position behind you and grabs his " + cockNoun(CockTypesEnum.HORSE) + ", bringing the mushroom-head of it down to your pussy. You shake, knowing what's coming next. The other imps watch and stroke themselves as their master readies his hips to push into you.\n\n", false);
            // (Low corruption)
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 1) outputText("The imps working your breasts suck harder, kneading your tit-flesh as though trying to milk you. ", false);
            outputText("You squirm and twist against the imps holding you down as the hot " + cockNoun(CockTypesEnum.HORSE) + " almost burns your sensitive cunt. You can smell the sweat steaming off his shaft, and your pussy-fluids start to steam as well as he forces his cock-head into your " + vaginaDescript(player, 0) + ". His huge cock-head bulges your groin, and you moan", false);
            // (Low corruption)
            if (player.cor < 50) outputText(" in helpless terror as you feel the bulge work up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous " + cockNoun(CockTypesEnum.HORSE) + " stretches your belly", false);
            // (High corruption)
            else outputText(", panting in lust as the monstrous " + cockNoun(CockTypesEnum.HORSE) + " pushes your flesh aside to make room for itself", false);
            outputText(". ", false);
            // (This is a good place for the virginity-loss message, if needed)
            cuntChange(player, monster.cocks.cockArea(1), true);
            ...
            outputText(" in your stuffed " + vaginaDescript(player, 0) + ". The big imp sneers and flexes his cock again, watching ", false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 2) outputText("your " + allBreastsDescript(player) + " roll on your chest as you squirm", false);
            // (If the character doesn't have breasts)
            else outputText("your eyes roll back as you squirm", false);
            outputText(".\n\n", false);

            outputText("Finally the big imp pulls back his " + cockNoun(CockTypesEnum.HORSE) + ", each ridge pulling on your pussy flesh as he slides out. You yelp and buck as the mushroom-head catches on your folds. ", false);
            // (If the character has a cock)
            if (player.cocks.length > 0) outputText("Your " + multiCockDescriptLight(game.player) + " bounces as the bulge passes over it.  ", false);
            outputText("You moan as the mushroom-head reaches the entrance of your " + vaginaDescript(player, 0) + ", your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ", false);
            // (Low corruption)
            if (player.cor < 50) outputText("You try to buck your " + hipDescription(player) + ", fighting to break free as the bulge of his cock-head works its way high up into your belly. You're held down by too many imps. You can only writhe around the hot shaft stretching out your " + vaginaDescript(player, 0) + ". The big imp grunts as his cock-head pops past your cervix, and you moan and shake in pain.  ", false);
            // (High corruption)
            else outputText("You moan in ecstasy as the hot " + cockNoun(CockTypesEnum.HORSE) + " pushes deep into your " + vaginaDescript(player, 0) + ", turning every inch of your pussy into a pleasure-sheath for the big imp. You know you're nothing but a fuck-toy for this corrupt creature, just a wet pussy for him to fill with cum, and the thought almost makes you orgasm as he forces his huge cock-head past your cervix.  ", false);
            outputText("Finally the corrupt cock bottoms out against your womb. The imp pulls back again, and starts to fuck you slowly.\n\n", false);

            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 2) outputText("The slow fucking shakes your breasts, and the imps sucking at your nipples cling tightly to your monstrously swollen " + allBreastsDescript(player) + ". Your " + biggestBreastSizeDescript(player) + " have grown three cup sizes since the imps pumped their venom into you. An ache starts deep in the base of your tits and works its way to your sore " + nippleDescription(player, 0) + ". Your already bloated nipples swell as the imps suckle and you gasp as the first rush of milk spills into their mouths. Your rider reaches around and starts to milk your udders, moving his hands between your " + allBreastsDescript(player) + " and forcing out more milk for his gangmates.\n\n", false);

            outputText("The big imp grinds his hips as he thrusts and pulls, rubbing his cock-ridges against every part of your " + vaginaDescript(player, 0) + ". While sliding his mutated " + cockNoun(CockTypesEnum.HORSE) + " in and out of you, the imp rubs his hands along your mound, pulling it open or forcing it tight as he takes you. Your pussy juices steam off his cock as he pumps, and hot pre-cum dribbles down your crack and ", false);
            // (If the character has a cock)
            if (player.cocks.length > 0) outputText("over your " + multiCockDescriptLight(game.player) + " where it ", false);
            outputText("drips onto the ground. ", false);
            // (Low corruption)
            if (player.cor < 50) outputText("The pain as this huge cock stretches you is overwhelming, but every thrust rubs more corrupt pre-cum into your pussy walls. You start to pant as the imp rapes you, using your body for his own pleasure. You tremble as the heat of his pre-cum soaks through your body. The huge shaft forces your " + clitDescription(player) + " out, and the steaming fluids splashing on it make it tingle almost painfully. Your whimpers and moans of pain start to take on a different tone, and the master imp starts to fuck you faster.", false);
            // (High corruption)
            else outputText("Pain and pleasure blend into one as the huge " + cockNoun(CockTypesEnum.HORSE) + " stretches you, rubbing pre-cum into your steaming pussy. You moan as the big imp fucks you, turning you into a mindless fuck-puppet. Your " + clitDescription(player) + " swells painfully as hot juices splash over it. Your shaking body only adds to the master imp's pleasure.", false);
            outputText("\n\n", false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 1) outputText("Imps gulp milk from your bloated " + biggestBreastSizeDescript(player) + ". As one imp drinks his fill and staggers away with a sloshing belly, another steps up to pump your milk-spewing udders.  ", false);
            // (If the character has a dick)
            if (player.cocks.length > 0) outputText("Your " + multiCockDescriptLight(game.player) + " swell painfully as the rough fucking pumps blood into your groin.  ", false);
            outputText("The big imp's snake tongue flicks out and slides around your " + vaginaDescript(player, 0) + ", pulling at your pussy lips. He moves his tongue back and forth along the sides of your steaming cunt, alternating between stretching and flicking the lips. ", false);
            // (If the character has a dick)
            if (player.cocks.length > 0) outputText("He draws his tongue back and wraps it around your " + cockDescript(game.player, 0) + ", sliding its length along your shaft and flicking his tongue over your cock-head.  ", false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 3) outputText("Your udders shake back and forth under your chest in time to the rough fucking. You arch your back to press your " + nippleDescription(player, 0) + " into eager mouths, moaning as your rider milks your distended " + allBreastsDescript(player) + ". ", false);
            // (Low Corruption).
            ...
            outputText("The master imp pounds into you as hard as he can, driving his " + cockDescriptShort(monster, 1) + " deeper into your cunt. His grunts come closer and closer together. Your rider grinds his cock into your back, rubbing his cock-head in your hair. He nips at your neck and shoulder as he pants. The master imp pounds into you and you can feel his " + ballsDescriptLight(monster) + " swell as they slap against you. Through the haze of your approaching orgasm you realize what's about to happen. Those oversized balls are about to pump more cum into you than any normal man could ever produce. They're going to pump demonic cum right into your womb. ", false);
            // (Low Corruption)
            if (player.cor < 50) outputText("You scream as the base of his " + cockNoun(CockTypesEnum.HORSE) + " bloats with corrupted jism, the thick bulge stretching your pussy even more as it pumps along the imp's shaft. The bulge swells your belly and you can feel it move through your stretched cunt towards your womb. Another thick bulge forms at the base of the master imp's cock and you thrash wildly, yelling in protest. \"<i>NOO - O - O - OOOOHhh!</i>\" The hot cum floods into your womb and you reach your own orgasm, shaking as your " + vaginaDescript(player, 0) + " clamps down on his cock and milks it of waves of cum. Another orgasm hits on the heels of the first one, and you buck as more demon-cum floods your womb. Gasping for air, you continue to come as your belly swells. Even as he pumps more corrupt cum into you the big imp keeps raping you, forcing you to another peak before you've come down from the last one.", false);
            // (High corruption)
            else outputText("The thought of all that demon-jism in your womb pushes you over the edge. You cum hard, bucking your hips against the " + cockNoun(CockTypesEnum.HORSE) + " pumping hot cum into your belly. Your eyes roll back in your head and you scream out in ecstasy as thick jets of cum fill your pussy. The imp keeps thrusting into his fuck-toy even as he fills your womb with his cum, forcing you to another peak before you've come down from the last one. The big imp is your master now.", false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("As orgasms wrack your body your breasts pump out even more milk, too much for the imps below to handle. Milk pours down your chest in great streams, soaking the imps and splashing onto the ground below you. The milk gushing through your tender " + nippleDescription(player, 0) + " pushes you to another orgasm. You shake your tits as you cum, mooing in mindless pleasure, spraying jets of milk everywhere. Your rider cums, soaking your " + hairDescription(player) + " with jets of imp-jism that run down your scalp and over your cheeks. ", false);
            // (High corruption)
            if (player.cor >= 50) outputText("You lap eagerly at the salty cum, licking up and drinking as much as you can.", false);
            outputText("\n\n", false);
            outputText("Imp-jism rains down on your helpless spasming body. The imps spew cum into your hair, across your back and " + hipDescription(player) + ", over your face", false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText(", and bouncing " + allBreastsDescript(player), false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("milk spurts from your " + nippleDescription(player, 0) + " and ", false);
        ...
        // Scene number 2 - male possible.
            ...
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("Your unbound " + biggestBreastSizeDescript(player) + " bounce out over the weathered log. ", false);
            outputText("The imps spread your arms wide, forcing your chest out, and tie them to the log with sweaty loincloths. Your " + hipDescription(player) + " are stuck high in the air. Imps rub their sweaty cocks and " + ballsDescriptLight(monster) + " over your legs and grope your crotch. The two imps riding your back start stroking and licking each other. ", false);
            // (Low Corruption)
            ...
            outputText("An imp quickly climbs up your body, planting his feet on your shoulders and grabbing your " + hairDescription(player) + " with one hand for support. He rubs his " + ballsDescriptLight(monster) + " over your mouth, smearing your lips with musky sweat, while he pries at your jaw with his other hand. ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("An imp mounts the log and slaps his " + cockDescriptShort(monster, 0) + " between your " + allBreastsDescript(player) + ", squeezing them tight over his cock as he rubs back and forth. He mauls your breasts cruelly, squeezing his fingers deep into your soft flesh.  ", false);
            // (If the player has a SINGLE cock)
            if (player.cocks.length == 1) outputText("An imp ducks under your body and grabs your " + cockDescript(game.player, 0) + ". His nimble tongue flicks over your cock-head while he pricks the shaft with his tiny claws.  ", false);
            // (If the player has a MULTI cock)
            if (player.cocks.length > 1) outputText("Two imps duck under your body and seize your " + multiCockDescriptLight(game.player) + ", licking the tips with their inhumanly flexible tongues while they stroke the shafts.  ", false);
            // (Low Corruption)
            ...
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("The imp fucking your " + biggestBreastSizeDescript(player) + " handles your soft flesh roughly, pressing and pulling your tits into a fuck-canal for his demon cock. Other imps slap your " + allBreastsDescript(player) + " and laugh as you cry out.  ", false);
            // (Low Corruption)
            if (player.cor < 50) outputText("You whimper as your mistreated flesh stings with dozens of pin-prick scratches and bites, and the " + monster.short + " slaps your chest and flanks. The abuse falls on you from all sides, leaving you with no escape. The imp on your shoulders pries your jaws open, and you gag on his " + ballsDescriptLight(monster) + ".", false);
            // (High Corruption)
            ...
            outputText("With a loud sucking sound, the imp pulls his balls out of your mouth. Spit and ball-sweat drip over your cheeks as he repositions himself, bending almost completely over on your shoulders to rub his cock-head against your lips. You nearly choke as pre-cum dribbles into your mouth and runs down the back of your throat. The " + cockDescriptShort(monster, 0) + " blocks most of your vision, but in the corners of your eyes you see the master of this imp horde step forward. Four feet tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has black fur, broad red demon wings, two long demon-horns on his head, and a " + cockNoun(CockTypesEnum.DOG) + " big enough to choke a minotaur. He leers at your helpless body and grabs ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("one of your sore " + biggestBreastSizeDescript(player) + " in his calloused hand, brutally pressing his fingers into your flesh", false);
            // (If the player doesn't have breasts)
            else outputText("your tail and yanks, brutally pulling on it", false);
            outputText(" until you shriek. The imp riding your shoulders plunges his " + cockDescriptShort(monster, 0) + " into your mouth, pounding at the top of your throat.\n\n", false);

            outputText("The master imp walks back to your hips, lightly dragging his sharp claws over your flanks. He kicks another imp out of the way and takes position behind your " + hipDescription(player) + ". He pulls his monstrously long " + cockNoun(CockTypesEnum.DOG) + " down and rubs the tip over your ", false);
            ...
            outputText("The big imp's hot pre-cum stings your flesh. The imps licking your crotch lap up the hot fluid, cooling you with their saliva. The big imp sneers as you whimper, and presses the head of his " + cockNoun(CockTypesEnum.DOG) + " against your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            ...
            // (High Corruption)
            else outputText("The scent of musk steaming off the" + cockNoun(CockTypesEnum.DOG) + " drives you wild, and you push back to try and capture the cock-tip.", false);
            outputText("\n\n", false);

            outputText("The pointed tip of the master imp's " + cockNoun(CockTypesEnum.DOG) + " plunges into your hole, splitting your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            ...
            outputText(".", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("  The imp fucking your aching " + biggestBreastSizeDescript(player) + " paints your tits with a massive load of cum. He falls off the log and another imp jumps up to take his place.", false);
            outputText("\n\n", false);

            outputText("The big imp fucks you roughly, clenching your " + hipDescription(player) + " in his clawed hands as he hammers his " + cockNoun(CockTypesEnum.DOG) + " into you. The head of his mutated shaft pounds ", false);
            // (If the player has a vagina)
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("The rough fucking shakes your cum-stained breasts, and the imp fucking your " + allBreastsDescript(player) + " clings tightly to your red and swollen tit flesh. Your " + biggestBreastSizeDescript(player) + " burn with agony as the " + monster.short + " slaps your tits like drums.  ", false);
            // (Low Corruption)
            ...
            // (If the player has a cock)
            if (player.cocks.length > 0) outputText("  Your " + multiCockDescriptLight(game.player) + " shoots cum across the ground and into the waiting mouths of the imps licking your crotch.", false);
            outputText("\n\n", false);

            outputText("Another imp-cock spasms in your throat as its owner rams deep into you. He floods your already swollen stomach with inhuman amounts of cum. Again you feel yourself about to black out as the demon pumps jism into you. He pulls out and again you orgasm as you wheeze for air. Another imp forces his cock down your throat as you moan and gasp. Your body shakes in pleasure on the big imp's " + cockNoun(CockTypesEnum.DOG) + ".  Tightening his grip on your " + hipDescription(player) + " the master imp howls and slams his shaft into your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            ...
            // (Low Corruption)
            if (player.cor < 50) outputText("You howl around the imp-cock stretching your throat. The bloated knot opens your hole far beyond anything you've endured before. Your violent thrashing throws the imps off your back and you buck uselessly, thrashing as the swollen " + cockNoun(CockTypesEnum.DOG) + " plunges deeper into you.", false);
            // (High corruption)
            else outputText("The master imp's bloated knot stretches your entrance and plunges into your hole with a loud <i>pop</i>. Another orgasm hits you as the " + cockNoun(CockTypesEnum.DOG) + " rams even deeper into you. You howl around the imp-cock stretching your throat, bucking as your orgasm shakes you. Your violent thrashing throws the imps off your back and slams your hips against the big imp, pushing him further into your hole.", false);
            outputText("  The big imp howls again as he cums, each wave of steaming demon-cum stretching his knot and shaft even more. His cum-pumping " + cockNoun(CockTypesEnum.DOG) + " is bottomed out deep in your ", false);
            // (If the player has a vagina)
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 2) outputText("The imp riding your " + biggestBreastSizeDescript(player) + " cums, his load lost in the flood of jism dripping off your abused fuck-udders. ", false);
            outputText("Your master isn't done with you yet, churning his " + cockNoun(CockTypesEnum.DOG) + " knot in your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            ...
            outputText("You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe, and your " + hairDescription(player) + " is soaked with jism. You haven't moved since you passed out. Your arms are still tied to the log, ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("your bruised and throbbing tits pressed against the rough wood, ", false);
            outputText("and your body rests in a cooling pool of cum. You couldn't move even if your " + legs(player) + " felt stronger. Your hideously bloated belly weighs you down, quivering with every orgasmic twitch that passes through you. The skin of your distended belly is drum-tight and shiny. As you slip back into unconsciousness, one last thought flits across your mind. ", false);
            // (Low Corruption)
            ...
            // Stretch!
            if (player.vaginas.length > 0) {
                if (cuntChange(player, monster.cocks.cockArea(2), true)) outputText("\n", false);
            }
            else {
                if (buttChange(player, monster.cocks.cockArea(2), true)) outputText("\n", false);
            }
        }
    }
    // NOT CENTAUR
        ...
        if (rand(2) == 0 && (player.cocks.length == 0 || player.gender == 3)) {
            ...
            outputText("The imps leap forward just as you start to ready your " + player.weaponName + ", one sweaty imp clinging to your arm", false);
            if (player.weaponName != "fists") outputText(" while another kicks your weapon out of reach", false);
            outputText(". The " + monster.short + " surges forward and grapples you. Imps grope your body and hump their " + cockDescriptShort(monster, 0) + " against your legs, smearing their sweat and pre-cum into your " + player.skinDesc + ". The rest of the " + monster.short + ", a dozen or more imps, all leer at you and laugh as they slap and pinch your body. The imps have sharp claws, tiny sharp teeth, and short horns on their heads. They scratch, claw, and bite at you with all of these weapons as they try to pull you down to the ground. One bold imp leaps forward and grabs your ", false);
            // (If the player has a cock)
            if (player.cocks.length > 0) outputText(cockDescript(game.player, 0), false);
            else outputText(nippleDescription(player, 0), false);
            ...
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("Imps surround your chest, slapping their " + cockDescriptShort(monster, 0) + "s on your " + allBreastsDescript(player) + " and rubbing their slippery pre-cum into your " + nippleDescription(player, 0) + ".  ", false);
            outputText("Others stand over your head, their cocks bobbing inches from your face as they jack off. A thick musk wafts off their cocks, and the smell of it makes your sinuses tingle. Two more imps take position between your legs, sliding their cocks along your thighs while stroking your " + vaginaDescript(player, 0) + " and flicking your " + clitDescription(player) + ".", false);
            // (If the player has a cock)
            if (player.cocks.length > 0) outputText("An imp rubs his hand across his cock-head, smearing it with his pre-cum. He rubs his hand over your " + multiCockDescriptLight(game.player) + ", making your cock-skin tingle as his fluid soaks into you.", false);
            outputText("\n\n", false);
            outputText("The " + monster.short + " snickers lewdly as your nipples harden and your pussy moistens. One of the imps between your legs slides his shaft along your pussy lips, teasing your " + clitDescription(player) + " with the tip of his cock.  ", false);
            // (Low corruption)
            ...
            // (High corruption)
            else outputText("You buck your hips, trying to capture his " + cockDescriptShort(monster, 0) + " with your " + vaginaDescript(player, 0) + ".  ", false);
            outputText("Before he can thrust into you, the imp is shoved aside by the biggest imp you've ever seen.\n\n", false);

            outputText("Four feet tall and broader and healthier looking than any imp you've seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a " + cockNoun(CockTypesEnum.HORSE) + " big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your groin. You shudder as the hot fluid stings the sensitive skin of your " + vaginaDescript(player, 0), false);
            // (If the player has a dick)
            if (player.cocks.length > 0) outputText(" and " + multiCockDescriptLight(game.player), false);
            outputText(". His " + ballsDescriptLight(monster) + " are each the size of your fist and slick with sweat. He slaps his sweaty balls against your " + vaginaDescript(player, 0) + " nearly scalding you with the heat.  ", false);
            // (Low corruption)
            if (player.cor < 33) outputText("You yelp and buck your hips to escape the heat.  ", false);
            outputText("He grabs your hips and slowly drags his shaft down your pussy, each ridge of his demonically-hot " + cockNoun(CockTypesEnum.HORSE) + " hitting your clit and pulling at your lips. Finally the broad horse-like head of his shaft catches on your " + clitDescription(player) + ", and the hot pre-cum dribbles over your sensitive flesh. The big imp sneers as you whimper, and drags his cock-head down to the opening of your " + vaginaDescript(player, 0) + ". The other imps watch and stroke themselves as their master pulls his hips back to push into you.\n\n", false);
            // (Low corruption)
            if (player.cor < 50) outputText("You scream for help", false);
            // (High corruption)
            if (player.cor >= 50) outputText("You moan with lust", false);
            outputText(" as the inhumanly hot cock-head stretches your pussy lips, your cries vanishing into the dark skies above. Two imps grab your hair and pull your head up, forcing you to watch as their master pushes his corrupted cock into you. Other imps spread your [legs] even wider, leaving you helpless as the big imp slides his swollen meat into your " + vaginaDescript(player, 0) + ". You squirm and twist against the imps holding you down as the hot flesh almost burns your sensitive cunt. You can smell the hot sweat steaming off his shaft, and your pussy-fluids start to steam as well as he forces his cock-head into your " + vaginaDescript(player, 0) + ". His huge cock-head bulges your groin, and you watch ", false);
            // (Low corruption)
            if (player.cor < 50) outputText("in helpless terror as the bulge inches up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous " + cockNoun(CockTypesEnum.HORSE) + " stretches your belly", false);
            // (High corruption)
            else outputText("panting in lust as the monstrous " + cockNoun(CockTypesEnum.HORSE) + " pushes your flesh aside to make room for itself", false);
            outputText(". ", false);
            // (This is a good place for the virginity-loss message, if needed)
            ...
            outputText(" in your stuffed " + vaginaDescript(player, 0) + ". The big imp sneers and bounces his cock again, watching ", false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 3) outputText("your " + allBreastsDescript(player) + " roll on your chest as you squirm", false);
            // (If the character doesn't have breasts)
            else outputText("your eyes roll back as you squirm", false);
            outputText(".  ", false);
            // (If the character has a cock)
            if (player.cocks.length > 0) outputText("Your " + multiCockDescriptLight(game.player) + " slaps against your distended belly as you shake.", false);
            outputText("\n\n", false);
            outputText("Finally the big imp pulls back his " + cockNoun(CockTypesEnum.HORSE) + ", each ridge pulling on your pussy flesh as he slides out. An imp reaches out and slaps the bulge as it withdraws, making you yelp and buck.  ", false);
            // (If the character has a cock)
            if (player.cocks.length > 0) outputText("Your " + multiCockDescriptLight(game.player) + " bounces as the bulge passes under it.  ", false);
            outputText("You moan as the mushroom-head reaches the entrance of your " + vaginaDescript(player, 0) + ", your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ", false);
            // (Low corruption)
            if (player.cor < 50) outputText("You try to pull your hips back, fighting to break free as the bulge of his cock-head works its way high up into your belly. You're held down by too many imps. You can only writhe around the hot shaft stretching out your " + vaginaDescript(player, 0) + ". Your head is held steady by two imps, you can't even look away as their master rapes you. The big imp grunts as his cock-head pops past your cervix, and you moan and shake in pain.", false);
            // (High corruption)
            else outputText("You moan in ecstasy as the hot " + cockNoun(CockTypesEnum.HORSE) + " pushes deep into your " + vaginaDescript(player, 0) + ", turning every inch of your pussy into a pleasure-sheath for the big imp. You know you're nothing but a fuck-toy for this corrupt creature, just a wet pussy for him to fill with cum, and the thought almost makes you orgasm as he forces his huge cock-head past your cervix.", false);
            outputText("Finally the corrupt cock bottoms out against your womb. The imp pulls back again, and starts to fuck you slowly.\n\n", false);

            outputText("The big imp grinds his hips as he thrusts and pulls, rubbing his cock-ridges against every part of your " + vaginaDescript(player, 0) + ".  While sliding his mutated " + cockNoun(CockTypesEnum.HORSE) + " in and out of you the imp rubs his hands along your mound, pulling it open or forcing it tight as he takes you. Your pussy juices steam off his cock as he pumps, and hot pre-cum dribbles down your crack to your " + assholeDescript(player) + ". ", false);
            // (Low corruption)
            ...
            // (High corruption)
            else outputText("Pain and pleasure blend into one as the huge " + cockNoun(CockTypesEnum.HORSE) + " stretches you, rubbing pre-cum into steaming pussy. You moan as the big imp fucks you, turning you into a mindless fuck-puppet. Your " + clitDescription(player) + " swells painfully as hot juices splash over it. Your " + nippleDescription(player, 0) + " tingle almost painfully as the heat of his pre-cum spreads through your body.", false);
            outputText("\n\n", false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 3) outputText("Your " + allBreastsDescript(player) + " bounce and jiggle back and forth as the master imp roughly fucks you.  ", false);
            // (If the character has a dick)
            if (player.cocks.length > 0) outputText("Your " + multiCockDescriptLight(game.player) + " swell painfully as the rough fucking pumps blood into your groin.  ", false);
            outputText("The big imp's snake tongue lashes out to incredible length and wraps around one of your " + nippleDescription(player, 0) + "s, pulling at it and stretching the flesh under it. He moves his tongue back and forth between your nipples, alternating between stretching and flicking them. ", false);
            // (If the character has a dick)
            if (player.cocks.length > 0) outputText("He draws his tongue back and wraps it around your " + cockDescript(game.player, 0) + ", sliding its length along your shaft and flicking his tongue over your cock-head.", false);
            // (If the character doesn't have a dick)
            else outputText("His tongue flicks down to your " + clitDescription(player) + ", the split ends of it teasing your clit.", false);
            outputText("  You gasp in time to the big imp's thrusts, whimpering when his cock or tongue hit a sensitive point.  ", false);
            // (Low Corruption)
            ...
            outputText("The master imp pounds at you as hard as he can, driving his " + cockDescriptShort(monster, 1) + " deeper into you. His grunts come closer and closer together. Your head still held up, you watch as the imps around you start to cum. They spray your body with thick globs of cum, splattering it across your belly", false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 3) outputText(" and " + allBreastsDescript(player), false);
            outputText(". The master imp pounds into you and you can see his " + ballsDescriptLight(monster) + " swell. Through the haze of your approaching orgasm you realize what's about to happen. Those oversized balls are about to pump more cum into you than any normal man could ever produce. They're going to pump demonic cum right into your womb.  ", false);
            // (Low Corruption)
            ...
            // (High corruption)
            else outputText("The thought of all that demon-jism in your womb pushes you over the edge. You cum hard, bucking your hips against the " + cockNoun(CockTypesEnum.HORSE) + " pumping hot cum into your belly. Your eyes roll back in your head and you scream out your ecstasy as thick jets of cum fill your pussy. The imp keeps thrusting into his fuck-toy even as he fills your womb with his cum, forcing you to another peak before you've come down from the last one. The big imp is your master now.", false);
            ...
            // (If the character has a cock)
            if (player.cocks.length > 0) outputText(", and cum-dripping " + cockDescript(game.player, 0), false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 3) outputText(", and bouncing " + allBreastsDescript(player), false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.biggestLactation() > 1) outputText("milk spurts from your " + nippleDescription(player, 0) + " and ", false);
            ...
            if (player.effects.findByType(StatusAffects.ImpGangBang) >= 0) {
                // (Subsequent encounters - Low Corruption)
                if (player.cor < 50) outputText("You can't tell if this is the same " + monster.short + " as last time or not - all imps look alike to you.  The " + monster.capitalA + " surges forward, grabbing at your " + legs(player) + " and arms and running their hands over your body. You struggle, but there are just too many to fight. The result is the same as last time...\n\n", false);
                // (Subsequent encounters - High Corruption)
                else outputText("It's about time they showed up. It's not like there's a lot to do in these rocks, and you were getting bored. You grab an imp dick in either hand and spread your legs as other imps grope your thighs...\n\n", false);
            }
            ...
            else outputText(legs(player) + " wide apart, holding them against the log.\n\n", false);

            outputText("The " + monster.short + " makes short work of your " + player.armorName + ", unbuckling straps and stripping you quickly. ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("An imp mounts your chest and slaps his " + cockDescriptShort(monster, 0) + " between your " + allBreastsDescript(player) + ", squeezing them tight over his cock as he rubs back and forth.  ", false);
            // (If the player has a SINGLE cock)
            if (player.cocks.length == 1) outputText("Your " + cockDescript(game.player, 0) + " is seized by an imp, who licks the tip with his inhumanly nimble tongue while he strokes the shaft.  ", false);
            // (If the player has a MULTI cock)
            if (player.cocks.length > 1) outputText("Two imps seize your " + multiCockDescriptLight(game.player) + ", licking the tips with their inhumanly nimble tongues while they stroke the shafts.  ", false);
            // (Low Corruption)
            ...
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 0) {
                outputText("Hands slide over your " + allBreastsDescript(player) + ", pinching and pulling at your nipples. The imp riding your " + biggestBreastSizeDescript(player) + " licks your tit-flesh, slowly working his tongue up towards your " + nippleDescription(player, 0) + ". Finally the imp's tongue reaches your nipple, wrapping around and pulling at the tingling flesh. ", false);
                // (Low Corruption)
                if (player.cor < 50) outputText("You can't escape the tongue lapping and pulling at your " + nippleDescription(player, 0) + ". You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.", false);
                // (High Corruption)
                ...
                outputText("  Suddenly you feel tiny needle-sharp teeth pierce your nipple. You scream as venom pumps into your tits, red-hot poison that makes your " + allBreastsDescript(player) + " feel as though they were being stung by bees. You moan in pain as your breasts start to swell, the imp rider biting into your other nipple to pump demon-taint into it.", false);
                if (player.breastRows.hasFuckableNipples()) outputText("With the imp's taint seeping into your " + nippleDescription(player, 0) + ", each one's cunt-like shape begins swelling. The fuckable orifices engorge into larger and fatter looking labia, becoming fuller cunts each with an engorged clitoral nub the size of a golf ball. Their color deepens as the skin of your nipple-cunts becomes tighter and smoother.  The imp giggles and continues nibbling the newly swollen sensitive flesh, injecting further doses of venom.", false);
                outputText("\n\n", false);
                // Grow tits!
                growTits(player, 2, player.breastRows.length, false, 1);
            ...
            else outputText("between your " + legs(player));
            outputText(", leering down at your trapped body. Four feet tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has grey fur, broad black demon wings, two long demon-horns on his head, and a " + cockNoun(CockTypesEnum.DOG) + " big enough to choke a minotaur. Pre-cum drips from the broad tip of it, dripping down onto your ", false);
            // (If the player has a cock)
            ...
            outputText(".  ", false);
            outputText("The heat stings your flesh. The imps licking your groin lap up the hot fluid, cooling you with their saliva. The big imp sneers as you whimper, and drags the head of his " + cockNoun(CockTypesEnum.DOG) + " down to your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            ...
            outputText(". ", false);
            // (Low Corruption)
            if (player.cor < 50) outputText("You screech in agony as the big imp forces his mutated wolf-cock into your hole, brutally shoving thick inch after inch of painfully hot " + cockNoun(CockTypesEnum.DOG) + " deeper into you than anything should ever go.  ", false);
            // (High Corruption)
            else outputText("The master imp's painfully hot " + cockNoun(CockTypesEnum.DOG) + " stretches your hole wider than it ever should be, and you moan in perverse ecstasy.  ", false);
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("The big imp reaches past your tit-rider and grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your " + allBreastsDescript(player) + " swell further at his touch.  ", false);
            ...
            outputText(". ", false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 1) outputText("The rough fucking shakes your breasts, and the imp sucking your nipples clings tightly to your monstrously swollen " + allBreastsDescript(player) + ". Your " + biggestBreastSizeDescript(player) + " have grown three cup sizes since the imp pumped his venom into you.  ", false);
            outputText("The imp fucking your face grabs your " + hairDescription(player) + " and jaw, forcing your head back so he can ram his cock into your throat. The obscene bulge sliding in your throat matches the bulge in your belly. The smaller imp pulls back just enough to let you gasp for air, then thrusts into your throat again. The big imp pounds the knot of his " + cockNoun(CockTypesEnum.DOG) + " against your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            ...
            outputText(" spasms on the steaming pole that impales it. Another imp shoves his cock in your mouth as you scream, throat convulsing around his cock-head.", false);
            // (If the player has a cock)
            if (player.cocks.length > 0) outputText("  Your " + multiCockDescriptLight(game.player) + " shoots cum across your belly and into the waiting mouths of the imps licking your crotch.", false);
            outputText("\n\n", false);
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 0) {
                outputText("Imps lick milk from your bloated " + biggestBreastSizeDescript(player) + " as your rider milks you.  As one imp drinks his fill, staggering away with a sloshing belly, another steps up to guzzle from your milk-spewing udders.\n\n", false);
                // Additional nipplefucking scene by Xodin
                if (player.breastRows.hasFuckableNipples()) {
                    outputText("The imp rider grabs the fat folds of one of your nipplecunt's 'labia' and grins mischeviously. He rubs his obscene erection all over the milk stained surface of your nipple-cunt's clit and begins to press the head of his bulbous imp cock into the swollen orifice against the flow of milk. You know no woman in your village could have handled an aroused cock this big, and yet now this imp on your " + allBreastsDescript(player) + " is about to ram just such an erection into one of your " + nippleDescription(player, 0) + "s. He tugs and pulls and pulls again on your nipple-cunt's sensitive labia, forcing his cock to push into the flesh of your " + biggestBreastSizeDescript(player) + ". Your taut flesh burns with his venom already, and is now violated by the presence of his demonic flesh rod.  ", false);
                    // [START BREAST SIZE SPECIFIC TEXT]
                    // [IF breastSize <= DD]
                    if (player.breastRows.biggestTitSize() <= 5) outputText("You feel the bulbous head of his cock squeeze further and deeper until it pushes up against your ribs.", false);
                    // [ELSE IF breastSize > DD]
                    else outputText("You feel the unnaturally large erection spear the fat filled depths of your " + biggestBreastSizeDescript(player) + " until at last the imp has shoved himself in to his hilt. He smiles at the sensation of having his manhood completely engulfed in your " + biggestBreastSizeDescript(player) + ".", false);
                    // [END BREAST SIZE SPECIFIC TEXT]
                    outputText("  Back and forth he begins fucking your tit as if it were a regular pussy, and it occurs to you that such a description isn't far from the truth. You gasp in pleasure as a strange kind of minor orgasm ripples through your tit and the taut skin of your mammary feels tighter as the " + biggestBreastSizeDescript(player) + " momentarily spasms around the imp's manhood. The horny little demon slaps your nipplecunt's clit in gleeful victory and jumps to the next breast to repeat his lewd fucking on a fresh hole.", false);
                    outputText("\n\n", false);
                }

            }
            outputText("The imp-cock in your throat spasms and its owner rams as deep into you as he can get. He floods your already swollen stomach with inhuman amounts of cum. Again you feel yourself about to black out as the demon pumps jism into you. He pulls out and again you orgasm as you wheeze for air. Another imp forces his cock down your throat as you moan and gasp. Your body shakes in pleasure on the big imp's " + cockNoun(CockTypesEnum.DOG) + ".  Tightening his grip on your " + hipDescription(player) + " the master imp howls and slams his shaft into your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // If the player doesn't have a vagina)
            else outputText(assholeDescript(player), false);
            outputText(". His unnaturally huge knot stretches the entrance of your hole, and he hammers into you again. ", false);
            // (Low Corruption)
            if (player.cor < 50) outputText("You howl around the imp-cock stretching your throat. The bloated knot opens your hole far beyond anything you've endured before. Your violent thrashing throws the imps off your " + legs(player) + " and you kick uselessly, thrashing and bucking as the swollen " + cockNoun(CockTypesEnum.DOG) + " plunges deeper into you.", false);
            // (High corruption)
            ...
            // (If the character has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("The imp riding your " + biggestBreastSizeDescript(player) + " finally cums, painting your distended fuck-udders with his massive load.  ", false);
            outputText("Your master isn't done with you yet, churning his " + cockNoun(CockTypesEnum.DOG) + " knot in your ", false);
            // (If the player has a vagina)
            if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0), false);
            // (If the player doesn't have a vagina)
            ...
            outputText("You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe. You haven't moved since you passed out. Your hips are still propped up over the log, and you rest in a cooling pool of cum, your " + hairDescription(player) + " plastered to the ground with drying jism. You couldn't move even if your " + legs(player) + " felt stronger. Your hideously bloated belly weighs you down, ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 0) outputText("and your milk-filled udders are still swollen with imp-venom, ", false);
            outputText("quivering with every orgasmic twitch that passes through you. The skin of your distended belly ", false);
            // (If the player has breasts)
            if (player.breastRows.biggestTitSize() > 3) outputText("and massive tits ", false);
...
export function impRapesYou(): void {
    outputText("", true);
    ...
    if (player.lust >= 100) {
        ...
        if (player.gender == 2 || player.gender == 3) {
            ...
            if (player.vaginalCapacity() < monster.cocks.cockArea(0)) {
                ...
                outputText("  His cock only sinks a few inches in, but he begins fucking you hard, each time claiming a bit more of your pussy for his demonic tool.  You feel a painful stretching as he gets half of it inside you, ruining your " + vaginaDescript(player, 0) + " for most humans.  He fucks you like this for what seems like forever, never getting much further. ", false);
                cuntChange(player, monster.cocks.cockArea(0), true);
            }
            else {
                outputText("  He plunges in violently, ramming his " + cockDescriptShort(monster, 0) + " in to the hilt, leaving you gasping in pain and surprise.  He leaves it there, giving you a second to get used to him, and then begins fucking you hard, slapping your ass every few thrusts to remind you who is in charge.", false);
                cuntChange(player, 12, true, true, false);
            }
            if (player.gender == 3) outputText("\n\nThe rough fucking becomes more and more pleasurable as time passes, until you cannot help but stroke your " + cockDescript(game.player, 0) + " along with each plunge he takes in your " + vaginaDescript(player, 0) + ".  You feel yourself clench around him as your sexual organs release, erupting spurts of cum and milking the demon's cock like your life depended on it.", false);
            if (player.gender == 2) outputText("\n\nThe rough fucking becomes more and more pleasurable as time passes.  You moan loudly and lewdly with each thrust, hips squeezing around the demon-cock, relishing the feeling of fullness.  Before long you cannot help but cum all over him, " + vaginaDescript(player, 0) + " locking around his cock like a vice, muscles rippling, milking him for his cum.", false);
            outputText("  The imp's " + cockDescriptShort(monster, 0) + " explodes inside you, pumping huge loads of hot demon-seed inside you with each eruption.  You swoon, feeling it fill your womb and distend your belly as the imp's orgasm fills you with an unnatural quantity of corrupted semen.\n\nWith a sigh, he pulls his dick free, and you flop back on your back, cum surging out onto the ground from your well-fucked hole.  ", false);
        ...
        if (player.gender == 0 || player.gender == 1) {
            ...
            if (player.gender == 1 && rand(2) == 0) {
                ...
                if (player.lowerBody == LowerBodyType.CENTAUR) outputText("hocks and knees, your " + cockDescript(game.player, 0) + " hurting from the massive blood pressure caused by your unbridled lust. He approaches you and stops about two feet in front of you, watching with delight your helpless state", false);
                else outputText("knees, pull out your " + cockDescript(game.player, 0) + " and begin mindlessly stroking yourself as the imp approaches you, a wicked grin on his face. Your mind races with thoughts and images of sucking the imp's cock. He approaches you and stops about two feet in front of you, watching with delight as you succumb to your own lust", false);
                outputText(". Your eyes glance down to his waist and see a massive bulge form under his loincloth, the sight of which causes your " + cockDescript(game.player, 0) + " to twitch and begin leaking pre-cum.\n\n", false);
                ...
                if (player.lowerBody == LowerBodyType.CENTAUR) outputText("Dropping down to the ground, your " + cockDescript(game.player, 0) + " trembles against your body to the rhythm of the imp's thrusts, leaving your underbelly smeared with its own pre-cum.\n\n", false);
                else outputText("On all fours now, your " + cockDescript(game.player, 0) + " bounces up and down against you to the rhythm of the imp's thrusts, leaving your belly smeared in your own pre-cum.\n\n", false);
                if (player.ballSize >= 5) outputText("Your huge " + ballsDescriptLight(player) + " swing heavily against you as well, responding to the force of the imp's thrusts, slapping your own ass and driving your " + cockDescript(game.player, 0) + " even stiffer with lust, the pre-cum pulsing out of your cock in time with the slapping.\n\n", false);
                ...
                outputText("You fall to the ground gasping, exhausted and unable to move, the demon cum on your face and inside you still burning with intense heat and corruption. You lose consciousness, your " + cockDescript(game.player, 0) + " still firmly erect, your lust not sated.", false);
                ...
                if (player.lowerBody != LowerBodyType.CENTAUR) outputText("as you are disrobed", false);
                outputText(".  As the imp presses a large bulk against your backside, you realize he has a massive penis!\n\nThe imp pushes his " + cockDescriptShort(monster, 0) + " into your ass and fucks you hard, with little regard to your pleasure.  After a rough fucking, he cums, stuffing your ass full of hot demon cum.  His orgasm lasts far longer than any human's, leaving your belly slightly distended.", false);
                buttChange(player, monster.cocks.cockArea(0), true, true, false);
                dynStats("lib", 1, "sen", 1, "lus", 1, "cor", 1);
                if (player.sens > 40) {
...
function noogaisNippleRape(): void {
    outputText("", true);
    outputText("You slowly walk over to the masturbating imp, your " + hipDescription(player) + " and " + buttDescription(player) + " swaying suggestively with every step.\n\n", false);
    ...
    // (if regular milky;
    else if (player.lactationQ() >= 50 && player.breastRows.biggestLactation() >= 1) outputText("releasing a steady trickle of warm milk and ", false);
    ...
    if (player.breastRows.biggestTitSize() <= 4) outputText("you feel his swollen cock bottom out, your petite breast unable to swallow any more of his throbbing maleness", false);
    // ((over D)
    ...
    // (if lactating)
    if (player.breastRows.biggestLactation() >= 1 && player.lactationQ() < 40) outputText("milk, ", false);
    outputText("fem-spunk and hot demon cum leaks out from your gaping nipple-cunt.\n\n", false);

    // (if corruption > 60)
    if (player.cor > 60) outputText("You thrust your digits into your " + nippleDescription(player, 0) + " once more, scooping out as much imp jizz as you can reach.  You happily drink up the thick goo, savoring the cloying taste before quickly getting dressed and leaving the imp to slumber.", false);
...
// IMP LORD
export function impLordEncounter(): void {
    clearOutput();
...
export function defeatImpLord(): void {
    clearOutput();
    if (monster.HP < 1) {
...
export function loseToAnImpLord(): void {
    clearOutput();
...
function sexAnImpLord(): void {
    ...
    if (!player.isTaur()) outputText("  You grab his hands, removing them from his " + cockDescriptShort(monster, 0) + ". This gets his attention immediately, and you grin widely, pinning him to the ground.");
...
function impLordBumPlug(): void {
    ...
    outputText("  You pull the imp's head towards your [hips], forcing his lips against the base of your " + cockDescript(game.player, x) + ".  The imp quickly gets the idea and begins to lick and suckle at your " + cockHead(player, x) + " expertly.");

    outputText("\n\nYou pet the top of his smooth head encouragingly, his tongue quickly soaking your length in saliva.  With little encouragement, the imp begins to take your " + cockDescript(game.player, x) + " into his mouth, focusing on milking the head of its delicious precum.  You soon remember what you'd intended to do with the little cock slut, and push him away from your length.  You could swear the imp whimpered in response to this, which makes you grin.");
    ...
    outputText("\n\nYou drape your " + cockDescript(game.player, x) + " between the imp's butt cheeks.  Teasingly you begin thrusting between the muscular cleft of his ass, much to the creature's dismay.  The greater imp whines submissively, desperate for you to enter him.  His " + cockDescriptShort(monster, 0) + " drools as you tease his ass, creating a small puddle of pre between his hooves.  The poor creature becomes so desperate that he reaches back and spreads his muscular cheeks with his hands.  You catch a small glimpse of his now gaping hole as your cock continues to slide between his cheeks.  You tease him with one last empty thrust, before grabbing his shoulders and forcing your " + cockDescript(game.player, x) + " deep inside the poor creature.");

    outputText("\n\nThe imp loses control immediately and nearly collapses from the massive orgasm.  His large, corruption-bloated balls clench up, and semen floods out of his " + cockDescriptShort(monster, 0) + " like a fountain.  The hot demon seed puddles around his feet, soaking the ground in his thick boy-goo.");

    outputText("\n\nHis orgasm also cause his anus to tighten and spasm around your " + cockDescript(game.player, x) + ", as if he was milking you of your seed.  Unable to resist, you start plunging yourself rapidly in and out of the spasming hole.");
    // (No line break)
    if (player.balls > 0) {
    ...
    if (player.breastRows.biggestTitSize() >= 1 && player.lactationQ() >= 200) {
        outputText("  You give a desperate groan, and grab one of your [chest] roughly.  You pinch and massage your ");
        if (player.breastRows.averageNipplesPerBreast() > 1) outputText("[nipples]");
    ...
    if (player.balls >= 4) {
        outputText("\n\nAs much as you would love to continue the pleasure you can't last any longer.  You howl in intense pleasure and cum.  Your [balls] tighten against your body, and empty their contents. Your " + cockDescript(game.player, x) + " pulses and spasms, releasing wave and wave of semen into the greater imp's belly.  The pleasure is intense, and almost painful as your cum.");
        // if(player is a herm)
        if (player.gender == 3) outputText("\n\nAs your " + cockDescript(game.player, x) + " erupts, your " + vaginaDescript(player, 0) + " tenses up tightly, spasming, desperate to be filled.  After a moment your girl juice begins to soak your inner thighs.  Your legs begin to tremble from the intensity of it all, and you question if you'll be able to make it back to your camp after this.");

        outputText("\n\nThe imp howls as his ass is flooded with cum.  His stomach begins to expand from the sheer amount of fluid you pump into him, and the sensation of being over filled causes the poor creature to cum a second time, spilling more semen into the already massive puddle.  His " + cockDescriptShort(monster, 0) + " pumps wave after wave of corrupt seed onto the ground, soaking his hooves even further.");

        outputText("\n\nYou're sure you must have blacked out at some point as you feel the last of your seed force its way out of your " + cockDescript(game.player, x) + " and into the imp's demon belly.  You wobble slightly and lean down, grabbing the ragged remains of the imp's loincloth.  Weakly you pull out of the demon's hole, and quickly stuff the cloth in its place as a makeshift plug.");
...
function getBlownByAnImpLord(): void {
    ...
    outputText("\n\n");
    // if(cock thickness < 7)
    if (player.cocks[player.cocks.biggestCockIndex()].cockThickness < 7) {
...
function femaleVagRape(): void {
    ...
    outputText("With little ceremony you grab the imp's " + cockDescriptShort(monster, 0) + " and begin to jerk your hand up and down roughly.  The little muscular beast begins to whine loudly, in protest to the rough and likely painful mistreatment of his " + cockDescriptShort(monster, 0) + ".  In spite of the protests, the rough treatment goes over well, as the creature begins to leak hot demon pre across your hand, which you smear across the shaft as a natural hot lube.");
    outputText("\n\nLicking your lips, you squat above the little demon, positioning your " + vaginaDescript(player, 0) + " above the thick log of meat.  You stay still for a moment, and question if this was a good idea.  The demon was so thick you hadn't even been able to fit your hand around his shaft.  There was little chance you'd get out of this without some rather rough stretching, but the scent of the demon's arousal, and your corrupt lust spur you onwards.");

    outputText("\n\nYou lower yourself, slowly, watching as the head of the demon's " + cockDescriptShort(monster, 0) + " begins to spread your " + vaginaDescript(player, 0) + ".  You begin to pant as you're stretched wide; you can hardly believe you can take him.  It takes you several minutes of steady, shallow, downward thrusts before the imp is completely hilted inside of your body.  You sit still for a moment to adjust to the intense sensation. The muscular creature doesn't seem to mind, as his tongue is hanging slightly out of the side of his mouth, panting like a dog.");
    cuntChange(player, monster.cocks.cockArea(0), true, true, false);

    outputText("\n\nAs you adjust, you get a devilish idea.  Both your hands gently begin to massage the imp's muscular abs and chest; before taking each of his fertite-pierced nipples between your fingers.  You pinch and tug upwards, roughly playing with his sensitive nipples.  The imp instinctively bucks his hips, slamming into you roughly, making you yelp and clench your [vagina] around his " + cockDescriptShort(monster, 0) + ".");
    ...
    if (player.cocks.length == 1) {
        if (player.cocks[0].cockLength < 8) {
            outputText("\n\nThe intense stimulation causes your " + cockDescript(game.player, 0) + " to begin leaking pre across the imp's stomach.  You remove one hand from the imp's nipples to pay attention to your aching " + cockNoun(CockTypesEnum.HUMAN) + ".  As you begin jerking yourself off, more and more pre puddles on the imp's abs.  Your pre begins to pool in and overflow from the imp's tight belly button.");
        }
        else if (player.cocks[0].cockLength <= 16) {
            outputText("\n\nThe intense stimulation causes your " + cockDescript(game.player, 0) + " to begin leaking pre across the imp's chest.  You remove one hand from the imp's nipples to pay attention to your aching " + cockDescript(game.player, 0) + ".  As you begin jerking yourself off, more and more pre drips across the imp's aching nipples, your pre a warm relief for the tender abused teats.  The warmth of your juice across his chest makes the little demon moan and buck harder into your " + vaginaDescript(player, 0) + ".");
        }
        else {
            outputText("\n\nAs your " + vaginaDescript(player, 0) + " is pounded relentlessly, your rock hard " + cockDescript(game.player, 0) + " slaps the imp in the face, each time leaving another trail of pre in its wake.  The imp's arousal only intensifies from this, and he's soon lapping and suckling at your " + cockDescript(game.player, 0) + ", expertly trying to get you off.");
            outputText("\n\nIt's not long before the imp's face is soaked in your pre, and thanks to the imp's oral ministrations your " + cockDescript(game.player, 0) + " is now leaking pre continually. It's only a matter of time before you blow now.");
        }
    }
    else if (player.cocks.length > 1) {
        outputText("\n\nAs you continue your cruel torture of the imp's now bruised nipples, he reaches up both of his hands and takes hold of your " + multiCockDescriptLight(game.player) + ".  The imp feverishly masturbates your " + cockDescript(game.player, 0) + " and " + cockDescript(game.player, 1) + " as he thrusts into your " + vaginaDescript(player, 0) + ".  The intense stretching of your " + vaginaDescript(player, 0) + " by the " + cockDescriptShort(monster, 0) + " sends waves of pleasure coursing through you, straight into your " + multiCockDescriptLight(game.player) + ", which are now leaking precum profusely onto the imp's muscular body.");
    }
    outputText("\n\nYou give one final rough tug on the imp's nipples, and that's all he can handle.  His cock spasms inside your " + vaginaDescript(player, 0) + " and erupts, sending wave after wave of hot, demon cum into your womb. His tainted testes spasm in their taut sack, continuing to pour their contents into you, until your belly is swollen and full of the hot seed.  You gasp in pleasure, unable to fight the pleasure of being filled so thoroughly.  Your " + vaginaDescript(player, 0) + " spasms, and you clench around the " + cockDescriptShort(monster, 0) + " inside of you, as your girl juices begin to flow out of you with your earth shattering orgasm.");

    if (player.cocks.length == 1) {
        outputText("\n\nWith your orgasm, your " + cockDescript(game.player, 0) + " erupts, sending waves of boy goo across the imp's already drenched body.  The imp mewls as your seed spills across his masculine body, marking him as the bitch he is.  He leans forward and takes your cock into his mouth, suckling what's left of your orgasm out of you.  The imp drinks down all the cum he can. His mouth feels so hot, you don't really want it to end, but soon the orgasm settles down and you come back to reality from your lust induced euphoria.");
        ...
        outputText("\n\nAs you ride out your orgasm the crafty imp pulls your " + cockDescript(game.player, 0) + " and " + cockDescript(game.player, 1) + " towards his mouth. Locking his lips around the tips of your two cocks, he suckles down every last drop of your jizz they offer.  You begin to mewl and whine in desperation as your orgasm seems to last an eternity.  The imp's skilled tongue and cock manage to work all of your favorite and most sensitive spots, sending you into complete euphoria.  Once your orgasm begins to settle the imp pulls back allowing the last few strings of semen to splatter across his face.");
    }
    outputText("\n\nAfter a few moments of recovery, you slowly lift yourself off the imp.  Cum rushes out of your " + vaginaDescript(player) + " and you clamp your muscles down as best as you can to keep the warm substance inside of you.  You give your swollen, cum-filled belly a motherly rub, before gathering your [armor].");
    player.orgasm();
    dynStats("cor", 1);
    player.slimeFeed();
    cleanupAfterCombat();
...
export function feederBreastfeedRape(): void {
    clearOutput();
...
function loseToImpLord(): void {
    ...
    if (player.cocks.length == 1) {
        outputText("\n\nYou feel your " + cockDescript(game.player, 0) + " grow harder than usual and throb.  You go to stroke yourself but it's far too sensitive. Any stroking you can do is far too little stimulation and anything else is too painful to withstand.  You whimper and curse in desperation.  Your lust clouded mind can only think of one solution; you bend over and reveal your [asshole] to the grinning imp.  The humiliation keeps you from looking back to see the imp's reaction, but you can tell by his chuckle that this is exactly what he wanted.");
    }
    else if (player.cocks.length > 1) {
        outputText("\n\nYou feel your " + multiCockDescriptLight(game.player) + " grow harder than usual and throb.  You go to stroke yourself but they are far too sensitive. Any stroking you can do is far too little stimulation and anything else is to painful too withstand.  You whimper and curse in desperation.  Your lust clouded mind can only think of one solution; you bend over and reveal your " + assholeDescript(player) + " to the grinning imp.  The humiliation keeps you from looking back to see the imp's reaction, but you can tell by his chuckle that this is exactly what he wanted.");
    }
    ...
    outputText("\n\nAfter longer than you'd have hoped for, the painful stretching sensation begins to disappear; and the pleasurable sensation of the imp's " + cockDescriptShort(monster, 0) + " thrusting in and out of your [asshole] becomes entirely pleasurable.  The way his " + cockDescriptShort(monster, 0) + " fills every inch of your ass, and rubs all your most sensitive spots.  The weird sensation his warm, demonic pre-cum coats your insides.  You find your lust-blinded mind has become lost in the sensations - so lost that you don't even notice the imp increasing his pace.");

    outputText("\n\nWithin moments the beast is wildly thrusting in and out of your [asshole].  Pre-cum is pumping out of his " + cockDescriptShort(monster, 0) + " like a faucet. The hot demon pre begins to spill back out of your abused [asshole], coating your [hips], and dripping to the ground beneath.  The imp gives you a few more rough thrusts before cumming hard into your [asshole].  The little demon's " + cockDescriptShort(monster, 0) + " spasms as he continues to roughly thrust and pump you full of his burning hot demon seed.");
...
function getRapedAsAGirl(): void {
    ...
    outputText(", draping his " + cockDescriptShort(monster, 0) + " across your wet crotch.  You groan, and unintentionally thrust against the magnificent tool between your legs.  The imp chuckles evilly as you coat his " + cockDescriptShort(monster, 0) + " in your girl juice, but he doesn't wait long before he slowly presses his head down against your [vagina].  His head slowly spreads your lips; the pleasure is unmistakable, and forces a loud moan from your lips.");

    outputText("\n\nWith a soft pop, the " + cockDescriptShort(monster, 0) + " pops into your [vagina], and both of you moan in unison, the demon beginning to thrust wildly into you.  His hips pumps back and forth into you.  The loud slapping sound of flesh on flesh echoes around you, drowning out the grunts of the vicious demon above you.");
    cuntChange(player, monster.cocks.cockArea(0), true, true, false);

    outputText("\n\nYou mewl softly as you're viciously fucked by the beast above you.  It doesn't take long before your [vagina] clenches tightly around the " + cockDescriptShort(monster, 0) + " as you orgasm.  You scream in pleasure as your inner walls begin to milk the imp's " + cockDescriptShort(monster, 0) + " of its seed.  The imp quickly succumbs and cums, his swollen balls tightening up against his crotch.  The hot jizz continues to pump into you for what feels like several painfully long minutes, until your belly bulges slightly, and your " + vaginaDescript(player, 0) + " begins to leak the white demonic fluid.");
