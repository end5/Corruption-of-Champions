...
export class ImpLord extends Imp {
    ...
    protected impFire(): void {
        ...
        // [-HP // +Lust(minor)]
        const damage: number = 40 + rand(10);
        player.takeDamage(damage);
        dynStats("lus", 20 + player.cor / 10);
        combatRoundOver();
    }
    ...
    protected impLordLustAttack(): void {
        ...
        // [+Lust]
        dynStats("lus", 5 + player.lib / 5 + player.cor / 5);
        combatRoundOver();
    }
    ...
    protected impLordLustAttack2(): void {
        ...
        outputText(" (" + damage + ")");
        // [-HP(minor) // +Lust]
        dynStats("lus", 5 + player.sens / 4 + player.cor / 10);
        combatRoundOver();
    }
    protected performCombatAction(): void {
        const choices: any[] = [impFire, impLordLustAttack2, impLordLustAttack, impLordHeavyEncounter, eAttack];
