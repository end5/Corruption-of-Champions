...
export function gargoylesTheShowNowOnWBNetwork(): void {
    clearOutput();
    ...
    if (player.effects.getValue1Of(StatusAffects.TelAdre) >= 1) outputText("or the ruins of one of Tel'Adre's more distant neighbors, ");
...
function breakZeChains(): void {
    ...
    outputText("\n\n\"<i>I am your humble servant, Master.  Your tool.  Your plaything.  Your toy.  Ask, and I shall obey.</i>\"  She seems to pause for a moment, and adds in an emotionless whisper, \"<i>this one would be honored to pleasure Master, if " + mf(player, "he", "she") + " desires.</i>\"");
...
export function returnToCathedral(woken: boolean = false): void {
    flags[kFLAGS.FOUND_CATHEDRAL] = 1;
    if (!woken) {
        clearOutput();
...
function strapOnGargoyle(): void {
    ...
    // (ELSE, PC has no vagina:)
        ...
        buttChange(player, 15, true, true, false);
...
function tailFuckGargoyleScene(): void {
    ...
    cuntChange(player, 15, true, true, true);
...
function ritualGargoyle(): void {
    ...
    if (player.effects.findByType(StatusAffects.Infested) >= 0 || player.effects.findByType(StatusAffects.Exgartuan) >= 0) addButton(2, "Banish", banishmentGargoyleRitual);
    addButton(4, "Leave", returnToCathedral);
}
...
function mindGargoyleRitual(): void {
    ...
    outputText("\n\n\"<i>Blessed is " + mf(player, "he", "she") + " who seeks purity,</i>\" she says, closing her eyes and bowing her head, \"<i>Blessed is " + mf(player, "he", "she") + " who seeks wisdom; and blessed is " + mf(player, "he", "she") + " who lusts after holiness and virtue before pleasures of the flesh.</i>\"");
...
function banishmentGargoyleRitual(): void {
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("a hitchhiker");
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("this bastard");
    ...
    // with Exgartuan:
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("  Booming from within you, you hear the demon's voice: \"<i>Hey!  What the shit is this!?  What're you doing? Fucking stop, you ingrate!</i>\"");
    // with Worms:
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("creature within you has");
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("possessed");
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) player.effects.remove(StatusAffects.Exgartuan);
    else player.effects.remove(StatusAffects.Infested);
...
function gargoyleKinkyRituals(): void {
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0 || player.effects.findByType(StatusAffects.Infested) >= 0) addButton(2, "Banish", banishPervRitual);
    addButton(4, "Leave", returnToCathedral);
}
...
function mindRitualPervy(): void {
    ...
    outputText("\n\n" + flags[kFLAGS.GAR_NAME] + " lifts herself to perch atop the alter with a single beat of her wings, coming to rest cross-legged before you.  Grinning a sexy grin, she picks up the small waterbasin and lifts it up to her lips, taking a small drink of it.  \"<i>Blessed is " + mf(player, "he", "she") + " who seeks the pure,</i>\" she begins to chant, lowering the bowl from her thin lips. As she does so, she slowly raises her left leg, resting her toes upon your chin.  Locking eyes with you, " + flags[kFLAGS.GAR_NAME] + " lowers the basin to her thigh and begins to pour, letting the water trickle sensually down her stone-smooth leg.  \"<i>Blessed is " + mf(player, "he", "she") + " who seeks wisdom,</i>\" she purrs as you instinctively open your mouth, allowing the holy water to trickle in from her foot.");

    outputText("\n\n" + flags[kFLAGS.GAR_NAME] + " quickly slips her small toes into your mouth, letting you suckle the holy water from them like an infant.  She lets you remain like that for nearly a minute, nursing from the rivulets that come your way, before pulling her leg back and taking your head with it.  She leans backwards as you approach, balancing upon her bat-like wings as she finishes the chant: \"<i>And blessed is " + mf(player, "he", "she") + " who lusts after holiness and virtue before pleasures of the flesh.</i>\"  She pours from the basin once again, releasing the holy water directly onto the large, shapely mounds of her breasts.  The water runs down her chest and belly, creating a tiny waterfall over the nub of her clitoris to which she guides you.");
...
function banishPervRitual(): void {
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("it");
    ...
    // If PC has Worms:
    if (player.effects.findByType(StatusAffects.Exgartuan) < 0) outputText("  You can feel her cock ram against your prostate, and shudder as the writhing mass of worms begin to panic as her cock pushes against them, obviously causing the creatures great pain.");
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText("Exgartuan is");
    ...
    if (player.effects.findByType(StatusAffects.Exgartuan) >= 0) outputText(" within you has");
...
// [b]Pointless[/b]
// (Confidence -2)
function pointlessGargoylesArePointless(): void {
    clearOutput();
    gargoyleConfidence(-5);
...
// [b]Encourage[/b]
// (Confidence +2)
function encourageGargoyleWaifuToDoSomething(): void {
    clearOutput();
    gargoyleConfidence(10);
...
// [b]Berate[/b]
// (Confidence -10)
function berateDatGargoyle4SomeSavin(): void {
    clearOutput();
...
function talkAboutGarName(): void {
...
function garNameA(): void {
    ...
    if (race(player) == "human") outputText("are");
...
// [b]Appearance[/b]
function gargoyleAppearance(): void {
    clearOutput();
