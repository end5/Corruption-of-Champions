...
export function enterTheBazaarAndMenu(demons: boolean = true): void {
    ...
    if (game.time.hours >= 15 && game.time.hours <= 20 && flags[kFLAGS.CINNABAR_NUMBER_ENCOUNTERS] > 0) rat = "Cinnabar";
    ...
    let demon: () => void = null;
    let tent: () => void;
    let benoit2: () => void = null;
    let benoitT: string = "MarketStall";
    if (game.time.hours >= 9 && game.time.hours <= 17) {
        if ((flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] == 1 && game.time.days >= flags[kFLAGS.FEMOIT_NEXTDAY_EVENT]) || flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] != 1) {
...
function joeyAndrogyny(): void {
    ...
    outputText("At last he turns back around, holding a cork-stoppered vial of pink-hued liquid in his manicured finger-tips.  \"<i>This is the stuff!  Now just close your eyes, we gotta get this worked into your skin and it stings worse than an angry wasp-girl if it gets in your eyes,</i>\" says Joey.  You blink your eyelids closed, as instructed, and you feel the soft skin of the feminine bunny rubbing over your " + face(player) + ", working something wet and tingly into your " + player.skinDesc + ".  The delicate facial massage takes roughly an hour, but you feel fresh and relaxed once it's finished.\n\n", false);
...
function joeysMassageWifNoExtraJizz(): void {
    ...
    outputText("The massage heads back towards your " + buttDescription(player) + "; Joey's hands fill with your flesh as he fondles and strokes, spreading the jism into every nook and cranny, even your " + assholeDescript(player), false);
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.balls > 0) outputText("his hand firmly rubs your " + sackDescript(player) + ", and ", false);
        outputText("gentle fingertips are probing between your cheeks, rubbing his dripping seed against the pucker of your " + assholeDescript(player) + ".\n\n", false);
    }
    // (FEM/GENDERLESS)
    else {
        if (player.vaginas.length > 0) outputText("presses his ruby lips into the glistening delta of your mons", false);
        else outputText("presses a finger against the semen-soaked ring of your " + assholeDescript(player), false);
    ...
    if (player.cocks.length === 0) {
        ...
        // (Female orgasm)
            ...
            outputText("  You give your " + nippleDescription(player, 0) + "s a hard tweak, torturing them as the building pleasure grows.  Aware of this, Joey curls a finger to press directly on a sensitive spot, deep inside you, and then you're arching your back and howling with unleashed release.\n\n", false);
        }
        // (continuing)
    ...
    // (Dickgasms
        ...
        outputText("Though Joey's mouth is making you burn with passion, it's the feeling of a single, intruding fingertip against your " + assholeDescript(player) + " that puts you on edge.  His warm jism slides down his finger, dripping into your violated backside with each slow pump.  Once you've adjusted, he continues the rectal massage, gently caressing the sides of your prostate while his mouth stays busy with your " + cockDescript(game.player, 0) + ".  The bunny's tongue thrashes, twisting like a slippery eel over your length, his cheeks hollowing from the suction he's applying.  A moment later, Joey curls the digit inside you to press firmly against your prostate, squeezing the sensitive organ tightly enough to make your hips pump into the air.\n\n", false);
        ...
        if (player.cumQ() >= 700) {
            outputText("  A smile widens your " + face(player) + " when you see the ", false);
...
function joeysMassageWithEXTRASpooge(): void {
    ...
    outputText("You're rolled back on to your front, crushing " + sMultiCockDesc(game.player) + " between you and the sheets.  Joey leaps back atop you, straddling your back and facing your " + buttDescription(player) + ", his hands locking onto the steam-moistened cheeks.   He slides forward slightly, placing his hardness between them, and it's then that you notice his discarded thong on the floor.  A moment later the first drops of Joey's own hot seed are dripping over your exposed derriere.  He slides himself through your buns, hotdogging the rapidly-slickening surface of your ass while his hands massage the tense flesh a little more enthusiastically than they ought to.\n\n", false);
    ...
    outputText("The bunny-boy springs back atop you, landing hard just below your hips.  His dripping seed washes over " + sMultiCockDesc(game.player) + ", and the sudden onslaught of fresh, liquid warmth on your groin sets off a small explosion of jism that splatters into your chin, leaving a long trail of slime behind like a snail.  The long-eared girly-boy smiles and shifts to rub his small cock against your " + cockDescript(game.player, 0) + ", frotting you aggressively while you both spray cum like faucets with the knobs torn off.  You don't mind that he seems to have forgotten the massage, and you run your hands up and down your " + chestDesc(game.player) + " to smear the heavy loads ", false);
    if (player.skinType != SkinType.FUR) outputText("over your " + player.skinDesc, false);
    else outputText("through your fur", false);
    outputText(".\n\n", false);

    outputText("The special potion makes it so your orgasm is long and languid, oozing out for minutes instead of seconds, and the small chamber is filled with breathy, exultant cries of passion.  Joey leans forward and grabs hold of your " + nippleDescription(player, 0) + "s, ", false);
    if (!player.breastRows.hasFuckableNipples()) outputText("squeezing and tugging on them", false);
    else outputText("sliding cum-coated fingers into their slippery depths", false);
    outputText(" while he grinds and cums on your " + cockDescript(game.player, 0) + ".  \"<i>Ungg... yes... gotta... squeeze... allofitooooouuuuut!</i>\" he pants.  He lets go of one tender nipple to reach underneath you, and before you can react, his cum-soaked finger is inside your " + assholeDescript(player) + ", pressing on your prostate.\n\n", false);
    outputText("Your eyes cross and you black out, twitching weakly as a regular orgasm bursts on top of your already-protracted potion-gasm.  A pleasant, heavily sexualized dream is interrupted by a finger poking your lotion-lubricated cheek and the best you can manage is an utterly contented \"<i>mmmm.</i>\"  A warm, moist towel is rubbed over your " + face(player) + ", wiping away the spooge you blasted over it, and now that you can see him, Joey says, \"<i>You had me worried for a minute there!  Keep the towel, it's on the house, and if you need to clean up, there's a shower in the back!", false);
...
function suckOffJoeysGardenHose(): void {
    ...
    outputText("You pull back and gasp for air, ignoring the ropes of spooge splattering across your face and into your " + hairDescription(player) + ".  Determined not to lose your progress, you take his none-too-impressive cock in your hand and jack it, spurring his body to take over and propel even more eruptions of slick seed onto your " + face(player) + ".  It's so messy, so decadent, but it's ", false);
...
function askGretaAboutInventory(): void {
    ...
    outputText("\n\nGreta laughs so hard that her tits quake, scattering her knitting needles away.  Her tail deftly catches one that rolls off the crude counter's edge, twirling it about before stuffing it into the tight bob she has atop her head.  \"<i>You think this is a sock!?  Ahahahahah!  No, dear " + mf(player, "boy", "girl") + ", this isn't an ordinary sock.  It's a cock-sock.  See the pattern here?  And the specially sewn gaps?  They let the wearer accentuate every sweet curve and throbbing vein on their erection, all while exposing the most sensitive bits for busy hands and hungry tongues, like mine.</i>\"  She lets her tongue slowly extend out from her mouth, inch after inch of the wiggling, slick organ slowly disappearing into the valley between her tits.  She slowly retracts it with a giggle.");
...
function yesPutDatSockOnMe(target: number): void {
    ...
    outputText("You nod to the busty succubus and strip off your [armor], revealing your naked body.  Greta's eyes light up as she looks over your body with barely-contained lust.  Finally her eyes settle onto your " + cockDescript(player, target) + ", and she licks her lips.  ");
...
function joeySweetMassage(): void {
    ...
    outputText("\n\nThe warm, steamy air caresses your " + skinFurScales(player) + " even before Joey does, running down your back, butt and [legs].  In no time at all, the air in the room is thick and warm, making you feel delightfully lazy with every breath, and a bead of sweat forms on your brow.  The bed shifts a bit as Joey leaps up onto it, straddling your waist, while he starts working his fingers over your shoulders.  His petite hands are surprisingly talented, finding every little knot you've accumulated in your journeys and unravelling them with one blissful touch.");
...
function eggsInButt(eggButt: boolean = false): void {
    ...
    else if (player.vaginas.length > 0) outputText("reaches between his legs, slipping two fingers inside you while his thumb gently rubs your clit");
    ...
    else if (player.vaginas.length > 0) outputText("your pussy tightens around his fingers, suddenly releasing a flood of femcum that splatters messily against the table");
...
function collectSomeButtSmex(): void {
    ...
    if (player.vaginas.length > 0) outputText("  Your " + vaginaDescript(player, 0) + " grows slick in your excitement.");
    ...
    outputText("  You hear the clopping of hooves on the floor as the satyr moves over to your " + buttDescription(player) + ".  His hands grip over your " + hipDescription(player) + " and drags you closer to the edge of the table so that your " + buttDescription(player) + " hangs over the side of the table.  You raise your head to watch with a ");
...
function finalGayFinallee(road: number = 0): void {
    ...
    if (road == 0) {
        ...
        else if (player.vaginas.length > 0) outputText("Fingers ram into your " + vaginaDescript(player, 0) + ", thrusting into you as he fucks your other hole.  ");
    ...
    // ((Ending 3 , suck none, //No furry plx))
        ...
        else if (player.vaginas.length > 0) outputText("  Fingers dip into your " + vaginaDescript(player, 0) + ", ramming into you as he fucks your other hole.");
