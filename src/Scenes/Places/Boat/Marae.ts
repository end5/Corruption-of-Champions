...
export function encounterMarae(): void {
    ...
    if (player.effects.findByType(StatusAffects.DungeonShutDown) < 0) {
        ...
        if (player.effects.findByType(StatusAffects.MetMarae) < 0) {
            player.effects.create(StatusAffects.MetMarae, 0, 0, 0, 0);
            ...
            // Explain the dungeon scenario
                ...
                player.effects.create(StatusAffects.MaraesQuestStart, 0, 0, 0, 0);
    ...
    // Dungeon inoperable
        ...
        // Corrupt!
            ...
            if (player.effects.findByType(StatusAffects.MetCorruptMarae) < 0) {
                player.effects.create(StatusAffects.MetCorruptMarae, 0, 0, 0, 0);
...
function maraeBadEnd(): void {
    ...
    if (player.effects.findByType(StatusAffects.MetCorruptMarae) < 0) outputText("The goddess flows out of the tree, stepping away from it as a living woman, curvy and nude.\n\n", false);
    ...
    if (player.effects.getValue3Of(StatusAffects.Marble) > 0) outputText("Any thought or need to drink Marble's milk vanishes from your mind and body.  ", false);
...
function maraeStealLethicite(): void {
    ...
    // (FAIL)
        ...
        if (player.gender == 1 || (player.gender == 3 && rand(2) == 1)) {
            ...
            outputText("'s tip.  Immediately a powerful suction draws your " + cockDescript(game.player, 0) + " inside the tentacle-maw, burying you up to the base in squirming pleasure.   Marae watches the plant go to work, squeezing teasingly until you orgasm.  It takes mere moments for the gifted tentacle to achieve its goal.  Your cum makes a tasty treat for the plant-beast, and it sucks and sucks until your body feels empty and drained.\n\n", false);
            ...
            outputText("You feel warm, enclosed in comfort and pleasure.  Is this heaven?  No, your head is throbbing and your eyes are closed... you open them and discover you're still lying at the base of the tree.  That greedy tentacle is still locked around your " + cockDescript(game.player, 0) + " pinning it in the throes of orgasm.  You watch thick bulges of cum pump up the tentacle, evidence of a truly garguantuan fluid output.  It goes on and on, and you realize the pleasure ought to drive you mad.\n\n", false);
...
export function level2MaraeEncounter(): void {
    ...
    // (FAIL-STOLE)
    else if (player.perks.findByType(PerkLib.MaraesGiftStud) >= 0 || player.perks.findByType(PerkLib.MaraesGiftFertility) >= 0) {
    ...
    if (player.perks.findByType(PerkLib.MaraesGiftFertility) >= 0 || player.perks.findByType(PerkLib.MaraesGiftStud) >= 0) outputText("second ", false);
...
function MaraeIIStageII(): void {
    ...
    if (player.gender == 2) {
        ...
        else outputText("your " + hairDescription(player), false);
    ...
    // [Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
        ...
        if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("Sticky wetness glistens between your thighs", false);
...
function MaraePt2RoundIIIPrizes(): void {
    ...
    // [HERMS]
        ...
        outputText("You awaken in the midst of a powerful orgasm.  Jism boils out of " + sMultiCockDesc(game.player) + ", pumping into the tight, sucking tentacle-hole.  Plant-spooge is pumping into your clenching birth-canal, and you can feel it worming its way into your over-packed womb.  Your eyes open wider, and your head clears while you rock your hips in bliss.  You're hanging upside down, suspended in the tentacle tree!  Marae isn't far from you, and she's busy deep-throating the fattest tentacle you've seen while another pair are working her openings.  She turns to you, aware of your wakefulness, and removes the oral intruder, though it manages to squirt a layer of spunk into her face in defiance.   The goddess smirks and slaps it, scolding it before she speaks, \"<i>", false);

        // (HAZ NEITHER)
        if (player.perks.findByType(PerkLib.MaraesGiftFertility) < 0 && player.perks.findByType(PerkLib.MaraesGiftStud) < 0) {
            // (RANDOM 1)
            if (rand(2) == 0) {
                outputText("You might feel a little sore.  I gave your little womb a makeover to make sure you'll be nice and fertile for all the boys out there.  You're going to serve me so well.  So many died fighting the demons, and you'll be popping out kids from every dick that gets anywhere near your little birth-hole.</i>\"  ", false);
                player.perks.create(PerkLib.MaraesGiftFertility, 0, 0, 0, 0);
            ...
            if (player.perks.findByType(PerkLib.MaraesGiftFertility) >= 0) outputText("<b>(New Perk Gained: Marae's Gift - Fertility)</b>", false);
            else outputText("<b>New Perk Gained: Marae's Gift - Stud)</b>", false);
        }
        // (HAZ BREEDER)
        else if (player.perks.findByType(PerkLib.MaraesGiftFertility) >= 0) {
