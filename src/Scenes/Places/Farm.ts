...
export function farmExploreEncounter(): void {
    ...
    if (player.effects.getValue1Of(StatusAffects.MetWhitney) < 2) {
        spriteSelect(62);
        if (player.effects.findByType(StatusAffects.MetWhitney) < 0) {
            player.effects.create(StatusAffects.MetWhitney, 0, 0, 0, 0);
            ...
            player.effects.addValue(StatusAffects.MetWhitney, 1, 1);
            if (player.effects.getValue1Of(StatusAffects.MetWhitney) == 2) outputText("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n", false);
        }
    ...
    // Repeat Offender
        ...
        if (player.effects.findByType(StatusAffects.MarbleRapeAttempted) < 0 && player.effects.findByType(StatusAffects.NoMoreMarble) < 0 && player.effects.findByType(StatusAffects.Marble) >= 0 && flags[kFLAGS.MARBLE_WARNING] == 0) {
...
function talkWhitney(): void {
    ...
    if (flags[kFLAGS.MURBLE_FARM_TALK_LEVELS] > 0 && player.effects.findByType(StatusAffects.Kelt) >= 0 && flags[kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT] == 0) {
    ...
    if (player.effects.findByType(StatusAffects.TelAdre) >= 0 && player.effects.getValue1Of(StatusAffects.TelAdre) >= 1 && flags[kFLAGS.WHITNEY_TALK_TELADRE] == 0 && flags[kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT] == 1) {
    ...
    if (flags[kFLAGS.DISCOVERED_HIGH_MOUNTAIN] > 0 && flags[kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN] == 0 && flags[kFLAGS.WHITNEY_TALK_TELADRE] == 1) {
        ...
        if (player.effects.findByType(StatusAffects.BreastsMilked) >= 0) {
        ...
        if (player.effects.findByType(StatusAffects.CockPumped) >= 0) {
            if (player.effects.findByType(StatusAffects.BreastsMilked) < 0) outputText("\n\n");
    ...
    if (player.effects.findByType(StatusAffects.DungeonShutDown) >= 0 && flags[kFLAGS.WHITNEY_TALK_DUNGEON] == 0 && flags[kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN] == 1) {
    ...
    if (player.effects.findByType(StatusAffects.FactoryOverload) < 0) outputText("how well the farm has been going", false);
...
function breastMilkerPurchase(): void {
...
function breastMilkerNoPurchase(): void {
    doNext(Camp.returnToCampUseOneHour);
...
export function exploreFarm(): void {
...
export function getMilked(): void {
    ...
    // Apply
    if (player.effects.findByType(StatusAffects.BreastsMilked) < 0 || application == 0) {
        if (player.effects.findByType(StatusAffects.BreastsMilked) < 0) player.effects.create(StatusAffects.BreastsMilked, 0, 0, 0, 0);
    ...
    if (player.effects.getValue1Of(StatusAffects.LactationEndurance) < 1.5) player.effects.addValue(StatusAffects.LactationEndurance, 1, .05);
    player.effects.addValue(StatusAffects.LactationEndurance, 1, .05);
    player.effects.create(StatusAffects.Milked, 8, 0, 0, 0);
    doNext(Camp.returnToCampUseOneHour);
}

export function cockPumping(): void {
    ...
    if (player.cocks.cockArea(0) < 20) {
        ...
        if (rand(2) == 0) {
            if (player.cocks.length == 1) {
                ...
                outputText("The mechanical snake's receptacles yawn open, showing an interior filled with wriggling pink material and dripping mucusy wetness.  They wiggle, closing the distance to your " + multiCockDescriptLight(game.player) + " in a flash.  Pausing just below your " + cockHead(player) + "s, they slowly flow upward, engulfing you with slick pleasure.  They suck hard, sliding up ", false);
    ...
    // Massive Scene – else
        ...
        if (player.cocks.length == 1) {
            ...
            outputText("An eruption of jism boils out, splattering with loud wet noises into the tube.  You can feel the pressure build up, pressing back against your " + multiCockDescriptLight(game.player) + " as the machinery fails to keep up with the gushes of white fluid erupting inside it.  Your eyes cross from the pleasure and you start drooling all over yourself, not even noticing that you're about to blast the milkers off your members.  The machinery grinds as it struggles to keep up, blaring out alarms.  You don't even notice, instead focusing on the feeling of depositing sticky globs inside the milker-tubes, pushing them halfway off your " + multiCockDescriptLight(game.player) + ".\n\n", false);
            outputText("Hurried footsteps enter the barn, and a pair of firm hands grab onto your bouncing " + multiCockDescriptLight(game.player) + ", pulling the milkers back against your groin and splattering spunk around the seal from the added pressure.  The machine-suction totally cuts off, but you don't even care.  You just keep cumming, pumping the thick white cream down the tubes by the force of your orgasm alone.   Eventually your incredible orgasm winds down, and the hands release you, allowing the tubes to pop off.  They slowly retracts to the wall as your harness is lowered, dropping your exhausted body in a wet heap on the ground.  When you regain your senses, your mysterious helper is gone.\n\n", false);
        }
        // (+40 fatigue)
        fatigue(40);
    }
