...
export class TelAdre {
    ...
    private telAdreCrystal(): void {
        ...
        else if (player.cor >= 50 || game.monk >= 5) {
        ...
        else if (player.cor >= 25 || player.effects.findByType(StatusAffects.FactoryOverload) >= 0) {
    ...
    public telAdreMenu(): void {
        ...
        if (!urtaQuest.urtaBusy() && flags[kFLAGS.PC_SEEN_URTA_BADASS_FIGHT] == 0 && rand(15) == 0 && game.time.hours > 15) {
            urtaIsABadass();
            return;
        }
    ...
    private piercingStudio(): void {
        ...
        outputText("She introduces herself, \"<i>Hello there " + mf(player, "sir", "cutie") + ", my name is Yara.  Would you like to get a piercing?</i>\"", false);
    ...
    public oswaldPawn(): void {
        ...
        if (player.effects.findByType(StatusAffects.Oswald) < 0) {
            ...
            player.effects.create(StatusAffects.Oswald, 0, 0, 0, 0);
        }
        else {
    ...
    public barTelAdre(): void {
        ...
        if (Edryn.pregnancy.type != PregnancyStore.PREGNANCY_TAOTH) { // Edryn is unavailable while pregnant with Taoth
            if (Edryn.edrynBar()) {
                ...
                else if (player.effects.getValue1Of(StatusAffects.Edryn) < 3) {
        ...
        if (flags[kFLAGS.ROGAR_PHASE] == 3 && flags[kFLAGS.ROGAR_DISABLED] == 0 && flags[kFLAGS.ROGAR_FUCKED_TODAY] == 0) {
            button = anotherButton(button, "HoodedFig", Rogar.rogarThirdPhase);
        ...
        // ROGAR #2
            ...
            button = anotherButton(button, "Rogar", Rogar.rogarPhaseFour);
    ...
    private function oldbarTelAdre():void {
        ...
        if(flags[kFLAGS.ROGAR_PHASE] == 3 && flags[kFLAGS.ROGAR_DISABLED] == 0 && flags[kFLAGS.ROGAR_FUCKED_TODAY] == 0) {
            rogarB = Rogar.rogarThirdPhase;
            //Wet Bitch screen text when Ro'gar phase = 3:
            outputText("\n\nYou notice a cloaked figure at the bar, though you're quite unable to discern anything else as its back is turned to you.", false);
        }
        else if(flags[kFLAGS.ROGAR_PHASE] >= 4 && flags[kFLAGS.ROGAR_DISABLED] == 0 && flags[kFLAGS.ROGAR_FUCKED_TODAY] == 0) {
            rogarB = Rogar.rogarPhaseFour;
            rogarT = "Rogar";
            //Wet Bitch bar text when Ro'gar phase = 4:
            outputText("\n\nRo'gar is here with his back turned to the door, wearing his usual obscuring cloak.", false);
        }
    ...
    */
    ...
    public tailorShoppe(): void {
        ...
        if (player.effects.findByType(StatusAffects.Victoria) < 0) {
            ...
            // Flag as meeting her
            player.effects.create(StatusAffects.Victoria, 0, 0, 0, 0);
        }
        else {
    ...
    private watchUrtaBeABadass(): void {
        ...
        if (player.effects.findByType(StatusAffects.Infested) >= 0) outputText("aroused", false);
    ...
    private goJogging(): void {
        ...
        // (<60 tou)
        else if (player.tou < 60) outputText("and you do quite well.  You jog around for nearly an hour, working up a healthy lather of sweat.  Even your " + legs(player) + " tingle and burn with exhaustion.", false);
        // (<80 tou)
    ...
    private yaraSex(girl: boolean = true): void {
        ...
        outputText(".  \"<i>Hold it,</i>\" Yara commands softly, pressing her hand against your " + chestDesc(game.player) + " and pushing you back in your chair.  \"<i>Do you think I'll let you get away without some... field testing?</i>\"\n\n", false);
    ...
    private flirtWithVictoria(itype: ItemType): void {
        ...
        if (player.cocks[x].cockLength > 5)
            outputText("making sure to give the " + cockHead(player, x) + " a quick suck every time it breaches her mounds.");
        else
        ...
        outputText("\n\nYou wrench your dick free of the confines of her pillowy mounds, and grab her around her plush middle.  She gives a surprised yelp as you nearly throw her against the counter and lift her butt up into the air.  <i>\"Ooooh, someone's excited!\"</i>  she nearly cheers, looking over her shoulder and wagging her plush rear up at you as her tail swishes back and forth, showing that she's nearly as consumed with lust as you are.  Flipping her long skirt up over her back, you violently pull her panties aside before you force your " + cockDescript(game.player, x) + " deep within her gushing folds.");
