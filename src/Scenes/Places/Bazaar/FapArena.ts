...
export class FapArena extends BazaarAbstractContent {
    ...
    private fapArenaPageII(): void {
        ...
        if (player.cor > 66) outputText("You eagerly seize the rod on your right side; squeezing it a bit in impatience.  A  little tingle alerts you to your left neighbor grabbing " + game.player.oMultiCockDesc() + " with an expert hand.  Everyone around you quickly grabs hold of the nearest cock on their right side.  Some of them are trembling because of the induced stress and arousal.\n\n", false);
        else if (player.cor > 33) outputText("You nervously grab hold of the rod on your right side.  In your stress, you can't help but give it a little squeeze.  You tremble as your left neighbor grabs " + game.player.oMultiCockDesc() + " with an expert hand.  Most people are as nervous as you, their whole naked body trembling in stress.\n\n", false);
        else {
            ...
            outputText("You can't help but start as your left neighbor grabs " + game.player.oMultiCockDesc() + " with an unsettling expertise.  Other players are also nervous, but none as much as you.  The sight of all these trembling dicks makes you shiver.\n\n", false);
        }
        ...
        const s: number = player.sens;
        // D the number of hours since he last came
        const d: number = player.hoursSinceCum;
        let c: number = 0;
        ...
        const r: number = lustPercent() / 100;
        // The game does a roll between 0 and 100, call it N.
        const n: number = rand(100);
    ...
    private fapResults(place: number = 3): void {
        ...
        if (place == 3) {
            ...
            outputText("Your left partner is very professional.  At the very moment your " + game.player.cockDescript(x) + " starts shooting its goo, your cock-milker yells in a thundering voice: \"<i>NUMBER " + (num - 1) + " SCORING!</i>\"  For a moment, lost in your pleasure, you pay little attention to the world outside of your " + game.player.cockDescript(x) + ".  Then you remember the game and shout in a whorish, pleasure-filled voice: \"<i>NUMBER " + num + " CUMMING!</i>\"", false);
            if (player.hasVagina()) {
                outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ", false);
            ...
            // [if player lactates]
            if (player.biggestLactation() > 1.5) outputText("  Your " + allBreastsDescript() + " also release a milky flood of their own, the soft feeling of cream teasing your nipple only prolonging your orgasm.", false);
            outputText("\n\n", false);

            outputText("The cute referee nods in acknowledgement.  Her eyes seem to be filled with longing for your cum-spilling " + game.player.multiCockDescriptLight() + ", and you know she'd eagerly finish you off if it weren't for her professionalism.\n\n", false);
        ...
        // [else, the player wins]
            ...
            outputText("You mentally keep track of all the people still in play.  They are fewer and fewer.  You just have to hold on a little longer.  Four... three... two... that's it.  You made it.  Phew, you have won just in time, your " + game.player.cockDescript(x) + " was starting to twitch on its own.  Unhindered by the fear of losing, you completely give up to your orgasm and sigh in utter satisfaction and relief as you release one of the most abundant cumshots you've ever had.  Lost in climax, you carelessly ", false);
            if (player.cumQ() >= 1000) outputText("soak", false);
            else if (player.cumQ() >= 500) outputText("pollute", false);
            else outputText("wet", false);
            outputText(" the floor with your sexual juices and yell in a whorish voice: \"<i>NUMBER " + num + " CUMMING!</i>\"  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + game.player.cockDescript(x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.", false);
            if (player.hasVagina()) {
                outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ", false);
                if (player.wetness() >= 4) outputText("large puddle", false);
                else if (player.wetness() >= 2) outputText("puddle", false);
        ...
        // [else, corr >50]
        else outputText("Your body tenses and " + game.player.sMultiCockDesc() + " hardens at the perspective of a collective anal orgy.  You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\" along with the others.  ", false);
        // end of global condition about PC's corr, display the following
        ...
        if (place <= 1) {
            ...
            if (rand(2) == 0) {
                temp = 3;
                outputText("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order wasn't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths. Nevertheless, your " + game.player.cockDescript(x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n", false);
            }
            else {
                outputText("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + game.player.cockDescript(x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + game.player.cockDescript(x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n", false);
                temp = 1;
            }
            ...
            if ((player.cockTotal() > 2) || (!tent && player.cockTotal() > 1)) {
                ...
                outputText("own, unhindered by the tight colon holding your first " + game.player.cockDescript(x) + ".", false);
            }
            ...
            outputText("\n\nWincing in pain and pleasure mixed, your " + game.player.cockDescript(x) + " lets out one ultimate spurt of goo before resting, deeply buried into your partner's ass.  You fall over, making the whole chain fall to rest by domino effect.  After this torrid anal session, the butt-fuck train has exhausted its fuel.\n\n", false);
        }
        // [else if you didn't win]
        else if (place == 2) {
            outputText("You almost instantly feel firm arms groping your " + buttDescript() + " and the tip of an erect dick knocking at the door of your colon.  It slowly massages your rectum, and you can't help but giggle a bit as you feel a tingle of pleasure passing through your body.\n\n", false);

            // [1/2 chance of this]
            if (rand(2) == 0) outputText("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order weren't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths.  Nevertheless, your " + game.player.cockDescript(x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n", false);
            else outputText("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + game.player.cockDescript(x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + game.player.cockDescript(x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n", false);

            outputText("You suddenly hear a muffled moan from behind.  Someone is probably being fucked at the back of the train.  You brace yourself for the imminent anal penetration... and here it comes.  The dong is projected at full force into your " + buttDescript() + " then all the way inside your colon; you're protected only by the copious glaze of pre-cum its bearer had been applying.", false);
            player.buttChange(5 + rand(15), true, true, false);
            outputText("  Instinctively, you reach for the butt awaiting your " + game.player.cockDescript(x) + " and push as if your life depended on it.  You notice how your sodomite partner does the same to the ass in front of him, and so on.  Eventually the last one is pushed face down onto the floor, but her butt bumps against the dong fucking her, making the cock's owner bump in response, and it all propagates back to your own pecker; you wince in pleasure as your asshole impales itself further on the turgescent member behind you.  You are pushed and pulled rhythmically, your throbbing member and the one fucking you serving both as vectors in this slow wave of anal pleasure.  Unable to keep control of your thrusts, you let go of your body, enjoying the feeling of being shoved in both directions, each motion bringing either a jolt of pleasure in your crotch or your " + buttDescript() + ".  You are being slowly overwhelmed by the delicious tightness of the butt you're fucking as well as the fat piece of meat pounding your own.  The raw feelings eventually prove too much for your horny body, and you jerk your head back and moan like a whore, waiting for the incoming orgasm.\n\n", false);
            tentacleFapSpecial(place);
            outputText("A pressure builds at your crotch as a milky torrent of seed finds its way into your urethra, down your sodomite partner's colon.  Almost simultaneously, you feel an equally thick spray of spooge being released inside your own " + buttDescript() + ". At the peak of your orgasm, you are unable to think of anything but the steady flow of jism coming in and out of you.", false);
            tent = tentacleFapCum();
            // [if player has multicocks]
            if ((player.cockTotal() > 2) || (!tent && player.cockTotal() > 1)) {
                if (player.cockTotal() > 3 || (!tent && player.cockTotal() > 2)) outputText("  Your other cocks also spill ", false);
            ...
            // end of conditions
            outputText("\n\nFinally the strength of all the ejaculations is starting to ebb, as well as your fading orgasm.  Wincing in pain and pleasure mixed, your " + game.player.cockDescript(x) + " lets out an ultimate spurt of goo before resting, deeply buried into your partner's ass.  You fall over, making everyone after fall too by domino effect.  As you slowly doze off, you have a special thought about the still semi-erect cock being lodged in your " + buttDescript() + ".  That wagon of butts and dicks... it felt so right, so powerful, so... connected.\n\n", false);
        }
        // [else, if you lost]
        else {
            outputText("You almost instantly feel firm arms groping your " + buttDescript() + " and the tip of an erect dick knocking at the door of your colon.  It slowly massages your rectum, and you can't help but giggle a bit as you feel a tingle of pleasure passing through your body.  You prepare yourself for the crazy butt-fuck train, with you being at the end of it.\n\n", false);

            outputText("You suddenly hear a muffled moan from behind. Someone is probably being penetrated at the back of the train.  You brace yourself for the imminent anal-fucking... and here it comes.  The dong is projected at full force into your " + buttDescript() + " then all the way inside your colon; you're protected only by the copious glaze of pre-cum its bearer had been applying.", false);
            player.buttChange(5 + rand(10), true, true, false);
            outputText("  You are pushed face down to the floor, bumping your ass against your anal partner's crotch, impaling it further on that fat erect meat.  Hands roughly dig their fingers into the soft flesh of your cheeks as their owner gives you another mad, uncontrolled thrust, an avatar of the train of lust behind him.  Probably the force of thirty people butt-fucking into him was too much for him to bear, so he passes it on to you.  But unlike him, you don't have an asshole to reach and bury your " + game.player.cockDescript(x) + " into, so you are smashed on the floor again, and again, and again.  It hurts, yet it feels so... powerful, so strong.  There is only one dong pounding your " + assDescript() + ", yet it feels like you're being ass-fucked by thirty people at once.  Propelled by the might of thirty thrusts, the huge shaft is tearing its way inside your poor " + assholeDescript() + " and, needless to say, being the end of this line is probably the hardest butt-fuck you'll ever have.\n\n", false);
            tentacleFapSpecial(place);
            outputText("You feel that towering rod grinding against the rough insides of your colon, the friction providing a shamefully amazing sensation of violation.  The large hands are gripping, pinching and groping your " + buttDescript() + " with ease and feverish lust, and your abused flesh feels every tingle, every prickle of it, bringing you to the edge every time.  You know you are being ferociously dominated by that cock, but the pleasure is too good for you to care.  The raw feelings eventually prove too much for your horny body, and you simply lay there in the dust, waiting for the incoming orgasm.  Eventually you feel an enormous spray of spooge being released in your insides.  Almost instantly, you start unloading thick ropes of jism at a steady rate.  Wallowing in a pool of dirt muddied with sexual juices like a worthless slut, you grind your " + buttDescript() + " against the cock harder, in order to milk every drop of semen coming out of it.  But your colon soon reaches its limits and spooge starts dribbling out in small spurts.", false);
            tentacleFapCum();
            // [if player has vagina]
            if (player.hasVagina()) {
                outputText("  Your feminine part squirts a ", false);
        ...
        // [if you didn't win]
            ...
            outputText("You awkwardly remove the dick from your ass and start dressing yourself.  Before you go, the cute little referee waves at you, her coy smile and nude body somehow awakening " + game.player.sMultiCockDesc() + " again.  Ye Gods, it never ends.  \"<i>I hope you enjoyed this session. Better luck next time...</i>\"", false);
            player.orgasm();
            doNext(camp.returnToCampUseOneHour);
        }
        // [if you lost]
        else {
            outputText("You awkwardly remove the dick from your ass, start cleaning yourself from all the dirt and cum before leaving the tent.  Before you go, the nude referee approaches you and gently squeezes your " + game.player.cockDescript(x) + "; you still wince from the pounding it took earlier.  \"<i>Awww, looks like you had a rough time, didn't you?  Well, it happens sometimes.  Hopefully you will get better at this.  See you next game!</i>\"", false);
            player.orgasm();
            doNext(camp.returnToCampUseOneHour);
        }
    ...
    private tentacleFapSpecial(place: number): void {
        ...
        if (player.cocks[x].cockLength >= 40 && player.cocks[x].cockType == CockTypesEnum.TENTACLE) {
            ...
            if (place <= 2) {
                outputText("Needless to say, this interference creates a general disruption in the whole butt-fuck wave.  The whole train of peckers and puckers starts moving irregularly, grinding against your " + game.player.cockDescript(x) + " at the most unexpected moments, and the pleasure you're getting from your extra tentacle cock doesn't help.  Everyone's butt and dick is affected to some extent; everyone's pleasure is increased because of your audacious initiative, and loud, reluctant moans are heard.", false);
