...
export class Roxanne implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (game.time.hours > 23 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00227] > 0) flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00227]--; // Reduce drinking contest bonus
        return false;
    }
...
export function RoxanneAppearance(): () => void {
    // When she there?
    if (game.time.hours > 12 && game.time.hours < 19) {
        // (Not Met)
...
function Roxanne1stApproach(): void {
    outputText("", true);
    spriteSelect(78);
...
function RoxanneChooseApproachOrRepeat(): void {
    spriteSelect(78);
    outputText("", true);
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00221] == 0) {
        ...
        outputText("\"<i>Now, I feel like getting drunk nearly as much as I feel like screwing.  Why don't we have a drinking contest?  If I win, I get to bury this bad-boy in your ass and cum 'til I'm normal.  If you win, I'll give you the best oral service a " + mf(player, "guy", "girl") + " can get!</i>\"\n\n", false);
...
function roxanneDrinkingContestNo(): void {
...
function roxanneDrinkingContestLoseDeliberately(): void {
    flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00226] = 1;
    roxanneDrinkingContest();
}

function roxanneDrinkingContest(): void {
    spriteSelect(78);
    outputText("", true);
    ...
    if (score < (45 + rand(20))) {
        ...
        outputText("Giggling and nearly tripping up on your own " + feet(player) + ", you stumble up to the corrupted deer-taur.  He looks at your wobbling stance, nearly-vacant eyes, and dopey grin before he shakes his head from side to side and says, \"<i>No.</i>\"  ", false);
        ...
        outputText("  A scaled hand slaps your " + buttDescription(player) + " spinning you around to fall drunkenly into the pirate's soft, cushy chest.  \"<i>Don't worry, I'll be gentle,</i>\" she whispers, hooking an arm around your sagging frame.", false);
        // CHOOSE SEX SCENE
    ...
    // [WIN]
        ...
        let fellatio: () => void = null;
        let cunnilingus: () => void = null;
        if (player.cocks.length > 0) fellatio = roxanneGivesABlowjob;
        if (player.vaginas.length > 0) cunnilingus = roxanneCunnilingus;
        simpleChoices("Cunnilingus", cunnilingus, "Fellatio", fellatio, "Rimming", roxanneRimjob, "", null, "", null);
    }
}

// [GET A JOB OF BLOWNESS]
function roxanneGivesABlowjob(): void {
    spriteSelect(78);
    outputText("", true);
    const x: number = player.cocks.biggestCockIndex();
    outputText("You open the lower portion of your " + player.armorName + " and, pulling it back, hang out your " + multiCockDescriptLight(game.player) + "; ", false);
    ...
    outputText(", squeezing you softly while she steadies herself on your " + cockDescript(game.player, x) + ".  She titters, letting a few inches of tongue slip through her lips to envelop your sensitive " + cockHead(player, x) + ", circling the engorged cock-flesh with drunken slobbers.  ", false);
    if (player.balls > 0) outputText("A set of warm, long-nailed fingers squeeze your " + ballsDescriptLight(player) + ", dragging sharp nail-tips along the underside of your sack to tease the poor, cum-packed orbs.  ", false);
    ...
    outputText("You slump back in your chair, sprawling out your " + legs(player) + " around you, completely relaxed by the alcohol flowing through your veins and the exquisite oral service.  Roxanne returns her focus to your " + cockDescript(game.player, x) + ", more tongue spilling from her gaping maw to curl around your lust-engorged shaft.  The drunken lizard's oral organ devours ", false);
    if (player.cocks.cockArea(x) < 60) outputText("the entire length of your " + cockDescript(game.player, x), false);
    else outputText("as much of your over-sized member as she can encompass", false);
    outputText(", constricting, snake-like, to hold you in a slippery, warm embrace.  Suddenly, a pointed nail drags over your taint, pressing just hard enough to hold you still while the shemale pirate works your " + cockDescript(game.player, x) + " with her tongue", false);
    if (player.vaginas.length > 0) outputText(", stopping just short of your female entrance", false);
    outputText(".\n\n", false);

    outputText("Slurping and pumping, the pulsating tongue drags over your " + cockDescript(game.player, x) + ", molding into a perfect cock-sleeve.  At first it's a slow, gradual pump that smears your shaft with booze-flavored spit, but when the first dollop of pre-cum rolls onto the flexible fellatio-tool, the pirate goes into over-drive, pumping with wild abandon.  Her tail whips back and forth, passionately slapping the table, chairs, and dirt in her excitement.\n\n", false);

    outputText("A few demons start to approach, but the lizan crew interposes themselves between their captain and the interlopers, shooing them off before they can start something.  You barely notice, so focused are you on the feel of saliva running down your " + cockDescript(game.player, x) + " and the soft, pliant mouth flesh trying to wring the jism from your shaft.   The warmth of orgasm starts to build in your loins, and you begin to pump at the tongue instinctively, hips rising up off the chair in spite of the sharp fingernail's warning.\n\n", false);

    outputText("SMACK!  Roxanne's tail slaps into your " + buttDescription(player) + ", stinging the exposed " + player.skinDesc + ".  Her eyebrows narrow in irritation while she wraps her arms around your waistline, steadying her off-balance body while she spanks and pumps you in a drunken frenzy.  You hump her face, pressing your " + cockHead(player, x) + " against her lips to smear them with a glaze of leaky pre-cum.  Grabbing her horns, you cry out and pleasure and try to pull her down, but she spanks you, HARD.  You yelp in pain, dropping her horns and submitting completely to her tongue and the pleasure it brings.\n\n", false);
    ...
    outputText(" with cum.  Her soft, oral organ squeezes the spit-slick surface, milking the rest of your cum from your " + cockDescript(game.player, x) + " and " + ballsDescriptLight(player) + ".\n\n", false);
    ...
    outputText(".  \"<i>Jeeze, pent up much?</i>\" she laughs, struggling to stand while giggling at her own supposed cleverness.  Happy with how the contest worked out, you slip " + sMultiCockDesc(game.player) + " into your " + player.armorName + " and wobble off back to camp.", false);
    // (-100 lust, -1 int)
    player.orgasm();
    dynStats("int", -1);
...
function roxanneCunnilingus(): void {
    spriteSelect(78);
    outputText("", true);
    outputText("You shimmy out of your " + player.armorName + " and lean back, exposing your " + vaginaDescript(player) + " to the drunken lizan's maw.  She wobbles back and forth for a moment, clutching at your " + legs(player) + " to steady herself before she leans in, brushing her scaled nose against your vulva.  You shudder from the sudden contact with her scales, but she doesn't rush it, instead huffing and sniffing at the moist entrance.  Her alcohol-lidded eyes gaze up at you, the bleary orbs slightly confused as her jaw slowly opens to release her tongue.\n\n", false);
    ...
    if (player.vaginas.wetness() >= 4) outputText("juice-dribbling ", false);
    else if (player.vaginas.wetness() >= 2) outputText("juicy ", false);
    outputText("entrance of your loins.  You swoon, leaning against the chair's back and scooting your crotch forward in a fit of wanton need.  The tip of Roxanne's nose disappears into the moist lips, her slick, oral organ engulfed by your lusty tunnel.  It squirms and wriggles inside you, the thick pink protrusion lashing about, swirling in a circular motion to lap the juice from your walls.  Spit begins to drip down your " + legs(player) + " and you see Roxanne's saliva frothing at the corners of her mouth while she works you over.\n\n", false);

    outputText("Face blushing red, you groan and hump at the lizan, grabbing her horns so that you can smear her nose into your " + clitDescription(player) + ".  She pulls back, resisting your insistent pulls with inebriated strength, but before you can let go she reverses direction and smashes her forehead into your gut, winding you.  Your fingers release immediately, clutching at your aching middle while the angry lizard snarls, \"<i>You're getting oral, NOT using me as a dildo!  ...but I am sorry I hit you that hard. I just needed to breathe.  Relax and let me make it up to you...</i>\"\n\n", false);

    outputText("The drunken lizan puts her soft, lightly scaled fingers around you to squeeze at your " + buttDescription(player) + " and leans in to give your " + clitDescription(player) + " a tender kiss.  Her massive tongue ", false);
    ...
    if (player.breastRows.hasFuckableNipples()) {
        ...
        if (player.breastRows.biggestLactation() < 1.5) outputText("expertly as the one below", false);
    ...
    if (player.vaginas.wetness() >= 5) outputText(" and squirt", false);
    ...
    if (player.cocks.length > 0) outputText(SMultiCockDesc(game.player) + " bounces on her head, between her horns, spilling a trail of sticky seed down her back.  ", false);
...
function roxanneRimjob(): void {
    spriteSelect(78);
    outputText("", true);
    outputText("You shimmy out of your " + player.armorName + " and smirk, turning around to lean onto the table", false);
    if (player.breastRows.biggestTitSize() >= 2) outputText(", your breasts cushioning you from the hard wood below", false);
    ...
    if (player.vaginas.length > 0) outputText("Meanwhile, your neglected box is getting wetter and wetter, with no sign of stopping.  The lizan's massive, questing tongue ignores it.  ", false);
    ...
    if (player.cocks.length > 0) {
        outputText("Surprisingly, one of the pirate's hands comes off your " + buttDescription(player) + " to tug on ", false);
    ...
    else if (player.vaginas.length > 0) outputText("Surprisingly, one of the pirate's hands comes off your " + buttDescription(player) + " to play with your " + vaginaDescript(player) + ".  Long, sharp nails play over the sensitive skin of your vulva while the supple, scaled thumb presses between them.  Juicy girl-cum quickly coats her fingers, but it doesn't seem to bother her in the least.\n\n", false);

    outputText("The long, wondrous tongue pushes forward, wriggling inside you.  At first it's just the tip, but the drunken lizard forces inch after inch inside you with constant intensity.  You can feel her spit frothing around your " + assholeDescript(player) + ", bubbling while she fills your backdoor with ever larger quantities of saliva-covered tongue.  Seeming to go on forever, the penetration robs you of strength, filling you with dozens more inches.  Roxanne hums at your dark hole, vibrating her tongue inside you while it arches to caress the most sensitive spots.\n\n", false);

    outputText("Orgasm hits you powerfully, your " + assholeDescript(player) + " doing its best to choke the massive, pink tongue inside it.  You throw back your head and howl with pleasure, bumping your " + buttDescription(player) + " into your lizan lover's nose unconsciously.  She gives you a knowing squeeze while her tongue keeps up the attack, forcing waves of unfiltered, raw pleasure into your cerebellum.", false);
    if (player.cocks.length > 0) {
        ...
        if (player.vaginas.wetness() >= 4) outputText("soaking them with your gushing orgasms ", false);
        else outputText("sliming them with copious girl-cum ", false);
        outputText("while she teases your " + clitDescription(player) + " with those terribly-slow touches.", false);
    }
    outputText("\n\n", false);
...
function roxanneReamsYouNormal(): void {
    spriteSelect(78);
    outputText("", true);
    ...
    outputText("You giggle underneath the lizan, your face mopping up the frothy brew that spilled during the previous bout of binge drinking.  Lapping it up, you forget about your predicament as you adjust to the warm, scaled body overtop you.  Roxanne leaves you to your distractions while she wiggles out of her pants, her tail swaying to counterbalance the now-freed bulk of her member.  The sodden shaft rises to bump your butt, dripping with pre-cum as it wiggles between your cheeks.  It prods your " + assholeDescript(player) + ", rousing you from your drunken stupor and earning a look back, but the pirate begins to tongue the edge of your ear, making you giggle and relax into sexy, alcohol-lubricated acquiescence.\n\n", false);
    ...
    buttChange(player, Math.floor(30 + (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00225] / 4)), true, true, false);
    outputText("\n\n", false);
    ...
    else if (player.vaginas.length > 0) outputText("against your " + vaginaDescript(player) + ", the sweaty orbs growing slick with the moisture of your growing need", false);
    ...
    if (player.tailType > TailType.NONE) outputText("tail-hole", false);
    else outputText("anus", false);
    outputText(".  It begins to feel good", false);
    if (player.cocks.length > 0) outputText(", very good,", false);
    ...
    outputText("You shake your head no, and are rewarded for your disobedience with a violent, butt-jiggling tail-whip.  Gasping from the pain, your " + assholeDescript(player) + " contracts involuntarily, milking Roxanne's member for a few more drops of pre.  The aggressive lizard really gets into it, her massive, DD-cup tits pinning you to the table while she smacks your cheeks with her flexible lizan tail.   Pounding and slapping you, she abuses you for every dick-milking squeeze you'll give her, ", false);
    if (player.perks.findByType(PerkLib.Masochist) < 0) outputText("until you're voluntarily working your abdominal muscles to avoid the pain.", false);
    ...
    // (DICKS:
    if (player.cocks.length > 0) {
        outputText("The constant pressure of her thrusts seems to flow straight to " + sMultiCockDesc(game.player) + " as if each of Roxanne's prostate-pounding bumps is injecting you with her lust.  ", false);
        if (player.cocks.length == 1) outputText("It bounces against your belly with your heartbeats, loving the forced anal pressure.  ", false);
        ...
        if (player.vaginas.length > 0) outputText("  To your delight and shame, your pussy is equally aroused by the action, musky and wet from the pirate's lewd anal battering.", false);
        outputText("\n\n", false);
    }
    // (JUST CUNTS:
    else if (player.vaginas.length > 0) {
        outputText("The constant pressure of her thrusts seems to flow straight to your " + vaginaDescript(player) + ", inflaming your moistening labia.  You can feel her mass compressing your feminine organs, rubbing your inner walls against each other.  The sublime internal friction releases your natural lubricants, ", false);
        if (player.vaginas.wetness() < 2) outputText("sliming your ready vulva.", false);
        else if (player.vaginas.wetness() <= 3) outputText("soaking your ready vulva.", false);
        else if (player.vaginas.wetness() <= 4) outputText("which drip from your ready vulva.", false);
    ...
    if (player.cocks.length > 0) {
        outputText("  ", false);
        if (player.cumQ() >= 1500) {
            outputText("A torrential outpouring of spunk pours from " + sMultiCockDesc(game.player) + ", leaking in a steady, submissive flow to grow a ", false);
    ...
    if (player.breastRows.biggestLactation() >= 2) outputText("  Bursts of milk erupt from your pinned " + nippleDescription(player, 0) + "s, blasting out while you cum to stain the table white.", false);
    outputText("  As it winds down, you fall into slumber, snoring happily after your violation.\n\n", false);

    outputText("<b>LATER...</b>\n", false);
    outputText("You wake in the lizan's bed<b> with a nasty hangover</b>, her arm curled around your gurgling belly.  From how sore your rear feels, she kept 'winning' at least two or three more times.  Your head is pounding, your " + legs(player) + " are weak, and you dribble cum with every movement.  It takes some doing to extricate yourself from Roxanne's slumbering form, but you find your equipment and leave, hanging your head in shame under the leering eyes of the caravan-goers.", false);
...
function roxanneFucksYourAssOHGODITSHUGE(): void {
    spriteSelect(78);
    outputText("", true);
    ...
    outputText("\"<i>Ohhh, I've waited for this for so long " + player.short + ".  You have no idea of the need... it just builds and builds, getting bigger and bigger until every step is a cacophony of pleasure that never ends.  I'm never sated... never rested... I can't even please myself.  I just have to let it grow... and grow until I can find a drunk " + mf(player, "boy", "slut") + " like you to slake my need and give me reprieve from the curse,</i>\" explains Roxanne as she shreds her own clothes in a frenzy.  Her lithe, scaly body glitters in the candle-light, the pendulous swell of her breasts nearly threatening to pull your eyes from the blood-engorged beast between her thighs.  Lashing from side to side while she advances, the lizan's tail betrays the rampant emotions surging through her.\n\n", false);

    outputText("The nude newhalf pounces, her thigh-sized monster-cock gushing pre-cum over your belly and twitching in anticipation of the violation to come.  Her need is so cute... endearing even, but it isn't until your gear is being removed that you remember Roxanne intends to hilt that thing inside you.  Your eyes go wide, your mouth stammering and blubbering about how it will never fit.  She places a scaled finger on your boozy lips and whispers, \"<i>Shhhh, don't struggle.  You knew what you were getting into, " + mf(player, "my dear", "sexy girl") + ".  You wouldn't welch out on a wager, would you?  Besides, it's much too late for you to do anything... my nude, drunken cock-holster.</i>\"\n\n", false);

    outputText("Oh gods, Roxanne's right.  She finished getting you naked while she was talking, and you're completely exposed.  Her hands grab your arm and pull, rolling you onto your front and letting your " + legs(player) + " hang off the side of her bed toward the floor.  It's such a silly pose that you nearly forget how perfect a target it makes your " + buttDescription(player) + "!  The now-confident lizan gives your butt-cheek a slap, giggling when you start in surprise.  ", false);
    if (player.tailType > TailType.NONE) outputText("She lifts your tail ", false);
    else outputText("She spreads your cheeks ", false);
    outputText("to examine your " + assholeDescript(player) + ", licking the ring before she prods it with a gentle finger-tip.  The excited pirate exclaims, \"<i>", false);
    if (player.analCapacity() < 150) outputText("Oh, poor " + mf(player, "boy", "girl") + "!  I don't think you can handle me like this.  Let me get the ointment... after all, I want you to come back for more.", false);
    ...
    outputText("-slicked ass to accept ever-greater quantities of cock.  Sharp fingernails drag over your " + hipDescription(player), false);
    if (player.skinType == SkinType.PLAIN) outputText(", leaving red lines in your unblemished skin", false);
    ...
    if (player.skinType == SkinType.SCALES) outputText("a roughly-scratched 'X' on your scales", false);
    ...
    if (player.cocks.length > 0 && player.cumQ() >= 1000) outputText(", drooling spit into the huge puddle of cum you made on her bed", false);
    else if (player.vaginas.length > 0 && player.vaginas.wetness() >= 4) outputText(", drooling spit into the sloppy puddy of fem-cum you splattered on the mattress", false);
    outputText(".", false);
...
function bigBootyRoxanneContestLoss(): void {
    spriteSelect(78);
    clearOutput();
    outputText("Gods, your head is swimming!  The room is pitching from side to side, and you " + mf(player, "chuckle", "giggle") + " as you idly wonder if this is what it would be like aboard Roxanne's ship.  Still, the well-endowed shemale doesn't seem to mind your tipsy bumbling in the slightest.  She hooks her hands under your [butt] and squeezes the spacious ass-flesh appreciatively, then growls, \"<i>Such a nice, round ass; perfect for stuffing with cock!  You're wasted as a champion.  You'd make a better cabin-" + mf(player, "boy", "girl") + ",</i>\" into your ear as she drags you towards her wagon.");
    ...
    if (player.vaginas.length === 0) outputText(" \"<i>Oh, I know you don't have a pussy, but tonight, you're my big-bootied, cum-dumpster bitch!</i>\"");
    ...
    outputText("\n\nThe drunken, scaled dick-girl pulls most of the way out a few seconds later, teasing, \"<i>Feeling empty, dear?  I'll fill you up, my cabin-" + mf(player, "boy", "girl") + ",</i>\" before plunging back in.  Roxanne's hips begin to pump your gaping asshole with practiced efficiency, every stroke smearing her rounded cock-tip [if (hasCock = true) \"across your pre-greased prostate.  You cannot help but moan in forced bliss.\" else \"through every sensitive part of your body, hard enough to make you shiver from the onslaught of sensation.\"] Roxanne cries out ecstatically and shoots large globules of fluid into your rectum.  Muffled, wet squishes and audible liquid churning can be heard from your guts, but you're pretty certain it's still only pre-cum.");
    ...
    outputText("\n\n<b>LATER...</b>\n");
    outputText("You wake in the lizan's bed, her arm curled around your gurgling belly.  From how sore your rear feels, she probably kept 'winning' at least two or three more times.  Your head is pounding, your " + legs(player) + " are weak, and you dribble cum from your ass with every movement.  It takes some doing to extricate yourself from Roxanne's slumbering form, but you find your equipment and leave, hanging your head in shame under the leering eyes of the caravan-goers.");
    // (-100 lust, -1 int, hangover effect)
    player.orgasm();
    dynStats("int", -1);
    applyHangover();
