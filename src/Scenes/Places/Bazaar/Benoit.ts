...
function benoitKnocksUpPCCheck(): void {
    ...
    // Knock up chances:
    if ((player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.perks.findByType(PerkLib.HarpyWomb) >= 0 || player.perks.findByType(PerkLib.Oviposition) >= 0 || player.perks.findByType(PerkLib.BasiliskWomb) >= 0) && (player.pregnancyIncubation == 0 || player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)) {
        if (player.perks.findByType(PerkLib.BasiliskWomb) >= 0 && flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 1) {
...
export function benoitInClutch(): boolean {
    ...
    const startDay: number = flags[kFLAGS.FEMOIT_NEXTDAY_EVENT];
    const currDay: number = game.time.days;
...
export function benoitKnockUp(): boolean {
    ...
    const bounty: boolean = (player.perks.findByType(PerkLib.ElvenBounty) >= 0) ? true : false;
    const stud: boolean = (player.perks.findByType(PerkLib.MaraesGiftStud) >= 0) ? true : false;
    const alpha: boolean = (player.perks.findByType(PerkLib.FerasBoonAlpha) >= 0) ? true : false;
...
export function benoitIntro(): void {
    ...
    if (player.cocks.length > 0 && flags[kFLAGS.BENOIT_STATUS] > 0 && player.lust >= 33) {
    ...
    if (flags[kFLAGS.BENOIT_WOMB_TALK_UNLOCKED] == 1 && player.perks.findByType(PerkLib.BasiliskWomb) < 0 && flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 0 && flags[kFLAGS.BENOIT_STATUS] == 0) womb = tryToConvertToBassyWomb;
...
function eggySuggest(): void {
    ...
    if (player.isTaur()) {
        ...
        if (player.inHeat) outputText(" By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans.");
    ...
    player.orgasm();
    if ((player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.perks.findByType(PerkLib.HarpyWomb) >= 0 || player.perks.findByType(PerkLib.Oviposition) >= 0) && (player.pregnancyIncubation == 0 || player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)) {
...
function firstTimeAfterBoningEncounterBenoit(): void {
    ...
    outputText("A strange, faint noise is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug he is polishing, and beckons you inside.");
...
function repeatSexWithBenoitLetHim(): void {
    ...
    if (player.isTaur()) {
        ...
        if (player.breastRows.biggestTitSize() >= 1) outputText("and [fullChest] ");
        ...
        outputText("For the moment you don't do anything; you simply stand back and let his hands slowly move across your frame.  One of his hands comes to rest upon your " + nippleDescription(player, 0) + "; as he gently teases and kneads the soft, sensitive flesh his other hand drift downwards, across your belly, around over the crack of your [butt] then down to cup your behind.  Although he is familiar with your frame by now, Benoit never seems to stop being enthralled by your body; there is an unconscious frown of concentration on his face as his smooth hands move across your warm skin, as if he were mapping you in his mind's eye.");
...
function repeatBenoitFuckTakeCharge(): void {
    ...
    if (player.isTaur()) {
        ...
        // Large capacity:
        else outputText("You slide back down onto him, cooing this time as your groin meets his muscular thighs with a wet kiss, your sex swallowing his cock whole.  You begin to ride him hard and slow, bending his cock upwards to push at your sensitive walls, waves of pleasure starting to flow through you.");
        ...
        outputText("\n\nYour lust ratchets up as his warm hands continue to move over your behind, and at last losing your discipline you begin to thrust into him with abandon, eager now for your peak.  Benoit pants as you really begin to fuck him and thrusts with you; the wetness of your arousal spatters against his thighs as your body slaps into the basilisk's tight muscles.  He makes a harsh, bestial noise when he cums; his blind eyes roll as he clutches your back and shoots surge after surge of cum into your churning depths.  The warmth of his jizz and his helpless bucking thrust you to your own orgasm; irrepressible pulses of pleasure overwhelm your mind and you can do nothing for long minutes except cry and squeal against the basilisk, clutching him back as you work his dick for every last drop.");
        // [Herm:
        if (player.cocks.length > 0) outputText("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
...
function tryToConvertToBassyWomb(): void {
    ...
    // Ingredients in inventory:
        ...
        // [No oviposition:
        if (player.perks.findByType(PerkLib.Oviposition) < 0) outputText("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
...
function suggestSexAfterBasiWombed(later: boolean = true): void {
    ...
    if (!player.isTaur()) {
        ...
        else outputText("Benoit staggers into the back room, bumping into several things as he sets you down, thankfully onto a clear space on the ground.  He leans into you, his weight pushing you onto the ground, hands flying over your [armor], fumbling off clasps and belts when he finds them.  Now that your gut has settled you feel imbued with an odd, ripe sensation; your belly bulges ever-so-slightly with unfertilized eggs and you feel red, soft and ready.  You work with him, wriggling out of your clothes and ripping his own trousers off, spreading your [hips] eagerly as his long, smooth prick springs to attention.");
        ...
        if (player.cocks.length > 0) outputText("  He deliberately rubs himself up and down your body, and the [cock] trapped between your warm bodies quickly hardens against the warm, smooth friction.");
        ...
        // [Herm:
        if (player.cocks.length > 0) outputText("  He deliberately moves his hand over [oneCock] before trapping it in his warm grasp.   It quickly hardens against his warm, smooth friction.");
    ...
    player.perks.create(PerkLib.BasiliskWomb, 0, 0, 0, 0);
    outputText("\n\n(<b>Perk Unlocked: Basilisk Womb - You can now give birth to female basilisks.</b>)");
    if (player.perks.findByType(PerkLib.Oviposition) < 0) {
...
export function popOutBenoitEggs(): void {
    ...
    if (player.cocks.length > 0) outputText(" and a downpour of semen");
    outputText(".");
...
export function femoitNextDayEvent(): void {
    ...
    outputText("A strange, faint sound is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug she is polishing, and beckons you inside.");
...
export function femoitFirstTimeYes(): void {
    ...
    if (player.cocks.biggestCockLength() < 15 && player.balls == 0) outputText("  Eventually you manage to hilt yourself entirely in her depths, your stomach pressing against her own tight belly.");
    else if (player.cocks.biggestCockLength() < 15 && player.balls > 0) outputText("  Eventually you manage to hilt yourself in her depths, your [balls] pressing into her sex as your stomach bumps into her own tight belly.");
    else if (player.cocks.biggestCockLength() >= 15 && player.balls == 0) outputText("  Eventually you manage to bottom out, your dick pressed against her cervix.");
    else if (player.cocks.biggestCockLength() >= 15 && player.balls > 0) outputText("  Eventually you manage to bottom out, your dick pressed against her cervix, your [balls] swinging heavily below your shaft.");
...
export function femoitSexIntro(): void {
    ...
    if (!benoitPreggers()) {
        ...
        outputText("\n\n");
        if (player.cocks.biggestCockLength() < 15) outputText("Soon you are hilting yourself in her depths, making her gasp as you slap into her.");
    ...
    else if (benoitRegularPreggers() && (!player.isTaur() || (player.isTaur() && (player.tallness * (5 / 6) < player.cocks[player.cocks.longestCock()].cockLength)))) {
        clearOutput();
