...
export class Cinnabar extends BazaarAbstractContent {
    ...
    private cinnabarNonHugeDickings(): void {
        ...
        if (player.hasCock() && !player.hasVagina()) outputText("  Unbidden, " + game.player.sMultiCockDesc() + " begins to harden, stiffening with every brush against your prostate.", false);
        ...
        if (player.hasCock() && !player.hasVagina()) outputText("You gasp and moan when you feel her other hand wrap around your " + game.player.cockDescript(0) + ", pulling and tugging on it until it starts to leak a trail of squeezed-out man-slime on the mattress.  ", false);
        ...
        if (player.hasCock() && !player.hasVagina()) {
            outputText("  Her soft pads grip your " + game.player.cockDescript(0) + " tightly while she  presses on your prostate, milking your cum from you with professional skill.  Rather than coming in its usual spurts, it drips in a long, liquid chain of white-goo that drops into a submissive puddle on the bed.", false);
            if (player.cumQ() >= 250) {
    ...
    private fuckCinnabarHugerAndCorruptedEr(): void {
        ...
        outputText(", squirting juices from the callous, cunt-piercing thrust.  The " + player.cockHead(x) + " of your " + game.player.cockDescript(x) + " can be seen just below her neck", false);
        ...
        else outputText(", and only now do you realize you blew past her cervix long ago, the tight ring hugging your " + game.player.cockDescript(x) + " in a clutching embrace.", false);
        outputText("\n\n", false);
        ...
        outputText("  A half-second later your " + game.player.cockDescript(x) + " joins it, your internal muscles clenching and convulsing, heaving the jizz from your body in one huge, gooey blast.\n\n", false);
    ...
    private cinnabarHuger(): void {
        ...
        outputText("True to her words, she starts squeezing you through her fur, massaging your " + game.player.cockDescript(x) + " by dragging her stretchy puss along your length with her fingertips.  It's too much, her body is fucking perfect for your " + game.player.cockDescript(x) + ", and you feel warmth building inside you.  Her tail twists around your ", false);
        if (player.balls > 0) outputText(ballsDescriptLight(), false);
        else outputText("base", false);
        outputText(" to prod at your " + assholeDescript() + ", sliding in to push your prostate.  She hits it hard enough to make you pump cum with enough force to distort her already-ballooned body, and you go into an orgasmic frenzy, dragging your " + game.player.cockDescript(x) + " back to her entrance before jamming in all ", false);
    ...
    private cinnabarHuge(): void {
        ...
        outputText("", true);
        spriteSelect(91);
        outputText("You're almost there, and though you can feel the burgeoning warmth growing underneath your " + game.player.cockDescript(x) + ", you're determined to push the whole way inside her before you blow.  Cinnabar crams her muzzle through a particularly large mirror and locks onto your lips, ramming her tongue deeper into your throat, pushing harder with each inch of tightly-packed cock you shove inside her.  At last you bottom out, your " + player.cockHead(x) + " bumps the tight ring of her cervix while her tail pulls tight about your ", false);
        if (player.balls > 0) outputText(ballsDescriptLight(), false);
    ...
    private cinnabarMultiCockPortalFuckFest(): void {
        ...
        outputText("Cinnabar's hands fly through your " + player.armorName + ", diving through your undergarments to squeeze " + game.player.sMultiCockDesc() + ".  ", false);
        ...
        outputText("  Your " + player.legs() + " go wobbly from the delicate touches dancing through your " + game.player.multiCockDescriptLight() + ", making it hard to stand.  Cinnabar slides ", false);
        ...
        outputText("Your " + hipDescript() + " rock forwards of their own volition.  There's no way your " + game.player.multiCockDescriptLight() + " can resist such a lewd, needy specimen, or the chance to fuck her in so many different ways simultaneously.  First, your " + game.player.cockDescript(x) + " hits her swollen pussy, and as you batter your way inside the slippery entrance, her body rewards you with a gush of girl-cum that soaks your manhood and drips over your " + game.player.cockDescript(y) + ".  A split-second later that second member is pressing at her anus, forcing through the clenched star in a violent, animalistic thrust.\n\n", false);

        outputText("Cinnabar moans, utterly and completely delighted, but you muffle those moans by spearing her throat on your " + game.player.cockDescript(z) + ", filling her oral orifice completely.  You reach through a pair of portals at arm height to squeeze her spread ass-cheeks, holding tight while you push forwards to completely bury your three lucky members in furry fuck-sleeves.", false);
        ...
        outputText(".  Slippery-soft warmth wreathes your " + game.player.cockDescript(x) + ", the plush black love-tunnel contracting slightly to happily squeeze you every few seconds.  The darker, sinfully tight star of Cinnabar's anus glazes with your pre-cum, her ass-cheeks wobbling perfectly in your hands with each rhythmic penetration.", false);
        ...
        outputText("  The " + game.player.cockDescript(z) + " in her mouth feels so good it nearly makes you jump, the long pink tongue licking around it while a wet muzzle buries itself in your crotch.  Gods, it's too much!\n\n", false);

        outputText("You feel yourself cumming, that tell-tale warmth in your gut mixing with a few involuntary muscle-contractions that bounce your " + game.player.multiCockDescriptLight() + " inside Cinnabar.  You turn your head to watch her, " + num2Text(player.cockTotal()) + " penises pumping from holes in the wall to ravage her body completely and totally, her eyes rolled back in complete bliss.   Heat blooms in your " + game.player.multiCockDescriptLight() + ", the pumping gouts of seed deep into all of the slut's holes simultaneously.  She screams into your " + game.player.cockDescript(z) + ", vibrations making your orgasm that much stronger.  It drags on and on, the spit-roasted rat shaking while she's completely taken in every way", false);
        ...
        outputText("The slut-rat gargles in irritation around your sensitive, cummed-out cock, clearly displeased that you're still inside her.  You step back from the wall, your " + player.legs() + " giving out from the sensory overload of unsheathing your " + game.player.multiCockDescriptLight() + " simultaneously to drop you flat on your " + assDescript() + ".  At the same time, Cinnabar starts coughing and groaning, clearly exhausted from the one-" + player.mf("man", "woman") + " gangbang.  You understand how that feels; just touching " + game.player.sMultiCockDesc() + " makes your head swoon.\n\n", false);

        outputText("An incessant licking reaches your ears, getting your attention.  You stand up, careful about your " + game.player.multiCockDescriptLight() + " while you look towards the bed.  The portals are closed down, and Cinnabar is bent double, folded in half like a contortionist", false);
