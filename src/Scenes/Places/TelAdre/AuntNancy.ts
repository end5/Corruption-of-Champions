...
function timeForAuntNancySpiderCooch(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 1) outputText(" and taking the opportunity to grope your " + allBreastsDescript(player) + " right through your " + player.armorName, false);
    outputText(".\n\n", false);
...
function auntNancyPoonPartII(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (!titFucked && player.breastRows.biggestLactation() >= 1) {
...
function auntNancyPoonPartIII(): void {
    ...
    // [Else:]
        ...
        if (player.vaginas.wetness() >= 5 || player.breastRows.biggestLactation() >= 1)
