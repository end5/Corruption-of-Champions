...
export class Edryn implements TimeAwareInterface {
    // VARS
...
export function edrynBarTalk(): void {
    ...
    outputText("Edryn smiles pleasantly as you approach, ", false);
    if (player.lowerBody == LowerBodyType.CENTAUR) outputText("offering you a spot at the table across from her.  You realize your companion isn't on any kind of seat at all, and is instead 'sitting' on her lower half.  You do the same, settling down across from her, thankful this bar was made to accommodate centaurs.", false);
    ...
    if (player.lowerBody == LowerBodyType.CENTAUR && player.cocks.length > 0) {
        ...
        outputText(", now rock-hard.  You breathe deeply, your mind subsumed in a sea of equine instincts.  The hard floor squeezes your " + cockDescript(game.player, 0) + " painfully underneath you, forcing you to rise up onto your hooves.  You can feel more than a few curious gazes sliding along your now fully exposed maleness, and it twitches as if it were happy for the attention.\n\n", false);
        outputText("Swaying back and forth, alternatively snorting and breathing deeply of the female's scent, your gaze immediately locks onto the returning mare.  Your " + cockDescript(game.player, 0) + " jumps and bounces underneath you, painfully hard and swollen.  A thick dollop of pre beads on your " + cockHead(player) + ", as if the bar needed your display to be any more overtly sexual.  Edryn looks you over, walking alongside you she talks, \"<i>My my, someone liked what they saw.  Or was it smelled?  I can never tell what it is that gets you " + mf(player, "studs", "hotties") + " so worked up about me.</i>\"\n\n", false);
        outputText("A soft hand slides under your belly, hefting your " + cockDescript(game.player, 0) + " and smearing the bead of pre over it with long slick strokes.  She coos, \"<i>", false);
        dynStats("lus", 70);
        edrynOffer();
        return;
    }
        ...
        // Non horsedick
            ...
            if (player.cor > 50 || player.effects.getValue1Of(StatusAffects.Edryn) > 0 || flags[kFLAGS.LOW_STANDARDS_FOR_ALL]) {
                ...
                if (player.cocks[0].cockType == CockTypesEnum.TENTACLE) outputText("  She smiles coyly when your " + cockDescript(game.player, 0) + " wriggles, wrapping around her arm.", false);
                outputText("\n\n", false);
                dynStats("lus", 40);
                edrynOffer();
                return;
...
function edrynOffer(): void {
    ...
    if (player.lowerBody == LowerBodyType.CENTAUR && player.cocks.length > 0) {
        ...
        outputText("</i>\"\n\n", false);
        outputText("She releases your " + cockDescript(game.player, x) + " and looks expectantly at you.  Your body is ready to do anything for a chance to breed her.  What will you do?\n\n", false);
        ...
        if (player.cocks.horseCocks() > 0) {
            ...
            if (player.cocks.cockArea(x) >= 24) {
                ...
                outputText(cockDescript(game.player, x) + " twitch.  Will you accept her proposition and give her what she wants?\n\n", false);
                if (cost > 0) outputText("(Do you pay " + num2Text(cost) + " gems to fuck her?)", false);
                else outputText("(Do you fuck her?)", false);
                doYesNo(edrynSexSelecter, TelAdre.barTelAdre);
            }
...
function fuckEdrynTaur(): void {
    ...
    if (player.breastRows.biggestTitSize() <= 1) outputText("strokes your chest", false);
...
function fuckEdrynNonTaur(): void {
    ...
    outputText("She whinnies and pushes back against you, forcing your entire " + cockDescript(game.player, x) + " to slip up her well-lubed passage.  You slap her ass, watching the furred flanks jiggle slightly.  Edryn looks over her shoulder again to and gives you a sultry look.  Before you can puzzle out what she's trying to tell you, her entire cunt contracts tightly, like a silken vise.  It wrings your " + cockDescript(game.player, x) + " tightly, immobilizing you inside the powerful centauress and making your " + legs(player) + " feel wobbly and weak.\n\n", false);
...
function threesomeEdrynAndHel(): void {
    ...
    outputText("Hel gets to work without further prompting, grabbing Edryn's wide flanks and burying her face into the centauress's horse-cunt. You grab your " + cockDescript(game.player, x) + ", stroking yourself off to the show unfolding before you.  Edryn whinnies happily, wiggling her hips into Hel's face, crushing the salamander further and further into the wall.\n\n", false);
    ...
    outputText("Hel rolls over on her back, using her legs to steady on Edryn's back as you rub your " + cockDescript(game.player, x) + " against Edryn's gaping horsecunt, making the centauress shudder.  The shudder nearly throws Hel off her back, causing the salamander to dig her claws painfully into Edryn's sides.  She bucks back in surprise, impaling herself right on your " + cockDescript(game.player, 0) + ".\n\n", false);
...
export function fuckPregEdryn(): void {
    ...
    if (player.lowerBody != LowerBodyType.CENTAUR) {
        ...
        if (player.cumQ() < 100) {
            outputText("Your " + cockDescript(game.player, x) + " dumps the last of its seed into her hungry cunt and softens slightly.  Her warm cunt's constant milking motions continue, still pleasurable, but they prevent you from going soft until the centaur's orgasm has concluded.", false);
        }
        else if (player.cumQ() < 500) {
            outputText("Your " + cockDescript(game.player, x) + " spurts thick flows of seed into her hungry cunt, flooding her snatch with an abnormally large load of semen.  Her warm cunt's constant milking motions continue, forcing more and more of your spunk inside her until her own orgasm has concluded.", false);
        }
        else {
            outputText("Your " + cockDescript(game.player, x) + " erupts, pouring a thick wave of seed into the centaur's hungry cunt.  The constant milking motions don't let up while Edryn's orgasm continues, helping you flood her with even more spooge.  Even after your pregnant lover's orgasm has concluded, your " + cockDescript(game.player, x) + " keeps dumping more cum into her until it's soaking and dripping from her huge, furry ass.", false);
        }
    ...
    // TAURZILLA
        ...
        outputText("The centauress pulls back and moans, \"<i>Oh gods yes, it feels so much better pregnant!  My pu-pu-ahhhh-ussy is so much WETTER.  It's like I've got a faucet back there!  And I'm soo-ohhhh sensitive!  Ung fuck " + mf(player, "stud", "dear") + " don't stop.  Please don't stop!</i>\"\n\n", false);
...
function jizzFromEatingPregdrynOut(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas[0].vaginalWetness == VaginaWetness.SLAVERING) outputText("squirting", false);
...
function edrynFucktroduction(): void {
    ...
    if (!player.isTaur()) {
        ...
        if (player.cocks[0].cockType == CockTypesEnum.HORSE) outputText("making your " + cockDescript(game.player, x) + " twitch and ache.  It floods your body with animalistic desires, overpowering any sense of propriety you might have once had.", false);
...
export function eatEdrynPussyLikeABawss(): void {
    ...
    if (player.tongueType > TongueType.HUMAN) outputText("She's not even begun to taste the fruits of your talents, and you let your long, long tongue spool out inside her, pressing hard on her walls and tenderly flicking across each sensitive fold inside her.  ");
    ...
    outputText("\n\nArching your back, you allow the palpable relief to wash through you, bubbling lust pumping out from your [balls] to flood the poor centaur's slippery, equine slit.  You ejaculate with forceful lurches of your [hips], matching each rope of creamy jism by burying your " + cockDescript(game.player, x));
