...
export class Katherine implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (game.time.hours == 6) {
            ...
            const clothesPref: number = flags[kFLAGS.KATHERINE_CLOTHES_PREF];
            ...
            if (clothesPref < 0) { // She’s been told what to wear by the PC
                ...
                const clothes: number = flags[kFLAGS.KATHERINE_CLOTHES];
...
const KBIT_LOVER_AMILY: number = 1;
const KBIT_LOVER_COTTON: number = 2;
const KBIT_LOVER_EDRYN: number = 4;
const KBIT_LOVER_HELIA: number = 8;
const KBIT_LOVER_URTA: number = 16;
const KBIT_LOVER_VALA: number = 32;
    ...
    if (isAt(KLOC_URTAS_APT)) { // This option happens if you and Urta have no children
        ...
        switch (rand(4)) {
            ...
            default:
                ...
                outputText("When Kath spots you she gives you a big smile and you hear “<i>" + (player.vaginas.length > 0 ? "Mommy" : "Daddy") + "!</i>” from your offspring, who " + (kids > 1 ? "run over and hug" : "runs over and hugs") + " your legs.\n\n");
...
export function katherineOnDuty(): void {
    ...
    let getOral: () => void = null;
    let penetratePussy: () => void = null;
    let penetrateAss: () => void = null;
    if (player.gender > 0) getOral = katherineGivesPCOralAllDayLongDotJPG;
    if (player.cocks.length > 0 && (player.cocks.cockThatFits(70) >= 0)) {
...
function talkClothes(): void {
    ...
    const optionSSRobe: () => void = (hasClothing(KBIT_CLOTHES_SS_ROBE) ? talkClothesSSRobe : null);
    const optionTubeTop: () => void = (hasClothing(KBIT_CLOTHES_TUBETOP) ? talkClothesTubeTop : null);
    const optionNurseOutfit: () => void = (hasClothing(KBIT_CLOTHES_NURSECL) ? talkClothesNurseOutfit : null);
...
function talkClothesBlouse(): void {
    ...
    const insist: () => void = (submissiveness() > 1 ? talkClothesBlouseInsist : null);
...
function talkClothesBodysuit(): void {
    ...
    const insist: () => void = (submissiveness() > 1 ? talkClothesBodysuitInsist : null);
...
function talkClothesDress(): void {
    ...
    const insist: () => void = (submissiveness() > 1 ? talkClothesDressInsist : null);
...
function talkClothesSSRobe(): void {
    ...
    const insist: () => void = (submissiveness() > 1 ? talkClothesSSRobeInsist : null);
...
function talkClothesTubeTop(): void {
    ...
    const insist: () => void = (submissiveness() > 1 ? talkClothesTubeTopInsist : null);
...
function talkClothesNurseOutfit(): void {
    ...
    const insist: () => void = (submissiveness() > 1 ? talkClothesNurseOutfitInsist : null);
...
function talkLovers(): void {
    ...
    let loverCount: number = 0;
    let optionAmily: () => void = null;
    let optionCotton: () => void = null;
    let optionEdryn: () => void = null;
    let optionHelia: () => void = null;
    let optionUrta: () => void = null;
    let optionVala: () => void = null;
...
function talkLoversEdryn(): void {
    ...
    if (flags[kFLAGS.KATHERINE_EDRYN_AFFECTION] == 0) {
        const numLovers: number = knownLovers();
        const edrynKids: number = flags[kFLAGS.EDRYN_NUMBER_OF_KIDS];
...
function talkMilkMenu(): void {
    ...
    const canShareWithUrta: boolean = flags[kFLAGS.KATHERINE_URTA_AFFECTION] > 10;
    const canShareWithVala: boolean = flags[kFLAGS.KATHERINE_VALA_AFFECTION] > 10;
    const sharingWithFriends: boolean = milkOption(KBIT_MILK_SHARE_WITH_FRIENDS);
...
function useReductoOnKat(): void {
    ...
    if (getCockLength() <= dickMin && getBallSize() <= 1 && getKnotSize() <= 2 && breasts.cupSize == BreastCup.A) {
        ...
        const breasts: () => void = (breasts.cupSize > BreastCup.A ? useRreductoOnKatsBreasts : null);
...
function giveKatRefuzzFormulaChange(): void {
    ...
    outputText((player.vaginas.length > 0 ? " and moistening your cunt" : "") + ".\n\n");
...
function giveKatWhiskerFruit(): void {
    ...
    const doneBefore: boolean = doneSubmissive(KBIT_SUB_CAT_DICK);
...
function katSexMenu(): void {
    ...
    let helix: () => void = null;
    let suckle: () => void = null;
    if (player.lust >= 33) { // No penetration or helix if you’re at low lust
        if (player.cocks.length > 0) {
            if (player.cocks.cockThatFits(70) >= 0) penetrate = katPenetrate;
        }
        if (hasCock()) {
            getPen = letKatKnotYou;
            if (player.cocks.length > 0 && player.vaginas.length > 0 && player.cocks.cockThatFits(70) >= 0) helix = katDoubleHelixCraziness;
        }
    }
    if (player.lactationQ() > 0 && player.breastRows.biggestLactation() >= 1 && player.breastRows.biggestTitSize() >= 1) {
        if (doneSubmissive(KBIT_SUB_START_LACTATION))
            suckle = suckleMenu;
    ...
    if (isAt(KLOC_KATHS_APT)) {
        const bedBond: () => void = (hasAlready(KBIT_TRIED_BEDROOM_BONDAGE) || player.gems >= 40 ? bedroomBondage : null);
...
function katherineDrunkSeeUrta(): void {
    ...
    if (Urta.urtaDrunk()) {
        ...
        simpleChoices("Lick Out", KatherineThreesome.kathLicksOutUrta, "Sandwich", (hasCock() ? KatherineThreesome.sandwich : null),
            "Knothole", (player.gender != Gender.NONE && hasCock() ? KatherineThreesome.knothole : null), "", null, "", null); // Do not show knothole button for genderless
    }
...
function katPenetrate(): void {
    ...
    const vagina: () => void = penetrateKatsVag;
    const anus: () => void = pcPenetratesKatAnally;
...
export function penetrateKatsVag(): void {
    ...
    if (player.vaginas.length > 0) outputText("Your " + vaginaDescript(player) + " releases its own girl-cum in sympathy, even as y");
    else outputText("Y");
...
export function pcPenetratesKatDoubly(): void {
    ...
    const wombShot: boolean = player.cocks[x].cockLength > 14 && pregSize() == 0; // Only available if you’re huge and she doesn’t have a bun in the oven
    if (wombShot) {
        const cockLength: number = Math.round(player.cocks[x].cockLength);
    ...
    if (player.cumQ() < 500)
        ...
        if (wombShot) {
            ...
            if (player.cor >= 75) { // First time you have to be corrupt
...
export function suckNFuck(): void {
    ...
    const kathCum: number = cumQ();
    const playerCum: number = player.cumQ();
    if (kathCum <= 500)
...
function letKatKnotYou(): void {
    ...
    if (player.cocks.length > 0 && flags[kFLAGS.KATHERINE_TIMES_SEXED] > 0) sukn = suckedNFuckedByKat;
    const backroomFuck: () => void = (isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED) ? drunkFuck : null);
...
function letKatKnotYouCommonDialogue(inAlleyBehindBar: boolean): void {
    ...
    const hasPenetrated: boolean = hasAlready(KBIT_TRIED_GIVE_VAGINAL | KBIT_TRIED_GIVE_ANAL | KBIT_TRIED_GIVE_DOUBLE_PEN | KBIT_TRIED_GIVE_SUCK_N_FUCK);
    ...
    outputText("The " + cockType("mismatched ", "") + "herm steps up behind you and gives you a playful slap on your " + buttDescription(player) + ".  “<i>Well, I");
    if (hasDogCock())
    ...
    if (player.vaginas.length > 0) outputText("  “<i>So, which hole do you want me to use?</i>”");
}
...
export function letKatKnotYourCuntPussyFuck(): void {
    ...
    if (player.isNaga() || player.tailType == TailType.LIZARD || player.tailType == TailType.DEMONIC) outputText(", snaking your tail up between her breasts and playfully stroking her cheek,");
    ...
    outputText("You shudder and gasp as your own climax suddenly rocks through you, femcum splashing from your " + vaginaDescript(player));
    if (player.cocks.length > 0) {
...
export function getPenetrated(): void {
    ...
    if (isAt(KLOC_BAR) || isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED)) letKatKnotYouCommonDialogue(true); // At the bar
    outputText("You indicate to Katherine that you want it in your " + assholeDescript(player) + ".\n\n");

    outputText("“<i>Well, I can't say I'm a big fan of the idea, but okay...</i>”  Her" + catGirl("", " furry") + " hands promptly begin to rub possessively over your " + buttDescription(player) + ", slowly moving up to take hold of your " + hipDescription(player) + ".  “<i>... if that's what you want.</i>”  You feel her rubbing her " + cockMultiple("", "bottom-most ") + "cock against your anus, letting your cheeks feel its " + cockType("rubbery-smooth", "barb-covered") + " length, then, drawing back her hips, she suddenly thrusts it between them without any hesitation" + cockMultiple("", ", as her upper cock bounces along your back") + ".\n\n");
    ...
    if (harpyScore(player) >= 4 || sharkScore(player) >= 4 || catScore(player) >= 4 || dogScore(player) >= 4 || bunnyScore(player) >= 4)
    ...
    outputText("You moan and gasp, thrusting your ass back into your feline lover's midriff to facilitate your fucking");
    if (player.isNaga() || player.tailType == TailType.LIZARD || player.tailType == TailType.DEMONIC) {
    ...
    if (player.vaginas.length > 0) outputText(", femcum splashing from your " + vaginaDescript(player));
    if (player.cocks.length > 0) {
        outputText((player.vaginas.length > 0 ? " and " : ", ") + sMultiCockDesc(game.player) + " spurting ");
        if (player.cumQ() < 25) outputText("drops");
        else if (player.cumQ() < 100) outputText("splashes");
        else if (player.cumQ() < 250) outputText("puddles");
        else outputText("a veritable lake of spunk" + (isAt(KLOC_KATHS_APT) ? "onto the bed" : "into the alleyway"));
...
export function getDoublePennedByKat(): void {
    ...
    if (player.cocks.length > 0) outputText("  " + SMultiCockDesc(game.player) + (player.cocks.length > 1 ? " celebrate with arcs of their" : " celebrates with arcs of its") + " own semen, spraying them in a patter on her legs and the ground in front of you.");
...
export function giveKatOralPenisWingWang(): void {
    ...
    if (getCockNumber() == 1) {
        ...
        // [Naga tongue]
        if (player.tongueType == TongueType.SNAKE)
        ...
        // [Demonic tongue]
        else if (player.tongueType == TongueType.DEMONIC)
        ...
        if (player.tongueType == TongueType.SNAKE) outputText("molesting her clit with your tongue");
        else if (player.tongueType == TongueType.DEMONIC) outputText("penetrating her with your tongue as you give her head");
    ...
    // Double Cock Scene
        ...
        outputText("Seeing one of her cocks bob and twitch as your face gets closer you decide to reward the eagerness and start with it.  You open your lips and lick the tip before slowly sliding down her crown and then lower, onto the shaft and down to the base.  Then, you pull your lips up, assaulting her with your tongue all along the way.  Her " + cockType() + " cock leaves your mouth with a loud slurp and you quickly go down the other one.  Katherine strokes your " + hairDescription(player) + ", very gently helping push you down her shaft.\n\n");
        ...
        if (player.tongueType == TongueType.SNAKE || player.tongueType == TongueType.DEMONIC) outputText("  Said tongue wraps around one erection and then the other, stroking gently with its long, flexible length.");
        outputText("\n\n");
        ...
        if (hasBalls()) {
            ...
            if (player.tongueType == TongueType.SNAKE || player.tongueType == TongueType.DEMONIC) outputText("  Your long, flexible tongue once again slides out as you gently lift her testes up, easily dipping inside her wet pussy and stroking her inner walls as you suck.");
        ...
        // ([Normal/Naga Tongue]
        if (player.tongueType != TongueType.DEMONIC) {
            ...
            if (player.isNaga() || player.tailType == TailType.DEMONIC) outputText("  Wanting to grant her maximum stimulation and complete release, you slip the tip of your tail inside the cat-girl, feeling her pussy gratefully squeeze it.");
            outputText("\n\n");
    ...
    if (player.tongueType == TongueType.DEMONIC) outputText("She runs her fingers along your lips.  “<i>I think I might be falling in love with the monster inside that pretty cave.</i>”\n\n");
...
export function giveKatOralPussyLicking(): void { // This version assumes Kath has no cock
    ...
    if (player.tongueType == TongueType.SNAKE)
        outputText("You finally make full use of your long, flexible forked tongue.  You tighten the abnormal muscles given to you by your transformation and slip it all the way in, the forked tip bumping against the back of her vagina even as the base of your tongue undulates against the underside of her clit.\n\n");
    else if (player.tongueType == TongueType.DEMONIC)
        outputText("You finally make full use of your long and flexible demonic tongue.  You tighten the abnormal muscles given to you by your transformation and slip it all the way in, the tip bumping against the back of her vagina even as the base of your tongue undulates against the underside of her clit.\n\n");
...
function katherineGivesPCOralAllDayLongDotJPG(): void {
    ...
    if (player.cocks.length > 0 && (player.gender == 1 || rand(2) == 0)) doNext(katherineLicksAllTheBoyPenises);
...
function katherineGoesDownOnTheGirlsOhYahBabyLesbo(): void {
    ...
    outputText("Without hesitation, she licks you again, and again, deep strokes that slide up from the very base of your cunny all the way to the tip, lingering on your " + clitDescription(player) + " and teasing your lovebutton.  You can't help but wrap your " + legs(player) + " around her head and pull her into your crotch, thrusting her face right into your cunt.\n\n");
...
function katherineLicksAllTheBoyPenises(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.balls > 0) outputText(", past your " + ballsDescriptLight(player) + ",");
    ...
    if (player.vaginas.length > 0) {
        outputText("  Her fingers stroke and piston inside you with the synchronicity only another hermaphrodite could have perfected, rolling your " + clitDescription(player) + " and sending further sparks of pleasure coursing through your brain.");
    }
    outputText("\n\n");
...
function katDoubleHelixCraziness(): void {
    ...
    if (player.vaginas.wetness() <= 3) outputText("drizzling");
    else if (player.vaginas.wetness() <= 4) outputText("spurting");
    ...
    outputText(".  Katherine's knotty cock has locked her into your " + vaginaDescript(player) + ", leaving you tied together like a dog and his bitch.  ");
    if (hasAlready(KBIT_TRIED_DOUBLE_HELIX)) {
    ...
    if (player.balls > 0) outputText(", and even licking your " + ballsDescriptLight(player) + " clean");
    ...
    outputText("Katherine rolls you on your back and begins licking your belly, cleaning you of the sweat produced by your vigorous fucking earlier.  She moves through your " + chestDesc(game.player) + ", stopping shortly to give each of your " + nippleDescription(player, 0) + "s a little peck.  Then she continues to your neck and face.\n\n");
    ...
    if (player.isNaga() || player.tailType == TailType.DEMONIC) outputText("the tip of your tail");
    else outputText("your " + buttDescription(player));
...
function suckleFromKath(): void {
    ...
    const wasntLactating: boolean = breasts.lactationLevel == BreastStore.LACTATION_NONE;
...
function suckleTacularKats(): void {
    ...
    if (isAt(KLOC_BAR) || isAt(KLOC_BAR_DRUNK) || isAt(KLOC_BAR_URTA_REFUSED)) { // At the bar
        ...
        if (player.eyeType == EyeType.FOUR_SPIDER_EYES) outputText("both sets of ");
...
export function repeatAmbushKatherineRecruitMent(): void {
    ...
    if (player.cor < 75) outputText(" hope she’s here so you can get to know her better.");
        ...
        if (player.cocks.length > 0 && player.vaginas.length > 0) outputText("r dick swells and your pussy moistens");
        else if (player.cocks.length > 0) outputText("r dick swells");
        else if (player.vaginas.length > 0) outputText("r pussy moistens");
...
export function handjobbiesFurrDemCatFurries(): void {
    ...
    if (player.cocks.length > 0) outputText(", causing your " + multiCockDescriptLight(player) + " to rise to the occasion.");
    else if (player.vaginas.length > 0) outputText(", teasing your hot little slit.");
    // (genderless)
...
function bathTimeCentaurRide(): void {
    outputText("\n\nKath holds you for a while, just grinding against you slowly and laying kiss after kiss on your lips and neck.  Finally she lets go and walks along your flank.  She stops beside you, pets your side and jumps onto your back.  She reaches around your torso and " + (player.breastRows.hasBreasts() ? "starts to play with your breasts" : "runs her hands over your manly chest") + ".  Not satisfied with just that Kath begins grinding against you, " + (hasCock() ? "the " + cockMultiple("tip of her cock", "tips of her cocks") : "her pussy") + " grinding against what would be your ass if you were a human.  As a centaur it’s the sensitive spot where your spine curves.  Her " + (hasCock() ? "cockhead" + cockMultiple(" keeps", "s keep") : "groin keeps") + " rubbing against the small depression there, creating lovely sensations that travel in both directions.  At one end of your body your cock" + (player.cocks.length > 1 ? "s grow" : " grows") + " even harder, at the other your throw your head back and start to moan.\n\n");
...
function bathTimeCentaurPenetrated(): void {
    ...
    if (hasCock()) {
        ...
        if (player.vaginas.length > 0 && player.isPregnant()) {
            ...
            if (player.vaginas.length > 0) // Non-pregnant female/herm
    ...
    else outputText(player.vaginas.length > 0 ? "knucles grinding against the entrance to your womb" : "fingers flexing deep inside your colon");
...
function bathTimeFrustrated(): void { // You took away her cock. This is your fault
    ...
    outputText("Seconds become minutes and there's no end in sight.  You realize that the glacial stream is hindering your efforts.  You're so cold you can barely feel Kath's body pressing against yours.  Her fingers are still rubbing you" + (player.vaginas.length > 0 ? "r clit but even that bundle of nerves" : ", but every bit of your skin") + " is growing numb.  Your own hand is starting to cramp up and the only reason you're certain it's still rubbing against her pussy is that her thighs are keeping it in place.\n\n");
...
function bathTimePenetrated(): void {
    if (player.vaginas.length === 0) { // Genderless or Male - get this out of the way since it will be a little different
        ...
        if (getFurry())
            outputText("slick fur rubbing against your " + (player.skinType == SkinType.FUR ? "own" : "skin"));
        else
            outputText("wet skin " + (player.skinType == SkinType.FUR ? "rubbing against your fur" : "sliding over your body"));
        ...
        outputText("“<i>" + (playerMaster() ? "Thank you [master], I’m sorry if I got a bit carried away" : "Thanks [name], you’re the best") + ",</i>” she purrs.  You drag your sex kitten back to the river bank and gather your clothes in case you need to make a run for it." + (player.cocks.length > 0 ? "" : "  Despite your lack of endowments you feel sated.") + "  Kath is so relaxed she might qualify as a liquid");
        ...
        if (flags[kFLAGS.KATHERINE_UNLOCKED] >= 4 && (player.cor >= 75 || doneSubmissive(KBIT_SUB_ORGASM_DENIAL))) {
...
function bathTimePenetratedNormalEnd(): void {
    ...
    outputText("You assure her it was good for you too" + (player.cocks.length > 0 ? ", despite the fact you didn't cum" : "") + ", then you both get dressed and head back to Tel’Adre.");
...
function bedroomBondage(): void {
    ...
    const x: number = player.cocks.cockThatFits(70);
    if (player.cocks.length > 0 && x > -1) { // Males and herms (with small enough junk to fit)
        const pcLen: number = player.cocks[x].cockLength;
        ...
        outputText("\n\nYou put an end to her babbling by kissing her while you allow your " + (dPen ? "holes to swallow her cocks" : (player.vaginas.length > 0 ? "pussy" : "rectum") + " to swallow up her cock") + ".  Kath’s hips weakly try to push more inside, but her hot and solid knot" + (dPen ? "s prevent" : " prevents") + " it.\n\n");
...
function drunkFuck(): void {
    ...
    outputText("Once you’re naked she " + (player.vaginas.length > 0 ? "starts roughly playing with your clit" : "jabs a finger and then two inside your ass") + ".  She’s not gentle, aiming only to get you hotter and hotter.  Kath leans in close and whispers, “<i>It’s my turn now.  You wanted me - so for now I’m callin the shots.</i>”  She ");
    if (player.vaginas.length > 0)
