...
export class Niamh implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (player.effects.findByType(StatusAffects.BimboChampagne) >= 0) {
            player.effects.addValue(StatusAffects.BimboChampagne, 1, -1);
            if (player.effects.getValue1Of(StatusAffects.BimboChampagne) <= 0) {
                removeBimboChampagne();
                needNext = true;
            }
        }
        if (player.effects.getValue1Of(StatusAffects.BlackCatBeer) > 0) {
            player.effects.addValue(StatusAffects.BlackCatBeer, 1, -1);
            if (player.effects.getValue1Of(StatusAffects.BlackCatBeer) <= 0) {
                blackCatBeerExpires();
                needNext = true;
            }
...
export function telAdreNiamh(): void {
    if (flags[kFLAGS.MET_NIAMH] == 0) {
        ...
        if (game.time.hours <= 8) outputText("massive N-cup breasts, spilling over the table");
        else if (game.time.hours <= 9) outputText("huge M-cup breasts, taking up much of the table");
        else if (game.time.hours <= 10) outputText("great L-cup breasts, advancing far onto the table");
        else if (game.time.hours <= 11) outputText("heavy K-cup breasts, lying on the table for support");
        else if (game.time.hours <= 12) outputText("hefty J-cup breasts, resting comfortably on the table");
        else if (game.time.hours <= 13) outputText("weighty I-cup breasts, dangling to lie on the table");
        else if (game.time.hours <= 14) outputText("squeezable H-cup breasts, settling burdensomely on her chest");
    ...
    // This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
        ...
        if (game.time.hours <= 8) outputText("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
        else if (game.time.hours <= 14) {
            outputText("her ");
            if (game.time.hours <= 9) outputText("M-cup");
            else if (game.time.hours <= 10) outputText("L-cup");
            else if (game.time.hours <= 11) outputText("K-cup");
            else if (game.time.hours <= 12) outputText("J-cup");
            else if (game.time.hours <= 13) outputText("I-cup");
...
// Approach:
export function approachNiamh(): void {
    clearOutput();
...
// [Get Beer]
export function getANiamhBeer(): void {
    clearOutput();
...
// [Here]
function drinkNiamhsBeerInTelAdre(): void {
    clearOutput();
    if (player.lactationQ() >= 300 && player.breastRows.biggestTitSize() >= 5 && rand(2) == 0 && flags[kFLAGS.MET_NIAMH] > 0) {
...
// [To Go]
function blackCatBeerToGo(): void {
    clearOutput();
...
function talkToNiamh(): void {
    ...
    if (flags[kFLAGS.TALKED_NIAMH] == 0) {
        ...
        if (game.time.hours < 15) outputText("who usually sits over yonder ");
...
export function blackCatBeerEffects(player: Player, clearScreen: boolean = true, newLine: boolean = false): void {
    ...
    let lib: number = 0;
    if (player.effects.findByType(StatusAffects.BlackCatBeer) >= 0) {
        if (100 - player.lib >= 10) lib = 10;
        else lib = 100 - player.lib;
        player.effects.addValue(StatusAffects.BlackCatBeer, 1, 4);
        player.effects.addValue(StatusAffects.BlackCatBeer, 2, lib);
        ...
        player.effects.create(StatusAffects.BlackCatBeer, 8, lib, 0, 0);
...
export function blackCatBeerExpires(): void {
    dynStats("spe", 4.5, "int", 4.5, "lib", (-1 * player.effects.getValue2Of(StatusAffects.BlackCatBeer)));
    player.effects.remove(StatusAffects.BlackCatBeer);
...
function giveNiamphBimboLiquer(): void {
    taintNiamh(true);
}

function giveNiamphSuccubiMilk(): void {
    taintNiamh(false);
}
...
function corruptOrBimboNiamhIntro(): void {
    clearOutput();
...
// [Maybe Later]
export function maybeLaterNiamh(): void {
    clearOutput();
...
export function taintNiamh(bimbo: boolean = false): void {
    clearOutput();
...
export function niamhPostTelAdreMoveOut(): void {
...
// [no]
function niamhCorruptMobileSnackTurnDown(): void {
    clearOutput();
...
// [yup]
function niamhCorruptedMobileSnackDrinkTime(): void {
    clearOutput();
...
export function bimboChampagne(player: Player, clearScreen: boolean, intro: boolean): void {
    if (clearScreen) clearOutput();
    if (intro) {
        if ((player.perks.findByType(PerkLib.FutaFaculties) >= 0 && player.perks.findByType(PerkLib.FutaForm) >= 0) || (player.perks.findByType(PerkLib.BimboBody) >= 0 && player.perks.findByType(PerkLib.BimboBrains) >= 0)) {
        ...
        else if (player.effects.findByType(StatusAffects.BimboChampagne) < 0) outputText("You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor.  Breathing deeply, you open your mouth and begin pouring the ever-effervescent fluid inside.  It's sweet and slightly gooey, and the feel of it sliding down your throat is intensely... awesome?  Like, totally!");
    ...
    if (player.effects.findByType(StatusAffects.BimboChampagne) >= 0) {
        player.effects.addValue(StatusAffects.BimboChampagne, 1, 4);
        dynStats("spe", -2, "lib", 1, "lus", 10);
    }
    else {
        player.effects.create(StatusAffects.BimboChampagne, 8, 0, 0, 0);
        ...
        if (player.breastRows[0].breastRating < 5) {
            ...
            player.effects.setValue(StatusAffects.BimboChampagne, 2, 5 - player.breastRows.biggestTitSize());
        ...
        if (player.buttRating < 12) {
            ...
            player.effects.setValue(StatusAffects.BimboChampagne, 4, 12 - player.buttRating);
...
export function removeBimboChampagne(): void {
    ...
    if (player.effects.getValue2Of(StatusAffects.BimboChampagne) > 0) {
        player.breastRows[0].breastRating -= player.effects.getValue2Of(StatusAffects.BimboChampagne);
    ...
    player.effects.remove(StatusAffects.BimboChampagne);
    player.genderCheck();
    outputText("\n");
}

export function bazaarNiamh(): void {
    clearOutput();
    // Bimbo Niamh:
    if (flags[kFLAGS.NIAMH_STATUS] == 2) {
...
function bazaardNiamhDrink(): void {
    clearOutput();
    player.gems -= 2;
...
function getBimboChampFromNiamh(): void {
    clearOutput();
...
export function seanBimboBrewing(): void {
    clearOutput();
    ...
    simpleChoices("Yep", yep, "", null, "", null, "", null, "Back", incubusShop);
}

// yeah I got this
function yeahSeanLetsBimbooze(): void {
    clearOutput();
...
// 24 hours later
export function getBimboozeFromSean(): void {
    clearOutput();
...
function sellYourBooze(): void {
    ...
    if (player.breastRows.length == 1) outputText("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
...
function barBeerOrgyTits(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 20) outputText("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
    ...
    if ((player.pregnancyIncubation == 0 || player.pregnancyIncubation > 100) && player.breastRows.length > 1) outputText("  You are essentially a pile of " + boozeBoobsType() + " filled tits.");
    // [IF player is hugely pregnant with four or more tits]
    else if (player.breastRows.length > 1) outputText("  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");
    ...
    if (player.breastRows.hasFuckableNipples()) outputText("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + num2Text(player.breastRows.totalNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
    ...
    if (player.effects.getValue1Of(StatusAffects.Edryn) >= 5 && player.gender == 3 && game.time.hours == 14 && flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
        ...
        if (player.breastRows.length == 2) outputText("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
        // IF [player has six giant breasts]
        else if (player.breastRows.length == 3) outputText("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
    ...
    // IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy]
        ...
        if (player.breastRows.length > 1) outputText("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
...
function boozeBoobsType(): string {
    // New Years
