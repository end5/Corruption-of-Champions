...
export function circlejerk(): void { // Not available if Kath has no cock
    clearOutput();
    ...
    outputText("As soon as Urta’s top is off her, you move your hands to grope at her soft orbs, pinching and twisting her nipples, then pulling her head against your " + (player.breastRows.hasBreasts() ? breastDescriptOfRow(player, 0) : "chest") + ", and finally giving her perky fox-ear a gentle bite");
    ...
    if (player.cocks.length > 0) outputText("; [eachCock] erects, nice and ready for what’s to come");
    else if (player.vaginas.length > 0) outputText("; your pussy is drenched with your juices, some of it dripping on Urta’s tail");
    outputText(".");
    ...
    if (player.cocks.length > 0)
        outputText("wrapping around your cock" + (player.vaginas.length > 0 ? " and " : ""));
    if (player.vaginas.length > 0) outputText("gently teasing around your cunt");
    ...
    if (player.cocks.length > 0) outputText("   Her hand teases your [cock] with the soft pads on the tips of her fingers; every once in awhile, she reaches up to collect a dollop of pre and smear it along your shaft, making it easier for her to tease you into defeat.");
    if (player.vaginas.length > 0) outputText("   Her " + (player.cocks.length > 0 ? "other " : "") + "hand teases your labia by gently circling it, the fur tickling your entrance so deliciously, you can’t help but buck against her wandering finger, trying to capture it within your moist walls.");
    ...
    const kathCumQ: number = Katherine.cumQ();
    if (kathCumQ < 500)
    ...
    else if (player.vaginas.length > 0) { // Female
    ...
    if (player.cocks.length > 0) { // Male or herm
        if (player.cumQ() < 500)
    ...
    outputText("The two morphic herms moan and growl lustily, eagerly rubbing their stiff, mismatched dicks across your stomach, your " + (player.breastRows.hasBreasts() ? breastDescriptOfRow(player, 0) : "chest") + ", your [hips]");
    if (player.cocks.length > 0) outputText(" and especially against your " + cockDescript(player));
...
export function threeSixtyNine(): void {
    clearOutput();
    if (Katherine.isAt(Katherine.KLOC_BAR) || Katherine.isAt(Katherine.KLOC_BAR_DRUNK))
        outputText("You drag the " + (Katherine.getCockNumber() > 0 ? "eager herms" : "lusty pair") + " to one of the backrooms.  Urta locks the door");
    ...
    outputText("Kath moans and tries to thrust her hips forward but Urta holds her back.  " + (Katherine.getCockNumber() > 0 ? "While she continues to toy with Kath’s cock Urta uses her other hand to slip a couple of fingers into Kath’s pussy.  Not to be outdone Kath returns the favor" : "While her tongue explores the depths of Kath's love tunnel Urta rubs her nose against Kath's clit.  Not to be outdone Kath jams her fingers into Urta's slick cunt") + ".  In no time it’s like they’re locked in combat - each trying to force the other to lose control" + (Katherine.getCockNumber() > 0 ? "; trying to push their shaft deep down the other’s throat" : "") + ".\n\n");
    ...
    outputText(" like it’s a lollipop.  While Kath is distracted you come from behind and force your shaft between her thighs.  Urta " + (Katherine.getCockNumber() > 0 ? "uses her hand to guide" : "pulls her tongue out at the last second, guiding") + " you into Kath’s quim and you feel the cat " + Katherine.catGirl("girl", "morph") + " stiffen up.\n\n");
    ...
    const canImpregUrta: boolean = (flags[kFLAGS.URTA_FERTILE] == 1) && (!Urta.pregnancy.isPregnant);
    let doKath: boolean;
    if (Katherine.getFertile() && (Katherine.pregSize() == 0)) {
    ...
    if (player.cumQ() < 1500) {
        if (!doKath && !Katherine.getCockNumber() > 0)
    ...
    if (doKath || Katherine.getCockNumber() > 0) outputText("You feel a bit of resistance - after all there’s little space left for " + (doKath ? "Katherine" : "Urta") + "’s belly to expand.  ");
    ...
    outputText("You pull free and roll away.  It’s not that you want to, but you’ve got things to attend to.  " + (Katherine.getCockNumber() > 0 ? "The two herms" : "Kath and Urta") + " separate and lie back on the floor exhausted.  As you pass them, heading for the door, you notice they’re holding hands.  It’s actually quite a romantic scene.");
...
export function roastYou(): void { // Not available if Kath has no cock
    clearOutput();
    if (Katherine.isAt(Katherine.KLOC_BAR) || Katherine.isAt(Katherine.KLOC_BAR_DRUNK))
    ...
    if (player.breastRows.biggestTitRow() > BreastCup.DD)
    ...
    if (Katherine.getCockNumber() > 1)
        outputText("s of her cocks press firmly against " + (player.vaginas.length > 0 ? "both your holes" : "your ass"));
    else outputText(" of her cock presses firmly against your " + (player.vaginas.length > 0 ? "cunt" : "ass"));
    outputText(".\n\nKath pushes your legs apart with her knees and starts to force ");
    if (Katherine.getCockNumber() == 1)
        outputText("her " + Katherine.getCockLength() + " inch " + Katherine.cockType("doggy dick", "cat cock") + " into your " + (player.vaginas.length > 0 ? "pussy" : "rectum"));
    else if (player.vaginas.length > 0)
        outputText("her twin " + Katherine.cockType() + " shafts into your body");
    else outputText("her " + Katherine.cockType() + "  cocks into your body, caring little that your ass wasn’t built to take her twin " + Katherine.getCockLength() + " inch shafts");
    ...
    outputText("“<i>Oh yeah, that’s a good " + mf(player, "boy", "girl") + "!</i>” Urta shouts, loud enough for " + (Katherine.isAt(Katherine.KLOC_BAR) || Katherine.isAt(Katherine.KLOC_BAR_DRUNK) ? "anyone at the bar" : "people on the street") + " to hear.  The two herms start to work in tandem - Kath pulling out as Urta shoves her cock deeper and then Urta pulling back far enough for you to tongue the tip of her cock while Kath sinks in up to her knot" + Katherine.cockMultiple("", "s") + ".\n\n");
    ...
    if (player.vaginas.length === 0)
        outputText(" your ass");
    else if (Katherine.getCockNumber() > 1)
    ...
    outputText("Kath rocks her knot" + Katherine.cockMultiple("", "s") + " back and forth and whines.  Urta grins like a wolf and says “<i>You can do it Kath.  " + player.short + " asked for it, now give it to " + mf(player, "him", "her") + ".</i>”  Urta stands on wobbly legs and moves out of sight.  You’re still so full and out of breath that you can’t do anything to stop her, even if you wanted.\n\n");
    outputText("You hear a surprised “<i>Oh</i>” from Katherine.  Then Urta’s voice, low and sexy, “<i>");
    if (Katherine.getBallSize() > 0)
    ...
    switch (player.gender) {
        case 0: // Genderless
            outputText("“<i>Yes, of course I’ll help you,</i>” says Urta.  You don’t know what she’s doing back there, but you can hear Katherine’s breaths getting faster and sharper.  “<i>Kath, I’m going to keep milking your prostate until you pump every drop inside " + player.short + "’s ass to remind " + mf(player, "him", "her") + " how much fun genitals can be.</i>”\n\n");
...
export function spitroastKath(): void { // Works for all except genderless characters
    clearOutput();
    const atBar: boolean = Katherine.isAt(Katherine.KLOC_BAR) || Katherine.isAt(Katherine.KLOC_BAR_DRUNK);
    if (atBar)
        outputText("You lead the " + (Katherine.getCockNumber() > 0 ? "horny herms" : "lusty pair") + " toward one of the bar’s backrooms.  You get inside, lock the door and when you turn around you see that Urta has grabbed Kath from behind to make sure she couldn’t jump you.");
    else
    ...
    if (player.isTaur())
        outputText("her back is horizontal.  You carefully step over her, closing the distance with Urta until you feel your submissive pussy cat " + (player.cocks.length > 0 ? "swallow the tip of your cock" : "latch onto your cunt like a remora") + ".  You pull Urta in for a kiss and think to yourself that this is how a centaur was meant to fuck");
    else
        outputText("her mouth is lined up with your " + (player.cocks.length > 0 ? "cock" : "pussy") + ".  Your submissive pussy cat doesn’t need any instruction to " + (player.cocks.length > 0 ? "swallow the tip of your cock" : "slam her tongue into the depths of your pussy"));
    outputText(".\n\nUrta thrusts and drives Katherine’s ");
    if (player.cocks.length > 0)
        outputText("lips down your shaft. " + (player.cocks.longestCock() >= 12 ? " Despite your length" : ""));
    else
        outputText("face into your groin. ");
    outputText(" Kath takes it without complaint and her rough tongue does wonders for your mood and " + (player.cocks.length > 0 ? "member" : "your moist cunt") + ".\n\n");
    ...
    if (Katherine.getCockNumber() > 0) outputText("A hot spray of cum lands on and around your " + (player.isTaur() ? "rear " : "") + (player.isNaga() ? "tail" : "feet") + ".  ");
    outputText("Kath just came and if you had to guess her clenching pussy sent Urta over the edge.\n\n");
    if (player.cocks.length === 0 || player.cumQ() < 3000)
    ...
    outputText(".\n\nUrta pants like she’s just run a marathon and Kath" + (player.cocks.length > 0 ? ", her mouth still occupied by your shaft," : "") + " just moans contentedly.  You’re quite happy yourself and sit down with them for a minute to rest.\n\n");
...
export function pinAndFuck(): void { // Kath is sober, Urta is drunk //Not available if Kath has no cock
    clearOutput();
    outputText("When you get to Urta’s table she pulls you both in for a hug and takes the opportunity to bury her head between Katherine’s breasts" + (player.breastRows.hasBreasts() ? " and then between yours" : "") + ".  Not wanting to fuck your herms at the table you get Urta’s attention by tugging on her ear and motion at one of the vacant backrooms.  The drunken fox herm gets up and wanders toward the door, her arms around you both.  It’s not for support but so that she can grope all your favorite bits.  On Urta’s other side you can see Kath smiling as Urta massages the base of her tail.\n\n");
...
function pinAndFuckMountUrta(): void { // Plays for anyone without a cock and for herms who select this option
    clearOutput();
    outputText("You roll Kath and Urta over so you can get access to Urta’s equine cock.  It’s a little soft but a couple of strokes along its cum slicked length start to change that.  Urta opens her eyes and tries to focus on you but you’re already " + (player.vaginas.length > 0 ? "sliding your pussy against her shaft" : "rubbing her horsecock against your anus") + ".  Underneath you both Kath lets out a happy “<i>Oh!</i>”  You’re guessing Urta’s muscles are clamping down on Kath’s cock" + Katherine.cockMultiple("", "s") + ".\n\n");
    ...
    if (player.vaginas.length === 0)
        ...
        else if (player.effects.findByType(StatusAffects.Contraceptives) >= 0)
    ...
    outputText("  Urta starts to moan; now that her oversensitive dick is buried in your " + (player.vaginas.length > 0 ? "pussy" : "rectum") + " she’s really getting into it.  You flex the muscles of your " + (player.vaginas.length > 0 ? "vagina" : "ass") + " to please her more.  Kath helps out by reaching around Urta and playing with her nipples while you massage her balls and tell her you want to be filled.\n\n");
    ...
    outputText("Kath laughs and continues to gently fuck Urta.  You pull off Urta’s cock and wait for her massive load to finish gushing from your " + (player.vaginas.length > 0 ? "pussy" : "ass") + " before putting on your clothes.  Before you leave you give Urta a kiss and squeeze her belly a little.  Her only reply is to moan.  You tell Kath to take good care of Urta and then leave.  You know that if you stay much longer their display will get you excited enough that you’d stay around all afternoon and unfortunately you’ve got things you need to do.\n\n");
    player.orgasm();
    flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY]++;
...
function pinAndFuckStuffKath(): void { // Plays for any males and for herms who select this option
    clearOutput();
...
export function watch(urtaIsDrunk: boolean): void { // Kath is sober (or drunk), Urta is drunk
    clearOutput();
...
export function watchNoIntro(urtaIsDrunk: boolean): void { // Kath is sober (or drunk), Urta is drunk
    clearOutput();
    watchMainBody(urtaIsDrunk);
}

function watchMainBody(urtaIsDrunk: boolean): void { // Kath is sober, Urta is drunk
...
export function knothole(): void { // Kath is drunk, Urta is sober //Not available if Kath has no cock
    clearOutput();
...
export function sandwich(): void { // Kath is drunk, Urta is sober //Not available if Kath has no cock
    clearOutput();
...
function sandwichGetFucked(): void {
    ...
    outputText("Kath doesn’t wait for you or Urta to adjust." + (Katherine.getCockLength() > 14 ? "  You can actually feel a little bump just above Urta’s cock every time Kath thrusts inside her." : "") + "  She pounds Urta, picking up the pace, and you know it can’t be long before Katherine knots.\n\n");
    ...
    if (Katherine.cumQ() <= 1500)
        ...
        outputText("expanding against " + (player.isTaur() ? "your rump" : "the small of your back") + ".  The air is forced from Urta’s lungs and you get pushed to the ground as both your lovers collapse against you.\n\nYou smile to yourself and flex " + (player.vaginas.length > 0 ? "the muscles in your pussy" : "your sphincter") + ", milking Urta’s cock.  She may be packed with Kath’s sperm but Urta is still Urta.  She responds by gripping your " + (player.isTaur() ? "flanks" : "hips") + " once more");
    ...
    outputText("At last you feel the delightful blast of cum inside you.  Urta pulls your body against hers and floods you with her hot, sticky juices.  Your stomach muscles give out, unable to contain the load Urta pumps inside you.  Her throbbing cock brings you release and you cum, joining your lovers in ecstasy." + (player.cocks.length > 0 ? "  Your own cock contributes to the mess by spraying the floorboards with your wasted seed." : ""));
    sandwichCommonEnd(); // Play the common end
}

function sandwichGetLicked(): void {
    outputText("\n\nYou massage Urta’s shoulders and ask if she could help you out.  She’s not sure what you mean until you apply a little pressure, gently angling her head toward your " + (player.cocks.length > 0 ? "cock" : "pussy") + ".  Once she gets the idea Urta quickly gets down on all fours.  You have to smile to yourself as Urta " + (player.cocks.length > 0 ? "licks your cock and sucks out some pre" : "digs in") + ".  She puts her hands on your hips so that Katherine’s thrusts don’t " + (player.cocks.length > 0 ? "impale her" : "force her face into your pussy") + ".\n\n");
    ...
    outputText(".  Every time she thrusts her rack jiggles delightfully.  At the same time Urta’s expert tongue is " + (player.cocks.length > 0 ? "sliding around your cockhead" : "busy teasing your folds") + ", nudging you towards an orgasm.\n\n");
    outputText("With a powerful thrust Kath manages to force her partially inflated knot" + Katherine.cockMultiple("", "s") + " inside Urta.  Urta gasps, but more importantly she also loses her grip on your hips.  Kath is so lost in pleasure she doesn’t even notice.  She pulls back and you hear her knot" + Katherine.cockMultiple("", "s") + " pop out.  Then she rams " + Katherine.cockMultiple("it", "them") + " back in.  Urta is driven forward and " + (player.cocks.length > 0 ? "she ends up taking your entire cock down her throat in one stroke" : "her face gets mashed into your pussy") + ".\n\n");
    outputText("It’s the pleading look Urta gives you that sends you over the edge.  That and " + (player.cocks.length > 0 ? "the feeling of her whole throat clamping down on your member" : "her nose tickling your clit while her tongue probes your depths.  You cum hard, soaking her face with fem-cum") + ".\n\n");
    if (player.cocks.length > 0) {
        if (player.cumQ() < 500) {
...
function sandwichMaleCentaurLicked(): void {
...
function sandwichCommonEnd(): void {
...
export function orgy(): void { // Both Kath and Urta are drunk
    clearOutput();
    ...
    if (player.cocks.length > 0)
        outputText("crotch");
    else outputText(player.breastRows.hasBreasts() ? "breasts" : "ass");
    ...
    const valaHere: boolean = flags[kFLAGS.FREED_VALA] > 0 && game.time.hours >= 12 && game.time.hours <= 21;
    if (valaHere) {
        ...
        if (flags[kFLAGS.KATHERINE_VALA_TIMES_SEX] > 0)
            ...
            outputText("You tell Vala that this is Katherine.  You hear a muffled “<i>Hi</i>” from the locked lips of your two " + (Katherine.getCockNumber() > 0 ? "herms" : "lovers") + ".\n\n");
        }
    }
    ...
    if (valaHere)
        outputText("... Vala surprising Kath by " + (Katherine.getCockNumber() > 0 ? "settling on top of her " + Katherine.cockAdj() + "shaft and then taking it, knot and all, inside her tiny body" : "diving between her thighs and sucking on Kath's clit while pushing her whole hand inside Kath's dripping twat"));
    ...
    else outputText(player.vaginas.length > 0 ? "licked you out" : "fisted your ass");
...
export function doubleStuffKath(): void { // Both Kath and Urta are drunk
    clearOutput();
    outputText("You ask your two " + (Katherine.getCockNumber() > 0 ? "herms" : "lovers") + " if they want to go somewhere more private.\n\n");
    const urtaHasHouse: boolean = player.keyItems.has("Spare Key to Urta's House") >= 0;
    outputText("Urta " + Katherine.clothesLowerChoice("runs her hand up Kath’s thigh", "slides her hand under Kath’s skirt", "rubs the " + (Katherine.getCockNumber() > 0 ? Katherine.cockAdj() + "bulge in the " : "") + "crotch of Kath’s bodysuit", "slides her hand under Kath’s dress", "slides her hand between the folds of Kath’s robe") + " and says, “<i>That’s a good idea.  My " + (urtaHasHouse ? "old " : " ") + "apartment’s close enough.</i>”\n\n");
    flags[kFLAGS.KATHERINE_LOCATION] = Katherine.KLOC_URTAS_APT;
    outputText("It’s an interesting walk.  Both Urta and Kath need your support and both are horny.  They’re constantly groping and kissing you or each other.  When you get to Urta’s " + (urtaHasHouse ? "old " : " ") + "apartment the two " + (Katherine.getCockNumber() > 0 ? "herms" : "girls") + " don’t even make it to the bed.  They crash to the floor and start tearing each other’s clothes off while you close the door.\n\n");
    outputText("“<i>You want to see us fuck?  Is that it?</i>” Kath asks.\n\n");
    outputText("Urta strokes Katherine’s " + (Katherine.getCockNumber() > 0 ? "cock" : "tail") + " and says, “<i>Yeah, [he] loves " + (Katherine.getCockNumber() > 0 ? "herms so much" : "it when we fuck") + ", let’s give [him] a show!</i>”\n\n");
    outputText("Urta lifts Kath’s leg and presses the flat face of her cock against Kath’s dripping pussy.  Kath distracts her by squeezing Urta’s breasts, then " + (Katherine.getCockNumber() > 0 ? "presses her own cock against" : "slides her finger over") + " Urta’s sex.\n\n");
    outputText("You’re barely out of your clothes by the time your two vixens slam their hips against each other.  It’s like a small thunderclap" + (Katherine.getCockNumber() > 0 ? ".  Both of them look down at where their respective cocks disappear into each other’s bodies" : " and they both seem well pleased with the result") + ".\n\n");
    outputText("Kath starts to laugh.  “<i>" + (Katherine.getCockNumber() > 0 ? "No room left for you" : "Too slow" + player.short) + ",</i>” she says, “<i>but I bet we can still make you feel good.</i>”\n\n");
    outputText("Urta licks her lips and says, “<i>Oh yeah, we’re gonna make you feel so good.</i>”  She reaches out for your hand and pulls you closer.  She mumbles “<i>If I’m fucking your girl’s pussy this is the least I can do.</i>”  Her tongue licks the " + (player.cocks.length > 0 ? "tip of your cock before she takes it into her mouth" : "length of your pussy before she sucks on your clit") + ".\n\n");
    outputText("Urta’s alcohol consumption must have loosened her up.  She sucks " + (player.cocks.length > 0 ? "your cock right down her throat, barely gagging at all" : "and rolls her tongue over your clit like a possessed woman") + ".  Kath looks " + (player.cocks.length > 0 ? "impressed as Urta deepthroats you" : "a little envious, despite the horse cock that's filling her") + ".  When Urta finally pulls back for some air Kath gleefully shouts, “<i>My turn!</i>” and " + (player.cocks.length > 0 ? "takes your whole length in one gulp" : "dives in, her rough tongue attacking your clit as she tries to one-up her captain") + ".\n\n");
    outputText("“<i>Show off,</i>” says Urta.  She tweaks Katherine’s nipples, causing a low moan that vibrates " + (player.cocks.length > 0 ? "your dick" : "through your pussy") + " in a pleasing way.  Soon Kath " + (player.cocks.length > 0 ? "has to release" : "has to release") + " you and coughs as she sucks in a new breath.\n\n");
    outputText("Before she can " + (player.cocks.length > 0 ? "swallow your meat" : "go down on you") + " again Urta grabs your " + (player.cocks.length > 0 ? "pole and directs it between her own lips" : "hips and presses her face against your groin") + ".  “<i>Hey! I wasn’t finished,</i>” says Kath.  She starts to " + (Katherine.getCockNumber() > 0 ? "thrust her " + Katherine.cockType() + " member into Urta’s hole" : "rock her hips, her hand rolling Urta's balls between her fingers") + ".  Urta’s eyes close and she lets Kath" + (Katherine.getCockNumber() > 0 ? "’s thrusts rock" : "do as she likes with") + " her body, " + (player.cocks.length > 0 ? "sliding her throat up and down your shaft" : "her tongue burrowing deep inside you") + ".\n\n");
    outputText("You were horny before these insatiable " + (Katherine.getCockNumber() > 0 ? "herms" : "sluts") + " started fighting over your " + (player.cocks.length > 0 ? "cock" : "cunt") + " and the feeling brings you to your knees.  By the time Urta lets go Kath is ready.  In a single stroke her nose starts rubbing against your belly and you look down to witness her slight smile and horny gaze as her rough tongue starts doing unspeakable things " + (player.cocks.length > 0 ? "to your cock" : "deep inside your sex") + ".  Meanwhile Urta is " + (Katherine.getCockNumber() > 0 ? "providing the same ‘service’ Kath did," : "") + " hammering her cock into Kath’s cunt like it owes her money.\n\n");
    if (Katherine.getCockNumber() > 0)
    ...
    outputText("Kath regains her strength first and forces herself up onto her elbows.  She smiles at you and simply opens her mouth.  You smile back, " + (player.cocks.length > 0 ? "feed her your cock" : "press your crotch against her face") + " and relax, enjoying the slower pace" + (player.cocks.length > 0 ? ", the texture of Kath’s tongue and the inside of her throat" : " and the texture of Kath’s tongue") + ".\n\n");
    outputText("Your two lovers trade you" + (player.cocks.length > 0 ? "r cock" : "") + " back and forth a few times until you feel that you’re about to cum.  ");
    if (player.cocks.length === 0) {
        ...
        if (player.cumQ() < 500) {
            outputText("They both swallow your additional contribution easily." + (Katherine.getCockNumber() > 0 ? "  It’s nothing compared to the amount they pumped into each other’s pussies." : ""));
            if (player.cocks.length > 2) outputText("  Your other cock" + (player.cocks.length > 3 ? "s" : "") + " produce a few squirts which coat both Kath and Urta’s bodies.");
            outputText("  As you pull out they give each other a tired hug and giggle drunkenly " + (Katherine.getCockNumber() > 0 ? "as they compare their swollen bellies" : "as Urta rubs Kath's swollen belly proudly") + ".");
        }
        else if (player.cumQ() < 1500) {
            if (Katherine.getCockNumber() > 0)
            ...
            if (player.cocks.length > 2) outputText("  You realize that your free cock" + (player.cocks.length > 3 ? "s" : "") + " fired their streams all over the room, adding to the mess Urta will have to clean up later.");
            ...
            if (player.cocks.length > 2) outputText("  Your other cock" + (player.cocks.length > 3 ? "s" : "") + " release a torrent of cum, coating Kath, Urta, the floor and a few of the walls.  Though it’s messy you’re sure both your lovers are happy that at least some of your sperm didn’t wind up inside them.");
            outputText("\n\nWhen it’s over you pull your cocks free and Kath giggles at Urta’s faux pregnancy.  “<i>Too full,</i>” Urta croaks." + (Katherine.getCockNumber() > 0 ? "  Kath rubs Urta’s larger belly and then flicks her finger against it, watching the waves ripple across Urta’s skin" : "\n\nKath's jaw drops and she point at her own, even larger, belly.  “<i>You do <b>not</b> get to complain,</i>” she says, pressing her hand none too gently against Urta's stomach") + ".  “<i>Oh - not fair!</i>” cries Urta, clutching her overstuffed gut.");
        }
    }
    else { // Just one cock
        if (rand(2) == 0) { // Urta takes it
            ...
            if (player.cumQ() < 500)
                outputText("Urta swallows all of it easily.  Your contribution does nothing to " + (Katherine.getCockNumber() > 0 ? "her expanded" : "expand her") + " abdomen, but she rubs your leg and smiles contentedly.");
            else if (player.cumQ() < 1500)
                outputText("Urta has a little difficulty taking your deposit" + (Katherine.getCockNumber() > 0 ? " since her belly is already filled with Kath’s seed" : "") + ".  She gulps it all down but rubs her sides afterward, trying to reduce the pressure.");
            else {
                outputText("Urta’s gut starts to expand " + (Katherine.getCockNumber() > 0 ? "under the added" : "thanks to the") + " pressure from your contribution.  She makes a worried “<i>Mmph</i>” noise, but you’re busy right now.  She leans back and her stomach grows enough to develop some stretch marks.");
                outputText("When it’s over you pull your cock free and Kath giggles at Urta’s faux pregnancy.  “<i>Too full,</i>” Urta croaks.  Kath rubs Urta’s " + (Katherine.getCockNumber() > 0 ? "larger " : "") + "belly and then flicks her finger against it, watching the waves ripple across Urta’s skin.  “<i>Oh - not fair!</i>” cries Urta, clutching her overstuffed gut.");
            }
        }
        else { // Kath takes it
            ...
            if (player.cumQ() < 500)
                ...
                outputText("When it’s over you pull your cock free and Urta starts to rub Katherine’s " + (Katherine.getCockNumber() > 0 ? "larger " : "") + "belly.  Kath lies back on the floor and moans, her tail lazily moving from side to side.");
            }
        }
    }
    if (Katherine.getCockNumber() > 0) {
...
export function doublePenetrateKath(): void { // Both Kath and Urta are drunk
    clearOutput();
    outputText("You ask your two " + (Katherine.getCockNumber() > 0 ? "herms" : "sluts") + " if it’s time to commandeer one of the back rooms.\n\n");
    ...
    outputText("You watch the show, your cock beginning to strain against the confines of your clothes" + (player.vaginas.length > 0 ? " as your pussy begins to wet them" : "") + ".  Katherine starts to tug at Urta’s clothes and despite her drunkenness she does a good job of removing them.\n\n");
    ...
    outputText("You pull the joined " + (Katherine.getCockNumber() > 0 ? "herms" : "lovers") + " away from the wall and tell Kath you’re here to give her support.  She looks a bit confused, so you step behind her and spread her cheeks with your hands.\n\n");
...
export function doubleStuffVala(): void { // Not available if Kath has no cock
    valaCommonStart();
    ...
    valaCommonPostSex(Katherine.getCockNumber() > 0);
...
export function eatOutVala(): void { // Not available if Kath has no cock
    valaCommonStart();
...
export function fistKathAndVala(): void {
    valaCommonStart();
    ...
    outputText("Vala gets back to kissing but her hands find Katherine's tail.  It's been flicking back and forth, sometimes coiling itself around Vala's waist, but now the faerie starts using her delicate fingers to tease the tip.  Kath groans and " + (Katherine.getCockNumber() > 0 ? "her ignored manhood jiggles in time with her heartbeat" : "her whole body shakes from the unexpected stimulation") + ".\n\n");
...
function valaCommonStart(): void {
    ...
    if (Katherine.getCockNumber() > 0) outputText("  Her cock" + Katherine.cockMultiple(" is hard enough that it's", "s are hard enough that they're") + " jutting out horizontally.");
    outputText("\n\n");
}

function valaCommonPostSex(theyWereFucking: boolean): void { // This bit happens in all three Vala sex scenes
    if (flags[kFLAGS.KATHERINE_VALA_TIMES_SEX] == 0) {
