...
private  useTheSexMachine():void {
	...
	if(player.gender == 1) {
		if(flags[kFLAGS.TIMES_USED_SEX_MACHINE_AS_MALE] == 0) {
			...
			outputText("It takes little time for " + game.player.sMultiCockDesc() + " to grow hard, your lust betraying you as another phallic shaped metallic length is presented in front of your face. A small shock is delivered to your " + assholeDescript() + ", causing you to gasp, and allowing the phallic length to slip deep into your mouth, giving you little choice but to suck on the device. The tube-like length on your " + game.player.cockDescript(0) + " continues its assault, humming and stroking your firm cock, starting to suck softly to pleasure your mighty rod; so much that you nearly don't notice as a small tube slips into the tip of your penis, beginning to slowly devour your leaking pre-cum.\n\n", false);
	...
	//[If you decide to mess with the machine, female:]
		...
		if(flags[kFLAGS.TIMES_USED_SEX_MACHINE_AS_FEMALE] == 0) {
			...
			if(player.biggestTitSize() >= 2) outputText("  It slips a fourth cock between your " + game.player.chestDesc() + ", using two arms to press them together and fuck your tits as well, splashing a burst of mysterious cum on your face with every thrust.", false);
			outputText("\n\n", false);
	...
	//[Sexless]
		...
		//[Sexless Repeat]
			...
			outputText("You shrug it off, slipping, as directed, into the various straps and buckles to secure yourself to the machine, and once more it lifts you into the air. You feel a firm pressure against your " + assholeDescript() + " once again, teasing you with a surprise: you feel a flexible latex head probing your asscheeks, leaking cum all over them!\n\n", false);
	...
	//[Hermaphrodite]
		...
		if(flags[kFLAGS.TIMES_USED_SEX_MACHINE_AS_HERM] == 0) {
			...
			if(player.biggestTitSize() >= 2) outputText("  Well, not every part of you, but that is rapidly fixed as a final cock rubs against your " + game.player.chestDesc() + ", starting to stimulate your chest as well.", false);
			outputText("\n\n", false);
			...
			if(player.biggestTitSize() >= 2) outputText("  The cock on your chest pumps cum onto your " + game.player.chestDesc() + ", as y", false);
