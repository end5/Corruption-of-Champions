...
export function procMaddieOneIntro(): void {
    outputText("", true);
...
function followMinotaurIntoBackroom(): void {
    ...
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00241] == 0) {
        ...
        // [Give Them] [Leave]
        if (player.inv.hasItem(ConsumableLib.BEEHONY) && player.inv.hasItem(ConsumableLib.L_DRAFT))
...
// [Not Yet/No]
export function nopeAintGotNoneODemSpeculIngredimathings(): void {
    outputText("", true);
...
// [Yes – baking]
export function handOverIngredientsItBeBakingTimeYo(): void {
    outputText("", true);
    ...
    if (player.perks.findByType(PerkLib.MinotaurCumAddict) >= 0) outputText("  You lurch forward involuntarily, craving the rest of his jism, but he pushes you into the wall and grunts, \"<i>No,</i>\" in a tone that brooks no disagreement.  It actually shocks you out of your addicted haze.", false);
    outputText("\n\n", false);
    ...
    if (player.perks.findByType(PerkLib.MinotaurCumAddict) >= 0) outputText("  Your mouth salivates at the thought.", false);
    else outputText("You aren't sure you want to.", false);
    outputText("\n\n", false);
    if (player.perks.findByType(PerkLib.MinotaurCumAddict) >= 0) doNext(waitForSlutCake);
    // [Wait] [Sneak Out]
...
export function runAwayMaddieFollowup(): void {
    spriteSelect(39);
    outputText("", true);
    flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00242] = -2;
...
function talkToMaddie(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        outputText("Suction starts, hollowing the cupcake-girl's plush cheeks into a concave, cock-slurping form.  The constant squeezing of your " + cockDescript(game.player, x) + " combines with the sucking to make you swell larger inside Maddie's gullet while she kisses your groin.  The confection's oral charms show no signs of stopping as she noisily slurps away at her treat, and her pillowy breasts are so spongy-soft and calming that you're happy to let her sample your 'icing' if it means you can feel like this.  Your " + hipDescription(player) + " push back into the baby blue lips, pumping and thrusting as your instinct to fuck and breed takes over, working your " + cockDescript(game.player, x) + " in and out of the pastry's puckered mouth.\n\n", false);

        outputText("Maddie pushes further forward, her bosom crushing you against the wall to hold your hips immobile while she sucks harder and harder.  Your cock balloons from the suction, thickening inside her neck and beginning to twitch from the irresistible fellative pleasure. An orgasm grows in your " + ballsDescriptLight(player), false);
        ...
        outputText("Oven-warmed tiles kiss your exposed " + buttDescription(player) + " as you're gently placed on the floor next to your discarded equipment.  Exhausted and satiated as you are, your eyes drift closed, lulling you into slumber.\n\n", false);
    ...
    // (FEMALE/Genderpoots)
        ...
        outputText("\"<i>Shhh, shhh... that's a good " + mf(player, "boy", "girl") + ".  Isn't my icing the absolute best?</i>\" she verbally gushes, just like the nipple between your teeth.  \"<i>Drink up", false);
        ...
        outputText("Once the beast-man has finished talking you realize the cupcake-girl, Madeleine, is standing behind him.  Her blue-iced 'clothes' have been remade, shaped into a form-fitting apron that accentuates her massive, otherworldly curves.  The minotaur chef utters, \"<i>We go now.  Get dressed.  Maybe sometime can visit Maddie.</i>\"  Maddie claps her hands, bouncing and jiggling with excitement as the two of them leave you there to get dressed.", false);
        outputText(modThickness(player, 100, 10), false);
...
export function bakeryEpilogue(): void {
    outputText("", true);
