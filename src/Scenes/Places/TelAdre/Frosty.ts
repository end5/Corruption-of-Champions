...
export function approachFrosty(): void {
    ...
    if (flags[kFLAGS.MET_FROSTY] == 0) {
        ...
        outputText("\n\n“<i>Well, " + mf(player, "sir", "madam") + ", I was thinking of a new business plan last night. You could... be a... beta tester for it. All you have to do is sign these agreement papers.</i>”");
...
export function frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp(): void {
    ...
    if (player.cocks.length > 0 && player.vaginas.length === 0) {
        ...
        outputText("\n\n“<i>Ooooh, your ");
        if (player.cocks.biggestCockLength() < 7) outputText("little");
        else outputText("big");
...
function pitcherInTheRyeAndByRyeIMeanFrostysBrownHole():void
{
    ...
    outputText("\n\nYou squeeze behind her, there being just enough room for the two to stand front to butt from one another, your body squishing against her soft back-side.  You reach into your [armor] and release your " + multiCockDescriptLight(game.player) + " from ");
    if(player.cocks.length == 1) output("its");
    else output("their");
    outputText(" prision, the hot mass getting tangled in the curly hair draping over her back. It’ll take some maneuvering from the both of you but eventually your cock-head makes contact with her tight pink hole. Frosty take a deep breath and starts easing her-self down your " + cockDescript(game.player, x) + ". It takes her only a few moment for your cock to  be snugly secured in her insides. She stands up straight and the grip around your cock becomes tighter. She breath out “This is like the only service where I can man the shop and pleasure a customer, a bit exhausting but business is business.”
...
It moves only few inches on your onsides. Your mind starts racing with pleasure as Frosty’s other hand wanders over your [allBreastsDescript] and pinches on your [nippleDescription(player, 0)], pulling and twisting the nip and taking nice hard grabs at your [ breastDescriptOfRow(game.player, 0)] A familiar build-up forms down in your crotch, you start pushing back against Frosty’s “cock” to hurry up the climax and start pinching your other [nippleDescription(player, 0)] minicing the same action as the torturous nymph does, abusing the sensitive bud all in the name of sexaul pleasure.
