...
export function gangbangVillageStuff(): void {
    clearOutput();
...
function acceptRebeccsPlea(firstTime: boolean = false, sacrificed: boolean = false): void {
    ...
    if (player.cor > 70 && player.gender > 0) {
        ...
        if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0));
...
function intoTheDemonPit(sacrifice: boolean = true): void {
    ...
    if (game.time.hours < 21) {
        const passed: number = 21 - game.time.hours;
...
function fightZeDemons(sacrifice: boolean = true): void {
    ...
    if (sacrifice) {
        ...
        monster.effects.create(StatusAffects.BowDisabled, 0, 0, 0, 0);
        if (player.str < 80 && player.spe < 80) {
            monster.effects.create(StatusAffects.AttackDisabled, 0, 0, 0, 0);
            monster.effects.create(StatusAffects.RunDisabled, 0, 0, 0, 0);
            monster.effects.create(StatusAffects.PhysicalDisabled, 0, 0, 0, 0);
        }
        else {
            if (!player.canFly()) monster.effects.create(StatusAffects.RunDisabled, 0, 0, 0, 0);
        }
    }
...
export function loseOrSubmitToVapula(): void {
    clearOutput();
    ...
    if (player.vaginas.length > 0) outputText(", followed by your " + vaginaDescript(player, 0));
    ...
    // [(no Buttslut)
    if (player.perks.findByType(PerkLib.Buttslut) < 0) outputText("No!  You aren't supposed to enjoy it...  ");
    ...
    outputText("\n\nAfter what seems like hours of forceful penetration, the gigantic members inside you release a milky torrent of corrupted seed, completely packing your colon through your " + assholeDescript(player));
    if (player.vaginas.length > 0) outputText(" and filling your womb to its maximum capacity");
    ...
    // [if vagina]
    if (player.vaginas.length > 0) outputText("\n\nYour own " + vaginaDescript(player, 0) + " clenches too, doing its best to waste as little semen as possible.  Your muscles quiver as they feel the incoming last release.  Then, in climax, you squirt your own girl-cum, splattering the demon dicks with your juice.");
    // [if lactating breasts]
    if (player.lactationQ() > 0) outputText("\n\nThe combined efforts of a dozen demons proves too much for your poor " + chestDesc(game.player) + " and your nipples let out a white creamy flood, which is immediately gobbled by thirsty imps.  ");
    else outputText("\n\n");
    ...
    else if (player.effects.findByType(StatusAffects.LostVillagerSpecial) >= 0) doNext(morningAfterRape);
...
// Victory (Z)
export function defeetVapulasHorde(): void {
    clearOutput();
    flags[kFLAGS.REBECCS_LAST_PLEA] = 0;
...
function rapeZeVapula(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        // [if vagina]
        if (player.vaginas.length > 0) outputText("You stick a finger in your own " + vaginaDescript(player) + " in order to lubricate it.  ");
        ...
        if (player.vaginas.length > 0) outputText("bitch in heat");
        ...
        outputText(" cumdumpster and grab another slut.  This time you feel like you could use a blowjob to clean off the demonic sweat and slime from your cock; you forcefully open her jaw and shove your " + cockDescript(game.player, 0) + " down her throat without any care.  The sudden influx of dickflesh blocking her air-flow makes her choke and cough, but a withdrawal and a good cock-slap reminds her of her place.  \"<i>You fucking bitch!  You goddamn worthless slut!  What good are you if you can't even give proper head?  Lick, you whore!  I want your tongue all over my crotch!  You can breathe when you've properly moistened my ");
        if (player.balls > 0) outputText("balls");
        else outputText(skin(player));
        ...
        outputText("\n\nAt last, you reach the limit of your prodigious stamina.  When you're done pounding the last untouched hole in the pit, when your " + cockDescript(game.player, 0) + " is done squirting its ultimate drop of goo over the wanton succubus, you relax and drop the fuck-toy you were using.  Your " + cockDescript(game.player, 0) + " is finally softening and you have no further use of any broken toy in the pit.  All but one...");
    ...
    // Victory rape using vagina/anus [Take Dicks in All Holes](Z)
        ...
        outputText("\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, an incubus; shorter and younger than Vapula, his tight, pert body is a delight to your eyes.  Without ceremony you shove him to the ground and grind your " + vaginaDescript(player) + " into his crotch.  He looks at you, semi-comatose and bewildered, as you get a rise out of his dick, which hardens against your damp strokes.  You pin him down as you sink onto his red length, sighing as you rapidly engulf him, then begin to ride him ruthlessly.  ");
        ...
        outputText("\n\nYou keep working the incubus's hot cock in front of the once-powerful dominatrix, panting as you accelerate the pace until you finally reach your climax, clenching your thighs tightly to his fit body as your " + vaginaDescript(player) + " clenches and spasms.  ");
        // [if squirter]
        if (player.vaginas.wetness() >= 5) outputText("The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus's cock, coating him in your fragrant slime.  ");
        ...
        outputText("\n\nYou throw your expendable cock toy to one side and grab another slut, this time a succubus.  Your semen-dribbling cunt is now in need of a cleaning, and without any ado you sit yourself straight down onto her face, banging the back of her head against the ground.  She whines into your " + vaginaDescript(player) + ", to which you respond by pulling her hair.  \"<i>You fucking bitch!  You worthless slut!  What good are you if you can't even munch rug properly?  Lick, you whore!  I want your tongue to clean up what your useless fuckhead of a friend couldn't keep to himself.</i>\"  The poor demoness mutters an apology and proceeds to lap up the juices drooling out of your hole, licking your labia and mons clean of what the incubus left, darting her wet muscle across your " + clitDescription(player) + " before reaching into your vagina, carefully teasing out and drinking every drop of jizz she can find, making your own juices run afresh.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your " + nippleDescription(player, 0) + "s harden");
        if (player.breastRows.biggestTitSize() >= 2) outputText(" and your " + breastDescriptOfRow(game.player, 0) + " bounce gently");
        ...
        if (player.breastRows.hasFuckableNipples()) outputText("s");
        ...
        if (player.breastRows.hasFuckableNipples()) outputText(", teats, ");
        else outputText(" ");
...
function owcaMainScreenOn(): void {
    ...
    let pit: () => void = null;
    let herd: () => void = null;
    let tavern: () => void = null;
    if (game.time.hours >= 16 && flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) {
...
// Tavern (Z)
export function owcaTavern(): void {
    clearOutput();
...
function owcaBuySetup(item: ItemType): void {
...
function buyOwcaShit(bleh: ItemType, price: number = 0): void {
    clearOutput();
    player.gems -= price;
    statScreenRefresh();
...
function rebeccBathScene(): void {
    ...
    if (player.gender == 0) outputText(".");
        ...
        if (player.vaginas.length > 0) outputText("your " + vaginaDescript(player) + " leaks a little trickle of girl-juice");
...
function rapeRebecc(outside: boolean = false): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks[0].cockLength > 12) outputText("  You keep burying more of your " + cockDescript(game.player, 0) + " deep inside her rectum until you reach her limit and can't fit more cock in.");
        ...
        outputText("\n\nAfter a few minutes of resistance, her body tenses up and you ease yourself into effectively butt-fucking her.  You pull her white fuzzy hair, seize her squeezable hips for more leverage and start raping her ass; the thrusts come hard and fast, almost bringing your " + cockDescript(game.player, 0) + " out before jamming it all the way inside again.  This forceful and tigerish motion makes her shriek in excruciating ecstasy; she groans and moans rhythmically at the pace of your regular ass-pounding.  The torment you're inflicting on her poor interior walls is too much to bear and she twists violently to escape the dick-beast dominating her pucker; however, all she can do is wave her appreciable butt, bringing extra sensations to your " + cockDescript(game.player, 0) + " and adding extra spice to your already ardent anal-rape.");
        outputText("\n\nThe cadenced wobbling of her rump as well as your constant pumping is bringing you to a higher state of pleasure; the heavenly tingle your " + cockDescript(game.player, 0) + " is receiving is making you heedless of anything else but the burning heat of her insides cooking your meat.  You feel your lust rise as you keep riding the buttslut below you, ignoring her muffled complaints and whining, her tight confines gradually taking over any control you have over your rod.  You pound Rebecc's ass as if your life depended on it, clutching her curvy hips instinctively like an imp in rut.  Your groin impacts her buttocks with a loud SLAP at every thrust");
        // [if balls]
        if (player.balls > 0) outputText(", and your " + ballsDescriptLight(player) + " almost feel sore from colliding with her dribbling cunt");
        ...
        outputText("\n\nWith a last, satisfied sigh, you fall over, " + cockDescript(game.player, 0) + " deeply buried inside Rebecc's butt as you feel her warm, curvy body slowly waggling below you.  ");
...
export function beatUpOwca(): void {
    clearOutput();
    ...
    let torch: () => void = null;
    if (player.effects.findByType(StatusAffects.KnowsWhitefire) >= 0 || player.perks.findByType(PerkLib.FireLord) >= 0 || player.perks.findByType(PerkLib.Hellfire) >= 0)
...
// End of quest
// Lose to Villagers (Z)
export function loseToOwca(): void {
    clearOutput();
    ...
    player.effects.create(StatusAffects.LostVillagerSpecial, 0, 0, 0, 0);
}
// Post-Mob Encounter (Z)
function morningAfterRape(): void {
    player.effects.remove(StatusAffects.LostVillagerSpecial);
    clearOutput();
...
function slaveToVapulaBadEnd(): void {
    ...
    outputText("\n\n\"<i>Enough.  Remove that dick from my pet's mouth, I want to talk to " + mf(player, "him", "her") + " for a second.</i>\"");
