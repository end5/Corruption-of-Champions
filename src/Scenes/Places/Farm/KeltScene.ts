...
function bowSkill(diff: number): number {
    player.effects.addValue(StatusAffects.Kelt, 1, diff);
    if (player.effects.getValue1Of(StatusAffects.Kelt) >= 100) player.effects.setValue(StatusAffects.Kelt, 1, 100);
    return player.effects.getValue1Of(StatusAffects.Kelt);
}
...
export function keltEncounter(): void {
    ...
    outputText("", true);
    player.effects.addValue(StatusAffects.Kelt, 3, 1);
    // If First Encounter
    if (player.effects.findByType(StatusAffects.Kelt) < 0) {
        player.effects.create(StatusAffects.Kelt, 0, 0, 1, 0);
        keltFirstTime();
    }
    // Repeated encounter
    else {
        // Second/Third Events - Normal
        if (player.effects.getValue3Of(StatusAffects.Kelt) <= 3) {
            keltMainEncounter();
            return;
        }
        // Bad Ends
        if (player.effects.getValue2Of(StatusAffects.Kelt) >= 130) {
        ...
        if (player.effects.getValue3Of(StatusAffects.Kelt) == 4 && player.effects.findByType(StatusAffects.NakedOn) < 0) {
            keltRequiresNakedness();
            return;
        }
        // 60+ Submissiveness—First Time Blowjob Requirement
        if (player.effects.getValue2Of(StatusAffects.Kelt) >= 40 && player.effects.findByType(StatusAffects.KeltBJ) < 0) {
            keltRequiresBlowjobs();
            return;
        }
        // 75+ Submissiveness, 60+ Lust—Lust Encounter
        // Remaining events
        if (player.effects.getValue3Of(StatusAffects.Kelt) > 4) {
            keltMainEncounter();
        }
    }
}
...
function keltRefuseNakedness(): void {
    ...
    player.effects.create(StatusAffects.KeltOff, 0, 0, 0, 0);
    doNext(Camp.returnToCampUseOneHour);
}
...
function keltEagerlyGetNaked(): void {
    ...
    if (player.gender == 1 && player.breastRows.biggestTitSize() < 1) outputText("\"<i>What are you, a girl?  Get on with it so I can see what I'm dealing with.</i>\"\r\r", false);
    else {
        if (player.breastRows.biggestTitSize() == 0) outputText("\"<i>Oops!  Could have sworn I'd find a pair of tits on you.  That's okay... I'm sure you'll hit puberty some day!</i>\"\r\r", false);
        // (Too small, A to DD:
        else if (player.breastRows.biggestTitSize() <= 5) outputText("\"<i>Ha! No wonder you cover up like a sniveling human!  Any centaur maiden would be ashamed to go out in public with " + num2Text(player.breastRows.totalBreasts()) + " mosquito bites like that!  What do they call you back home... 'Tiny Tits'?  Or maybe they all just assume you're a guy!</i>\"\r\r", false);
        // (Medium, E to HHH:
        else if (player.breastRows.biggestTitSize() <= 11) outputText("\"<i>Uh oh, boys... look out!  Miss Melons here has been putting on a little weight!  How did someone like you become an adventurer?  From the waist up, you're good for whoring and not much else!</i>\"\r\r", false);
    ...
    // (No Cock:
    if (player.cocks.length == 0) outputText("\"<i>Well, aren't you a pretty little thing, all together.  You and I will get along real nice, that's for sure!</i>\"\r\r", false);
    else {
        // (Any Cock:
        outputText("and almost inevitably, his eyes drop down to your " + cockDescript(game.player, 0) + ".  He snorts, ", false);
        // (Human Cock:
    ...
    player.effects.create(StatusAffects.NakedOn, 0, 0, 0, 0);
    // (+10 Submissive)
    player.effects.addValue(StatusAffects.Kelt, 2, 10);
...
function keltReluctantlyGetNaked(): void {
    ...
    // (+7 Submissive)
    player.effects.addValue(StatusAffects.Kelt, 2, 7);
    dynStats("lus", int(player.lib / 10) + 5);
    keltMainEncounter2();
}
...
function keltRequiresBlowjobs(): void {
    spriteSelect(35);
    player.effects.create(StatusAffects.KeltBJ, 0, 0, 0, 0);
...
function keltBlowjobRequirementShamefully(newl: boolean = true): void {
    ...
    outputText("Kelt moves aside, looking down at you with a smug look of satisfaction.  You weakly wave him off, embarrassed by what you just did, and he laughs nastily.\r\r", false);
...
function keltBlowjobRequirementEagerly(newl: boolean = true): void {
    ...
    // (Blowjob On.)
    player.effects.create(StatusAffects.BlowjobOn, 0, 0, 0, 0);
    // (+15 Submissiveness)
    player.effects.addValue(StatusAffects.Kelt, 2, 15);
    doNext(Camp.returnToCampUseOneHour);
}
...
function keltMainEncounter(): void {
    ...
    // (Submissive 0-30:
    if (player.effects.getValue2Of(StatusAffects.Kelt) <= 30) outputText("You grind your teeth in irritation, but swallow your pride enough to ask him for help.  ", false);
    // (Submissive 30-70:
    else if (player.effects.getValue2Of(StatusAffects.Kelt) <= 70) outputText("You nod reluctantly, and Kelt grins.  He may be arrogant, but he is rather good.  ", false);
    // (Submissive 70-100:
...
function keltMainEncounterAfterNakedReq(): void {
    ...
    // (Naked On)
    if (player.effects.findByType(StatusAffects.NakedOn) >= 0) {
        ...
        // (+5 Submissive)
        player.effects.addValue(StatusAffects.Kelt, 2, 3);
    }
    // (Naked Off)
    else {
        // (40% Chance:
        if (rand(10) <= 5) {
            ...
            if ((player.cor + player.lib + player.lust >= 180 && player.inte < 30) || player.effects.getValue2Of(StatusAffects.Kelt) >= 60) {
                ...
                if ((player.cor + player.lib + player.lust >= 220 && player.inte < 40) || player.effects.getValue2Of(StatusAffects.Kelt) >= 70) {
        ...
        // (Otherwise:
            ...
            // (Corruption 60+, or Submissive 60+:
            if ((player.cor + player.lib + player.lust >= 180 && player.inte < 40) || player.effects.getValue2Of(StatusAffects.Kelt) >= 60) {
                ...
                if ((player.cor + player.lib + player.lust >= 220 && player.inte < 40) || player.effects.getValue2Of(StatusAffects.Kelt) >= 75) {
...
function keltMainEncounter2(): void {
    ...
    if (player.effects.getValue2Of(StatusAffects.Kelt) >= 60 && rand(4) == 0 && player.effects.findByType(StatusAffects.KeltBJ) >= 0) {
...
function keltMainEncounter3(): void {
    ...
    if (player.effects.findByType(StatusAffects.NakedOn) < 0) {
        ...
        // (25% Chance:
        if (temp == 3) {
            if (race(player) != "centaur") outputText("\"<i>If you were a centaur, I'd recommend suicide.  Since you're a " + race(player) + ", I'd say your best option is to fuck off.</i>\"\r\r", false);
            else outputText("\"<i>As a centaur, I'd recommend suicide.  Really, it's that or man the fuck up.</i>\"\r\r", false);
        }
        // player.effects.addValue(StatusAffects.Kelt,1,5+rand(4));
        bowSkill(5 + rand(4));
    }
    // NAKERS
    else {
        ...
        if (player.inHeat && player.gender > 1) {
            ...
            // (Submissive, 0-30:
            if (player.effects.getValue2Of(StatusAffects.Kelt) <= 30) outputText("You shiver and groan, unable to help yourself.  It is clear that Kelt has every intention of breeding you, and you are helpless to stop the urges of your body.  Terrifying images of being raped daily by this cruel beast fill your head... of your belly swelling with his young again and again.  You let out a moaning cry, and orgasm helplessly even as Kelt laughs.\r\r", false);
            // (Submissive, 30-70:
            else if (player.effects.getValue2Of(StatusAffects.Kelt) <= 70) outputText("The thought of that fills you with a dreadful shiver of lust, from your head to your toes.  Your body longs to be bred, again and again, and the idea of submitting to this powerful creature is so powerfully erotic that you cum on the spot, orgasming with delightful abandon.  The thought of being this centaur's breeding slave feels so right!\r\r", false);
            // (Submissive, 70-100:
            ...
            // (+5 Submissive)
            player.effects.addValue(StatusAffects.Kelt, 2, 5);
            // (Pregnancy Chance)
        ...
        if (temporary <= 2) {
            ...
            dynStats("lus", 10);
            // player.effects.addValue(StatusAffects.Kelt,1,4);
            bowSkill(4);
            doNext(Camp.returnToCampUseOneHour);
            return;
        }
        // (No Breasts—Do standard Naked event)
        if (player.breastRows.biggestTitSize() == 0 && temporary == 3) temporary = 4;
        // (Naked, 20% Chance)
        if (temporary == 3) {
            ...
            outputText("To your surprise, he suddenly leans forward, fist moving towards your head.  You raise your arms to deflect the blow, but he changes tactics suddenly and grabs one of your " + breastDescriptOfRow(game.player, 0) + " instead.  You stiffen, but before you can react further, he squeezes them brutally, mauling your breasts roughly with his hands.\r\r", false);
            // (Small Size, A-DD:
            if (player.breastRows.biggestTitSize() <= 5) outputText("\"<i>Ha!  Even with your itty-bitty-titty, you have to admit to a certain... weakness, is it?  Awfully sensitive, aren't they?  Ooh, am I making the little girl wet?  Naughty slut!</i>\"\r\r", false);
            // (Medium Size, E-HHH:
            else if (player.breastRows.biggestTitSize() <= 11) outputText("\"<i>Just look at these mommy melons!  You want to be an archer?  I'm amazed you don't slap yourself in the tits with every shot!  Easy target to grab onto.  But hey, I bet you like it that way.  Like it when people grope these fat titties of yours?</i>\"\r\r", false);
...
function keltMainEncounterPostBlowjob(): void {
    ...
    if (player.effects.findByType(StatusAffects.BlowjobOn) >= 0) {
        // (Submissiveness 75+, Lust 60+)
        if (player.lust >= 75 || player.effects.getValue2Of(StatusAffects.Kelt) >= 90 && rand(2) == 0) {
            ...
            // (Penis:
            if (player.cocks.length > 0) outputText("  Your " + cockDescript(game.player, 0) + " is rock hard, but he ignores it almost contemptuously.  You almost feel ashamed of it, compared to the slowly-growing manhood between his legs.", false);
            // (Vagina:
            if (player.vaginas.length > 0) outputText("  For a moment, his fingers trace the line of your exposed " + vaginaDescript(player, 0) + ", giving you the slightest of warnings before he crudely jams two fingers deep inside, as though scouting out your depths.  You whimper urgently at the treatment, and more so as Kelt removes his fingers, licking with obvious pleasure.", false);
            ...
            buttChange(player, 70, true, true, false);
            outputText("\r\r", false);
            ...
            // (Penis:
            if (player.cocks.length > 0) outputText("  Beneath you, pressed firmly into the harsh wood of the mounting board, your " + cockDescript(game.player, 0) + " erupts, splattering your stomach with your own cum.  As the thick semen slides down towards your face, you begin slipping on your own warm seed, rocking back and forth with each harsh pounding Kelt delivers to your backside.", false);
            // (Vagina:
            if (player.vaginas.length > 0) outputText("  Your poor, neglected pussy quivers with delight, convulsing without even being touched.  Thick juices run freely down your leg, dripping off to splatter the hay below you as you moan like a bitch in heat.", false);
            // (Genderless:
            ...
            player.slimeFeed();
            // (+10 Submissiveness)
            if (buttChange(player, 70, true)) outputText("\r\r", false);
            player.effects.addValue(StatusAffects.Kelt, 2, 10);
            player.orgasm();
            dynStats("cor", 1);
            return;
        ...
        // (Otherwise)
            ...
            // (+5 Submissiveness)
            player.effects.addValue(StatusAffects.Kelt, 2, 5);
            // player.effects.addValue(StatusAffects.Kelt,1,3);
            bowSkill(3);
    ...
    // (Blowjob Requirement Off, 40% Chance)
        ...
        if (player.cor + player.lib + player.lust >= 220 && player.effects.getValue2Of(StatusAffects.Kelt) >= 80) {
...
function keltSubmitGivingBJ(): void {
    ...
    if (player.effects.getValue2Of(StatusAffects.Kelt) >= 90) {
        player.effects.create(StatusAffects.BlowjobOn, 0, 0, 0, 0);
    }
...
function keltBadEndEpilogue(): void {
    ...
    outputText("What she found was not what she expected.  Two centaurs, wrapped in carnal bliss.  Well, the female, at least, seemed wrapped in carnal bliss.  She groaned worshipfully beneath her mate, whimpering and kneading her own watermelon sized breasts as the powerful male pumped her again and again with his oversized, equine cock.  Even as the heroine watched, fascinated, the female centaur orgasmed explosively, shouting her satisfaction to the high heavens.  If the thick puddle of juices below them was any judge, this must have been her last orgasm among many.  She was almost half the size of her mate, and clearly savoring every thrust of his oversized cock into her distended equine cunt, lewdly thrusting back and begging him to cum inside her again.\r\r", false);
    outputText("In time, the male obliged, pulling cruelly on the centauress' hair as he let out a low roar and came forcefully.  The heroine stared in wonder as his massive balls convulsed again and again, pumping his mare full of seed.  If she listened, she could actually hear the lewd squish of every blast of semen as it filled her, pumping steady gouts of babycream into her womb.\r\r", false);
...
function keltResistancePussyOut(): void {
    ...
    // (+2 submission)
    player.effects.addValue(StatusAffects.Kelt, 2, 2);
    doNext(Camp.returnToCampUseOneHour);
}
...
function fuckKeltsShitUp(): void {
    ...
    if (player.faceType == FaceType.SNAKE_FANGS && player.tongueType == TongueType.SNAKE && player.lowerBody == LowerBodyType.NAGA) {
        ...
        if (player.cocks.length > 0) {
            ...
            if (player.cocks[0].cockLength <= 10) {
                ...
                outputText("You feel a mighty groan around your " + cockDescript(game.player, 0) + " and Kelt's cock begin to pulse against your tail urgently.  Quickly and expertly, you wrap the end around the centaur's penis tightly, denying him release and rewarding yourself with another pained, muffled squeal reverberating through your prick.\r\r", false);
                ...
                outputText("You punctuate these last words with three final thrusts and then, spent, slowly withdraw your " + cockDescript(game.player, 0) + " from his mouth, spit and cum trailing on your captive's lips.  Smirking, you loosen your bonds just a little by withdrawing your tail from his ass.  Kelt doesn't even orgasm; he raggedly moans as cum simply drools out of his abused dick by the bucket load.\r\r", false);

            }
            // If >10 inches:
            else {
                outputText("Slowly but surely you feed more of your formidable member into his mouth until you feel the back of his throat, with most of your " + cockDescript(game.player, 0) + " still outside his lips.  The centaur is now entirely engaged with your length, moving his head back and forth as his tongue slathers it with attention, perhaps in the hope that the sooner he gets you off the sooner this nightmare will end.  You smirk and grip his hair, moving his head in rhythm with your own movements.\r\r", false);

                outputText("You feel a mighty groan around your " + cockDescript(game.player, 0) + " and your snake tip feels Kelt's cock begin to pulse urgently.  Quickly and expertly, you wrap the end of your tail around the centaur's penis tightly, denying him release and rewarding yourself with another pained, muffled squeal reverberating through your cock.\r\r", false);

                outputText("\"<i>Ah, ah, ah,</i>\" you hiss.  \"<i>Sluts don't get off before their masters.</i>\"  A wicked idea strikes you.  Whilst still holding onto the centaur's cock with your coils, you begin to wind the tip of your tail towards Kelt's ass.  It isn't easy; you have invested the entirety of your frame into holding onto the centaur, so it is by measures that you constrict him tighter as your tail inches towards his anus.  He is bone-crushingly gripped in your coils by the time you find his sphincter, which you softly but surely sink your tip into.  Kelt struggles with the last of his strength against this final humiliation, but there is nothing he can do; with your venom plaguing his limbs, your coils wrapped hard around his frame, and your cock buried in his face, you have robbed him of everything.  Slowly you begin to feed even more of your " + cockDescript(game.player, 0) + " into his mouth, until you see his throat bulge with your manhood.  The tightness of it is sensational and you groan.  You let him get used to the sensation of deepthroating you before you begin to pick up the pace, ramming the majority of your dick into his mouth until your " + ballsDescriptLight(player) + " begin to slap against his chin.  At your other end you feel your snake tip touch something which pulses, and you gently probe it as you continue to roughly fuck his mouth, beginning to reach your peak.  Kelt only manages a few muffled screams as you milk his prostate while continuing to deny his cock release, before you silence him with a torrent of cum, pouring it directly into his stomach.\r\r", false);
        ...
        // If Female:
            ...
            outputText("\"<i>Pretty pony stepped on a snake.  Now pretty pony has to pay the price,</i>\" you say, sighing as you part your lips and allow your " + vaginaDescript(player, 0) + " to open and feel the fresh air. \"<i>You put your bud in my mouth and I will bite it off,</i>\" snarls Kelt. \"<i>Will you?</i>\" you sneer.  \"<i>I will grow one back.  You, on the other hand, will die a slow, agonising death.  Be smart.</i>\"  He stares up at you, and yes, it's fear: pure, animalistic fear of a horse for a viper. You open your mouth and bare your fangs in a wide, triumphant smile at him; venom dribbles down your chin.  He lowers his eyes and opens his mouth in submission. Needing no further invitation, you lower your " + vaginaDescript(player, 0) + " onto his tongue, massaging your hands into his hair as you do so.\r\r", false);

            outputText("He is unpracticed at first, unsure; you feel his teeth rub against your " + clitDescription(player) + " and for a moment you wonder if he actually will carry through his threat.  You tease at his horsecock with your tail, circling his head faintly and then, gently, sticking the very tip of it into his urethra.  He moans against your " + vaginaDescript(player, 0) + " and his teeth seem to vanish, replaced with a sucking, eager wetness.  You press your abdomen against him tightly, forcing your " + clitDescription(player) + " into his mouth and his tongue into your wet hole.\r\r", false);

            outputText("Girl cum begins to drip down Kelt's chin.  He is now entirely engaged with your " + vaginaDescript(player, 0) + ", moving his head back and forth as his tongue slathers your clit with attention, perhaps in the hope that the sooner he gets you off the sooner this nightmare will end.  You smirk and grip his hair, forcing his head this way and that so that no corner of your pink opening goes without attention.\r\r", false);
        ...
        // If Genderless:
            ...
            outputText("\"<i>Ah, ah, ah,</i>\" you hiss.  \"<i>Sluts don't get off before their masters.</i>\"  A wicked idea strikes you.  Whilst still holding onto the centaur's cock with your coils, you begin to wind the tip of your tail towards Kelt's ass.  It isn't easy; you have invested the entirety of your frame into holding onto the centaur, so it is by measures that you constrict him tighter as your tail inches towards his anus.  He is bone-crushingly gripped in your coils by the time you find his sphincter, which you softly but surely sink your tip into.  Kelt struggles with the last of his strength against this final humiliation, but there is nothing he can do; with your venom plaguing his limbs, your coils wrapped hard around his frame, and his tongue buried in your ass, you have robbed him of everything.  His saliva coats your opening as you clench your muscles and trap his tongue deep inside before roughly rubbing your " + buttDescription(player) + " against him, face-fucking him for all you are worth; the soft, slippery sensation inside you is immense and you pick up the pace.  At your other end you feel your tip touch something which pulses deep in his anal passage, and you gently probe it as you roughly fuck his mouth, beginning to reach your peak.  Kelt only manages a few muffled screams as you milk his prostate while continuing to deny his cock release, before you feel something clench inside of you and you begin to ride your strange but satisfying anal orgasm.\r\r", false);
    ...
    // Standard anti-kelt scene
        ...
        if (player.tallness < 50 || player.cocks.length == 0) {
        ...
        if (player.gender == 0) {
            outputText("You turn around, bending over to expose your " + assholeDescript(player) + ".  \"<i>Lick it, bitch,</i>\" you command.  When he doesn't, you give the rope a pull, and his hooves paw at the dirt in pain.  \"<i>I said LICK!</i>\" you scream, and this time he does.  His tongue slips between your " + buttDescription(player) + ".  \"<i>Deeper,</i>\" you sigh, and he complies, no longer capable of resistance.  You make him tonguefuck you until you come to a shuddering orgasm.\r\r", false);
        }
