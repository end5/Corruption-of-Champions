...
export class Kelly implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        trace("\nKelly time change: Time is " + game.time.hours + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
        if (game.time.hours > 23) {
            if (flags[kFLAGS.KELLY_REWARD_COOLDOWN] > 0 && game.time.days % 3 == 0) flags[kFLAGS.KELLY_REWARD_COOLDOWN] = 0;
...
function hasPinkEgg(): boolean {
    return (player.inv.hasItem(ConsumableLib.PINKEGG) || player.inv.hasItem(ConsumableLib.L_PNKEG));
}
...
function resistKeltsBSBreakHimIntro(): void {
    ...
    if (flags[kFLAGS.KELT_BREAK_LEVEL] == 0) {
        ...
        if (!(player.inv.hasItem(ConsumableLib.SUCMILK, 15) || (player.inv.hasItem(ConsumableLib.SUCMILK, 10) && hasPinkEgg()) || (player.inv.hasItem(ConsumableLib.P_S_MLK, 10) && hasPinkEgg()) || player.inv.hasItem(ConsumableLib.P_S_MLK, 15))) {
        ...
        if (!(player.inv.hasItem(ConsumableLib.SUCMILK, 10) || player.inv.hasItem(ConsumableLib.P_S_MLK, 10) || (player.inv.hasItem(ConsumableLib.SUCMILK, 5) && hasPinkEgg()) || (player.inv.hasItem(ConsumableLib.P_S_MLK, 5) && hasPinkEgg()))) {
    ...
    // Third encounter
        ...
        if (!(player.inv.hasItem(ConsumableLib.SUCMILK, 5) || player.inv.hasItem(ConsumableLib.P_S_MLK, 5))) {
...
function breakKeltGo(): void {
    ...
    outputText("\n\n\"<i>What do you want, you little " + mf(player, "sissy", "bitch") + "?  I'm done with you.  I'm already doing you a favor by teaching you a skill sluts like you will never use nor master.</i>\"");
    ...
    outputText(" small droplets of pre-cum begin dripping out of your " + multiCockDescriptLight(game.player) + " before being swallowed by the potent, feminine mixture down below.  The white spots are drowning in that milky goo, but you're confident that Kelt will be able to recognize the taste.");
    ...
    if (hasPinkEgg()) {
        ...
        if (player.inv.hasItem(ConsumableLib.SUCMILK, 10))
            player.inv.consumeItem(ConsumableLib.SUCMILK, 10);
        else
            player.inv.consumeItem(ConsumableLib.P_S_MLK, 10);

    }
    else {
        if (player.inv.hasItem(ConsumableLib.SUCMILK, 15))
            player.inv.consumeItem(ConsumableLib.SUCMILK, 15);
        else
            player.inv.consumeItem(ConsumableLib.P_S_MLK, 15);
    }
...
export function defeatKellyNDBREAKHIM(): void {
    ...
    outputText(" your erect " + multiCockDescriptLight(game.player) + ".");
    ...
    if (hasPinkEgg()) {
        ...
        if (player.inv.hasItem(ConsumableLib.SUCMILK, 5))
            player.inv.consumeItem(ConsumableLib.SUCMILK, 5);
        else
            player.inv.consumeItem(ConsumableLib.P_S_MLK, 5);
...
export function breakingKeltNumeroThree(): void {
    ...
    outputText(" Kelly at last: you can hear her moan and sigh, and her hands move erratically on your " + multiCockDescriptLight(game.player) + ", sometimes violently yanking it as she loses control.  This brings all kinds of painful yet pleasurable tingles to your body, and only entices you to feel her up even more.");
...
function finalKeltBreaking(): void {
    ...
    outputText("\n\nYou laugh triumphantly as you whip your " + multiCockDescriptLight(game.player) + " out of your [armor]; the slut is more than ready!");
    ...
    outputText("\n\nThe cum-flow eventually starts to ebb; less and less goo pours from your " + multiCockDescriptLight(game.player) + " and your orgasm gradually fades out.  When your euphoric pleasure wears off, you finally get a good look at the creature standing before you.");
...
function approachKelly(): void {
    ...
    if (flags[kFLAGS.KELLY_VAGINALLY_FUCKED_COUNT] > 0 && flags[kFLAGS.TIMES_PUNISHED_KELLY] == 0 && rand(3) == 0 && player.cocks.length > 0) {
    ...
    if (game.time.hours >= 9 && game.time.hours <= 11 && flags[kFLAGS.KELLY_KIDS] >= 2) {
    ...
    if (flags[kFLAGS.KELLY_CUNT_TYPE] == 0) {
        if (player.inv.hasItem(ConsumableLib.EQUINUM)) {
        ...
        if (player.inv.hasItem(ConsumableLib.SUCMILK)) {
    ...
    if (player.inv.hasItem(ConsumableLib.CANINEP)) {
    ...
    if (flags[kFLAGS.KELLY_VAGINALLY_FUCKED_COUNT] > 0 && flags[kFLAGS.KELLY_DISOBEYING_COUNTER] >= 3 && player.cocks.length > 0) {
...
function kellySexMenu(): void {
    ...
    if (player.cocks.length > 0 && player.lust >= 33) {
        if (player.cocks.cockThatFits(300) >= 0 || flags[kFLAGS.KELLY_CUNT_TYPE] == 1) {
            ...
            if (player.cocks.tentacleCocks() >= 2) addButton(1, "TentaFuck", tentaFuckKelly);
        }
        else outputText("\n<b>You're too big to fuck her vagina.</b>");
        if (flags[kFLAGS.KELLY_BONUS_BOOB_ROWS] == 0 && player.cocks.cockThatFits(18, "length") < 0 && !player.isTaur()) {
...
function fuckKellysCunt(): void {
    ...
    outputText("Holding your " + cockDescript(game.player, x) + " in one hand, you press your " + cockHead(player, x) + " against her entrance.   The steamy cunt pulses wetly around your girth, slowly spreading its thick lips to accept your rigid endowment.");
    if (player.cocks.length > 1) {
        outputText("  Your other erection");
        if (player.cocks.length > 2) outputText("s hang");
        else outputText(" hangs");
        outputText(" down below, collecting her drippings and smearing across her belly.");
    }
    outputText("  You slowly slide inside the tight tunnel, feeling Kelly's powerful muscles squeeze delightfully around you, massaging your " + cockDescript(game.player, x) + " with slow undulations.");
    ...
    if (player.cocks.cockArea(x) >= 300) outputText("bottom out inside her");
...
function taurOnTaurSexKelly(): void {
    ...
    outputText("\n\nKelly groans through her pain, \"<i>Ooohh, yes, give me your foal, [Master]!  My pussy needs it!</i>\"  True to her words, her walls begin to tug at your " + cockDescript(game.player, x) + " with familiar, milking contractions, squeezing you from base to " + cockHead(player, x) + " again and again.  Her fluids gush out everywhere, turning the ground underneath her to mud as she climaxes.  You pinch her nipples as the heat builds within you, and when you can take it no more, you reward your cum-slut with the liquid orgasm she so craved.  Jism basts her inner walls when you explode.  Again and again, your muscles clench with ecstatic blast, ejaculating thick flows of your lust straight into the cum-dumpster's spasming cunny.  ");
    if (player.cumQ() >= 1000) {
...
function tentaFuckKelly(): void {
    ...
    outputText("\n\nSLAM!  You profit from the slut's babbling to shove your " + cockDescript(player, one) + " right into her gaping mouth, forcefully filling her throat with dick-flesh.  Her lips instinctively cling to your shaft, sucking it like a popsicle.  Drops of pre-cum are quickly swallowed and ingested by the hungry whore as they escape your urethra.  You begin thrusting in and out of her warm mouth, but her lips squeeze your rod so tight that you have to fight for every inch.  As you stuff her full of your " + cockDescript(player, one) + ", you can hear her endless muffled moans; the slut is obviously getting off to the taste of your meaty junk.  Completely driven with lust, the green tentacle prick is vibrating on its own as it fucks Kelly's throat, sending you soft, prickling sensations.  You can't help but release a grunt as the centaur slut fellates you so expertly.");
    outputText("\n\nKelly is doing an incredible job with her mouth, teasing you into oblivion.  Your " + cockDescript(player, two) + " is convulsing recklessly in frustration, begging to be stuffed into some warm hole.  Therefore, you naturally direct it toward the centauress' wet snatch; it is easily spottable due to the regular streams of juices that ooze from it.  Moreover, the slut is incredibly turned on from sucking you off, and the fem-spunk trails are even thicker than usual.  It doesn't take long for your secondary tentacle dong to reach the entrance of her moist love-tunnel.  You grind the tip of your " + cockDescript(player, two) + " against it, using her genitals as a masturbation aid.  When it is hard as bark and ready to go, you ram it through Kelly's fuck-hole, utterly piercing her from behind.");

    outputText("\n\nYou ream the centaur slut from both sides, making her whinny and thrash violently.  She tramples the floor with her hooves, arches her back and screams in ecstasy through the inches of dick-flesh blocking her throat; she is completely heedless of the world beside the two hard vegetal poles fully impaling her on either end.  You pump away in rhythm, synchronizing each thrust in both holes.  Her twat is almost inhumanly hot, and gloriously tight.  However, the slut tries her best to wrap her lips even tighter around your main flexible pecker, and her tongue is teasing your " + cockDescript(player, one) + " in the most excruciating way.  You stuff the mare slut full of your green appendages, treating her body like a bipolar cocksleeve.");

    // [if (cocks > 2)
    if (three >= 0) {
        outputText("\n\nThe display of Kelly trembling in pleasure as you spit her like a pig on your duo of plant-like pricks is enticing, and your " + cockDescript(player, three) + " is aching with need.  It almost moves on its own volition toward the woman's perky tits, ready for a good boobjob.  The supple green wang lodges itself between the busty melons; the slut instantly begins to squeeze her heavy globes around the rod of dick-flesh, fully entrapping it in her bosom.  Her tits are so tender and soft you feel like your tentacle pecker is about to melt.  Groaning in pleasure, you thrust your " + cockDescript(player, three) + " up and down as the titty-fucked whore accompanies your motion.  You grind in rhythm so the slut doesn't forget your other tentacle dongs.  Fortunately, Kelly quickly picks up your intention and soon you can't tell which member is providing you with the most pleasure; her mouth, vagina and soft tits are all working in conjunction, and you are fully lost in a never-ending spiral of ecstasy.");
    }
    // if (cocks > 3)
    if (four >= 0) {
        outputText("\n\nYour " + cockDescript(player, four) + " trembles furiously, unable to find some body part that isn't taken by its colleagues.  You grin wickedly as an idea comes to your mind.  The protruding appendage writhes as it threads its way toward Kelly's right hand; the slut is still focusing on your " + cockDescript(player, three) + ", squeezing her tits as tightly as she can.  Needless to say, she is surprised when another tentacle shoves itself in the way, right under her palm.  She doesn't lose her composure, though; using her forearm to press on her chest, her hand quickly grabs hold of your tree-like pecker and begins stroking your length.  Although her handjob doesn't feel as warm and tight as the wet depths of her love-tunnel or her throat, or even the squelching contact of her tender boob-flesh, she teases you efficiently with her fingers.  Her hand massages your " + cockDescript(player, four) + " and clutches it like a vice, milking some drops of pre-cum out of your shaft.  She takes care of the most sensitive points and often makes you moan when she squeezes the towering rod.");
    }
    // [if (cocks > 4)
    if (five >= 0) {
        outputText("\n\nKelly still has a free hand, and your " + cockDescript(player, five) + " is left alone, flustered.  The squirming tentacle quickly finds a comfortable spot near her boob, but this time you don't need to do anything; the eager slut catches the vegetal rod and begins to stroke it furiously.  Her hands pump along your lengths in turn; both pricks are so hard that you don't know which one will cum first.  Her fingers move across the surface of your meaty green dongs with expert care, teasing you more and more.  You enjoy the double handjob, your duo of plant-like dongs ready to explode in a mess of cum.");
    }
    // [if (cocks > 5)
    if (player.cocks.tentacleCocks() > 5) {
...
function giveKellyEquinum(): void {
    ...
    player.inv.consumeItem(ConsumableLib.EQUINUM);
    menu();
    addButton(0, "Next", approachKelly);
}
...
function getARimjobFromKelly(): void {
    ...
    // [Centaur:]
    else if (player.isTaur()) outputText("\n\nYou gesture at the ground.  \"<i>Lie down,</i>\" you command.  Kelly carefully folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You lead yourself back until your rear end is near to her face; grinning to yourself, you slowly brush your semi-erect " + cockDescript(game.player, x) + " against her soft face, teasing her with the idea that you've changed your mind; you cannot see her but from the small bitten off grunts and moans you know she's struggling with the urge to wrap her lips around it.  Finally, you carefully lean downwards, so that instead of your dick she's faced with your [butt].");

    // [Biped/Other:
    else outputText("\n\nYou gesture at the ground.  \"<i>Lie down,</i>\" you command.  Carefully Kelly folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You step forwards and for a few moments slowly brush your semi-erect " + cockDescript(game.player, x) + " against her soft face, teasing her with the idea that you've changed your mind; she struggles with the urge to wrap her lips around it, emitting small bitten off grunts and moans to your ministrations.  Finally, with a grin you pull away and turn around, so that her mouth is now facing your [butt].");
    ...
    if (player.cocks.length > 1) outputText("s harden");
    ...
    // [Repeat:
        ...
        outputText("\n\n\"<i>Th-thank you for the lesson, [Master],</i>\" she says.  She knows the drill by now; she quickly gets to her feet and then trots over to the off-white slimy mess you have created.  What a good girl.  Before she can get started you present your " + cockDescript(game.player, x) + "; she murmurs thanks for the gift and licks your oozing head spotless first before getting on with the main meal.  You rub her behind the ear as she licks away at the spent seed at your feet and then leave her to it.");
        if (player.cumQ() >= 1000) outputText("  She is, after all, going to be here for a while.");
    }
    player.orgasm();
...
function rideKellyForPunishment(): void {
    ...
    if (flags[kFLAGS.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT] == 1) {
        ...
        // [PC not rider:
            ...
            if (!player.isTaur()) {
                ...
                outputText("\n\nNeither of you are in any condition to keep this up for long, and after five minutes of fevered fucking, you groan as you clutch Kelly's ass firmly and pour yourself into her, your " + cockDescript(game.player, x) + " surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were inside her.");
            }
            // Centaur:
            else {
                outputText("\n\nEventually you arrive back at camp, and not a moment too soon.  Your nostrils are full of the smell of Kelly, the sight of her hot, moist skin and you are practically champing at the bit yourself; even her rippling horse body speaks directly to your own, urging you irresistibly to mount her.  You bring her to a shuddering halt, rip off your [armor], and then without any further ado rear onto her back and sink your straining " + cockDescript(game.player, x) + " straight into her cunt.  It is absolutely soaked with arousal and you encounter virtually no resistance as you thrust your hips into her soft butt and immediately begin to fuck her as hard as you can.  In your sex daze you realize you are still clutching the reigns; you pull at them intermittently as you push as much of yourself in the centauress hot, grasping warmth as you can, making her rear, squealing and then screaming around her gag to the dual sensation of her [master]'s dick burying itself in her and the harness pulling tight upon her head and bit, thrusting back into you as best she can.");
                outputText("\n\nYou wish you could live forever in these luscious moments; it is so indefinably right to be reared over your eager submissive, encompassing her with your own flesh as you penetrate her deep wetness, both your human and horse parts cry out for it; mounting her, breeding her, imprinting yourself upon her and making her yours.  Neither of you are in any condition to keep it up for long though, and after five minutes of this fevered fucking you groan as you clutch Kelly's breasts hard and pour yourself into her, your " + cockDescript(game.player, x) + " surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were inside her.");
            }
        }
        kellyPreggers();
        // All go to:
    ...
    // First repeat:
        ...
        // PC not rider:
            ...
            outputText("\n\nUpon the close streets though everyone stares; even for as liberal a place as Tel'Adre the sight of a " + race(player) + " leading a practically naked bound and gagged centauress draws a slight hush and open mouths.  The centaurs you canter past in particular look absolutely gobsmacked; by turns unbelieving, disgusted, or shocked into laughter.  Nobody attempts to stop you though and you affect to be completely unaware of the attention; you after all are a champion upon an important errand.  But you know the attention and pure humiliation is soaking into Kelly when she makes a sobbing, muffled moan.");
        ...
        // [Not centaur:
        if (!player.isTaur()) outputText("   You sigh and enjoy the sight of her pretty head bobbing over your groin, your " + cockDescript(game.player, x) + " encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile and hold her face as she basks in an afterglow earned from pure debasement.");
        else outputText("  You sigh and enjoy your " + cockDescript(game.player, x) + " encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile, turn and hold her face as she basks in an afterglow earned from pure debasement.");
    ...
    // Second+ repeat:
        ...
        if (player.tallness < 78 && !player.isTaur()) {
            ...
            outputText("\n\nUpon the close streets though everyone stares; even for as liberal a place as Tel'Adre the sight of a " + race(player) + " riding a practically naked bound and gagged centauress draws a slight hush and open mouths, even among the ones who have already seen this display.  The centaur you canter past in particular look absolutely gobsmacked; by turns unbelieving, disgusted, or shocked into laughter.  Nobody attempts to stop you though and you affect to be completely unaware of the attention; you after all are a champion upon an important errand.  But you know the attention and pure humiliation are soaking into Kelly when she makes a sobbing, muffled moan.");
            ...
            outputText("\n\nYou lead her down a side alley - giving a hard look to the onlookers which makes most of them remember they have business elsewhere- and then, braced against her formidable weight, slowly dip your " + cockDescript(game.player, x) + " into her cunt until it is completely slathered in girl slime.  She sighs to the agonisingly gentle treatment and then gives out a muffled cry out of shock as you withdraw completely and, using the lubrication she provided, push into her tight asshole.");
        ...
        // PC not rider:
            ...
            outputText("\n\nUpon the close streets though everyone stares; even for as liberal a place as Tel'Adre the sight of a " + race(player) + " leading a practically naked bound and gagged centauress around like a show horse draws a slight hush and open mouths, even among the ones who have already seen this display.  The centaur you canter past in particular look absolutely gobsmacked; by turns unbelieving, disgusted, or shocked into laughter.  Nobody attempts to stop you though and you affect to be completely unaware of the attention; you after all are a champion upon an important errand.  But you know the attention and pure humiliation are soaking into Kelly when she makes a sobbing, muffled moan.");
...
function takeKellysVirginity(): void {
    ...
    if (!player.isTaur()) outputText("get down behind her and line your " + cockDescript(game.player, x) + " with her dribbling lips.");
    else outputText("lie down behind her, sliding your hindlegs behind hers in order to properly align your " + cockDescript(game.player, x) + " with her dribbling lips.");
    outputText("  The first contact of " + cockHead(player, x) + " on her feminine mound sends an electric current of pleasure through your member, one that has you immediately and heedlessly pushing forward, sliding your full, sensitive phallus as deeply into that slick opening as you can.  That turns out not to be very far, as you almost immediately hit her virginal hymen.  It blocks off the rest of that hot, dripping hole almost spitefully.");
    ...
    else outputText(", pressing your " + cockHead(player, x) + " up against the barrier her cervix makes");
    ...
    outputText("\n\nYou pull on her tail as you begin to slide in and out, asking, \"<i>Did you like that, slut?  Does your cunt like being broken in by a real " + mf(player, "man", "breeder") + "'s cock?</i>\"");

    outputText("\n\nKelly moans, \"<i>Y-yes!  My... pussy...  My pussy feels so good!  It's so good to be yours, [Master]!</i>\"  Her eyes cross slightly as her pussy begins to squeeze down around you, driven by instinctual desires the lust-crazed centauress barely understands.  She's been so conditioned to obey that when her body tells her to do something (like milk your " + cockDescript(game.player, x) + "), she doesn't question - she simply, deliciously complies, her pussy growing ever-wetter from her submissiveness.");
...
function kellyTitJob(): void {
    ...
    if (flags[kFLAGS.KELLY_BONUS_BOOB_ROWS] == 0) x = player.cocks.cockThatFits(18, "length");
    else x = player.cocks.biggestCockIndex();
...
function kellyPregSex(): void {
    ...
    else x = player.cocks.cockThatFits(300);
    if (x < 0) x = player.cocks.smallestCockIndex();
    ...
    outputText("\n\nYou remove your [armor] and watch your " + multiCockDescriptLight(game.player) + " grow into a rock-hard erect state under the aphrodisiac scent; you're ready to stuff the breeding mare before you.  Driven mad with lust, you jump the centaur's behind and slap her angrily, making her moan and snort in submissive pleasure.  You can't take it anymore and with feverish excitement, you grab hold of [oneCock] and plunge it deep into Kelly's dark recesses.");
...
function talkNHandToKelly(): void {
    ...
    if (flags[kFLAGS.KELLY_TALK_N_HAND_TIMES] == 0) {
        ...
        if (!player.isTaur()) {
            ...
            if (player.cocks.length == 1) outputText("[oneCock], gently tugging it.  It quickly hardens with the pleasing friction she coils around it.");
        ...
        // Centaur:
            ...
            if (player.cocks.length == 1) outputText("a warm hands wraps itself around your [cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.");
    ...
    // 2.
        ...
        if (player.cocks.length > 1) outputText(" and [cock 2]");
...
function rewardKelly(): void {
    ...
    if (player.inv.hasItem(ConsumableLib.BLACK_D) && flags[kFLAGS.KELLY_HAIR_COLOR] != "sable black") addButton(1, "Black Dye", dyeKellysBitchAssHair, ConsumableLib.BLACK_D);
    if (player.inv.hasItem(ConsumableLib.BLOND_D) && flags[kFLAGS.KELLY_HAIR_COLOR] != "slutty blonde") addButton(2, "Blond Dye", dyeKellysBitchAssHair, ConsumableLib.BLOND_D);
    if (player.inv.hasItem(ConsumableLib.PURPDYE) && flags[kFLAGS.KELLY_HAIR_COLOR] != "garish purple") addButton(3, "Purple Dye", dyeKellysBitchAssHair, ConsumableLib.PURPDYE);
    if (player.inv.hasItem(ConsumableLib.PINKDYE) && flags[kFLAGS.KELLY_HAIR_COLOR] != "bright pink") addButton(4, "Pink Dye", dyeKellysBitchAssHair, ConsumableLib.PINKDYE);
    if (player.inv.hasItem(ConsumableLib.BROWN_D) && flags[kFLAGS.KELLY_HAIR_COLOR] != "chestnut brown") addButton(5, "Brown Dye", dyeKellysBitchAssHair, ConsumableLib.BROWN_D);
...
function dyeKellysBitchAssHair(color: ItemType): void {
    ...
    // Brown dye: Kelly smiles as she examines the regained chestnut brown color of her hair and tail.
        ...
        player.inv.consumeItem(ConsumableLib.BROWN_D);
    }
...
function giveKellyAppleSauce(): void {
    ...
    if (!player.isTaur()) {
        ...
        // 1 < cock:
        if (y >= 0) {
            outputText("\n\nYour eyes fasten upon her tight, puckered asshole as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your " + cockDescript(game.player, x) + " from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your " + cockDescript(game.player, y) + " with her gaping vagina.");
            outputText("\n\nYou wait for Kelly's head to descend towards the bowl again and then thrust forward, penetrating both her holes at the same time.  She cries out from the intense sensation and then bucks back into you, helping you sink your " + cockDescript(game.player, x) + " deeper and deeper into her ass, spreading her deliciously tight anal passage whilst your " + cockDescript(game.player, y) + " slides effortlessly into the fleshy folds of her cunt.  You slap her ass exultantly, making her squeak again, as you find your limit before resuming the same rhythm, fucking both her sopping vagina and tight, hot ass now with deep, slow strokes.");
        }
        ...
        outputText("as you push towards your reward.  Panting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging " + cockDescript(game.player, x) + " in milking ripples");
        if (y >= 0) outputText(" as her anus clenches around your " + cockDescript(game.player, x));
    ...
    // Centaur:
        ...
        outputText("\n\nYou tut mockingly and pull it away until she lowers her hands, then bring it in close to her face.  Her hands fall on her erect nipples as she dips her head into the bowl, scoops up a long lap of the green mash with her tongue and guides it into her wet mouth.  Her throat works and she closes her eyes.  The intermittent dripping of her femcum on the straw behind you turns into a steady trickle and your own arousal urgent now, you push your " + cockDescript(game.player, x) + " against her sopping pussy gently.  She is radiating heat and her wet depths suck at your " + cockHead(player, x) + " deliciously.");
        ...
        outputText("\n\n\"<i>Oh [Master]...</i>\" Kelly groans, arching her head back from her feed so it spoons into your neck.  A series of clenches grip your " + cockDescript(game.player, x) + " and a sudden gush of fluid warms your ");
        if (player.balls > 0) outputText("[balls]");
        else outputText("thighs");
        ...
        // 1 < cock:
        if (y >= 0) {
            outputText("\n\nYou think about her tight, puckered asshole beneath you as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your " + cockDescript(game.player, x) + " from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your " + cockDescript(game.player, y) + " with her gaping vagina.  You wait for Kelly's head to descend towards the bowl again and then thrust forward, penetrating both her holes at the same time.  She cries out from the intense sensation and then bucks back into you, helping you sink your " + cockDescript(game.player, x) + " deeper and deeper into her ass, spreading her deliciously tight anal passage whilst your " + cockDescript(game.player, y) + " slides effortlessly into the fleshy folds of her cunt.  You grab one of her soft tits and squeeze exultantly, making her squeak again, as you find your limit before resuming the same rhythm, fucking both her sopping vagina and tight, hot ass now with deep, slow strokes.");
        }
        ...
        outputText("\n\nPanting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging " + cockDescript(game.player, x) + " in milking ripples");
        if (y >= 0) outputText(" as her anus clenches around your " + cockDescript(game.player, y));
...
function kellyBJsAhoy(): void {
    ...
    if (!player.isTaur()) {
        ...
        if (player.balls > 0 && player.vaginas.length > 0) {
            ...
            if (player.ballSize <= 8) {
                ...
                outputText("breasts whilst slavishly polishing the [balls] of the " + mf(player, "man", "woman") + " who made her into, well, a ball-licking bitch.  Whatever the cause the result is the same: pleasure thrums through your groin and up your [cock biggest], and you close your eyes, lost to the sensations of her skillful tongue.");
            }
            // [Huge balls:
            else {
                ...
                if (pregnancy.isPregnant) outputText(", milk laden");
                outputText(" breasts whilst slavishly polishing the [balls] of the " + mf(player, "man", "woman") + " who made her into, well, a ball-licking bitch.");
                // [Dominika dreams:
        ...
        if (player.cocks.biggestCockLength() > 15) outputText("It's a long way up and the effect is like a lift slowly rising, turning on a glow of lights on each level as it goes.  ");
    ...
    // Centaur:
        ...
        // [Cock >15 inches:
        if (player.cocks.biggestCockLength() > 15) {
        ...
        outputText("\n\nBit by bit, she takes more and more of your [cock biggest] into her milking redness.  ");
        // [Cock >12:
        if (player.cocks.biggestCockLength() > 12) {
