...
export function corruptFollowers(): number {
    ...
    if (AmilyScene.amilyCorrupt()) count++;
    if (IzmaScene.izmaFollower()) count++;
    if (SophieBimbo.bimboSophie()) count++;
    if (Vapula.vapulaSlave()) count++;
    if (CeraphScene.ceraphIsFollower()) count++;
...
export function updateFarmCorruption(): number {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && SophieBimbo.bimboSophie()) modValue += 1;
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 1 && flags[kFLAGS.FOLLOWER_PRODUCTION_JOJO] == 1 && flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1) {
        ...
        if (JojoScene.tentacleJojo()) flags[kFLAGS.FARM_INCUDRAFT_STORED]++;
    }
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_LATEXY] == 1) {
        LatexGirl.gooHappiness(-0.5, false);
        LatexGirl.gooObedience(0.5, false);
    }
...
export function farmProtection(): number {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1) {
        if (SophieBimbo.bimboSophie()) protection += 1;
        else protection += 2;
    }
...
export function farmValue(): number {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1) {
        if (SophieBimbo.bimboSophie()) fValue += 4;
        else fValue += 3;
    }
...
export function takeoverPrompt(): boolean {
    ...
    if (player.cor >= 70 && corruptFollowers() >= 2 && player.level >= 12) {
        if (flags[kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY] == 0) {
            ...
            else if (player.effects.findByType(StatusAffects.MarbleRapeAttempted) >= 0) {
...
function takeoverPromptKelly(): void {
    ...
    if (AmilyScene.amilyCorrupt() || JojoScene.campCorruptJojo()) {
...
export function takeoverPromptNow(): void {
    ...
    if (flags[kFLAGS.KELT_BREAK_LEVEL] >= 4 || player.effects.findByType(StatusAffects.MarbleRapeAttempted) >= 0) {
...
export function rootScene(): void {
    ...
    if (!whitneyCorrupt()) {
        ...
        if (silly()) {
            outputText(mf(player, "You feel an irresistible hankering for a cigar.", "You feel an irresistible hankering for a cigarette holder."));
        }
        if (flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1) outputText(" A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.");
        if (flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 1) outputText(" Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.");
    }
    ...
    // Bimbo Sophie
    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && SophieBimbo.bimboSophie()) {
...
function slavesAtFarmMenu(): void {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 1) addButton(1, "Jojo", JojoScene.corruptCampJojo);

    if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && SophieBimbo.bimboSophie()) addButton(2, "Sophie", SophieBimbo.approachBimboSophieInCamp);

    if (flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1) addButton(3, "Vapula", Vapula.callSlaveVapula);

    if (flags[kFLAGS.FOLLOWER_AT_FARM_LATEXY] == 1) addButton(4, flags[kFLAGS.GOO_NAME], LatexGirl.approachLatexy);

    if (flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1) addButton(5, flags[kFLAGS.MILK_NAME], MilkWaifu.milkyMenu);
...
function loversAtFarmMenu(): void {
    ...
    if (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 2) addButton(0, "Izmael", gameOver);

    if (flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 1) addButton(1, "Isabella", IsabellaFollowerScene.callForFollowerIsabella);
...
function dontDeFurDoge(): void {
    ...
    if (player.cocks.length > 0) outputText(" crotch into");
...
function firstOralTraining(): void {
    ...
    if (player.cocks.length > 0) outputText(" cocksucker");
    ...
    outputText("\n\n“<i>Yes, [master]. I will tr- I will.</i>” She takes a long breath and transfers her gaze to " + ((player.cocks.length > 0) ? "[eachCock]" : "[vagina]") + ", looking for all the world like she’s about to embark on a marathon.");
...
function cockOralTrainingStageOne(): void {
    ...
    if (player.cocks.biggestCockLength() > 12) outputText(" She still struggles a bit with her gag reflex but it only takes her a few tries to swallow your thick prick, allowing you to pump into the delightful tightness of her throat without issue.");
...
function firstCockOralTrainingStageTwo(): void {
    ...
    if (player.vaginas.length > 0 && player.balls > 0) {
        ...
        outputText("\n\nEventually you cannot hold your lust back. You grab her by the head and begin to thrust deep into her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ". Her breath hisses through her nose as she lets you use her mouth, struggling to keep her fingers schlicking away; you coo as your own hard movements force her to move her hands fast and erratically, skittering across your urgently bulging button and making you pump into her wet, milking warmth all the more frenetically.");

        outputText("\n\n“<i>Cum, bitch.</i>” you grit as you surge to your own high. Whitney’s muffled moans are what you hear as your cock-slit dilates and pumps jizz down her throat; you crow with delight as your [vagina] quivers and then clenches in orgasm in ecstatic sympathy with your male sex, " + ((player.vaginas.wetness() <= 2) ? "wetting your slave’s hand" : "absolutely soaking your slave’s arm") + " with femjizz as it does. She shudders around you and her eyes roll as she finds her own high rooting in her pussy; you are extremely gratified to note that, despite being on a plane of submissive pleasure far away, she still manages to swallow every drop of cum you eject");
    ...
    // Herm without balls
        ...
        outputText("\n\nEventually you cannot hold your lust back. You grab her by the head and begin to thrust deep into her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ". Her breath hisses through her nose as she lets you use her mouth, struggling to keep her fingers schlicking away; you coo as your own hard movements force her to move her hands faster and more erratically, skittering across your urgently bulging button and making you pump into her wet, milking warmth all the more frenetically.");
    ...
    // Male w/ balls:
        ...
        if (player.cocks.length == 1) {
            ...
            outputText("\n\nEventually you cannot hold your lust back. You grab her by the head and begin to thrust deep into her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ". Her breath hisses through her nose as she lets you use her mouth, struggling to keep her fingers schlicking away; you coo as your own hard movements force her to push slightly into your ass, making you pump into her wet, milking warmth all the more frenetically.");
...
function firstCockOralTrainingStageThree(): void {
    ...
    if (!player.vaginas.length > 0) {
        ...
        outputText("\n\nThe sight and sound of her inundates your senses with the same drowning intensity of the pleasure enveloping your maleness and you pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her" + ((player.cocks.biggestCockLength() < 8) ? " mouth" : " throat") + ". She moans deeply as you begin to use her like this, and even when you’re face fucking her she does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of " + ((player.cocks.biggestCockLength() < 8) ? "the back of her mouth" : "her gullet") + " as you spear decisively inwards. ");
    ...
    // Herm
        ...
        outputText("\n\nThe sight and sound of her inundates your senses with the same drowning intensity of the pleasure enveloping your extensive sex and you pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ". She moans deeply as you begin to use her like this, and even when you’re face fucking her she does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of ");
        if (player.cocks.biggestCockLength() < 8) outputText("the back of her mouth");
        ...
        if (player.vaginas.wetness() > 2) outputText("soaking her arm as it gutters your juices, ");
...
function cockOralTrainingStageThree(): void {
    ...
    if (player.vaginas.length > 0 && player.cocks.length >= 2) outputText(" and");
    if (player.vaginas.length > 0) outputText(", leisurely exposing your moist [vagina]");
    if (player.cocks.length >= 2 && player.vaginas.length > 0) outputText(" at the same time");
    ...
    if (!player.vaginas.length > 0) {
        ...
        outputText("\n\nYou grab hold of those thoughts, filling them with a black power as the drowning intensity of the pleasure inundating your maleness increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suck of her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ". With some effort you stay afloat and consider the dog woman as she whimpers in delight to you hammering into her juicy mouth");
        ...
        outputText("\n\nEven when you’re face fucking her Whitney does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of " + ((player.cocks.biggestCockLength() < 8) ? "the back of her mouth" : "her gullet") + " as you spear decisively inwards. You can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark wordlessly with each bucket of jizz you fountain down her throat. ");
    ...
    // Herm
        ...
        if (player.cocks.biggestCockLength() < 8) outputText("mouth");
...
function cockOralTrainingMaxed(): void {
    ...
    if (!player.vaginas.length > 0) {
        ...
        outputText("\n\nPushing her all the way down to the base of your [cock biggest] so that the heavy breath through her nose warms your crotch and holding her there for a time, you enjoy the glorious closeness of the back of her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat"));
        ...
        outputText(" pulling and kneading you now with real need. The drowning intensity of the pleasure inundating your maleness increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ".");

        outputText("\n\nAs always when you’re face fucking her Whitney does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of " + ((player.cocks.biggestCockLength() < 8) ? "the back of her mouth" : "her gullet") + " as you spear decisively inwards, her lush cock pillows providing... not resistance but a wonderful squeezing lushness moving up and down your urgently bulging length.");
    ...
    // Herm
        ...
        outputText(" as if they never left them, pulling, kneading and curling into you now with real need. The drowning intensity of the pleasure inundating your sex increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her " + ((player.cocks.biggestCockLength() < 8) ? "mouth" : "throat") + ".");

        outputText("\n\nEven when you’re face fucking her Whitney does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of " + ((player.cocks.biggestCockLength() < 8) ? "the back of her mouth" : "her gullet") + " as you spear decisively inwards. Her lush cock pillows provide no resistance but a wonderful squeezing lushness running up and down your urgently bulging length. You can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark wordlessly with each bucket of jizz you fountain down her throat. ");
...
function firstVaginaOralTrainingStageTwo(): void {
    ...
    if (player.vaginas.wetness() <= 2) outputText(" wetting both your slave’s hand and mouth");
    ...
    if (player.vaginas.wetness() >= 5) outputText(" which is no mean feat, given it feels like you gutter what feels like a gallon down her gullet. Once you’re almost done, as ever you get up and spurt the last of it on her hair and clothes");
    outputText(".");
...
function vaginaOralTrainingStageTwo(): void {
    ...
    if (player.vaginas.wetness() >= 5) outputText(", fountaining fem juice down her throat");
    ...
    if (player.vaginas.wetness() >= 5) outputText(", soaking her again and again with warm, female approval");
    outputText(".");
...
function firstVaginaOralTrainingStageThree(): void {
    ...
    if (player.vaginas.wetness() >= 5) outputText(", soaking her mouth and face again and again with warm, female approval");
    ...
    if (player.vaginas.wetness() >= 5) outputText(" It is in a state of ecstatic momentum that you rise away from her mouth and spurt the last of it onto her hair and clothes. Red with her own huge high, the dog woman does her level best to catch your spotting fluids in her mouth.");
...
function whitneyDomPleasure(): void {
    ...
    const scenes: any[] = new Array();

    if (!player.isTaur()) scenes.push(whitneyDomBondageOral);
    if (player.cocks.length > 0 || player.vaginas.length > 0) scenes.push(whitneyDomStraponDoggy);
    if (player.cocks.length > 0) scenes.push(whitneyDomRide);
...
function whitneyDomStraponDoggy(): void {
    ...
    // wet and/or loose
    const capacity: number = (player.vaginas.length > 0) ? player.vaginalCapacity() : player.analCapacity();
    if (capacity >= 36) {
        outputText("\n\nThe dog woman huffs in surprise as she slides her prosthetic length in with ease, your ");
        if (player.vaginas.length > 0) outputText("deep, eager vagina");
        ...
        if (player.vaginas.length > 0) outputText("sit and spread ‘em for anything going, don’t you?");
        ...
        if (player.vaginas.length > 0) outputText("take of your tight cunt");
        ...
        if (player.vaginas.length > 0) outputText("servants, you tight, bossy little bitch");
...
export function whitneyDomRide(): void {
    ...
    outputText("\n\n“<i>Stop fussing now, sweetheart,</i>” Whitney croons, adjusting the black, elastic blindfold so it is firmly secured over your eyes." + ((player.eyeType == EyeType.FOUR_SPIDER_EYES) ? " Once she’s finished with that she places another blindfold over your second pair of eyes. You can only groan with laughter at the sheer level of preparation the dog woman is capable of." : "") + " “<i>Wouldn’t want you to see what’s coming, would we? That’d ruin half the fun.</i>” ");
    ...
    let cockI: number = player.cocks.cockThatFits(whitneyVagCapacity() * 1.33) + 1;
    ...
    else if (player.cocks.cockArea(cockI - 1) > (whitneyVagCapacity() * 0.66)) {
...
function brandingMenu(): void {
    ...
    if (flags[kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] == 0) {
        ...
        if (player.gems >= 50) {
            ...
            if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && SophieBimbo.bimboSophie() && hasFreeTattooSlot("sophie")) addButton(3, "Sophie", brandBimboSophie);
...
export function brandAmily(): void {
    ...
    else if (player.vaginas.length > 0) outputText(" pussy");
...
export function brandSelect(slot: number): void {
    ...
    if (player.cocks.length > 0 && whitneyMaxedOralTraining()) addButton(4, "Cocksucker", champCocksuckerTattoo, slot);
    if (player.vaginas.length > 0 && whitneyMaxedOralTraining()) addButton(5, "Pussylicker", champPussylickerTattoo, slot);
...
export function amilyBrandSelect(slot: number): void {
    ...
    if (player.cocks.length > 0 && slot == 2) addButton(4, "Cock Here", amilyCockGoesHereTattoo, slot);
    if (player.vaginas.length > 0) addButton(5, "Mommy's Girl", amilyMommysGirlTattoo, slot);
...
export function jojoBrandSelect(slot: number): void {
    ...
    if (player.cocks.length > 0 && slot == 2) addButton(4, "Cock Here", jojoCockGoesHereTattoo, slot);
    if (player.vaginas.length > 0) addButton(5, "Mommy's Boy", jojoMommysBoyTattoo, slot);
...
export function bimboSophieBrandSelect(slot: number): void {
    ...
    if (player.cocks.length > 0 && slot == 2) addButton(6, "Cock Goes Here", bimboSophieCockGoesHereTattoo, slot);
...
export function smallMilkyBrandSelect(slot: number): void {
    ...
    if (slot == 0 && player.cocks.length > 0) addButton(6, "Cock Cozy", smallMilkyCockCozyTattoo, slot);
...
export function bigMilkyBrandSelect(slot: number): void {
    ...
    if (slot == 0 && player.cocks.length > 0) addButton(5, "Cock Cozy", bigMilkyCockCozyTattoo, slot);
...
function orgyRoomRouter(): void {
    ...
    if (flags[kFLAGS.FARM_UPGRADES_ORGYROOM] == 1) {
        if (whitneyDom()) {
            ...
            if (!player.isGoo() && !player.isTaur() && (player.cocks.length > 0 || player.vaginas.length > 0)) doFunctor = orgyRoomSubMassage;
        }
    }
...
function orgyRoomSubMassage(): void {
    ...
    if (player.cocks.length > 0) outputText(" grips your [cock]");
    else if (player.vaginas.length > 0) outputText(" traces the entrance of your [vagina]");
    ...
    if (player.isBiped()) {
        ...
        if (!(player.lowerBody == LowerBodyType.HOOFED || player.lowerBody == LowerBodyType.NAGA || player.lowerBody == LowerBodyType.GOO || player.lowerBody == LowerBodyType.PONY)) {
            ...
            if (player.cocks.length > 0) outputText(" cock thicken");
            if (player.cocks.length > 0 && player.vaginas.length > 0) outputText(" and your");
            if (player.vaginas.length > 0) outputText(" pussy moisten");
            outputText(".");
            ...
            if (player.lowerBody == LowerBodyType.DEMONIC_CLAWS || player.lowerBody == LowerBodyType.LIZARD || player.lowerBody == LowerBodyType.HARPY || player.lowerBody == LowerBodyType.DRAGON) outputText(" claws");
    ...
    // Naga:
        ...
        if (player.tongueType == TongueType.SNAKE) outputText(" You grin, flicking your forked tongue out to smell her arousal.");
...
function orgyRoomSubMassageHappyEnding(): void {
    ...
    if (player.cocks.length > 0 && player.cocks.cockThatFits(whitneyVagCapacity() * 1.33) != -1 && player.vaginas.length > 0) {
        outputText(" You take a long moment to consider which of your sexes you will satisfy yourself with.");
    }

    // [Male][Female]/[Next]
    menu();
    if (player.cocks.cockThatFits(whitneyVagCapacity() * 1.33) != -1) addButton(0, "Male", subHappyEndingMale);
...
function subHappyEndingMale(): void {
    ...
    if (player.cocks.cockArea(cockThatFits - 1) != player.cocks.biggestCockArea()) hasBiggerCock = true;
...
function subHappyEndingFemale(): void {
    ...
    if (player.vaginas.wetness() >= 5) outputText(" Your pussy spurts excitement gleefully with each thrust, utterly drowning her own in your juices, polluting the water around you with your musk.");
    if (player.cocks.length > 0) outputText(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
