...
export function playerHuntScore(): number {
    ...
    let baseVal: number = (player.inte + player.spe) - (player.fatigue * 2);
...
function firstWildHuntChase(waited: boolean = false): void {
    ...
    if (waited == false) {
        ...
        if (player.effects.findByType(StatusAffects.MetWhitney) >= 0) outputText(", and for a moment all you can think of are Whitney’s canine peppers");
        outputText(".\n\n");
...
function whatsMyPrize(): void {
    ...
    const gemFind: number = 10 + rand(15);
...
function surrenderToTheHounds(): void {
    ...
    if (player.breastRows.hasBreasts()) outputText("  Your chest first flattens out, then swells, as");
    else outputText("  T");
...
function predatoryPrey(): void {
    ...
    if (!player.isTaur()) {
        ...
        if (player.vaginas.length > 0 && !player.cocks.length > 0) {
            ...
            if (player.breastRows.biggestLactation() > 0) outputText("  The friction begins milking you, making you ooze milk down the trunk.");
...
function gwynnNomsDaCunts(): void {
    ...
    if (player.breastRows.hasBreasts()) outputText(" between your breasts");
