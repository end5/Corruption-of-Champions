...
export function essrayleMeetingI(): void {
    clearOutput();
    if (flags[kFLAGS.MET_ESSY] == 0) {
        ...
        if (player.breastRows.biggestTitSize() >= 10) outputText("own");
...
function plantsForMe(): void {
    ...
    // (If breasts
    if (player.breastRows.biggestTitSize() > 1) outputText("Your nipples stiffen, breasts puffing out with a heated breath");
    ...
    if (player.breastRows[0].breastRating >= 1) {
        ...
        if (player.breastRows.length > 1) outputText(" of your uppermost pair");
        ...
        if (player.breastRows.length > 1) outputText(", steadily working over the multitude of fleshy mounds on your body");
        outputText(".");
        if (player.breastRows.hasFuckableNipples()) outputText("  Her fingers play with your nipples and press gently to tease at them.  She seems quite surprised as they sink in with minimal effort.  A grin spreads over her face as she begins to steadily thrust those digits in and out of your nipples, fingers swirling about the interiors all the while.");
...
function plantsForMe2(BE: number = 0): void {
    clearOutput();
    // Yes
    ...
    if (BE == 1) {
        ...
        if (player.breastRows.totalNipples() > 2) outputText("s have");
        else outputText(" has");
        ...
        if (player.breastRows.length > 1) outputText("  She eventually dislodges with a wet pop, setting your new breasts to wobbling about as she moves down your body, repeating the gestures upon each set of breasts in return.");

        outputText("\n\nWeak from the pleasure, the last thing you see is her smiling face before you fall forward into her embrace.  The next thing you know, you're alone in the glade, staring at the canopy above.  There's a heavy weight on your chest and you struggle to sit up.  " + Num2Text(player.breastRows.length * 2) + " massive breasts jiggle and sway upon your chest, all full and aching to be milked");
        if (player.breastRows.length > 1) outputText(", the top pair being largest, descending in size with each subsequent pair");
        outputText(".");
        // Boost size, set lactation quantity.
        growTits(player, 7, player.breastRows.length, false, 2);
    ...
    if (player.breastRows.hasBreasts() && player.isLactating()) {
        player.milked();
...
// Look Closer
export function approachTrappedEssy(): void {
    clearOutput();
...
// (Cock)
function hasCockFeedEssy(newPage: boolean = true): void {
...
function hasPussyFeedEssy(newPage: boolean = true): void {
    if (newPage) clearOutput();
    else outputText("\n\n");
...
// [No]
function declineEssyPrizes(): void {
    clearOutput();
...
// [Yes]
function acceptEssyPrizes(): void {
    clearOutput();
    ...
    if (player.breastRows.biggestTitSize() <= 1) {
        ...
        if (player.breastRows.length == 1) player.breastRows.createBreastRow();
    ...
    // if four+ breasts:
        ...
        growTits(player, 6, player.breastRows.length, false, 2);
    }
    player.boostLactation(player.breastRows.length);
...
// [Yes]
function yesGimmeGiantNipplesEssy(): void {
    clearOutput();
    ...
    Inventory.takeItem(ConsumableLib.PRFRUIT, Camp.returnToCampUseOneHour);
}
