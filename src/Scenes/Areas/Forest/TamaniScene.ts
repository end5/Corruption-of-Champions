...
export class TamaniScene implements TimeAwareInterface {
    ...
    */
    ...
    public timeChange(): boolean {
        ...
        trace("\nTamani time change: Time is " + game.time.hours + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
...
function tamaniFirstTimeConsentual(): void {
    ...
    outputText("As if to emphasize her point, she curls her toes around your " + cockDescript(game.player, 0) + ", squeezing as she slides her feet up and down your length, milking out a few large drops of pre-cum.  You groan and kiss her again – too turned on to care if the drug-laced lipstick turns your orgasm into a pregnancy-inducing flood. ", false);
    if (player.balls > 0) outputText("Your " + ballsDescriptLight(player) + " swell with seed, spurring your desire to new heights.", false);
    ...
    // (FITS)
    if (player.cocks.cockArea(0) <= 90) {
        outputText("Tamani breaks the kiss and gives you a coy smile as she shimmies down your body, dropping her moist cunt onto your " + cockDescript(game.player, 0) + "'s " + cockHead(player) + ".  She swings her hips in a little circle, teasing you with her moist entrance as your drug-enhanced pre-cum bubbles and drools around her lips, mixing with her own copious fluids as it flows down your length", false);
        if (player.balls > 0) outputText(" and drips from your " + ballsDescriptLight(player), false);
        ...
        outputText("All good things eventually end, and with a sigh you pull the insensate goblin slut free of your " + cockDescript(game.player, 0) + ", watching a river of whiteness drain from between her thighs.  You set her down and the escaping jism suddenly stops, the remainder held inside by some kind of reflex.  Tamani giggles and pats her still pregnant-looking belly, \"<i>Wasn't the sample nice?  Come see me when your dick has had a chance to recover and we can do this again, and again, and again.  You're practically hooked already aren't you " + mf(player, "stud", "hun") + "?</i>\"\n\n", false);
        ...
        // [CORRUPT]
        if (player.cor > 66) outputText("  Your " + cockDescript(game.player, 0) + " twitches at the thought, ready and wanting more.", false);
    }
    // (TOO BIG)
    else {
        outputText("Tamani breaks your sloppy kiss and shimmies down your body, clutching tightly to your " + cockDescript(game.player, 0) + " and " + player.skinDesc + " as she settles down lower on your groin.  The goblin somehow manages to turn herself around so that is she is hanging upside-down, with her legs and arms clutching tightly to your member while her tongue ", false);
        ...
        if (player.balls > 0 && player.hoursSinceCum > 200) outputText(", visibly draining your " + ballsDescriptLight(player) + " down to their normal size", false);
...
function tamaniMaleRepeatEncounter(): void {
    ...
    if (pregnancy.isPregnant || flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0) outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + boyGirl(player) + "?\"\n\n", false);
    // (ELSE)
...
export function tamaniSexLetHer(): void {
    ...
    if (game.inCombat) {
        ...
        if (player.lowerBody == LowerBodyType.CENTAUR) {
            ...
            else outputText("You stumble, hooves weaving drunkenly as " + sMultiCockDesc(game.player) + " ache and drool with need.  ", false);
        ...
        // Not taur
            ...
            if (player.cocks.length == 1) outputText("an erection emerges", false);
    ...
    // Voluntary submission
        ...
        // Taur
        if (player.lowerBody == LowerBodyType.CENTAUR) {
            outputText("You meekly undress and ask Tamani how she would like to take you today.  She titters and slowly walks around you, taking her time, taking in your formidable equine body and your " + multiCockDescriptLight(game.player) + ". \"<i>All this, for little ol' me,</i>\" she breathes. She sounds more tantalized than daunted. \"<i>Stand still for me, horsie,</i>\" she commands. Unable to see her any more behind your flank, you do as she says. You feel a pair of tiny hands on your back thigh and then a small but dense weight climbing steadily towards your groin. You bite back laughter at the ticklish sensation.\n\n", false);
        }
        else {
    ...
    if (player.lowerBody == LowerBodyType.CENTAUR) {
        ...
        if (player.cocks[0].cockLength <= 18) {
            outputText("Before you can get too far she stops and agonizingly shifts her attention; you feel her transfer her weight to her hands, and then the rough but soft bottoms of her feet work their way down to the other end of your " + cockDescript(game.player, 0) + ", pressing against your inner thighs. The wet velvet sensation of her snatch envelopes your head and then slowly works its way down your shaft. Before she can bottom out, Tamani stops, teasingly and slowly withdraws until once again your head is pressed against her sopping entrance. She begins to work her thighs, smearing herself against the end of your straining cock with increasing urgency until she squeals in orgasm, slathering her juices onto your tip. Then, with a sigh and a snicker and again with agonising slowness, she works herself down onto your " + cockDescript(game.player, 0) + ". ", false);
            // No balls:
            if (player.balls == 0) outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.\n\n", false);
            else outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by mashing her soft soles into your " + ballsDescriptLight(player) + ".\n\n", false);
            ...
            outputText("\"<i>Go on then,</i>\" she breathes eventually. \"<i>I guess you've had enough. Cum for Mistress Tamani, " + mf(player, "stud", "slut") + ". Fill me to the brim.</i>\" With this she finally works herself all the way to the bottom of your shaft, and with surprising strength grips your stomach and begins to pound her plump behind against your inner thighs with increasing force.\n\n", false);
        ...
        // If >18 Inches
            ...
            outputText("Before you can get too far she stops and agonizingly shifts her attention; you feel her rough but soft bottoms of her feet work their way down to the other end of your " + cockDescript(game.player, 0) + ". Hanging onto the base of your member she begins to rub her plump, diminutive form along the bottom of it, her tits and thighs caressing your length. ", false);
            if (player.balls == 0) outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.\n\n", false);
            else outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by beginning to mash her soft soles into your " + ballsDescriptLight(player) + ".\n\n", false);
            ...
            outputText("\"<i>Go on then,</i>\" she breathes eventually. \"<i>I guess you've had enough. Cum for Mistress Tamani, " + mf(player, "stud", "slut") + ". Cover me from head to toe.</i>\" As she says this, she walks her hands down your belly, positions herself directly over your head, uses her own lubrication to rub her feet furiously down your " + cockDescript(game.player, 0) + " and then, with one last evil giggle, sticks her tongue directly into your urethra.\n\n", false);

            // Low cum:
            if (player.cumQ() < 1200) {
                outputText("With a ragged sigh, you reach your peak. Your first blast of jizz goes straight into Tamani's mouth, but you feel the goblin nimbly and hastily changing her position so that the very tip of your " + cockDescript(game.player, 0) + " can feel her moist snatch. Being teased for so long makes the sensation incredible, and it feels like entire minutes go by whilst you stand there, sweat dripping off you as you cum over and over again until your cock is doing nothing but flexing mindlessly.\n\n", false);
            ...
            // High cum:
                ...
                if (player.balls > 0) outputText("Your " + ballsDescriptLight(player) + " ache and you feel very thirsty indeed.", false);
    ...
    // Not-taurs
        ...
        outputText("She gets down on all fours, crawling up your legs until her gloss-covered lips reach your " + multiCockDescriptLight(game.player) + ".   Warm wetness slides along your length as the gobbo-slut gets you slick and wet with her saliva.   It has quite the effect on you, turning your " + cockDescript(game.player, 0) + " into a hard trembling pleasure-center.  You sigh, enjoying the attention but more than ready to release your pent-up sexual need into the willing girl.\n\n", false);
        // (fits)
        if (player.cocks.cockArea(0) <= 90) {
            outputText("Tamani turns about, dragging her wet gash across you and giving you a nice view of her plump butt-cheeks.  The soft skin of her palms squeezes around you as she lifts you up, positioning your " + cockDescript(game.player, 0) + " against her womanhood.   She grinds against your " + cockHead(player) + ", her folds slowly parting to take you.  Slowly, inch after inch of dickflesh sinks into her amazingly elastic yet tight pussy.  ", false);
            ...
            outputText(" of your " + nippleDescription(player, 0) + "s.  She eases up her grip a bit when she sees how bad it hurts, but she does not release them.  The noisy squelching of her cunt fucking you draws your attention back to your groin.  Your body begins thrusting up to meet her, finally understanding it's on the receiving end of a passionate screw.  ", false);
            if (player.breastRows.biggestTitSize() >= 2) outputText("Your " + allBreastsDescript(player) + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.", false);
            else outputText("Your " + nippleDescription(player, 0) + "s burn with pain as the goblin twists and pulls on them, abusing them.  Pleasure and pain mix together, overwhelming you, reducing your world to little more than the slap of genital against genital and the painful torture happening to your chest.\n\n", false);
            ...
            if (player.balls > 0) outputText("Your " + ballsDescriptLight(player) + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.", false);
            ...
            if (player.cumQ() < 250) outputText(".  I really enjoy our fucks " + mf(player, "stud", "hun") + ", and I can't wait to 'bump' into you again.", false);
            else {
                outputText(".  You make sure and let your ", false);
                if (player.balls > 0) outputText(ballsDescriptLight(player) + " fill back up with cum, ok?", false);
                else outputText(cockDescript(game.player, 0) + " fills back up with cum, ok?", false);
            }
            outputText("</i>\"\n\n", false);
            // Combat end:
            if (game.inCombat) {
        ...
        // (Doesnt fit)
            ...
            outputText(" a rough slap.  Whimpering, you still your rebellious hips and allow her to play you like the world's most sensitive fiddle.  Tamani pulls your " + cockHead(player) + " up and rocks herself against it, openly moaning as you dribble pre into her sopping gash.\n\n", false);
            ...
            if (player.vaginas.length > 0) {
                outputText("  ", false);
                cuntChange(player, player.vaginalCapacity() * .95, true);
                ...
                buttChange(player, player.analCapacity() * .95, true);
            ...
            outputText("The dildo slips out of your abused " + assholeOrPussy(player) + " forgotten in the heat of your lovemaking, leaking pink goo.\n\n", false);
...
export function tamaniSexLost(): void {
    ...
    else outputText("Somehow the ring fits onto you perfectly, as if it was made exactly for your " + cockDescript(game.player, 0) + ", though it remains painfully tight.\n\n", false);
    ...
    if (player.breastRows.biggestTitSize() >= 6) outputText("but can't manage to see your bloated prick past the wall of titflesh.\n\n", false);
    else outputText("at your bloated prick, looking almost overinflated from all the blood that tight ring has forced into it.\n\n", false);
    outputText("<b>*SMACK*</b>  Tamani's hand cracks against your " + buttDescription(player) + " making you lurch and bite back a cry of pain.   \"<i>Bad boys get punished!</i>\" she yells as she brings her hand down again, slapping your other cheek.  You're sure there must be two tiny red hand-prints forming already. The kinetic impacts of her blows vibrate through your hips, traveling into your sensitive " + cockDescript(game.player, 0) + " and working with the magic-ring to spread equal measures of pain and pleasure through its length.\n\n", false);
    ...
    outputText("Tamani pours oil over your abused backside, letting it soak in and start to numb the pain.  She pours quite a different type of oil into her hands and starts massaging you, letting the alchemical mixture make your skin tingle and enhance the sensation of pleasure while numbing away the pain of your cock being kept so... tightly bound.  Your arms and " + legs(player) + " tremble from supporting yourself at such an awkward angle, struggling not to relax at the sudden influx of pleasure.  The feelings bring you to the edge, and you feel your body's internal muscles clenching, but the tight vibrating ring blocks your cum and your orgasm, leaving you backed up and desperate.\n\n", false);
    outputText("Your goblin mistress ", false);
    if (player.balls > 0) outputText("cups your " + ballsDescriptLight(player) + " tightly", false);
    else outputText("caresses you around the ring", false);
    outputText(" and asks, \"<i>So are you sorry for making me wait for my cream " + mf(player, "stud", "slut") + "?  If you are, just say <b>I'm sorry mistress Tamani, please milk my cock whenever you want</b> and then beg me to cum.</i>\"\n\n", false);
...
export function tamaniSexWon(): void {
    ...
    if (x == -1) x = player.cocks.biggestCockIndex();
    outputText("", true);
    if (player.cocks.cockArea(x) <= 90) {
        ...
        outputText("It's too tempting of a target to resist.  You open your " + player.armorName + " and allow your " + multiCockDescriptLight(game.player) + " to flop free.  You're already hard from the enticing display, and in a moment you're pressing against her lust-slicked pussy", false);
        if (player.cocks.length > 1) outputText(" and tight asshole", false);
        ...
        if (player.cocks.length > 1) {
            ...
            outputText("Your " + multiCockDescriptLight(game.player) + " pulsates, spasming inside your goblin-flesh prison, spraying a bit of sticky goo into the happy slut.   You can hear her babbling, pleasure-drunk as she quivers around you, \"<i>Fuck yes! Cu-ah-ahm in me!  Fuck fuckfuckfucKFUCKYEAH!  Oooh, that's it, put me in the dirt and show me who's boss!</i>\"  The words seem to have the desired effect, helping you empty every ounce of cock-cream into the slut's tight hole.", false);
            if (player.cumQ() >= 250) outputText("  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ", false);
            if (player.cumQ() >= 500) outputText("  In no time at all a river of jism pours from her, pooling below as you overwhelm her body's capacity to store spunk.", false);
        }
        outputText("\n\nTamani winks, sucking on her finger as she staggers up.  She coos, \"<i>Mmmm, Tamani loves it when her " + mf(player, "sexy lover", "stud") + " dominates her.  Do you think your ", false);
    ...
    // Too big? Jerk off with feet and bukkake
        ...
        if (player.cocks.length > 1) outputText("any of your massive members", false);
        ...
        if (player.cumQ() >= 250) outputText("  You continue working your " + cockDescript(game.player, x) + " with her supple feet, watching your dripping spooge froth and bubble as you continue to pump more onto the sassy wench.", false);
        ...
        outputText("Tamani winks, blinking the cum out of an eye as you finish up, releasing her seed-covered form.  She coos, \"<i>Mmmm, Tamani loves it when you dominate her, " + mf(player, "stud", "sexy") + ".  Do you think your ", false);
        if (player.balls > 0) outputText("sexy balls have ", false);
        else outputText("sweet cock has", false);
...
function tamaniPregnantFuck(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 2) outputText(allBreastsDescript(player), false);
    else outputText("chest", false);
    outputText(".\n\n", false);
    outputText("You grab her under the arms and lift her up, examining her milk-laden nipples.  They shine with moisture, practically begging to be sucked.  You nuzzle them, gently sucking one into your mouth and drinking down the nutritious beverage.  Tamani grinds her hips against one of your " + nippleDescription(player, 0) + "s as she moans softly into your ear, practically cumming on you from the suckling alone.  You take a break when her milk-flow slows and take the other into your mouth, squeezing both her " + tamaniChest() + ", enjoying the enhanced fullness her pregnancy has brought.", false);
    // (Very large:
    ...
    if (player.cocks.cockArea(0) <= 50) {
        ...
        outputText("\"<i>Thanks for the good fuck, " + mf(player, "stud", "lover") + ",</i>\" she says, smiling cruelly as you find yourself getting hard again from her drug-laced kisses.  Tamani doesn't hang around to help though; she waddles off, cradling her belly and smiling.\n\n", false);
        player.orgasm();
        dynStats("lus", 25);
    }
    // (Doesn't Fit)
    else {
        outputText("\"<i>Sorry " + mf(player, "stud", "lover") + ", you're just too big and <b>hard</b> for me.   Don't worry, I'll help you unload everything straight into my sweet cunt,</i>\" she apologizes, ", false);
        if (player.balls > 0) outputText("massaging your balls with her feet", false);
        else outputText("jacking the base of your " + cockDescript(game.player, 0) + " with her feet", false);
        outputText(" while her hands caress and tease you.   Tamani pulls herself downwards, pressing the " + cockHead(player) + " partway up her drooling box.  She grinds hard against your cock-head, nearly driving you mad with pleasure.  She grabs her " + tamaniChest() + " and squeezes, splattering you with a thick coating of milk before returning to jacking you off with her feet and hands.\n\n", false);
        ...
        outputText("Tamani grinds on you a bit longer before staggering up and stretching.  She pauses to get a narrow bottle and abruptly jams the end into your urethra.  With one practiced motion, she squeezes your " + cockDescript(game.player, 0) + " from bottom to top, squeezing the last of your spoo into the container.  She pops it free, corks it, and pockets it.  She pats her pockets and explains, \"<i>A few of the girls back home aren't pregnant, and I thought you might like to help contribute to the local slut population.  Don't worry though baby, I'll always own your cock.</i>\"\n\n", false);
...
export function tamaniKnockUp(): void {
    ...
    if (player.perks.findByType(PerkLib.MaraesGiftStud) >= 0) flags[kFLAGS.TAMANI_PREGNANCY_COUNT] += 3;
...
export function getRapedByTamaniYouHypnoSlut(): void {
    ...
    const primary: number = player.cocks.cockThatFits(65);
    ...
    outputText("She smirks and bends over, looking back up at you from between her legs.  She offers, \"<i>Go ahead and stick it in.  A horny " + mf(player, "boy", "herm") + " like you needs to have that delicious cock inside a nice, tight wife like myself.</i>\"\n\n", false);
    // [FITS]
    if (primary >= 0) {
        ...
        outputText(cockDescript(game.player, 0) + ".  Her warmth spreads over your " + cockHead(player, primary) + " slowly, making you twitch and drip inside her, lubricating her already dripping channel with even more slick fluids.  With a mighty heave you push inside her, feeling yourself penetrating deep inside her, directly into her fertile womb.  Tamani rubs your " + legs(player) + " with her hands, giving you a massage as you bottom out against her, feeling her juices stain your ", false);
        ...
        if (cocks > 1) {
            ...
            outputText("She moans and pulls your " + cockDescript(player, secondary) + " into her mouth, running her tongue around the tip and sliding her palm along the underside.", false);
        }
        else outputText("She moans and pants, babbling about how good you are to her the whole time.", false);
        outputText("  You don't stop until your " + cockHead(player, primary) + " is the only thing inside her.\n\n", false);
        ...
        if (cocks > 0) outputText(" around your " + cockDescript(player, secondary), false);
        ...
        outputText("  She visibly orgasms, shaking and trembling from head to toe as her pussy milks your " + cockDescript(player, primary) + ".  It only lasts a few seconds, though they feel like heaven, before her body goes nerveless.\n\n", false);
        ...
        if (player.skinType == SkinType.FUR) outputText("matted", false);
        ...
        else outputText("and her mouth is too full of " + cockDescript(player, secondary) + " to do anything but gurgle out nonsensical pleasure-noises.", false);
        outputText("  Swelling wider inside the wet hole, your " + cockDescript(player, primary) + " tingles and clenches, about to unload.\n\n", false);

        outputText("An intense orgasm rolls through you, making your body clench as your piston your " + cockDescript(player, primary) + " forwards, burying it deeply inside the goblin-shaped cum-receptacle.  ", false);
        ...
        else if (player.cocks[primary].cockType == CockTypesEnum.DEMON) outputText("The corrupted nodules along your " + cockDescript(player, primary) + " pulsate in rippling, wave-like motions, massaging the interior of Tamani's twat", false);
    ...
    // [DOESNT FIT]
        ...
        outputText(cockDescript(game.player, 0) + " inside the sheath, constricting it inside the tight, transparent material.  You sigh while you endure the treatment, disappointed you won't get to feel her clenching about you as you pour sperm into her fertile womb. She finishes up, slowly pushing the tube up her gorgeous, feminine hole until you're certain it'll be depositing your precious cargo into her deepest recesses. Tamani claps her hands together excitedly, \"<i>I haven't had a man big enough to use this with until you.  Oh husband, you're going to love this, just lie back for Tamani, okay?</i>\"\n\n", false);

        outputText("You do as you are told, like a good husband, and Tamani stays close, bound to you by a flexible, artificial umbilical.  She sits next to you, propping her feet up onto your " + cockDescript(game.player, 0) + ".   The goblin doesn't have any shoes on her green-tinted feet, and in spite of the rough living conditions, they're softer than her hands, and nearly as dextrous.  You look over at her, questioningly at first, though it shifts to confused happiness as she begins stroking her left foot up and down your length.  Your wife shifts her right leg underneath you, and curls her toes around to interlock them with her left.\n\n", false);
        ...
        outputText(mf(player, "strong man's", "sexy lover's") + " " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "?</i>\"\n\n", false);
        ...
        if (cocks > 1) {
            if (cocks == 2) outputText("  Your other" + cockDescript(game.player, 1), false);
            else outputText("  Each of your other " + multiCockDescriptLight(game.player), false);
        ...
        outputText("Tamani smiles as she pulls the tube out and stands up.  White jism and clear feminine fluids drool down her thighs, and your mistress couldn't be happier.  She peels her device off your still-dripping " + cockDescript(game.player, 0) + ", pats it affectionately, and says, \"<i>You're such a wonderful husband.  I can't wait to see how you get along with your new daughters!</i>\"\n\n", false);
...
export function tamaniBeaten(): void {
    ...
    if (flags[kFLAGS.TIMES_OVIPOSITED_TAMANI] == 0) outputText("my baby-" + mf(player, "daddy", "momma"));
    ...
    if (player.cocks.length > 0) outputText("  White coats the goblin from her shoulders up, the only evidence of your male satisfaction.  You were so focused on the act of impregnation and the unique pleasure that it provides that you barely noticed [eachCock] erupting in sympathetic pleasure.");
