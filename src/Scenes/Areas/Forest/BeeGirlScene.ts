...
function beeEncounterClassicSex(postCombat: boolean = true): void {
    ...
    // non-dicked version
        ...
        // Coochie talk!
            ...
            if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_WET && player.vaginas[0].vaginalWetness < VAGINA_WETNESS_DROOLING) outputText("growing more potent around your " + vaginaDescript(player, 0) + " as it begins to leak with the honey of your desire.  ", false);
            if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) outputText("inflaming your " + vaginaDescript(player, 0) + " with need, and allowing a river of your wetness to gush from between your puffy vulva.  ", false);
        }
        // Boobies!
        if (player.breasts.biggestTitSize() > 1) {
...
function beeMaidenPlay(): void {
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.vaginas.wetness() <= CoC.VAGINA_WETNESS_NORMAL)
            outputText("barely");
        else if (player.vaginas.wetness() <= CoC.VAGINA_WETNESS_SLICK)
            outputText("easily");
...
export function rapeTheBeeGirl(): void {
    ...
    if (player.cor >= 75 && player.str >= 60 && (player.tongueType == TONUGE_SNAKE || player.cocks.length > 0 || player.vaginas.length > 0 || player.breasts.biggestTitSize() >= 4)) {
...
function rapeTheBeeGirlWithADick(): void {
    ...
    if (player.isTaur()) {
        ...
        if (player.gender == 3 && player.lowerBody != LOWER_BODY_TYPE_CENTAUR) outputText("  You reach down and finger your " + vaginaDescript(player, 0) + " roughly, lost in the throes of your orgasm.", false);
        outputText("\n\n", false);
        // wind down
...
function futaRapesBeeGirl(): void {
    ...
    if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_NORMAL) outputText("Your " + vaginaDescript(player, 0) + " becomes puffy and moist with excitement, ready for what you have planned.", false);
    if (player.vaginas[0].vaginalWetness > VAGINA_WETNESS_NORMAL && player.vaginas[0].vaginalWetness < VAGINA_WETNESS_DROOLING) outputText("Your " + vaginaDescript(player, 0) + " squicks wetly from your gentle ministrations as your vulva become sensitive and engorged.", false);
    if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) outputText("Your " + vaginaDescript(player, 0) + " gushes in response, dripping steadily over your victim's face, covering it with a spattering of girl-cum.", false);
    // New PG
    ...
    if (player.cumQ() >= 25 && player.cumQ() < 250) {
        ...
        if (player.lowerBody != LOWER_BODY_TYPE_CENTAUR) outputText("you rise up off her, shivering as her slender and squirming tongue retracts from your " + vaginaDescript(player, 0) + ".", false);
...
function beeAlternate(): void {
    ...
    if (rand(2) == 0) {
        ...
        if (player.lowerBody != LOWER_BODY_TYPE_CENTAUR) outputText("stuffing your " + multiCockDescriptLight(game.player) + " inside your " + player.armorName, false);
