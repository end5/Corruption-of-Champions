...
const BEE_GIRL_CONVERSATION: number = 0x7FFF0000; // Used to track conversations when the player is a female bee morph (Highest bit is for the bad end warning)
const BEE_GIRL_ATTITUDE: number = 0x0000FFFF;
...
const BEE_GIRL_PLAYER_AFRAID: number = 4;
const BEE_GIRL_PLAYER_VOLUNTARY_EGGING: number = 5; // End of the afraid chain, from now on player gets egged when they meet her
...
function beeEncounterSelect(clearScreen: boolean = true): void {
    ...
    if (player.effects.getValue1Of(StatusAffects.Exgartuan) == 1 && player.cocks.cockArea(0) > 100 && player.effects.getValue2Of(StatusAffects.Exgartuan) == 0) { // Exgartuan messes with things!
        beeEncounterWithExgartuan();
        return;
    }
    if (player.effects.findByType(StatusAffects.Infested) >= 0 || player.effects.findByType(StatusAffects.WormPlugged) >= 0) { // Worms now mess with things too!
        beeEncounterWithWorms();
        return;
    }
    const isBeeMorph: boolean = race(player) == "bee-morph";
    if (player.cocks.length > 0 && (player.cocks.cockArea(0) >= 50 || player.cocks[0].cockType == CockTypesEnum.BEE || isBeeMorph)) {
...
function beeEncounterAfraidFirstTimeSex(): void {
    ...
    const cockIndex: number = player.cocks.cockThatFits(50);
    if (cockIndex >= 0) {
        if (player.vaginas.length > 0) {
            outputText("You smile at her as she examines your " + multiCockDescriptLight(game.player) + " and your " + vaginaDescript(player, 0) + " and ask her which of your toys she wants to play with.  She looks up at you and licks her luscious black lips before whispering to you, <i>“Both.”</i> and pushes you onto your back.  In a flash she is hovering over top of you, upside down.  She takes a firm hold of your " + multiCockDescriptLight(game.player) + " and gives it a few initial strokes while simultaneously using her long flexible tongue to probe and explore your " + vaginaDescript(player, 0) + ".  Astounded by the talents of this bee girl, you can do nothing but push yourself up against her, desperate for more of this incredible stimulation.\n\n");
            outputText("The handmaiden seems to think that you need more stimulation too.  She promptly buries her face and tongue as far inside your " + vaginaDescript(player, 0) + " as possible, while simultaneously bringing your " + cockDescript(game.player, 0) + " to her exotic breasts and crushing it between them.  You cry out in dismay and grip the petals of the giant flower you’re spread eagle on.  You look up at your bee lover, who is now hovering at about a forty five degree angle above you while kicking her lower legs back and forth in amusement.  Soon she starts to pump her body up and down to stimulate your " + cockDescript(game.player, 0) + " trapped between her love pillows, while also maneuvering her long insectoid tongue around inside your " + vaginaDescript(player, 0) + ".\n\n");
...
function beeEncounterAfraidRepeatSex(): void {
    ...
    if (player.cocks.length > 0) {
        outputText("gently running her fingers along your " + multiCockDescriptLight(game.player) + " bringing " + (player.cocks.length > 1 ? "them" : "it") + " to full hardness" + (player.vaginas.length > 0 ? ", " : " and "));
    }
    if (player.vaginas.length > 0) {
        outputText("slipping her fingers about your " + vaginaDescript(player, 0) + " and your " + assholeDescript(player) + " till you’re wet and ready");
    }
    else outputText("slipping her fingere inside your " + assholeDescript(player) + "till you’re ready");
    ...
    if (player.analCapacity() < 25) {
        ...
        if (player.ass.analLooseness == AnalLooseness.VIRGIN)
            outputText("<b>Well, at least you’re losing your anal virginity willingly.  That’s something to be said in this world.</b>");
        else outputText("<b>Your " + assholeDescript(player) + " has become looser thanks to the knotted appendage penetrating you.</b>");
        ...
        outputText("Your experiance in this world has well prepared your " + assholeDescript(player) + " for the knotted appendage.  Its lubricated surface gives nothing but pleasure to your used rear, and both you and your insectoid lover give nothing but moans of pleasure as each new bump passes into your body.\n\n");
    }
    ...
    const cockIndex: number = player.cocks.cockThatFits(50);
    if (cockIndex >= 0) {
        outputText("Left wanting a bit more pleasure, you lift up the bee girl, and gently lower her onto your waiting " + cockDescript(player, cockIndex) + " so that she can mount you in the same way you mounted her.  Well, almost, since you’re quite sure her rough slick interior is very different than the inside of your " + buttDescription(player) + ".  While a bit surprised at first, she quickly gets really into bouncing on top of your " + cockDescript(player, cockIndex) + " while her egg layer keeps working its magic.\n\n");
    }
    if (player.vaginas.length > 0) {
        outputText("Your " + vaginaDescript(player, 0) + " is left a bit wanting, so you free up one of your hands to deal with the lack of attention.  No sooner had you started playing with your " + clitDescription(player) + ", you find that the bee girl has brought one of her hands into the fray to deal with the lack of attention herself, all the while grinning at you.  You return her smile and move your hand instead to her honey pot’s little button and start playing with that while she deals with your clit.\n\n");
    }
...
function beeEncounterSheBeatsYouRegularlyAndYouLetHerLaysEggs(clearScreen: boolean = true): void {
    ...
    outputText("The bee above you gives a happy buzz and you feel a warm viscous fluid start to fill the depths of your violated " + buttDescription(player) + ".  You gasp again and the bee girl says <i>“Now the real fun can zzztart.  Let me fill that hole of yourzzz with all thezzze eggzzz.  I’ll make zzzure you enjoy the whole thing.”</i>.  While still ensuring that your " + buttDescription(player) + " remains fully penetrated, the bee girl gently lifts you up onto her stinger.  You are vaguely aware of looking down and seeing it sticking between your legs.\n\n");
    if (player.cocks.length > 0) {
        outputText("However, you quickly become far more interested in the feeling of her hands wrapping around your " + multiCockDescriptLight(player) + " and gently pumping them.  You give a giddy laugh as the bee girl continues her hand job, and in moments you release your load and spray the flower in front of you.\n\n");
        outputText("<i>“That’zzz one, but we’ll get many more by the time I’m finished with you,”</i> she whispers in your ear, and you give a nod while wearing a vacant smile on your face.  Her hands once again start to rub your " + multiCockDescriptLight(player) + " as you feel something solid start to pass into your " + buttDescription(player) + ".  The sensations overwhelm you, and you lose track of time.  You have no idea how many times the stimulation of your bowels being filled and her hands on your " + multiCockDescriptLight(player) + " bring you to wonderful release.\n\n");
    }
    else if (player.vaginas.length > 0) {
        outputText("That’s not what really attracts your attention, as the feeling of fingers gently running over your " + vaginaDescript(player, 0) + " quickly demonstrate.  You give a small cry as some of those fingers are soon inserted inside you, and others start to dance around your vulva and " + clitDescription(player) + ".  The combination of stimulation from one of your holes being filled as another is played with push you over the edge into an orgasm.\n\n");
        outputText("<i>“That’zzz one, but we’ll get many more before I’ve finished with you,”</i> she whispers in your ear, and give a nod while wearing a vacant smile on your face.  Her hands once again begin their dance across your girly lower lips as you feel something solid start to pass into your " + buttDescription(player) + ".  It doesn’t take long for the dual sensations to reduce you down to a moaning, orgasming, mess.  You have no idea how long you were there, or how many times you were pushed over the edge as egg after egg was pushed inside you.\n\n");
    }
    else {
...
function beeEncounterWithWorms(): void { // If she won't fuck infested players after combat then she shouldn't fuck them here either
    ...
    outputText("The bee maiden puts her hands on your shoulders.  She draws you gently into a kiss, but as her sweet saliva prepares you for what is to come you feel one of your annelid passengers slither down " + (player.effects.findByType(StatusAffects.WormPlugged) >= 0 ? "from your cervix and poke out of your cunt" : "your urethra and wrap itself around the tip of your cock") + ".  The bee girl pulls back from the kiss, looks down and jumps back, the color draining from her face.\n\n");
...
function beeEncounterClassicSex(postCombat: boolean = true): void {
    ...
    if (player.isTaur()) {
        ...
        // Route 2
        if (player.cocks.longestCockLength() >= 100 || player.gender == 0) {
            // [Male (bypasses the bucking scene):
            if (player.gender == 1) {
                outputText("You grasp ", false);
                if (player.cocks.length > 1) outputText("one of ", false);
                outputText("your engorged " + multiCockDescriptLight(game.player) + " and jerk it off violently, cumming again and again as the bee-girl violates your " + assholeDescript(player) + ".  ", false);
                // [Without testicles:
                ...
                if (player.cumQ() < 100) outputText("You spurt out more cum than you thought your " + ballsDescriptLight(player) + " could hold. White speckles the base of the tree and tiny puddles form underneath you as you groan in satisfaction.\n\n", false);
                // [Huge cum amount:
                else outputText("Your " + ballsDescriptLight(player) + " goes off like a firehouse, coating the base of the tree along with a number of the ones behind it. Soon you're standing in a massive puddle of your own splooge, the scent threatening to overpower that of the honey. The bee-girl compensates by splashing more onto your back, causing you to release yet another burst of thick white cum.\n\n", false);
            }
            // [Herm (bypasses the bucking scene):
            else if (player.gender == 3) {
                outputText("You grasp your engorged " + multiCockDescriptLight(game.player) + " and jerk it off violently, cumming again and again as the bee-girl violates your " + assholeDescript(player) + " and your " + vaginaDescript(player, 0) + " pumps out enough femcum to drench your hind legs.  ", false);
                // [Without testicles:
                if (player.balls == 0) outputText("With your cock quivering and spasming and your untended to " + vaginaDescript(player, 0) + " squirting repeatedly, you continue to lap at the honey covered tree until the vibrating bee-girl finishes.  ", false);
                // [With testicles:
                else outputText("With your cock quivering and spasming semen erupts from the tip.  ", false);
                // [Little-normal cum amount:
                if (player.cumQ() < 100) outputText("You spurt out more cum than you thought your " + ballsDescriptLight(player) + " could hold. White speckles the base of the tree and tiny puddles form underneath you as you groan in satisfaction.\n\n", false);
                // [Huge cum amount:
                else outputText(SMultiCockDesc(game.player) + " goes off like a firehouse, coating the base of the tree along with a number of the ones behind it. Soon you're standing in a massive puddle of your own splooge, the scent threatening to overpower that of the honey. The bee-girl compensates by splashing more onto your back, causing you to release yet another burst of thick white cum.\n\n", false);
            }
        ...
        // Route 1
            ...
            // [Male:
            if (player.gender == 1) outputText("Your inability to reach your engorged " + multiCockDescriptLight(game.player) + " quickly becomes a problem as the pleasure from the bee-maiden's scent courses through you. You ache to be tended to, gushing out pre.", false);
            // [Female:
            else if (player.gender == 2) outputText("Your inability to reach your " + vaginaDescript(player, 0) + " quickly becomes a problem as the pleasure from the bee maiden's scent courses through you, it starts reflexively rippling, trying to milk the cock it so desperately wants while your " + clitDescription(player) + " quivers.", false);
            // [Herm:
            ...
            outputText("The stimulation grows too painful and you begin to buck wildly. The bee-girl holds on desperately, her stinger still lodged in your " + assholeDescript(player) + " and pumping fluids. She seems to realize the problem though and attempts to solve it.  ", false);
            // [Male:
            if (player.gender == 1) {
                outputText("Her legs wrap around your body and her 'feet' clasp onto your " + multiCockDescriptLight(game.player) + ". The strangeness of the sensation doesn't register though as just the contact is enough to send you over the edge.  ", false);
                // [Without testicles:
                if (player.balls == 0) outputText("You cum violently, your " + multiCockDescriptLight(game.player) + " quivering and spasming.  ", false);
                // [With testicles:
                else outputText("You cum violently, your " + multiCockDescriptLight(game.player) + " quivering and spasming while semen erupts from the tip.  ", false);
                // [Little-normal cum amount:
                if (player.cumQ() < 100) outputText("You spurt out more cum than you thought your " + ballsDescriptLight(player) + " could hold. White speckles your underside and tiny puddles form underneath you as you groan in satisfaction.\n\n", false);
                // [Huge cum amount:
                else outputText("Your " + ballsDescriptLight(player) + " goes off like a firehouse, coating your underside with white and dripping down your front legs. Soon you're standing in a massive puddle of your own splooge, the scent threatening to overpower that of the honey. The bee-girl compensates by splashing more onto your back, causing you to release yet another burst of thick white cum.\n\n", false);
            }
            // [Female:
            else if (player.gender == 2) {
                outputText("Her legs curl around your hind legs and her 'feet' begin rubbing your " + vaginaDescript(player, 0) + ". The strangeness of the sensation doesn't register though as just the contact is enough to send you over the edge.  ", false);
                // [Tiny-normal clit:
                if (player.clitLength < 3) outputText("Her awkward ministrations to your " + clitDescription(player) + " cause a gush of fluids from your " + vaginaDescript(player, 0) + " and you let out a lewd moan of satisfaction.  ", false);
                // [Cock-like clit:
                else outputText("She quickly finds your " + clitDescription(player) + " and begins to jerk it off like a cock.  The sensation is incredible and you moan lewdly as your " + vaginaDescript(player, 0) + " gushes fluid.  ", false);
            ...
            // [Herm:
                ...
                if (rand(2) == 0) {
                    ...
                    outputText("Thinking you still not satisfied, she pulls her 'feet' up slightly and she begins to rub your " + vaginaDescript(player, 0) + ".  ", false);
                ...
                // [Same as female followed by:]
                    ...
                    outputText("Thinking you're still not satisfied, she lowers her 'feet' slightly and she begins to rub your " + multiCockDescriptLight(game.player) + ".  ", false);
    ...
    // Dicked version...
        ...
        if (player.cocks.length == 1) outputText("member", false);
        if (player.cocks.length > 1) outputText("members", false);
    ...
    // non-dicked version
        ...
        // Coochie talk!
            ...
            if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("centering around your now puffy vulva.  ", false);
            if (player.vaginas[0].vaginalWetness >= VaginaWetness.WET && player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("growing more potent around your " + vaginaDescript(player, 0) + " as it begins to leak with the honey of your desire.  ", false);
        ...
        if (player.vaginas.length > 0) outputText(" as your hand is guided to your " + vaginaDescript(player, 0), false);
        outputText(".\n\n", false);
        // Cum text
        ...
        if (player.vaginas.length > 0) outputText(", the constant aphrodisiac honey you're mindlessly devouring, and your fingers' instinctual ravaging of your " + vaginaDescript(player, 0) + ", ", false);
...
export function beeSexForCocks(clearScreen: boolean = true): void {
    ...
    if (giantCockIndex != -1) {
        ...
        outputText("She gently runs her fingers over your " + multiCockDescriptLight(player) + " gently flicking the tip" + (player.cocks.length > 1 ? "s" : "") + " until you’re rock hard.  She seems to be oddly disappointed by what she sees, gently tapping her lips.  <i>“It lookzzz like you need zzzome zzzpecial honey to grow big and zzztrong.”</i>  It isn’t like you can be indignant at her words though, considering just how messed up in the head her smell and buzzing has made you.  While still absentmindedly stroking your member" + (player.cocks.length > 1 ? "s" : "") + ", she uses her free hand to reach into her bag and extract a small plain looking bottle that she then inserts into her honeypot.\n\n");
        outputText("<i>“Zzztill,”</i> she continues, <i>“you dezzzerve to feel good while I make your honey.”</i> before beginning a much more intense assult on your " + multiCockDescriptLight(player) + " with her dexterous hands.  You can’t help but start panting under her ministrations.  ");
        if (player.cocks.length == 1) {
            outputText("With one hand, she runs her fingers up and down the shaft of your " + cockDescript(player, 0) + " while her other hand rubs the tip with the palm of her hand.  <i>“Ah, you like thizzz don’t you?  Juzzzt wait, when it getzzz bigger, it will feel even better,”</i> she whispers in your ear and starts to pump your shaft with her whole hand.  It almost feels like she is trying to force out more length through her pumping alone.  At the same time, her other hand starts to move in more and more rapid circles, quickly pushing you to your limit.\n\n");
        }
        else {
            outputText("Starting with your " + cockDescript(player, 0) + ", the bee girl starts stroking your shaft with one hand, while her other hand runs in circles around the tip.  She doesn’t limit her attention to just your leading member though, she is quick eager to lavish the same treatment onto your " + cockDescript(player, 1) + " too.  <i>“Hmm, it’zzz hard to get all of you when you’re like thizzz.  It’ll be zzzo much better when you juzzzt have one.”</i>  " + (player.cocks.length > 2 ? "This doesn’t stop her from performing the same treatment on the whole of your " + multiCockDescript(player) + ", alternating between each of your manhoods.  " : "") + "Her hands start to increase the rate of their stimulations, quickly pushing you to your limit.\n\n");
        }
...
function beeSexForCocksPart2(giantCockIndex: number): void {
    ...
    outputText(" of cum from the tip of your " + cockDescript(player, giantCockIndex) + ".  Since the bee was at the top of her ride up and down your member, she catches your load full in the face.\n\n");
...
function beeMaidenPlay(): void {
    ...
    if (player.vaginas.length > 0) {
        outputText("  <i>“Repeat after me.”</i>  She intones before giving a soft buzzing giggle and slips one of her hands into her honey pot and liberally covers it in her private sweetness.  You follow suit slipping your fingers to your " + vaginaDescript(player, 0) + " and ");
        if (player.vaginas.wetness() <= VaginaWetness.NORMAL)
            outputText("barely");
        else if (player.vaginas.wetness() <= VaginaWetness.SLICK)
            outputText("easily");
        else outputText("liberally");
        outputText(" cover them with your own fluids.\n\n");
        outputText("She raises her drenched hand, gives it a playful lick, and moves it down to your " + vaginaDescript(player) + " while you do the same to hers.  You gasp as her honey covered lower fingers quickly slip inside your " + vaginaDescript(player, 0) + " and start to wriggle around.  You try to focus on matching her stimulations.  While you can hear her gasp in enjoyment from your ministrations, you can barely keep your mind straight let alone match her expert movements.  Keeping your brain from becoming mush becomes especially difficult when she brings her thumb and index finger to your " + clitDescription(player) + ".\n\n");
        if (player.sens < 40)
        ...
        outputText("Just having her chitin covered fingers playing around with your " + buttDescription(player) + " feels great, and it’s worth just staying there for a few moments.  Still, best to get to the main course you figure, and move your finger to the edge of her rear entrance.  You gently poke at the entrance to for a moment to help her get ready for your invasive activities.  Your insectoid lover gives no such time to prepare, and simply shoves her wet digit as deep as she can into your " + assholeDescript(player) + ".\n\n");
...
function seduceBeeGirl(): void {
    ...
    if (player.gender == 3) {
        ...
        outputText("Pulling back, she releases your " + cockDescript(game.player, 0) + ", now slick with her saliva. She lies on her back, her legs spread wide with he abdomen laying flat against the ground. Her hands urge you forward, and you follow immediately. Her body is soft and accepting, and you quickly slide the head of your " + cockDescript(game.player, 0) + " into her pussy lips, a mutual moan escaping from both of your lips. Pushing further in, the bee-girl wraps her arms around you, holding you against her as you begin to thrust. Each push makes her wetter, the slickness making your cock slide faster and faster. She arches her back and you feel the tip of her stinger against your " + vaginaDescript(player, 0) + ". Before you can protest, it pushes in, gently but quickly, until it pierces your cervix.\n\n", false);
...
export function beeRapesYou(): void {
    ...
    if (player.isTaur()) {
        // Damage:
        if (player.HP < 1) {
            outputText("The bee-girl smirks at you and approaches. Her chitin covered hand slides along your flank and you shiver with anticipation. She slaps your haunch, hard, and you reflexively buck. Displeased, she gives you a shove and you run into a tree. A honey covered hand is rammed inside your " + assholeDescript(player) + " and you grab the tree for support.\n\n", false);
            ...
            outputText("With you thoroughly incapacitated, she walks up to your front and smears your " + chestDesc(game.player) + " with honey. The scent wafts up to your nose and your mind begins to go blank. Your tongue tries to reach for the sweet substance, but can't. You whimper at her, desperate for the taste of the mind altering honey. She obliges you by splashing some onto your face and you quickly begin lapping at it with your tongue.\n\n", false);
            // Male with testicles:
            if (player.gender == 1 && player.balls > 0) {
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing " + sMultiCockDesc(game.player) + " to go rock hard.\n\n", false);
                ...
                outputText("Chitin covered arms begin stroking and scraping at you as you close your eyes as hard as possible, having exhausted your face of the mind numbing honey. Your pain and lust filled mind comes up with an idea and you quickly act; rubbing your sticky " + chestDesc(game.player) + " against the tree before swinging your head down violently and gnawing at it.\n\n", false);
                ...
                outputText("They seem to understand your problem though and, more out of a desire to not get tossed off than to help you with your needs, begin to kick at your " + ballsDescriptLight(player) + " with their feet.\n\n", false);
            ...
            // [Female:
                ...
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing your " + vaginaDescript(player, 0) + " to squirt fluid behind you.\n\n", false);
                ...
                outputText("The sensation proves too much for your massively engorged " + clitDescription(player) + " to take and you begin to shake violently, incapable of touching it as it's trapped underneath you and the weight of the bee-girls preventing you from getting the leverage needed to hump the hard ground.\n\n", false);

                outputText("They seem to understand your problem though and, more out of a desire to not get tossed off than to help you with your needs, begin to kick at where your balls ought to be with their feet. A handful of blows land on your " + clitDescription(player) + " and you cum hard, squirting behind you with surprising force.\n\n", false);
            ...
            // [Herm:
                ...
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing " + sMultiCockDesc(game.player) + " to go rock hard and your " + vaginaDescript(player, 0) + " to squirt fluid behind you.\n\n", false);
                ...
                outputText("The sensations prove too much for your massively engorged " + cockNoun(player.cocks[0].cockType) + " and sopping " + vaginaDescript(player, 0) + " to take and you begin to shake violently, incapable of touching it as one is trapped underneath you with the weight of the bee-girl preventing you from getting the leverage needed to hump the hard ground and the other is hopelessly out of reach.\n\n", false);

                outputText("They seem to understand your problem though and, more out of a desire to not get tossed off than to help you with your needs, begin to kick and punch at your " + vaginaDescript(player, 0), false);
                ...
                // [If huge cum amount:
                if (player.cumQ() > 1500 || player.vaginas.wetness() >= 5) {
            ...
            // [Genderless:
                ...
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing your " + assholeDescript(player) + " to wink.\n\n", false);
                outputText("She can't see your " + assholeDescript(player) + ", of course, and pays no attention to your outbursts, instead busying herself with something on your back. A loud unpleasant buzzing fills the air and you manage to break free of the control of the honey long enough to twist around.\n\n", false);
                outputText("Your stomach churns as you see four more bee maidens appear out of the undergrowth, stingers exposed and glistening with a clear fluid. The bee on your back stabs with her own stinger and you turn away again, desperately licking at the honey in an attempt to deal with the awful heat in your lower body.\n\n", false);
                ...
                outputText("They seem to understand your problem though and, more out of a desire to not get tossed off than to help you with your needs, begin to kick and punch at your " + buttDescription(player) + ". The volley of blows is enough to stop you from shaking, but the pressure doesn't go away.\n\n", false);
        ...
        // Lust:
            ...
            outputText("The bee-girl smiles at you and sways her hips sensuously as she approaches. Her chitin covered hand slides along your flank and you shiver with anticipation. She slaps your haunch gently, and you feel your anticipation grow. Pleased, she guides you toward a tree before gently sliding a honey covered hand inside your " + assholeDescript(player) + ", causing you to grab the tree for support.\n\n", false);
            outputText("She tests the insides of your ass for a bit before pulling her arm out and your hind legs start to quake as your desires aren't satisfied. You feel her stroke your hindquarters, encouraging you to lie down. You happily do so, using the tree as support.  With you safely down, she walks up to your front and smears your " + chestDesc(game.player) + " with honey. The scent wafts up to your nose and your mind begins to go blank. Your tongue tries to reach for the sweet substance, but can't. You whimper at her, desperate for the taste of the mind altering honey. She obliges you by holding a handful of the thick elixir to your mouth and you lap at it eagerly. Once you've consumed a fair amount she spreads more onto your face and giggles cutely as you stretch your tongue to lick at it.\n\n", false);
            // [Male with testicles:
            if (player.gender == 1 && player.balls > 0) {
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing " + sMultiCockDesc(game.player) + " to go rock hard.\n\n", false);

                outputText("She can't see that, of course, and busies herself with inserting her ovipositor into your " + assholeDescript(player) + ". Its thick lubrication allows it quick entrance, and soon she's pumping her fluids into your bowels. The sensation proves too much for your massively engorged " + multiCockDescriptLight(game.player) + " to take and you begin to shake violently, incapable of touching ", false);
                if (player.cocks.length == 1) outputText("it as it's ", false);
                ...
                outputText("She seems to understand your problem though and, wanting to help you with your needs, begins to massage your " + ballsDescriptLight(player) + " with her 'feet'. As she does this she starts to hum, a sound that obliterates what few thoughts you had left and which causes her entire body to vibrate.\n\n", false);
                ...
                outputText("She continues pumping into you as you pass out, " + sMultiCockDesc(game.player) + " and " + ballsDescriptLight(player) + " aching.  The combined scent of cum and honey completely overwhelms you.", false);
            }
            // [Male without testicles:
                ...
                outputText("She continues pumping into you as you pass out, " + sMultiCockDesc(game.player) + " aching and the scent of honey overwhelming you.\n\n", false);
            }
            // [Female:
            else if (player.gender == 2) {
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing your " + vaginaDescript(player, 0) + " to squirt fluid behind you.\n\n", false);
                outputText("She sees the spray and strokes your ass gently, but can do nothing to help as she busies herself with inserting her ovipositor into your " + assholeDescript(player) + ". Its thick lubrication allows it quick entrance, and soon she's pumping her fluids into your bowels. The sensation proves too much for your massively enflamed " + vaginaDescript(player, 0) + " and you begin to shake violently, incapable of touching yourself and alleviating the dreadful pressure. Your " + clitDescription(player) + " begins to throb painfully as it swells larger than normal.\n\n", false);

                outputText("She seems to understand your problem though and, wanting to help you with your needs, begins to stroke your " + vaginaDescript(player, 0) + " with her 'feet'. As she does this she starts to hum, a sound that obliterates what few thoughts you had left and which causes her entire body to vibrate. This combined with the handful of times her chitin covered appendages slide across your " + clitDescription(player) + " cause you to cum hard, squirting behind you with surprising force.\n\n", false);
                // [Huge cum amount:
                if (player.vaginas.wetness() == 5) outputText("Your massive cum amount soon causes you to be lying in a deep puddle of femcum, the scent so strong that it's almost enough to overwhelm that of the honey, despite it coating your face.\n\n", false);
            ...
            // [Herm:
                ...
                outputText("She returns to your rear end and assesses it some more. With an almost desperate scrabbling she climbs on top of you, jabbing you repeatedly with her outstretched stinger and causing " + sMultiCockDesc(game.player) + " to go rock hard and your " + vaginaDescript(player) + " to squirt fluid behind you.\n\n", false);

                outputText("She can't see your cock, of course, and while she sees the spray and strokes your ass gently, can do nothing to help as she busies herself with inserting her ovipositor into your " + assholeDescript(player) + ". Its thick lubrication allows it quick entrance, and soon she's pumping her fluids into your bowels.\n\n", false);

                outputText("The sensation proves too much for your massively engorged " + cockNoun(player.cocks[0].cockType) + " and " + vaginaDescript(player, 0) + " to take and you begin to shake violently, incapable of touching it as one is trapped underneath you with the weight of the bee-girl preventing you from getting the leverage needed to hump the hard ground and the other is hopelessly out of reach.\n\n", false);
                outputText("She seems to understand your problem though and, wanting to help you with your needs, begins to massage your " + vaginaDescript(player) + " with her 'feet' while periodically stroking the back of your " + cockDescript(game.player, 0) + ".\n\n", false);
        ...
        // Male + venomz
        if ((player.effects.findByType(StatusAffects.ParalyzeVenom) >= 0 || player.effects.findByType(StatusAffects.lustvenom) >= 0) && !sexed && player.cocks.length == 1) {
        ...
        if (player.gender == 2) {
            ...
            outputText("A sharp pain hits your stomach as the bee-girl's stinger penetrates your flesh. Before you can cry out, a sickening warmth floods from the wound, washing away the pain and oozing through your body. Your " + vaginaDescript(player, 0) + " juices flow between your ", false);
            ...
            if (player.lowerBody != LowerBodyType.CENTAUR) {
                outputText("She slips further down your chest, grabbing your " + allBreastsDescript(player) + " playfully. You feel her knee nudging against your " + vaginaDescript(player, 0) + ", grinding roughly against it and sending shivers through your body. You yelp slightly as she bites one of your nipples, the sudden sensation giving way to moans of pleasure as she massages and pinches the other one. Her knee slides against your " + vaginaDescript(player, 0) + ", flicking your " + clitDescription(player) + " and coating the bee-girl with your juices. You feel her straddle your legs, her feet deftly pulling your thigh into contact with her pussy, before she starts to rock against you.\n\n", false);
            ...
            outputText(" sliding against your nether-lips, pushing against them and parting the outer folds. Her abdomen shifts, shifting the stinger up and down, keeping your " + vaginaDescript(player, 0) + " parted while rubbing your clit.\n\n", false);
...
export function rapeTheBeeGirl(): void {
    ...
    // OPTIONS HERE!
    let naga: () => void = null;
    let multiCock: () => void = null;
    let cock: () => void = null;
    let vagina: () => void = null;
    let herm: () => void = null;
    let gentleman: () => void = null;
    let eggs: () => void = null;
    if (player.ovipositor.canOvipositSpider() && (player.faceType == FaceType.SNAKE_FANGS || player.faceType == FaceType.SPIDER_FANGS)) {
        eggs = layEggsInABeeSpiderLike;
        outputText("(You could dose her with venom and lay YOUR eggs in her.)\n");
    }
    if (player.cocks.length > 0) {
        outputText("(You could fuck her with " + oMultiCockDesc(game.player) + ".)\n", false);
        cock = rapeTheBeeGirlWithADick;
    }
    if (player.cocks.length > 1) {
        outputText("(You could use more than one of your " + multiCockDescriptLight(game.player) + " on her.)\n", false);
        multiCock = rapeTheBeeMultiCockStuff;
    }
    if (player.vaginas.length > 0) {
        outputText("(You could make her get off your " + vaginaDescript(player) + ".)\n", false);
...
function rapeTheBeeMultiCockStuff(): void {
    ...
    if (player.cocks.length >= 5 && player.breastRows.biggestTitSize() > 2
        ...
        outputText("Unhindered, you start to fuck her.  Her pussy tightens and convulses around your " + cockDescript(game.player, 0) + ", her ample honey dribbling down onto your lowermost shaft and adding a little more lube on every thrust into the tight heat of her ass.  Her whimpers turn to moans, her scent shifting from fear to lust as the bee-girl discovers the unexpected pleasure of being reamed in both holes at once.  You keep your breathing steady, listening to her moans grow more lustful with every thrust and more needy every time you pull back in preparation for another.  Soon you've fucked all thought of resistance out of her, in favor of squealing with glee every time your mighty cock forces a little deeper into her tight honey pot.\n\n", false);
        outputText("Letting go of her arms, you grip the base of her abdomen for leverage and let her arms and legs slide around you as your pace increases.  You show her how to press her breasts together against your " + cockDescript(game.player, 0) + ", and she obliges, rubbing them back and forth as you fuck her.  With your jiggling breasts added to the mix, it becomes even more enjoyable, your " + breastDescriptOfRow(game.player, 0) + " mashing against hers.  The milk leaking out of your immense jugs runs down your bodies and adds further to the ocean of fluids forming between your hips.\n\n", false);
        outputText("The heads of your " + cockNoun(CockTypesEnum.HUMAN) + " rub back and forth against the bee-girl's lips and face, and finally curiosity leads her to begin tasting, her surprisingly long tongue lassoing and swirling around every tip she can reach in turn, darting from one to another as if tasting flowers to provide an experience almost as unique as her pussy.\n\n", false);
    ...
    if (player.cocks.length == 2 && rand(2) == 0) {
        outputText("The bee-girl turns around, scampering across the floor to try to escape. As you close in to your prey you can feel your " + multiCockDescriptLight(game.player) + " harden and rise up at the sight of her form and you swiftly grab on to her abdomen and hold fast. She screams in futility as you flip her over and greedily look across her body.\n\n", false);
        outputText("Forcing her onto her back, you hold her wrists tightly in one hand to allow you to pull off your " + player.armorName + ", freeing your " + multiCockDescriptLight(game.player) + ". The bee-girl opens her eyes for a scant moment and realizes what's about to happen. She tries one more time to plead for you to let her escape as your " + multiCockDescriptLight(game.player) + " slide up and down against her nectar-drenched pussy lips. Your grin widens as you penetrate her roughly in one savage stroke, taking her with one of them. Screaming and squirming, the bee-girl struggles.  Your right hand braces against her flapping arms to settle them to the ground as you push in once again. Letting her know just how little control she has against your will, you lean forward, planting her hands against the ground and licking her cheek as your " + multiCockDescriptLight(game.player) + " push up deep inside her. Looking into her eyes, you see a faint glimmer of defiance, and under your body you feel her shifting slightly, muscles growing firm and tense. The realization hits you, and you manage to shift before her stinger comes into contact with you. Moving your knee, you pin her thick ", false);
        ...
        outputText("You decide that it's time to bring your other gift into play. Atop the bee-girl you fire in one more stroke of your " + cockDescript(game.player, 0) + " and then you run your free hand from her black lips across her breast and across her abdomen before you rest it on her clit. You tweak her sensitive spot along with the strokes from your intruding member and then when the rush of nectar and cum juices from her pussy seem up to the task you grab hold of your other " + cockDescript(game.player, 1) + " and forcefully ram it into her pussy right below your first entry.\n\n", false);
...
function rapeTheBeeGirlWithADick(): void {
    ...
    if (player.isTaur()) {
        ...
        outputText("Pulling back, you consider her for a moment. She watches as you slip out of your " + player.armorName + ", licking her lips at the sight of your " + chestDesc(game.player) + ". Once your torso is free you reach down again and pick up the bee maiden. She's surprisingly light and you lift her with ease, feeling her legs wrap around you as you hug and kiss her.\n\n", false);
        ...
        outputText("The sweet substance causes your hindquarters to heat up. You feel your " + assholeDescript(player) + " pucker and " + sMultiCockDesc(game.player) + " grow, dripping pre onto the ground. At the same time, your thoughts start to get fuzzy and you have to fight off the sensation to throw the bee down and fuck her wildly.\n\n", false);
        ...
        if (player.cocks.cockArea(x) <= monster.vaginalCapacity()) {
            outputText("She looks under your arm toward your manhood and seems delighted by the sight of your " + cockDescript(game.player, x) + ". Her body swings about your torso with the help of her wings and she crawls down your back toward your rear. You lift your tail and allow the bee maiden to examine your " + assholeDescript(player) + ", sliding her fingers around the edge and teasing at it.", false);
            // Has testicles:
            if (player.balls > 0) outputText("  Her other hand slides down and begins to massage your " + ballsDescriptLight(player) + ", squeezing and tugging gently at your scrotum. Her tongue joins them, sliding in and out of your ass before wrapping around your sack. You feel incredible and your " + cockDescript(game.player, x) + " drips pre, forming a puddle on the ground.", false);
            // No testicles:\
            else outputText("Her other hand starts pulling it open and soon her tongue joins them, sliding in and out of your ass delicately. Your " + cockDescript(game.player, x) + " quivers with anticipation and you swish your tail happily.", false);
            outputText("\n\n", false);
            ...
            outputText("You think at first she's going to ride you like a horse, but she doesn't. Instead, you feel her whole body slide sideways as she flips around to grab onto your belly. You're surprised at her strength, managing to hold herself up fairly effectively with just her arms and legs gripping your body. She doesn't intend to rely on that alone though, as her abdomen is gently held in place between your rear legs. You feel something latch onto your " + assholeDescript(player) + ", but are unable to do anything about it or even see what it is. The sudden changes are surprising and you can't help but wonder what she intends.\n\n", false);

            outputText("You get your answer as a hot, wet, vice wraps around your " + cockDescript(game.player, x) + " and whatever is inside your ass pushes slightly further in. It feels good, but you know that even with her surprising strength she wouldn't be able to hold on if you tried driving into her. Thinking this defeats the purpose somewhat you're about to propose that some other arrangement be considered when she starts to buzz.\n\n", false);
            ...
            outputText("The sensation is incredible; her muscles spasm and milk your " + cockDescript(game.player, x) + " while the object in your " + assholeDescript(player) + " (which you think is probably her ovipositor) begins pumping fluids into you, all while your genitals are vibrated unceasingly.", false);
            // Has testicles:
            if (player.balls > 0) outputText("  Your " + ballsDescriptLight(player) + " really feel spectacular, the vibrations causing the hot spunk inside to roil about.", false);
            outputText("  Her hot cunt constricts around you as she cums repeatedly, coating your hind legs in sweet honey.\n\n", false);

            outputText("You find it increasingly difficult to control yourself as your equine body demands to hump something. The unusual ministrations are spectacular, but your " + cockDescript(game.player, x) + " is demanding a mare. With a supreme effort you overcome your bestial desires, reveling in the pleasure without feeling the overwhelming desire to inseminate. Just as you're celebrating this personal victory you feel the familiar pressure of your orgasm build in your groin", false);
            // has testicles:
            if (player.balls > 0) outputText(", and your " + ballsDescriptLight(player) + " constrict", false);
            outputText(".\n\n", false);
            ...
            if (player.balls == 0) outputText("  It dribbles into the bee-girl as her honey squirts out of her once again. Her body trembles as the extreme exertions take their toll, and she slides off of your member with a damp plop, splashing into a puddle of her own fluids.", false);
                ...
                // Huge cum amount:
                else outputText("  It sprays inside the bee-girl, quickly overfilling her and causing her belly to expand. Her sudden increased girth and the extreme exertion cause her to lose her grip on you, and she is propelled off of you into a puddle of spunk and honey. You can't control yourself as your " + ballsDescriptLight(player) + " continue to pump out more and more fluid, spraying down the exhausted bee and coating her flower with enough gunk to cause it to wilt slightly. Eventually it stops, with only periodic gushes spraying out.", false);
            }
            outputText("\n\n", false);
        ...
        // Cock too long to penetrate her:
            ...
            outputText("She looks under your arm toward your manhood but seems disappointed at your " + cockDescript(game.player, x) + "'s large size. . Her body swings about your torso with the help of her wings and she crawls down your back toward your rear. You lift your tail and allow the bee maiden to examine your " + assholeDescript(player) + ", sliding her fingers around the edge and teasing at it. Her tongue licks at you but you can tell her mind is elsewhere and you wonder what the problem is.\n\n", false);
            ...
            outputText("She doesn't though, as with a buzz her body swings about beneath you. As she slides sideways, one arm is pulled off for a moment, long enough to catch your " + cockDescript(game.player, x) + " between her body and your own, before catching onto your other flank. Her abdomen slides up behind your rear legs and you feel something latch onto your " + assholeDescript(player) + ", but are unable to do anything about it or even see what it is.\n\n", false);
            ...
            outputText("Your " + cockDescript(game.player, x) + " feels wonderful though, pumping out fluid and swelling with more and more blood.", false);
            // Has testicles:
            if (player.balls > 0) outputText("  The vibrations do a number on your " + ballsDescriptLight(player) + " too, as the semen stored within starts to shake and dance about inside.", false);
            outputText("  The thing in your " + assholeDescript(player) + " (which you believe is her ovipositor) is vibrating along with her as it pumps fluid inside, and you feel yourself puckering in pleasure.", false);
            // Cock long enough to masturbate:
            if (player.tallness * (5 / 6) < player.cocks[player.cocks.longestCock()].cockLength) {
                outputText("  Her tongue wraps around your " + cockDescript(game.player, x) + " and starts jerking off the small section it can reach. The wet ring feels amazing and you decide to help her efforts by gripping the head of your member and playing with it yourself. Soon though, your cock trembles as the familiar sensation of orgasm begins creeping up on you.", false);
            }
            // Other cocks:
            else outputText("  Her tongue wraps around your " + cockDescript(game.player, x) + " and slides up and down the small area it can reach before flicking about at the tip of your urethra. Her expert licking feels amazing and it's not long before your feel the familiar sensation of orgasm creeping up on you.", false);
            outputText("\n\n", false);

            outputText("With all the combined sensations your " + cockDescript(game.player, x) + " begins to unload. ", false);
            // With testicles, masturbatable cock:
            if (player.balls > 0 && player.tallness * (5 / 6) < player.cocks[player.cocks.longestCock()].cockLength) {
                outputText("Your " + ballsDescriptLight(player) + " contract and propel your seed down your member toward the fresh air. It fires out of you and hits the bee's flower, splattering it in white.", false);
                // Huge cum amount:
            ...
            // With testicles, other cocks:
                ...
                outputText("Your " + ballsDescriptLight(player) + " contract and propel your seed down your member toward the waiting bee-girl. It fires out of you and hits her face, splattering it in white.", false);
                // Huge cum amount:
            ...
            // Without testicles:
            else outputText("Your " + assholeDescript(player) + " tightens uncontrollably around its intruder, making the vibrations feel even stronger there. Your member jerks as you orgasm and a few droplets of semen land on the bee maiden.", false);

            outputText("\n\nShe releases you from her grasp and lands with a wet thud on the ground. Looking down, you see that her vibrating against your " + cockDescript(game.player, x) + " was enough to get her off, a lot. She's lying in a puddle of mixed sexual fluids and honey, unconscious after what must have been an extreme effort on her part.", false);
        ...
        if (player.isTaur()) outputText("as your " + cockDescript(game.player, x) + " hardens under your belly.", false);
        else outputText("as you disrobe, revealing your " + cockDescript(game.player, x) + " to her.", false);
        outputText("  Wasting no time, you get down to the business of ", false);
        if (player.cocks.length == 1) outputText("penetrating her slick snatch, forcing in inch after inch.  ", false);
        if (player.cocks.length == 2) outputText("cramming your " + multiCockDescript(game.player) + " into her, double-stuffing the poor bee-slut with your double dongs.  ", false);
        if (player.cocks.length >= 3) outputText("cramming two out of your " + num2Text(player.cocks.length) + " " + multiCockDescript(game.player) + "s into her, double-stuffing the poor bee-slut with double the dicks.  ", false);
        ...
        // sex
        outputText("You begin pounding away at her, splattering honey over her thighs with your enthusiastic fucking.  Her passage is unlike anything you've ever had before.  It's tight, but overly slick and textured with small nubs and bumps that tease and caress in wonderful ways.   Her muscles clamp and squeeze her vaginal entrance tightly, turning it into an organic cock-ring.  You hold still while her cunt begins twitching and slightly rotating back and forth around your " + cockDescript(game.player, x) + ".    Her head is thrown back in what you assume is an orgasm, her arms mashing her tits together for even more pleasure.  The vacuum seal around your " + cockDescript(game.player, x) + " only gets tighter as her rippling, squeezing, twisting cunt stimulates your over-engorged " + cockDescript(game.player, x) + ".\n\n", false);
        // cum
        ...
        if (player.gender == 3 && player.lowerBody != LowerBodyType.CENTAUR) outputText("  You reach down and finger your " + vaginaDescript(player, 0) + " roughly, lost in the throes of your orgasm.", false);
        outputText("\n\n", false);
        // wind down
    ...
    // Male 2
        ...
        outputText("You begin to push forward, sliding your " + cockDescript(game.player, x) + " against the soft folds of her pussy, her whimpers shifting gently into moans. Your grin widens as you penetrate her roughly in one strong stroke, the bee-girl's body tensing underneath you as a squeal leaves her lips. Looking into her eyes, you see a faint glimmer of defiance, and under your body you feel her shifting slightly, muscles growing firm and tense. The realization hits you, and you manage to shift before her stinger comes into contact with you. Moving your knee, you pin her thick abdomen down and remove the threat.\n\n", false);
        outputText("Unhindered, you start to fuck her. Her pussy tightens and convulses around your cock, becoming slick with her juices and your pre-cum. You can smell the sickly combination of nectar, sweat and love juices hanging thickly in the air. Your breathing becomes heavy while her moans get deeper and lust-filled. Her reluctance evaporates, her struggling shifts into more receptive movements against your body, her hips rocking against yours with each thrust, forcing your " + cockDescript(game.player, x) + " deeper into her tight honeypot.\n\n", false);
        outputText("Letting go of her arms, you grip the base of her abdomen for leverage and let her arms and legs slide around you as your pace increases. You feel her abdomen pressing against your rear as you drive into her, the pressure in your " + ballsDescriptLight(player) + " building as her pussy tightens around you. With a groan you cum, your hot seed gushing inside of her. She clenches you tightly, her pussy milking your " + cockDescript(game.player, x) + " for every ounce, until the sticky white fluid is pouring from around your " + cockDescript(game.player, x) + ". Exhausted from the battle and the fuck, she relaxes and collapses on the ground, panting heavily and half-conscious.", false);
        player.orgasm();
        cleanupAfterCombat();
    }
}
...
function rapeABeeGirlWithYourVagina(): void {
    ...
    if (player.isTaur()) {
        ...
        outputText("You make your intentions clear by sliding the " + player.armorName + " from your torso and stroking your " + chestDesc(game.player) + ". As you slide your hands around your chest and play with your " + nippleDescription(player, 0) + "s, you begin to moan sensually. Your own ministrations cause your " + vaginaDescript(player, 0) + " to heat up and you feel small dribbles of femcum slowly slide down your hind legs. Responding to your arousal, your tail starts to wave about on its own to spread your musky scent and your " + assholeDescript(player) + " begins to open and close.\n\n", false);
        ...
        outputText("She's not done yet though, as her finger is quickly replaced with a long, wet tongue. It writhes about inside you while cum continues to spray out of your " + vaginaDescript(player), false);
        if (player.hasLongTail() > 0) outputText(" and your flailing tail is held firmly in place by her hand", false);
        outputText(". Her other hand slides into your " + vaginaDescript(player) + " and scrapes around inside, causing an explosion of pleasure inside you. Your love canal grabs her and pulls her arm deeper inside you, your equine muscles making her powerless to resist.", false);
        // [Asses other than tight:
        if (player.ass.analLooseness >= 2) outputText("  Not wanting this, she attempts to brace her other hand between your " + buttDescription(player) + "'s cheeks, but your " + assholeDescript(player) + " opens and she slides inside. With an immense effort she pulls out of your ass, nearly pushing you over the edge in the process, but the arm stuck on your vagina is stuck tight.", false);
        outputText("  It's not long before her hand finds your uterus and begin probing it, pushing you beyond your limit.\n\n", false);

        // [Huge cum amount:
        if (player.vaginas.wetness() >= 5) outputText("Your vagina ripples and squeezes with enough force to hurt most normal cocks. Her exoskeleton protects her from the harmful effects though, and with a massive gush of fluids the bee-girl's arm is propelled out of you.", false);
        // [All other cum amounts:
...
function futaRapesBeeGirl(): void {
    ...
    if (player.isTaur()) outputText("as you idly fondle your " + allBreastsDescript(player) + " as you feel your blood-gorged " + multiCockDescriptLight(game.player) + " swaying under your belly.  ", false);
    else outputText("as you disrobe, revealing your " + allBreastsDescript(player) + ", " + vaginaDescript(player, 0) + ", and " + multiCockDescriptLight(game.player) + ".  ", false);
    outputText("You step forwards, straddling her and rubbing your outer lips in preparation.  ", false);
    if (player.vaginas[0].vaginalWetness <= VaginaWetness.NORMAL) outputText("Your " + vaginaDescript(player, 0) + " becomes puffy and moist with excitement, ready for what you have planned.", false);
    if (player.vaginas[0].vaginalWetness > VaginaWetness.NORMAL && player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("Your " + vaginaDescript(player, 0) + " squicks wetly from your gentle ministrations as your vulva become sensitive and engorged.", false);
    if (player.vaginas[0].vaginalWetness >= VaginaWetness.DROOLING) outputText("Your " + vaginaDescript(player, 0) + " gushes in response, dripping steadily over your victim's face, covering it with a spattering of girl-cum.", false);
    // New PG
    ...
    // BALLZ
    if (player.balls >= 2) {
        if (player.ballSize < 5) outputText("Your " + ballsDescriptLight(player) + " droop into her eyes, blinding and humiliating her in equal measure.  ", false);
        else outputText("Your " + ballsDescriptLight(player) + " obscure most of her face, making it difficult to see her reactions, but her mouth gives you all the feedback you need.  It doesn't take more than a few moments for her struggles to cease, and soon, her long insectile tongue is worming its way into your " + vaginaDescript(player, 0) + ".  ", false);
    }
    outputText("Happy with your victim's compliance, you grind harder into her face.  Allowing your hands a bit of freedom, you begin to stroke your already erect " + cockDescript(game.player, 0), false);
    ...
    outputText("You nearly bounce off her in alarm as her tongue probes deep, pressing against and penetrating your cervix in one slippery stroke.  Being licked from the inside out makes you squeal with equal parts surprise and pleasure, and you mash your " + vaginaDescript(player, 0) + " harder against her in response.  Your hands masturbate your " + multiCockDescriptLight(game.player) + " in time with her licks, the speed gradually increasing to keep pace with the fire of your ever-growing lust.  ", false);
    if (player.cumQ() >= 100) {
        outputText(" More and more pre begins to drip from your " + cockDescript(game.player, 0) + ", soaking your ", false);
        if (player.balls >= 2) outputText(ballsDescriptLight(player) + " and ", false);
    ...
    if (player.cumQ() < 25) {
        outputText("You erupt, your " + cockDescript(game.player, 0) + " pulsing and spraying ribbons of cum.  You milk your dick hard, pistoning your hand up and down mercilessly, trying to squeeze out every drop.  You watch each burst splatter into the bee-girl's hair and grin cruelly.  ", false);
        if (player.lowerBody != LowerBodyType.CENTAUR) outputText("Spent at last, you rise up off of her, shivering as her tongue retracts from your woman-hood.", false);
    }
    if (player.cumQ() >= 25 && player.cumQ() < 250) {
        outputText("You erupt, your " + cockDescript(game.player, 0) + " pulsing in your hands and spraying out thick ribbons of cum.  You milk your dick hard, pistoning it relentlessly as it spurts more and more jism.  The orgasm drags on and on, your cum soaking your poor victim's hair and forehead with your sticky white juices.   Satisfied at last, ", false);
        if (player.balls >= 2) outputText("with empty balls, ", false);
        if (player.lowerBody != LowerBodyType.CENTAUR) outputText("you rise up off her, shivering as her slender and squirming tongue retracts from your " + vaginaDescript(player, 0) + ".", false);
        else outputText("you walk away from her smiling, and sated.", false);
    }
    if (player.cumQ() >= 250 && player.cumQ() < 500) {
        outputText("Your " + cockDescript(game.player, 0) + " erupts, writhing in your hands from the force of your orgasm as thick ropes of cum burst from you.   Each pulse of juice seems to drag on and on, each nearly as long as a normal man's orgasm.  Looking down, you see the bee's hair and face totally slimed with your cum, and a puddle forming below her.  ", false);
        if (player.lowerBody != LowerBodyType.CENTAUR) outputText("You rise up off of her in mid-orgasm, aiming lower and painting her with each blast of seed.  ", false);
        else outputText("You aim right at the oval of her face and paint her with each blast of seed.  ", false);
        outputText("Your " + cockDescript(game.player, 0) + " flexes powerfully with each load, until your victim is soaked from the waist up, her breasts and face plastered with goo.  You sigh contentedly and step away, watching the bee scoop up your leavings in both hands, one going to her mouth, the other to her sweet wet snatch.  Giggling to yourself, you walk away, sated.", false);
    }
    if (player.cumQ() >= 500) {
        outputText("Your " + cockDescript(game.player, 0) + " erupts like a volcano, shooting a constant thick stream of cum that pulses with each clench of your pelvic muscles.  The pressure only seems to increase as your urethra tries to deal with the huge load your body is producing.  ", false);
        if (player.lowerBody != LowerBodyType.CENTAUR) outputText("You stand up off your victim, letting her tongue slip free from your soaked folds, and", false);
        else outputText("You", false);
        outputText(" plunge your " + cockDescript(game.player, 0) + " deep into the bee-girl's mouth and throat, pumping cum directly into her belly.  She writhes underneath you, her oxygen supply cut off as your spunk floods her belly.  The feeling of her inhuman lips wrapped tight around your base only makes it worse, intensifying the eruption of baby-batter that's pumping into her.  Her eyes roll back as cum begins leaking from her nose, her belly beginning to look distended and pregnant with the amount of jism you've pumped in.   Certain she's had enough, you pull free, marveling at how your last spurt made her look nearly nine months pregnant.   You note she's passed out, but your orgasm is far from over, so you resume jacking off, hosing her down from head to toe in thick white goop.  ", false);
...
function beeGirlRapeForTheDistinguishedGentleman(): void {
    ...
    if (select == 0) {
        ...
        // [(if penis then)
        if (player.cocks.length > 0) outputText(", spraying the fuzzy side of the bee's abdomen with your sticky come,", false);
    ...
    // (if penis)
        ...
        outputText("Seeing the girl's resistance end in an eye-rolling gasp, you decide to get in on the fun too.  With some difficulty you unfasten and shed the bottoms of your " + player.armorName + " with your one free hand and maneuver your " + cockDescript(game.player, 0) + " to her now-gushing honeypot.  Considering there's enough foreplay going on with her ovipositor to wet her enough for a minotaur, you unceremoniously slam your " + cockDescript(game.player, 0) + " into her.", false);
        // [(if dicksize>beevagdepth)
        if (player.cocks.cockArea(0) > monster.vaginalCapacity()) outputText("  The bee-girl barely winces as the head of your shaft bumps up painfully against her cervix, quickly returning to a rictus of ecstasy as you continue jerking her ovipositor off with your now-sticky hand.", false);
...
function beeAlternate(): void {
    ...
    if (rand(2) == 0) {
        ...
        // Catchall scenario
        if (player.cocks[x].cockType == CockTypesEnum.DEMON || player.cocks[x].cockType > 4) outputText("You loosen your " + player.armorName + " and unleash your cocks.  The glans of the largest is already drooling pre as you enjoy the sight of the helpless, squirming bee-girl.  Guiding it forwards, you spank her ass, making the girl yelp and leaving trails of sticky pre across the globes of her ass.  Grinning, you drag your shaft back and forth across her pussy, slicking it in her honey.  Finally, you press the tip against her opening.  She's a tight fit for you, and she squeals more loudly as your shaft spreads her little cunny inexorably.  ", false);
        // CONTINUE
        outputText("You enjoy loosening the girl up and exploring the depths of her cunt, taking your time.  Eventually, you draw yourself out of her, your " + cockDescript(game.player, x) + " escaping with a wet, slurping sound, dragging a thick drool of honey in its wake.  Taking a moment to inspect your handiwork, you see her pussy gapes wantonly, stretched out of shape by your shaft.", false);
        // New PG
        outputText("\n\n", false);
        // 2nd cock love
        outputText("Taking your slightly smaller " + cockDescript(game.player, y) + " in hand, you slip it in as a replacement.  She's not as tight as she was, poor dear, but just as warm and twice as slick as she was for your first " + cockDescript(game.player, x) + ".  Your larger " + cockDescript(game.player, x) + " slides along the side of her ass, painting the black-and-yellow fur there with her honey and matting it down.  But that's less than satisfying, and glancing down, you notice she has a hole back here that's not getting used.  ", false);
        outputText("Withdrawing from her once more, you set the tip of your largest " + cockDescript(game.player, x) + " against the pucker of her ass.  She squeals and bucks as you press against her sphincter, but it's not long before the ring of muscle surrenders and your honey-soaked " + cockDescript(game.player, x) + " slides into her body.  She whimpers in humiliation, hands clawing at the grass as she shifts her hips to try to get more comfortable around your thickness.  You root around in the exquisite tightness of her ass, enjoying the way she shifts and wriggles under and around you.", false);
        // New PG
        outputText("\n\n", false);
        outputText("Drawing nearly, but not entirely, out of her, you take up your next largest cock and set it back at the entrance of her pussy again, and then thrust with your hips, sinking your largest deep into her ass while spearing her once more, burying your " + cockDescript(game.player, y) + " in her pussy.  She squeals loudly, wings flapping wildly a moment, then goes utterly limp with a whimper of surrender.  Grinning broadly, you begin to piston in and out of her, the sweet perfume of her lust wreathing your nostrils.  The plundering of her nether-regions fills the air with wet, slurping sounds, and soon she is moaning in time with every thrust, antennae quivering with need.  ", false);
        ...
        outputText("After falling to the ground, the bee-girl looks up in dazed fear as you expose your " + multiCockDescriptLight(game.player) + ", her eyes watching as you throb with lust at the scared and helpless look on her face.", false);
        // New PG
        outputText("\n\n", false);
        outputText("Once you're close enough, you grab her by the arms and force yourself down against her, shifting your knees to spread her legs.  Your " + multiCockDescriptLight(game.player) + " slap down against her slightly slick " + vaginaDescript(monster, 0) + ". Her face lights up with a look trapped somewhere between apprehension and interest at the unexpected touch, although the interest doesn't stop her from struggling valiantly to try and get away from you.  Finally, you force her arms against the ground and spread her wide enough to impale her " + vaginaDescript(monster, 0) + " on one of your " + multiCockDescriptLight(game.player) + " while watching her face contort with an alluring mixture of enjoyment and pain.  Her struggles weaken as you start to pound into her, tight abdominal muscles relaxing under you as she throws back her head, releasing a whimper at the rough penetration.  ", false);
        outputText("At first, taking her more than satisfies you, but an idea strikes as you watch her face become distracted with each body-shaking thrusts of your hips.  Grabbing your remaining " + cockDescript(game.player, y) + " you push it against the already-impaled member, and on the next thrust you drive them both in.", false);
        if (player.cocks.averageCockThickness() > 2) outputText("  She stretches against you amazingly, squeezing down on you like a vice.  You theorize it's only because of her strange body and the copious amounts of honey she's leaking against you that you aren't ripping her apart.", false);
        // New PG
        outputText("\n\nThe bee-girl shakes violently and screams in equal parts buzz and moan as a thick torrent of honey splashes against your dual members, making the pair of them slide in that much easier.  Her arms move up, the free one grabbing at your shoulder and clutching hard enough to leave bruises while the other squirms under your hand, her body tense under you. Stretched to the brink by the pair of " + multiCockDescriptLight(game.player) + "s, her hole is like a vice against you and every thrust visibly stretches her. The more you thrust the wetter she gets and her eyes seem to be wildly fluttering underneath you as you take her, her struggles turning to her thrusting her lower body up against you appreciatively. Your " + multiCockDescriptLight(game.player) + "s slap and pound into her abused " + vaginaDescript(monster, 0), false);
        if (player.balls > 0) outputText(" with your " + ballsDescriptLight(player) + " hitting her abdomen hard enough to make smacking sounds ring out in the forest around you.", false);
        else outputText(".", false);
        // New PG
        outputText("\n\n", false);
        // Extra multicock text
        if (player.cocks.length >= 3) {
            outputText("As you pound into her, you realize that large amounts of her juices are leaking from her " + vaginaDescript(monster, 0) + " over her abdomen, against the hole near the base of her stinger.  Reaching down with wicked intent, you grab one of your free " + multiCockDescriptLight(game.player) + "s and force it against her tight asshole while you pull your hips back.  Her eyes widen as she looks up at you, renewing her struggles as she kicks her legs at you, her wings slapping the ground with fierce determination as she tries to wriggle away.  You hold her tightly, pushing your hips against her to bury your " + multiCockDescriptLight(game.player) + " inside her.  The flow of honey barely lessens the friction as your " + cockDescript(game.player, 2) + " sinks into her ass inch by inch.  Her " + vaginaDescript(monster, 0) + " is stretched even wider and she screams out something that sounds like a humming moan as her asshole begins to gape a bit with each of your thrusts. It takes a minute or two, but finally you hit bottom with all three of your " + multiCockDescriptLight(game.player) + "s.\n\n", false);
        }
        // Orgasm PG
        outputText("After a while of fucking her, her buzzing complaints and wild struggles begin to melt as she cums against you, shuddering and forcing her hips up towards you, plunging your " + cockDescript(game.player, x) + " into her even deeper.  She begins to climax, her muscles becoming more and more relaxed with each time her stretched holes squeeze down on you.  Surges of pleasure begin to overwhelm you and soon you begin fucking her as hard as you can, honey splattering wetly over her body as she finally seems to collapse under your body's assault. Her eyes dim visibly, the buzzing cries of pleasure echoing faintly from her lips while her body goes completely limp underneath you.  Not to be stopped, you continue to take her, eventually feeling seed swell up inside ", false);
        ...
        if (player.cocks.length >= 3) {
            outputText("  Her battered " + vaginaDescript(monster, 0) + " grips down on your dual " + multiCockDescriptLight(game.player) + "s tightly, draining you of every drop she can.  Rivers of white begin to leak from her abused holes and mingle with her own release as it trails down her abdomen and drips off it slowly onto the forest floor.", false);
            if (player.cocks.length == 4) outputText("  The " + cockDescript(game.player, 3) + " not inside her shakes wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.", false);
            if (player.cocks.length > 4) outputText("  The " + multiCockDescriptLight(game.player) + "s not inside her shake wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.", false);
        }
        // New PG
        outputText("\n\n", false);
        outputText("The both of you smell like sex as you pull free of her with a groan.  You move to kneel over her and use her unstained lips to dry your " + multiCockDescriptLight(game.player) + " one by one.  Her lips part slightly on instinct and you can feel her swallow the globs of jism that coat you.  Once she's done you stand up quickly, ", false);
        if (player.lowerBody != LowerBodyType.CENTAUR) outputText("stuffing your " + multiCockDescriptLight(game.player) + " inside your " + player.armorName, false);
        else outputText("your glorious but spent " + multiCockDescriptLight(game.player) + " dangling under your belly,", false);
...
function nagaRapesPt2TheExtremeContinuationOfAwesome(): void {
    ...
    if (player.gender == 1 || (player.gender == 3 && rand(3) == 0)) {
        ...
        outputText("your " + multiCockDescriptLight(game.player));
        ...
        if (player.cocks.length > 1) outputText("they free themselves from ", false);
        ...
        if (player.cocks.length > 1) outputText("one of ", false);
        outputText("your " + multiCockDescriptLight(game.player) + ", slurping soundly and driving you wild; ", false);
        // [if player has testicles]
        if (player.balls > 0) outputText("Not while her hands cup your " + sackDescript(player) + " and knead tenderly, infusing you with the craving for release; ", false);
        else outputText("not ", false);
        outputText("even when she takes your " + cockDescript(game.player, 0) + " to the hilt, her eyes straining up in their sockets. You watch " + int(player.cocks[0].cockLength) + " inches of " + cockDescript(game.player, 0) + " disappear inside her mouth, and the vibrations in her throat tell you that you'd hear her cooing if it wasn't packed tight.\n\n", false);
        ...
        // [if player has testicles]
        if (player.balls > 0) outputText("your " + sackDescript(player) + " clenches as ", false);
    ...
    // [Player is female]
        ...
        if (player.vaginas[0].vaginalWetness < VaginaWetness.WET) outputText("Damp", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("Wet", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("Dripping", false);
        else outputText("Soaking", false);
        outputText(" with lubrication, you demand to be satisfied. With one hand you point to your " + vaginaDescript(player, 0) + ", and you snap your fingers with the other. Your prey saunters up to you with a vacant, distant look in her eyes and begins to remove your " + player.armorName + " obediently. You tenderly caress her cheek as she finishes, and watch a rosy glow infuse her skin. Your captive handmaiden runs her hands tenderly up your body, smoothing the flesh of your " + biggestBreastSizeDescript(player) + " and making you pant. You allow yourself to enjoy the sensation for a brief while, and just before you can snap your fingers again, she goes down on her own.\n\n", false);

        outputText("Her eyelids flutter even as her gaze remains locked, and she deeply inhales your musky and arid scent. Deep inside, you feel a level of affection that isn't betrayed by your facial features. You gasp as the bee-girl's lengthy tongue shoots inside your snatch, and retracts as quickly as it came; once, twice, again and again, striking your cervix and twisting out again. Her hands grip your scaly " + hipDescription(player) + " as she thrusts her tongue in and out, threatening the fixation of your glare with pleasure. You are moaning and screaming in your mind, while your stony expression betrays nothing; nothing but the locking gaze that makes your will her own. The bee-maiden's mouth wraps around your " + clitDescription(player) + ", rubbing it through its hood with the inside of her upper lip until it protrudes its full length.", false);
        // [if clit size => 1"]:
        ...
        if (player.vaginas[0].vaginalWetness < VaginaWetness.SLICK) outputText("A few drops ", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.DROOLING) outputText("A gush ", false);
        else if (player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("Wave after wave ", false);
    ...
    // [Player is herm]
        ...
        outputText("Touching a single index finger to the underside of your captive's chin, you pull with the slightest of forces and watch as she is dutifully drawn to your " + multiCockDescriptLight(game.player) + ", running her lengthy prehensile tongue up and down ", false);
        if (player.cocks.length > 1) outputText("each of your shafts", false);
        else outputText("your shaft", false);
        outputText(". As the pulsing movement inside your body draws you slowly closer to orgasm, the bee-bitch uses her other hand to draw your " + cockDescript(game.player, 0) + " down to an angle so it slides neatly into her throat", false);
        // [if Player cock => 12\"]
        ...
        if (player.cocks.length > 1) {
            ...
            // [three or more]
            if (player.cocks.length > 2) outputText("s, giving each a few pumps before moving on to the next", false);
        ...
        // [if Player has multiple cocks]
        if (player.cocks.length > 1) outputText("jacking, ", false);
