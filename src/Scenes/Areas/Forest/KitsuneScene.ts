...
export function enterTheTrickster(): void {
...
// Lose:
export function loseKitsuneImpFight(): void {
    clearOutput();
...
// Win:
export function winKitsuneImpFight(): void {
    clearOutput();
    // randomly assign hair color
    if (rand(3) == 0) monster.hairColor = "blonde";
    else if (rand(2) == 0) monster.hairColor = "black";
...
function followTheWillOWisp(firstTime: boolean = false): void {
    ...
    // Else:
        ...
        if (!game.inCombat) simpleChoices("Fight", fightSomeKitsunes, "Talk", talkAfterResistingKitsunellusion, "", null, "", null, "", null);
...
// [Leave] (C)
function leaveKitsune(talked: boolean = false): void {
    clearOutput();
    kitsuneSprite();
    ...
    if (game.inCombat) {
        flags[kFLAGS.BONUS_ITEM_AFTER_COMBAT_ID] = ConsumableLib.KITGIFT.id;
        cleanupAfterCombat();
    }
    else {
        // add Kitsune's Gift to inventory
        Inventory.takeItem(ConsumableLib.KITGIFT, Camp.returnToCampUseOneHour);
    }
}
...
function nonTentaclePCMansion(willing: boolean): void {
    ...
    if (player.cocks.tentacleCocks() >= 3) {
        ...
        if (player.cocks.length > 0) doNext(createCallBackFunction(kitsuneMaleOrHermMansion, willing));
...
function kitsuneMaleOrHermMansion(willing: boolean): void {
    ...
    outputText("The three tails recede from [eachCock] with an agonizing slowness as the black-haired girl lowers the voluptuous globes of her ass down against your " + cockDescript(player, player.cocks.biggestCockIndex()) + ".  Her hips rise and fall slowly, teasing the underside of your cock with the silky soft flesh of her rear, and the sight of her tattooed hindquarters gently cradling your prick sends fresh blood to your groin.  Your pleasure is only amplified when the blonde begins to stroke and knead it, casting you a sultry smirk as she lowers her lips toward the " + cockHead(player, player.cocks.biggestCockIndex()) + ".  Her soft tongue slides warmly along the top edge, traveling from base to tip and back again, each and every movement carefully measured and controlled to tease and excite you.\n\n");
...
// end function
// fomerly letHer()
function kitSuneLetHerMansion(willing: boolean = true): void {
    clearOutput();
    kitsuneSprite();
    ...
    outputText("The blonde's prodigious posterior bounces and jiggles as she pumps her hips up and down instinctively.  The quivering cheeks entice you, drawing you in almost hypnotizingly, and before you know it your " + cockDescript(player, player.cocks.biggestCockIndex()) + " is already " + ((player.balls > 0) ? "balls" : "hip") + "-deep in her slippery snatch.\n\n");
    outputText("The voluptuous fox-woman groans in pleasure as your " + cockDescript(player, player.cocks.biggestCockIndex()) + " plunges into her deepest fathoms, her warm flesh enveloping your hot prick and squeezing tightly as a rush of feminine fluids spills over the black-haired girl's face." + ((player.cocks.biggestCockArea() > 50) ? "  Her folds seem to stretch forever, swallowing your engorged tool like a bottomless pit of warmth and pleasure." : "") + "  Your hands sink deep into the pillowy mounds of her mountainous ass, mashing them together as you knead her cheeks like warm dough.\n\n");
    outputText("Slender fingers close around your " + hipDescription(player) + " as the redhead pulls herself tight against your [butt], attempting to line up against you while your pelvis mashes wildly against her sister's rippling rump.  Her cock slides up between your cheeks, dripping pre-cum that tingles on your flesh, and she finally manages to align herself to your ");
    if (player.gender >= 2) outputText(vaginaDescript(player));
    else outputText(assholeDescript(player));
    ...
    if (player.gender == 1) buttChange(player, 14, true, true, false);
    else cuntChange(player, 14, true, true, false);
    outputText("\n\n");
    outputText("Sandwiched between the two amorous kitsune, you shudder in pleasure, feeling the blonde's slick pussy squeeze and milk along your length while the redhead bucks up against your twitching hips with an animalistic fervor.  You feel something slick sliding along the " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " of your " + cockDescript(player, player.cocks.biggestCockIndex()) + ", the black-haired kitsune's long tongue wrapping around and slurping the blonde's juices off of it hungrily." + ((player.balls > 0) ? "  Her pillowy lips press up against your " + ballsDescriptLight(player) + " lovingly, slurping loudly and humming through them with glee." : "") + "\n\n");
    ...
    outputText(" around her shaft sends her over the edge, and she begins to groan loudly in your ear, digging her fingertips into your " + hipDescription(player) + ".  Her turgid rod trembles in your ");
    if (player.gender >= 2) outputText(vaginaDescript(player));
    else outputText(assholeDescript(player));
    ...
    outputText("The pleasant pressure in your expanding belly pushes your pleasure to new extremes, and you begin mashing your pelvis into the blonde's quivering ass cheeks in pursuit of your glorious release." + ((player.cocks.biggestCockArea() > 80) ? "  She lurches forward, her fingers clawing at the wooden deck that runs along the edge of the pool, her entire body heaving with pleasure." : "  As you feel the coiling spring in your lower body near its limit, you suddenly feel the redhead's hand wrap around your " + cockDescript(player, player.cocks.biggestCockIndex()) + ".  She slyly leans over your shoulder, raising a finger to her lips and whispering to you, \"<i>Shh...</i>\"\n\nWithout warning, she yanks you back, pulling you from the blonde's quivering cunt just as you begin to climax, adjusting the angle quickly and bucking her hips forward to slam your twitching shaft into the blonde's ass!  The soft ring of her anus yields to the invasion with surprising ease, spreading open as if by reflex and then clamping shut around your base when your hips make contact with her quivering cheeks again.  She lurches forward, releasing a surprised gasp and nearly collapsing under your compulsory anal assault, her entire body heaving with a combination of shock and pleasure.") + "\n\n");
    if (player.cumQ() <= 150)
        outputText("A few virile streams spurt into her, sloppily smearing her " + ((player.cocks.biggestCockArea() > 80) ? "passage" : "intestines") + " with your thick load.  Her inner muscles squeeze you involuntarily, a deep crimson blush spreading across her cheeks as a thick spray of girlcum erupts from her pussy.  It splatters over the black-haired girl's face, and she opens her mouth to catch it all, pressing her plump lips against her sister's quivering cunt and drinking deeply as she hums a satisfied melody into her clitoris.");
    else if (player.cumQ() <= 350) outputText("Numerous streams spurt into her, sloppily smearing her tunnel with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft to draw your seed " + ((player.cocks.biggestCockArea() > 80) ? "toward her convulsing womb." : "deeper into her convulsing bowels.") + "  A deep crimson blush spreads across her cheeks as a thick spray of girlcum erupts from her pussy, splattering over the black-haired girl's face, and she opens her mouth to catch it all, pressing her plump lips against her sister's quivering cunt and drinking deeply as she hums a satisfied melody into her clitoris.");
    else if (player.cumQ() <= 1000) outputText("Thick ribbons of cum spurt into her, sloppily smearing her " + ((player.cocks.biggestCockArea() > 80) ? "passage" : "intestines") + " with your potent load.  Her inner muscles squeeze you greedily, massaging your shaft to draw your seed " + ((player.cocks.biggestCockArea() > 80) ? "toward her convulsing womb.  Her pussy" : "deeper into her convulsing bowels.  Her rectum") + " clenches involuntarily as your cloying seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "quivering cunt and floods her passage" : "ass and floods her intestines") + ", a deep crimson blush spreading across her cheeks as a thick spray of girlcum erupts from her pussy.  It splatters over the black-haired girl's face, and she opens her mouth to catch it all, pressing her plump lips against her sister's quivering cunt and drinking deeply as she hums a satisfied melody into her clitoris.");
    else if (player.cumQ() <= 2500) outputText("Hot ropes of cum splash into her " + ((player.cocks.biggestCockArea() > 80) ? "slit" : "ass") + ", smearing her with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft as it swells with virile seed over and over again, thick streams spurting out of her hole as her stomach begins to distend into a jiggling round belly from the pressure.  Her " + ((player.cocks.biggestCockArea() > 80) ? "pussy" : "rectum") + " clenches involuntarily as your cloying seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "quivering cunt and floods her passage" : "ass and floods her intestines") + ", her stomach expanding with a jiggle and hanging low and heavy with the weight of your seed.  A deep crimson blush spreads across her cheeks as a thick spray of girlcum erupts from her pussy, splattering over the black-haired girl's face, and she opens her mouth to catch it all, pressing her plump lips against her sister's quivering cunt and drinking deeply as she hums a satisfied melody into her clitoris.");
    else if (player.cumQ() > 2500) outputText("Hot waves of cum crest against her " + ((player.cocks.biggestCockArea() > 80) ? "cervix, flooding her womb" : "sphincter, flooding her ass") + " with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft as it swells with virile seed over and over again, pressurized streams erupting from her " + ((player.cocks.biggestCockArea() > 80) ? "snatch" : "butt") + " as her abdomen swells to an obscene size.  Her " + ((player.cocks.biggestCockArea() > 80) ? "pussy" : "rectum") + " squeezes you involuntarily as your cloying seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "quivering cunt and floods her passage" : "ass and floods her intestines") + ", her stomach rapidly swelling into a heavy, gravid belly, sloshing audibly.  A deep crimson blush spreads across her cheeks as a thick spray of girlcum erupts from her pussy, splattering over the black-haired girl's face, and she opens her mouth to catch it all, pressing her plump lips against her sister's quivering cunt and drinking deeply as she hums a satisfied melody into her clitoris.");
    ...
    outputText("Her warm " + ((player.cocks.biggestCockArea() > 80) ? "cunt" : "asshole") + " still grips your cock tightly, dragging you down with her.  You collapse on top of her, sliding off of the redhead's cock as you tumble into the golden curls of the blonde's hair.");
    ...
    outputText("  As your twitching cock relieves itself of the last of your seed inside the blonde's " + ((player.cocks.biggestCockArea() > 80) ? "pussy" : "ass") + ", you feel your strength slipping away from you with each spasm, your eyelids growing heavy with an uncommon weariness.\n\n");
    player.orgasm();
    doNext(kitsuneStillHungryMansion);
} // End letHer()

// Formerly shoveHer()
function kitsuneShoveHerMansion(willing: boolean = true): void {
    clearOutput();
    kitsuneSprite();
    ...
    if (player.gender == 1 || ((player.gender == 3) && flags[kFLAGS.redheadIsFuta] == 0)) {
        ...
        outputText("You edge closer and closer to your climax, shivering with pleasure as pressure builds in your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + ".  The " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " of your " + cockDescript(player, player, player.cocks.biggestCockIndex()) + " bulges outwardly as the first thick glob of cum swells through it, forcing its way into the blonde's unprotected womb.  ");
        if (player.cumQ() <= 150) {
            outputText("A few virile streams spurt into her, sloppily smearing her tunnel with your thick load.  Just as your potency begins to wane, you feel her golden tail constrict around your base, blocking the flow.  You groan deeply in protest as your shaft spasms dryly, her muscular snatch forcing you out with a loud 'plop!'  As the remainder of your seed builds up painfully behind her tail, she raises a finger to her lips, leering at you slyly.  Before you know what is going on, her tail directs your throbbing shaft downward and you feel yourself plunge into the redhead's " + ((player.cocks.biggestCockArea() > 80) ? "vagina!" : "defenseless anus!") + "  The unsuspecting girl lurches forward with a moan, and at the same time the blonde releases her grip on your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ", allowing your load to freely spill into her sister's tender " + ((player.cocks.biggestCockArea() > 80) ? "pussy. Her slit " : "butt. Her rectum") + " squeezes you involuntarily, her face caught halfway between shock and orgasmic pleasure.");
        }
        else if (player.cumQ() <= 350) {
            outputText("Numerous streams spurt into her, sloppily smearing her tunnel with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft to draw your seed toward her convulsing womb.  Just as your potency begins to wane, you feel her golden tail constrict around your base, blocking the flow.  You groan deeply in protest as your shaft spasms dryly, her muscular snatch forcing you out with a loud 'plop!'  As the remainder of your seed builds up painfully behind her tail, she raises a finger to her lips, leering at you slyly.  Before you know what is going on, her tail directs your throbbing shaft downward and you feel yourself plunge into the redhead's " + ((player.cocks.biggestCockArea() > 80) ? "vagina!" : "defenseless anus!") + "  The unsuspecting girl lurches forward with a moan, and at the same time the blonde releases her grip on your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ", allowing your load to freely spill into her sister's tender " + ((player.cocks.biggestCockArea() > 80) ? "pussy. Her slit " : "butt. Her rectum") + " squeezes you involuntarily as a final glut of seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "passage" : "intestines") + ", her face caught halfway between shock and orgasmic pleasure.");
        }
        else if (player.cumQ() <= 1000) {
            outputText("Thick ribbons of cum spurt into her, sloppily smearing her tunnel with your potent load.  Her inner muscles squeeze you greedily, massaging your shaft to draw your virile seed toward her convulsing womb, thin streams messily spurting from her entrance as her abdomen begins to bulge from the pressure.  Just as your potency begins to peak, you feel her golden tail constrict around your base, blocking the flow.  You groan deeply in protest as your shaft spasms dryly,  her muscular snatch forcing you out with a loud 'plop!'  One particularly eager spurt of seed manages to escape and dribbles weakly down the tip, and as the remainder of your seed builds up painfully behind her tail, she raises a finger to her lips, leering at you slyly.  Before you know what is going on, her tail directs your throbbing shaft downward and you feel yourself plunge into the redhead's " + ((player.cocks.biggestCockArea() > 80) ? "vagina!" : "defenseless anus!") + "  The unsuspecting girl lurches forward with a moan, and at the same time the blonde releases her grip on your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ", allowing your load to freely spill into her sister's tender " + ((player.cocks.biggestCockArea() > 80) ? "pussy. Her slit " : "butt. Her rectum") + " squeezes you involuntarily as your cloying seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "pussy and floods her womb" : "ass and floods into intestines") + ", her face caught halfway between shock and orgasmic pleasure as her stomach begins to swell slightly.");
        }
        else if (player.cumQ() <= 2500) {
            outputText("Hot ropes of cum splash into her womb, smearing her with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft as it swells with virile seed over and over again, thick streams spurting out of her entrance as her stomach begins to distend into a jiggling round belly from the pressure.  At the peak of your orgasm, you feel her golden tail constrict around your base, blocking the flow.  You groan deeply in protest as your shaft spasms valiantly, her muscular snatch forcing you out with a loud 'plop!'  The pressure in your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + " is too great for her to hold all of it back, and a number of pressurized streams force their way by, streaking her body with hot seed.  She opens her mouth and happily catches an errant stream on her tongue, then raises a finger to her lips and leers at you slyly.  Before you know what is going on, her tail directs your throbbing shaft downward and you feel yourself plunge into the redhead's " + ((player.cocks.biggestCockArea() > 80) ? "vagina!" : "defenseless anus!") + "  The unsuspecting girl lurches forward with a moan, and at the same time the blonde releases her grip on your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ", allowing your load to freely spill into her sister's tender " + ((player.cocks.biggestCockArea() > 80) ? "pussy. Her slit " : "butt. Her rectum") + "  squeezes you involuntarily as your cloying seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "pussy and floods into her passage" : "ass and floods her intestines") + ", her face caught halfway between shock and orgasmic pleasure as her stomach expands with a jiggle, hanging low and heavy with the weight of your seed.");
        }
        else if (player.cumQ() > 2500) {
            outputText("Hot waves of cum crest against her " + ((player.cocks.biggestCockArea() > 80) ? "cervix" : "sphincter") + ", flooding her " + ((player.cocks.biggestCockArea() > 80) ? "womb" : "anus") + " with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft as it swells with virile seed over and over again, pressurized streams erupting from her " + ((player.cocks.biggestCockArea() > 80) ? "snatch" : "pucker") + " as her abdomen swells to an obscene size.  At the peak of your orgasm, you feel her golden tail constrict around your base, trying in vain to stem the flow.  You groan deeply in protest as your shaft spasms violently, her muscular " + ((player.cocks.biggestCockArea() > 80) ? "snatch" : "pucker") + " forcing you out with a loud 'plop!'  The pressure in your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + " is far too much for her to hold back, and her efforts only succeed in turning your eruptive streams into a sporadic shower of semen that streaks her front with a percussive patter.  She revels in the showering seed, happily catching a few streams on her tongue, then raises a finger to her lips and leers at you slyly.  Before you know what is going on, her tail directs your throbbing shaft downward and you feel yourself plunge into the redhead's " + ((player.cocks.biggestCockArea() > 80) ? "vagina!" : "defenseless anus!") + "  The unsuspecting girl lurches forward with a moan, and at the same time the blonde releases her grip on your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ", allowing your abundant load to freely splatter into her sister's tender " + ((player.cocks.biggestCockArea() > 80) ? "pussy" : "butt") + ".  Her " + ((player.cocks.biggestCockArea() > 80) ? "slit" : "rectum") + " squeezes you involuntarily as your cloying seed erupts into her " + ((player.cocks.biggestCockArea() > 80) ? "pussy" : "ass") + " and floods her " + ((player.cocks.biggestCockArea() > 80) ? "passage" : "intestines") + ", her face caught halfway between shock and orgasmic pleasure as her stomach rapidly swells into a heavy, gravid belly, sloshing audibly.");
        }
        outputText("\n\nThe sudden intrusion " + ((player.cocks.biggestCockArea() > 80) ? "" : "into her behind ") + "drives her over the edge in an instant, and soon your ears are met with the sound of hungry slurping.  " + (flags[kFLAGS.redheadIsFuta] > 0 ? "The black-haired kitsune swallows eagerly around her sister's engorged cock, slurping down stream after stream of thick futa-jizz.  She raises her hand up and slides two fingers into the girl's convulsing cleft, moaning in approval as the copious fountain of semen in her throat thickens even more.  Her other hand slowly massages her gradually expanding stomach, caressing the growing bulge while she pounds her fingertips into her sister's pussy, sucking out every creamy, salty morsel." : "The black-haired kitsune laps eagerly at her sister's convulsing pussy, a thick spray of girlcum splattering across her face as she tries her best to catch it in her mouth.  Her lips wrap around the girl's quivering cunt, and she begins to drink deeply from the viscous fountain, humming her pleasure the entire time.") + "  After what certainly must have felt like an eternity, the girl's climax begins to ebb, and she collapses fantastically beneath the blonde's weight with a splash, narrowly avoiding the raven-haired girl's face on the way down.\n\n");
        outputText("Her warm " + ((player.cocks.biggestCockArea() > 80) ? "cunt" : "asshole") + " still grips your cock tightly, dragging you down with her.  You collapse into the blonde's open arms, finding yourself pressed into the marshmallowy embrace of her mountainous breasts.  As your twitching cock relieves itself of the last of your seed inside the redhead's " + ((player.cocks.biggestCockArea() > 80) ? "pussy" : "ass") + ", you feel your strength slipping away from you with each spasm, your eyelids growing heavy with an uncommon weariness.\n\n");
    }
    else if ((player.gender = 3) && flags[kFLAGS.redheadIsFuta] > 0) {
        outputText("\n\nThe blonde casually wades over to the redhead, flipping her onto her back and sliding onto her sister's hips with a haughty smirk.  The poor redhead is pinned under her weight, flushing with wanton desire, her cock twitching needfully between the blonde's ass cheeks.  One of the blonde's tails winds around the " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " of your " + cockDescript(player, player.cocks.biggestCockIndex()) + ", pulling it into the shimmering tangle of golden coils, and though a fluffy forest obscures your view, you can feel the " + cockHead(player, player.cocks.biggestCockIndex()) + " pressing up against the blonde's warm, inviting snatch.\n\n");
        outputText("Her tail coils and constricts around your member, drawing you forward to plunge into the waiting folds with a wet slurp." + ((player.cocks.biggestCockArea() > 80) ? "  Her walls draw you inward for ages, rippling sensually along your length as her cavernous cunt seems to go on forever.  Impossible though it might sound, it seems she intends to take ALL of you." : "") + "  She shudders with satisfaction as your hips lightly press against hers, a few of her tails lashing around your " + hipDescription(player) + " to draw you in as far as possible" + ((player.cocks.length > 1) ? ", [eachCock] entangled in a jungle of gold that caresses and tickles them teasingly." : ".") + "  The redhead's tails flail sporadically beneath her, bristling with pleasure as your " + vaginaDescript(player) + " slides across the tip of her cock teasingly, sending a shiver up her spine and a blush to her cheeks.\n\n");
        outputText("You feel something brushing against you and look down to find the black-haired girl pressing her face against your [butt], running her hands up and down your " + hipDescription(player) + " and smacking your cheek once lightly for good measure.  An involuntary shudder zigzags up your spine as her slick tongue runs up your crack, lining your [asshole] with saliva.  Her fingertips dig into the flesh of your bottom, the other hand sliding along your " + vaginaDescript(player) + ", pinching and rolling your " + clitDescription(player) + ".  A moment later, you rise an inch or two off the ground as a squirming invader spears your ass.  Her plush lips press against you like soft pillows, a muffled giggle vibrating up her tongue and into your prostate.  Your strength gives out as she prods your prostate, dragging you down with her weight at the same time to impale you on the redhead's cock.");
        ...
        outputText("You edge closer and closer to your climax, shivering with pleasure as pressure builds in your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + ".  The " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " of your " + cockDescript(player, player.cocks.biggestCockIndex()) + " bulges outwardly as the first thick glob of cum swells through it, forcing its way into the blonde's unprotected womb.  ");
        if (player.cumQ() <= 150) {
...
function kitsuneStillHungryMansion(): void {
    ...
    outputText(((player.balls > 0) ? "She exposes her upturned palms, rubbing them together for a moment and then blowing into them lightly.  With a snapping crackle, a bright blue flame bursts to life in her hands, and she presses her palms to your " + ballsDescriptLight(player) + ", massaging them gently." : "She extends her index and middle fingers, and with a snapping crackle, a bright blue flame bursts to life at her fingertips.  Before you can protest, your [butt] is invaded by her pyrotechnic digits, pressing against your prostate insistently.") + "  As the cooling tingle of her mystical flame spreads across your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + ", you can feel a tense pressure building up, pleasure coursing through you as your spent stores of seed miraculously begin to replenish themselves, your flagging " + cockDescript(player, player.cocks.biggestCockIndex()) + " springing to readiness instantly!\n\n");
    outputText("Your rock-hard shaft is immediately engulfed in the soft embrace of the girl's breasts, the tip entrapped by the moist prison of her warm mouth.  A tongue of improbable length slides out of the girl's mouth, slathering saliva across your turgid rod and coiling around it with a tight squeeze.  Her plush pink pillows caress your " + cockHead(player, player.cocks.biggestCockIndex()) + " with a sensual suckling, drawing you inward and nibbling on the flesh delicately.\n\n");
    outputText(((player.cocks.biggestCockArea() < 70) ? "In a single stroke she swallows your shaft, her lips gliding all the way down to the hilt as her warm throat envelopes you, squeezing gently." : "She draws the shaft as far into her mouth as possible, her throat stretching around your head and pulling it in.  When she can swallow no more, she pauses to stabilize her breathing through her nose, then sighs contentedly.") + "  Her eyes roll back pleasantly and she glances up at you, the corners of her mouth curled up in a devious smirk, her tongue still curled around your stiff rod just past the edges of her lips.\n\n");
    ...
    outputText("Still overly sensitive from the exhilarating romp you had just taken part in, your " + cockDescript(player, player.cocks.biggestCockIndex()) + " begins to throb and twitch excitedly within the black-haired girl's throat, already being pushed to the limits of your endurance.  ");
    if (flags[kFLAGS.redheadIsFuta] > 0) {
...
function kitsuneFemaleOrGenderless(willing: boolean): void {
    ...
    outputText("One fingertip is shortly followed by a second, gently invading your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + " up to the second knuckle and making you moan and clench in pleasure.  Tender caresses slide down your [fullChest] as the sisters lavish you with affection - a gentle tweak here, a nip there, every action carefully measured to tease and arouse you.  The black-haired one carefully climbs on top of you, wrapping her lips around one of your " + nippleDescription(player, 0) + "s and flicking it gently with her tongue." + ((player.lactationQ() > 0) ? "  She titters with excitement as your fresh cream begins to pour into her mouth, massaging your [chest] gently to stimulate the flow and drinking it down eagerly.\n\n\"<i>Mmmm...  So very tasty,</i>\" she says with a smirk, dipping down for another drink." : "Her teeth gently play along the tender flesh, sending shocks of pleasure through you.  Pale blue flames spread from the corners of her lips, elevating the pleasure even further, and you find it hard to stop shivering.") + "\n\n");
...
function kitsunesGenderlessLetHer(willing: boolean): void {
    clearOutput();
    kitsuneSprite();
    outputText(((willing) ? "You decide to let the girl have her fun, allowing her to shift positions to begin rubbing her throbbing shaft against your " + ((player.gender == 2) ? vaginaDescript(player) : "featureless groin") + " eagerly." : "You shake your head sluggishly, but your slurred protests fall on deaf ears.  She easily sidesteps your " + leg(player) + " as you try to shove her away, sliding forward to lay a disarming kiss on your cheek.  A bright blue wisp of flame crackles from her lips, and that is that - the resulting spark of pleasure that arcs up your spine shatters the last of your resistance, and your cares melt like butter.") + "  While she grinds her pulsating shaft against you slowly, her sisters close in around you to join in the fun.\n\n");
    outputText("Your head is drawn into the blonde's lap, fingers softly gliding " + ((player.hairLength > 0) ? "through your " + hairDescription(player) : "across the sides of your face") + ", brushing you lightly with her mystical flames.  A pair of large, shapely ass cheeks emblazoned with a sun-shaped tattoo enters your field of vision, and soon your view is entirely blocked by the black-haired girl's expansive behind, save for a sliver of light filtering down between her and the blonde.  Her slick pussy glides across your lips, coaxing your tongue from your mouth as she bends down, pulling the redhead's cock into her mouth for a moment to slather it with saliva, spreading your " + ((player.gender == 2) ? "labia" : "butt cheeks") + " with her first two fingers, egging her on.\n\n");
    outputText("The redhead wastes no time taking the invitation, spearing your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + " in a single thrust.  Her thick rod pierces your waiting hole, bringing with it a distinctive effervescent tingle.  Her hands close around your " + hipDescription(player) + ", and she begins to thrust wantonly, playing up her pleasure with an exaggerated moan as her breasts jiggle to and fro with each buck of her hips.");
    if (player.gender >= 2) cuntChange(player, 14, true, true, false);
    else buttChange(player, 14, true, true, false);
...
function kitsunesGenderlessShoverHer(willing: boolean): void {
    ...
    outputText("Her tails coil and constrict around your thighs, drawing you forward to press against her quivering lips while the tips sensually brush your sensitive nethers, sending light tingles up and down your spine.  " + ((player.clitLength > 4) ? "You groan a bit as you feel your " + clitDescription(player) + " gently slipping between the moist folds of her cunt, caressed on all sides by warm, wet flesh, the muscles squeezing down to conform to its shape perfectly.  " : "") + "She shudders with satisfaction as you lightly press against her, a few of her tails lashing around your " + hipDescription(player) + " to hold you tight.  The redhead's tails flail sporadically beneath her, bristling with pleasure as " + (flags[kFLAGS.redheadIsFuta] > 0 ? "your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + " slides across the tip of her cock teasingly" : "you are rubbed against her slavering slit") + ", sending a shiver up her spine and a blush to her cheeks.\n\n");
    outputText("You feel something brushing against you and look down to find the black-haired girl pressing her face against your [butt], running her hands up and down your " + hipDescription(player) + " and smacking your cheek once lightly for good measure.  An involuntary shudder zigzags up your spine as her slick tongue runs up your crack, lining your [asshole] with saliva.  Her fingertips dig into the flesh of your bottom, " + ((player.gender == 2) ? "the other hand sliding along your " + vaginaDescript(player) + ", pinching and rolling your " + clitDescription(player) + "." : "a fingertip gently tracing the curvature of your cheek as sparks snap and pop in its wake.") + "  A moment later, you rise an inch or two off the ground as a squirming invader spears your ass.  Her plush lips press against you like soft pillows, a muffled giggle vibrating up her tongue.  Your strength gives out as she prods your innards, " + (flags[kFLAGS.redheadIsFuta] > 0 ? "dragging you down with her weight at the same time" + ((player.gender == 2) ? " to impale you on the redhead's cock." : ", her tongue receding just as you are impaled on the redhead's cock.  The slick organ slides around the base of her cock, constricting tightly, and you can feel it swell as the veins and ridges become more pronounced.") : "and while you are at your most vulnerable, she twists two of the redhead's tails into a tight bundle, thrusting it into your " + ((player.gender == 2) ? "vagina." : "ass.")));
    cuntChange(player, 24, true, true, false);
    outputText("\n\n");
    ...
    outputText("Holding the blonde's shapely hips, you begin to grind against her slippery gash, moaning shamelessly as the heat washes over you.  She licks her lips hungrily, rolling her eyes back and groaning in ecstasy, playing up the pleasure to extremes in order to tease her sisters.  Smirking deviously, she raises an index finger into the air and presses it to your " + ((player.gender == 2) ? clitDescription(player) : "bare crotch") + ", a pale blue flame sending a spark of pleasure through you.\n\n");
...
function genderlessKitsuneStillHungry(willing: boolean): void {
    ...
    outputText("\"<i>Still hungry...</i>\" she says softly, rubbing her chubby stomach gently, a quiet rumble issuing forth.\n\n");
    outputText("She exposes her upturned palms, rubbing them together for a moment and then blowing into them lightly.  With a snapping crackle, a bright blue flame bursts to life in her hands, and she presses her palms to your [chest], massaging gently.  As the cooling tingle of her mystical flame spreads across your chest, you can feel a tense pressure building up, pleasure coursing through you.  Your " + ((player.breastRows.biggestTitSize() < 3) ? ((player.breastRows.biggestTitSize() == 0) ? "flat chest begins" : "breasts begin") + " to swell, jiggling titflesh spilling out with a tense tingle until you are the proud new owner of a sizeable pair of C cups" : "breasts seem to plump up a bit, your nipples growing hard as a tense tingle spreads through them.") + "  You arch your back, clutching your chest as the tingling intensifies, an inferno of pleasure burning in your chest.  At the height of ecstasy, your milk begins to spurt from your nipples, flying into the air like a lewd fountain.\n\n");
...
function outro(tentacles: boolean = false): void {
    ...
    if (player.fatigue >= 100) {
        ...
        if (player.cumQ() < 10000 || (player.cocks.length === 0 && player.lactationQ() < 10000)) {
...
export function loseToKitsunes(): void {
    const scene: any[] = [];
    // [LOSE FIGHT]
    // Shared Scenes
    // For Males and Herms
    if (player.cocks.length > 0) scene.push(loseFightToHerms);
    // For Females and Genderless
    if (player.gender != 1) scene.push(femalesAndNuetersLoseToKitsunes);
    // Blonde-exclusive
    // Single cock < 9 inches long
    if (monster.hairColor == "blonde" && player.cocks.length == 1 && player.cocks.longestCockLength() < 9) {
    ...
    if (player.cocks.length > 0 && player.balls > 0 && player.cumQ() > 1000) {
...
function femalesAndNuetersLoseToKitsunes(): void {
    ...
    outputText("Humming quietly, she carefully removes your " + player.armorName + ", neatly arranging them into a pile and shoving them off to the side.  Eying your nude form hungrily, she begins to slowly trail her hands down your [fullChest], letting her fingertips cascade over every ridge and curve of your body while painting streaks of blue fire across your flesh.  Her touch sends tingles up and down your spine, making you shiver and moan involuntarily.  Slowly but surely her hands work their way down toward your loins, teasing around your [hips] very gently" + ((player.isBiped()) ? ", tickling the insides of your thighs" : "") + " before trailing up to your " + ((player.gender == 2) ? "rapidly moistening cleft.  Her fingers dance along the inside of your labia" : assholeDescript(player) + ".  Her fingers dance along between your cheeks") + ", circling the entrance and nearly - but never quite - pushing a fingertip inside, snickering in satisfaction as you release a small rush of fluids into her palm." + ((player.gender == 2) ? "  She glides the tip of one of her tails across your [clit] delicately, like a feather, flicking it up and down." : "") + "\n\n");
...
function blondeKitsuneRapesSmallWangs(): void {
    ...
    outputText("Her tails snake forward, curling around your limbs, and you find yourself pinned, struggling in vain against the uncommonly strong furred coils.  She nods, satisfied with your helplessness, and leans down to gently nuzzle your " + cockDescript(game.player, 0) + ", cooing softly and sliding her soft, wet tongue along the underside from base to tip with a small giggle.  Her tongue swirls back and forth along the underside of your " + cockHead(player) + ", wrapping around it and slurping up your slick precum.\n\n");
    ...
    outputText("Her entire body shudders a bit, and you can now feel her soft breasts squashing against your groin as she leans down to lightly kiss the end of your " + cockDescript(game.player, 0) + ".  She starts to rock her wide, curvy hips back and forth across your face, grinding her crotch into your mouth and moaning.  The tails that aren't pinning your limbs down gently curl underneath your neck, lifting your head up.  You moan into her pussy as you feel her lips softly wrap around the tip of your shaft, her tongue playing along the " + cockHead(player) + " lightly.  Her lips make a soft smacking noise as she pulls away, and she starts to toy with your member gleefully, stroking it teasingly with her index finger just to feel your reactions trembling up through her loins.\n\n");
    outputText("\"<i>Oh yeah, that's right, just like that!  Be a good " + mf(player, "boy", "girl") + " and lick mama's pussy,</i>\" she groans, mashing her cunt into your face and riding your tongue with an animalistic passion.\n\n");
    ...
    outputText("\"<i>No!  You behave, naughty " + mf(player, "boy", "girl") + "!</i>\" she growls.  You can practically feel her scowling, despite the fact that your vision is almost entirely obscured by her colossal keister.  To punish you for your transgression, her tails pull your hands away from her ass, and she sits up, smothering you with it.\n\n");
    ...
    outputText("\"<i>Mm, hey there lil' guy,</i>\" she says in a singsong voice, wrapping one arm around her breasts to squash them down and hold them in place while gently fiddling with the tip of your " + cockDescript(game.player, 0) + " using her index finger.\n\n");

    outputText("\"<i>Oooh, right there, lick right there!  If you're a good " + mf(player, "boy", "girl") + ", I might just let you cum on my tits.</i>\"\n\n");
    outputText("You nod your agreement before you can stop yourself, realizing that you are not in control of your own body anymore.  Her tails loosen their grip, permitting you to grab handfuls of her plush ass once again, and she begins to slowly slide her precum-lathered tits up and down your pole, giggling incessantly to herself as she watches it twitch within her cleavage.  She rewards your obedience and submission, happily running her tongue across your " + cockHead(player) + " and matching her pace according to how well you please her.\n\n");
    ...
    outputText("Her lips rise and fall around your spasming prick, low moans vibrating through it each time she pulls back.  You can feel her cheeks pulling inward from the strength of her suction, loud smacks filling the air each time a gap forms in the almost airtight seal of her lips.  She squashes her breasts down as much as she can, lowering her lips to kiss your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ((player.balls > 0) ? " and reaching down to gently stroke your " + ballsDescriptLight(player) : "") + ((player.gender == 3) ? ", one of her long, dextrous fingers sliding into your " + vaginaDescript(player) : "") + "\n\n");
...
function loseToKitsunesWithBallsAndHighCumQ(): void {
    ...
    outputText("Her coy grin turns to one of delighted surprise as her fingertips pass over your " + ballsDescriptLight(player) + ", giving them a firm but gentle squeeze.  She licks her lips and eagerly pulls your " + player.armorName + " off with a hungry glint in her eyes, lifting up your [cock biggest] and flexing it slightly as though examining it.  She leans down, her face close to your balls, and drinks in their scent with a long, drawn-out sigh.\n\n");
    outputText("\"<i>Smells like you've got a thick one brewing...</i>\"  she says, softly licking the " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " with a giggle.  Six inky black tails slither forth to bind your limbs, twisting your wrists behind your back.  \"<i>Let's see if we can't help it along...</i>\"\n\n");
    ...
    outputText("Giggling at your shuddering moans and desperate bucking, she pushes your hips back down, dropping low and pinning you in place with her elbows.  Her soft, voluptuous breasts slide around your [cock biggest]" + ((player.cocks[player.cocks.biggestCockIndex()].cockLength > 8) ? " while she lavishly licks the exposed portion, wrapping her improbably large tongue around it and squeezing in a tight spiral." : ", and she dips her head down to wrap her plump lips around the head, lavishly licking the underside with her improbably large tongue.") + "  Her jiggling rump waggles from side to side cheerfully as she goes down on your cock" + ((player.cocks.biggestCockArea() > 70) ? " as far as she can, letting your oversized shaft stuff her throat with a moan." : ", lips pressed tight to your groin, slurping up your precum with a sublime grin lifting the corners of her mouth.") + "\n\n");
    ...
    outputText("As if to demonstrate her supreme skill with her tongue, she allows her hands to wander freely along your lower body while she continues to bounce and squeeze your balls with her tongue.  " + ((player.gender == 3) ? "The tip snakes down, hugging the curvature of your balls, and then begins to experimentally prod your " + vaginaDescript(player) + ", gently flicking your clitoris.  " : "") + "Pleasure turns to surprise when you feel her index finger poking around by your " + assholeDescript(player) + ", and she buries it to the base inside you before you have a chance to formulate a protest.  The sudden intrusion makes your shaft flex, the veins bulging against the surface of the skin as it swells a bit in response.\n\n");
    ...
    if (player.cumQ() <= 2500) {
        outputText("The second that the first streams of salty cream hit her throat, she begins to moan in ecstasy, cupping your balls softly with both hands.  With her lips sealed tight around your shaft, her tongue swirls to and fro inside her mouth, gathering up your thick seed and funneling it into her gullet.  Overcome by gluttonous desire, she draws your shaft into the back of her throat, massaging your " + ballsDescriptLight(player) + " while she drinks them dry.  She continues to suck down each new stream of cum even after her stomach begins to expand, swelling up with your roiling load.  As your explosive climax begins to slow, she begins to massage the sides of your shaft, squeezing out the last few globs of semen.  Her tongue coils around your length one last time, and she sucks hard, slurping up the last drop with great embellishment.\n\n");
        ...
        outputText("She moans deeply the moment that your first thick streams of seed begin to flow into her throat, cupping your balls softly with both hands.  With her lips sealed tight around your shaft, her tongue swirls to and fro inside her mouth, gathering up your thick seed and funneling it into her gullet.  Overcome by gluttonous desire, she draws your shaft into the back of her throat, massaging your " + ballsDescriptLight(player) + " while she drinks them dry.  Even as her stomach begins to swell with the added weight of your seed, she never stops sucking down your thick load.  Your explosive climax continues to flow, and although she puts up a valiant effort to swallow it all, a few thick streams manage to escape from the corners of her mouth and nostrils.  As the lengthy orgasm finally begins to wind down, her tongue coils around your length one last time, and she sucks hard, slurping up the last drop with great embellishment.\n\n");
...
function getRapedByRedHeadFutaKitsune(): void {
    ...
    outputText("\"<i>Finished already?  I thought a big strong " + mf(player, "boy", "girl") + " like you would put up more of a fight.</i>\"\n\n");
    ...
    outputText("Though her impressive endowment isn't quite as fearsome as some of the others in this strange land, that eerie blue fire that encircles it gives you reason to worry.  There's little you can do about it though, wrapped up in her tails as you are, unable to resist her advances.  You squirm a bit when you feel her press up against your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + ", struggling in vain against the tails binding you.  Your fears are assuaged somewhat when you feel the head pushing past your ");
    if (player.vaginas.length > 0) outputText("entrance");
    ...
    if (player.vaginas.length > 0) cuntChange(player, 24, true, true, false);
    else buttChange(player, 24, true, true, false);
    ...
    if (player.gender >= 2) {
        outputText(((player.vaginas[0].vaginalLooseness < VaginaLooseness.GAPING) ? "\"<i>Ah!  Nice and tight, just how I like it!</i>\"  she groans, gripping your waist tightly and beginning to strongly pump her hips." : "\"<i>Hmm...  a little loose for my tastes, but I suppose it'll have to do.  You haven't been slutting it up on minotaurs and tentacle beasts, have you cutie?</i>\"  she says teasingly, giving you a patronizing pinch on the cheek."));
    }
    else {
    ...
    outputText("As she uses your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + " as her own personal fuckhole, her many tails begin to stroke and tease along your naked body, tickling you with their bristly tips and painting trails of fire across your flesh.  You are caught up in shivers each time one of them crosses one of your erogenous zones, the cool flames tickling and tingling across your most sensitive areas.  Thrashing and wriggling underneath her, you belt out a trembling moan, every muscle in your body resonating with pleasure from the odd contrast of the gentle caress of her tails with the rough pistonlike pounding of her cock.\n\n");
    ...
    outputText("You feel yourself being lifted off the ground slightly, a single tail gliding up and down your back.  Your muscles clench up involuntarily as it brushes against you, and it is almost as though you can make out every vein and wrinkle on the brash redhead's cock now, feeling the rapid throb of her heartbeat through it.  Her cock swells with a thick load of seed, and you can feel slick hot jets of semen flooding your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + ".  Your insides milk her swollen member of every ounce of futa-jism she has, her hips pumping continuously and sloppily pounding into you like a horny teenager.  Her voluminous load is so immense that you can feel your abdomen begin to swell as pressurized streams of her seed begin to escape, spilling out onto the ground.\n\n");
    outputText("When she finally calms down and pulls out with a satisfied sigh, your " + ((player.gender >= 2) ? vaginaDescript(player) : assholeDescript(player)) + " lies agape, a small river of seed slowly pouring out.  Your stomach has swollen to the size of a watermelon, jiggling heavily with the weight of her incredible load of futa-cum.  Her spent cock hangs limply between her thighs, a mixture of cum and feminine juices dripping from the tip.\n\n");
...
// [WIN FIGHT]
// Damage victory
export function defeatTheKitsunes(display: boolean = true): void {
    if (display) {
        clearOutput();
        kitsuneSprite();
        if (monster.HP < 1) {
    ...
    if (player.cocks.length > 0) button = kitsuneButton(button, "FuckHerVag", fuckAKitsuneVaginally);
    if (player.cocks.cockThatFits(144) >= 0)
    ...
    if (monster.hairColor == "black") {
        ...
        if (player.cocks.cockThatFits(108) >= 0)
    ...
    if (monster.hairColor == "red") {
        ...
        if (flags[kFLAGS.redheadIsFuta] == 0 && player.cocks.length > 0)
        ...
        if (player.vaginas.length > 0 && flags[kFLAGS.redheadIsFuta] > 0)
            button = kitsuneButton(button, "RideHerCock", rideDatRedheadKitsuneCockIntoTheSkyDiamonds);
        if (flags[kFLAGS.redheadIsFuta] > 0 && player.vaginas.length > 0 && player.breastRows.biggestTitSize() >= 4 && player.armorName == "lusty maiden's armor")
...
function kitsuneButton(button: number, nam: string, func: () => void): number {
...
function putItInAKitsunesAssWin(): void {
    clearOutput();
    kitsuneSprite();
    const x: number = player.cocks.cockThatFits(144);
    ...
    outputText("Giving her a firm exploratory slap, you chuckle giddily as the two quivering cheeks shake and jiggle like gelatin for what feels like a full minute before coming to rest.  The kitsune gives a cute cry of arousal, looking back at you and smirking gently, resting her cheek on her arms now.  Far from resistant to your advances, she seems almost delighted when you fish out your " + cockDescript(game.player, x) + " and lay it across her rounded rump, raising her tails out of the way automatically.\n\n");

    outputText("\"<i>Oh, no, such a big, strong cock...  what </i>ever<i> will I do?</i>\"  she says with a wry grin, beginning to slide her plump cheeks up and down your pole, one of her tails coiling down to gently wrap around the " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ".\n\n");
...
function tribbingWithAKitsune(): void {
    ...
    outputText("\"<i>Haha, what do you think you're doing, you silly mortal...</i>\" she says, chuckling as you position yourself over her, settling your [butt] down on her splayed thighs, your " + vaginaDescript(player) + " meshing seamlessly with hers." + ((player.cocks.length > 0) ? sMultiCockDesc(game.player) + " drape" + ((player.cocks.length > 1) ? "s" : "") + " over the plump round cheeks of her prodigious posterior, leaking strands of precum onto her bundle of luxurious tails" + ((monster.hairColor == "red" && flags[kFLAGS.redheadIsFuta] > 0) ? ", while her own member dangles a few inches above her nose." : ".") : "") + "  When your slow, measured grinding resumes, you moan with approval, finding this new position to be much more satisfying.\n\n");
...
function tailJobKitsuneWin(): void {
    ...
    outputText("It is at the height of one such cycle of pleasure and denial that you feel something cool and tingly washing over you, looking down to see the kitsune's lips pursed at " + ((player.balls > 0) ? "the underside of your " + ballsDescriptLight(player) : "the " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " of your " + cockDescript(game.player, 0)) + ", gently blowing pale flames across your groin.\n\n");

    outputText("The intense tingling is all that you needed, hurtling off the edge and into the full force of your climax.  [EachCock] pulses, the tight rings of fur preventing you from achieving your full release even as your muscles spasm in vain." + ((player.balls > 0) ? "  Thick spunk begins to well up in your " + ballsDescriptLight(player) + ", making them swell terribly with backed-up seed." : "") + "  Thankfully she seems to tire of teasing you shortly, and withdraws the offending tails.  ");
    if (player.cumQ() <= 150) {
...
function kitsunesGetBonedBy3PlusTentacles(): void {
    ...
    outputText("As your " + num2Text(player.cocks.tentacleCocks()) + " writhing tentacles twist free of the confines of your " + player.armorName + ", the kitsune's eyes open wide with desire, her jaw hanging slack and her tongue lolling out of her mouth.  A trail of drool slides down her chin as she hungrily gawks at your slithering tendrils, leaning forward to crawl on her hands and knees toward you.\n\n");
...
// Blonde-exclusive
// [Fuck Draft]
// {all " + cockDescript(game.player, 0) + " calls for this scene should prioritize dog cocks; if PC is in Rut, prioritize other animal dicks second, otherwise use the biggest cock.}
function fuckDraftBlond(): void {
    clearOutput();
    kitsuneSprite();
    let x: number = -1;
    let temp: number = 0;
    while (temp < player.cocks.length && x < 0) {
    ...
    outputText("The powerful scent of feminine musk fills the air almost instantly, a viscous puddle spreading between her legs and soaking through the crotch of her robes." + ((player.cocks.dogCocks() > 0) || (player.inRut) ? "  As the potent smell fills your nostrils, your " + cockDescript(game.player, x) + " swells in anticipation, reacting instinctively to the scent of a female in heat.  Lurid fantasies of plugging her sloppy snatch with your " + ((player.cocks.dogCocks() > 0) ? "knot" : cockDescript(game.player, x)) + " and stuffing her belly full of kits rush into your mind unbidden" + ((player.cor < 20) ? ", disturbing you slightly" : "") + "." : "") + "  Her body trembles and quakes with carnal need, pure animalistic desire burning in her eyes.  As the effects of her chemical-induced heat come into full swing, she struggles to maintain some semblance of rational thought, desperately willing herself to crawl away.\n\n");
    ...
    if (player.cocks.dogCocks() > 0 || player.inRut) outputText("The sweet musk of her voluminous fem-jizz fills your mind with a lustful haze, higher-order thought processes gradually shutting down as animalistic mating instinct begins to take over.  ");
    outputText("You slide your " + cockDescript(game.player, x) + " up between the supple mounds of her ass, groaning in pleasure as she begins to glide her hips back and forth along the shaft, cooing softly.  The warm, moist breath radiating off her sodden vagina caresses your ");
    if (player.balls > 0) outputText(ballsDescriptLight(player));
    else outputText(legs(player));
    ...
    outputText("  She raises her hips higher, whimpering a little as she desperately tries to line up your " + cockDescript(game.player, x) + " with her hungry fuckhole.\n\n");

    outputText("\"<i>S-stop t-teasing...</i>\" she says breathlessly, whimpering as she fails to catch your " + cockHead(player, x) + " in her quivering snatch for the third time in a row.  Her hips clumsily rock back, a desperate whine escaping her throat as she tries to calm her shaking body enough to finally impale herself on your ever-hardening rod.");
    if (player.cocks.dogCocks() > 0 || player.inRut) {
        ...
        if (player.cocks.cockArea(x) > 50) outputText(" heedless of whether she can handle it or not.  Thankfully, you needn't worry - her cunt eagerly swallows your cock whole with room to spare, somehow");
    }
    else {
    ...
    if (player.cocks.dogCocks() > 0 || player.inRut) outputText("  Vivid fantasies of stuffing her pussy full of your virile semen race through your mind, unable to think of anything else.  Come whatever may, you know you MUST impregnate her - there's simply no other option.");
    ...
    outputText("Gathering her tails into your arms, you pull them tight to your chest, eliciting a pleasured moan from the lust-maddened kitsune.  She lets out a pained squeal when you tug on them suddenly, using them for additional leverage as you redouble your powerful thrusting.  You can feel every pulse of her fluttering heartbeat through the walls of her vagina, liquid heat caressing and milking your cock as more juices squirt out around your " + ((player.cocks.dogCocks() > 0) ? "knot" : cockDescript(game.player, x)) + ".\n\n");

    outputText("Your pleasure begins to rise to a head, every passionate thrust bringing you closer and closer to your final, incredible release.  In preparation, you drop down over her, placing your hands on top of " + ((player.cor < 33) ? "hers." : "her wrists to pin her down.") + (((player.cocks.dogCocks() > 0) || player.inRut) ? "  You are stricken with the urge to bite her neck, and indulge it, pinching the scruff of her shoulder between your teeth." : "") + "  With a roaring moan, you buck your hips one last time, slamming down against her well-cushioned rear with one final thrust, holding there as the pressure in your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + " finally boils over.  ");
...
function doseAKitsuneWithOviElixirs(): void {
    ...
    outputText("Realizing that if you delay for much longer you really will miss your chance, you hastily remove your finger and reposition yourself between her splayed legs, stroking your [cock biggest] up to full mast.  She moans and arches her back a little as the " + cockHead(player, x) + " presses up against her warm love hole, though the weight of her stomach makes it a little difficult for her.\n\n");

    outputText("As you prepare to plunge yourself into her quivering lips, a pair of golden tails slither up around your throbbing member, one curling around your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " while the other slowly tickles across the underside of the shaft." + ((player.gender >= 2) ? "  It travels slowly up and down your prick, then drifts down toward your " + vaginaDescript(player) + ", teasing your [clit] gently." : "") + "  Her swollen cunt parts around you easily as you delve into her depths, rippling around your member sensually.  The muscular walls stretch around you, a steady trickle of feminine juices spilling out around your " + cockDescript(game.player, x) + ".\n\n");

    outputText(((player.cocks.biggestCockArea() > 50) ? "No matter how much pulsating cock meat you stuff into her hungry pussy, her quaking walls continue to stretch on forever, swallowing your shaft greedily.  " : "") + "Your pelvis gently bounces against hers, sending a ripple through her gravid stomach, and you slowly start to buck your hips.  Leaning down over her, you begin to grope and knead her swollen tits, running your hands over the soft, pliable flesh and squeezing firmly to force out a little milk.\n\n");
    ...
    outputText("As you sink your hands into the soft, squishy flesh of her engorged udders, she reaches up to wrap her arms around your shoulders, pulling you down against her chest.  Her rounded belly juts up underneath you, making it a bit difficult to maintain your balance in this position, but you plant your " + feet(player) + " in the dirt as best you can and redouble the thrusting of your hips.  Running her fingers across the back of your neck, she draws you down to one of her turgid, leaking nipples, moaning softly.\n\n");
    ...
    // Large eggs
    if (rand(3) == 0) {
        itype = ConsumableLib.LARGE_EGGS[rand(ConsumableLib.LARGE_EGGS.length)];
    }
    // Small eggs
    else {
...
function hotdogAnalInKitsuneButtDontLetTailTickleYourNose(): void {
    ...
    outputText("You grip her love handles tightly, sinking your fingertips into the soft, plushy flesh, and eagerly pummel her tight ass, slapping your hips into her bountiful butt cheeks over and over.  Her thighs tense and relax in time with your thrusts, working along with the inner muscles to give your " + cockDescript(game.player, x) + " a sensual massage rivalling that of a master masseuse.  Everything above her waist, however, is utterly relaxed, splayed out into a lewd heap of decadent desire.  The merest hint of a sliver of emerald is visible as her eyes roll into the back of her head, her tongue flopping out of her mouth along with a healthy trail of drool.\n\n");

    outputText("One last slap echoes through the clearing when you slam yourself into her ass for the final time, moaning in ecstasy as your " + cockDescript(game.player, x) + " twitches wildly in climax.  ");
    ...
    if (player.cumQ() <= 150) {
        ...
        outputText("The convulsing muscles of her rectum milk your shaft strongly, heavy rippling sensations squeezing you repeatedly from base to tip as you pour out truly monstrous amounts of seed into her innards, stuffing her with your cloying cum.  A husky groan escapes from her throat as she begins to rise off the ground, the obscene amounts of semen filling her stomach like a balloon as your seed surges through her intestines.  Her hindquarters are raised into the air as she tilts forward on her cum-bloated belly, now filled out to the size of an exercise ball.  At the height of your pleasure, you resume your lustful thrusting, her flesh quivering continuously as you fill her to excess, pressurized jets of semen spurting out around your " + cockDescript(game.player, x) + ".  As the last of your load stuffs her abused asshole, you dig your fingertips into her pliable, cum-streaked flesh and squeeze her ass cheeks together around your shaft.  Her anus clamps down hard around your throbbing rod as you slowly pull back, hungrily milking the last of your load from your urethra.\n\n");
...
function feederTheKitsunes(): void {
    ...
    outputText("She becomes wetter with each passing moment, a new rush of fluid soaking your " + leg(player) + " with each gulp she takes.  Rewarding the girl for her admirable efforts in draining your milk-laden tits, you slide a finger in between her legs and grant her the pleasure she sought for herself earlier, rolling her cherry red clitoris back and forth under your fingertip slowly.  You slip your middle and ring fingers into her entrance, forcing a gasp from her throat that causes her to choke a bit, milk sputtering out and soaking her front.\n\n");
...
function getABJFromAFoxGirl(): void {
    clearOutput();
    kitsuneSprite();
    const x: number = player.cocks.cockThatFits(108);
    ...
    outputText("Stripping out of your " + player.armorName + ", you pull her into a kneeling position and press her back against a tree, presenting your " + cockDescript(game.player, x) + " with a grin.  She ogles it hungrily and licks her lips, eyes wide with desire.  As she reaches up to take it into her hands, you grab her wrists instead and pin them to the trunk of the tree.  You explain that she is to use only her mouth, and that if she does her job properly, <i>then</i> you will satiate her hunger.\n\n");

    outputText("She pouts coyly, pursing her plush, kissable lips.  Your eyes are drawn to them, and it is only now that you notice just how juicy and full-looking they truly are.  As she leans forward to press them to your " + cockHead(player, x) + ", they seem to plump up even more, their warm wetness sending a shiver of pleasure through your groin.\n\n");

    outputText("Grinning up at you, she dips underneath your shaft, lifting it up with her nose, flattening out her tongue around the underside of your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " and running it slowly upward.  Having her wrists restrained doesn't seem to be hampering her ability to pleasure you much at all - it even seems to be turning her on.  She pushes your " + cockDescript(game.player, x) + " up against your stomach using her nose, kissing along the underside longingly and making wet smacking sounds.\n\n");
    ...
    if (player.cocks[x].cockLength > 9) {
        ...
        outputText(".  She hungrily swallows along your length, moaning into your engorged prick as she finally presses her lips to your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " and rolls her eyes back in pleasure, her neck bulging slightly from the presence of your shaft.\n\n");
    }
    else outputText("her lips squeezing sensually around the " + ((player.cocks.hasSheath()) ? "sheath" : "base") + " each time she goes down you.  Moaning intensely, she closes her eyes and rolls your " + cockDescript(game.player, x) + " back and forth in her mouth, swirling her tongue sensually along it.\n\n");
    ...
    outputText((((player.balls > 0) || (player.vaginas.length > 0)) ? "  Trailing them across your sides, she " + ((player.balls > 0) ? "cups your " + ballsDescriptLight(player) + " in her hand, squeezing tightly" + ((player.vaginas.length > 0) ? " and " : "") : "") + ((player.vaginas.length > 0) ? "runs her index finger teasingly along your slit" : "") + "." : ""));

    outputText("  Her throat muscles relax entirely, and she happily submits herself to your brutal face-fucking.\n\n");

    outputText(((player.cocks[x].cockLength > 15) ? "Again and again, you thrust your [hips] forward, mashing your pelvis against her face.  Your " + cockDescript(game.player, x) + " glides down her throat, making her neck bulge outward, and her only response is to moan into it, swallowing hungrily.  She massages your shaft sensually with her lips, using her tongue and throat" : "She massages your shaft sensually with her lips, using her tongue") + " to eagerly draw you inside again each time you pull back.  From the intense moaning that is vibrating through your bucking groin, you judge that she must enjoy being used like this, a thought that only heightens your desire even more.\n\n");

    outputText("You feel a tightness swelling in your " + ((player.balls > 0) ? ballsDescriptLight(player) : "prostate") + " as your pleasure starts to come to a head, running your fingers down through her soft, raven-colored hair.  Her glistening green eyes roll back in her head as she opens her throat to accept your orgasm gleefully, her tongue dancing along your length.  When your climax hits, a thick bulge begins to swell up just behind where her tongue still has a tight grip on your " + ((player.cocks.hasSheath()) ? "sheath" : "base") + ".  It builds until the pressure is too much to hold back, and then releases all at once, the kitsune moaning powerfully into your cock as the delicious, salty cream pours into her throat.\n\n");
...
function rideDatRedheadKitsuneCockIntoTheSkyDiamonds(): void {
    ...
    outputText("Her cock swells within you, sending a pulse of tingling coolness rippling up your torso.  You shiver, " + ((player.breastRows.biggestTitSize() > 3) ? "your [fullChest] jiggling from side to side, " : "") + "unable to prevent a moan from escaping your throat.  In retaliation, you rise up and drop your [hips] against hers a tad harder than previously, using your pelvic muscles to squeeze her cock forcefully.  Again and again you thrust her wide hips against the ground, knocking the wind out of her sails and reducing her once more to a quivering heap.  Six crimson tails thrash wildly beneath her, curling toward your body instinctively as they seek to wrap about whatever they can reach.  The silky coils slither across your " + skin(player) + " as well as her own, sparks crackling in their wake and bringing the two of you to the heights of pleasure.\n\n");

    outputText("The sounds of your pelvises mashing together fills the forest, lewd slapping noises echoing through the trees.  Her engorged dick pulses against your walls, twitching almost constantly as her climax builds toward a marvelous conclusion.  You are close to your limit as well, your vaginal muscles clenching down freely as you ride her throbbing rod with a determined zeal, prepared to milk it of all she has to offer.  The first stream of sticky jism hits your womb like a jolt of ice, making you grimace with surprised pleasure as it resolves into a warm tingle that spreads calm through your convulsing nethers.  Your muscles ripple against her shaft, drawing her load upward and into your waiting womb.  Her output is quite prodigious to say the least, ribbon after ribbon of roiling, foaming cum spurting up into your uterus with an audible sloshing sound.  Your belly jiggles and swells, slowly filling with her tingling seed as it expands to the size of a watermelon, thick cum gushing from your gash" + ((player.vaginas.wetness() == 5) ? " while a viscous spray of femcum soaks her front" : "") + ".\n\n");

    outputText("Her rapid, panting breath slows as her climax begins to wane, two of her tails wrapping almost protectively around your cum-swollen belly.  With one hand resting on top of it, you clumsily shift your weight to dismount from the spent kitsune, sighing with relief as a veritable river of seed begins to rush from your " + vaginaDescript(player) + ", spilling out over the kitsune's front.  Her spent rod hangs limply over one thigh, still twitching ever so slightly.\n\n");
...
function helixZeKitsunes(): void {
    ...
    outputText("Reaching down," + ((player.balls > 0) ? " you lift your " + ballsDescriptLight(player) + " out of the way and reach around her underside," : "you reach around her underside,") + " taking hold of her thick prick and bending it gently toward your " + vaginaDescript(player) + ".  She catches your intention easily enough and slides a tail down to assist, curling the bushy coil around her thick member and guiding it toward your waiting snatch.  It takes a bit of finagling, but a moment or two later, she is moaning in pleasure as the head begins to penetrate you.");
    cuntChange(player, 12, true, true, false);
    outputText("\n\n");
...
function nonFutaRedHeadIsWorstRedheadLapsittingHandjobThingIDontKnow(): void {
    ...
    outputText("She hooks her fingers into your " + player.armorName + " and slowly pulls them away, exposing [eachCock].  " + ((player.balls > 0) ? "She gently cradles your " + sackDescript(player) + " in one palm, " : ((player.gender == 3) ? "She gently cups one hand over your " + vaginaDescript(player) + " and lightly inserts her middle finger, " : "She gently cups the underside of your groin, ")) + "wrapping her other hand around your [cock biggest] and beginning to stroke it slowly.\n\n");
...
// Discovery
export function kitsuneShrine(): void {
    clearOutput();
...
function takeAKitsuneStatue(): void {
    ...
    dynStats("lus", 10);
    Inventory.takeItem(UseableLib.GLDSTAT, Camp.returnToCampUseOneHour);
...
// Use:
export function kitsuneStatue(): void {
