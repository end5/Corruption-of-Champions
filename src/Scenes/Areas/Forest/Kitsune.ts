...
export class Kitsune extends Monster {
    ...
    private kitsuneEntwine(): void {
        outputText("The kitsune closes in on you with a mischievous glint in her eyes.  You raise your guard, keeping your eyes trained on her to ensure that she doesn't try to pull anything.  Suddenly, you feel something coiling around your " + leg(player) + ", and let out a yelp as you are suddenly lifted into the air, entangled in the kitsune's tails!");
        ...
        this.effects.create(StatusAffects.PCTailTangle, 0, 0, 0, 0);
        dynStats("lus", 10 + player.sens / 8);
        combatRoundOver();
    }
    ...
    public kitsuneStruggle(): void {
        ...
        // Success
        if (rand(20) + player.str / 20 + this.effects.getValue1Of(StatusAffects.PCTailTangle) >= 12) {
            ...
            this.effects.remove(StatusAffects.PCTailTangle);
            doAI();
        }
        // Failure - +5-10 LUST
        else {
            ...
            this.effects.addValue(StatusAffects.PCTailTangle, 1, 3);
            combatRoundOver();
        }
    }
    ...
    private illusionKitsuneAttack(): void {
        ...
        if (rand(100) < resist) {
            ...
            this.effects.create(StatusAffects.Illusion, 0, 0, 0, 0);
            spe += 20;
        }
        combatRoundOver();
    }
    ...
    private kitsuneSealAttack(): void {
        ...
        if (select == 0) {
            ...
            player.effects.create(StatusAffects.Sealed, 4, 0, 0, 0);
        }
        else if (select == 1) {
            // Tease:
            ...
            player.effects.create(StatusAffects.Sealed, 4, 1, 0, 0);
        }
        // Spells:
        else if (select == 2) {
            ...
            player.effects.create(StatusAffects.Sealed, 4, 2, 0, 0);
        }
        // Items:
        else if (select == 3) {
            ...
            player.effects.create(StatusAffects.Sealed, 4, 3, 0, 0);
        }
        // Run:
        else if (select == 4) {
            outputText("\"<i>Tsk tsk, leaving so soon?</i>\"  the kitsune says, popping up in front of you suddenly as you attempt to make your escape.  Before you can react, she draws a small circle on your chest with her fingertip, leaving behind a glowing rune made of crackling blue flames.  You try to run the other way, but your " + legs(player) + " won't budge!\n\n\"<i>Sorry baby, you'll just have to stay and play~.</i>\" she says in a singsong tone, appearing in front of you again.  <b>The kitsune's spell prevents your escape!</b>  You'll have to tough it out until the spell wears off.");
            player.effects.create(StatusAffects.Sealed, 4, 4, 0, 0);
        }
        // P.Special:
        else if (select == 5) {
            ...
            player.effects.create(StatusAffects.Sealed, 4, 5, 0, 0);
        }
        // M.Special:
        else {
            ...
            player.effects.create(StatusAffects.Sealed, 4, 6, 0, 0);
        }
        if (resist >= rand(100)) {
            ...
            player.effects.remove(StatusAffects.Sealed);
        }
        combatRoundOver();
    }
    ...
    private kitSuneTeases(): void {
        ...
        dynStats("lus", 5 + player.sens / 7);
        combatRoundOver();
    }

    protected performCombatAction(): void {
        const moves: any[] = [foxFireAttack, foxFireAttack, kitSuneTeases, kitSuneTeases];
        if (player.effects.findByType(StatusAffects.Sealed) < 0) moves.push(kitsuneSealAttack);
        if (player.effects.findByType(StatusAffects.Sealed) < 0) moves.push(kitsuneSealAttack);
        if (this.effects.findByType(StatusAffects.PCTailTangle) < 0) moves.push(kitsuneEntwine);
        if (this.effects.findByType(StatusAffects.Illusion) < 0) moves.push(illusionKitsuneAttack);
    ...
    public constructor(hairColor: string) {
        ...
        if (hairColor == "red" && game.flags[kFLAGS.redheadIsFuta] == 1) {
            this.cocks.createCock(rand(13) + 14, 1.5 + rand(20) / 2, CockTypesEnum.HUMAN);
            this.balls = 2;
