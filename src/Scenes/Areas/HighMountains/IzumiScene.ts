...
export function heightDesc(doName: boolean = true): string {
...
export function HeightDesc(): string {
...
export function actsLikeACow(): boolean {
...
export function enterAfterMet(): void {
    clearOutput();
...
function abortabortleavethefukkencave(): void {
    clearOutput();
...
function enterFirstTimePartII(): void {
    clearOutput();
...
const SMOKE_SPEED_REDUCE: number = 1.1;
const SMOKE_SENS_BOOST: number = 1.1;
const SMOKE_LIBIDO_BOOST: number = 1.1;
const SMOKE_DURATION: number = 24;

// Applies the smokeeffect to a player, based on if they used the pipe directly or not
function smokeEffect(smokedPipe: boolean): void {
    let deltaSpd: number = player.spe - (player.spe * SMOKE_SPEED_REDUCE);
    let deltaSns: number = (player.sens * SMOKE_SENS_BOOST) - player.sens;
    let deltaLib: number = (player.lib * SMOKE_LIBIDO_BOOST) - player.lib;
    let lustMod: number = 7;
...
export function updateSmokeDuration(hours: number): void {
...
function smokeEffectWearsOff(): void {
    const affectIndex: number = player.effects.findByType(StatusAffects.IzumisPipeSmoke);
...
function dontSmokeThePipe(): void {
    clearOutput();
 ...
 * FIGHT SHIT
 */
function fightTheFuckhugeOni(): void {
    hideUpDown();
 ...
 * SURRENDER SCENES
 */
function fuckhugeOniGetsWhatSheWants(): void {
    hideUpDown();
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.biggestCockLength() <= 6) surrenderSmallCock();
        else if (player.cocks.biggestCockLength() <= 14) surrenderMediumCock();
...
function surrenderSmallCock(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 1 && player.breastRows.biggestTitSize() < 3) outputText(", your [chest] dwarfed by the larger womans own pair.");
    else if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.biggestTitSize() < 5) outputText(", your [chest] moulding themselves against the underside of the Oni’s larger pair.");
    else if (player.breastRows.biggestTitSize() >= 5 && player.breastRows.biggestTitSize() <= 8) outputText(", your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh between your bodies.");
...
function surrenderMediumCock(): void {
...
function surrenderMediumCockObey(): void {
...
function surrenderMediumCockRefuse(): void {
...
// Urtadicks itt
function surrenderLargeCock(): void {
...
function surrenderOhGodTheFemaleSurrenderContentIsFuckingHugeSendHelp(): void {
    ...
    if (flags[kFLAGS.IZUMI_TIMES_SUBMITTED] == 0) {
        ...
        // Big knockers!
        if (player.breastRows.biggestTitSize() >= 15) {
    ...
    if (flags[kFLAGS.PC_FETISH] >= 1) {
        ...
        if (player.cocks.length > 0) outputText(", your [cock] slowly thickening under her scrutiny");
...
function surrenderFemaleNonExhibitionVariant(): void {
    clearOutput();
    ...
    // C-Cup+
        ...
        if (player.looseness() <= VaginaLooseness.NORMAL) outputText("  She takes it slowly, pushing into you with agonizing care so as not to cause you any distress.  Inch by inch, centimeter by centimeter, she slides her finger into your pussy, finally bottoming out down to the knuckle.  You let out a little gasp of surprise and alarm as she gives her finger a sudden quick, tickling twitch, as though tickling a pet under its chin, then grins savagely at your reaction.\n\n");
        else if (player.looseness() >= VaginaLooseness.GAPING) outputText("  Izumi’s finger slips between your cavernous walls with ease, quickly bottoming out inside you.  She lets out an amused grunt of surprise at the ease with which she’s able to fit her finger inside you.\n\n");
...
function surrenderFemaleExhibitionVariant(): void {
    clearOutput();
...
function surrenderFemaleLookitYouTheCenterOfAttentionYouSlut(): void {
 ...
 * LOSS SCENES
 */
export function fuckedUpByAFuckhugeOni(titLoss: boolean = false): void {
    flags[kFLAGS.IZUMI_LAST_ENCOUNTER] = 1;
    flags[kFLAGS.IZUMI_SEEN_PC_GENDER] = player.gender;
...
function lossSceneStay(): void {
    ...
    if (player.cocks.length > 0) outputText("“<i>Or maybe,</i>”  She whispers seductively into your ear, “<i>You wanna bury that little Oni of yours in </i>me<i>.  Hmmmm?</i>”  You shudder as you feel her hand snake into your underwear and run sinuously across your crotch, readying you for the ordeal ahead...\n\n");
}
...
export function deathBySnuSnuIMeanGiantOniTits(): void {
    ...
    if (player.tallness <= 60) {
        ...
        if (player.cocks.biggestCockLength() <= 5) {
            ...
            if (player.cocks.length > 1) outputText(" one of your");
    ...
    if (player.cocks.length > 1) outputText("s relent");
...
function cowedIntoPuttingItInIntoHerGiantOniButt(): void {
    lossSceneStay();
    ...
    if (player.cocks.length > 1) outputText(" [multicock]");
...
function fuckhugeOniWantsYourBabiesOrSomeShit(): void {
    lossSceneStay();
...
function noDickNoVagNoService(): void {
    lossSceneStay();
    flags[kFLAGS.IZUMI_SEEN_PC_GENDER] = player.gender;
...
export function touchThatFluffyHorn(): void {
    ...
    // Super awesome Gedan edit mode!
        ...
        if (player.spe >= 90) {
            outputText("her arm lunges out in an uppercutting grab, but your reflexes are faster, managing to dodge out of the way at the last moment.  Sensing an opportunity with the Oni suddenly off-balance, you make for a grab of your own - reaching out for the bright red curve of Izumi’s horn.\n\n");
        }
        ...
        else if (player.lib >= 40 && (player.breastRows.biggestTitSize() >= 2 || player.cocks.biggestCockLength() >= 5)) {
            ...
            if (player.breastRows.biggestTitSize() > 2) outputText("maybe giving her face a little personal time nestled between your [chest] will finally distract the Oni enough.  You reach out for the bright red curve of Izumi’s horn, ");
            ...
            outputText("You lash out, swinging at Izumi’s face, but you can’t concentrate while being held off the floor by your neck!  In desperation, you make a grab for the one thing you can think of to try steadying yourself to brace for a strike - the bright red curve of Izumi’s horn.\n\n");
        }
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.tentacleCocks() >= 8) addButton(5, "Tentacruel", tentacruelRevengeFux);
    }
}

// Pfft, nofux
function letGoAndLeaveYouSillyFuck(): void {
    clearOutput();

    outputText("Against the wishes of your libido, you decide to let Izumi off the hook.  You release her horn, soliciting a relieved gasp from Izumi, then take a step back.  The Oni woman straightens hastily and shoots you a grateful look before scurrying back into her tent, emerging a few moments later with an oversized fistful of gems.\n\n");
    ...
    outputText("You stow the gems safely away, reassuring Izumi that you have absolutely no intent of suddenly leaping on her and grabbing her horn again, then turn to make your way out of the cave.  Just as you pass the rock, however, you hear Izumi calling out from behind you.  You turn to see her hovering in the cave mouth looking uncharacteristically nervous.\n\n");
...
// Kinda friendly fux
function letGoAndFuck(): void {
    clearOutput();

    outputText("You decide to let Izumi off the hook - up to a point.  You release her horn, soliciting a relieved gasp from Izumi, then take a step back.  The Oni woman straightens hastily and shoots you a grateful look before turning back to her tent.\n\n");
    ...
    if (player.breastRows.biggestTitSize() >= 1 && player.breastRows.biggestTitSize() < 3) outputText(" smothering your [chest] with her own, much larger, pair.\n\n");
    else if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.biggestTitSize() < 5) outputText(" your [chest] moulding themselves against the Oni’s larger pair.\n\n");
    else if (player.breastRows.biggestTitSize() >= 5 && player.breastRows.biggestTitSize() <= 8) outputText(" your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh.\n\n");
    else if (player.breastRows.biggestTitSize() > 8) outputText(" your [chest] dominating the Oni’s own in the battle for space between the two of you.\n\n");
    ...
    if (player.breastRows.biggestTitSize() > 0) outputText(" against your own");
    ...
    outputText("“<i>Haha, hahh,</i>” Izumi mutters, sounding almost regretful, “<i>Ah, geez. I really came, huh... and from you playing with my horn, too...</i>” she laments.  She covers her eyes with one hand and grins ruefully, her face still reddened from her recent orgasm.  “<i>That’s so embarrassing...</i>”\n\n");
...
function gonnaGetMeSomeRevengeButtsexin(): void {
    clearOutput();

    outputText("You know <i>exactly</i> how you want to get your revenge on the uppity giantess.  Firstly, you order her to disrobe; she looks like she’s considering a refusal, but a quick squeeze to the horn ensures she acquiesces, blushing furiously.\n\n");
    ...
    outputText("Finally, you lean in close to her ear, running your fingers up and down her horn, causing her to whimper and moan as she struggles to remain in her assigned position, and then you make her the offer: If she swears to do everything you want, you’ll let go of the horn - just this once.  But if she disobeys or makes any kind of move against you? You’ll torture that horn for hours, until she’s nothing but a snivelling, mind-broken wreck, wallowing in the cooling proof of her own shameless orgasms.\n\n");
    ...
    // Small cocks
    if (player.cocks.biggestCockLength() <= 4) {
    ...
    // Regular dicks
    else if (player.cocks.biggestCockLength() > 4 && player.cocks.biggestCockLength() <= 12) {
...
// Huehue, Tentacruel!
function tentacruelRevengeFux(): void {
    clearOutput();

    outputText("You release your grip on Izumi’s horn, but only to reach down to remove your [armor].  As you open your clothing and unveil yourself, your [multicockdescriptlight] arch themselves like some kind of deadly snakes preparing to strike.  Izumi slips backwards in surprise, falling onto her rump.\n\n");
    ...
    if (player.cocks.tentacleCocks() >= 10) outputText("  You decide to test the theory, snaking another prehensile appendage into each of her open, outstretched palms.  Sure enough, after a few fumbling attempts, Izumi manages to grasp the concept of a handjob and awkwardly starts to jerk them off.");
...
// Facefuck
function sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt(): void {
    clearOutput();

    outputText("An evil idea begins to form inside your head.  You give Izumi’s horn a quick yank, causing her to yelp out loud, flinching massively.\n\n");
    ...
    outputText("You smile, and on a whim, you decide to start stroking Izumi’s horn in the same way, matching stroke for stroke.  She flinches again and her hands reach up to grip weakly at your wrist.  “<i>Hands down!</i>” you bark, and they fall away instantly to rest on her knees, her hands clenching and unclenching repeatedly as she grits her teeth against the feeling of you rubbing at her horn.\n\n");
    ...
    const cockIndex: number = player.cocks.biggestCockIndex();
    ...
    outputText("Satisfied, you decide it’s time to move onto phase three.  Yanking hard on Izumi’s horn, you thrust your [hips] forwards at the same time, forcing your cock into her mouth.  She lets out a garbled yell of surprise as you slam into her throat, although you’re not sure if she’s more upset about the treatment of her mouth or her horn.  Regardless, it’s all the same to you; nothing more than a pleasant vibration....\n\n");

    outputText("“<i>Suck it!</i>” you command, as Izumi stares up at you with a pleading expression, tears beginning to form in the corners of her eyes due to lack of oxygen - or possibly just the rough treatment of her hyper-sensitive horn.  She lets out another whimper as you squeeze it, showing her what happens when she’s too slow, then she finally begins to work you over in earnest.\n\n");

    outputText("Her mouth latches onto you like a vacuum, sucking on your [cockHead biggest] so strongly it makes your knees weak, her tongue performing bizarre oral gymnastics around your cock.  You can only grunt in surprised appreciation; whatever else you might think about her, Izumi is <i>good</i> at this.  Or maybe she’s just desperate, trying to force you to cum as quickly as possible so that you’ll release her precious horn?  That seems distinctly possible, so you decide to take steps to counter it.\n\n");

    outputText("Yanking forwards on Izumi’s horn again, you pull her bodily down your length until you’re buried to the [sheath], swallowed up completely into Izumi’s tingling-hot mouth, her nose pressed up against your stomach.  She lets out another strangled, slightly frustrated cry, staring up at you in confusion.\n\n");

    outputText("You order her to stop, and you can see the confusion bloom in her eyes.\n\n");

    outputText("Izumi’s oral assault on your cock falters, awaiting your next move.  You simply stand there for a while, enjoying the feeling of having the brash Oni woman completely at your mercy...  and of course, forcing her to think about her situation as well.  How helpless she is, the thick meat filling her throat, her nose pressed deep in against you, every breath filling her lungs with your thick scent.  Idly, you begin to roll her head this way and that, using the poor Oni’s horn as a sort of joystick as you tilt her unresisting head to look at one side, then the other.\n\n");
    ...
    outputText("“<i>Wha-</i>” she stammers hazily, but you jerk her horn powerfully upwards, causing her face to freeze up in an agonized grimace as you force her down onto her back.  Repositioning yourself, you forcibly insert your cock once again into the warm, welcoming confines of Izumi’s throat.  Unfortunately, you still can’t really get into it from this angle, but then an idea hits you.  Still buried in Izumi’s mouth, you suddenly spin around to rest one leg on either side of her face - and causing Izumi to let out an incensed squeak as your [cock biggest] swirls around inside her mouth.  You lean back, grabbing her horn with a fresh hand before releasing the by now quite awkward grip you had before.\n\n");
    ...
    if (player.cumQ() > 100) {
        ...
        outputText("It goes exactly how you’d hoped it would, and you paint Izumi’s leonine features with your seed, laying thick ropes of jism across her face; lips, eyes, even her nose and horn.  The last couple of jets go a little wild, ending up landing in the tangled mess of her hair, spread out beneath her on the cave floor. You collapse, panting, into a seated position on Izumi’s breasts.  After a few seconds, and with some effort, Izumi manages to raise her head and you snap back to reality as you feel your cock once again enveloped in the heat of her mouth, drawn back inside for her to suck the last few drops of cum from your deflating member.\n\n");
    }
...
function tribThatRockyHornGirl(): void {
    clearOutput();
    ...
    outputText("Regardless, any further experimentation will have to wait, as before you know it your orgasm is upon you.  Clutching at Izumi’s horn, you ram yourself home one final time, almost as though punishing your wanton pussy by slamming it against the hard surface.  Beneath you, Izumi bucks and jerks, letting out a high and surprisingly girlish wail as you orgasm around her horn, your juices splattering down to cover her face, her horn, your increasingly numb-feeling hand....\n\n");
