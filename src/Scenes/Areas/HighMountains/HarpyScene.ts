...
function harpyDamageLoss(): void {
    ...
    if (player.cocks.length > 0 && x >= 0) {
        ...
        if (player.vaginas.length === 0) {
            ...
            // (If breasts)
            if (player.breastRows.biggestTitSize() >= 1) outputText("her petite bosom grinding across your " + allBreastsDescript(player) + ", her nipples tickling at your " + nippleDescription(player, 0) + ", ", false);
        ...
        // [(Herm)
            ...
            // (If breasts)
            if (player.breastRows.biggestTitSize() >= 2) outputText(", causing your " + allBreastsDescript(player) + " to bounce and sway violently", false);
            outputText(".  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold", false);
            // (if breasts)
            if (player.breastRows.biggestTitSize() >= 2) outputText(" her toned chest and pert breasts squishing your " + breastDescriptOfRow(game.player, 0) + " beneath them", false);
            outputText(".\n\n", false);
        }
        // Apply harpy status.
    ...
    // [(Female)
        ...
        // (If breasts)
        if (player.breastRows.biggestTitSize() >= 3) outputText(" and causing your " + allBreastsDescript(player) + " to jiggle, slapping into one another", false);
        outputText(". She leans slowly over you, her gilded lips glistening with saliva as they approached yours", false);
        // {(If breasts)
        if (player.breastRows.biggestTitSize() >= 2) outputText(", her petite chest squashing your " + breastDescriptOfRow(game.player, 0) + " against hers", false);
        outputText(".\n\n", false);
    ...
    if (x >= 0 && player.cocks.length == 1) {
        ...
        else if (player.breastRows.biggestTitSize() >= 2) outputText(" while your " + allBreastsDescript(player) + " are shaking and jiggling around helplessly.", false);
        else outputText(".", false);
    ...
    // [(Female)
        ...
        // {(If breasts)
        if (player.breastRows.biggestTitSize() >= 2) outputText("Their hungry feminine mouths creep beneath the body of the harpy atop you and grab hard onto your " + breastDescriptOfRow(game.player, 0) + ".  They pull them towards their mouths, those gold lips wrapping themselves around your " + nippleDescription(player, 0) + "s to suckle gently.  ", false);
        // {(If lactating)
        if (player.breastRows.biggestLactation() >= 1) outputText("You hear some subtle slurps around the big feathery earmuffs crushing your head, and feel your " + breastDescriptOfRow(game.player, 0) + " begin to drain, sending motherly shivers through your whole being.  The monster-girls drink and drink, those stubby tongues circling each nipple while the drugged golden lips make you tingle sensually.  ", false);
        ...
        // {(If breasts)
        if (player.breastRows.biggestTitSize() >= 2) outputText("her belly squishing your " + allBreastsDescript(player) + " beneath her weight, ", false);
    ...
    // [(Female)
        ...
        // {(If lactating)
        if (player.breastRows.biggestLactation() >= 1) outputText("  Their bellies seem a little pudgier than when they'd started, a result of the endless drinking from your " + nippleDescription(player, 0) + "s.  ", false);
...
function winAndRapeHarpyAnally(): void {
    ...
    if (player.breastRows.biggestTitSize() >= 2) outputText(" Her petite bust squishes against your " + allBreastsDescript(player) + ". Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.", false);
    outputText("\n\n", false);
    ...
    // [(Herms/Shemales)
        ...
        if (player.breastRows.biggestTitSize() >= 2) outputText(", the motions causing your " + allBreastsDescript(player) + " to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust", false);
    ...
    // (if breasts)
    if (player.breastRows.biggestTitSize() >= 2) outputText("her chest mashing against yours as she becomes still, ", false);
...
function WinOnHarpyAndOralRape(): void {
    ...
    // (if boobs)
    if (player.breastRows.biggestTitSize() >= 2) outputText(", but soon she presses her toned, petite chest against your " + allBreastsDescript(player) + ", that stubby tongue tangling around yours lustily.", false);
    ...
    // [(Male, Shemale, Herm, multicock)
        ...
        if (player.breastRows.biggestTitSize() >= 2) {
            ...
            if (player.breastRows.biggestLactation() >= 1) outputText("  Milk dribbles out gently onto the areola, trickling sensually between your cleavage.", false);
        }
        else outputText(".", false);
    ...
    if (player.gender == 2) {
        ...
        // (If breasts)
        if (player.breastRows.biggestTitSize() >= 2) outputText("You grasp onto your " + allBreastsDescript(player) + " violently, kneading and tweaking your " + nippleDescription(player, 0) + " in a horny haze as the harpy's fingers tease around your pussy.  ", false);
