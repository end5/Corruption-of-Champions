...
export function harpyVictoryuuuuu(): void {
    ...
    let eggs: () => void = null;
    if (player.ovipositor.canOvipositSpider() && (player.faceType == FaceType.SNAKE_FANGS || player.faceType == FaceType.SPIDER_FANGS)) eggs = spoidahsLegEggsInHarpeis;
    let anal: () => void = null;
    let pussy: () => void = null;
    let scissor: () => void = null;
    let clitFuck: () => void = null;

    if (player.vaginas.length > 0 && player.clitLength >= 3.5) clitFuck = clitFuckAHarpy;
    if (player.vaginas.length > 0) {
        if (player.isNaga()) outputText("  If you weren't a naga, you could scissor her.");
        else scissor = harpyScissorSurprise;
    }
    if (player.cocks.cockThatFits(monster.vaginalCapacity()) >= 0) pussy = victoryHarpyGetsHerPussyRaped;
    if (player.cocks.cockThatFits(monster.analCapacity()) >= 0) anal = winAndRapeHarpyAnally;
...
function harpyLossLust(): void {
    ...
    outputText("It's not just her claws moving across your body! From behind and from the side, three more ornately plumed ladies have descended and are running their talons curiously across the last trappings holding your gear to your body. When these are undone, your " + player.armorName + " falls to the ground. You cover your " + allBreastsDescript(player) + " and crotch as the foursome leer at you unnervingly with their pitch black eyes and yellow pupils. The sensation is not so bad however, being surrounded by these fair and elegant creatures. When the four spread their wings, they screen your body from the outside world in a palette of colorful plumage. The one that took you down lets a single lock of your " + hairDescription(player) + " fall across her talons admiringly", false);
    // if long hair
    ...
    if (player.vaginas.length > 0 && x < 0) {
        ...
        if (player.skinType == SkinType.PLAIN) outputText(" and quickly alleviating the pain by brushing their wings across your hide", false);
        ...
        if (player.skinType == SkinType.PLAIN) outputText(", always followed by a soothing brush of their feathers", false);
        outputText(". They line the curve of your " + breastDescript(game.player, 0) + ", brush around your aurela, and harden your " + nippleDescription(player, 0) + "s. They trace your inner thigh, and not even your " + feet(player) + " are safe. It does not tickle, but your nerves are lit up like a Christmas tree and a tear wells up in your eyes as you're forced to lie still in the face of such a soft, yet overpowering sensation.\n\n", false);

        outputText("Suddenly, there's a different feeling altogether. You look down and through the feathered fans, spot one of the harpies with her face between your spread legs. She looks at you, a sultry expression in her black-yellow eyes as her black, stout tongue runs up and down your " + vaginaDescript(player, 0) + ". You close your eyes and focus on that, instead of the maddening sensation of the rolling waves of talons", false);
        if (player.skinType == SkinType.PLAIN) outputText(" across your reddening skin", false);
        outputText(". Her little black tongue feels divine as it bumps up against your " + clitDescription(player) + ". WHen you let out a small, alleviated coo, the other harpies once again follow suit. The two previously holding your arms, bend down and put their stiff lips on top of your antagonized nipples, rewarding them after having brushed past them so many times.\n\n", false);

        outputText("You feel your " + legs(player) + " being lifted up and out of the way. Feathered arms curl around either side of your thighs to keep them in place as the harpy teasing and suckling your " + clitDescription(player) + " gives the last harpy enough room to scoot in below her. A teasing of soft harpy hair feathers against your " + buttDescription(player) + " is the only warning you receive before you feel the wet warmth of that final black tongue pressing against your " + assholeDescript(player) + ".\n\n", false);
    ...
    // (Reminds me of what my own Succubus does in my novel)
        ...
        outputText(cockDescript(game.player, x) + " whenever the one on top of you doesn't take you up to the hilt.\n\n", false);
...
function harpyDamageLoss(): void {
    ...
    if (player.cocks.length > 0 && x >= 0) {
        ...
        // [(Herm)
            ...
            // (If breasts)
            if (player.breasts.biggestTitSize() >= 2) outputText(", causing your " + allBreastsDescript(player) + " to bounce and sway violently", false);
            outputText(".  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold", false);
            // (if breasts)
            if (player.breasts.biggestTitSize() >= 2) outputText(" her toned chest and pert breasts squishing your " + breastDescript(game.player, 0) + " beneath them", false);
            outputText(".\n\n", false);
        }
        // Apply harpy status.
    ...
    if (x >= 0 && player.cocks.length == 1) {
        ...
        if (player.balls >= 0) {
            outputText(".  Your " + ballsDescriptLight(player) + " bounce with such force as to fetch the cock-speared harpy a stinging slap on the rear with every brutal, mechanical fuck", false);
            // {(If herm)
            if (player.gender == 3) outputText(" before swinging back to spanking your " + vaginaDescript(player, 0) + " wetly.", false);
            else outputText(".", false);
        }
        else if (player.breasts.biggestTitSize() >= 2) outputText(" while your " + allBreastsDescript(player) + " are shaking and jiggling around helplessly.", false);
        else outputText(".", false);
    ...
    // [(Female)
        ...
        // {(If breasts)
        if (player.breasts.biggestTitSize() >= 2) outputText("Their hungry feminine mouths creep beneath the body of the harpy atop you and grab hard onto your " + breastDescript(game.player, 0) + ".  They pull them towards their mouths, those gold lips wrapping themselves around your " + nippleDescription(player, 0) + "s to suckle gently.  ", false);
        // {(If lactating)
        if (player.breasts.biggestLactation() >= 1) outputText("You hear some subtle slurps around the big feathery earmuffs crushing your head, and feel your " + breastDescript(game.player, 0) + " begin to drain, sending motherly shivers through your whole being.  The monster-girls drink and drink, those stubby tongues circling each nipple while the drugged golden lips make you tingle sensually.  ", false);
        outputText("Their hands find their way to your " + vaginaDescript(player, 0) + ", and two sets of fingers start to explore your lush, velvety depths.  They seem intent on making their captive quiver and scream in ecstasy beneath the stifling birdy bottom. The riding harpy leans down, ", false);
        // {(If breasts)
        if (player.breasts.biggestTitSize() >= 2) outputText("her belly squishing your " + allBreastsDescript(player) + " beneath her weight, ", false);
        outputText("and her drug-laced mouth joins the two intruding fingers in exploring your moist, feminine sex. Even as your " + vaginaDescript(player, 0) + " lets out its first wave of sticky girl-cum, your body tightens from the strength of the orgasm.  The harpy's stubby tongue drives into you, licking up your hot, sticky fluids.  Those tingly drugs even slip past your nether lips, seeming to warm you from the inside out while her insane birdy tongue thrashes around in your " + vaginaDescript(player, 0) + ". The other bird-girls' practiced fingers don't let up in the slightest.\n\n", false);
    }
    // [(Male, Herm, Shemale)
    if (x >= 0 && player.cocks.length > 0) {
        ...
        // {(If herm)
        if (player.gender == 3) outputText(", and the number of times your " + vaginaDescript(player, 0) + " has dripped your sweet feminine cum over your thighs", false);
        outputText(". Every part of your being screams in pain from the hours of relentless hammering by the harpies' immense, wobbling rears; you think a few bones might even have broken.  Yet every time you came close to blacking out from the pain, the three harpy girls had squawk and trill loudly, abandoning their actions to all lick and slurp at your " + multiCockDescriptLight(game.player) + ".  Their drug-laced kisses pull you back from the brink and allow them to continue their relentless pounding!", false);
    }
    // [(Female)
    else {
        ...
        // {(If lactating)
        if (player.breasts.biggestLactation() >= 1) outputText("  Their bellies seem a little pudgier than when they'd started, a result of the endless drinking from your " + nippleDescription(player, 0) + "s.  ", false);
    ...
    if (x >= 0 && player.cocks.length > 0) {
        ...
        if (player.cocks.length > 1) outputText("each of ", false);
        outputText("your " + multiCockDescriptLight(game.player) + " shakes and quivers under the attentions of the merciless bird-girls, releasing a powerful blast of hot, sticky seed, coating the three cruel harpies.  They're still rubbing and probing their pussies with delicate fingers while you cum. You hear some angry squawking and a few syllables of recognizable language before you finally give in to unconsciousness, fading into a warm dream of soft, velvet thighs...\n\n", false);
    }

    outputText("After a few hours, when you wake, every muscle in your body is aching as though you've just run a marathon. Looking down at your " + legs(player) + " in a weary haze, you see signs that even after you'd blacked out, your body had continued to be abused by the three lust-crazed harpies.", false);
    fatigue(20);
    cleanupAfterCombat();
    player.orgasm();
...
function victoryHarpyGetsHerPussyRaped(): void {
    ...
    outputText("", true);
    outputText("You step over the now-submissive feathered beauty and grin, your " + hipDescription(player) + " swaying from side to side idly as you eye up the sweet curves of your latest conquest. Stripping off your " + player.armorName + " and tossing it aside carelessly, you waste no time with words, pushing her roughly down to the ground face-down and rubbing your " + multiCockDescriptLight(game.player) + " in anticipation. She raises her magnificently-plumed head and turns it, looking at you pathetically with a glint of fear in her eye, squawking and crooning as if pleading with you. This passes unnoticed to you, however, as the sight of her thick, fluffy ass and meaty thighs has captured your attention FAR more effectively.\n\n", false);

    outputText("Wrapping your arms around those immense, motherly hips, you drag her rump up so that it sways and jiggles in the air, forcing the big-butted bird-girl into the doggy position. Her cheeks burn red with embarrassment, but the way she is jiggling and shaking her blubbery bottom at you tells you she wants it just as much as you do. Finally getting a really good view of it, your eyes widen as you run your hands over every inch of her bouncy rear - it's more like standing behind a horse than a harpy! An ass this fine isn't to be wasted, and so you lean down and bury your " + face(player) + " between those luxurious orbs, planting your mouth firmly over her drooling pussy and tasting her sweet juices, suckling and kissing around her beautifully-formed clit and breathing her intoxicating feminine scent in with enjoyment, letting it fill your lungs as your expert ministrations coax a plethora of moans and subtle squawks from your partner.\n\n", false);

    outputText("But as sumptuous as her enormous rump is, you can't resist taking her right then and there. The scent from her pussy has taken a serious effect on you as, with an almost feral roar, you break away from her drooling velvet pussy and grasp onto those immense hips, which feel as soft as digging your hands into the most luxurious cushions you can imagine. Guiding her sweet, dribbling fuck-hole onto your " + cockDescript(game.player, x) + ", you aim to impale the disheveled harpy with all your might, pulling hard on her waist to bury yourself as deep as you can.\n\n", false);

    // (Added bit for multi-dicks)
    if (player.cocks.length > 1) outputText("As your head creeps past her tight entrance, your " + cockDescript(game.player, y) + " brushes past her hot, clenched anus and a wicked idea forms in your brain. Pulling your " + cockDescript(game.player, x) + " free for the time being, you grasp your two lengths of meat in one hand and smear them enthusiastically against her slick honeypot, coating them in slippery-sweet feminine fluids, before trying a second time. You squeeze her plump rump and force those two hot, throbbing cocks deep inside her holes, letting out a breathless moan which is echoed by the stricken harpy as she tries to adjust to the sudden intrusion.\n\n", false);

    outputText("You slowly slip your " + multiCockDescriptLight(game.player) + " into her hot, clutching depths, inch by inch, seeming to take an eternity as all the sensations of her rough, ribbed pussy ", false);
    if (player.cocks.length > 1) outputText("and viciously-clenching ass ", false);
    ...
    // (If balls)
    if (player.balls > 0) outputText("You eventually bottom out, your " + ballsDescriptLight(player) + " coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your " + sackDescript(player) + " maddeningly.\n\n", false);
    // (If balls-less)
    else outputText("Leaning low over her back, you adjust to drive yourself ever deeper, forcing the whole length of your " + cockDescript(game.player, x) + " into her, the soft down on her gelatinous thighs prickling at your thighs maddeningly.\n\n", false);

    if (player.gender == 1) {
        // (Male, no breasts)
        if (player.breasts.biggestTitSize() < 1) {
            outputText("Taking care not to waste your opportunity with an immense butt by cumming too soon, you slowly start to push and pull her fat, quivering cheeks up and down your " + cockDescript(game.player, x) + ", feeling her rough walls and clutching, gasping convulsions massage your manhood", false);
            if (player.cocks.length > 1) outputText("s", false);
            outputText(" along ", false);
            if (player.cocks.length > 1) outputText("their", false);
        ...
        // [(Male w/ breasts)
            ...
            if (player.cocks.length > 1) outputText("s massage", false);
            else outputText(" massages", false);
            outputText(" your " + cockDescript(game.player, x), false);
            if (player.cocks.length > 1) outputText(" and " + cockDescript(game.player, y), false);
            outputText(" while you knead over your " + allBreastsDescript(player) + " hungrily.  You start to pound her tight, harsh hole roughly, groaning and licking your lips, trying to master the power of the sensations her hot, clenching hole are giving to you. You lean down low over her back, your " + allBreastsDescript(player) + " pressing against the tickly, prickling feathers, as you bite down onto her shoulder.  Your " + hipDescription(player) + " raise and drop mercilessly into that jiggling, jelly-like ass, giving a noticeable bouncing effect from it as the squishing flesh catapults your rear back into the air with every lewd, slapping thrust.\n\n", false);
        }
    }
    // [(Herm)
    else if (player.gender == 3) {
        outputText("Grinning and massaging your conquest's round, soft ass in anticipation, you gently begin to thrust your " + hipDescription(player) + ", moaning loudly as her rough, grasping walls drag over the sensitive flesh of your " + multiCockDescriptLight(game.player) + ". Your " + vaginaDescript(player, 0) + " practically spurts fluids out, dribbling down the inside of your thighs as you pump in and out of the helpless harpy.  You love every second you get to spend in her harsh inner walls.\n\n", false);
    }

    outputText("It seems the exposed girl is enjoying herself just as much.  Every tiny motion of your " + hipDescription(player) + " causes her to writhe and moan, her moans punctuated with birdlike squawks as she pushes her huge egg-bearing hips back at you in sheer, undisguised desire to have you fuck her harder and deeper.\n\n", false);

    outputText("You decide to give her what she so clearly wants and plant both your hands onto her massive bottom, squashing and groping it around your impaling " + multiCockDescriptLight(game.player) + " as you speed up the pace of your thrusts, driving the full length of your " + cockDescript(game.player, x), false);
    if (player.cocks.length > 1) outputText(" and " + cockDescript(game.player, y), false);
    outputText(" into her grinding, overwhelming hole", false);
    if (player.cocks.length > 1) outputText("s", false);
    outputText(".  ", false);
    // {(If balls)
    if (player.balls > 0) outputText("Your " + sackDescript(player) + " slaps wetly into her immense, wobbling thighs.  ", false);
...
function winAndRapeHarpyAnally(): void {
    ...
    if (player.breasts.biggestTitSize() >= 2) outputText(" Her petite bust squishes against your " + allBreastsDescript(player) + ". Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.", false);
    outputText("\n\n", false);

    outputText("Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your " + multiCockDescriptLight(game.player) + ".  Blood flows through your " + cockDescript(game.player, x) + " as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped " + cockDescript(game.player, x) + ", your slowly-pumping " + hipDescription(player) + " smearing her own sticky sexual juices over those downy butt cheeks.\n\n", false);

    outputText("The bird-girl's attempts to escape only turn you on more, and you grab HARD onto those gelatinous orbs, guiding your " + cockDescript(game.player, x) + " into her gloriously tight and hot rear entrance; it feels harsh and rough, practically scratching every inch of your member as it's driven hard against her resisting inner walls.  She's far tighter and hotter than any human girl could possibly be! Your " + cockDescript(game.player, x) + " pushes so viciously and painfully into her clenching tight depths that the harpy squeals and cries out, trying to fly away from you again.  However, she only succeeds in clenching and waving her enormous ass, sending viciously sharp sensations through your blood-engorged dick. Inch by inch you force your " + cockDescript(game.player, x) + " into her, her movements getting more frantic as you plumb her most lustful and forbidden hole.  Finally, you bottom out and stop for breath, her powerful, jiggling thighs pressing against yours.", false);
    // (if balls)
    if (player.balls > 0) outputText("  Your " + ballsDescriptLight(player) + " come to rest against her thick, wiggly rear.", false);
    outputText("\n\n", false);
    ...
    // [(Males, no breasts)
    if (player.breasts.biggestTitSize() < 1 && !player.vaginas.length > 0) {
    ...
    // [(Herms/Shemales)
        ...
        if (player.breasts.biggestTitSize() >= 2) outputText(", the motions causing your " + allBreastsDescript(player) + " to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust", false);
        ...
        // (If herm)
        if (player.gender == 3) {
            outputText("  Her struggles are driving you wild, and your " + vaginaDescript(player, 0) + " is seeping fluids, growing more and more aroused as she bounces and slamming that immense, rippling rear into you, the force squishing your moist lips together, releasing yet more sticky juices and adding an extra little tone of pleasure.", false);
            // {(If balls)
            if (player.balls > 0) outputText("  She even makes your " + ballsDescript(player) + " slap against your " + vaginaDescript(player, 0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.", false);
        }
        outputText("\n\n", false);
    }
    outputText("With an ear-splitting squawk, the impaled harpy's drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting " + hipDescription(player) + ", her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ", false);
    // (if breasts)
    if (player.breasts.biggestTitSize() >= 2) outputText("her chest mashing against yours as she becomes still, ", false);
    outputText("submitting to your will. Growling, you start to fuck her in earnest and grab a handful of her squishy rear in each hand, pounding her up and down on your " + cockDescript(game.player, x) + ". Though the harpy has collapsed, her body is still panicking, forcing her rough inner passage to clutch and massage your shaft, feeling harsh and grating, but oh-so wonderful! Pathetic squawks and trills of pain escape her golden lips as you roughly pound her tender ass, and her hips pound back against you, ass bouncing on your lap with a dull SLAP every time, which spatters pre-cum across your legs as well as her enormous thighs.\n\n", false);

    outputText("Throwing your head back and crying out in sheer lust-consumed ecstasy, you hilt your " + cockDescript(game.player, x) + " deep inside her thick, tight anus, releasing a flood of hot, sticky seed into her butt. The sudden surge of cum into her rough depths causes the exhausted harpy to tense up once more, and she releases a second, more pathetic wave of her fluids, a little less than last time. Her creamy feminine cum leaks out over your thighs as you lock with her. Your eyes roll back from the intensity of your orgasm, and you bite down on your bottom lip; for what seems like weeks you cum into her plump rump, her hands pressed on your chest as if trying to push you away. Eventually, your flow subsides, and the harpy collapses on top of you, sound asleep.\n\n", false);
...
function WinOnHarpyAndOralRape(): void {
    ...
    // (if pussy)
    if (player.vaginas.length > 0) {
        outputText(", fluids seeping from your plush " + vaginaDescript(player, 0) + " all the while", false);
    }
    // (if cock)
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.length > 1) outputText("each of ", false);
        outputText("your " + multiCockDescriptLight(game.player) + " jumps to attention, throbbing and begging to be touched", false);
    }
    ...
    // (if boobs)
    if (player.breasts.biggestTitSize() >= 2) outputText(", but soon she presses her toned, petite chest against your " + allBreastsDescript(player) + ", that stubby tongue tangling around yours lustily.", false);
    ...
    if (player.cocks.length == 1) {
        outputText("Breaking the kiss, your lips tingle sharply as you realize her golden lipstick is drugged! The heat flows to your groin, your " + multiCockDescriptLight(game.player) + " swelling noticeably and growing even more sensitive.  Even the slightest breeze feels like a lover's hands passing across the tender flesh. Almost apologetically, the harpy leans in to wrap her hands around your " + cockDescript(game.player, 0) + " those drugged lips brushing sensuously across your cock-tip. Her beautifully formed hands give your shaft a squeeze, sending a wave of pleasure through you that brings you to your knees.  You throw both hands out behind you as you lean away from her, pushing your " + hipDescription(player) + " towards her. The submissive bird-girl waggles her huge, jiggling ass behind her, softly guiding the very tip of your " + cockDescript(game.player, 0) + " between those tainted lips, that tingling sensation slipping down your hyper-sensitive member. Her mouth is warm, soft and suckling while her short tongue is flicking frantically across the underside of your shaft, the pleasure being harsh but oh-so intense.", false);
        // (if herm)
        if (player.gender == 3) outputText("  Her expert fingers tease up the sides of your thighs, coming to softly brush across your moist nether lips, her fingers spreading your " + vaginaDescript(player, 0) + " wide before plunging into the warmth within, adding another sweet sensation of pleasure to the amazing orchestra of sensations her lips were giving to you.", false);
        // (If balls)
        if (player.balls > 0) outputText("  Her hands creep down to softly squeeze and massage your " + ballsDescriptLight(player) + ", the harpy's eager head bobbing back and forth along the sweet tasting shaft of your " + cockDescript(game.player, 0) + ", sending waves of powerful, overwhelming pleasure resonating through your body.", false);
        outputText("\n\n", false);
    }
    // [(Male, Shemale, Herm, multicock)
        ...
        if (player.breasts.biggestTitSize() >= 2) {
            outputText(", and your " + allBreastsDescript(player) + " jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your " + nippleDescription(player, 0) + "s perk up and harden.", false);
            if (player.breasts.biggestLactation() >= 1) outputText("  Milk dribbles out gently onto the areola, trickling sensually between your cleavage.", false);
        }
        else outputText(".", false);
        outputText("  Surging down your body, an alien sensation focuses on your crotch, the pinpricks conglomerating on your " + multiCockDescriptLight(game.player) + ", and you fall to your knees, hunched over and quivering from the powerful orgasmic sensations. The big-bottomed harpy creeps forward on her knees; her eyes gaze expressively at your face. One arm reaches out to wrap her sweet hand around your " + cockDescript(game.player, 0) + ". As she pumps it gently, the pleasure is magnified tenfold by the lingering effects of those treacherous golden lips.", false);
        // (If balls)
        if (player.balls > 0) outputText("  She moves that hand to softly squeeze and pet at your " + ballsDescriptLight(player) + ", massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your " + sackDescript(player) + " really ups its production, almost feeling heavier.", false);
        // (If herm)
        if (player.gender == 3) outputText("  Her other perfect hand creeps beneath your tense " + multiCockDescriptLight(game.player) + " to the " + vaginaDescript(player, 0) + " that's nestled between your thighs. At the first touch of her hand, an explosive ecstasy shoots through you, forcing you to tense up and arch your back.  You grit your teeth while puissant waves of orgasmic glee flood through you.", false);
        outputText("\n\n", false);
    }
    // (Female)
    if (player.gender == 2) {
        outputText("Tearing your lips away from hers, you feel a distinct, sharp tingle across your mouth and cheeks as you realized that her shining, gilded lipstick was drugged! The tingle spreads through your body, prickling across your whole body, flowing inexorably in a wave of sensation towards your groin, the ticklish feeling concentrating on your " + vaginaDescript(player, 0) + ". You fall to your knees at the sheer unbridled pleasure shooting up and down your body, the thick hipped harpy crawling towards you meekly, applying a gentle pressure to your shoulder and pressing your back to the ground, trilling and squawking quietly, her perfectly formed hand reaching to your overwhelmed pussy. At the slightest first touch of her hand an explosive ecstasy shoots through you, forcing you to tense up and arch your back, teeth gritted. ", false);
        // (If breasts)
        if (player.breasts.biggestTitSize() >= 2) outputText("You grasp onto your " + allBreastsDescript(player) + " violently, kneading and tweaking your " + nippleDescription(player, 0) + " in a horny haze as the harpy's fingers tease around your pussy.  ", false);
        outputText("She parts your thighs gently, her eyes focused on yours as her hands come together to tease at your " + vaginaDescript(player, 0) + ", gently lowering her magnificently-plumed head to plant those drug-laced lips on your sweet, seeping pussy. Her stubby tongue tickles around your " + clitDescription(player) + " before plunging hungrily into your warm depths. The heightened sensitivity of your " + vaginaDescript(player, 0) + " means that every tiny flick of the bird's tongue coaxes a violent muscular spasm as you struggle to deal with the overwhelming sensations, her expert ministrations keeping you just on the edge of orgasm, but never driving you over the edge.\n\n", false);
    }
    // [(Male, Shemale, Herm, multicock, cont. from last paragraph)
    if (player.cocks.length > 1) {
        outputText("The harpy isn't finished yet; she grasps your " + cockDescript(game.player, 0) + " with one hand and takes your " + cockDescript(game.player, 1) + " in the other, pressing them against one another. Little pinpricks of pleasure course through your groin.  She forces both of those quivering cocks into her aphrodisiac-tainted lips, working them into her well-practiced throat. As the suction mashes them together pleasurably, her tight throat's grip on your dicks forces them to grind together. The feelings are just incredible, especially since her drugged lips are still forcing chemicals into your body, magnifying the intensity of the blissful sensations by the second.\n\n", false);
    }
    ...
    if (player.cocks.length == 1) {
        ...
        // (if herm)
        if (player.gender == 3) outputText(", her other hand doing the exact same to your own " + vaginaDescript(player, 0) + ", digging those perfect fingers deep into the wet, hyper-sensitive flesh, spreading those lips wide", false);
        outputText(".  Her mouth greedily gobbles up inch after inch of your " + cockDescript(game.player, 0) + ", until at last she bottoms out, pressing her nose against your groin, those tainted lips tingling all the way down, increasing the intensity of the pleasure by the second. It takes a surprisingly short time of the harpy bobbing her head, that stubby tongue wetly caressing the sweet shaft of your " + cockDescript(game.player, 0) + ", before you let out a vast, quaking moan, blasting a much larger load than normal into her gentle throat. She doesn't stop though, quite the opposite in fact, as her attentions to your dick became even more aggressive. Her submissive side all but vanishes as her golden lips brush up and down the hot shaft of that " + cockDescript(game.player, 0) + ".  ", false);
        // (If balls)
        if (player.balls > 0 && player.cocks[0].cockLength <= 16) outputText("She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your " + ballsDescriptLight(player) + ".  ", false);
    ...
    if (player.cocks.length > 1) {
        ...
        // (If herm)
        if (player.gender == 3) outputText("  Her fingers never stop their sweet teasing of your " + vaginaDescript(player, 0) + ", using all four fingers on her hand to plumb your velvet depths.", false);
        outputText("  For one last time, you feel the rising intensity of the pleasure, galloping headlong towards an explosive final orgasm.  Reaching down, you plant both your hands on the back of her head and violently thrust your " + hipDescription(player) + " forward to cram the full length of your cocks into her gagging, clenching throat. A final detonation of seed escapes down her esophagus and into her already-swollen belly.  You shiver, biting on your lower lip, as your orgasm gradually subsides and you collapse backwards as your leg muscles give way, wrenching your " + multiCockDescriptLight(game.player) + " away from the breathless bird-girl. Your vision becomes hazy as her beautiful face comes into view above you, descending to softly smooch across your lips, giving a last dose of those drugs into your brain. You black out, catching one final glimpse of her fine, feathered rear before you do.\n\n", false);
        // Apply harpy status.
        SophieScene.luststickApplication(8);
    }
    monster.lust = 98;
...
function clitFuckAHarpy(): void {
    ...
    if (player.vaginas.wetness() < 3) outputText("  Runnels of your lubricant are dripping down your [legs] in streams,");
    else if (player.vaginas.wetness() < 4) outputText("  Streams of lubricant are soaking down your [legs] in waves,");
    ...
    if (player.vaginas.wetness() < 3) outputText("  A fresh gush of fluid washes out of your pussy at the peak of your pleasure even though you're far from a squirter");
    else if (player.vaginas.wetness() < 4) outputText("  A fresh spout of girlspunk washes out of your pussy at the peak of your pleasure");
