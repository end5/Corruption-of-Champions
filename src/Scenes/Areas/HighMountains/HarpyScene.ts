...
export class HarpyScene {
    ...
    private harpyLossLust(): void {
        ...
        if (player.hasVagina() && x < 0) {
            ...
            outputText("You feel your " + legs(player) + " being lifted up and out of the way. Feathered arms curl around either side of your thighs to keep them in place as the harpy teasing and suckling your " + clitDescript() + " gives the last harpy enough room to scoot in below her. A teasing of soft harpy hair feathers against your " + buttDescript() + " is the only warning you receive before you feel the wet warmth of that final black tongue pressing against your " + assholeDescript() + ".\n\n", false);
    ...
    private harpyDamageLoss(): void {
        ...
        if (player.hasCock() && x >= 0) {
            ...
            if (!player.hasVagina()) {
                ...
                if (player.balls > 0) outputText(", craning their necks to slurp and massage your " + ballsDescriptLight() + " eagerly, trying to coax up your cum production as much as they can", false);
                ...
                // (If breasts)
                if (player.biggestTitSize() >= 1) outputText("her petite bosom grinding across your " + allBreastsDescript(player) + ", her nipples tickling at your " + nippleDescript(0) + ", ", false);
            ...
            // [(Herm)
                ...
                // (If balls)
                if (player.balls > 0) outputText("their hands moving up to grope and fondle your " + ballsDescriptLight() + ", massaging them to boost your cum production as much as they can, ", false);
        ...
        if (x >= 0 && player.cockTotal() == 1) {
            ...
            if (player.balls > 0) outputText(", her huge feathery bottom coming to rest on your " + ballsDescriptLight(), false);
            ...
            if (player.balls >= 0) {
                outputText(".  Your " + ballsDescriptLight() + " bounce with such force as to fetch the cock-speared harpy a stinging slap on the rear with every brutal, mechanical fuck", false);
                // {(If herm)
                if (player.gender == 3) outputText(" before swinging back to spanking your " + vaginaDescript(0) + " wetly.", false);
        ...
        // [(Female)
            ...
            // {(If breasts)
            if (player.biggestTitSize() >= 2) outputText("Their hungry feminine mouths creep beneath the body of the harpy atop you and grab hard onto your " + breastDescript(game.player, 0) + ".  They pull them towards their mouths, those gold lips wrapping themselves around your " + nippleDescript(0) + "s to suckle gently.  ", false);
            // {(If lactating)
            ...
            outputText("Their hands find their way to your " + vaginaDescript(0) + ", and two sets of fingers start to explore your lush, velvety depths.  They seem intent on making their captive quiver and scream in ecstasy beneath the stifling birdy bottom. The riding harpy leans down, ", false);
            // {(If breasts)
        ...
        if (x >= 0 && player.hasCock()) {
            ...
            // {(If herm)
            if (player.gender == 3) outputText(", and the number of times your " + vaginaDescript(0) + " has dripped your sweet feminine cum over your thighs", false);
        ...
        // [(Female)
            ...
            // {(If lactating)
            if (player.biggestLactation() >= 1) outputText("  Their bellies seem a little pudgier than when they'd started, a result of the endless drinking from your " + nippleDescript(0) + "s.  ", false);
    ...
    private victoryHarpyGetsHerPussyRaped(): void {
        ...
        // (If balls)
        if (player.balls > 0) outputText("You eventually bottom out, your " + ballsDescriptLight() + " coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your " + sackDescript(player) + " maddeningly.\n\n", false);
        // (If balls-less)
    ...
    private winAndRapeHarpyAnally(): void {
        ...
        outputText("Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your " + multiCockDescriptLight(game.player) + ".  Blood flows through your " + cockDescript(game.player, x) + " as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped " + cockDescript(game.player, x) + ", your slowly-pumping " + hipDescript() + " smearing her own sticky sexual juices over those downy butt cheeks.\n\n", false);
        ...
        // (if balls)
        if (player.balls > 0) outputText("  Your " + ballsDescriptLight() + " come to rest against her thick, wiggly rear.", false);
        outputText("\n\n", false);
        ...
        // [(Herms/Shemales)
            ...
            if (player.gender == 3) {
                ...
                // {(If balls)
                if (player.balls > 0) outputText("  She even makes your " + ballsDescript() + " slap against your " + vaginaDescript(0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.", false);
            }
            outputText("\n\n", false);
        }
        outputText("With an ear-splitting squawk, the impaled harpy's drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting " + hipDescript() + ", her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ", false);
        // (if breasts)
    ...
    private WinOnHarpyAndOralRape(): void {
        ...
        // (if pussy)
        if (player.hasVagina()) {
            outputText(", fluids seeping from your plush " + vaginaDescript(0) + " all the while", false);
        }
        // (if cock)
        if (player.hasCock()) {
            outputText(", ", false);
        ...
        if (player.cockTotal() == 1) {
            ...
            // (If balls)
            if (player.balls > 0) outputText("  Her hands creep down to softly squeeze and massage your " + ballsDescriptLight() + ", the harpy's eager head bobbing back and forth along the sweet tasting shaft of your " + cockDescript(game.player, 0) + ", sending waves of powerful, overwhelming pleasure resonating through your body.", false);
            outputText("\n\n", false);
        }
        // [(Male, Shemale, Herm, multicock)
            ...
            if (player.biggestTitSize() >= 2) {
                outputText(", and your " + allBreastsDescript(player) + " jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your " + nippleDescript(0) + "s perk up and harden.", false);
            ...
            // (If balls)
            if (player.balls > 0) outputText("  She moves that hand to softly squeeze and pet at your " + ballsDescriptLight() + ", massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your " + sackDescript(player) + " really ups its production, almost feeling heavier.", false);
            // (If herm)
            if (player.gender == 3) outputText("  Her other perfect hand creeps beneath your tense " + multiCockDescriptLight(game.player) + " to the " + vaginaDescript(0) + " that's nestled between your thighs. At the first touch of her hand, an explosive ecstasy shoots through you, forcing you to tense up and arch your back.  You grit your teeth while puissant waves of orgasmic glee flood through you.", false);
            outputText("\n\n", false);
        }
        // (Female)
        if (player.gender == 2) {
            ...
            // (If breasts)
            if (player.biggestTitSize() >= 2) outputText("You grasp onto your " + allBreastsDescript(player) + " violently, kneading and tweaking your " + nippleDescript(0) + " in a horny haze as the harpy's fingers tease around your pussy.  ", false);
        ...
        if (player.cockTotal() == 1) {
            ...
            // (if herm)
            if (player.gender == 3) outputText(", her other hand doing the exact same to your own " + vaginaDescript(0) + ", digging those perfect fingers deep into the wet, hyper-sensitive flesh, spreading those lips wide", false);
            ...
            // (If balls)
            if (player.balls > 0 && player.cocks[0].cockLength <= 16) outputText("She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your " + ballsDescriptLight() + ".  ", false);
