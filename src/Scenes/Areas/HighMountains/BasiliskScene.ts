...
export function basiliskGreeting(): void {
    spriteSelect(75);
    outputText("", true);
    // First encounter:
...
// wins
export function defeatBasilisk(): void {
    spriteSelect(75);
    outputText("", true);
    let evil: () => void = null;
    let eggs: () => void = null;
...
function defeatBasiliskAndAnal(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks[x].cockLength <= 13) {
            outputText("Soon your " + hipDescription(player) + " are clapping a staccato rhythm against its warm, muscly butt,", false);
            if (player.balls > 0) outputText(" your " + ballsDescriptLight(player) + " slapping against it,", false);
        ...
        // <(13 inches or less:)
        if (player.cocks[x].cockLength <= 13) outputText("gripping its hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.", false);
        // (More than 13 inches:)
        else outputText("gripping its hips and pushing as much of your huge, bulging cock into it as you can, its stomach bulging as you reach a glorious, skin tingling peak.", false);
...
// Player Defeated:
export function loseToBasilisk(): void {
    spriteSelect(75);
    outputText("", true);
    // Speed 0 loss:
    if (player.spe <= 1) {
...
function basiliskHasVagEggStuff(): void {
    ...
    // (heat or perk:
    if (player.inHeat || player.perks.findByType(PerkLib.Oviposition) >= 0 || player.perks.findByType(PerkLib.BasiliskWomb) >= 0) outputText("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.", false);
    outputText("\n\n", false);
    ...
    outputText("The creature suddenly raps something out in its strange, dry tongue, and you feel something - something deep and red - flinch in your mind.  Suddenly, you are wet, wetter than you've ever been, your pussy slavering so badly you can hear the pitter-patter of your juices hitting the ground beneath you.  The basilisk says something else, more softly this time as it slides in close, its long claws reaching around to clutch your " + buttDescription(player) + ".  With a kind of horror you feel your limbs move involuntarily, your arms reaching around the thing's thin, muscled back, your bottom half slackening until you are supported entirely by the wiry strength of the lizard.  You expose your crotch in complete submission as the basilisk walks forward, and you feel the head of its cock teasingly touch your dripping, treasonous cunt.  As soon as your limbs are where it wants them to be, they lock in position again; you heave at them desperately, but once again your ability to do anything with your own body is simply not there.  You are less of a statue and more of a fuck toy, an extension of the basilisk's lust-maddened will.  Your jailer keeps walking until you feel the rough surface of a boulder against your back, and using this support the basilisk pushes itself straight into your " + vaginaDescript(player, 0) + ".", false);
    ...
    // <(Tight:
    if (player.looseness() < 4) outputText("Though it is not girthy, it is a perfect fit for your tight hole, and its long length coupled with your drawn up position has you panting as it touches your deepest, most sensitive depths.", false);
    // (Loose:
...
export function basiliskBirth(): void {
    spriteSelect(75);
    outputText("\n");
    ...
    if (player.perks.findByType(PerkLib.BasiliskWomb) >= 0) {
        ...
        if (player.cocks.length > 0) outputText(" and [eachCock] starting to leak pre-cum");
        ...
        if (player.cocks.length > 0) outputText(" and a downpour of hermcum");
        outputText(".");
...
function basiliskBadEnd(): void {
    ...
    if (player.gender == 3) outputText("  You moan through your still throat as you feel blood rushing to your groin, your " + multiCockDescriptLight(game.player) + " stiffening and your " + vaginaDescript(player, 0) + " beginning to drip.", false);
    if (player.gender == 1) outputText("  You moan through your still throat as you feel blood rushing to your groin, your " + multiCockDescriptLight(game.player) + " stiffening.", false);
    if (player.gender == 2) outputText("  You moan through your still throat as you feel blood rushing to your groin, your " + vaginaDescript(player, 0) + " beginning to drip.", false);
...
function basiliskAdvantageHarpy(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        outputText(", your sex drive or what you are thinking about; you are a prisoner of lust and you quickly subside under the sexual concussion, hoping eventually the harpies and your own body will stop fucking you.  It does sink in around your second involuntary orgasm, your aching " + cockDescript(game.player, 0) + " spurting ribbon after ribbon of jizz into a harpy's clenching warmth, that the two arriving harpies are not happy with the first one; with your mouth and hands frozen you can't pleasure them with anything but your manhood, which leaves two of them fighting each other for time with your groin whilst a third grinds frustratedly at your face, mainly using your nose to rub at her inner walls and clit, forcing her juices down your nostrils and making you cough raggedly, struggling to breathe.\n\n", false);

        // Single cock:
        if (player.cocks.length == 1) outputText("Their inability to properly satisfy themselves on your frame raises their tempers and frustration to the point where the three of them are physically fighting each other, treading all over you as they scream, bite and tear, their feathers flying all over the place.  Eventually the biggest of the trio drives the other two off, flapping and screeching into the mountains, before throwing herself onto her scratched and battered prize, her eyes lit up and wild.  She fucks you with a gusto born of bloodlust, her petit breasts bouncing up and down as she slams her powerful thighs into your " + hipDescription(player) + ", picking up the pace.  She quickly forces you to another achingly pleasurable peak, your " + cockDescript(game.player, 0) + " spurting more of your seed deep into her, and then just keeps on working you. Your cock seems incapable of going soft.  You feel the involuntary erotic backwash build in your skull again...\n\n", false);
        // Multicock:
        else {
            outputText("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your " + cockDescript(game.player, 0) + " forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + cockDescript(game.player, 1) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can", false);
            if (player.cocks.length >= 3) outputText(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,", false);
    ...
    // Genderless:
        ...
        outputText("The harpy comes to a halt behind you and begins to eagerly run her cold but soft hands over your bottom half, stroking your thighs and squeezing your " + buttDescription(player) + ", as if appraising a piece of meat.  Whilst caressing your neck she runs her hands between your legs and grabs around your groin, searching for something that isn't there.  She lets loose a squawk of pure frustration and wheels around you to glare in your eyes angrily.  The sex-crazed harridan is clearly deeply irritated about finding such a helpless victim only to further discover they lack a cock which she can abuse.  She shifts her eyes to your mouth, but quickly arrives at the same conclusion you've already come to: unable to move your mouth, you can't even be forced to give oral pleasure.  You feel a bizarre sense of triumph over the creature; you stare into space smugly as the harpy paces in front of you, glaring, thwarted but unwilling to give up her prize.  Perhaps eventually she will leave you alone...?\n\n", false);
...
function basiliskAdvantageGoblin(): void {
    ...
    // Unsexed:
        ...
        outputText("\"<i>Get caught with our pants down by a basilisk, did we?</i>\" the goblin purrs.  \"<i>Well, not to worry. I'll take good c- hey, what gives?!</i>\" From behind you, you feel her hands thread their way around your " + legs(player) + " before feeling and slapping all around your featureless groin.  \"<i>This is fucking bullshit!</i>\" she howls.  \"<i>I get a toy that can't stop me all to my lonesome, and it's some colossal jerkoff who thinks it's funny to have no sex!</i>\"  She stomps around to your front and glares at you, simmering with rage.  \"<i>I bet you think you're a real smartass, you bastard... you bitch... you... whatever!  Ooh, I'll fix you!</i>\"  With a look of complete disgust she storms off.  Having faced down this bizarre rant, you allow yourself to feel a tiny bit of relief.  You were worried for a moment there she'd go for your ass... a short distance away, you hear the goblin calling to someone.\n\n", false);
        ...
        outputText("The huge bull-man is not one to look a gift fuck in the mouth.  Without bothering to take you in, bar a long wet sniff of your " + hairDescription(player) + ", he sticks his cock between your ass cheeks.  He grunts as he forces his head past your sphincter, squirting pre-cum into your passage as he does so.  Your body is incapable of clenching instinctively against the invasion, but there is no escaping how huge the dong feels as he impatiently forces your ass open.", false);
    ...
    // Female:
        ...
        outputText("  You feel the goblin crawl on top of you, pulling the dildo upwards so it rubs against your " + clitDescription(player) + " and then with a squealing giggle impales herself on the other end.  Her lustful movements translate through the artificial cock buried in your dripping sex, sending tremors and waves of pleasure to your very core.  A gush of female fluid splatters over your own sex as the goblin works herself right down the shaft connecting you until she is squelching against you wetly.  The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing back and forth over your own.  By now your passage feels as stuffed as it ever has been, crammed totally full of the squishy expanded double-dong.  Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your " + vaginaDescript(player, 0) + ".  Judging by how wonderful it feels rubbing and twisting against your sensitive walls, the aphrodisiac it is leaking into you is definitely having an effect.  You whine at the goblin twists herself and her dildo around, rubbing and riding you so effectively, the involuntary tide of erotic memories building again...\n\n", false);
...
function basiliskAdvantageMinotaur(): void {
    ...
    outputText("Eventually the minotaur's balls swell against your " + buttDescription(player) + " and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ", false);
    if (player.gender == 1) outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + feet(player) + ".", false);
...
function driderPCEggLaysBasilisk(): void {
    ...
    if (player.cocks.length > 0) outputText("wrap tightly around your waiting prick");
...
function layBeeEggsInABasilisk(): void {
    ...
    if (player.wingType == WingType.BEE_LIKE_SMALL || player.wingType == WingType.BEE_LIKE_LARGE) outputText("buzz lightly over to the basilisk's writhing body");
    ...
    if (player.isTaur() || player.gender == 0) outputText("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
        ...
        if (player.cocks.length) outputText("s");
