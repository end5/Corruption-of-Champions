...
export class MinotaurMobScene implements TimeAwareInterface {
...
function runFromMinotaurs(): void {
    ...
    // FAIL:
        ...
        outputText("A furry arm catches hold of your " + leg(player) + " and yanks on it hard enough to slam you into the cliff face.  Disoriented and bleeding, you wobble free, dazed.  It's a fight!", false);
        // go to combat
        fightOTaurs();
    }
}
...
export function minotaurDeFeet(): void {
    ...
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] >= 25 && minotaurNeed()) minotaurGangBadEnd();
...
function nonAddictMinotaurGangBang(): void {
    ...
    if (player.isTaur()) {
        ...
        if (player.tailType == TailType.NONE) outputText("asshole", false);
        else outputText("tailhole", false);
        outputText(".  ", false);
    }
    else if (player.isNaga()) outputText("A sudden, forceful yank stretches out your tail, and a larger member presses between your " + buttDescription(player) + " to prod at your clutching asshole.  ", false);
    ...
    outputText("The pressure on your " + assholeDescript(player) + " suddenly subsides, not because the minotaur is pulling back, but because part of the flare suddenly slipped through the ring of your tightly-stretched hole.  You swoon and try to relax, fighting with your sphincter's natural reaction to squeeze shut against the intruder.  A pleased rumble echoes behind you, and the minotaur penetrating your " + assholeDescript(player) + " pushes hard.  His flare slips inside with an audible, gut-stretching 'pop'.  ", false);
    ...
    outputText("Once free, the beast wipes the mess from the tip of his fat cock on your lips. He pushes you back towards the one violating your " + assholeDescript(player) + ", allowing him to support you.  You groan at the rough treatment while licking at the delicious glaze on your lips.  The potent sperm in your throat and stomach have already shut down any thoughts more complex than 'find cum' and 'fuck'.  The minitaur approaches hesitantly, placing his pre-cum-slicked hardness in your hand to stroke while he whimpers, \"<i>Please big bro, can I fuck her cunt?</i>\"\n\n", false);
    ...
    cuntChange(player, 60, true, false, true);
    ...
    outputText("I can feel it coming, are you ready, Ma?</i>\" asks the beast-man.  You nod enthusiastically, rocking back and forth while the minitaur clutches your " + buttDescription(player) + " and fucks your stationary form.\n\n", false);
...
function loseToMinoMobVeryPregnant(): void {
    ...
    if (player.isNaga() || player.tailType == TailType.DEMONIC || player.tailType == TailType.COW || player.tailType == TailType.SHARK || player.tailType == TailType.LIZARD || player.tailType == TailType.KANGAROO) outputText("  Even your tail reaches up to curl around a third, stroking the thick meat.", false);
    outputText("\n\n", false);
    ...
    outputText("The frantic face-humping comes to a final, explosive end with an eruption of stringy seed falling into your already-full belly.  This beast-man withdraws coolly.  He pulls out quickly enough that the bulk of his cum is added to your frosted face and " + hairDescription(player) + ".  Much of it falls down to soak your " + chestDesc(game.player) + " and huge, pregnant belly.  You look more like a creme-covered doughnut than a person at this point, and you nearly climax from the mental picture of your huge, pregnant form lying in a puddle of minotaur goo while the beast-men use your face as a cum-receptacle.\n\n", false);
...
function analSpearSemiPregMinotaurGangbang(): void {
    ...
    if (player.breastRows.biggestLactation() >= 1.5) outputText(", you're soon expressing drops of milk into his squeezing mitts.", false);
    ...
    outputText("\"<i>Fuck ma, how do you keep your throat so tight and still swallow dick like a pro?</i>\" queries the voice of your current oral fixation, while his heavy fingers slide through your " + hairDescription(player) + ".  You're too busy licking the heavenly shaft, feeling the urethra bulge with each dollop of aphrodisiac it releases into your hungry stomach.  The stud's fingers twist your hair, just barely painfully, drawing you out of your drug-seeking daze long enough to make a gurgling noise of assent.  Smirking, the minotaur hollers, \"<i>Come on, somebody take her ass already.  ", false);
    if (!minotaurAddicted()) outputText("We've got to get mom so high on us that she'll float back to her camp and come crawling back to us tomorrow.", false);
    ...
    // (Lactation, no nipcunts)
        ...
        if (player.breastRows.totalBreasts() == 2) outputText("Your ", false);
    ...
    if (player.breastRows.biggestTitSize() >= 3 && player.breastRows.biggestLactation() >= 1) outputText(", your pregnant, milky tits swaying underneath you while they're suckled", false);
    ...
    if (!player.breastRows.hasFuckableNipples() && player.breastRows.biggestLactation() >= 1) outputText(", and the minitaur's lips disentangle from your " + nippleDescription(player, 0), false);
...
export function victoryMinotaurGang(): void {
    ...
    if (player.breastRows.hasFuckableNipples()) nipFuck = victoryBJNippleFuckMinotaurGang;
    let titFuck: () => void = null;
    if (player.breastRows.biggestTitSize() >= 6) titFuck = victoryMinotaurGangTitFuck;
...
function victoryMinotaurGangTitFuck(): void {
    ...
    if (minotaurAddicted()) outputText("Of course it helps that pleasing them will give you exactly what you want - more of that delicious, wondrous cock-cream.  ", false);
    ...
    outputText("You press your hands into the sides of your " + biggestBreastSizeDescript(player) + " to mash the flesh tightly about the two lucky penises.  The minotaurs moo in delight, immediately slicking your " + skinFurScales(player) + " with another huge wave of pre-cum, and before they can adjust to the tightness of your chest, you begin to drag your weighty love-cushions up... and finally down.  You stroke their completely-enveloped cocks languidly, enjoying the complete control and dominance it's giving you over your herd.  Trembling mightily, they fight their bodies to be still for you.  Such good boys.\n\n", false);
    ...
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] >= 10) {
        ...
        if (minotaurAddicted()) outputText("I'd hate to see you go mad with need around a lesser beast.", false);
        ...
        outputText("You awaken in camp, still a little woozy and naked as the day you were born.  Your " + player.armorName + " lies a few feet away.  Judging by how dizzy you are, it hasn't been long since your mountain tryst.  You wash the dried cum from your " + skinFurScales(player) + " and gear before you get dressed.  Wow... that was nice.", false);
    }
    // (SMALL)
    else {
        ...
        if (!minotaurAddicted()) outputText("Maybe we can get Mom completely hooked.  Then we won't have to fight her?", false);
...
function victoryAllThePenetrationsMinotaurGangBang(): void {
    ...
    if (player.vaginalCapacity() >= 60) outputText("It slips into your " + vaginaDescript(player, 0) + " with ease, nestling into your accommodating opening as if it were a second home.", false);
    else if (player.vaginalCapacity() >= 40) outputText("It slips into your " + vaginaDescript(player, 0) + " ever so slowly, gradually widening your quim until it's able to handle its size.", false);
    else outputText("It stretches your " + vaginaDescript(player, 0) + " with painful force as it penetrates you, forcibly widening your quim until it's able to nestle its sheath against your vulva.", false);
    ...
    outputText("Within seconds a crowd has formed behind you.  Numerous hands squeeze and caress your " + buttDescription(player) + ", a few even daring to press at the tight ring of your " + assholeDescript(player) + ".  They jostle around a bit as they fight over the honor of penetrating your back door, but a winner is selected before you bore of the spectacle.  He grabs your " + buttDescription(player) + " in both hands to aid in his efforts to align his cock with your pucker, squeezing softly.\n\n", false);

    outputText("The beast-man finally hits your anus with his tip and pushes forward.  ", false);
    if (player.analCapacity() < 40) outputText("Your " + assholeDescript(player) + " is so tight that he's forced to push and release over and over.  The painful penetration drags on and on as he widens your hole to accept his monstrous cock.", false);
    else if (player.analCapacity() < 60) outputText("Your " + assholeDescript(player) + " slowly widens to accept the cock that's being deposited into its depths.  You find yourself somewhat grateful that you've been stretched enough to prevent this from being painful.", false);
    else outputText("Your " + assholeDescript(player) + " devours the cock easily.  The only thing even close to a tight fit is the feeling of your pucker being stretched at his very base.", false);
    ...
    if (player.breastRows.hasFuckableNipples() && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] >= 4) outputText("  There's still more you could be doing.", false);
    outputText("\n\n", false);

    // (Nipplefuck PG:)
    if (player.breastRows.hasFuckableNipples() && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] > 4) {
        if (player.breastRows.biggestLactation() >= 1) outputText("Milky lube", false);
        ...
        else if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] >= 5 && player.breastRows.totalBreasts() == 2) outputText("Two studs assemble around ", false);
        else {
            let dicks: number = player.breastRows.totalBreasts();
    ...
    if (player.breastRows.hasFuckableNipples() && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] > 4) outputText("your nipples squirt, ", false);
    ...
    outputText("The one in your ass gets off first, basting your colon in his juices.  Narcotic spooge slops from your " + assholeDescript(player) + " as he overfills the poor rectal opening, but you get more than enough inside you to make you woozy and light-headed - as if your orgasm hadn't already.  Spunk erupts in your " + vaginaDescript(player) + " with equal fervor, packing itself into your womb with ", false);
...
function forceMinitaurToGiveOral(choice: number = 0): void {
    ...
    if (choice == 0) {
        ...
        if (player.isTaur()) {
            ...
            if (player.tailType > TailType.NONE) outputText("and lift your tail ", false);
        ...
        // ELSE
            ...
            outputText("spread your " + vaginaDescript(player) + " wide with your fingers, exposing the glistening pinkness of your womanhood.", false);
        }
        outputText("  Your " + clitDescription(player) + " slowly peeks out from its hood", false);
        ...
        outputText("You begin to undulate your " + hipDescription(player) + " into the minitaur's bestial muzzle, grinding and groaning in joy at the forced cunnilingus.  ", false);
    ...
    // (PENISCOCKSUCKERFUCK)
        ...
        if (player.isTaur()) outputText("flex your " + multiCockDescriptLight(game.player) + " to slap against your belly.", false);
        else outputText("stroke your " + multiCockDescriptLight(game.player) + " with one hand.", false);
        ...
        outputText("The girly 'monster' crawls forward, ", false);
        if (player.isTaur()) outputText("disappearing under your hind-legs as he attends to his newly commanded duty.", false);
        else outputText("looking up at you longingly as he leans in towards " + oMultiCockDesc(game.player) + ".", false);
        outputText("  His big brown eyes meet yours as he opens his muzzle and lets the full length of his tongue flop free.  It's roughly a foot and a half long fully extended, and " + sMultiCockDesc(game.player) + " twitches as you imagine what it must feel like.  Thankfully, he does not keep you waiting long.\n\n", false);
...
function victoryBJNippleFuckMinotaurGang(): void {
    ...
    if (player.breastRows.totalNipples() == 2) outputText("each", false);
    ...
    if (player.breastRows.biggestTitSize() <= 2) outputText("a few inches", false);
    else if (player.breastRows.biggestTitSize() <= 4) outputText("inches", false);
    else if (player.breastRows.biggestTitSize() <= 8) outputText("most of a foot", false);
    else if (player.breastRows.biggestTitSize() <= 14) outputText("over a foot", false);
    else if (player.breastRows.biggestTitSize() <= 30) outputText("nearly two feet", false);
    ...
    if (player.breastRows.biggestLactation() >= 3) outputText(" while you hose sprays of milk around the twin rods", false);
    else if (player.breastRows.biggestLactation() >= 1.5) outputText(" while you dribble milk around the twin rods", false);
    outputText(".\n\n", false);
    ...
    if (!minotaurAddicted()) outputText(" 'meal'", false);
    ...
    outputText(".  You re-dress sensually, giving them as much of a show as you dare, not even bothering to wipe the cum-coating from your " + skinFurScales(player) + " before you finish slipping into your " + player.armorName + " and depart.", false);
    player.orgasm();
...
function minotaurGangBadEnd(): void {
    ...
    if (player.breastRows.hasFuckableNipples()) outputText("slide inside your nipple-cunts and finger your chest pussies in a small approximation of what's happening below.", false);
    else if (player.breastRows.biggestLactation() >= 1) outputText("tug and pull at your drippy nipples, releasing thick flows of creamy milk.", false);
    ...
    outputText(", releasing so much that a squirt of it escapes to run down your " + leg(player) + ".  That was just the first spurt!  The second burst of semen fills every nook and cranny of your " + vaginaDescript(player) + ", your entrance turning to a frothy mess of white goo and slippery lady-spunk.  The pent-up pecker keeps flexing in orgasm, firing jet after jet of narcotic cream into your semen-spurting box, the pillows under you quickly soaking up the excess, addictive jizz.\n\n", false);
...
function disciplineEldestMinotaurSon(): void {
    ...
    outputText("After the slap, you quickly remove the lower portion of your " + player.armorName + ", your son's wandering eyes meets your " + vaginaDescript(player, 0) + ".  He's clearly pleased with what he sees; his equine-like cock twitches with approval and anticipation.  You certainly are not mother of the year; your womanhood glistens with sexual excitement.  Leaning down, you grab his chin with your right and force him to look you in the eyes.\n\n", false);
