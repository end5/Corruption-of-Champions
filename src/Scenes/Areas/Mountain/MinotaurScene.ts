...
export function minoVictoryRapeChoices(): void {
    ...
    else if (player.effects.findByType(StatusAffects.Feeder) >= 0 && feedposit == "B. Feed") {
...
function rapeMinotaurTentacles(): void {
    ...
    if (player.cocks.length != player.cocks.tentacleCocks()) {
        ...
        outputText("Your " + cockDescript(player, temp2) + " slams into his tailhole, spreading it wider as you thrust in and out of the opening, relishing in the feeling of the rough, yet lubricated insides of the minotaur.  ", false);
    }
    // Not mismatched
    else {
        temp2 = 0;
...
function rapeMinotaurTentacle(): void {
    ...
    if (player.clitLength > 6) {
        outputText("Eager for more stimulation, you muse that your clit's big enough to fuck with, so why not!? You take your fingers, spit on them, and slide them into place, opening him up wider to accept your " + clitDescription(player) + ". It's so sensitive, sliding into that tight hole, and the sensations are driving you wild as you hump against him with your dick-like clit.  ", false);
    }
    // Free pussy!
    if (player.vaginas.length > 0) {
        temp = 2;
        outputText("You're a woman, and it's about time you got some pussy action too!  You slide your " + vaginaDescript(player, 0) + " onto " + cockDescript(player, x) + "-wrapped minotaur-prick, further driving the beast mad with lust.  He plaintatively groans, struggling to get his dick free or to cum, you aren't quite sure.  ", false);
        ...
        if (player.cumQ() < 50) outputText("Your cock made quite a mess of things, and when you pull him out, your " + vaginaDescript(player, 0) + " makes a wet sucking sound.  ", false);
        if (player.cumQ() >= 50 && player.cumQ() < 400) outputText("Your cock made quite a mess of things inside your " + vaginaDescript(player, 0) + ".  As you pull free, cum drips freely down your thighs.  The heavenly scent of minotaur spunk fills your nose, and begin scooping the mixed jizz up to sample.  ", false);
        if (player.cumQ() >= 400) outputText("Your cock made quite a mess of things inside your " + vaginaDescript(player, 0) + ".  As you pull free, a river of cum pours from betwixt your thighs.  The heavenly scent of minotaur spunk fills your nose, and you begin scooping the mixed jizz up to sample.  ", false);
    }
    // Free ass!
    else {
        temp = 2;
        outputText("You're feeling a little kinky so you step in front of him, offering him a little bit more, pushing yourself down onto his meaty dick.  You slowly push down, enjoying every inch as it travels down your " + assholeDescript(player) + ", the pre leaking from it acting as a lubricant as his tentacle-wrapped cock works its way inside you.  ", false);
        // Ass cum
        outputText("Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your " + multiCockDescript(game.player) + " in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ", false);
...
function minoRapeIntro(): void {
    ...
    if (monster.HP <= 0) {
        ...
        if (player.vaginas.length > 0) outputText("  Glancing back at you with hope in his eyes, the minotaur's " + cockDescriptShort(monster, 0) + " twitches as he lays his gaze upon your " + vaginaDescript(player, 0) + ".", false);
...
function girlRapeAMinotaur(): void {
    ...
    if (player.lowerBody == LowerBodyType.CENTAUR) outputText("hindquarters, forcefully grinding your vagina along his length like a toy.  ", false);
...
export function getRapedByMinotaur(autoRape: boolean = false): void {
    ...
    if (!autoRape) {
        ...
        // Oral rape chance
        if (rand(2) == 0 && player.lowerBody != LowerBodyType.CENTAUR) {
...
function takeMinoCumDirectly(): void {
    ...
    outputText("An hour or so later, you pick yourself up out of a puddle of mud that reeks of sex.  Still feeling groggy and euphoric, you get dressed, not bothering to clean up.  You have to work to keep your " + assholeDescript(player) + " clenched, lest you lose some of the drug making you so happy.", false);
...
export function minoCumAddiction(raw: number = 10): void {
...
export function minotaurAddicted(): boolean {
    return player.perks.findByType(PerkLib.MinotaurCumAddict) >= 0 || flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 1;
}

export function minotaurNeed(): boolean {
