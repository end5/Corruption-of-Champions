...
export class HellHoundScene {
    ...
    public hellhoundRapesPlayer(): void {
        ...
        if (player.vaginas.length > 0 && player.lowerBody != LOWER_BODY_TYPE_NAGA) {
            ...
            if (player.cuntChange(monster.cockArea(0), false)) outputText("The beast howls as your " + vaginaDescript(0) + " is stretched to accommodate the large shaft.  ", false);
            outputText("The hellhound pants for a few seconds before continuing.  ", false);
            if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS_WET) outputText("The pause gives your " + vaginaDescript(0) + " time to moisten, ensuring the next thrust won't be quite as rough.  ", false);
