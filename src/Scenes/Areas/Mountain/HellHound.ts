...
export class HellHound extends Monster {
    protected hellhoundFire(): void {
        ...
        if (player.perks.findByType(PerkLib.Evade) >= 0 && player.spe >= 35 && rand(3) != 0) {
        ...
        else if (player.perks.findByType(PerkLib.Misdirection) >= 0 && rand(100) < 20 && player.armorName == "red, high-society bodysuit") {
        ...
        else if (player.perks.findByType(PerkLib.Flexibility) >= 0 && player.spe > 30 && rand(10) != 0) {
    ...
    public defeated(hpVictory: boolean): void {
        if (hpVictory) {
            ...
            if (player.vaginas.length > 0 && player.lust >= 33 && !player.isNaga()) {
                ...
                simpleChoices("Fuck it", HellHoundScene.hellHoundPropahRape, "", null, "", null, "", null, "Leave", cleanupAfterCombat);
            } else {
                cleanupAfterCombat();
            }
        } else {
            ...
            if (player.gender > 0 && player.lust >= 33) {
                ...
                if (player.vaginas.length > 0 && player.lust >= 33 && !player.isNaga()) {
                    ...
                    temp2 = HellHoundScene.hellHoundPropahRape;
                }
                outputText(".  What do you do?", false);
                simpleChoices("Lick", HellHoundScene.hellHoundGetsRaped, "Fuck", temp2, "", null, "", null, "Leave", cleanupAfterCombat);
            }
            else {
    ...
    public won(hpVictory: boolean, pcCameWorms: boolean): void {
        if (pcCameWorms) {
            ...
            HellHoundScene.hellhoundRapesPlayer();
        }
    }
    ...
    public constructor(noInit: boolean = false) {
        ...
        // this.plural = false;
        this.cocks.createCock(8, 2, CockTypesEnum.DOG);
        this.cocks.createCock(8, 2, CockTypesEnum.DOG);
        this.balls = 2;
        this.ballSize = 4;
