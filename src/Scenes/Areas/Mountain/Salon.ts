...
export function salonPurchaseMenu(): void {
    ...
    let cutMedium2: () => void = null;
    let cutLong2: () => void = null;
    let lengthening: () => void = null;
    let minoCum: () => void = null;
    let mudFacial2: () => void = null;
    let sandFacial2: () => void = null;
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00142] > 0) minoCum = buyMinoCum;
...
function hairDresserGreeting(): void {
    ...
    if (player.cocks.length == 0) {
        ...
        outputText("  I'll even do you the favor of letting you blow it in my mouth, I've already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you " + mf(player, "kinky boy", "naughty girl") + "?</i>\"\n\n", false);
...
function gloryholeMinotaur(): void {
    ...
    outputText("Driven on by the minotaur's reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your " + player.armorName + ", freeing up your " + allBreastsDescript(player) + ". With each pass, you're able to take more and more of the bull dick down your throat. You caress your " + biggestBreastSizeDescript(player) + " while moaning around the dick. When you get your mouth around the second ring, the minotaur's cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You ");
    if (player.breastRows.biggestTitSize() > 1) outputText("hold up ");
...
function minotaurCumBukkakeInSalon(): void {
    ...
    outputText("Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your " + feet(player) + " as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that's pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won't come from just a hand though.  They need something... tighter.\n\n", false);
    ...
    if (player.breastRows.biggestTitSize() >= 2) {
        outputText("  You wrap the pillowy flesh of your " + allBreastsDescript(player) + " around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ", false);
        if (player.breastRows.biggestTitSize() < 6) outputText("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ", false);
    ...
    outputText("You lower and raise yourself, bouncing up and down on your " + legs(player) + " to enhance the ", false);
    if (player.breastRows.biggestTitSize() >= 2) outputText("tit-fucking ", false);
    ...
    if (player.breastRows.biggestTitSize() >= 2) outputText("The sudden girth change sends an enticing ripple through your " + allBreastsDescript(player) + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ", false);
    ...
    outputText("The strength goes completely out of your " + legs(player) + ", but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...", false);
    // [NEXT]
    player.orgasm();
...
function fuckLynnette(): void {
    ...
    outputText("\n\nTiny hands latch onto your shoulders as you pour your distilled lusts into the lucky woman's womb, digging deeply into your " + skinFurScales(player) + ". Lynnette's lips crash back against yours, her tongue burrowing into your mouth before she traps your lower lip, biting it painfully hard. Her eyes roll back, and her body shakes with so much pleasure that she almost looks to be having a seizure.  Your cock doesn't care about that any more than her sperm-hungry uterus does, the two organs far too busy exchanging genetic material to care what state their owners are in.");
    ...
    else if (player.cumQ() < 6000) outputText("\n\nThere's so much cum flowing out of your " + cockHead(player, x) + " that Lynnette's once-taut tummy immediately plumps up with spermy weight, filled into a jiggly paunch by your first few blasts.  As you continue to flood her womb with your slurry of baby-making goodness, her belly continues to rise up between you, eventually arcing up into a gravid dome.  Somehow, her body holds it all inside, and only a small trickle of spunk leaks out of her luscious slit as you finish.");
    ...
    else outputText("\n\nOh gods!  Every torrential outflow of cum is so substantial that it's literally stretching the poor goblin out as it rushes into her womb.  The first two blasts are enough to turn her once-taut middle into a sperm-stuffed dome, and you just keep shooting after that, each time making her gravid tummy jiggle and swell, rounding fuller and fuller until her bulging midsection seems almost as big as her.  Her belly button long ago popped out into an outtie, and the smooth, emerald skin shines with its overstretched tightness.  Lynnette whimpers softly, and a sudden torrent of spunk rolls out around your " + cockDescript(game.player, x) + ".  It seems she just couldn't hold it all in, and you aren't even done cumming yet!  You resume sliding in and out as you jizz, washing your soon-to-be-pregnant fucktoy's tunnel with generous waves of spooge until you finally exhaust yourself.");
