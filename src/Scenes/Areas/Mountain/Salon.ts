...
public  salonPurchaseMenu():void {
	...
	spriteSelect(38);
	var cutShort2:() => void = null;
	var cutMedium2:() => void = null;
	var cutLong2:() => void = null;
	var lengthening:() => void = null;
	var minoCum:() => void = null;
	var mudFacial2:() => void = null;
	var sandFacial2:() => void = null;
	if(flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00142] > 0) minoCum = buyMinoCum;
...
private  goblinHairDresserFacefuck():void {
	spriteSelect(38);
	outputText("Lynnette licks her lips and practically tears her way into your " + player.armorName + ", having your crotch exposed in seconds.  Your " + game.player.cockDescript(0) + " flops out immediately, slapping her on the nose as it grows hard.  She wraps both hands around you and begins pumping with practiced ease, flicking her tongue over your crown and wrapping her lips ", true);
...
private  minotaurCumBukkakeInSalon():void {
	...
	else if(player.hasCock()) outputText(" and turning " + game.player.sMultiCockDesc() + " into a turgid, pulsating mass.\n\n", false);
	else outputText("\n\n", false);
	...
	else outputText("presses on your prostate as it squeezes by, making " + game.player.sMultiCockDesc() + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + game.player.sMultiCockDesc() + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.", false);
	outputText("\n\n", false);
...
private  fuckLynnette():void {
	...
	outputText("\n\nYour body has been ready to go off for some time now, and the words act like an irresistible trigger for your climax, causing you to lurch your " + game.player.cockDescript(x) + " ");
	...
	else outputText("\n\nOh gods!  Every torrential outflow of cum is so substantial that it's literally stretching the poor goblin out as it rushes into her womb.  The first two blasts are enough to turn her once-taut middle into a sperm-stuffed dome, and you just keep shooting after that, each time making her gravid tummy jiggle and swell, rounding fuller and fuller until her bulging midsection seems almost as big as her.  Her belly button long ago popped out into an outtie, and the smooth, emerald skin shines with its overstretched tightness.  Lynnette whimpers softly, and a sudden torrent of spunk rolls out around your " + game.player.cockDescript(x) + ".  It seems she just couldn't hold it all in, and you aren't even done cumming yet!  You resume sliding in and out as you jizz, washing your soon-to-be-pregnant fucktoy's tunnel with generous waves of spooge until you finally exhaust yourself.");
