...
function defeatFemale(): void {
    ...
    // (Combat Intro: HP)
        ...
        outputText("Knowing that she plans to force herself on you, you can't help but become aroused by the notion of the curvy spider-vixen riding your face while her smooth carapace clenches around your head.  You shake the thought away, but she sees your distraction and begins to trace a pointed fingertip around one of your " + nippleDescription(player, 0) + "s before asking, \"<i>Oh, are you ready for the sex already?  It's been so long that I was going to savor this, but I suppose we should get started.</i>\"\n\n", false);
    }
    ...
    if (player.tongueType == TongueType.SNAKE) outputText("the snake-like length the whole way into her womb, slithering right past her cervix.", false);
    else if (player.tongueType == TongueType.DEMONIC) outputText("the massive, demonic length into her vagina, curling it around the walls and stroking them all with even motions.", false);
...
function spiderMorphFemaleRidesACawk(): void {
    ...
    // (Noncombat Intro:)
    if (!game.inCombat) {
        outputText("You shuck your " + player.armorName + " and toss it aside, feeling " + sMultiCockDesc(game.player) + " ", false);
...
function femaleSpiderMorphTooBigWebRape(): void {
    ...
    // (Consensual)
    if (!game.inCombat) {
        outputText("You hastily remove your " + player.armorName + " and toss it aside, glad to be able to let " + sMultiCockDesc(game.player) + " flop out and breathe.  The spider-girl's eyes widen as she takes in ALL of your ", false);
        ...
        outputText("\"<i>W-wow, you're a big " + mf(player, "boy", "girl") + ", aren't you?  Why don't you lie down and get comfortable while I get you restrained... I wouldn't want that beast between your legs to kill me!</i>\" she says.\n\n", false);
    ...
    // (COMBAT LOSS)
        ...
        if (player.lust > 99) outputText("stick your hands into your groin, fondling " + oMultiCockDesc(game.player) + " with need", false);
        else outputText("groan in obvious pain, unable to move", false);
        outputText(".  The spider-girl advances, giggling girlishly at your state while she painstakingly removes every piece of armor, visibly gasping at the size of your " + cockDescript(game.player, x) + " as it flops out.\n\n", false);
...
function fSpiderMorphRape(): void {
    ...
    outputText("You lie down beside the spider-slut and thread your " + legs(player) + " between her thighs and lower body.  She gasps and moans at every tiny touch against her needy cunt as you get into position, streaking your lower body with liberal amounts of her copious fem-cum.  Once your " + vaginaDescript(player) + " finally contacts her sloppy, gushing box, the result is instantaneous and powerful - she cums hard.  Her legs twist and spasm around your " + legs(player) + ", the nerveless convulsions spread up her body until she's writhing in the dirt and her tits are flopping and jiggling.\n\n", false);

    outputText("While the stimulation of her twitching body grinding against your " + vaginaDescript(player) + " is wonderful, you quickly tire of her thrashing.  You locate her bulky abdomen and find one of the spinnerets.  The fleshy protuberance is surprisingly quite easy to handle, and you aim it at one of the spider-girl's legs and squeeze.  A torrent of webbing splatters over her carapace-clad legs, and though it doesn't stick to her, it does completely engulf her ankle and adhere to the ground, securing her in place.  You pull back and repeat the action with her other leg, leaving her stuck fast.\n\n", false);

    outputText("You thread yourself back under and around her, sliding back up against her still-quivering cunt until your slippery sex is mashing against it.  With two pairs of slippery pussy lips gliding over each other, squishing wetly from the mixed dribbles of fem-cum, the rest of the swamp fades to a barely-noticed background.  Your " + clitDescription(player) + " quickly emerges from its sheath", false);
...
function fSpiderMorphRapeDude(): void {
    ...
    outputText("", true);
    spriteSelect(73);
    outputText("You push the spider-girl onto her back, getting a good long look at your conquest before you take off your " + player.armorName + ".  At the sight of your " + multiCockDescriptLight(game.player) + ", she ", false);
    ...
    outputText("You advance, " + sMultiCockDesc(game.player) + " ", false);
    if (player.lust < 70) outputText("thickening ", false);
    else outputText("pulsating", false);
    outputText(" and ready to penetrate that beckoning, arachnid pussy.  The moist lips slip around your " + cockDescript(game.player, x) + " easily, squelching wetly while you slide home.  You gasp from the sudden change, rocking your " + hipDescription(player) + " slowly against the spider-morph's while you acclimate yourself, reveling in the heat of her loins.  She grabs your head in that moment, guiding you down with a weak tug to place your lips upon her breast, and with a smile, you start to suckle the chocolate skin of her nub.\n\n", false);
    ...
    outputText(".  Jism boils out of your cum-slit, packing itself deep inside, right against her cervix, and the spider-girl's violet eyes cross from the new sensation in her still-cumming cunt.  You pump your " + hipDescription(player) + " in time with each sloppy eruption of seed, pushing more and more of your spooge inside her until you're sure it'll take hold in her womb.", false);
    if (player.cumQ() >= 500) {
    ...
    outputText("The soon-to-be expecting mother comes down from her orgasm first, regaining control of herself while your " + cockDescript(game.player, x) + " is giving the last of its gifts to her pussy.  Her hands roam over her ", false);
...
function evilSpiderGirlVictoryAnal(): void {
    ...
    outputText("Of course you will.  You answer her query by withdrawing from the velvet tunnel and planting your " + cockDescript(game.player, x) + " squarely at the spider-morph's tight asshole.  She yelps in surprise and tries to scrabble away, but you hold her fast, leaning on her to push her tits and face into the loamy swamp dirt.  Pressing on, you mount her, thrusting your " + cockDescript(game.player, x) + " deep into her dark hole", false);
    if (y >= 0) outputText(", while your " + cockDescript(game.player, y) + " slips into her warmed-up cunt.", false);
    ...
    if (y == -1) outputText("by splattering her fragrant lady-spunk into the dirt", false);
        ...
        outputText("by splattering your " + cockDescript(game.player, y), false);
