...
export function nagaEncounter(): void {
    ...
    if (player.lowerBody == LowerBodyType.NAGA) {
        ...
        player.effects.setValue(StatusAffects.Naga, 1, 1);
        // Not met as naga before
        if (player.effects.getValue2Of(StatusAffects.Naga) == 0) {
            player.effects.setValue(StatusAffects.Naga, 2, 1);
        ...
        // [If player has fangs]
        if (player.faceType == FaceType.SNAKE_FANGS) outputText("your own.", false);
        // [player has no fangs]
        ...
        if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
            ...
            if (player.cocks.cockArea(0) > 35) {
                outputText("The kiss continues as both of your bodies rub together sensually, your tails continuing to wrap around one another. There is a pulsing in your " + cockDescript(game.player, 0) + " as it starts to grow hard against the naga's soft belly. Feeling this, the naga gently pulls away, slowly letting your tongue out of her mouth as she does so. Her hands make their way over your shoulders, down your abs, and stop at your " + multiCockDescriptLight(game.player) + ".\n\n", false);
                outputText("\"<i>You're quite the big boy, aren't you?</i>\" she says as she wraps both of her hands around your now throbbing " + cockDescript(game.player, 0) + ". She starts to slide her hands up and down your length. Sticking out her tongue, she wraps it around the tip and licks at the pre that is starting to leak out.\n\n", false);
                outputText("A hiss of pleasure escapes your lips as the naga strokes and licks at your " + cockDescript(game.player, 0) + ", her talented fingers and tongue bringing you into a further state of arousal. The naga stops her caressing and brings your " + cockDescript(game.player, 0) + " to her chest, pressing her breasts around it. She slowly starts to slide her body up and down your shaft, using her tongue to bring some of your pre and lubricate her body. Once she is satisfied with her body's slickness, she quickens her pace.\n\n", false);
            ...
            // [Cock isn't too big]
                ...
                outputText("The kiss continues and both of your bodies rub together sensually, your tails continuing to wrap around one another. There is a pulsing in your " + multiCockDescriptLight(game.player) + " as ", false);
                if (player.cocks.length == 1) outputText("it starts ", false);
                else outputText("they start ", false);
                outputText("to grow hard against the naga's soft belly. Feeling this, the naga gently pulls away, slowly letting your tongue out of her mouth as she does so. Her hands make their way over your shoulders, down your abs, and stop at your " + multiCockDescriptLight(game.player) + ".\n\n", false);
                // [player has one dick]
                if (player.cocks.length == 1) outputText("Gently she starts to stroke the length of your " + cockDescript(game.player, 0) + " with one hand while circling the tip with the other. ", false);
                // [player has two dicks]
                if (player.cocks.length == 2) outputText("She takes one in each of her hands and gently strokes them up and down. Every few seconds she stops at the tip to rub her palms over them. ", false);
                // [player has three or more dicks]
                if (player.cocks.length >= 3) outputText("She takes one in each hand, stroking them slowly and making sure to pay attention to the tip. Every so often she switches to a different dick to make sure that each and every one of your throbbing cocks has some love given to them. ", false);
                outputText("A hiss of pleasure escapes your lips as the naga strokes your " + multiCockDescriptLight(game.player) + ", her talented fingers bringing you further into a state of arousal. She stops her caress and brings her hand to a scaly covering at her crotch, spreading it wide to reveal her soft pussy.\n\n", false);

                // [player has one dick]
                if (player.cocks.length == 1) outputText("She carefully lines it up with your member and starts to tease the tip before gently inserting the first few inches. ", false);
                // [player has two dicks]
                if (player.cocks.length == 2) outputText("She carefully lines it up with both of your members and starts to tease their tips before gently inserting the first few inches. ", false);
                // [player has three or more dicks]
                if (player.cocks.length >= 3) outputText("She carefully lines it up with one of your members and starts to tease the tip before gently inserting the first few inches. ", false);
        ...
        // chick secks
            ...
            outputText("You untangle the end of your tail from hers and bring it up between the two of you, sliding it between your " + breastDescriptOfRow(game.player, 0) + " and inserting it sensually into your mouth. You suck on the end, making sure to coat it liberally before bringing it down to the naga's awaiting slit.\n\n", false);
    ...
    if (player.effects.getValue1Of(StatusAffects.Naga) == 0) {
        if (player.effects.findByType(StatusAffects.Naga) < 0) player.effects.create(StatusAffects.Naga, 0, 0, 0, 0);
    ...
    if (player.effects.getValue1Of(StatusAffects.Naga) > 0) {
        ...
        player.effects.setValue(StatusAffects.Naga, 1, 0);
...
function gooNagaRape(): void {
    ...
    if (player.cor > 50) {
        // (if corrupt male)
        if (player.gender == 1) {
            outputText("your " + cockDescript(game.player, 0) + " growing harder at the thought.", false);
        }
        // (if corrupt female)
    ...
    if (player.cocks.length > 0) {
        if (player.cocks.length == 1) outputText("your " + cockDescript(game.player, 0), false);
        else outputText("one of your " + multiCockDescriptLight(game.player), false);
    ...
    // (if herm)
        ...
        if (player.cocks.length == 1) outputText("your " + cockDescript(game.player, 0), false);
        else outputText("your " + multiCockDescriptLight(game.player), false);
    ...
    // (if female)
        ...
        // (if squirter)
        if (player.vaginas[0].vaginalWetness == VaginaWetness.SLAVERING) outputText(", your girlcum gushing out and pooling on the stomach of the naga", false);
        outputText(".", false);
    }
    // (if herm)
    if (player.gender == 3) {
        outputText("You scream in ecstasy as you hit your peak, your girlcum ", false);
        if (player.vaginas[0].vaginalWetness < VaginaWetness.SLAVERING) outputText("leaking out", false);
...
function nagaVictoryMale(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.lowerBody == LowerBodyType.CENTAUR) {
            outputText("As your massive bulk looms over the defeated naga, your eyes gleam with desire. Your heart pounds in your ears as lusty thoughts echo in your head. You lift the snakewoman up into your arms, bringing her close before licking her neck and tasting her smooth scaly skin as her tail dangles down to the sand. As you touch her, she recovers, and her tail slowly wraps around your equine half to support her own weight. She returns your attentions by running her long forked tongue down your chest. Her scaly tail rubs sensually over your flesh as she continues to wind around you, caressing your " + cockDescript(game.player, 0) + "  with her tip, making you hard in a flash. The naga wriggles free from your embrace and coils behind you. Her entire length is now wrapped around various portions of both your human and equine halves, and she uses her grip to dangle underneath you. Her hands reach out to take a firm hold of your " + cockDescript(game.player, 0) + ".  You shiver as her tongue darts out and slides around your cock, pre-cum dripping from your head as she licks you. The agile naga lowers more of herself beneath you, spreading her pussy wide open before sliding onto your " + cockDescript(game.player, 0) + " and letting out a hissy moan as you penetrate her. Your body tingles as her entire length quivers in pleasure, sending ripples over the flesh her tail is wrapped around.  Jolts of pleasure run through you as the naga woman grinds her hips against your " + cockDescript(game.player, 0) + " , driving it into her as far as she can take.  With a loud and drawn-out moan, ", false);
            // [normal cum load]
            ...
            // [SUPER CUM]
            else outputText("you explode with huge fountains of cum spurting from your " + cockDescript(game.player, 0) + ", gushing out from where the two of you are joined to rain on the sands below, ", false);
        ...
        // Uncorrupted
            ...
            if (player.cocks.cockArea(0) <= monster.vaginalCapacity()) {
                ...
                // [Spermageddon]
                else outputText("your " + cockDescript(game.player, 0) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.", false);
            }
            // [If cock too large]
            else {
                outputText("You draw away from her lips and return to your upright posture. Relaxed as she is, the naga looks nervously at your " + cockDescript(game.player, 0) + ", fully aware that it won't fit inside her small body. This occurs to you as well, and you begin to formulate a different plan. You move up onto her body so that you're straddling her stomach, fitting your " + cockDescript(game.player, 0) + " neatly between her small C-cup breasts. You gently coax her hands to her breasts, and she begins to comprehend your intentions. She begins to squeeze your " + cockDescript(game.player, 0) + " between her two warm mounds of flesh, doing her best to keep it snugly between them. Of her own accord, the naga begins to run her tongue across the " + cockHead(player) + ", obviously understanding what you want. She looks up at you with an unsure look, as if asking whether or not she is doing it right. You nod to her, and that's all the encouragement she needs. She begins to stroke your " + cockDescript(game.player, 0) + " with her breasts as if it came naturally to her, covering as much of your " + cockHead(player) + " as she can with her warm and trembling mouth.\n\n", false);
                ...
                outputText("  The naga removes your " + cockDescript(game.player, 0) + " from her mouth", false);
                // [lots of cum OR more]
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.lowerBody == LowerBodyType.CENTAUR) {
            ...
            if (monster.lust > 99) {
                ...
                if (player.gender == 3) outputText("  As her tail gently squeezes your body, it draws your own " + multiCockDescriptLight(game.player) + " up against your body, her soft underbelly stimulating the sensitive skin.", false);
                outputText("  With the added support, you are able to move your arm away from her stomach, moving your hand down to her covered snatch. It isn't hard to find as there is practically a river of her juices dripping from it. You peel away the scaly covering, and reveal her soaking wet lovehole. As you begin to play with her engorged clit, her soft moans begin to become more pronounced and you can feel her tail begin to wrap more tightly around your body. You feel a strange sensation as her tail brushes past your own " + vaginaDescript(player, 0) + ", causing you to tense up a bit. It seems that she noticed, because she does it again a few seconds later. As she begins to rub your " + vaginaDescript(player, 0) + " with her tail, your body begins to betray your own lusts.\n\n", false);
                outputText("She begins to gently toy with your " + clitDescription(player) + " using the very end of her tail, flicking it back and forth. You can't help but start moaning softly yourself. You respond by slowly teasing two of your fingers inside her waiting pussy, slowly beginning to move them in and out. As you begin to accelerate the motion of your fingers, the naga steps it up as well, sliding the tip of her tail inside your " + vaginaDescript(player, 0) + ".  ", false);
                cuntChange(player, 20, true, false, true);
...
export function nagaFUCKSJOOOOOO(): void {
    ...
    if ((player.perks.findByType(PerkLib.BimboBrains) >= 0 || player.perks.findByType(PerkLib.FutaFaculties) >= 0) && player.vaginas.length > 0) {
        ...
        outputText("The naga then lies flat on the sandy ground, pulling you down with her. She lays you down on her body, face up, with your head resting on her soft breasts. She reaches around your body, placing her soft hands on your own " + allBreastsDescript(player) + ", gently holding you in place as her tail moves closer to your " + vaginaDescript(player, 0) + ". Her now slick tail begins to rub against your clit and as the scaly skin runs along your " + clitDescription(player) + ", you begin to moan air-headedly. There is no holding it back, this feeling is one of the best you have experienced. Her tail continues to flick and rub your clit as you begin to buck your hips gently. The naga pokes and rubs at your hard clit for some time, eventually driving you to the first of many coming orgasms. After your breath has become even again and your muscles have relaxed, the naga resumes her advances. She begins to rub the tip of her tail up and down along your " + vaginaDescript(player, 0) + ", and you FINALLY get what she is trying to do.\n\n", false);
    ...
    // too)
        ...
        // (if defeated by lust)
        if (player.lust > 99) outputText("all your resistance seems to do is make your " + cockDescript(game.player, 0) + " harder.", false);
        // if defeated by KO)
        ...
        if (player.cor > 75) outputText("  The thought somehow makes your " + cockDescript(game.player, 0) + " throb even more.", false);
        outputText("\n\n", false);
        outputText("You close your eyes and ready yourself to meet your fate. Instead, you feel the hand of the naga at your crotch, sliding your " + player.armorName + " down to give her better access to your " + cockDescript(game.player, 0) + ". A gasp of surprise escapes you as the naga grabs ahold of your " + cockDescript(game.player, 0) + " and starts to stroke it, licking at the " + cockHead(player) + " with her serpentine tongue. You moan as she quickens her pace, causing a wave of heat to rise through your shaft.\n\n", false);

        // (Cock fits)
        if (player.cocks.cockArea(0) <= monster.vaginalCapacity()) {
            outputText("She stops suddenly and you look at her, a pleading look in your eyes despite yourself. The naga isn't done with you yet, and she makes this known by opening up the scaly slit at her crotch to reveal her moist cunt. She rubs the opening on to the " + cockHead(player) + " of your " + cockDescript(game.player, 0) + ", teasing your throbbing member before slowly sliding it in. She carefully lowers herself onto you, swallowing up the first few inches of your " + cockDescript(game.player, 0) + ". Her hips slide back, before bringing them back down and sliding more of your cock into her. She continues to gently swallow more of you inside of her, the folds of her love canal massaging you with each progressive thrust. Finally there is no more of you that she can push into her, her stomach presses against you in an attempt to try to bring more of you inside her.\n\n", false);
            outputText("You feel the hands of the snake woman firmly grab your shoulders and watch as she pulls away from you, shouting with pleasure as she thrusts herself down on the full length of your shaft. Unable to move or do anything to help her, you watch helplessly as she manipulates your body, her slick netherlips completely enveloping your " + cockDescript(game.player, 0) + ".  Again and again she thrusts her hips onto your " + cockDescript(game.player, 0) + ", her pussy squeezing you almost as tightly as her tail. She continues to impale herself on your " + cockDescript(game.player, 0) + ", her thrusts becoming more intense by the moment. Finally she gives one last squeeze and tilts her head back with a yell. Unable to hold back, her tight walls begin to convulse, squeezing your " + cockDescript(game.player, 0) + " tighter than ever before. You can feel a pressure welling up inside as her soaking pussy attempts to milk your " + cockDescript(game.player, 0) + " of its tasty seed. You try to hold back, but it's no use. As she continues to pump at your " + cockDescript(game.player, 0) + ", a torrent of warm, sticky cum ", false);
            // Cum variants
        ...
        // (Cock too big)
            ...
            outputText("As she continues to stroke your " + cockDescript(game.player, 0) + ", her nimble fingers doing their best to wrap around your prodigious girth, she moves her head in closer to wrap her lips around the tip of your " + cockDescript(game.player, 0) + ". She begins to gently suck at the head of your " + cockDescript(game.player, 0) + ", her thin, reptilian tongue dancing across the slick surface. ", false);
            // (if PC has balls)
            if (player.balls > 0) {
                outputText("She shifts one of her hands down from your " + cockDescript(game.player, 0) + ", to your " + ballsDescriptLight(player) + ", cupping them in her slick palm.  ", false);
            }
            outputText("As her tongue continues to slide across the " + cockHead(player) + " of your " + cockDescript(game.player, 0) + ", her slender tongue wanders ever so closely to the center. You feel a strange sensation as her tongue wanders inside the hole at the tip of your " + cockDescript(game.player, 0) + ", just barely tickling the inside. You try to pull away, but her tail binds you too tightly to move much. The naga notices your reaction and begins to exploit it as much as she can. She begins to force the rest of her tongue inside of your urethra, her forked tip tickling you from the inside as she pushes deeper. The naga, now that her tongue is completely buried inside your " + cockDescript(game.player, 0) + ", turns her attentions to the actual shaft, attempting to sandwich it between her modest C-cup breasts. She begins to stroke them up and down along your " + cockDescript(game.player, 0) + ", sucking at the " + cockHead(player) + " and tickling your urethra at the same time.\n\n", false);
    ...
    // b) female //http://nekomimichan.org/mg/src/12844585127.jpg
        ...
        // (if uncorrupted)
        else outputText("and while you know you probably won't like it, your " + vaginaDescript(player, 0) + " says otherwise.  ", false);
        ...
        outputText("Then the naga lies flat on the sandy ground, pulling your body down onto on hers, face up, with your head resting between her soft breasts. She reaches around your body, placing her hands on your own " + biggestBreastSizeDescript(player) + " gently holding you in place as her tail moves closer to your " + vaginaDescript(player, 0) + ". As she begins to rub her presently-slick tail against your slit, the scaly tip brushes your " + clitDescription(player) + " and you moan with passion in response.  There is no holding it back; this feeling is one of the best you have experienced. Her tail continues to flick and rub your " + clitDescription(player) + " as you begin to buck your hips gently. The naga pokes and rubs at your " + clitDescription(player) + " for some time, eventually driving you to the first of many coming orgasms. After your breath has become even again and your muscles have become relaxed, the naga resumes her advances. She begins to rub the tip of her tail up and down along your " + vaginaDescript(player, 0) + ", and you finally understand her intentions.\n\n", false);
...
export function nagaPlayerConstrict(): void {
    ...
    if (rand(player.spe + 40) > monster.spe) {
        ...
        monster.effects.create(StatusAffects.Constricted, 1 + rand(4), 0, 0, 0);
    }
    // Failure
    else {
        // Failure (-10 HPs) -
...
export function naggaTease(): void {
    ...
    // (Otherwise)
        ...
        const breasts: boolean = false;
        const penis: boolean = false;
        const balls: boolean = false;
        const vagina: boolean = false;
        const anus: boolean = false;
        const ass: boolean = false;
        ...
        // Normal cases for other monsters
            ...
            if (monster.gender == 1) {
                outputText("Your nimble tail begins to gently stroke his " + cockDescriptShort(monster, 0) + ", and you can see it on his face as he tries to hold back the fact that it feels good.", false);
            }
            if (monster.gender == 2) {
