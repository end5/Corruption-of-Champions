...
function oasisTalkAccept(): void {
    ...
    if (player.lowerBody == LowerBodyType.CENTAUR) outputText("your rear legs losing their balance and sending you crashing on your flank.  ", false);
    ...
    if (player.effects.findByType(StatusAffects.VoluntaryDemonpack) < 0) player.effects.create(StatusAffects.VoluntaryDemonpack, 0, 0, 0, 0);
    else {
        player.effects.addValue(StatusAffects.VoluntaryDemonpack, 1, 1);
    }
    // TO THE SECKSIN!
    doNext(oasisSexing);
}
export function oasisSexing(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks[0].cockLength > 25) outputText("feel your " + cockDescript(game.player, 0) + " slide into a hell-girl's sloppy cunt. The demoness moans and pants in pleasure as your " + cockDescript(game.player, 0) + " bottoms out in her gaping pussy and she begins to ride you like the motionless whore you momentarily are, her gigantic breasts threatening to concuss you as she slides the huge distance up and down your cock in her thirst to fill her cavelike cunt.  ", false);
            ...
            outputText("see a smaller and younger demon lowering her very tight pussy onto your " + cockDescript(game.player, 0) + ". It's a tight fit, but her almost-virginal pussy is dripping wet. She bottoms her pussy out ", false);
            if (player.cocks[0].cockLength > 17) outputText("with hardly any of you inside her ", false);
            else if (player.cocks[0].cockLength > 12) outputText("about halfway down your " + cockDescript(game.player, 0) + " ", false);
            else if (player.cocks[0].cockLength > 6) outputText("with almost all of you inside of her ", false);
            else if (player.cocks[0].cockLength <= 6) outputText("as her pussy lips touch the base of your " + cockDescript(game.player, 0) + " ", false);
            ...
            if (player.cocks[0].cockLength > 6) {
                outputText("The realisation that her pussy is slowly stretching over your " + cockDescript(game.player, 0) + " gives you a thrill that can only be described as positively evil.  ", false);
                dynStats("cor", 1);
            }
            outputText("Soon the slender cock-whore begins to cum and she starts on what is about to be the first of several shuddering orgasms. You smile evilly as she cums around your " + cockDescript(game.player, 0) + " and when her eyes roll back to the front of her head you know that there is more to come.  ", false);
        }
        // Orgasmzzzzzzz
        ...
        if (player.cocks.length > 1) {
            ...
            if (player.cocks.length == 2) outputText(cockDescript(game.player, 1) + " is", false);
    ...
    else if ((player.HP < 1 || player.lust > 99) && game.inCombat) {
        // ►Oasis Demons Defeat PC as part of antm
        // Antmorph stuff
        if (monster.effects.findByType(StatusAffects.phyllafight) >= 0) {
...
function oasisBadEnd(): void {
    ...
    // [If female/herm]
    if (player.lowerBody != LowerBodyType.CENTAUR) outputText("Your feet are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ", false);
    if (player.lowerBody == LowerBodyType.CENTAUR) outputText("Your four legs are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ", false);
    ...
    outputText("\"<i>There will no longer be any of that back talk, slave. As for what I'm talking about... After all of our encounters with you, my tribesmen and I have made a decision.</i>\" The leader's smirk widens as he reaches down and grips your face tightly, forcing you to look up at him. His eyes look you over appraisingly as he pulls out a black collar with a long chain attached and fastens it to your neck. His other hand reaches down to one of your " + nippleDescription(player, 0) + "s and grabs it roughly, giving it a hard squeeze and causing you to let out a small moan of pain and pleasure. He relinquishes his grip and walks around you, continually looking you over and scrutinizing your body from every angle. \"<i>We have decided that you would make an excellent addition to our group, and have laid claim to you as our sex slave... to be used in any way that we want.</i>\" He stops in front of you and continues, \"<i>We were lucky to catch you while you were still unconscious, before you could sneak off on us. I will personally enjoy using you for my own purposes. Maybe I'll even use you to carry my children once you're properly broken in.</i>\"\n\n", false);
    // [If herm]
    if (player.gender == 3) {
        if (player.lowerBody == LowerBodyType.CENTAUR) outputText("\"<i>Everyone will have a great time playing with you,</i>\" the leader says, smirking wider still as he stares at your centaur body. He reaches out and gives your " + buttDescription(player) + " a hard smack, causing you to yelp and quickly stand to your feet. You stagger a bit as your legs strain against the shackles as you stand. Without warning, he sticks his fingers inside of your " + assholeDescript(player) + " and stretches it out as his free hand reaches between your legs to play with both your " + vaginaDescript(player, 0) + " and " + cockDescript(game.player, 0) + ". You gasp and moan in pleasure as your cock becomes erect in his hand. He laughs as he lets go and moves back in front of you. \"<i>Plus we could use you to carry our heavy loads like a pack animal. You have the perfect body for it.</i>\"\n\n", false);
        else outputText("His smirk widens even more as he stares down at your uncovered " + cockDescript(game.player, 0) + " laying flaccidly between your legs against the hot desert sand. You gasp in surprise as he suddenly reaches down and takes a firm hold of your " + cockDescript(game.player, 0) + ". He strokes it and gives it a rough squeeze, making you whimper sensually as it becomes fully erect in his hand. He laughs as he lets it go, \"<i>Our girls will definitely be having fun with you, slave.</i>\"\n\n", false);
    }
