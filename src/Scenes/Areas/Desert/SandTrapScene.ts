...
export function encounterASandTarp(): void {
    clearOutput();
    spriteSelect(97);
    if (flags[kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && player.eyeType == EyeType.BLACK_EYES_SAND_TRAP && player.wingType == WingType.GIANT_DRAGONFLY) {
        sandTrapBadEnd();
        return;
    }
...
function saveTheSandTarps(): void {
    clearOutput();
    spriteSelect(97);
    // (femininity <= 65):
    if (player.femininity <= 65) {
    ...
    startCombat(new SandTrap());
    monster.effects.setValue(StatusAffects.Level, 1, 2);
    doNext(playerMenu);
}

// [Don't Save]:
function dontSaveTheTarps(): void {
    clearOutput();
    spriteSelect(97);
...
function startSandTarpFight(): void {
    startCombat(new SandTrap());
    playerMenu();
    spriteSelect(97);
}

export function sandtrapmentLoss(clear: boolean = false): void {
    if (flags[kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && player.eyeType == EyeType.BLACK_EYES_SAND_TRAP && player.wingType == WingType.GIANT_DRAGONFLY) {
...
export function pcBeatsATrap(): void {
    clearOutput();
    spriteSelect(97);
    ...
    let nagaThreeSome: () => void = null;
    let putYourDickInIt: () => void = null;
...
// Male/Herm loss (Z)
function dickwieldersLoseToSandTarps(): void {
    clearOutput();
    spriteSelect(97);
    ...
    monster.effects.create(StatusAffects.Fertilized, 0, 0, 0, 0);
    sandTrapPregChance();
    player.orgasm();
    dynStats("lib", 1);
    player.slimeFeed();
...
// Female loss (Z)
function chicksLoseToSandTarp(): void {
    clearOutput();
    spriteSelect(97);
    ...
    if (monster.effects.findByType(StatusAffects.Fertilized) >= 0) outputText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
    ...
    if (monster.effects.findByType(StatusAffects.Fertilized) >= 0) sandTrapPregChance();
    player.orgasm();
    dynStats("lib", 1);
    player.slimeFeed();
    // reduce lust, increase lib, slimefeed, reset hours since cum
...
function genderlessLoss(): void {
    ...
    if (monster.effects.findByType(StatusAffects.Fertilized) >= 0) outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
...
function sandTrapPregChance(): void {
    // Only happens if PC aint already buttpregged
    if (monster.effects.findByType(StatusAffects.Fertilized) >= 0)
...
export function birfSandTarps(): void {
    ...
    if (player.gender == 1) {
        ...
        if (!player.isTaur() || (player.tallness * (5 / 6) < player.cocks[player.cocks.longestCock()].cockLength)) outputText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
    }
    // Female:
    else if (player.gender == 2) {
        outputText("  You roll your eyes to the sky and moan, your " + vaginaDescript(player, 0) + " moistening as you push out the egg.");
        // [(no fukken horses from here)]
        if (!player.isTaur()) outputText("  Your oily hands push softly into your cleft, fingering your needy " + clitDescription(player) + " as you feel the pressure in your bowels intensify again.");
    }
    // Herm:
        ...
        // [(no horses)]
        if (!player.isTaur()) outputText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + clitDescription(player) + " as you feel the pressure in your bowels intensify again.");
    }
...
// \"<i>Hands</i>\" (Z)
// Requires: Genitals
function useSandTarpsHands(): void {
    clearOutput();
    spriteSelect(97);
    ...
    if (player.gender == 1) {
        ...
        if (player.cocks[0].cockLength < 6) {
            // balls:
            if (player.balls == 0) outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + cockDescript(game.player, 0) + " to stimulate you even further.");
            else outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + cockDescript(game.player, 0) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
        ...
        // Two cocks:
            ...
            outputText("  As the sandtrap begins to pump you faster, your disregarded " + cockDescript(game.player, 1) + " bobs in hard, eager concert with your " + cockDescript(game.player, 0) + ".  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
            outputText("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
        }
        // Three cocks:
        else if (player.cocks.length == 3) {
            outputText("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " + cockDescript(game.player, 0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your " + cockDescript(game.player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.");
        ...
        // Four cocks:
        else if (player.cocks.length == 4) {
            outputText("\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " + cockDescript(game.player, 0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your " + cockDescript(game.player, 2) + " and " + cockDescript(game.player, 3) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  \"<i>At last, I've found somebody who complements my body perfectly.</i>");
    ...
    // Herm:
        ...
        // Cock 6-16 inches:
            ...
            if (player.balls == 0) outputText("\n\nThe sandtrap glides its hand up and down your " + cockDescript(game.player, 0) + ", picking up a warm smooth rhythm.");
            else outputText("\n\nThe sandtrap glides its hand up and down your " + cockDescript(game.player, 0) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.");
        }
        else {
        ...
        // More than three cocks:
            ...
            outputText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + cockDescript(game.player, 0) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + cockDescript(game.player, 1) + " and " + cockDescript(game.player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
...
function rideDatSandTarpLikeIts1999(): void {
    ...
    cuntChange(player, 14, true, true, false);
    ...
    if (player.cocks.length > 0) outputText("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
...
// \"<i>Vagina</i>\" (Z)
function stickWangInSandgina(): void {
    clearOutput();
    spriteSelect(97);
    // Requires: Penis and str requirement
    ...
    outputText("\n\nYou take a moment to look your strange conquest over.  Even up close you can't tell from its thin, fey beauty whether it is male or female.  Although it is affecting to look angry at you, its sculpted cheeks are quite flushed and there is a definite undercurrent of desire in its eyes; you suspect your display of strength and dominance appeals to it on some perverse, animalistic level.  Still smiling victoriously, you bend your face into it, deliberately invading its personal space, silently demonstrating who is in charge here.  It can't hold your gaze and, blushing, looks downwards.  This submissive demureness sends heat rushing towards your " + multiCockDescriptLight(game.player) + "; the desire to fuck builds in you, to plough, to sink your cock into this strange, eerily beautiful creature, to establish once and for all who is giving and who is receiving around here... the only question is, how?  Its slim, androgyne upper body is devoid of obvious sexual features.  Your eyes roam steadily downwards.  Where its insect half meets its human half, where its cock should be, it instead has a genital slit; from this is emerging a long, black prong, dripping with clear oil.  Your show of dominance is turning this creature on, and it is powerless to disguise it.  It looks up at you with timid hopefulness... but you are in no mood for that.  There is a definite air of femininity to this creature, a pheromone scent which speaks directly to your " + cockDescript(game.player, 0) + ", makes you hard");
    if (player.balls > 0) outputText(", makes your " + ballsDescriptLight(player) + " swell in readiness,");
    ...
    outputText("\n\n\"<i>Hhhhyes!  That'sz it lion, oh do me, do it to me hard!  Give me yyyyour strong spunk - uh! give hhit to me szo I can sink yyour children into evvryone I catch - yes!  Hhhhso the whole world can know hhwhat a " + mf(player, "man", "woman") + " you arrrrrhhhhhe!</i>\"  The creature's words subside into a high fluttering cry as it cums.  Its ovipositor spasms and clenches, milking you down your " + cockDescript(game.player, x) + " for all it is worth.  Even if you wanted to, you could not pull back from the brink.  You come with near-painful intensity, your jizz shooting out of you as if your dick were trapped in a vacuum, losing yourself in ecstasy - you feel nothing but the grasping wet around your " + cockDescript(game.player, x) + ", your cream spurting out of you like liquid gold, and the sun beating down on your back.");
...
// Naga Threesome (Z)
function nagaThreesomeWithSandTrap(): void {
    clearOutput();
    spriteSelect(97);
    // Requirements: Player is naga with tail and fangs, has met desert naga as naga at least once
...
// Leave:
function leaveSandTrapBadEnd(): void {
    clearOutput();
    spriteSelect(97);
...
// Lose fight:
function loseLastFightWithSandTrap(): void {
    clearOutput();
    spriteSelect(97);
...
// Both go to:
function sandTrapBadEndFinale(): void {
    spriteSelect(97);
    ...
    if (player.gender > 0) {
        ...
        if (player.cocks.length > 0) outputText("cock harden");
