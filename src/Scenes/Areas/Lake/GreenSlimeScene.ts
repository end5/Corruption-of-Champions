...
export class GreenSlimeScene extends AbstractLakeContent {
    ...
    private serviceLowCorruptionHighLust(): void {
        ...
        if (player.vaginas.length > 0) {
            outputText("As you rub the creature's shaft you toy with the idea of inserting the thing into your " + vaginaDescript(0) + ", biting your lip slightly and sliding forward a bit to rub yourself against the slime's " + cockDescriptShort(monster, 0) + ".  You moan a bit and lose yourself for a moment, caught between desire ", false);
        ...
        if (player.tallness < 82) {
            ...
            if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
                ...
                if (player.cocks.length > 1) {
                    if (player.cocks.length > 0) outputText("Eventually your own arousal becomes unbearable and you reach down to work your one of your " + multiCockDescriptLight(game.player) + ", massaging the monster's swollen, throbbing head with your other hand.  You gasp slightly as soon as you wrap your hand around your " + cockDescript(game.player, 1) + ", the creature's fluid making you exceptionally sensitive.  You moan in spite of yourself and start to thrust your hips a bit against your fist, feeling both yourself and the creature begin to throb harder and harder.  Before long you can hold out no longer, and feel your body racked by an orgasmic spasm, squeezing tightly with both hands as you blow your " + multiCockDescriptLight(game.player) + " cover the creature with cum.  ", false);
            ...
            // female or herm version (if the herm didn't trigger male)
                ...
                if (player.cockTotal() > 0) {
                    ...
                    // Single prick
                        ...
                        if (player.biggestTitSize() == 0) outputText("You delicately grasp the head of your " + cockDescript(game.player, 0) + ", your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.", false);
                    }
                }
        ...
        // PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
            ...
            if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
                ...
                // SingleCock
                    ...
                    // Compare cocks
                    if (player.cocks[0].cockType == CockTypesEnum.HUMAN) outputText("a near exact replica of your own " + cockDescript(game.player, 0) + "!  ", false);
            ...
            // Female/Herm Masturbation
                ...
                if (player.cockTotal() > 0) {
                    ...
                    if (player.biggestTitSize() == 0 || player.breastRows.length == 0) {
                        outputText("You begin to moan as the creature rubs the " + vaginaDescript(0) + " beneath your " + multiCockDescript(game.player) + ", working you with incredible skill, but sadly ignoring your needy ", false);
                        // multi!
    ...
    private maleRapesOoze(): void {
        ...
        // anal rape text
            ...
            // Single dicks
            else outputText("You shrug at the lack of obvious orifices and decide to go for the most common one, pulling out your " + cockDescript(game.player, 0) + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  ", false);
            ...
            if (player.cocks[0].cockLength >= 24) outputText("By the time you finally bottom out, your " + cockDescript(game.player, 0) + " is nearly halfway through its torso.", false);
            else if (player.cocks[0].cockLength >= 48) outputText("By the time you finally bottom out, your " + cockDescript(game.player, 0) + " reaches nearly into the slime's neck.", false);
            else if (player.cocks[0].cockLength >= 60) outputText("Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + cockDescript(game.player, 0) + ".", false);
            ...
            outputText("  The thing seems to clamp down on you even tighter as you pull out, and you relish the sensation.  Its tight ass leaves your " + cockDescript(game.player, 0) + " almost perfectly clean afterward, a glob of white still stuck inside the creature as it changes back into its original, amorphous state.  The cloud disperses as the slime slinks off.", false);
        }
        player.orgasm();
        dynStats("sen", 3);
    }
    ...
    private femaleRapesOoze(): void {
        ...
        if (player.cocks.length > 0) {
            outputText(" your " + vaginaDescript(0) + " growing wet and your " + cockDescript(game.player, 0) + " growing firm as you imagine it inside you.  ", false);
        }
        else {
        ...
        if (player.cockTotal() > 0) {
            if (player.cockTotal() > 1) {
                ...
                outputText("Your " + cockDescript(game.player, 0) + " explodes into orgasm and releases thick streams of semen onto both you and the creature, your mind blanking at the release.  You vaguely notice your ejaculate sinking into the creature's body and getting absorbed as you give yourself to pleasure.  ", false);
            }
        }
    ...
    private oozeRapesYouVaginally(): void {
        ...
        if (player.cor <= 20) {
            ...
            if (player.cockTotal() > 0) {
                ...
                // Single dick
                else outputText("Your " + cockDescript(game.player, 0) + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, rapidly stroking your " + cockDescript(game.player, 0) + " in time with the creature's movements.  ", false);
            }
    ...
    public rapeOozeWithMilk(): void {
        outputText("", true);
        outputText("You look over the ooze, wondering what to do about your need to nurse now that it has lost cohesion. After a while of puzzling things out, you decide to wing it, removing the top of your " + player.armorName + " and pressing the mess of a monster to your " + breastDescript(game.player, 0) + " and giving it a squeeze to get the milk to it. The slime responds almost immediately, applying pressure from the base of your " + breastDescript(game.player, 0) + " to the tip of your " + nippleDescript(0) + ", earning it a shot of milk to your immense satisfaction. As it tends to your " + nippleDescript(0) + ", it slowly works its way down your body, almost lovingly ", false);

        // [If male-
        if (player.gender == 1) {
            outputText("caressing your " + multiCockDescriptLight(game.player), false);
            if (player.balls > 0) {
