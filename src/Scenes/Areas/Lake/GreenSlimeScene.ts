...
export class GreenSlimeScene extends AbstractLakeContent {
    ...
    private serviceLowCorruptionHighLust(): void {
        ...
        if (player.vaginas.length > 0) {
            outputText("As you rub the creature's shaft you toy with the idea of inserting the thing into your " + vaginaDescript(player, 0) + ", biting your lip slightly and sliding forward a bit to rub yourself against the slime's " + cockDescriptShort(monster, 0) + ".  You moan a bit and lose yourself for a moment, caught between desire ", false);
        ...
        if (player.tallness < 82) {
            ...
            if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
                ...
                if (player.cocks.length > 1) {
                    ...
                    if (player.vaginas.length > 0) outputText("Your " + vaginaDescript(player, 0) + " quivers and shares in the sensation as you grind yourself against the slime.  ", false);
                    // Create cumz
                ...
                // Single cocks
                    ...
                    if (player.cocks[0].cockLength <= 30) {
                        ...
                        // Herm orgasm text
                        if (player.vaginas.length > 0) {
                            outputText("Your " + vaginaDescript(player, 0) + " quivers and shares in the sensation as you grind yourself against the slime.  ", false);
                        }
                        outputText("The creature reaches its peak as well, and a thick, powerful gout of more green fluid shoots from the slime's cock and into your waiting palm.  You shudder as your orgasm seems to drag on for longer than usual, and notice that the slime's body regains some of its definition for a moment before its cock quickly recedes back into its body.", false);
                    }
                    // FUGGIN' 'UGE PRICKS!
                        ...
                        outputText("Instinctively you hold onto the creature's own shaft for support as you do and with a final mighty pulse it also comes, spraying its thin green fluid in a tall gout into the air.  You feel it splash onto your back and " + hairDescription(player) + " as you gasp for breath, before almost falling over as the beast's erection rapidly recedes into its own body.", false);
                    }
                }
                // Newline PG
                outputText("\n\n", false);
            ...
            // female or herm version (if the herm didn't trigger male)
                ...
                outputText("Eventually your own arousal becomes unbearable and you let one of your hands slide down the creature's erection and body and up your leg, purring softly as a slight warm sensation spreads everywhere you get the green fluid the creature has been so readily leaking.  You let your hand crawl between your legs and gasp slightly as you run a hand over the lips of your " + vaginaDescript(player, 0) + ", savoring the sensation before going to work on yourself.  ", false);
                // Big clitties
                if (player.clitLength >= 2) outputText("You moan and lean back slightly as you grip your " + clitDescription(player) + " between your thumb and forefinger, biting your lip at the intense sensations as you rub your thumb over your " + clitDescription(player) + " and shuddering in anticipation and delight.  ", false);
                // Small clitties
                else outputText("You moan and lean back slightly as you start to flick your " + clitDescription(player) + ", the slime's fluid providing excellent lubrication and a luxurious warming feeling as you make small circles around your nub.  ", false);
        ...
        // PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
            ...
            if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
                ...
                if (player.vaginas.length > 0) {
                    outputText("The intense sensations ripple through your entire body, and your " + vaginaDescript(player, 0) + "grows wet with arousal.  You push against the creature and rhythmically slide back and forth, its soft, moist body conforming to your folds and creating a wonderfully arousing sensation.", false);
                    outputText("\n\nIt isn't long before you can no longer hold yourself back and with a groan, you explode, showering the creature with your seed as your " + multiCockDescript(game.player) + " and " + vaginaDescript(player, 0) + " simultaneously explode into orgasm.  The ooze's penis also seems to swell and a torrent of the green fluid gushes from the tip, creating a large, slippery pool that coats your bottom half and forms a small pool on the ground around you before soaking into the earth.  As you catch your breath the creature's cock begins to lose definition and then recedes as the thing pulls itself back into a blob and slides out from under you, quietly escaping into the nearby water as you watch.  As you get up, you notice a still partly solid blob of green gel on the ground, and pick it up.", false);
                }
                // Pure male orgasm
            ...
            // Female/Herm Masturbation
                ...
                if (player.breastRows.length > 0 && player.biggestTitSize() > 0) {
                    ...
                    if (player.biggestTitSize() > 8) outputText("Its hand expands, struggling to cover all of your " + allBreastsDescript(player) + ", ", false);
                    // Middling ta-tas get covered
                    else if (player.biggestTitSize() > 3) outputText("Its hand expands and gently covers your " + allBreastsDescript(player) + ", ", false);
                    // Small tits?  Pwned.
                ...
                // Just clits, no cocks.
                    ...
                    // since players without breasts are wholly surprised here, this needs to be acknowledged.
                    if (player.biggestTitSize() == 0 || player.breastRows.length == 0) outputText("You begin to moan as the creature rubs your " + vaginaDescript(player, 0) + ", working you with incredible skill, but what comes next is an utter surprise, making you unconsciously wring the head of the slime's cock.  ", false);
                    // Clit engulph'ed!
                    outputText("The creature's smooth, gel-like body envelopes your " + clitDescription(player) + " and slowly pulsates, stroking it from multiple angles at once and making you gasp with pleasure.  You're torn with the urges to both thrust your hips towards it to seek more pleasure and to draw away from the sheer intensity of the sensations, and settle for helplessly moaning as your hands unthinkingly tighten and squeeze around the head of the creature's cock, causing it to release a massive stream of green fluid across your chest and body.", false);
                }
                // New PG, CUM!
    ...
    private maleRapesOoze(): void {
        ...
        if (rand(2) == 0) {
            ...
            // Balls ftw
            if (player.balls > 1) outputText(" as your " + ballsDescriptLight(player) + " slap against its face", false);
            ...
            // vaginal orgasm text
            if (player.vaginas.length > 0) outputText("  This is quickly followed by a quivering between your nether-lips as your " + vaginaDescript(player, 0) + " quivers in orgasm shortly afterwards.", false);
    ...
    private femaleRapesOoze(): void {
        ...
        if (player.cocks.length > 0) {
            outputText(" your " + vaginaDescript(player, 0) + " growing wet and your " + cockDescript(game.player, 0) + " growing firm as you imagine it inside you.  ", false);
        }
        else {
    ...
    private oozeButtRapesYou(): void {
        ...
        // Girls get fondled.
        if (player.vaginas.length > 0) outputText(vaginaDescript(player, 0) + " and " + assholeDescript(player), false);
        // No coochie
        else {
            // Ballsy
            if (player.balls >= 2) outputText(assholeDescript(player) + " and " + ballsDescript(player), false);
        ...
        // Vaginas!
        if (player.vaginas.length > 0) outputText("Your " + vaginaDescript(player, 0) + " grows wet as the creature continues to assault you, and your cleft begins to ache for a soft touch.  As if sensing your thoughts the creature obligingly wraps a hand around and under you and begins alternating between sliding along your gash and stroking your " + clitDescription(player) + ".", false);
        // New PG
        outputText("\n\n", false);
    ...
    public rapeOozeWithMilk(): void {
        ...
        if (player.gender == 1) {
            ...
            if (player.balls > 0) {
                outputText(", and slipping slightly further down to engulf your " + ballsDescriptLight(player) + " as well. ", false);
            }
            else outputText(". ", false);
        }
        // [if female-
        else if (player.gender == 2) {
            outputText("manipulating your " + clitDescription(player) + ". ", false);
        }
        // [If Herm-
            ...
            if (player.balls > 0)
                outputText("and slipping slightly further down to engulf your " + ballsDescriptLight(player) + " as well. ", false);
            else outputText("and almost dripping down to coat your " + clitDescription(player) + " to add to your pleasure. ", false);
        }
        // [If Genderless-
        else {
            ...
            outputText("\n\n", false);
            outputText("The barrage of pleasurable feelings causes you to fall over onto your " + buttDescription(player) + " and just soak in them, your hands ", false);
        }
        ...
        // [if female-
        else if (player.gender == 2) outputText(clitDescription(player) + " and ", false);
        // [if Herm-
        else if (player.gender == 3) {
            outputText(multiCockDescriptLight(game.player), false);
        ...
        // [if female-
        else if (player.gender == 2) outputText("rapidly entering it in and out of your " + vaginaDescript(player, 0) + " by way of your fingers.  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you release your juices into it.  You flick it off your fingers ", false);
        // [if herm-
        else if (player.gender == 3) outputText("feverishly jacking yourself off with it while rapidly entering it in and out of your " + vaginaDescript(player, 0) + " by way of your fingers.  The cool feel of it contrasts enough to send you over the edge relatively quickly.  You release your cum into it while it absorbs the juices from your " + vaginaDescript(player, 0) + ".  You drop some of its bulk ", false);
        // [if genderless-
        else outputText("rubbing it over your " + assholeDescript(player) + ".  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you drop it as you feel your muscles spasm with the phantom orgasm ", false);

        outputText("while using the rest of it to coat your " + nippleDescription(player, 0) + " in order to give it the last of your milk.\n\n", false);
