...
private  serviceLowCorruptionHighLust():void
{
	...
	{
		outputText("As you rub the creature's shaft you toy with the idea of inserting the thing into your " + vaginaDescript(0) + ", biting your lip slightly and sliding forward a bit to rub yourself against the slime's " + monster.cockDescriptShort(0) + ".  You moan a bit and lose yourself for a moment, caught between desire ", false);
	...
	if(player.cockTotal() > 0) {
		...
		if (player.cockTotal() > 1) outputText("Your own erect " + game.player.multiCockDescriptLight() + " rub against the creature's as you work, getting in the way somewhat.  ", false);
		//Big single cocked folk get in their own way too!
		else if(player.cocks[0].cockThickness >= 36) outputText("Your own erect " + game.player.cockDescript(0) + " rubs against the creature's as you work, getting in the way somewhat.  ", false);
		//Single dick contemplation
		if(player.cockTotal() == 1) outputText("You feel your " + game.player.cockDescript(0) + " begin to throb and entertain the idea of trying to penetrate it somehow.  You turn a bit and look around the creature for an orifice, but find nothing.  After a few frustrated moments of searching you give up and decide to handle things yourself.", false);
		//Multidick contemplation
		else outputText("You feel your " + game.player.multiCockDescriptLight() + " begin to throb and entertain the idea of trying to penetrate it somehow.  You turn a bit and look around the creature for an orifice, but find nothing.  After a few frustrated moments of searching you give up and decide to handle things yourself.", false);
		outputText("\n\n", false);
	}
	//Next PG
	//Get it hard
	...
	if(player.tallness < 82)
	{
		...
		if (player.gender == 1 || (player.gender == 3 && rand(2) == 0))
		{
			...
			{
				if(player.cocks.length > 0) outputText("Eventually your own arousal becomes unbearable and you reach down to work your one of your " + game.player.multiCockDescriptLight() + ", massaging the monster's swollen, throbbing head with your other hand.  You gasp slightly as soon as you wrap your hand around your " + game.player.cockDescript(1) + ", the creature's fluid making you exceptionally sensitive.  You moan in spite of yourself and start to thrust your hips a bit against your fist, feeling both yourself and the creature begin to throb harder and harder.  Before long you can hold out no longer, and feel your body racked by an orgasmic spasm, squeezing tightly with both hands as you blow your " + game.player.multiCockDescriptLight() + " cover the creature with cum.  ", false);
				//Herms - vegemita vaga-cum.
				if(player.vaginas.length > 0) outputText("Your " + vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ", false);
				//Create cumz
			...
			else 
			{
				...
				{
					outputText("Eventually your own arousal becomes unbearable and you reach down to work your own " + game.player.cockDescript(0) + " as well, massaging the monster's swollen, throbbing head with your other hand.  You gasp slightly as soon as you wrap your hand around your " + game.player.cockDescript(0) + ", the creature's fluid making you exceptionally sensitive.  You moan in spite of yourself and start to thrust your hips a bit against your fist, feeling both yourself and the creature begin to throb harder and harder.  Before long you can hold out no longer, and feel your body racked by an orgasmic spasm, squeezing tightly with both hands as you blow your load onto the creature.  ", false);
					//Herm orgasm text
					if(player.vaginas.length > 0)
					{
						outputText("Your " + vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ", false);
					}
					outputText("The creature reaches its peak as well, and a thick, powerful gout of more green fluid shoots from the slime's cock and into your waiting palm.  You shudder as your orgasm seems to drag on for longer than usual, and notice that the slime's body regains some of its definition for a moment before its cock quickly recedes back into its body.", false);
				}
				//FUGGIN' 'UGE PRICKS!
		...
		else if (player.gender == 2 || player.gender == 3)
		{
			...
			//Big clitties
			if(player.clitLength >= 2) outputText("You moan and lean back slightly as you grip your " + clitDescript() + " between your thumb and forefinger, biting your lip at the intense sensations as you rub your thumb over your " + clitDescript() + " and shuddering in anticipation and delight.  ", false);
			//Small clitties
			else outputText("You moan and lean back slightly as you start to flick your " + clitDescript() + ", the slime's fluid providing excellent lubrication and a luxurious warming feeling as you make small circles around your nub.  ", false);
			...
			{
				outputText("You gasp a little as the slime's massive arms reach up from behind you to cup your " + allBreastsDescript() + ", and the soft material ", false);
				//Tig ol' bitties
			...
			if (player.cockTotal() > 0)
			{
				...
				{
					outputText("You feel your " + game.player.multiCockDescriptLight() + " surge to their full size as the slime works you over.  They quickly begin to throb and ache for release, a need made only more pressing by your desire to have it fondled.  ",false);
				...
				{
					outputText("You feel your " + game.player.cockDescript(0) + " surge to full size as the slime works you over.  It begins to throb and ache for release, a need made only more pressing by your desire to have it fondled.  ",false);
					//Play with it if your boobs aren't being manhandled
					if(player.biggestTitSize() == 0) outputText("You delicately grasp the head of your " + game.player.cockDescript(0) + ", your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.", false);
				}
			}
			...
			{
				//Multidicked herms
				if (player.cockTotal() > 1)	outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + game.player.multiCockDescript() + " explodes into orgasm, showering both you and the creature in jizz.  ", false);
				//Single cocked herms
				else outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + game.player.cockDescript(0) + " explodes into orgasm, sending a jet of your warm jizz into the air.  ",false);
			}
	...
	else if(player.tallness >= 82)
	{
		...
		{
			//Multicock foreplay
			if(player.cockTotal() > 1) outputText("Eventually your own arousal becomes unbearable and you reach down to work one of the shafts in your " + game.player.multiCockDescriptLight() + ", as well, continuing to run your other hand up and down the creature's length.  You gasp slightly as soon as you wrap your hand around your " + game.player.cockDescript(1) + ", the creature's fluid making you exceptionally sensitive.  You moan in spite of yourself and start to thrust your hips a bit against your fist, stroking yourself and the creature from crest to root.  The creature's erection gains definition as it starts pulsing harder and faster, turning into a distinctly human penis.  ", false);
			//SingleCock
			else
			{
				outputText("Eventually your own arousal becomes unbearable and you reach down to work your own " + game.player.cockDescript(0) + " as well, continuing to run your other hand up and down the creature's length.  You gasp slightly as soon as you wrap your hand around your " + game.player.cockDescript(0) + ", the creature's fluid making you exceptionally sensitive.  You moan softly in spite of yourself and begin to thrust your hips a bit against your fist, stroking yourself and the creature from crest to root.  The creature's erection gains definition as it starts pulsing harder and faster, turning into ", false);
				//Compare cocks
				if(player.cocks[0].cockType == CockTypesEnum.HUMAN) outputText("a near exact replica of your own " + game.player.cockDescript(0) + "!  ", false);
			...
			if(player.vaginas.length > 0)
			{
				outputText("The intense sensations ripple through your entire body, and your " + vaginaDescript(0) + "grows wet with arousal.  You push against the creature and rhythmically slide back and forth, its soft, moist body conforming to your folds and creating a wonderfully arousing sensation.", false);
				outputText("\n\nIt isn't long before you can no longer hold yourself back and with a groan, you explode, showering the creature with your seed as your " + game.player.multiCockDescript() + " and " + vaginaDescript(0) + " simultaneously explode into orgasm.  The ooze's penis also seems to swell and a torrent of the green fluid gushes from the tip, creating a large, slippery pool that coats your bottom half and forms a small pool on the ground around you before soaking into the earth.  As you catch your breath the creature's cock begins to lose definition and then recedes as the thing pulls itself back into a blob and slides out from under you, quietly escaping into the nearby water as you watch.  As you get up, you notice a still partly solid blob of green gel on the ground, and pick it up.", false);     
			}
			//Pure male orgasm
		...
		else if(player.gender == 2 || player.gender == 3)
		{
			...
			if(player.breastRows.length > 0 && player.biggestTitSize() > 0)
			{
				...
				if(player.biggestTitSize() > 8) outputText("Its hand expands, struggling to cover all of your " + allBreastsDescript() + ", ", false);
				//Middling ta-tas get covered
				else if(player.biggestTitSize() > 3) outputText("Its hand expands and gently covers your " + allBreastsDescript() + ", ", false);
				//Small tits?  Pwned.
				else outputText("Its hand easily smothers your " + allBreastsDescript() + ", ", false);
			...
			if(player.cockTotal() > 0)
			{
				...
				{
					outputText("You begin to moan as the creature rubs the " + vaginaDescript(0) + " beneath your " + game.player.multiCockDescript() + ", working you with incredible skill, but sadly ignoring your needy ", false);
					//multi!
			...
			{
				//since players without breasts are wholly surprised here, this needs to be acknowledged.
				if(player.biggestTitSize() == 0 || player.breastRows.length == 0) outputText("You begin to moan as the creature rubs your " + vaginaDescript(0) + ", working you with incredible skill, but what comes next is an utter surprise, making you unconsciously wring the head of the slime's cock.  ", false);
				//Clit engulph'ed!
				outputText("The creature's smooth, gel-like body envelopes your " + clitDescript() + " and slowly pulsates, stroking it from multiple angles at once and making you gasp with pleasure.  You're torn with the urges to both thrust your hips towards it to seek more pleasure and to draw away from the sheer intensity of the sensations, and settle for helplessly moaning as your hands unthinkingly tighten and squeeze around the head of the creature's cock, causing it to release a massive stream of green fluid across your chest and body.", false);
			}
			//New PG, CUM!
...
private  maleRapesOoze():void {
	...
	if(rand(2) == 0)
	{
		...
		else outputText("Shrugging, you decide to go with the closest thing available and walk towards its head, pulling out your " + game.player.cockDescript(0) + " as you do so.  You kneel over the thing's head and stroke your penis lightly to ensure it is fully ready,  ", false);
		//Balls ftw
		...
		if(player.vaginas.length > 0) outputText("  Your " + vaginaDescript(0) + " begins to grow wet as you repeatedly force yourself into the creature's throat.  You take a single deep stroke and bottom out into the creature, grinding your moist femininity against the soft, sensual material of the creature's face.", false);
		//New PG for cumming!
		outputText("\n\n", false);
		...
		//vaginal orgasm text
		if(player.vaginas.length > 0) outputText("  This is quickly followed by a quivering between your nether-lips as your " + vaginaDescript(0) + " quivers in orgasm shortly afterwards.", false);
		outputText("  You pull your " + game.player.cockDescript(0) + " out of the thing's face slowly, relishing the sensation, producing an audible *pop* when your tip finally exits the creature.  A satisfied smirk crosses your face as the defeated beast's erection shrinks back into itself before sliding off, back into the nearby water.", false);
	}
	//anal rape text
	else
	{
		...
		if(player.cockTotal() > 1)
		{
			...
			outputText(" of your" + game.player.multiCockDescriptLight() + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  ", false);
		}
		//Single dicks
		else outputText("You shrug at the lack of obvious orifices and decide to go for the most common one, pulling out your " + game.player.cockDescript(0) + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  ", false);
		...
		if(player.cocks[0].cockLength >= 24) outputText("By the time you finally bottom out, your " + game.player.cockDescript(0) + " is nearly halfway through its torso.", false);
		else if(player.cocks[0].cockLength >= 48) outputText("By the time you finally bottom out, your " + game.player.cockDescript(0) + " reaches nearly into the slime's neck.", false);
		else if(player.cocks[0].cockLength >= 60) outputText("Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + game.player.cockDescript(0) + ".", false);
		...
		//vaginal arousal text
		if(player.vaginas.length > 0) outputText("Your " + vaginaDescript(0) + " grows wet as you work the thing's hole, every slap of your hips against the thing's ass sending a pang of pleasure through your nethers.  ", false);
		...
		if(player.cockTotal() > 1) outputText("your " + game.player.multiCockDescriptLight() + " explodes into orgasm, your load filling the space inside the creature and spraying in thick streams across its back.", false);
		else outputText("exploding inside it, your load filling the space around your cock with a white, milky fluid.", false);
		//vaginal orgasm text
		if(player.vaginas.length > 0) outputText("  As you pump your jizz into the creature an almost electrical shock runs through your body, starting from your " + vaginaDescript(0) + " and radiating outwards.", false);

		outputText("  The thing seems to clamp down on you even tighter as you pull out, and you relish the sensation.  Its tight ass leaves your " + game.player.cockDescript(0) + " almost perfectly clean afterward, a glob of white still stuck inside the creature as it changes back into its original, amorphous state.  The cloud disperses as the slime slinks off.", false);
	}
	player.orgasm();
	dynStats("sen", 3);
}
...
private  femaleRapesOoze():void {
	...
	{
		outputText(" your " + vaginaDescript(0) + " growing wet and your " + game.player.cockDescript(0) + " growing firm as you imagine it inside you.  ", false);
	}
	else
	{
	...
	outputText("Before long you cannot take the temptation anymore and lift your hind end into the air, moaning softly as you rub the tip of its cock against your " + vaginaDescript(0) + ".", false);
	//defloration text
	...
	if(player.cockTotal() > 0)
	{
		if(player.cockTotal() == 1)
		{
			outputText("  Your " + game.player.cockDescript(0) + " grows hard as you are penetrated, and with the creature handling the movement you reach down to jack yourself off.", false);
		}
		else
		{
	...
	if(player.cockTotal() > 0)
	{
		...
		{
			outputText("Your " + game.player.cockDescript(0) + " explodes into orgasm and releases thick streams of semen onto both you and the creature, your mind blanking at the release.  You vaguely notice your ejaculate sinking into the creature's body and getting absorbed as you give yourself to pleasure.  ", false);
		}
	}
...
private  oozeButtRapesYou():void 
{
	...
	//No coochie
		...
		//Ballsy
		if(player.balls >= 2) outputText(assholeDescript()+ " and " + ballsDescript(), false);
	...
	if(player.cockTotal() > 0)
	{
		...
		//Multi!
		else outputText("Your " + game.player.multiCockDescript() + " quickly grows erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub at a different cock with each hand as it pumps you, and quickly approach an orgasm.  ", false);
	}
	//Vaginas!
 	if(player.vaginas.length > 0) outputText("Your " + vaginaDescript(0) + " grows wet as the creature continues to assault you, and your cleft begins to ache for a soft touch.  As if sensing your thoughts the creature obligingly wraps a hand around and under you and begins alternating between sliding along your gash and stroking your " + clitDescript() + ".", false);
 	//New PG
	outputText("\n\n", false);
...
private  oozeRapesYouOrally():void
{
	...
	if(player.cor >= 70)
	{
		...
		if(player.cockTotal() > 0)
		{
			if(player.lust > 40) outputText("Your erect " + game.player.multiCockDescriptLight() + " throbs as you work the creature, a surge of even greater arousal flowing through your body.  You feel almost desperate for release and even thrust your hips impotently against the empty air, wishing for some form of release.  You try to reach for yourself to masturbate, but find yourself unable to reach around the slime's massive bulk.  ", false);
			else outputText("You feel your " + game.player.multiCockDescriptLight() + " begin to harden just slightly as you work the creature, a surge of arousal flowing through your body.", false);
		}
		if(player.vaginas.length > 0) {
...
private  oozeRapesYouVaginally():void
{
	...
	if(player.cor <= 20)
	{
		...
		if(player.cockTotal() > 0)
		{
			...
			if(player.cockTotal() > 1)
			{
				outputText("Your " + game.player.multiCockDescript() + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, ", false);
			...
			//Single dick
			else outputText("Your " + game.player.cockDescript(0) + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, rapidly stroking your " + game.player.cockDescript(0) + " in time with the creature's movements.  ", false);
		}
		...
		{
			//Multi
			if(player.cockTotal() > 1) outputText("Your " + game.player.multiCockDescript() + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.", false);
			//Single
			else outputText("Your member climaxes as well, and you pull a hand to cover your eyes and face as your jism explodes onto yourself.  Your other hand slides to the base of your " + game.player.cockDescript(0) + " and squeezes as you coat your chest in semen.", false);
		}
		player.orgasm();
		dynStats("sen", 4);
	}
	...
	else
	{
		...
		outputText("You arch your back in pleasure as the slime begins moving, slowly pumping your wet " + vaginaDescript(0) + ".  It picks up speed and a small amount of the thing's greenish fluid builds up around your lower mouth, making your skin tingle and filling you with an unusually strong arousal.", false);
		//Herm text goes here
		if(player.cockTotal() > 0)
		{
			//Multi herms
		...
		{
			//multi
			if(player.cockTotal() > 1) outputText("Your " + game.player.multiCockDescript() + " climaxes as well, and you open your mouth as you spray rope after rope of hot, thick jism onto your face and chest.  You relish its salty taste, continuing to stroke yourself even as you come.  ", false);
			//single cock
			else outputText("Your own member finally reaches its climax as well, and you open your mouth as jism explodes onto your face and chest.  Your hands slide to the base of your " + game.player.cockDescript(0) + " and squeeze as you release thick ropes of ejaculate onto yourself, coating your face and chest as you relish its salty taste.  ", false);
		}
...
public  rapeOozeWithMilk():void {
	outputText("", true);
	outputText("You look over the ooze, wondering what to do about your need to nurse now that it has lost cohesion. After a while of puzzling things out, you decide to wing it, removing the top of your " + player.armorName + " and pressing the mess of a monster to your " + game.player.breastDescript(0) + " and giving it a squeeze to get the milk to it. The slime responds almost immediately, applying pressure from the base of your " + game.player.breastDescript(0) + " to the tip of your " + nippleDescript(0) + ", earning it a shot of milk to your immense satisfaction. As it tends to your " + nippleDescript(0) + ", it slowly works its way down your body, almost lovingly ", false);

	// [If male- 
	if(player.gender == 1) 
	{ 
		outputText("caressing your " + game.player.multiCockDescriptLight(), false);
		if(player.balls > 0) 
		{
			outputText(", and slipping slightly further down to engulf your " + ballsDescriptLight() + " as well. ", false);
		}
	...
	{ 
		outputText("manipulating your " + clitDescript() + ". ", false);
	}
	//[If Herm- 
	...
	{ 
		...
		if(player.balls > 0) 
			outputText("and slipping slightly further down to engulf your " + ballsDescriptLight() + " as well. ", false);
		else outputText("and almost dripping down to coat your " + clitDescript() + " to add to your pleasure. ", false); 
	}
	//[If Genderless- 
	else {
	...
	//[if female or Herm- 
	else outputText("Your hands fly to your " + vaginaDescript(0) + " and frantically plunge your fingers in and out of it.  You wish the slime would engulf it as well.  ", false);
	
	outputText("You cum many times into the mass, but that's not what truly matters to you. It's an extremely welcome, mind-shatteringly satisfying bonus, but not the main event. What does matter is the slow, meticulous draining of milk from both of your " + game.player.breastDescript(0) + " at once that the ooze is doing for you. Feeling that it's on the last of your milk, you urge the ooze on, trying to get it to crank up its work on your now-overly sensitive ", false);
	...
	//[if female- 
	else if(player.gender == 2) outputText(clitDescript() + " and ", false);
	//[if Herm- 
	else if (player.gender==3) {
		outputText(game.player.multiCockDescriptLight(), false);
		if(player.balls > 0) outputText(", " + ballsDescriptLight() + "," , false);
		outputText(" and " + clitDescript() + " and ", false);
	}
	outputText(game.player.breastDescript(0) + ".\n\n", false);
	...
	//[if female- 
	else if(player.gender == 2) outputText("rapidly entering it in and out of your " + vaginaDescript(0) + " by way of your fingers.  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you release your juices into it.  You flick it off your fingers ", false);
	//[if herm- 
	else if(player.gender == 3) outputText("feverishly jacking yourself off with it while rapidly entering it in and out of your " + vaginaDescript(0) + " by way of your fingers.  The cool feel of it contrasts enough to send you over the edge relatively quickly.  You release your cum into it while it absorbs the juices from your " + vaginaDescript(0) + ".  You drop some of its bulk ", false);
	//[if genderless- 
	else outputText("rubbing it over your " + assholeDescript() + ".  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you drop it as you feel your muscles spasm with the phantom orgasm ", false);
