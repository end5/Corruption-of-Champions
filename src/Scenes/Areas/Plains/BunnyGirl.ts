...
export class BunnyGirl {
    ...
    public bunnbunbunMeet(): void {
        ...
        // Met her
            ...
            outputText("You lean forwards for a better look, dislodging a small pebble with your " + player.foot() + " while shifting position.  The bunny-girl leaps to her feet in a panic until her terrified irises lock onto your " + face(player) + ".  Her panic turns to embarrassment and in a flash every inch of her tanned skin is colored red.  She asks, \"<i>Ummm, since I'm still in s-season, c-could we b-breed or something?  Maybe just a little fuck?</i>\"\n\n", false);
    ...
    private bunbunFucksYourVag(): void {
        ...
        if (player.cockTotal() > 0) {
            outputText("  " + game.player.SMultiCockDesc() + " twitches and drools ", false);
        ...
        if (player.cockTotal() > 0) {
            outputText("  At last, " + game.player.sMultiCockDesc() + " blasts sticky ropes of seed everywhere, cumming with you as your body slides sideways off the exhausted bunny into the dirt.  A few thick strands hit her tits and face, but judging by her half-closed eye-lids and questing tongue, she doesn't mind.", false);
            if (player.cumQ() > 500) outputText("  Not content with that, " + game.player.sMultiCockDesc() + " continues to pump until the bunny is coated with a generous layer of seed", false);
    ...
    private bunbunFucksPCInAss(): void {
        ...
        if (player.totalCocks() > 0) outputText("Each time she bumps and slides past your prostate a dollop of your pre-cum spurts from " + game.player.sMultiCockDesc() + " onto the ground, and you have a hard time not using your new lover like an anal toy and masturbating yourself.  ", false);
        ...
        if (player.cockTotal() > 0) {
            outputText(", and " + game.player.sMultiCockDesc() + " pouring incredibly sticky pre-cum each time she squeezes your prostate", false);
        }
        ...
        if (player.cockTotal() > 0) {
            outputText("and as it squeezes past your prostate, " + game.player.sMultiCockDesc() + " spurts thick cum on the ground, though you don't QUITE orgasm.", false);
        }
        ...
        if (player.cockTotal() > 0) {
            outputText("  " + game.player.SMultiCockDesc() + " erupts in truth, spraying and splattering white goop over the grass and ground.", false);
    ...
    private bunbunGetsFucked(): void {
        ...
        const x: number = player.cockThatFits(40);
        // Second wang that fits for DP
        const y: number = player.cockThatFits2(40);

        outputText("You disrobe and toss your " + player.armorName + " to the side, immediately forgetting about it as the bunny-girl lifts her sack to expose the bright pink flesh of her femininity.  It glistens, practically steaming up the air with her plentiful lubricants.  She runs a slender finger around the moist hole, beckoning you to plunge inside and fill her hungry flesh with your " + game.player.cockDescript(x) + ".  Lust burns through you as you drop to your knees and line your " + game.player.cockDescript(x) + " up with that ready opening", false);
        if (y != -1) outputText(", taking care to align your " + game.player.cockDescript(y) + " with her tightly puckered backdoor as well.", false);
        ...
        outputText("and your " + game.player.cockDescript(x), false);
        if (y != -1) outputText(" and " + game.player.cockDescript(y) + " sink", false);
        ...
        if (y != -1) outputText("Her tight asshole slowly loosens around your " + game.player.cockDescript(y) + ", and so much lubricant splatters from her cunt with each thrust that the sensations of both your dicks become nigh-identical, save for the tight ring gripping the bottom one.", false);
        else {
        ...
        outputText(" rubs her passion-inflamed pussy-lips.  " + game.player.SMultiCockDesc() + " unloads, splattering out ropes of cum into ", false);
    ...
    private getEggflated(): void {
        ...
        // Futa:
        if (player.hasCock()) outputText("  The steady movement of the orbs pushing past your prostate drills another, even messier orgasm from your " + game.player.multiCockDescriptLight() + ", cum arcing into the air to shower down on the two of you in pearl beads.");
        ...
        // Futa:
        if (player.hasCock()) {
            outputText("Your exhausted " + game.player.multiCockDescriptLight() + " somehow find" + (player.cocks.length > 1 ? "" : "s") + " the strength for a gushing orgasm of " + (player.cocks.length > 1 ? "their" : "its") + " own, strands of spoo spilling across your own body, white hot spunk bringing a fresh sheen to your sweat-slick [chest].  ");
        }
