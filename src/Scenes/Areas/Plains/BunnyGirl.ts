...
export function bunnbunbunMeet(): void {
    ...
    // Met her
        ...
        outputText("You lean forwards for a better look, dislodging a small pebble with your " + foot(player) + " while shifting position.  The bunny-girl leaps to her feet in a panic until her terrified irises lock onto your " + face(player) + ".  Her panic turns to embarrassment and in a flash every inch of her tanned skin is colored red.  She asks, \"<i>Ummm, since I'm still in s-season, c-could we b-breed or something?  Maybe just a little fuck?</i>\"\n\n", false);
        ...
        let DickInV: () => void = null;
        let Vagina: () => void = null;
        let sixtyNine: () => void = null;
        let eggs: () => void = null;
        if (player.cocks.cockThatFits(40) >= 0) {
...
function talkToBunnyBunBun(): void {
    ...
    else if (player.cocks.length > 0) outputText("  <b>You're too big to fit inside her...</b>", false);
    if (player.gender > 0) {
        sixtyNine = bunbun69;
        outputText("  Sixty-nine her?", false);
    }
...
function bunbunFucksYourVag(): void {
    ...
    if (player.vaginas[0].vaginalWetness >= VaginaWetness.DROOLING) outputText("  Moisture leaks in a steady drizzle, mixing with bunny-pre to totally slick the soon-to-be invading member.", false);
    outputText("\n\n", false);
    ...
    if (player.vaginas[0].vaginalWetness >= VaginaWetness.SLICK) outputText(", not that you needed it", false);
    ...
    if (player.breastRows.biggestLactation() >= 1) outputText("  Milk begins to bead on your " + allBreastsDescript(player) + " from the sensation of the bunny's brutal, almost mechanical fucking.", false);
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.length == 1) outputText("half's needs, slathering it in pre-cum and stroking hard and fast.", false);
...
function bunbunFucksPCInAss(): void {
    ...
    if (player.cocks.length > 0) outputText("Each time she bumps and slides past your prostate a dollop of your pre-cum spurts from " + sMultiCockDesc(game.player) + " onto the ground, and you have a hard time not using your new lover like an anal toy and masturbating yourself.  ", false);
    ...
    if (player.vaginas.length > 0) {
        if (player.vaginas[0].vaginalWetness >= VaginaWetness.SLICK) outputText("Juices begin to run from your " + vaginaDescript(player, 0) + ", but you ignore the empty feeling and focus on the throbbing meat inside your backdoor.  ", false);
    }
    ...
    if (!player.breastRows.hasFuckableNipples()) {
        ...
        if (player.breastRows.biggestLactation() >= 1) {
            ...
            if (player.breastRows.biggestLactation() >= 4) outputText("sprays", false);
    ...
    if (player.breastRows.biggestLactation() >= 1) {
        ...
        if (player.breastRows.biggestLactation() < 2) outputText("leaks", false);
        else if (player.breastRows.biggestLactation() < 4) outputText("drips", false);
...
function bunbunGetsFucked(): void {
    ...
    const x: number = player.cocks.cockThatFits(40);
    // Second wang that fits for DP
    const y: number = player.cocks.cockThatFits2(40);

    outputText("You disrobe and toss your " + player.armorName + " to the side, immediately forgetting about it as the bunny-girl lifts her sack to expose the bright pink flesh of her femininity.  It glistens, practically steaming up the air with her plentiful lubricants.  She runs a slender finger around the moist hole, beckoning you to plunge inside and fill her hungry flesh with your " + cockDescript(game.player, x) + ".  Lust burns through you as you drop to your knees and line your " + cockDescript(game.player, x) + " up with that ready opening", false);
    if (y != -1) outputText(", taking care to align your " + cockDescript(game.player, y) + " with her tightly puckered backdoor as well.", false);
    else outputText(", barely able to restrain yourself as the time for penetration approaches.", false);
    outputText("\n\n", false);

    outputText("The nubile bunny's eyes cross as you push forwards, sliding your " + cockHead(player, x) + " over her love-slicked lips until it has a generous coating and pressing harder.  ", false);
    ...
    outputText("and your " + cockDescript(game.player, x), false);
    if (y != -1) outputText(" and " + cockDescript(game.player, y) + " sink", false);
    else outputText(" sinks", false);
    outputText(" into her slippery, warm depths.  She pants and groans with happiness, going so far as to vocalize her pleasure, \"<i>Oooh yes, I don't know why but it feels just as good as breeding season!  If you keep this up, " + mf(player, "stud", "sexy") + ", you might... ahhhh.. make me egg!</i>\"\n\n", false);
    ...
    if (player.cocks.cockArea(x) >= 25) outputText("Her belly bulges pleasantly, displaying the vaguely cylindrical shape of a cock through her skin in a way that makes you feel utterly dominant over this 'female'.  ", false);
    ...
    if (y != -1) outputText("Her tight asshole slowly loosens around your " + cockDescript(game.player, y) + ", and so much lubricant splatters from her cunt with each thrust that the sensations of both your dicks become nigh-identical, save for the tight ring gripping the bottom one.", false);
    else {
    ...
    outputText(" rubs her passion-inflamed pussy-lips.  " + SMultiCockDesc(game.player) + " unloads, splattering out ropes of cum into ", false);
    if ((player.cocks.length == 2 && y == -1) || player.cocks.length > 2) outputText("and onto ", false);
    ...
    let z: number = 8 + rand(player.cumQ() / 100);
    if (z > 50) z = 50;
...
function bunbun69(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (isEaster()) {
            ...
            if (player.tailType != TailType.RABBIT) {
                // [No Tail:
                if (player.tailType == TailType.NONE) outputText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as <b>a fluffy bunny's tail pops out of your back</b>, twitching excitedly!", false);
                // [Existing tail:
        ...
        // REGULAR SHOW
            ...
            if (player.tailType != TailType.RABBIT) {
                // [No Tail:
                if (player.tailType == TailType.NONE) outputText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as a fluffy bunny's tail pops out of your back, twitching excitedly!", false);
                // Existing tail:
            ...
            if (player.earType != EarType.BUNNY) outputText("new ", false);
...
function getEggflated(): void {
    ...
    // Futa:
    if (player.cocks.length > 0) outputText("  The steady movement of the orbs pushing past your prostate drills another, even messier orgasm from your " + multiCockDescriptLight(game.player) + ", cum arcing into the air to shower down on the two of you in pearl beads.");
    ...
    // Futa:
    if (player.cocks.length > 0) {
        outputText("Your exhausted " + multiCockDescriptLight(game.player) + " somehow find" + (player.cocks.length > 1 ? "" : "s") + " the strength for a gushing orgasm of " + (player.cocks.length > 1 ? "their" : "its") + " own, strands of spoo spilling across your own body, white hot spunk bringing a fresh sheen to your sweat-slick [chest].  ");
    }
