...
export class GnollScene {
    ...
    public getRapedByGnoll(): void {
        ...
        if (player.cockTotal() > 0) {
            ...
            if (player.thickestCockThickness() > 2) {
                // (if cockTotal>1)
                if (player.cockTotal() > 1) outputText("The gnoll fishes into your " + game.player.multiCockDescriptLight() + " and quickly snaps up the thickest one.  ", false);
                // (if cockTotal=1)
                else outputText("The gnoll's hands dart down to your crotch and grabs " + game.player.cockDescript(0) + ".  ", false);
        ...
        // (if breastRating>C)
        if (player.biggestTitSize() >= 3) outputText("  Your " + player.allBreastsDescript() + " bounce up and down as the gnoll grinds her anatomy into yours, slick with sweat under the hot sun.  Your " + nippleDescript(0) + "s tighten and flush as your whole body submits to the rough fuck administered by the hyena girl.", false);
        // (if cockTotal>0)
        if (player.cockTotal() > 0) {
    ...
    private dickDownGnollClit(): void {
        ...
        if (player.cocks[x].cockLength > 9) outputText("Finally you can feel your cockhead push free into a deeper, wetter place.  You look down at her strange anatomy, the entire length of her tube bulging thicker to accommodate your " + game.player.cockDescript(x) + ".  You've made it all the way to her vagina!\n\n", false);
        else outputText("Soon you feel the end of her clitoral tunnel butting up against your crotch.  You are as far in as you'll get.  You look down the length of her strange anatomy, the tube bulging thicker to accomodate your " + game.player.cockDescript(x) + ".  There are still inches between your cockhead and the end of her clit-dick.\n\n", false);
        ...
        // (if cumQ()<25)
        if (player.cumQ() < 25) outputText("Your " + game.player.cockDescript(x) + " convulses, pumping down the length of her clitoris.  She squirms and howls beneath you, hissing for more.", false);
        // (if cumQ()>25 AND cumQ()<500)
        else if (player.cumQ() < 500) outputText("Your " + game.player.cockDescript(x) + " convulses, pumping cum into her.  Her clitoris bulges even more as the jism backs up.  The warm, wet, sticky insides swirl around your dick.", false);
        // (if cumQ()>500)
        else outputText("Your " + game.player.cockDescript(x) + " convulses, pouring cum into her.  Her clitoris bulges even more as the jism backs up, the skin pulling tight and shiny as the pressure builds.  Finally gouts of cum begin spurting back out the tip of her clitoris, splashing against your crotch.", false);
    ...
    private dickInGnollAss(): void {
        ...
        if (rand(2) == 0) {
            outputText("The gnoll must be taught a lesson, but you're staying the hell away from her freaky anatomy.  You roughly roll her onto her belly and pull her lean ass up into the air.  You line up your " + game.player.cockDescript(x) + " and ram it home into her tiny puckered entrance, eliciting a half-conscious gasp from the hyena girl.\n\n", false);
            // (if cockTotal>1)
            if (player.totalCocks() > 1 && player.cockArea(x) < monster.analCapacity() && y != -1) {
                ...
                // (if cockTotal>3)
                if (player.cockTotal() > 3 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) < monster.analCapacity()) outputText("  From there it's some time before her anus is stretched wide enough for more.  By now you can feel your orgasm on the horizon, so you're less careful with your fourth " + game.player.cockDescript(y) + ".  You slap it into the bundle of dicks and then ram it home.", false);
                // (if cockTotal>4)
                ...
                // (if cockTotal>6)
                if (player.cockTotal() > 6 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) + player.cockArea(4) + player.cockArea(5) + player.cockArea(6) < monster.analCapacity()) outputText("  And the next, until all of your " + game.player.multiCockDescriptLight() + " are inside her wide-spread ass.", false);
                outputText("\n\n", false);
            }
            ...
            outputText("You stroke your " + game.player.cockDescript(x) + " eagerly, bringing yourself to full mast and squeezing out a few drops of pre to pool against her puckered anus, leaning over and adding a little bit of saliva for good measure.  Sliding your shaft between her flat ass cheeks, you make sure to get the tip nice and lubed up, then you drop her hips down and ram yourself home in one go.\n\n", false);

            outputText("The gnoll lets out a squeal that sounds halfway like a maniacal laugh, sliding forward in the dirt a little.  Your " + game.player.cockDescript(x) + " throbs wonderfully, fully engulfed by the tight passage of the gnoll's anus", false);
            ...
            // Med CumQ):
                ...
                outputText("Gripping her sides tightly, you let out a bellowing moan that echos through the grasslands, " + game.player.sMultiCockDesc() + " swelling noticeably as you pump thick ribbons of spunk into the gnoll's innards", false);
                ...
                outputText(".  The muscular ring of her anus bears down on your " + game.player.cockDescript(x) + " as you pull out, making an audible *schlick!*, and the gnoll drops to the ground in exhaustion, shamelessly playing with herself in an effort to get off.\n\n", false);
            }
            // (High CumQ):
            else {
                outputText("You bear down on the limber hyena girl with all your force, groaning through clenched teeth as " + game.player.sMultiCockDesc() + " distends with the pressure of your virile load.  Her puckered anus stretches around the swell of your seed, and she lets out a shuddering, laughing moan as her belly begins to distend with your thick jism.  The involuntary spasms of her rectum milk your " + game.player.cockDescript(x) + " for every drop, clenching your member tightly", false);
                if (player.cockTotal() > 1) {
