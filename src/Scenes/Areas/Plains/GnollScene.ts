...
// Gnoll Entrance
export function gnollEncounter(): void {
    spriteSelect(11);
    outputText("", true);
...
export function getRapedByGnoll(): void {
    ...
    if (player.cocks.length > 0) {
        ...
        if (player.cocks.thickestCockThickness() > 2) {
            ...
            outputText("\"<i>Yes, this will do nicely,</i>\" she says with relish, pumping your " + cockNoun(player.cocks[player.cocks.thickestCock()].cockType) + " until it plumps up, filling with blood.  She cantilevers her body over yours, knees bent sharply, and brings the tip of her massive clitoris up against the head of your dick.  The moment a dollop of pre forms at your urethra, her hips surge forward, sinking her giant clit down the length of your " + cockNoun(player.cocks[player.cocks.thickestCock()].cockType) + ".  ", false);
    ...
    // (if breastRating>C)
    if (player.breastRows.biggestTitSize() >= 3) outputText("  Your " + allBreastsDescript(player) + " bounce up and down as the gnoll grinds her anatomy into yours, slick with sweat under the hot sun.  Your " + nippleDescription(player, 0) + "s tighten and flush as your whole body submits to the rough fuck administered by the hyena girl.", false);
    // (if cockTotal>0)
    if (player.cocks.length > 0) {
...
export function defeatHyena(): void {
    outputText("", true);
...
function dickDownGnollClit(): void {
    ...
    // (if cumQ()<25)
    if (player.cumQ() < 25) outputText("Your " + cockDescript(game.player, x) + " convulses, pumping down the length of her clitoris.  She squirms and howls beneath you, hissing for more.", false);
    // (if cumQ()>25 AND cumQ()<500)
    else if (player.cumQ() < 500) outputText("Your " + cockDescript(game.player, x) + " convulses, pumping cum into her.  Her clitoris bulges even more as the jism backs up.  The warm, wet, sticky insides swirl around your dick.", false);
    // (if cumQ()>500)
    else outputText("Your " + cockDescript(game.player, x) + " convulses, pouring cum into her.  Her clitoris bulges even more as the jism backs up, the skin pulling tight and shiny as the pressure builds.  Finally gouts of cum begin spurting back out the tip of her clitoris, splashing against your crotch.", false);
