...
function keepDrinking(): void {
    ...
    // [hymen check]
    if (player.vaginas.length > 0) cuntChange(player, 25, true, true, false);
    else buttChange(player, 25, true, true, false);
...
export function loseToSatyr(): void {
    ...
    else buttChange(player, monster.cocks.cockArea(0), true, true, false);
...
function femaleTakesAdvantageOfSatyr(): void {
    ...
    if (player.cocks.length > 0) outputText(", your own cock belching its usual load across his stomach and the surroundings");
    outputText(".");
...
export function satyrBirth(vag: boolean): void {
    ...
    if (vag) cuntChange(player, 40, true, false, true);
    else buttChange(player, 40, true, false, true);
