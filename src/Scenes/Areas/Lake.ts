...
export function exploreLake(): void {
    ...
    if (player.level >= 2 && player.effects.findByType(StatusAffects.DungeonShutDown) >= 0)
        choice[choice.length] = 3;
    // Slimes/Ooze = level >= 2
    if (player.level >= 2)
        choice[choice.length] = 4;
    // Izma
    if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00230] > 0 && (player.exploredLake >= 10) && (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00233] == 0 || player.effects.findByType(StatusAffects.Infested) < 0) && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00238] <= 0)
    ...
    // Sword Discovery
    if (player.effects.findByType(StatusAffects.TookBlessedSword) < 0 && player.effects.findByType(StatusAffects.BSwordBroken) < 0)
        choice[choice.length] = 10;
    // Pre-emptive chance of finding the boat
    if (player.effects.findByType(StatusAffects.BoatDiscovery) < 0)
    ...
    // Chance of dick-dragging! 10% + 10% per two foot up to 30%
        ...
        if (player.effects.findByType(StatusAffects.DungeonShutDown) >= 0 && player.effects.findByType(StatusAffects.FactoryOverload) < 0)
            girlOdds += 25;
        ...
        // OOZE!
            ...
            if (player.inte >= 25) {
                ...
                if (player.effects.findByType(StatusAffects.DungeonShutDown) >= 0 && player.effects.findByType(StatusAffects.FactoryOverload) < 0) outputText("\n\n<b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>", false);
                return;
            }
    ...
    // Find whitney or equinum
        ...
        // Find Whitney
            ...
            if (player.effects.findByType(StatusAffects.MetWhitney) >= 0) {
                ...
                if (player.effects.getValue1Of(StatusAffects.MetWhitney) > 1) {
        ...
        if (player.effects.findByType(StatusAffects.FetishOn) < 0) {
            player.effects.create(StatusAffects.FetishOn, 0, 0, 0, 0);
...
function ottahGirl(): void {
    ...
    if (flags[kFLAGS.MET_OTTERGIRL] == 1) {
        ...
        if (player.effects.findByType(StatusAffects.FactoryOverload) < 0) outputText("crystal clear, giving you a stunning view of the lakebed");
        ...
        outputText("\n\n\"<i>Fish'n'fuck,</i>\" she replies, simply.  \"<i>I fish, you fuck. Ya ain't dense, are you " + mf(player, "boy", "girl") + "?</i>\"");
...
function ottergirlLikesDongs(): void {
    ...
    outputText("\n\nYou feel slender but powerful fingers wrap around your cock, followed shortly after by a pair of lips. They encircle your " + cockHead(player, x) + " and suck, creating a delightful tingling sensation that travels down your cock and into your core.");
    ...
    outputText("\n\nLikewise, her tongue and lips dance over your " + cockDescript(game.player, x) + " like a trio of dancers. They spin, twist, hop and tease, ensuring that no inch is left untouched.");
    ...
    outputText("\n\nAfter several minutes of this, Callu relinquishes her hold on your member and says, \"<i>Mm, I reckon that'll work just fine.</i>\"  She sits up and positions herself over your " + cockDescript(game.player, x) + ".  Slowly she lowers herself, first taking your " + cockHead(player, x) + ".  Her cunt, slick and aroused as it is, offers no resistance despite its tightness.  Its walls pulse and quiver around you, as though the otter has complete control over it.  Inch by inch she sinks down further, ");
    // (dicks 10" or less)
    ...
    outputText("\n\nA sudden, strange high-pitched sound suddenly rings out and your head bolts upright, only to see Callu reeling in a fish.  She looks it over, nods once to herself and tucks it away in an ice chest cleverly buried under the sand right next to the two of you.  Afterwards she stands up, letting your dick fall out of her.  Your " + cockDescript(game.player, x) + " feels strange, and uncomfortably naked somehow, especially as a cool wind blows over its saliva and femcum-covered skin.");
